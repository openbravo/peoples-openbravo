...
public class RDBMSIndependent {
  ...
  public static Vector<String> getCallableResult(Connection conn, ConnectionProvider connectionProvider, String sql, Vector<String> parameters, Vector<String> types, int totalOutParameters) throws Exception {
    ...
    if (totalOutParameters==1) {
      ...
      if (init==-1) throw new ServletException("Badly formed sql: " + sql);
      strSql.append(sql.substring(0, init+1));
      int end = sql.lastIndexOf(")");
      if (end==-1) throw new ServletException("Badly formed sql: " + sql);
    ...
    if (conn==null) st = connectionProvider.getPreparedStatement(strSql.toString());
    else st = connectionProvider.getPreparedStatement(conn, strSql.toString());
    ResultSet result;
    ...
    try {
      ...
      else connectionProvider.releaseTransactionalStatement(st);
    }
    return(total);
  }
}
