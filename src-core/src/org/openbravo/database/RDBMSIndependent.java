...
public class RDBMSIndependent {
    ...
    public static Vector<String> getCallableResult(Connection conn,
            ConnectionProvider connectionProvider, String sql,
            Vector<String> parameters, Vector<String> types,
            int totalOutParameters) throws Exception {
        ...
        if (totalOutParameters == 1) {
            ...
            int count = 0;
            StringTokenizer stoken = new StringTokenizer(sql.substring(
                    init + 1, end), ",", false);
            while (stoken.hasMoreTokens()) {
                String token = stoken.nextToken();
                if (token.indexOf("?") != -1) {
                    if (!found
                            && types.elementAt(count).equalsIgnoreCase("out")) {
                        found = true;
                    } else {
        ...
        try {
            if (parameters != null) {
                for (int i = 0; i < parameters.size(); i++) {
                    ...
                    if (!typeAux.equalsIgnoreCase("out")) {
                        iParameter++;
                        UtilSql.setValue(st, iParameter, 12, "Test", parameters
                                .elementAt(i));
                    }
                }
            }
            if (!strSql.toString().toUpperCase().trim().startsWith("ALTER ")) {
                ...
                int pos = 0;
                if (result.next() && totalOutParameters > 0
                        && parameters != null) {
                    for (int i = 0; i < parameters.size(); i++) {
