...
public class MultipartRequest implements FieldProvider {
  ...
  public void addField(String value) {
    if (vector==null) vector = new Vector<Object>();
    vector.addElement(value);
  }

  private String setFormatSeparator(String _format) {
    if (_format.equalsIgnoreCase("F")) return("FIXED");
    else if (_format.equalsIgnoreCase("T")) return("\t");
    else if (_format.equalsIgnoreCase("S")) return(";");
    else if (_format.equalsIgnoreCase("P")) return("+");
    else if (_format.equalsIgnoreCase("C")) return(",");
    else return("");
  }
  ...
  public FieldProvider setFieldProvider(String linea) {
    if (format.equalsIgnoreCase("FIXED")) return lineFixedSize(linea);
    else if (format.equals("")) return lineComplete(linea);
    else return lineSeparatorFormated(linea);
  }
  ...
  public FieldProvider lineFixedSize(String linea) {
    if (linea == null || linea.length() < 1) return null;
    MultipartRequest data = new MultipartRequest();
    if (rows==null || rows.length==0) data.addField(linea);
    else {
      for (int i=0;i<rows.length;i++) {
        ...
        if (init>linea.length()) {
          ...
          if (init<0) init = 0;
          if (end<0 || end<init) end=init;
          else if (end>linea.length()) end = linea.length();
