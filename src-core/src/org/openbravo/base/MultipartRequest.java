...
public class MultipartRequest implements FieldProvider {
    ...
    public MultipartRequest(VariablesBase vars, String filename,
            boolean firstLineHeads, String format, FieldProvider[] data)
            throws IOException {
        init(vars, filename, firstLineHeads, format, data);
        readSubmittedFile();
    }

    public MultipartRequest(VariablesBase vars, InputStream in,
            boolean firstLineHeads, String format, FieldProvider[] data)
            throws IOException {
    ...
    public void init(VariablesBase vars, String filename,
            boolean firstLineHeads, String format, FieldProvider[] data)
            throws IOException {
        if (vars == null)
    ...
    public FieldProvider lineFixedSize(String linea) {
        ...
        if (rows == null || rows.length == 0)
            ...
            for (int i = 0; i < rows.length; i++) {
                int init = Integer.valueOf(rows[i].getField("startno"))
                        .intValue();
