...
public class XmlVectorValue extends Vector<Object> {
    ...
    public XmlVectorValue(XmlVectorTemplate xmlVectorTemplate,
            XmlDocument xmlDocument) {
        if (xmlDocument != null)
            this.xmlDocument = xmlDocument;
        for (Enumeration<Object> e = xmlVectorTemplate.elements(); e
                .hasMoreElements();) {
            XmlComponentTemplate xmlComponentTemplate = (XmlComponentTemplate) e
                    .nextElement();
            log4jXmlVectorValue.debug("Adding XmlComponentTemplate");
            addElement(xmlComponentTemplate
    ...
    StringBuffer printStringBuffer() {
        ...
        if (handler != null && handler.getTabId() != null
                && !handler.getTabId().equals("")) {
        ...
        for (Enumeration<Object> e = elements(); e.hasMoreElements();) {
            ...
            if (textMap != null) {
                if (xmlComponentValue.print() != null
                    ...
                    log4jXmlVectorValue
                            .debug("printStringBuffer(HashMap<String, String> textMap) - result: "
                                    + result);
                    log4jXmlVectorValue
                            .debug("checking for existence of text in textdata: "
                                    + result);
                    String translation = textMap.get(result);
                    if (translation != null && !translation.equals("")) {
                        log4jXmlVectorValue
                                .debug("printStringBuffer() appending xmlComponentValue: "
                                        + xmlComponentValue.print()
            ...
            if (result != null && !result.equals("")) {
                if (result.contains("alt=") || result.contains("title=")) {
                    ...
                    if (result.contains("alt=\"")) {
                        log4jXmlVectorValue.debug("alt found: "
                                + result.substring(result.indexOf(prefix)));
                        if (tabLabels != null && !tabLabels.isEmpty()) {
                            int index = 0;
                            while (index < result.length()
                                    && result.indexOf(prefix, index) > 0) {
                                ...
                                if (trl != null && !trl.equals("")) {
                                    result = result.replace(prefix + text
                                            + suffix, prefix + trl + suffix);
                                }
                                index = start + index + 1;
                            }
                        } else if (textMap != null) {
                            result = replaceString(result, prefix, suffix,
                                    textMap);
                        }
                    }
                    prefix = "title=\"";
                    if (result.contains("title=\"")) {
                        log4jXmlVectorValue.debug("title found: "
                                + result.substring(result.indexOf(prefix)));
                        if (tabLabels != null && !tabLabels.isEmpty()) {
                            int index = 0;
                            while (index < result.length()
    ...
    private String replaceString(String originalLine, String prefix,
            String suffix, HashMap<String, String> textMap) {
        ...
        if (trlText != null && !trlText.equals("")) {
            log4jXmlVectorValue.debug("Found a match - ORIGINAL: " + prefixStr
                    + text + suffixStr + ", TRANSLATION: " + prefixStr
                    + trlText + suffixStr);
            inputLine = inputLine.replace(prefixStr + text + suffixStr,
                    prefixStr + trlText + suffixStr);
        }
