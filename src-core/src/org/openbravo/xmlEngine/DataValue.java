...
class DataValue implements XmlComponentValue {
    ...
    public void initialize() {
        ...
        for (Enumeration<Object> e1 = dataTemplate.vecFieldTemplate.elements(); e1
                .hasMoreElements();) {
        ...
        for (Enumeration<Object> e1 = dataTemplate.vecFunctionTemplateData
        ...
        for (Enumeration<Object> e1 = dataTemplate.vecFunctionTemplateOutSection
                .elements(); e1.hasMoreElements();) {
            FunctionTemplate functionTemplate = (FunctionTemplate) e1
                    .nextElement();
            FunctionValue functionValue = functionTemplate
            ...
            log4jDataValue.debug("Function (OutSection): "
                    + functionValue.functionTemplate.fieldName);
        }
        ...
        for (Enumeration<Object> e1 = dataTemplate.vecSectionTemplate
                .elements(); e1.hasMoreElements();) {
            SectionTemplate sectionTemplate = (SectionTemplate) e1
                    .nextElement();
            SectionValue sectionValue = sectionTemplate.createSectionValue(
                    xmlDocument, this);
            vecSectionValue.addElement(sectionValue);
            if (sectionTemplate.name
                    .equals(dataTemplate.firstSectionTemplate.name)) {
                firstSectionValue = sectionValue;
                log4jDataValue.debug("First Section: "
                        + sectionValue.sectionTemplate.name);
            }
            log4jDataValue.debug("Section: "
                    + sectionValue.sectionTemplate.name);
        }

        log4jDataValue.debug("vector Detail");
        vecDetailValue = new XmlVectorValue(dataTemplate.vecDetailTemplate,
                xmlDocument);

        vecParameterValue = new Vector<Object>();
        for (Enumeration<Object> e1 = dataTemplate.vecParameterTemplate
                .elements(); e1.hasMoreElements();) {
            ParameterTemplate parameterTemplate = (ParameterTemplate) e1
                    .nextElement();
            ParameterValue parameterValue = parameterTemplate
                    .createParameterValue(xmlDocument);
            vecParameterValue.addElement(parameterValue);
            log4jDataValue.debug("Parameter: "
        ...
        if (firstSectionValue == null) {
            ...
            if (data == null && dataArray == null) {
                if (preparedStatement == null) {
                    ErrorManagement.error(101, xmlDocument.xmlTemplate.strName
                            + "." + dataTemplate.strName);
                    return "";
                }
                query();
                return execute();
    ...
    public String executeArray() {
        ...
        if (log4jDataValue.isDebugEnabled())
            log4jDataValue.debug("StringBuffer length:"
                    + firstSectionValue.strSection.length());
        ...
        try {
            ...
            if (dataTemplate.strSQL != null) {
                preparedStatement = connection
                        .prepareStatement(dataTemplate.strSQL);
                if (log4jDataValue.isDebugEnabled())
    ...
    public void query() {
        try {
            ...
            // it is a query (SQL)
            for (Enumeration<Object> e = vecParameterValue.elements(); e
                    .hasMoreElements();) {
                ParameterValue parameter = (ParameterValue) e.nextElement();
                if (parameter.parameterTemplate.type == java.sql.Types.INTEGER) {
                    if (log4jDataValue.isDebugEnabled())
                        log4jDataValue.debug("setInt: " + i + " valor: "
                                + Integer.parseInt(parameter.strValue));
                    preparedStatement.setInt(i, Integer
                            .parseInt(parameter.strValue));
                    ...
                    if (parameter.xmlComponentValue != null) {
                        ...
                        if (parameter.parameterTemplate.section != null) {
                            strValue = parameter.xmlComponentValue
                                    .printPrevious(); // detailed in section
                                                      // print printPrevious
                        } else {
            ...
            log4jDataValue.error("SQL error in query: " + dataTemplate.strSQL
                    + "Exception:" + e);
        }
    }

    private void readFields() {
        for (Enumeration<Object> e = vecFieldValue.elements(); e
                .hasMoreElements();) {
    ...
    private void readFieldsArray(FieldProvider fieldProvider) {
        for (Enumeration<Object> e = vecFieldValue.elements(); e
                .hasMoreElements();) {
            FieldValue fieldValue = (FieldValue) e.nextElement();
            fieldValue.read(fieldProvider.getField(fieldValue.fieldTemplate
                    .name()));
        }
    }
    ...
    private void init() {
        ...
        for (Enumeration<Object> e = vecSectionValue.elements(); e
                .hasMoreElements();) {
        ...
        for (Enumeration<Object> e = vecFunctionValueOutSection.elements(); e
                .hasMoreElements();) {
    ...
    public void printDetail() {
        for (Enumeration<Object> e = vecDetailValue.elements(); e
                .hasMoreElements();) {
            XmlComponentValue xmlComponentValue = (XmlComponentValue) e
                    .nextElement();
