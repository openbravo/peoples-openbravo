...
public class XmlEngine extends HttpServlet {
  ...
  public void loadParams() {
    ...
    log4jXmlEngine.info("InternationalCurrencySymbol: " + dfs.getInternationalCurrencySymbol());
    ...
    dfsSpanish.setGroupingSeparator('.');
    // various formats, pending of create a configuration file
    FormatCouple fc;
    fc= new FormatCouple(new DecimalFormat("#,##0.00",dfs), new DecimalFormat("#0.00",dfs));
    formatHashtable.put("euroInform", fc);
    fc = new FormatCouple(new DecimalFormat("#0.00",dfs), new DecimalFormat("#0.00",dfs));
    formatHashtable.put("euroEdition", fc);
    fc = new FormatCouple(new DecimalFormat("#,##0",dfs), new DecimalFormat("#0",dfs));
    formatHashtable.put("integerInform", fc);
    fc = new FormatCouple(new DecimalFormat("#0",dfs), new DecimalFormat("#0",dfs));
    formatHashtable.put("integerEdition", fc);
    fc = new FormatCouple(new DecimalFormat("#,##0.##",dfs),new DecimalFormat("#0.00",dfs));
    ...
    */
    ...
    htmlReplaceVector.addElement(new ReplaceElement("&","&amp;")); // this must be the first for not replace the next &
  ...
  public void initialize() {
    hasXmlTemplate = new Hashtable<String, XmlTemplate>();  // vector of XmlTemplates
  ...
  /**
    ...
    @param strXmlTemplateFile A configuration file of the XmlTemplate in XML format
    */
  ...
  /**
    ...
    file. The elements with a id equal to a discard are not readed
    */
  ...
  /**
    ...
    that there are in the Stack. The XmlTemplates are added to the Stack in the addXmlTemplate function
    or in the readFile function
    @param strXmlTemplateName The name that identifie the XmlTemplate
    @param strXmlTemplateFile The configuration file of the XmlTemplate
    @param discard A vector of Strings with the names of the discards in the template
    */
  public XmlTemplate readAllXmlTemplates(String strXmlTemplateName, String strXmlTemplateFile, String[] discard) {
  ...
  /**
    this function add the XmlTemplate to the list of XmlTemplates or return an
  ...
  /**
    ...
    @param xmlTemplate The XmlTemplate object
    */
  public void readFile(XmlTemplate xmlTemplate) {
    ...
    xmlTemplate.clear();
    try {
      //if (!isResource) xmlParser.parse(new InputSource(new FileReader(fileXmlEngineConfiguration)));
      if (!isResource) xmlParser.parse(new InputSource(new InputStreamReader(new FileInputStream(fileXmlEngineConfiguration), "UTF-8")));
      else xmlParser.parse(new InputSource(ClassLoader.getSystemResourceAsStream(strFile)));
    } catch (FileNotFoundException e) {
      if (!isResource) log4jXmlEngine.error("not found fileXmlEngineConfiguration: " + fileXmlEngineConfiguration + "\n" +
          e.getMessage());
      else log4jXmlEngine.error("not found fileXmlEngineConfiguration: " + strFile + "\n" +
          e.getMessage());
      return;
    } catch (IOException e) {
      if (!isResource) log4jXmlEngine.error("IOException in fileXmlEngineConfiguration: " + fileXmlEngineConfiguration);
      else log4jXmlEngine.error("IOException in fileXmlEngineConfiguration: " + strFile);
      e.printStackTrace();
      return;
    } catch (Exception e) {
      if (!isResource) log4jXmlEngine.error("Exception in fileXmlEngineConfiguration: " + fileXmlEngineConfiguration);
      else log4jXmlEngine.error("Exception in fileXmlEngineConfiguration: " + strFile);
      e.printStackTrace();
      return;
    }
    ...
    try {
      //if (!isResource) templateParser.parse(new InputSource(new FileReader(fileXmlEngineTemplate)));
      if (!isResource) templateParser.parse(new InputSource(new InputStreamReader(new FileInputStream(fileXmlEngineTemplate), "UTF-8")));
      else templateParser.parse(new InputSource(ClassLoader.getSystemResourceAsStream(strPath + xmlTemplate.configuration.strTemplate)));
    } catch (FileNotFoundException e) {
      if (!isResource) log4jXmlEngine.error("not found fileXmlEngineTemplate: " + fileXmlEngineTemplate + "\n" +
          e.getMessage());
      else log4jXmlEngine.error("not found fileXmlEngineTemplate: " + strPath + xmlTemplate.configuration.strTemplate + "\n" +
          e.getMessage());
      return;
    } catch (IOException e) {
      if (!isResource) log4jXmlEngine.error("IOException in fileXmlEngineTemplate: " + fileXmlEngineTemplate);
      else log4jXmlEngine.error("IOException in fileXmlEngineTemplate: " + strPath + xmlTemplate.configuration.strTemplate);
      e.printStackTrace();
      return;
    } catch (Exception e) {
      if (!isResource) log4jXmlEngine.error("Exception in fileXmlEngineTemplate: " + fileXmlEngineTemplate);
      else log4jXmlEngine.error("Exception in fileXmlEngineTemplate: " + strPath + xmlTemplate.configuration.strTemplate);
      e.printStackTrace();
      return;
    }
  }
  ...
  @SuppressWarnings("unchecked") //It's not possible to cast from xmlTemplate to Report
public void connect(){
