...
public class XmlEngine extends HttpServlet {
    ...
    public void init(ServletConfig config) throws ServletException {
        ...
        if (log4jXmlEngine.isDebugEnabled())
            log4jXmlEngine.debug("Replace attribute value: \"" + strReplaceWhat
                    + "\" with: \"" + strReplaceWith + "\".");
    ...
    public void loadParams() {
        /*
         * // decimal separator:. thousands separator: , DecimalFormatSymbols
         * dfs = new DecimalFormatSymbols();
         * log4jXmlEngine.info("InternationalCurrencySymbol: " +
         * dfs.getInternationalCurrencySymbol());
         * log4jXmlEngine.info("CurrencySymbol: " + dfs.getCurrencySymbol());
         ...
         * dfsSpanish.setGroupingSeparator('.'); // various formats, pending of
         * create a configuration file FormatCouple fc; fc= new FormatCouple(new
         * DecimalFormat("#,##0.00",dfs), new DecimalFormat("#0.00",dfs));
         * formatHashtable.put("euroInform", fc); fc = new FormatCouple(new
         * DecimalFormat("#0.00",dfs), new DecimalFormat("#0.00",dfs));
         * formatHashtable.put("euroEdition", fc); fc = new FormatCouple(new
         * DecimalFormat("#,##0",dfs), new DecimalFormat("#0",dfs));
         * formatHashtable.put("integerInform", fc); fc = new FormatCouple(new
         * DecimalFormat("#0",dfs), new DecimalFormat("#0",dfs));
         * formatHashtable.put("integerEdition", fc); fc = new FormatCouple(new
         * DecimalFormat("#,##0.##",dfs),new DecimalFormat("#0.00",dfs));
        ...
        htmlPreformatedReplaceVector
                .addElement(new ReplaceElement("®", "&reg;"));
        ...
        htmlPreformatedTextareaReplaceVector.addElement(new ReplaceElement("®",
                "&reg;"));
        replaceHashtable.put("htmlPreformatedTextarea",
                htmlPreformatedTextareaReplaceVector);
    ...
    public void initialize() {
        ...
        if (log4jXmlEngine.isDebugEnabled())
            log4jXmlEngine.debug("fileXmlEngineFormat: "
                    + fileXmlEngineFormat.toString());
        try {
            // xmlParserFormat.parse(new InputSource(new
            // FileReader(fileXmlEngineFormat)));
            xmlParserFormat.parse(new InputSource(new InputStreamReader(
                    new FileInputStream(fileXmlEngineFormat), "UTF-8")));
        } catch (FileNotFoundException e) {
            log4jXmlEngine.error("not found fileXmlEngineFormat: "
                    + fileXmlEngineFormat + "\n" + e.getMessage());
            return;
        } catch (IOException e) {
            log4jXmlEngine.error("IOException in fileXmlEngineFormat: "
                    + fileXmlEngineFormat);
            e.printStackTrace();
            return;
        } catch (Exception e) {
            log4jXmlEngine.error("Exception in fileXmlEngineFormat: "
                    + fileXmlEngineFormat);
            e.printStackTrace();
            return;
        }
    ...
    /**
     * this function reads a file that defines a XmlTemplate with a vector of
     * discard
     * 
     * @param strXmlTemplateFile
     *            A configuration file of the XmlTemplate in XML format
     * @param discard
     *            A vector of Strings with the names of the discards in the
     *            template file. The elements with a id equal to a discard are
     *            not readed
     */
    public synchronized XmlTemplate readXmlTemplate(String strXmlTemplateFile,
            String[] discard) {
    ...
    /**
     * this function add the XmlTemplate to the list of XmlTemplates and read
     * all the XmlTemplates that there are in the Stack. The XmlTemplates are
     * added to the Stack in the addXmlTemplate function or in the readFile
     * function
     * 
     ...
     *            template
     */
    public XmlTemplate readAllXmlTemplates(String strXmlTemplateName,
            String strXmlTemplateFile, String[] discard) {
        XmlTemplate xmlTemplate = addXmlTemplate(strXmlTemplateName,
                strXmlTemplateFile, discard);
        while (!stcRead.empty()) {
    ...
    /**
     * this function add the XmlTemplate to the list of XmlTemplates or return
     * an existing XmlTemplate if it was found in the list
     * 
     * @param strXmlTemplateName
     *            The name that identifie the XmlTemplate
     * @param strXmlTemplateFile
     ...
     *            template
     */
    private XmlTemplate addXmlTemplate(String strXmlTemplateName,
            String strXmlTemplateFile, String[] discard) {
        if (log4jXmlEngine.isDebugEnabled())
        ...
        xmlTemplate = new XmlTemplate(strXmlTemplateName, strXmlTemplateFile,
                discard, this);
    ...
    public void readFile(XmlTemplate xmlTemplate) {
        ...
        if (!isResource) {
            ...
            if (log4jXmlEngine.isDebugEnabled()) {
                log4jXmlEngine.debug("fileXmlEngineConfiguration: "
                        + fileXmlEngineConfiguration.toString());
        ...
        try {
            ...
            if (!isResource)
                xmlParser.parse(new InputSource(new InputStreamReader(
                        new FileInputStream(fileXmlEngineConfiguration),
                        "UTF-8")));
            else
                xmlParser.parse(new InputSource(ClassLoader
                        .getSystemResourceAsStream(strFile)));
        } catch (FileNotFoundException e) {
            if (!isResource)
                log4jXmlEngine.error("not found fileXmlEngineConfiguration: "
            ...
            if (!isResource)
                log4jXmlEngine
                        .error("IOException in fileXmlEngineConfiguration: "
                                + fileXmlEngineConfiguration);
            else
                log4jXmlEngine
                        .error("IOException in fileXmlEngineConfiguration: "
            ...
            if (!isResource)
                log4jXmlEngine
                        .error("Exception in fileXmlEngineConfiguration: "
                                + fileXmlEngineConfiguration);
            else
                log4jXmlEngine
                        .error("Exception in fileXmlEngineConfiguration: "
        ...
        int posExtension = xmlTemplate.configuration.strTemplate
                .lastIndexOf(".");
        XMLReader templateParser;
        if (xmlTemplate.configuration.strTemplate.substring(posExtension)
                .equals(".html")) {
            if (log4jXmlEngine.isDebugEnabled())
                log4jXmlEngine.debug("Html file: -"
                        + xmlTemplate.configuration.strTemplate
                                .substring(posExtension) + "-");
        ...
        if (log4jXmlEngine.isDebugEnabled())
            log4jXmlEngine.debug("XmlEngine file template: "
                    + xmlTemplate.configuration.strTemplate);
        File fileXmlEngineTemplate = null;
        String strPath = "";
        if (!isResource) {
            fileXmlEngineTemplate = new File(fileXmlEngineConfiguration
                    .getParent(), xmlTemplate.configuration.strTemplate);
            if (log4jXmlEngine.isDebugEnabled())
                log4jXmlEngine.debug("fileXmlEngineTemplate: "
                        + fileXmlEngineTemplate.toString());
        } else {
            int finPath = -1;
        ...
        try {
            ...
            if (!isResource)
                templateParser.parse(new InputSource(new InputStreamReader(
                        new FileInputStream(fileXmlEngineTemplate), "UTF-8")));
            else
                templateParser.parse(new InputSource(ClassLoader
                        .getSystemResourceAsStream(strPath
                                + xmlTemplate.configuration.strTemplate)));
        } catch (FileNotFoundException e) {
            if (!isResource)
                log4jXmlEngine.error("not found fileXmlEngineTemplate: "
                        + fileXmlEngineTemplate + "\n" + e.getMessage());
            else
                log4jXmlEngine.error("not found fileXmlEngineTemplate: "
                        + strPath + xmlTemplate.configuration.strTemplate
                        + "\n" + e.getMessage());
            return;
        } catch (IOException e) {
            if (!isResource)
                log4jXmlEngine.error("IOException in fileXmlEngineTemplate: "
                        + fileXmlEngineTemplate);
            else
                log4jXmlEngine.error("IOException in fileXmlEngineTemplate: "
                        + strPath + xmlTemplate.configuration.strTemplate);
    ...
    public void doGet(HttpServletRequest request, HttpServletResponse response)
        ...
        // SQL connect();
        for (DataValue elementDataValue : report.xmlDocument.hasDataValue
                .values()) {
            elementDataValue.connect(); // SQL
            for (Enumeration<Object> e2 = elementDataValue.vecParameterValue
                    .elements(); e2.hasMoreElements();) {
                ParameterValue parameter = (ParameterValue) e2.nextElement();
                parameter.strValue = request
                        .getParameter(parameter.parameterTemplate.strName);
                ...
                if (log4jXmlEngine.isDebugEnabled())
                    log4jXmlEngine.debug("getParameter: "
                            + parameter.parameterTemplate.strName + " valor: "
                            + parameter.strValue);
            }
        }

        // Connection of the subreports
        for (XmlDocument subXmlDocument : report.xmlDocument.hasSubXmlDocuments
                .values()) {
            for (DataValue elementDataValue : subXmlDocument.hasDataValue
                    .values()) {
                elementDataValue.connect(); // SQL
            }
        }
        ...
        for (ParameterValue parameter : report.xmlDocument.hasParameterValue
                .values()) {
            parameter.strValue = request
                    .getParameter(parameter.parameterTemplate.strName);
            if (parameter.strValue == null) {
                log4jXmlEngine.debug("getParameter of: "
                        + parameter.parameterTemplate.strName
                        + " default assigned");
                parameter.strValue = parameter.parameterTemplate.strDefault;
            }
            log4jXmlEngine.debug("getParameter: "
                    + parameter.parameterTemplate.strName + " value: "
                    + parameter.strValue);
        }
        ...
        for (LabelValue label : report.xmlDocument.hasLabelValue.values()) {
            ...
            if (label.strValue == null) {
                log4jXmlEngine.debug("getLabel of: "
                        + label.labelTemplate.strName + " default assigned");
                label.strValue = label.labelTemplate.strName;
            }
            log4jXmlEngine.debug("getLabel: " + label.labelTemplate.strName
                    + " value: " + label.strValue);
        }
    ...
    public void connect() {
        for (Enumeration e1 = hasXmlTemplate.elements(); e1.hasMoreElements();) {
            ...
            for (Enumeration<DataValue> e2 = report.xmlDocument.hasDataValue
                    .elements(); e2.hasMoreElements();) {
    ...
    public static void main(String argv[]) {
        ...
        for (DataValue elementDataValue : report.xmlDocument.hasDataValue
            ...
            for (Enumeration<Object> e2 = elementDataValue.vecParameterValue
                ...
                if (parameter.strValue == null) {
                    if (log4jXmlEngine.isDebugEnabled())
                        log4jXmlEngine
                                .debug("Parameter(main): default assigned");
                    parameter.strValue = parameter.parameterTemplate.strDefault;
                }
                if (log4jXmlEngine.isDebugEnabled())
                    log4jXmlEngine.debug("Parameter(main): "
                            + parameter.parameterTemplate.strName + " valor: "
                            + parameter.strValue);
            }
            for (Enumeration<Object> e3 = elementDataValue.vecLabelValue
                    .elements(); e3.hasMoreElements();) {
                ...
                if (log4jXmlEngine.isDebugEnabled())
                    log4jXmlEngine.debug("Label(main): "
                            + labelValue.labelTemplate.strName + " valor: "
                            + labelValue.strValue);
            }
        }
        ...
        for (String id : report.xmlDocument.xmlTemplate.configuration.hashtable.vecKeys) {
            IDComponent iDComponent = (IDComponent) report.xmlDocument.xmlTemplate.configuration.hashtable
                    .get(id);
            if (log4jXmlEngine.isDebugEnabled())
                log4jXmlEngine.debug("id: " + id + " type: "
                        + iDComponent.type());
        }
        if (log4jXmlEngine.isDebugEnabled()) {
            log4jXmlEngine.debug("Template: "
                    + report.xmlDocument.xmlTemplate.configuration.strTemplate);
