...
class TemplateConfiguration extends DefaultHandler {
    ...
    static Logger log4jTemplateConfiguration = Logger
            .getLogger(TemplateConfiguration.class);
    StringBuffer buffer;

    public TemplateConfiguration(Vector<Object> hasDataTemplate,
            XmlEngine xmlEngine,
            Hashtable<String, ParameterTemplate> hasParameter,
    ...
    public void processingInstruction(String name, String remainder) {
        if (log4jTemplateConfiguration.isDebugEnabled())
            log4jTemplateConfiguration
                    .debug("TemplateConfiguration: processingInstruction "
                            + name);
    }
    ...
    public void startElement(java.lang.String uri, java.lang.String name,
        ...
        if (log4jTemplateConfiguration.isDebugEnabled())
            log4jTemplateConfiguration
                    .debug("TemplateConfiguration: startElement is called: "
                            + name);
        readBuffer();
        ...
        if (name.trim().equalsIgnoreCase("FIELD")) {
            ...
            for (int i = 0; i < amap.getLength(); i++) {
                if (amap.getQName(i).equals("id")) {
                    ...
                    xmlComponentBooleanWithId = (XmlComponentTemplate) hashtable
                            .get(strBooleanWithId);
                } else if (amap.getQName(i).equals("format")) {
                    formatCouple = xmlEngine.formatHashtable.get(amap
                            .getValue(i));
                    if (formatCouple == null) {
                        log4jTemplateConfiguration.warn("  format "
                                + amap.getValue(i) + " not found");
                    }
                } else if (amap.getQName(i).equalsIgnoreCase(
                        "replaceCharacters")) {
                    vecReplace = xmlEngine.replaceHashtable.get(amap
                            .getValue(i));
                    if (vecReplace == null) {
                        log4jTemplateConfiguration.warn("  replaceCharacters "
                                + amap.getValue(i) + " not found");
                    }
                }
            }
            ...
            for (int i = 0; i < amap.getLength(); i++) {
                if (log4jTemplateConfiguration.isDebugEnabled())
                    log4jTemplateConfiguration.debug("  LABEL: attribute name="
            ...
            if (id != null) {
                log4jTemplateConfiguration
                        .debug("putting label template in hashtable: " + id);
                hashtable.put(id, label);
            }
            log4jTemplateConfiguration
                    .debug("hashtable size checking if Id added to hashtable: "
                            + hashtable.vecKeys.size());
            log4jTemplateConfiguration.debug("hasLabels size: "
                    + hasLabels.size());
            ...
            for (int i = 0; i < amap.getLength(); i++) {
                if (log4jTemplateConfiguration.isDebugEnabled())
                    log4jTemplateConfiguration
                            .debug("  FUNCTION: attribute name="
                                    + amap.getQName(i) + " value="
                                    + amap.getValue(i));
                if (amap.getQName(i).equals("id")) {
                    id = amap.getValue(i);
                } else if (amap.getQName(i).equals("attribute")) {
                    ...
                    xmlComponentArg1 = (XmlComponentTemplate) hashtable
                            .get(strArg1);
                } else if (amap.getQName(i).equals("arg2")) {
                    String strArg2 = amap.getValue(i);
                    xmlComponentArg2 = (XmlComponentTemplate) hashtable
                            .get(strArg2);
                }
            }
            ...
            for (int i = 0; i < amap.getLength(); i++) {
                if (log4jTemplateConfiguration.isDebugEnabled())
                    log4jTemplateConfiguration
                            .debug("  SUBREPORT: attribute name="
                                    + amap.getQName(i) + " value="
                                    + amap.getValue(i));
                if (amap.getQName(i).equals("id")) {
                    id = amap.getValue(i);
            ...
            subreport = xmlEngine.readAllXmlTemplates(xmlTemplate.strName + "&"
                    + strSubreportName, strSubreportFile, new String[0]);
            ...
            XmlComponentTemplate xmlComponent = (XmlComponentTemplate) hashtable
                    .get(withId);
            if (log4jTemplateConfiguration.isDebugEnabled())
                log4jTemplateConfiguration.debug("XmlComponent of id: "
                        + withId + " xmlComponent: " + xmlComponent);
            // Search in the parameterSQL of the structuers of the subreport
            for (Enumeration<Object> e1 = subreport.hasDataTemplate.elements(); e1
                    .hasMoreElements();) {
                DataTemplate elementDataTemplate = (DataTemplate) e1
                        .nextElement();
                for (Enumeration<Object> e2 = elementDataTemplate.vecParameterTemplate
                        .elements(); e2.hasMoreElements();) {
                    ParameterTemplate parameterTemplate = (ParameterTemplate) e2
                            .nextElement();
                    if (parameterTemplate.strName.equals(strArgumentName)) {
                        ...
                        if (log4jTemplateConfiguration.isDebugEnabled())
                            log4jTemplateConfiguration
                                    .debug("Argument: asigned to parametro_SQL: "
                                            + parameterTemplate.strName);
                    }
                }
            }
            // Search in the parameters of the subreport
            ParameterTemplate parameterTemplate = subreport.hasParameterTemplate
                    .get(strArgumentName);
            if (parameterTemplate != null) {
                ...
                if (log4jTemplateConfiguration.isDebugEnabled())
                    log4jTemplateConfiguration
                            .debug("Argument: asigned to parameter: "
                                    + parameterTemplate.strName);
            }
            ...
            SectionTemplate section = new SectionTemplate(id,
                    activeDataTemplate);
            if (strField != null) {
                section.breakFieldTemplate = activeDataTemplate
                        .addField(strField);
            }
            if (id != null) {
            ...
            for (int i = 0; i < amap.getLength(); i++) {
                if (log4jTemplateConfiguration.isDebugEnabled())
                    log4jTemplateConfiguration
                            .debug("  TemplateConfiguration (CONNECTION): attribute name="
                                    + amap.getQName(i)
                                    + " value="
                                    + amap.getValue(i));
            ...
            for (int i = 0; i < amap.getLength(); i++) {
                if (log4jTemplateConfiguration.isDebugEnabled())
                    log4jTemplateConfiguration
                            .debug("  TemplateConfiguration (STRUCTURE): attribute name="
                                    + amap.getQName(i)
                                    + " value="
                                    + amap.getValue(i));
            ...
            for (int i = 0; i < amap.getLength(); i++) {
                if (log4jTemplateConfiguration.isDebugEnabled())
                    log4jTemplateConfiguration
                            .debug("  TemplateConfiguration (TEMPLATE): attribute name="
                                    + amap.getQName(i)
                                    + " value="
                                    + amap.getValue(i));
            ...
            for (int i = 0; i < amap.getLength(); i++) {
                if (amap.getQName(i).equals("name")) {
                    ...
                    parameter.vecReplace = xmlEngine.replaceHashtable.get(amap
                            .getValue(i));
    ...
    public void endElement(java.lang.String uri, java.lang.String name,
        ...
        if (log4jTemplateConfiguration.isDebugEnabled())
            log4jTemplateConfiguration
                    .debug("TemplateConfiguration: endElement is called: "
                            + name);
        readBuffer();
    ...
    public void characters(char[] ch, int start, int length) { // throws
        ...
        if (log4jTemplateConfiguration.isDebugEnabled()) {
            log4jTemplateConfiguration
                    .debug("TemplateConfiguration: characters is called: "
    ...
    public void readBuffer() {
        ...
        if (buffer != null) {
            ...
            log4jTemplateConfiguration.debug("Configuration(" + strElement
                    + "): characters is called: " + strBuffer);
            if (strElement.trim().equalsIgnoreCase("FIELD")) {
                log4jTemplateConfiguration.debug("characters: (" + strElement
                        + ") antes de addField");
                FieldTemplate fieldTemplate = activeDataTemplate.addField(
                        strBuffer, formatCouple.formatOutput,
                        formatCouple.formatSimple, vecReplace);
                log4jTemplateConfiguration.debug("characters: (" + strElement
                        + ") despues de addField");
                if (id != null) {
                    if (strAttribute != null || strBooleanAttribute != null) {
                        ...
                        AttributeComponentTemplate attributeComponentTemplate = new AttributeComponentTemplate(
                                fieldTemplate, strAttribute, strReplace,
                                strBooleanAttribute, xmlComponentBooleanWithId);
                        hashtable.put(id, attributeComponentTemplate);
                    } else {
                        log4jTemplateConfiguration.debug("Inside id");
                ...
                log4jTemplateConfiguration.debug("characters: (" + strElement
                        + ") before addFunction");
                FunctionTemplate functionTemplate = activeDataTemplate
                        .addFunction(strFunction, strBuffer,
                                formatCouple.formatOutput,
                                formatCouple.formatSimple, xmlComponentArg1,
                                xmlComponentArg2, id);
                log4jTemplateConfiguration.debug("characters: (" + strElement
                        + ") after de addFunction");
                if (id != null) {
                    if (strAttribute != null || strBooleanAttribute != null) {
                        log4jTemplateConfiguration.debug("Inside strAttribute");
                        AttributeComponentTemplate attributeComponentTemplate = new AttributeComponentTemplate(
                                functionTemplate, strAttribute, strReplace,
                                strBooleanAttribute, xmlComponentBooleanWithId);
                        hashtable.put(id, attributeComponentTemplate);
                    } else {
                        log4jTemplateConfiguration.debug("Inside id");
