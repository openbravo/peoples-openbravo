...
class FieldValue implements XmlComponentValue {
  ...
  public String print() {
    double total=0.0;
    if (fieldValue == null || fieldValue.equals("") || fieldValue.equalsIgnoreCase("NULL")) return ""; // if the string is empty then Double.parseDouble cannot be done 
    else
      if (fieldTemplate.formatOutput != null) {
        try {
          total = Double.parseDouble(fieldValue);
        } catch (Exception e) {
          log4jFieldValue.error("FieldValue.print() - Could not parse to double the string: " + fieldValue + "\n" + e);
        }
  ...
  public String printSimple() {
    if (fieldValue == null || fieldValue.equals("")) return ""; // if the string is empty then Double.parseDouble cannot be done
    else
      if (fieldTemplate.formatSimple != null) {
  ...
  public String printPrevious() {
    if (previousFieldValue == null || previousFieldValue.equals("")) return "";
    else
      if (fieldTemplate.formatOutput != null) {
