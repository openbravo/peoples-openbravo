...
class FunctionTemplate implements XmlComponentTemplate, IDComponent {
  ...
  public FunctionValue createFunctionValue(XmlDocument xmlDocument) {return null;}

  /* CHX
     public FunctionValue searchFunction(XmlDocument xmlDocument) {
     log4jFunctionTemplate.debug("Searching : " + fieldName);
     for (Enumeration e1 = xmlDocument.hasDataValue.elements() ; e1.hasMoreElements();) {
     DataValue dataValue = (DataValue)e1.nextElement();
     for (Enumeration e2 = dataValue.vecFunctionValueData.elements() ; e2.hasMoreElements();) {
     FunctionValue functionValue = (FunctionValue)e2.nextElement();
     log4jFunctionTemplate.debug("Comparing in vecFunctionValueData: " + fieldName + " & " + functionValue.functionTemplate.fieldName);
     if (functionValue.functionTemplate.fieldName.equals(fieldName)) {
     log4jFunctionTemplate.debug("Found in vecFunctionValueData: " + fieldName);
     return functionValue;
     }
     }
     for (Enumeration e3 = dataValue.vecFunctionValueOutSection.elements() ; e3.hasMoreElements();) {
     FunctionValue functionValue = (FunctionValue)e3.nextElement();
     log4jFunctionTemplate.debug("Comparing in vecFunctionOutSection: " + fieldName + " & " + functionValue.functionTemplate.fieldName);
     if (functionValue.functionTemplate.fieldName.equals(fieldName)) {
     log4jFunctionTemplate.debug("Found in vecFunctionOutSection: " + fieldName);
     return functionValue;
     }
     }
     }
