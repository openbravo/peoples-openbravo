...
class LabelTemplate implements XmlComponentTemplate, IDComponent {
  ...
  public LabelValue createLabelValue(XmlDocument xmlDocument) {
    log4j.debug("running createLabelValue() method - hasLabelValue: " + xmlDocument.hasLabelValue.size());
    LabelValue labelValue = xmlDocument.hasLabelValue.get(strName);
    //log4j.debug("running createLabelValue() method - labelValue: " + labelValue.labelTemplate.strName);
    if (labelValue == null) labelValue = new LabelValue(this, xmlDocument);
    return labelValue;
  }
