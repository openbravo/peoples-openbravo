...
class SectionValue {
  ...
  public SectionValue(SectionTemplate sectionTemplate, XmlDocument xmlDocument, DataValue dataValue) {
    ...
    for (Enumeration<Object> e1 = sectionTemplate.vecFunctionTemplate.elements() ; e1.hasMoreElements();) {
  ...
  public void init() {
    log4jSectionValue.debug("(Init) Levels: " + sectionTemplate.dataTemplate.intTotalLevels + " actual level: " + sectionTemplate.intLevel);
    ...
    if (sectionTemplate.intLevel != sectionTemplate.dataTemplate.intTotalLevels) {
      SectionValue section = (SectionValue)dataValue.vecSectionValue.elementAt(sectionTemplate.intLevel + 1);
  ...
  public void close() {
    log4jSectionValue.debug("(Close) Levels:" + sectionTemplate.dataTemplate.intTotalLevels + " actual level: " + sectionTemplate.intLevel);
    if (sectionTemplate.intLevel != sectionTemplate.dataTemplate.intTotalLevels) {
      SectionValue section = (SectionValue)dataValue.vecSectionValue.elementAt(sectionTemplate.intLevel + 1);
      section.close();
    }
    ...
    if (sectionTemplate.intLevel != sectionTemplate.dataTemplate.intTotalLevels) {
      ...
      //      log4jSectionValue.debug("Adding strDetail length detail: " + dataValue.strDetailValue.length() + " section length: " + strSection.length());
      //      System.gc();
      strSection.append(dataValue.strDetailValue);
      //      log4jSectionValue.debug("Añadido strDetail"+ " section length: " + strSection.length());
    }
    // Foot of the section
