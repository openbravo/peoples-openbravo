...
class SectionValue {
    ...
    public SectionValue(SectionTemplate sectionTemplate,
            XmlDocument xmlDocument, DataValue dataValue) {
        this.sectionTemplate = sectionTemplate;
        this.dataValue = dataValue;

        vecHeadValue = new XmlVectorValue(sectionTemplate.vecHeadTemplate,
                xmlDocument);
        vecFootValue = new XmlVectorValue(sectionTemplate.vecFootTemplate,
                xmlDocument);

        if (sectionTemplate.breakFieldTemplate == null) {
            breakFieldValue = null;
        } else {
            breakFieldValue = sectionTemplate.breakFieldTemplate
                    .createFieldValue(xmlDocument);
        }

        // vector of Functions
        vecFunctionValue = new Vector<FunctionValue>();
        for (Enumeration<Object> e1 = sectionTemplate.vecFunctionTemplate
                .elements(); e1.hasMoreElements();) {
            FunctionTemplate functionTemplate = (FunctionTemplate) e1
                    .nextElement();
            FunctionValue functionValue = functionTemplate
                    .createFunctionValue(xmlDocument);
            vecFunctionValue.addElement(functionValue);
            log4jSectionValue.debug("Function: "
                    + functionValue.functionTemplate.fieldName);
        }
    }

    public void init() {
        log4jSectionValue.debug("(Init) Levels: "
