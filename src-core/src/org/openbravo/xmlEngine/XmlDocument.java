...
public class XmlDocument implements XmlComponentValue {
    ...
    public XmlDocument(XmlTemplate xmlTemplate, XmlDocument parentXmlDocument) {
        ...
        for (final Enumeration<Object> e1 = xmlTemplate.hasSubXmlTemplates
                .elements(); e1.hasMoreElements();) {
            final XmlTemplate subXmlTemplate = (XmlTemplate) e1.nextElement();
            log4jXmlDocument.debug("Creation of SubXmlTemplate: "
                    + subXmlTemplate.strName);
            final XmlDocument subXmlDocument = subXmlTemplate
                    .createXmlDocument(this);
        ...
        for (final ParameterTemplate parameterTemplate : xmlTemplate.hasParameterTemplate
                .values()) {
            final ParameterValue parameterValue = parameterTemplate
                    .createParameterValue(this);
            parameterValue.strValue = parameterTemplate.strDefault;
            hasParameterValue.put(parameterTemplate.strName, parameterValue);
            log4jXmlDocument.debug("Parameter: "
                    + parameterValue.parameterTemplate.strName + " valor: "
                    + parameterValue.strValue);
        }

        hasLabelValue = new Hashtable<String, LabelValue>();
        for (final LabelTemplate labelTemplate : xmlTemplate.hasLabelTemplate
                .values()) {
            log4jXmlDocument.debug("hasLabelTemplate: "
                    + xmlTemplate.hasLabelTemplate.size());
            final LabelValue labelValue = labelTemplate.createLabelValue(this);
            log4jXmlDocument
                    .debug("LabelValue created: " + labelValue.strValue);
            hasLabelValue.put(labelTemplate.strName, labelValue);
            log4jXmlDocument
                    .debug("Label: " + labelValue.labelTemplate.strName);
        }

        // vector of Data
        hasDataValue = new Hashtable<String, DataValue>();
        for (final Enumeration<Object> e1 = xmlTemplate.hasDataTemplate
                .elements(); e1.hasMoreElements();) {
            ...
            log4jXmlDocument.debug("End of Data: "
                    + dataValue.dataTemplate.strName);
        }
        ...
        for (final XmlDocument subXmlDocument : hasSubXmlDocuments.values()) {
            log4jXmlDocument.debug("parameters of subdocument: "
        ...
        /*
         * it comes from the XmlTemplate clear() for (Enumeration e =
         * hasDataTemplate.elements() ; e.hasMoreElements();) { DataTemplate
         * elementDataTemplate = (DataTemplate)e.nextElement();
         * elementDataTemplate.clear(); } // see if this loop necessary or if it
         * is here whre the DataTemplate clear is executed
         */
    ...
    private void setXmlComponentValueParameters() {
        for (final ParameterValue parameterValue : hasParameterValue.values()) {
            ...
            log4jXmlDocument.debug("setXmlComponentValue: "
                    + parameterValue.parameterTemplate.strName);
        }
    }

    private void setXmlComponentValueLabels() {
        log4jXmlDocument
                .debug("setXmlComponentValueLabels() - hasLabelValues size: "
                        + hasLabelValue.size());
        for (final LabelValue labelValue : hasLabelValue.values()) {
            labelValue.setXmlComponentValue(this);
            log4jXmlDocument
                    .debug("setXmlComponentValueLabels() - labelValue.labelTemplate.strName: "
    ...
    private void setXmlComponentValueParametersOfDataValues() {
        for (final DataValue dataValue : hasDataValue.values()) {
            for (final Enumeration<Object> e2 = dataValue.vecParameterValue
                    .elements(); e2.hasMoreElements();) {
                final ParameterValue parameter = (ParameterValue) e2
                        .nextElement();
                parameter.setXmlComponentValue(this);
                log4jXmlDocument
                        .debug("setXmlComponentValue of Parameter of DataValue: "
    ...
    public void setData(String dataName, FieldProvider[] data) {
        ...
        if (dataValue == null) {
            log4jXmlDocument.warn("Data: " + dataName + " not found in "
                    + xmlTemplate.strName);
        }
    ...
    public void setData(String subXmlDocumentName, String dataName,
            FieldProvider[] data) {
        log4jXmlDocument.debug("setData FieldProvider[] of dataName: "
                + dataName + " in subDocument " + subXmlDocumentName + " in "
                + xmlTemplate.strName);
        final XmlDocument xmlDocument = hasSubXmlDocuments
                .get(xmlTemplate.strName + "&" + subXmlDocumentName);
        if (xmlDocument == null) {
            log4jXmlDocument.warn("Subdocument: " + subXmlDocumentName
                    + " not found in " + xmlTemplate.strName);
        }
    ...
    public void setParameter(String strName, String strValue) {
        ...
        if (parameter != null
                && !parameter.parameterTemplate.strName.equals("menu")) {
            log4jXmlDocument.debug("Parameter: "
    ...
    public void setLabel(String strName, String strValue) {
        log4jXmlDocument
                .debug("###setLabel(): trying to setLabel with strName: "
                        + strName);
        if (strName != null && !strName.equals("")) {
            if (hasLabelValue.get(strName) != null) {
                ...
                if (label != null
                        && !label.labelTemplate.strName.equals("menu")) {
                    log4jXmlDocument.debug("###setLabel(): "
                            + label.labelTemplate.strName + " value: "
                            + label.print());
                }
            }
        }
    ...
    public String print(String strBlank) {
        if (!ignoreTranslation && xmlTemplate != null
                && xmlTemplate.strName != null) {
            ...
            if (hasParameterValue != null && !hasParameterValue.isEmpty()) {
                ...
                if (hasParameterValue.get("processId") != null
                        ...
                        && !hasParameterValue.get("processId").strValue
                                .equals("")) {
                    // String formType =
                    // hasParameterValue.get("trlFormType").strValue;
                    // if (formType.equalsIgnoreCase("PROCESS"))
                    log4jXmlDocument.debug("ProcessId: "
                            + hasParameterValue.get("processId").strValue);
                    handler.setXmlDocumentType(TranslationHandler.PROCESS);
                    handler.setXmlDocumentTypeId(hasParameterValue
                            .get("processId").strValue);
                }
                ...
                if (hasParameterValue.get("language") != null
                    ...
                    handler
                            .setLanguage(hasParameterValue.get("language").strValue
                ...
                log4jXmlDocument
                        .debug("print(String strBlank) - properties file not found");
            }
        } else {
            log4jXmlDocument
                    .debug("print(String strBlank) - hasParameterValue is null");
        }
