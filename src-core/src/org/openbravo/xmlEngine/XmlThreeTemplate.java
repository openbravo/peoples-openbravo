...
/**
 * class for represent the structure of the value of an attribute. It is used
 * because part of the value of an attribute can be modified with the value of
 * an XmlComponent. The value is represented by a three with nodes of three
 * branchs the fist and the third branchs are another XmlThreeTemplate, a String
 * or null the second is an XmlComponent
 */
class XmlThreeTemplate implements XmlComponentTemplate {
    ...
    static Logger log4jXmlThreeTemplate = Logger
            .getLogger(XmlThreeTemplate.class);
    ...
    public XmlThreeTemplate(String characterBegin,
            XmlComponentTemplate xmlComponentTemplate, String characterEnd) {
        stringBegin = characterBegin;
        stringEnd = characterEnd;
        xmlMiddle = xmlComponentTemplate;
        log4jXmlThreeTemplate.debug("constructor with stringBegin: "
                + stringBegin + " stringEnd: " + stringEnd);
    }
    ...
    public void replace(AttributeComponentTemplate attributeComponentTemplate) {
        if (isBeginXmlThreeTemplate) {
            ...
            if (stringBegin != null) {
                int index = stringBegin.indexOf(attributeComponentTemplate
                        .replace());
                if (index != -1) {
                    xmlBegin = new XmlThreeTemplate(stringBegin.substring(0,
                            index), attributeComponentTemplate
                            .xmlComponentTemplate(), stringBegin
                            .substring(index
                                    + attributeComponentTemplate.replace()
                                            .length()));
        ...
        if (isEndXmlThreeTemplate) {
            ...
            if (stringEnd != null) {
                int index = stringEnd.indexOf(attributeComponentTemplate
                        .replace());
                if (index != -1) {
                    xmlEnd = new XmlThreeTemplate(
                            stringEnd.substring(0, index),
                            attributeComponentTemplate.xmlComponentTemplate(),
                            stringEnd.substring(index
                                    + attributeComponentTemplate.replace()
                                            .length()));
