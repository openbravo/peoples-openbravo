...
public class XmlTemplate extends DefaultHandler implements
    ...
    public XmlTemplate(String strName, String fileConfiguration,
            String[] discard, XmlEngine xmlEngine) {
        ...
        configuration = new TemplateConfiguration(hasDataTemplate, xmlEngine,
                hasParameterTemplate, hasLabelTemplate, this); // XmlEngineNP:
                                                               // unique
                                                               // argument in
                                                               // the
                                                               // cosntructor:
    ...
    public void clear() {
        /*
         * pass to XmlDocument for (Enumeration e = hasDataTemplate.elements() ;
         * e.hasMoreElements();) { DataTemplate elementDataTemplate =
         * (DataTemplate)e.nextElement(); elementDataTemplate.clear(); } // see
         * if this loop necessary or if it is here whre the DataTemplate clear
         * is executed
         */
        activeSection = null;
    ...
    public void startPrefixMapping(java.lang.String prefix, java.lang.String uri) {
        if (log4jXmlTemplate.isDebugEnabled())
            log4jXmlTemplate
                    .debug("XmlTemplate: startPrefixMapping is called, prefix:"
                            + prefix + " uri: " + uri);
        this.prefix = prefix;
    }
    ...
    public void startElement(java.lang.String uri, java.lang.String name,
        ...
        if (log4jXmlTemplate.isDebugEnabled())
            log4jXmlTemplate.debug("XmlTemplate: startElement is called3:"
                    + name + " strElement: " + strElement);
        ...
        TagTemplate tag = new TagTemplate(qName, amap,
                xmlEngine.strReplaceWhat, xmlEngine.strReplaceWith, prefix, uri);
        prefix = null;
        ...
        for (int i = 0; i < amap.getLength(); i++) {
            if (log4jXmlTemplate.isDebugEnabled())
                log4jXmlTemplate
                        .debug("  XmlTemplate (attribute list): attribute name="
                                + amap.getQName(i)
                                + " value="
                                + amap.getValue(i));
        ...
        if (id != null || strClass != null) {
            ...
            for (String strKey : configuration.hashtable.vecKeys) {
                if (strKey.equals(id) || strKey.equals(strClass)) {
                    iDComponent = (IDComponent) configuration.hashtable.vecObjects
                            .elementAt(i);

                    if (iDComponent != null) {
                        log4jXmlTemplate.debug("id: " + id + " tipo: "
                                + iDComponent.type());
                        ...
                        if (stackElement.printEnabled()) {
                            ...
                            case IDComponent.SECTION:
                                ...
                                activeSection.dataTemplate.vecSectionTemplate
                                ...
                                if (activeSection.dataTemplate.firstSectionTemplate == null) {
                                    ...
                                    vecXmlVector
                                            .addElement(activeSection.dataTemplate);
                                }
                                if (activeSection.breakFieldTemplate == null) {
                                    log4jXmlTemplate.debug("Add to detail");
                                    activeXmlVector = activeSection
                                            .dataTemplate().vecDetailTemplate;
                                } else {
                            ...
                            case IDComponent.ATTRIBUTE:
                                ...
                                if (functionAttributeComponent != null) {
                                    if (activeSection == null) {
                                        ...
                                        activeSection
                                                .addFunction(functionAttributeComponent);
                                    }
                                }
                            ...
                            case IDComponent.FUNCTION:
                                ...
                                if (activeSection == null) {
                                    function.dataTemplate.vecFunctionTemplateOutSection
                                            .addElement(function); // XmlEngineNP
                                                                   // like the
                                                                   // last one
                                } else {
            ...
            /*
             * else { if(stackElement.printEnabled()) {
             * activeXmlVector.addElement(tag); } }
             */
        } else {
            if (stackElement.printEnabled()) {
    ...
    public void endElement(java.lang.String uri, java.lang.String name,
        ...
        do {
            if (log4jXmlTemplate.isDebugEnabled())
                log4jXmlTemplate.debug("XmlTemplate: endElement is called: "
                        + name + " strElement: " + strElement);
    ...
    public void characters(char[] ch, int start, int length) { // throws
                                                               // SAXException {
        if (log4jXmlTemplate.isDebugEnabled()) {
            log4jXmlTemplate.debug("XmlTemplate: characters is called: "
                    + new String(ch, start, length) + " element:" + strElement
                    + " function of:" + strFunctionOfElement
        ...
        if (!peekElement().skipCharacters()) {
            CharacterComponent character = new CharacterComponent(new String(
                    ch, start, length));
            activeXmlVector.addElement(character);
        }
    }
    ...
    public XmlComponentValue createXmlComponentValue(XmlDocument xmlDocument) {
        ...
        XmlDocument subXmlDocument = xmlDocument.hasSubXmlDocuments
                .get(strName);
        if (subXmlDocument == null) {
            if (log4jXmlTemplate.isDebugEnabled())
                log4jXmlTemplate.debug("new subXmlDocument : " + strName
                        + " in createXmlComponentValue of "
