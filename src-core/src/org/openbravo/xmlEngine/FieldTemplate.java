...
class FieldTemplate implements XmlComponentTemplate, IDComponent {
  ...
  public FieldTemplate(String fieldName, DecimalFormat formatOutput, DecimalFormat formatSimple,
      Vector<ReplaceElement> vecReplace) { //, StructureSQL structureSQL) {
  ...
  /* CHX
     ...
     for (Enumeration e2 = dataValue.vecFieldValue.elements() ; e2.hasMoreElements();) {
     ...
     if (fieldValue.fieldTemplate.name().equals(fieldName)) {
     return fieldValue;
     }
     }
     }
     log4jFieldTemplate.info("New FieldValue: " + fieldName);
     FieldValue fieldValue = new FieldValue(this, xmlDocument);
     return fieldValue;
     }
     */
  //CHX
