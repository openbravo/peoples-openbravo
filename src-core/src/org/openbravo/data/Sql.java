...
class Sql {
    ...
    public Parameter addParameter(boolean sequence, String strName,
            String strDefault, String strInOut, String strOptional,
            String strAfter, String strText) {
        if (log4j.isDebugEnabled())
            log4j.debug("addParameter sequence: " + sequence + " name: "
                    + strName);
        if (strOptional != null)
            boolOptional = true;
        if (log4j.isDebugEnabled())
            log4j.debug("previous new Parameter");
        Parameter parameterNew = new Parameter(sequence, strName, strDefault,
                strInOut, strOptional, strAfter, strText);
        if (log4j.isDebugEnabled())
            log4j.debug("called new Parameter");
        for (Enumeration<Parameter> e = vecParameter.elements(); e
                .hasMoreElements();) {
