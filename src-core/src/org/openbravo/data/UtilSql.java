...
public class UtilSql {
    ...
    try
    {
      ...
      for(int i=1;i<=numColumns;++i) {
        ...
        switch (rmeta.getColumnType(i)) {
          case 2:   f.set(sqlObject, Long.toString(result.getLong(strNombreColumna)));
                    break;
          case 12:  f.set(sqlObject, result.getString(strNombreColumna));
                    break;
          case 93:  f.set(sqlObject, result.getDate(strNombreColumna));
                    if(result.wasNull())
  ...
  public String TransformaNombreColumna(String strColumn){
    ...
    for (int i=0; i<intNumCaracteres; i++){
      if (i==0) strNombreTransformado = new Character(Character.toUpperCase(strColumn.charAt(i))).toString();
      else{
        if (new Character(strColumn.charAt(i)).compareTo(BarraBaja) == 0) blnFueBarraBaja = true;
        else{
          if (blnFueBarraBaja){
            strNombreTransformado = strNombreTransformado + new Character(Character.toUpperCase(strColumn.charAt(i))).toString();
            blnFueBarraBaja = false;
          }
          else strNombreTransformado = strNombreTransformado + new Character(Character.toLowerCase(strColumn.charAt(i))).toString();
        }
      }
    }
  ...
  public boolean AsignarValoresEntrada(PreparedStatement ps, int posicion, int tipo, String strValor){
    try{
      if(strValor!=null)
      {
        ...
        {
          switch (tipo) {
            case 2:   ps.setLong(posicion, Long.valueOf(strValor).longValue());
                      break;
            case 12:  ps.setString(posicion,strValor);
                      break;
          }
        ...
        else
        {
          switch (tipo) {
            ...
            case java.sql.Types.LONGVARCHAR:
                      ...
                      break;
            case 0: ps.setDouble(posicion, Double.valueOf(strValor).doubleValue());
                    break;
          }
  ...
  public static String getValue(ResultSet result, String strField) throws java.sql.SQLException{
    ...
    if (result.wasNull()) strValueReturn = "";
  ...
  public static String getDateValue(ResultSet result, String strField, String strDateFormat) throws java.sql.SQLException{
  ...
  public static String getStringCallableStatement(CallableStatement cs, int intField) throws java.sql.SQLException{
    String strValueReturn = cs.getString(intField);
    if (strValueReturn == null) strValueReturn = "";
    return strValueReturn;
  }
}
