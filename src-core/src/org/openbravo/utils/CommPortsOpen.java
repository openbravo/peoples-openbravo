...
public class CommPortsOpen {
  ...
  public static void main(String[] args) throws Exception, IOException, NoSuchPortException, PortInUseException, UnsupportedCommOperationException {
    if (args.length != 1) {
  ...
  public CommPortsOpen() throws Exception, IOException, NoSuchPortException, PortInUseException, UnsupportedCommOperationException {
    ...
    switch (thePortID.getPortType()) {
      case CommPortIdentifier.PORT_SERIAL:
        thePort = thePortID.open("Openbravo CommData", TIMEOUTSECONDS * 1000);
        SerialPort myPort = (SerialPort) thePort;
        // set up the serial port
        myPort.setSerialPortParams(BAUD, SerialPort.DATABITS_8, SerialPort.STOPBITS_1, SerialPort.PARITY_NONE);
        break;
      case CommPortIdentifier.PORT_PARALLEL:
        thePort = thePortID.open("Openbravo Printing", TIMEOUTSECONDS * 1000);
  ...
  private void choosePort(int portType) throws Exception {
    ...
    if (portType==0) portType = CommPortIdentifier.PORT_PARALLEL;
    while (listaPort.hasMoreElements()) {
      thePortID = (CommPortIdentifier) listaPort.nextElement(); 
      if (thePortID.getPortType() == portType) { 
        if (thePortID.isCurrentlyOwned()) throw new Exception("The port is use now by: " + thePortID.getCurrentOwner()); 
        return;
      }
    }
    thePortID=null;
  ...
  protected void transmit_data(String inputFileName) throws IOException {
    ...
    while ((line = file.readLine()) != null) sb.append(line).append("\r\n");
