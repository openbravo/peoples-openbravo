...
public class CryptoUtility {
    ...
    private static void initCipher() {
        try {
            s_key = new SecretKeySpec(new byte[] { 100, 25, 28, -122, -26, 94,
                    -3, -72 }, "DES");
    ...
    public static String encrypt(String value) throws ServletException {
        ...
        if (s_cipher == null)
            throw new ServletException(
                    "CryptoUtility.encrypt() - Can't load cipher");
        String result = "";
        try {
            s_cipher.init(Cipher.ENCRYPT_MODE, s_key);
            encString = s_cipher.doFinal(clearText.getBytes());
            result = new String(org.apache.commons.codec.binary.Base64
                    .encodeBase64(encString), "UTF-8");
        } catch (Exception ex) {
            ex.printStackTrace();
            throw new ServletException(
                    "CryptoUtility.encrypt() - Can't init cipher");
        }
        return result;
    }
    ...
    public static String decrypt(String value) throws ServletException {
        ...
        if (s_cipher == null || value == null || value.length() <= 0)
            throw new ServletException(
                    "CryptoUtility.decrypt() - Can't load cipher");
        byte out[];
        byte decode[];
        try {
            ...
            ex.printStackTrace();
            throw new ServletException(
                    "CryptoUtility.decrypt() - Can't init cipher");
        }
