...
public class FileUtility {
    ...
    public FileUtility(String path, String name, boolean newFile)
            throws IOException {
    ...
    public static int copy(File source, File destiny, DirFilter dirFilter,
            boolean discardHidden, boolean overwrite) throws Exception {
        File[] list;
        int total = 0;
        if (dirFilter != null)
        ...
        for (int i = 0; i < list.length; i++) {
            ...
            if (fileItem.isDirectory()) {
                if (!discardHidden || !fileItem.isHidden()/*
                    ...
                    total += copy(fileItem, faux, dirFilter, discardHidden,
                            overwrite);
                }
            } else {
                File fileAux = new File(destiny, fileItem.getName());
                if (overwrite || !fileAux.exists()
                        || fileItem.lastModified() > fileAux.lastModified()) {
    ...
    public static void copyFile(File in, File out) throws Exception {
        ...
        try {
            ...
            long size = src.size();
            MappedByteBuffer buf = src.map(FileChannel.MapMode.READ_ONLY, 0,
                    size);
            cp.write(buf);
        } finally {
            if (src != null)
     ...
     * //FileInputStream fis = new FileInputStream(in); BufferedReader
     * fileBuffer = new BufferedReader(new FileReader(in)); FileOutputStream fos
     * = new FileOutputStream(out); OutputStreamWriter printWriterData = new
     * OutputStreamWriter(fos, "UTF-8"); String nextLine =
     * fileBuffer.readLine(); while (nextLine != null) {
     * printWriterData.write(nextLine); printWriterData.write("\n"); nextLine =
     * fileBuffer.readLine(); } printWriterData.flush(); fos.close();
     * fileBuffer.close(); /while((i=fis.read(buf))!=-1) { fos.write(buf, 0, i);
     * } fis.close(); fos.close(); }
     */
