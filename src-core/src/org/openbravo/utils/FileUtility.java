...
public class FileUtility {
  ...
  public FileUtility(String path, String name, boolean newFile, boolean readonly) throws IOException {
    if (path == null || path.equals("")) throw new IllegalArgumentException("directory cannot be null");
    if (name == null || name.equals("")) throw new IllegalArgumentException("file name cannot be null");
  ...
  public void dumpFile(OutputStream outputstream) {
    ...
    try {
      BufferedInputStream bufferedinputstream = new BufferedInputStream(new FileInputStream(new File(dir, filename)));
      int i;
  ...
  public int copy(File source, File destiny, DirFilter dirFilter, boolean discardHidden, boolean overwrite) throws Exception {
    ...
    else list = source.listFiles();
    for(int i = 0; i < list.length; i++) {
      File fileItem = list[i];
      if (fileItem.isDirectory()) {
        if (!discardHidden || !fileItem.isHidden()/*!fileItem.getName().startsWith(".")*/) {
  ...
  /*public void copyFile(File in, File out) throws Exception {
    ...
    OutputStreamWriter printWriterData  = new OutputStreamWriter(fos, "UTF-8");
    String nextLine = fileBuffer.readLine();
    while (nextLine != null) {
      printWriterData.write(nextLine);
      printWriterData.write("\n");
