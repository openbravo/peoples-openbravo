...
public class TranslationUtils {
  ...
  public static HashMap<String, String> processFormLabels(ConnectionProvider con, String filename, String lang) {
    fileName = filename;
    language = lang;
  ...
  public static WindowLabel[] processWindowLabels(ConnectionProvider con, String tabId, String lang, String moduleLang) {
    ...
    try {
      ...
      int i = 0;
      for (Iterator<WindowLabel> iteratorWinLabel = windowLabelsCol.iterator(); iteratorWinLabel.hasNext();) {
  ...
  public static WindowLabel[] processProcessLabels(ConnectionProvider con, String lang, InterfaceInfo uiInfo) {
    ProcessLabelsData[] processLabels;
    try {
      if (lang.equals(uiInfo.getModuleLanguage()) || lang.equals("") || uiInfo.getModuleLanguage().equals("")) {
  ...
  public static InterfaceInfo getInterfaceHeaderTrlInfo(ConnectionProvider con, InterfaceInfo info, String lang) {
  ...
  private static void populateFieldLabels(FieldLabelsData[] fieldLabels) {
    for (int labelsCount = 0; labelsCount < fieldLabels.length; labelsCount++) {
      ...
      WindowLabel label = new WindowLabel(labelData.adColumnId, labelData.fieldName, labelData.fieldtrlName);
  ...
  private static void populateFieldGroupLabels(FieldGroupLabelsData[] fieldGroupLabels) {
    ...
    for (int labelsCount = 0; labelsCount < fieldGroupLabels.length; labelsCount++) {
      ...
      WindowLabel label = new WindowLabel(WindowLabel.FIELD_GROUP_LABEL, labelData.fieldgroupid, labelData.fieldgroupname, labelData.fieldgrouptrlname);
  ...
  private static void populateProcessLabels(ProcessLabelsData[] fieldLabels) {
    ...
    for (int labelsCount = 0; labelsCount < fieldLabels.length; labelsCount++) {
      ...
      WindowLabel label = new WindowLabel(labelData.processparacolumnname, labelData.processparaname, labelData.processparatrlname);
