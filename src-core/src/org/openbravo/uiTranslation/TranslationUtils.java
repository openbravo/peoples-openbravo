...
public class TranslationUtils {
    ...
    public static HashMap<String, String> processFormLabels(
            ConnectionProvider con, String filename, String lang) {
        fileName = filename;
        language = lang;
    ...
    public static InterfaceInfo getModuleLang(ConnectionProvider con,
            InterfaceInfo info) {
        String lang = "";
        try {
            if (info.getId() != null && !info.getId().equals("")) {
                ...
                if (info.getInterfaceType() == InterfaceInfo.TAB) {
                    moduleInfo = InterfaceModuleInfoData.selectTabModuleLang(
                            con, info.getId());
                    ...
                    moduleInfo = InterfaceModuleInfoData
                            .selectProcessModuleLang(con, info.getId());
                    for (int i = 0; i < moduleInfo.length; i++) {
                        InterfaceModuleInfoData module = moduleInfo[i];
                        if (module != null && module.modulelanguage != null) {
                            lang = module.modulelanguage;
    ...
    private static void defineInterfaceInfo(InterfaceInfo info,
            InterfaceModuleInfoData module) {
    ...
    public static WindowLabel[] processWindowLabels(ConnectionProvider con,
            String tabId, String lang, String moduleLang) {
        FieldLabelsData[] fieldLabels;
        FieldGroupLabelsData[] fieldGroupLabels;
        try {
            if (lang.equals("") || lang.equals(moduleLang)
                    || moduleLang.equals("")) {
                fieldLabels = FieldLabelsData.selectModuleFieldLabels(con,
                        tabId);
                ...
                fieldGroupLabels = FieldGroupLabelsData.selectFieldGroupTrl(
                        con, tabId, lang);
                populateFieldLabels(fieldLabels);
                if (fieldGroupLabels.length > 0) {
                    populateFieldGroupLabels(fieldGroupLabels);
            ...
            int i = 0;
            for (Iterator<WindowLabel> iteratorWinLabel = windowLabelsCol
                    .iterator(); iteratorWinLabel.hasNext();) {
    ...
    public static WindowLabel[] processProcessLabels(ConnectionProvider con,
            String lang, InterfaceInfo uiInfo) {
        ProcessLabelsData[] processLabels;
        try {
            if (lang.equals(uiInfo.getModuleLanguage()) || lang.equals("")
                    || uiInfo.getModuleLanguage().equals("")) {
                processLabels = ProcessLabelsData.selectOriginalParameters(con,
                        uiInfo.getId());
                populateProcessLabels(processLabels);
            } else {
                processLabels = ProcessLabelsData.selectTranslatedParameters(
                        con, uiInfo.getId(), lang);
                populateProcessLabels(processLabels);
            }
        } catch (ServletException e) {
            e.printStackTrace();
    ...
    public static InterfaceInfo getInterfaceHeaderTrlInfo(
            ConnectionProvider con, InterfaceInfo info, String lang) {
        InterfaceInfo uiInfoResult = info;
        if (info.getInterfaceType() == InterfaceInfo.TAB) {
            InterfaceTrlInfoData[] data;
            try {
                data = InterfaceTrlInfoData.selectProcessTrlInfo(con,
                        uiInfoResult.getId(), lang);
    ...
    private static void populateFieldLabels(FieldLabelsData[] fieldLabels) {
        for (int labelsCount = 0; labelsCount < fieldLabels.length; labelsCount++) {
            ...
            WindowLabel label = new WindowLabel(labelData.adColumnId,
                    labelData.fieldName, labelData.fieldtrlName);
            windowLabelsCol.add(label);
        }
    }

    private static void populateFieldGroupLabels(
            FieldGroupLabelsData[] fieldGroupLabels) {
        ...
        for (int labelsCount = 0; labelsCount < fieldGroupLabels.length; labelsCount++) {
            ...
            WindowLabel label = new WindowLabel(WindowLabel.FIELD_GROUP_LABEL,
                    labelData.fieldgroupid, labelData.fieldgroupname,
                    labelData.fieldgrouptrlname);
    ...
    private static void populateProcessLabels(ProcessLabelsData[] fieldLabels) {
        ...
        for (int labelsCount = 0; labelsCount < fieldLabels.length; labelsCount++) {
            ...
            WindowLabel label = new WindowLabel(
                    labelData.processparacolumnname, labelData.processparaname,
                    labelData.processparatrlname);
