...
public class DynamicEntityTest extends BaseTest {
	...
	if (true) {
	    assertTrue(
		    ...
		    (System.currentTimeMillis() - bog.getCreated().getTime()) < 2000);
	    assertTrue(
		    "Updated time not updated",
		    (System.currentTimeMillis() - bog.getUpdated().getTime()) < 2000);
	}
	...
	for (CategoryAccounts bga : bogas) {
	...
	for (Property p : e.getProperties()) {
		...
		// place...
		System.err.println(indent + "<" + p.getName() + ">" + value
			+ "</" + p.getName() + ">");
	    } else {
		...
		System.err.println(indent + "<" + p.getName() + ">"
			+ referencedObject.getId() + "</" + p.getName() + ">");
	    }
	}
