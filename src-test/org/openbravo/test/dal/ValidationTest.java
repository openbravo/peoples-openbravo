...
public class ValidationTest extends BaseTest {
	...
	setValue(c, Currency.PROPERTY_UPDATEDBY, "test",
		"only allows reference instances of type");
	setValue(c, Currency.PROPERTY_CLIENT, "test",
		"only allows reference instances of type");
	setValue(c, Currency.PROPERTY_PRICEPRECISION, new Double(400.0),
		"only allows instances of");
	setValue(c, Currency.PROPERTY_CREATED, new BigDecimal(100.0),
		"only allows instances of");
	try {
	...
	setValue(bpGroup, Category.PROPERTY_CLIENT, "test",
		"only allows reference instances of type");
	setValue(bpGroup, Category.PROPERTY_DESCRIPTION, new Double(400.0),
		"only allows instances of");
    ...
    private void setValue(BaseOBObject bob, String propName, Object value,
	    String expectedMessage) {
	try {
	    bob.set(propName, value);
	    if (value == null) {
		fail("Validation not performed on " + bob.getEntityName() + "."
			+ propName + " for null value");
	    } else {
		fail("Validation not performed on " + bob.getEntityName() + "."
			+ propName + " for value of type "
			+ value.getClass().getName());
	    }
	} catch (ValidationException e) {
	    assertTrue("Unexpected exception " + e.getMessage(), e.getMessage()
	...
	setErrorOccured(true);
	setBigBazaarAdminContext();
	final OBCriteria<AlertRule> obc = OBDal.getInstance().createCriteria(
		AlertRule.class);
	for (AlertRule ar : obc.list()) {
	    try {
		ar.setType("A");
		fail("List value check not performed");
	    } catch (ValidationException ve) {
		assertTrue("Illegal exception " + ve.getMessage(), ve
			.getMessage().contains(
	...
	final OBCriteria<Requisition> obc = OBDal.getInstance().createCriteria(
		Requisition.class);

	for (Requisition r : obc.list()) {
	    try {
	...
	try {
	    ...
	    for (InvoiceSchedule is : obc.list()) {
		try {
		    is.setInvoiceDay(40);
