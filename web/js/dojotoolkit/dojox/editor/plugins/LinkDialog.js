if(!dojo._hasResource["dojox.editor.plugins.LinkDialog"]){ //_hasResource checks added by build. Do not use _hasResource directly in your code.
dojo._hasResource["dojox.editor.plugins.LinkDialog"] = true;
dojo.provide("dojox.editor.plugins.LinkDialog");
...
dojo.declare("dojox.editor.plugins.LinkDialog",
	dojox.editor._Plugin,
	{
		...
		setValue: function(args){
			...
			if(dojo.isIE){ //see #4151
				var a = dojo.withGlobal(this.editor.window, "getAncestorElement", dojox.editor.selection, [this.tag]);
				if(a){
					dojo.withGlobal(this.editor.window, "selectElement", dojox.editor.selection, [a]);
				}
			}
			args.tag = this.tag;
		...
		_onOpenDialog: function(){
			...
			if(a){
				...
				dojo.withGlobal(this.editor.window, "selectElement", dojox.editor.selection, [a, true]);
			}else{
				text = dojo.withGlobal(this.editor.window, dojox.editor.selection.getSelectedText);
			}
...
//TODO we don't show this state anymore
		updateState: function(){
			...
			if(this.button){
				...
				var hasA = dojo.withGlobal(this.editor.window, "hasAncestorElement", dojox.editor.selection, [this.tag]);
...
dojo.subscribe(dijit._scopeName + ".Editor.getPlugin",null,function(o){
	...
	case "createLink": case "insertImage":
		o.plugin = new dojox.editor.plugins.LinkDialog({command: o.args.name});
	}
});
