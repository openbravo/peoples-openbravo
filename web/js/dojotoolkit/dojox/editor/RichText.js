if(!dojo._hasResource["dojox.editor.RichText"]){ //_hasResource checks added by build. Do not use _hasResource directly in your code.
dojo._hasResource["dojox.editor.RichText"] = true;
dojo.provide("dojox.editor.RichText");

dojo.require("dijit._Widget");
dojo.require("dojox.editor.selection");
dojo.require("dojox.editor.range");
dojo.require("dojox.editor.html");
...
dojox.editor.RichTextIframeMixin = {
	_writeOpen: function(html){
		...
		if(dojo.isIE || dojo.isSafari || dojo.isOpera){ // contentEditable, easy
			...
			if(dojo.config["useXDomain"] && !dojo.config["dojoBlankHtmlUrl"]){
				console.warn("dojox.editor.RichText: When using cross-domain Dojo builds,"
	...
	onLoad: function(/* Event */ e){
		...
		dojox.editor.RichText.prototype.onLoad.call(this, e);
	},
	...
	addStyleSheet: function(/*dojo._Url*/uri){
		...
		if(dojo.indexOf(this.editingAreaStyleSheets, url) > -1){
			// console.debug("dojox.editor.RichText.addStyleSheet: Style sheet "+url+" is already applied");
			return;
		}
	...
	removeStyleSheet: function(/*dojo._Url*/uri){
		...
		if(index == -1){
			// console.debug("dojox.editor.RichText.removeStyleSheet: Style sheet "+url+" has not been applied");
			return;
		}
	...
	_setDisabledAttr: function(/*Boolean*/ value){
		...
		dojox.editor.RichText.prototype._setDisabledAttr.call(this, value);
	},
...
dojo.declare("dojox.editor.RichText", dijit._Widget, {
	constructor: function(params){
		// summary:
		//		dojox.editor.RichText is the core of dojox.editor.Editor, which provides basic
		...
		if(this.useIframe){
			dojo.mixin(this, dojox.editor.RichTextIframeMixin);
		}
	},
	...
	postCreate: function(){
		if("textarea" == this.domNode.tagName.toLowerCase()){
			console.warn("RichText should not be used with the TEXTAREA tag.  See dojox.editor.RichText docs.");
		}
	...
	_localizeEditorCommands: function(){
		...
		while(node){
			dojox.editor.selection.selectElement(node.firstChild);
			dojo.withGlobal(this.window, "selectElement", dojox.editor.selection, [node.firstChild]);
	...
	open: function(/*DomNode?*/element){
		...
		if(	(dn["nodeName"])&&
			...
			html = this._preFilterContent(dojox.editor.getChildrenHtml(dn));
			dn.innerHTML = "";
		}
		if(html == ""){ html = "&nbsp;"; }
	...
	setDisabled: function(/*Boolean*/ disabled){
		dojo.deprecated('dojox.editor.Editor::setDisabled is deprecated','use dojox.editor.Editor::attr("disabled",boolean) instead', 2);
 ...
 *****************/
	...
	_sCall: function(name, args){
		// summary:
		//		run the named method of dojox.editor.selection over the
		//		current editor instance's window, with the passed args
		dojo.withGlobal(this.window, name, dojox.editor.selection, args);
	},
	...
	_postFilterContent: function(
		...
		if(!dojo.isString(dom)){
			...
			ec = dojox.editor.getChildrenHtml(dom);
		}else{
			ec = dom;
		}
	...
	getNodeHtml: function(/* DomNode */node){
		dojo.deprecated('dojox.editor.Editor::getNodeHtml is deprecated','use dojox.editor.getNodeHtml instead', 2);
		return dojox.editor.getNodeHtml(node); // String
	},

	getNodeChildrenHtml: function(/* DomNode */dom){
		dojo.deprecated('dojox.editor.Editor::getNodeChildrenHtml is deprecated','use dojox.editor.getChildrenHtml instead', 2);
		return dojox.editor.getChildrenHtml(dom);
	},
	...
	destroy: function(){
		...
		//dojox.editor.RichText.superclass.destroy.call(this);
	},
