...
OB.App.StateAPI.Ticket.registerUtilityFunctions({
  ...
  processTicket(ticket, payload) {
    ...
    if (!OB.App.State.Ticket.Utils.isBooked(newTicket)) {
  ...
  calculateChange(ticket, payload) {
    ...
    // And clone payments
    const ticketPayments = ticket.payments.map(payment => {
      return { ...payment };
    });
    ...
    // short circuit out if there's no cash payment type
    if (!firstCashPaymentType) {
      return ticket;
    }

    // find payments that have change using the "overpayment" property for subtotals or just find cash payment if not present
    const paymentsWithChange = ticket.paymentSubtotals
      ? ticketPayments.filter(payment => payment.overpayment)
      : ticketPayments.filter(payment => payment.isCash && ticket.change);

    // short circuit out also if there are no payments with change
    ...
    // make an array for positive payments and put in exact payments if they exist
    const positivePayments =
      exactPayments !== null && exactPayments.length ? [...exactPayments] : [];
