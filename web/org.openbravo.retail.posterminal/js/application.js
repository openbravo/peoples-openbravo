    ...
    if (params) {
      ...
      for (var i in params) {
        if (typeof params[i] === 'string') {
          p[i] = { value: params[i], type : 'string'}
        } else if(typeof params[i] === 'number') {
          if (params[i] === Math.round(params[i])) {
            p[i] = { value: params[i], type : 'long'}
          } else {
            p[i] = { value: params[i], type : 'bigdecimal'}
          }
        } else if(typeof params[i] === 'boolean') {
          p[i] = { value: params[i], type : 'boolean'}
        } else {
          p[i] = params[i];
        }      
      }
      ...
      success: function (data, textStatus, jqXHR) {
        if (data._entityname) {
          ...
          if (status === 0) {
            ...
            callback({ exception : {message : response.errors.id}});
          } else {
            callback({ exception : {message : response.error.message}});
          }
        } 
      },
