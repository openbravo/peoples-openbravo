  ...
  init: function (model) {
    ...
    this.$.renderCombo.change = function (inSender, inEvent) {
      var cond = inSender.getValue(),
          ...
          moveScrollDown, me = this;
      moveScrollDown = function () {
        me.bubble('onMoveScrollDown', {
          lineHeight: document.getElementById(me.parent.parent.parent.parent.id).clientHeight,
    ...
    this.model.get('lines').on('change:obrdmDeliveryMode', function (model) {
      if (model.get('obrdmDeliveryMode') !== 'HomeDelivery' && model.has('hasDeliveryServices')) {
        ...
        this.model.get('lines').forEach(function (line) {
          if (line.get('product').get('productType') === 'S' && line.get('product').get('obrdmIsdeliveryservice')) {
            var relatedLines = line.get('relatedLines').filter(function (relatedLine) {
...
var ReceiptDeliveryDateDefinition = {
  ...
  applyValue: function () {
    ...
    if (deliveryModeProperty === 'PickupInStoreDate' || deliveryModeProperty === 'HomeDelivery') {
      value.setHours(0);
      value.setMinutes(0);
      value.setSeconds(0);
      this.model.set(this.modelProperty, value);
...
OB.UI.ModalReceiptPropertiesImpl.prototype.newAttributes.push(ReceiptDeliveryModeDefinition);
OB.UI.ModalReceiptPropertiesImpl.prototype.newAttributes.push(ReceiptDeliveryDateDefinition);
OB.UI.ModalReceiptPropertiesImpl.prototype.newAttributes.push(ReceiptDeliveryTimeDefinition);
...
(function () {
  ...
  OB.UI.ReceiptPropertiesDialogApply.prototype.tap = _.wrap(OB.UI.ReceiptPropertiesDialogApply.prototype.tap, function (wrapped) {
    var tap = _.bind(origTap, this),
        line_ReceiptDeliveryDate = this.owner.owner.$.bodyContent.$.attributes.$.line_ReceiptDeliveryDate,
        dateComponent = line_ReceiptDeliveryDate ? line_ReceiptDeliveryDate.$.newAttribute.$.ReceiptDeliveryDate : undefined,
        line_ReceiptDeliveryTime = this.owner.owner.$.bodyContent.$.attributes.$.line_ReceiptDeliveryTime,
        timeComponent = line_ReceiptDeliveryTime ? line_ReceiptDeliveryTime.$.newAttribute.$.ReceiptDeliveryTime : undefined;
