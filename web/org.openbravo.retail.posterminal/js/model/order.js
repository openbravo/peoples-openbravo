  ...
  var Order = Backbone.Model.extend({
    ...
    initialize: function(attributes) {
      ...
      if (attributes && attributes.documentNo) {
        ...
        this.set(
          ...
          new Backbone.Collection().reset(attributes.orderManualPromotions)
        );
    ...
    _deleteLine: function(line, selectedLines, callback) {
      ...
      if (
        ...
        me.getStoreStock(line.get('product'), qtyAdded, options, null, function(
          hasStock
        ) {
          if (hasStock) {
            deleteLineOnceChecked();
          ...
          OB.UTIL.HookManager.executeHooks(
            ...
            function(args) {
              ...
              if (!args.allowToAdd) {
                OB.UTIL.showConfirmation.display(
                  OB.I18N.getLabel('OBMOBC_Error'),
                  OB.I18N.getLabel('OBPOS_CannotSellWithoutStock', [
                    p.get('_identifier'),
                    productStatus.name,
                    allLinesQty,
                    attrs.warehouse.warehouseqty,
                    attrs.warehouse.warehousename
                  ]),
                  [
                    {
                      label: OB.I18N.getLabel('OBMOBC_LblOk'),
              ...
              line = me.get('lines').find(function(l) {
                if (
                  ...
                  if (attributeSearchAllowed && attrs) {
                    if (
                      attrs.attributeValue &&
                      l.get('attributeValue') === attrs.attributeValue
                    ) {
                      return true;
                    }
                  } else {
    ...
    addProduct: function(p, qty, options, attrs, callback) {
      ...
      if (
        ...
        this.get('priceList') !== OB.MobileApp.model.get('terminal').priceList
      ) {
        var criteria = {};
        if (!OB.MobileApp.model.hasPermission('OBPOS_remote.product', true)) {
          criteria = {
