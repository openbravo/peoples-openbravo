...
(function() {
  ...
  var Order = Backbone.Model.extend({
    ...
    cancelAndReplaceOrder: function(context, deferredLines) {
      ...
      me.get('lines').models.forEach(line => {
        line.get('promotions').forEach(p => {
          if (p.manual) {
            ...
            if (!manualPromo || !discountRule || !discountRule.isManual) {
              return;
            }
            const addPromotion = {
              ...manualPromo,
              ...
              noOrder: manualPromotions.length + byTotalManualPromotions.length
            };
            if (discountRule.getAmountProperty()) {
              addPromotion[discountRule.getAmountProperty()] = p.userAmt;
            }
