...
enyo.kind({
  ...
  tap: function() {
    ...
    if (isMultiOrder) {
      var receipts = this.owner.model.get('multiOrders').get('multiOrdersList')
        .models;
      receipts.forEach(function(receipt) {
        if (
          (receipt.get('orderType') === 2 || receipt.get('orderType') === 3) &&
          receipt.get('bp').id ===
        ...
        if (!avoidPayment) {
          ...
          var errorMsgLbl,
            ...
            totalToPaid = OB.DEC.abs(
              isMultiOrder
                ? me.owner.model.get('multiOrders').getTotal()
                : me.owner.receipt.getTotal()
            ),
            isReturnOrder = isMultiOrder
              ? false
              : me.owner.receipt.isNegative();

          if (
            _.filter(payments.models, function(payment) {
              return (
    ...
    if (myModel.get('leftColumnViewManager').isOrder()) {
      ...
      receiptHasPrepaymentAmount =
        this.owner.receipt.get('orderType') !== 1 &&
        this.owner.receipt.get('orderType') !== 3;
      pendingPrepayment = OB.DEC.sub(
      ...
      prepaymentLimitAmount = this.owner.model
        .get('multiOrders')
      ...
      pendingPrepayment = OB.DEC.sub(
        ...
        OB.DEC.add(
          paymentStatus.totalAmt,
          this.owner.model.get('multiOrders').get('existingPayment')
        )
      );
    }
    receiptHasPrepaymentAmount =
