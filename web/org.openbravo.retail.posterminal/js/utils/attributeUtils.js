...
(function() {
  ...
  OB.UTIL.AttributeUtils.generateDescriptionBasedOnJson = function(
    ...
    _.each(
      ...
      function(key) {
        ...
        if (
          ...
          currentValue &&
          currentValue.value &&
          _.isString(currentValue.value) &&
          currentValue.value.length > 0
        ) {
          attValueContent += currentValue.label
            ? currentValue.label
            : currentValue.name;
          attValueContent += ': ';
    ...
    if (attSetInstanceDescription && attSetInstanceDescription.length > 1) {
      attSetInstanceDescription = attSetInstanceDescription.substring(
        0,
        attSetInstanceDescription.length - 1
      );
    }
