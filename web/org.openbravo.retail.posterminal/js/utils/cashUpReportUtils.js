...
(function() {
        ...
        function(tax) {
          if (tax.length === 0) {
            ...
            tax
              ...
              .set(
                'amount',
                OB.DEC.add(tax.at(0).get('amount'), cashuptaxes[i].taxAmount)
              );
            OB.Dal.saveInTransaction(
              tx,
              tax.at(0),
  ...
  function updateCashUpInfo(cashUp, receipt, j, callback, tx) {
    ...
    if (j < receipt.length && !receipt[j].has('obposIsDeleted')) {
      ...
      if (cashUp.length !== 0) {
        ...
        cashUp
          .at(0)
          .set('netSales', OB.DEC.add(cashUp.at(0).get('netSales'), netSales));
        cashUp
          .at(0)
          .set(
            'grossSales',
            OB.DEC.add(cashUp.at(0).get('grossSales'), grossSales)
          );
        cashUp
          .at(0)
          .set(
            'netReturns',
            OB.DEC.add(cashUp.at(0).get('netReturns'), netReturns)
          );
        cashUp
          .at(0)
          .set(
            'grossReturns',
            OB.DEC.add(cashUp.at(0).get('grossReturns'), grossReturns)
          );
        cashUp
          .at(0)
          .set(
            ...
            OB.DEC.sub(
              cashUp.at(0).get('grossSales'),
              cashUp.at(0).get('grossReturns')
            )
          );
          ...
          function(payMthds) {
            ...
            _.each(
              ...
              function(payment) {
                ...
                precision =
                  OB.MobileApp.model.paymentnames[auxPay.get('searchKey')]
                    .obposPosprecision;
                ...
                if (amount < 0) {
                  ...
                  auxPay.set(
                    'totalSales',
                    OB.DEC.add(auxPay.get('totalSales'), amount, precision)
                  );
                }
  ...
  OB.UTIL.createNewCashupFromServer = function(
    cashup,
    callback,
    oldCashupPaymentMethods
  ) {
    ...
    OB.Dal.save(
      ...
      function() {
        ...
        if (cashup.get('cashPaymentMethodInfo').length !== 0) {
          ...
          _.each(cashup.get('cashPaymentMethodInfo'), function(
            paymentMethodCashUp
          ) {
            ...
            var pAux = payments.filter(function(payMthd) {
              return (
                paymentMethodCashUpModel.get('paymentmethod_id') ===
                payMthd.payment.id
              );
            })[0];
            if (!pAux) {
            ...
            paymentsAlreadyAdded.push(
              paymentMethodCashUpModel.get('paymentmethod_id')
            );
            if (
              pAux.payment.active === true ||
              (pAux.payment.active === false &&
                paymentMethodCashUpModel.get('totalSales') !== 0 &&
                paymentMethodCashUpModel.get('totalReturns') !== 0 &&
                paymentMethodCashUpModel.get('totalDepostis') !== 0 &&
                paymentMethodCashUpModel.get('totalDrops') !== 0)
            ) {
              promises.push(
                new Promise(function(resolve, reject) {
                  OB.Dal.save(
                    paymentMethodCashUpModel,
                    function() {
                      resolve();
                    },
                    function() {
          ...
          if (oldCashupPaymentMethods && oldCashupPaymentMethods.length > 0) {
            _.each(oldCashupPaymentMethods.models, function(
              oldPaymentMethodCashup
            ) {
              if (
                paymentsAlreadyAdded.indexOf(
                  oldPaymentMethodCashup.get('paymentmethod_id')
                ) === -1
              ) {
                promises.push(
                  new Promise(function(resolve, reject) {
      ...
      function() {
        ...
        if (lastCashupModel && lastCashupModel.get('isprocessed') === 'Y') {
          ...
          OB.UTIL.initializePaymentMethodCashup(
            JSON.parse(lastCashupModel.get('objToSend')).cashCloseInfo,
            null,
            false,
            promises
          );
          ...
          OB.Dal.find(
            ...
            function(lastCashUp) {
              ...
              if (lastCashUp.length !== 0) {
                var promises = [];
                lastCashUpPayments = JSON.parse(
                  lastCashUp.at(0).get('objToSend')
                ).cashCloseInfo;
                OB.UTIL.initializePaymentMethodCashup(
                  lastCashUpPayments,
                  null,
                ...
                new OB.DS.Process(service).exec(
                  ...
                  function(data) {
                    ...
                    if (data[0]) {
                      ...
                      lastCashUpPayments = lastCashUp.get(
  ...
  OB.UTIL.initializePaymentMethodCashup = function(
    lastCashUpPayments,
    cashup,
    funcType,
    promises
    ...
    _.each(
      ...
      function(payment) {
        var startingCash = payment.currentBalance,
          pAux,
          cashupId,
          deposits = payment.payment.totalDeposits,
        ...
        if (
          ...
          promises.push(
            new Promise(function(resolve, reject) {
              OB.Dal.save(
                new OB.Model.PaymentMethodCashUp({
                  id: OB.UTIL.get_UUID(),
    ...
    new OB.DS.Process(service).exec(
      ...
      function(data) {
        if (data && data.exception) {
          var msg = data.exception.message
        ...
        var afterDeleteCallback = function(cashupPaymentMethods) {
          ...
          if (
            ...
            OB.UTIL.createNewCashupFromServer(
              cashUp,
              function() {
  ...
  OB.UTIL.initCashUp = function(
    ...
    if (!lastCashupModel) {
      OB.Dal.find(OB.Model.CashUp, criteria, function(cashUp) {
        //OB.Dal.find success
        if (cashUp.length === 0) {
          ...
          OB.Dal.find(OB.Model.CashUp, processedCriteria, function(
            cashUpProcessed
          ) {
    ...
    OB.Dal.find(
      ...
      function(lastCashUpPayments) {
        ...
        _.each(
          ...
          function(payment) {
            var pAux,
              startingCash = payment.currentBalance;
            if (lastCashUpPayments) {
              pAux = lastCashUpPayments.filter(function(payMthd) {
                return payMthd.get('paymentmethod_id') === payment.payment.id;
              })[0];
              if (
                OB.UTIL.isNullOrUndefined(pAux) &&
                payment.payment.active === true
              ) {
                OB.Dal.save(
                  new OB.Model.PaymentMethodCashUp({
  ...
  OB.UTIL.deleteUnactivePaymentMethod = function(lastCashUpPayment) {
    ...
    if (payments) {
      ...
      if (
        (OB.UTIL.isNullOrUndefined(pAux) || pAux.get('active') === true) &&
        lastCashUpPayment.get('totalReturns') === OB.DEC.Zero &&
        lastCashUpPayment.get('totalSales') === OB.DEC.Zero &&
        lastCashUpPayment.get('totalDepostis') === OB.DEC.Zero &&
        lastCashUpPayment.get('totalDrops') === OB.DEC.Zero
      ) {
