...
OB.UTIL.RfidController.startRfidWebsocket = function startRfidWebsocket(
  websocketServerLocation,
  reconnectTimeout,
  currentRetrials,
  retrialsBeforeWarning
) {
  OB.UTIL.RfidController.set('retrialsBeforeThreadCancellation', 100);
  OB.UTIL.RfidController.set(
    'rfidWebsocket',
    new WebSocket(websocketServerLocation)
  );
  ...
  OB.UTIL.RfidController.get('rfidWebsocket').onopen = function() {
    ...
    OB.UTIL.RfidController.set(
      ...
      new OB.UI.BarcodeActionHandler()
    );
    OB.UTIL.RfidController.set('connectionLost', false);
    OB.UTIL.RfidController.removeAllEpcs();

    if (
      !OB.UTIL.isNullOrUndefined(OB.UTIL.RfidController.get('previousStatus'))
    ) {
  ...
  OB.UTIL.RfidController.waitForAck(
    function(uuid) {
      OB.UTIL.RfidController.get('rfidWebsocket').send(
        'addEpcs:' + uuid + ':' + line.get('obposEpccode')
      );
...
OB.UTIL.RfidController.eraseEpcOrder = function(
  order,
  callback,
  errorCallback
) {
      ...
      function(uuid) {
        OB.UTIL.RfidController.get('rfidWebsocket').send(
          'removeEpcs:' +
            uuid +
            ':' +
            epcCodes.substring(0, epcCodes.length - 1)
        );
...
OB.UTIL.RfidController.removeEpcLine = function(line, callback, errorCallback) {
  OB.UTIL.RfidController.waitForAck(
    function(uuid) {
      OB.UTIL.RfidController.get('rfidWebsocket').send(
        'removeEpcs:' + uuid + ':' + line.get('obposEpccode')
      );
...
OB.UTIL.RfidController.processRemainingCodes = function(
  order,
  callback,
  errorCallback
) {
      ...
      function(uuid) {
        OB.UTIL.RfidController.get('rfidWebsocket').send(
          'addEpcs:' +
            uuid +
            ':' +
            epcCodesToAdd.substring(0, epcCodesToAdd.length - 1)
        );
        OB.debug('add:processRemainingCodes sent, UUID: ' + uuid);
      },
      function() {
        OB.debug(
          'add processRemainingCodes ended succesfully: ' + epcCodesToAdd
        );
        if (callback) {
          callback();
        }
  ...
  if (epcCodesToErase) {
    OB.UTIL.RfidController.waitForAck(
      function(uuid) {
        OB.UTIL.RfidController.get('rfidWebsocket').send(
          'removeEpcs:' +
            uuid +
            ':' +
            epcCodesToErase.substring(0, epcCodesToErase.length - 1)
        );
        OB.debug('remove processRemainingCodes sent, UUID: ' + uuid);
      },
      function() {
        OB.debug(
          'remove processRemainingCodes ended succesfully: ' + epcCodesToErase
        );
        if (callback) {
          callback();
        }
      ...
      function() {
        OB.debug(
          'error while remove processRemainingCodes: ' + epcCodesToErase
        );
        if (errorCallback) {
          errorCallback();
        }
  ...
  if (OB.MobileApp.model.get('terminal').terminalType.useSecurityGate) {
    OB.UTIL.RfidController.waitForAck(
      function(uuid) {
        OB.UTIL.RfidController.get('rfidWebsocket').send(
          'send:' + uuid + ':' + JSON.stringify(order.serializeToJSON())
        );
        OB.debug('sent processRemaingingCodes sent, UUID: ' + uuid);
      },
      function() {
        OB.debug(
          'send processRemainingCodes ended succesfully: ' + order.get('id')
        );
        if (callback) {
          callback();
        }
...
OB.UTIL.RfidController.disconnectRFIDDevice = function(
  callback,
  errorCallback
) {
  if (OB.UTIL.RfidController.get('rfidWebsocket')) {
...
OB.UTIL.RfidController.waitForAck = function(
  functionToExecute,
  callback,
  errorCallback,
  interval,
  ...
  if (index > -1) {
    ...
    for (
      i = OB.UTIL.RfidController.get('rfidAckArray').length - 1;
      i >= 0;
      i--
    ) {
...
OB.UTIL.RfidController.on(
  'change:isRFIDEnabled',
  function(model) {
    OB.MobileApp.view.originalRFIDMode = OB.UTIL.RfidController.get(
      'isRFIDEnabled'
    );
  },
  this
