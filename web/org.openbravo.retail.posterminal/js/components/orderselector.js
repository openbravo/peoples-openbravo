...
enyo.kind({
        ...
        components: [{
          ...
          tap: function () {
            ...
            this.bubble('onShowPopup', {
              popup: this.owner.$.orderlineInfoBtn.showinfoplus ? 'OBRDM_UI_OrderLineInfoPopup' : 'OBRDM_UI_OrderLineInfoPopupStandard',
              args: {
                context: this,
      ...
      components: [{
        ...
        components: [{
          ...
          components: [{
            ...
            tap: function () {
              var qty = parseInt(this.owner.$.orderlineQty.getValue(), 10),
              ...
              if (qty > min) {
                ...
                this.bubble('onNumberChange', {
                  ...
                  value: parseInt(this.owner.$.orderlineQty.getValue(), 10)
                });
              }
            }
          }, {
            ...
            tap: function () {
              ...
              this.bubble('onShowPopup', {
                ...
                args: {
                  ...
                  callback: function (context, warehouseId, warehouseName) {
                    context.bubble('onShowSelector');
                    if (warehouseId) {
                      context.owner.model.set('warehouseId', warehouseId, {
                        silent: true
                      });
                      context.owner.model.set('warehouseName', warehouseName);
                    }
                  }
                }
              });
  ...
  create: function () {
    ...
    if (this.model.get('ltype') === 'ORDER') {
      ...
      this.$.order.hide();
      this.$.orderline.show();
      this.$.orderlineInfo.setContent(OB.I18N.getLabel('OBRDM_LblLineInfo', [this.model.get('lineNo'), this.model.get('qtyOrdered'), this.model.get('qtyPending'), this.model.get('dateDelivered')]));
      var deliveryMode = this.model.get('deliveryMode'),
          delivery = _.find(OB.MobileApp.model.get('deliveryModes'), function (dm) {
          return deliveryMode === dm.id;
        });
      this.$.orderlineProduct.setContent(this.model.get('productName'));
      this.$.orderlineDeliveryMode.setContent(delivery ? delivery.name : deliveryMode);
      this.$.orderlineWarehouse.setContent(OB.MobileApp.model.get('warehouses').find(function (wh) {
        return wh.warehouseid === me.model.get('warehouseId');
      }).warehousename);
      if (OB.MobileApp.model.hasPermission('OBRDM_warehouse.preparation', true)) {
        this.$.orderlineWarehouse.tap = this.onTapCloseButton;
        this.$.orderlineWarehouse.removeClass('obrdm-cell-orderline-button');
        this.$.orderlineWarehouse.addClass('obrdm-cell-orderline-button-disabled');
      }
      ...
      OB.UTIL.HookManager.executeHooks('OBRDM_ShowLineInfo', {
        line: this.model,
        showInfo: false
      }, function (args) {
        if (me.$.orderlineInfoBtn) {
          if (args && args.showInfo) {
            ...
            me.$.orderlineInfoBtn.addRemoveClass('obrdm-button-info-normal', false);
            me.$.orderlineInfoBtn.addRemoveClass('obrdm-button-info-plus', true);
          } else {
            me.$.orderlineInfoBtn.showinfoplus = false;
            me.$.orderlineInfoBtn.addRemoveClass('obrdm-button-info-plus', false);
            me.$.orderlineInfoBtn.addRemoveClass('obrdm-button-info-normal', true);
          }
        }
      });
    }
...
enyo.kind({
  ...
  selectAll: function (inSender, inEvent) {
    ...
    _.each(this.$.stOrderSelector.collection.models, function (order) {
    ...
    this.$.stOrderSelector.$.theader.$.modalOrderScrollableHeader.$.buttonPrepareSelected.disableButton(mode === 'OFF');
    return true;
  },
  ...
  getNextOrderPreNoSuffixInPinckinglist: function (tx, callback) {
    var criteria = {
      'posSearchKey': OB.MobileApp.model.get('terminalName')
        ...
        suffixNext;
    OB.Dal.findInTransaction(tx, OB.Model.DocumentSequence, criteria, function (documentSequenceList) {
      if (documentSequenceList && documentSequenceList.length !== 0) {
        suffixNext = documentSequenceList.at(0).get('orderPreDocumentSequence') + 1;
        callback({
          orderprenoPreffix: OB.MobileApp.model.get('terminal').orderPreDocNoPrefix,
  ...
  prepareSelected: function (inSender, inEvent) {
    var showApproval = false,
        me = this,
        obposPrepaymentlimitamt, payment;

    function continueExecution(model) {
      var me = model,
          lines = [],
          documentNo, orderedDate, headerDescription, orderTotal, bpartner;
      _.each(model.ordersList.models, function (line) {
        if (line.get('ltype') === 'ORDER') {
          ...
          if (OB.MobileApp.model.get('terminal').terminalType.calculateprepayments && payment < obposPrepaymentlimitamt) {
            showApproval = true;
          }
        } else if (line.get('ltype') === 'ORDERLINE' && line.get('toPrepare') > 0) {
          var newline = {};
          var prop;
          ...
          lines.push(newline);
          OB.debug('Prepare Order "' + documentNo + '", Line: ' + line.get('productName') + ', Qty: ' + line.get('qtyOrdered') + ', Pend: ' + line.get('qtyPending') + //
          ', Del: ' + line.get('qtyDelivered') + ', Prepare: ' + line.get('toPrepare'));
        }
      });
      if (lines.length > 0) {
        OB.UTIL.HookManager.executeHooks('OBRDM_PrePreparedLines', {
          lines: lines
        }, function (args) {
          if (args && args.cancellation && args.cancellation === true) {
            ...
            if (stockAvail || lines.length > args.errors.length) {
              buttons.push({
                ...
                action: function () {
                  me.doShowSelector();
                  me.ordersList.trigger('verifyPrepareSelectedButton', me.ordersList.at(0));
                  _.each(args.errors, function (line) {
                    var orderline = _.find(me.ordersList.models, function (ol) {
                      return ol.get('orderLineId') === line.orderLineId;
                    });
                    if (orderline) {
                      var cmp = orderline.get('renderCmp');
                      cmp.$.orderlineQty.setValue(line.stockAvail);
                      cmp.changeIconClass(cmp.$.iconOrderLine, line.stockAvail === 0 ? 'OFF' : 'HALF');
            ...
            _.each(lines, function (line) {
              var groupedDeliveryMode = mappingGroupedDelivery[line.deliveryMode] ? mappingGroupedDelivery[line.deliveryMode] : line.deliveryMode;
              var item = _.find(groupedLinesToPrepare, function (grp) {
                return grp.bpId === line.bpId && grp.warehouseId === line.warehouseId && grp.deliveryMode === groupedDeliveryMode && grp.bp.bpLocId === line.bpartner.bpLocId;
              });
              if (!item) {
                item = {
                  id: OB.UTIL.get_UUID(),
                  posTerminal: OB.MobileApp.model.get('terminal').id,
                  organizationId: OB.MobileApp.model.get('terminal').organization,
                  bpId: line.bpId,
                  warehouseId: line.warehouseId,
                  prepareDate: prepareDate,
                  deliveryMode: groupedDeliveryMode,
                  bp: line.bpartner,
                  orgName: OB.MobileApp.model.get('terminal').organization$_identifier,
                  orgAddress: OB.MobileApp.model.get('terminal').organizationAddressIdentifier,
                  salesRepresentative_identifier: !OB.UTIL.isNullOrUndefined(OB.MobileApp.model.receipt.get('salesRepresentative$_identifier')) ? OB.MobileApp.model.receipt.get('salesRepresentative$_identifier') : OB.MobileApp.model.get('context').user._identifier,
            ...
            process.exec({
              ...
              if (data && data.exception) {
                ...
                if (data && data.response) {
                  OB.UTIL.showConfirmation.display(OB.I18N.getLabel('OBRDM_IssueSalesOrderTitle'), data.response, [{
                    label: OB.I18N.getLabel('OBMOBC_LblOk'),
                    action: function () {
                      return true;
                    }
                  ...
                  _.each(OB.MobileApp.view.$.containerWindow.$.pointOfSale.$.multiColumn.$.leftPanel.$.receiptview.$.orderview.$.listOrderLines.$.tbody.getComponents(), function (component) {
                    var renderOrderLine = component.renderline;
                    if (data.qtyDeliveredByOrderLine.hasOwnProperty(renderOrderLine.model.get('id'))) {
                      renderOrderLine.model.set("deliveredQuantity", data.qtyDeliveredByOrderLine[renderOrderLine.model.get('id')]);
                    }
                  });
                }
              }
    ...
    if (showApproval && !me.owner.owner.args.directNavigation) {
      var approval = _.find(OB.MobileApp.model.receipt.get('approvals'), function (approval) {
      ...
      if (!approval) {
        ...
        OB.MobileApp.view.$.containerWindow.getRoot().model, [{
          ...
          if (approved) {
            ...
            me.$.stOrderSelector.$.theader.$.modalOrderScrollableHeader.$.buttonPrepareSelected.preparing = false;
            me.$.stOrderSelector.$.theader.$.modalOrderScrollableHeader.$.buttonPrepareSelected.disableButton(false);
          }
        });
      } else {
        continueExecution(me);
  ...
  clearAction: function () {
    ...
    this.$.stOrderSelector.$.theader.$.modalOrderScrollableHeader.$.buttonSelectAll.changeTo(true);
    this.$.stOrderSelector.$.theader.$.modalOrderScrollableHeader.$.buttonSelectAll.setDisabled(true);
    this.$.stOrderSelector.$.theader.$.modalOrderScrollableHeader.$.buttonPrepareSelected.disableButton(true);
    return true;
  },

  searchAction: function (inSender, inEvent) {
    var me = this;
    me.$.stOrderSelector.$.theader.$.modalOrderScrollableHeader.$.buttonSelectAll.setDisabled(true);
    ...
    function successCallbackOrders(data) {
      ...
      if (data && !data.exception) {
        me.$.stOrderSelector.$.theader.$.modalOrderScrollableHeader.$.buttonSelectAll.setDisabled(data.length === 0);
        ...
        _.each(data, function (iter) {
          ...
          if (!order) {
            ...
            order = new OB.Model.OBRDM_OrderToSelectorIssue({
              ...
              orderedDate: moment(iter.orderDate, "YYYY-MM-DD").format(OB.Format.date.toUpperCase()),
              bpName: iter.bpName,
              bpId: iter.bpId,
          ...
          newline.dateDelivered = iter.dateDelivered ? moment(iter.dateDelivered, "YYYY-MM-DD").format(OB.Format.date.toUpperCase()) : '';
        ...
        me.$.stOrderSelector.$.theader.$.modalOrderScrollableHeader.$.buttonSelectAll.changeTo(true);
      } else {
    ...
    OB.UTIL.HookManager.executeHooks('OBRDM_GetExcludedDeliveryModes', {
      ...
      process.exec({
        ...
        orderby: inEvent.advanced ? inEvent.orderby : {
          name: filters[0].column === 'orderIds' ? 'documentNo' : filters[0].column,
          direction: 'asc'
        },
        parameters: {
          remoteModel: true,
  ...
  init: function (model) {
    ...
    this.templatePreparation = new OB.DS.HWResource(terminal.printOrderPreparationTemplate || '', 'printOrderPreparationTemplate');
    this.ordersList.on('verifyPrepareSelectedButton', function (item) {
      ...
      me.$.stOrderSelector.$.theader.$.modalOrderScrollableHeader.$.buttonPrepareSelected.disableButton(disablePrepareSelected);
      me.$.stOrderSelector.$.theader.$.modalOrderScrollableHeader.$.buttonSelectAll.changeTo(!setAllSelected);
    });
  }
...
enyo.kind({
  ...
  executeOnShow: function () {
    if (!this.initialized) {
      ...
      if (!this.notClear) {
        this.$.body.$.listOrders.$.stOrderSelector.$.theader.$.modalOrderScrollableHeader.$.buttonPrepareSelected.disableButton(true);
        this.$.body.$.listOrders.$.stOrderSelector.$.theader.$.modalOrderScrollableHeader.$.buttonSelectAll.setDisabled(true);
      }
  ...
  getFilterSelectorTableHeader: function () {
    return this.$.body.$.listOrders.$.stOrderSelector.$.theader.$.modalOrderScrollableHeader.$.filterSelector;
  },
  getAdvancedFilterBtn: function () {
    return this.$.body.$.listOrders.$.stOrderSelector.$.theader.$.modalOrderScrollableHeader.$.buttonAdvancedFilter;
  },
