...
enyo.kind({
  ...
  displayLogic: function() {
    ...
    if (
      this.model.get('order').get('isEditable') === false ||
    ...
    if (
      !_.find(OB.MobileApp.model.get('payments'), function(payment) {
  ...
  init: function(model) {
    ...
    receipt.on(
      'change:isEditable change:isQuotation change:gross change:orderType change:replacedorder',
...
enyo.kind({
  ...
  displayLogic: function() {
    ...
    if (
      ...
      if (
        this.model.get('orderList').current &&
        OB.UTIL.isCrossStoreReceipt(this.model.get('orderList').current)
      ) {
        this.hide();
      }
    } else {
...
enyo.kind({
  ...
  init: function(model) {
    ...
    var receipt = model.get('order'),
      me = this;
    receipt.on(
      'change:isEditable change:isQuotation change:orderType',
      function(model) {
        if (
          !model.get('isEditable') ||
          model.get('isQuotation') ||
          (model.get('orderType') === 1 &&
            !OB.MobileApp.model.hasPermission(
              'OBPOS_AllowLayawaysNegativeLines',
              true
            )) ||
    ...
    this.model.get('leftColumnViewManager').on(
      ...
      function(changedModel) {
        if (changedModel.isOrder()) {
          if (
            model.get('order').get('isEditable') &&
...
enyo.kind({
  ...
  init: function(model) {
    ...
    var receipt = model.get('order'),
      me = this;
    receipt.on(
      'change:generateInvoice change:bp change:isQuotation',
      function(model) {
        if (
          !model.get('isQuotation') &&
          !model.get('generateInvoice') &&
          model.get('bp').get('invoiceTerms') === 'I'
        ) {
...
enyo.kind({
  ...
  tap: function() {
    ...
    this.inherited(arguments); // Manual dropdown menu closure
    if (
      OB.MobileApp.model.get('terminal').terminalType.documentTypeForQuotations
    ) {
      if (OB.MobileApp.model.hasPermission(this.permission)) {
