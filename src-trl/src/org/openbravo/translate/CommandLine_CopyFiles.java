...
class CommandLine_CopyFiles {
    ...
    public static void main(String argv[]) throws Exception {
        ...
        if (argv.length < 6) {
            log4j
                    .error("Usage: java org.openbravo.erpCommon.utility.CommandLine_CopyFiles XmlPool.xml source destiny filter discard_hidden overwrite [language]");
            return;
        }
        ...
        if (!fileSource.exists())
            throw new Exception("Source directory doesn't exists: " + source);

        CPStandAlone conn = null;
        try {
            conn = new CPStandAlone(connectionFile);
            LanguageComboData[] data = LanguageComboData.selectOthers(conn,
                    language);
            if (data != null && data.length > 0) {
                for (int i = 0; i < data.length; i++) {
                    File fileLang = new File(destiny, data[i].adLanguage);
                    fileLang.mkdir();
                    log4j.info("files copied to "
                            + data[i].adLanguage
                            + ": "
                            + FileUtility.copy(fileSource, fileLang, filter,
                                    discardHidden, overwrite));
                }
            }
