...
public class RTLSkin {
   ...
   *              0- Path where are the skin files to modify (RTL folder of the skin).
   *              1- Path where are the skin files to modify (LTR folder of the skin).
   *              2- Path to the original skin files
   * @throws Exception
   */
  public static void main(String argv[]) throws Exception {
    ...
    if (argv.length!=3) {
      log4j.error("Usage: java RTLSkin pathRTLSkinFolder pathLTRSkinFolder pathSourceSkinFiles");
      return;
    }
    ...
    for (int i=0; i<files.length; i++) {
      ...
      fImagesToFlip = new File(srcDirOrig + "/" + files[i] + "/RTLFlippedImages.txt");
  ...
  public static void applySpriteTags(File file, String skinName, String textDirection) throws Exception {
    ...
    fileName = file.toString().substring(file.toString().lastIndexOf("/")+1,file.toString().length()-4);
    int initLTR, initRTL, midLTR, midRTL, endLTR, endRTL;
    
    while ((line=in.readLine())!=null){
      if (line.indexOf("xxspritexx")!=-1) {
        
        line= line.replace("xxspritexx", fileName+"_"+skinName+"_"+textDirection+"_Sprite")+"\n";
        midLine="";
        ...
        if (initRTL!=-1 || initLTR!=-1) {
          if (initRTL==-1){
            ...
            midLine = line.substring(initRTL, midRTL-4)+":"+line.substring(midRTL+1,endRTL)+";";
          }
          if (initLTR==-1){
            initLTR=line.length();
          } else if (textDirection.equals("LTR")) {
            midLine = line.substring(initLTR, midLTR-4)+":"+line.substring(midLTR+1,endLTR)+";";
          }
          
          line = line.substring(0, Math.min(initLTR,initRTL)) + midLine + line.substring(Math.max(endRTL, endLTR)+1, line.length());
        }
        vLines.add(line+"\n");
      } else {
        vLines.add(line+"\n");
