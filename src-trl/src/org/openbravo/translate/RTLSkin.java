 ...
 * 
 *         Performs required modifications in order to convert a skin to use it
 *         with RTL languages
 **/
public class RTLSkin {
     ...
     * 
     * @param argv
     *            List of arguments: 0- Path where are the skin files to modify
     *            (RTL folder of the skin). 1- Path where are the skin files to
     *            modify (LTR folder of the skin).
     * @throws Exception
     */
    public static void main(String argv[]) throws Exception {
        ...
        if (argv.length != 2) {
            log4j
                    .error("Usage: java RTLSkin pathRTLSkinFolder pathLTRSkinFolder");
        ...
        for (int i = 0; i < files.length; i++) {
            ...
            fImagesToFlip = new File(srcDirRTLSkin + "/" + files[i]
            ...
            if (!fImagesToFlip.exists()) {
                ...
                if (strDirModule.list().length == 1
                        && strDirModule.isDirectory()) {
                    fImagesToFlip = new File(srcDirRTLSkin + "/" + files[i]
                            + "/" + strDirModule.list()[0]
                            + "/RTLFlippedImages.txt");
                    pathModule = srcDirRTLSkin + "/" + files[i] + "/"
                            + strDirModule.list()[0];
                } else {
                    log4j.error("Not Founded File with images to flip! "
                            + srcDirRTLSkin + "/" + files[i]
                            + "/RTLFlippedImages.txt");
    ...
    /**
     * Runs through all the files of the specified path reads and, if
     * appropriate, modifies the file.
     * 
     * @param srcDir
     *            Path where are the files to modify (rtl folder of the skin).
     * @param strSkinFolderName
     *            Path to the folder that contains the original skin -where no
     ...
     */
    public static void runFolders(File srcDir, String strSkinFolderName,
            String relativePath, String folderContentType) {
        ...
        for (int i = 0; i < list.length; i++) {
            ...
            if (fileItem.isDirectory()) {
                ...
                runFolders(fileItem, strSkinFolderName, relativePath,
                        folderContentType);
     ...
     *            RTL or LTR
     */
    public static void applySpriteTags(File file, String skinName,
            String textDirection) throws Exception {
        ...
        while ((line = in.readLine()) != null) {
            if (line.indexOf("xxspritexx") != -1) {
                ...
                line = line.replace("xxspritexx", fileName + "_" + skinName
                        + "_" + textDirection + "_Sprite")
                        + "\n";
                midLine = "";
                ...
                if (initRTL != -1 || initLTR != -1) {
                    ...
                    line = line.substring(0, Math.min(initLTR, initRTL))
                            + midLine
                            + line.substring(Math.max(endRTL, endLTR) + 1, line
                                    .length());
                }
                vLines.add(line + "\n");
            } else {
            ...
            for (String fileName : vImagesToFlip) {
                if (file.toString().toLowerCase().indexOf(fileName) != -1) {
                    try {
                        ...
                        log4j.error("RTLSkin.modifySkin error in file "
                                + file.toString() + ": " + e);
                    }
                }
            }
