...
public class Translate extends DefaultHandler implements LexicalHandler {
     ...
     * @throws ServletException
     */
    public Translate(String xmlPoolFile, String _fileTermination)
            throws ServletException {
        this(xmlPoolFile);
        ...
        if (toLanguage.length == 0)
            log4j
                    .warn("No system languages defined, translation will parse all files.");
    }
     ...
     *            first one is the attribute to indicate if the
                    ...
                    log4j
                            .debug("dir: "
                                    ...
                                    + (moduleDirectories != null
                                            && moduleDirectories.size() > level ? moduleDirectories
                                            .get(level)
                                            : "--"));
                if (parse)
                    ...
                    if ((moduleDirectories.size() == level + 1)
                        ...
                        log4j.info("Start parsing module: "
                                + parent.replace("/", ""));
                        ...
                        try {
                            if (TranslateData.isInDevelopmentModulePack(pool,
                                ...
                                log4j
                                        .info("Module is not in development: skipping it");
                        } catch (final Exception e) {
                            e.printStackTrace();
                        }
    ...
    private static void parseFile(File fileParsing, String relativePath,
        ...
        if (log4j.isDebugEnabled())
            log4j.debug("Module name: " + module + " - oldRelativePath:"
    ...
    /**
     * Start of an element of the file. When the parser finds a new element in
     * the file, it calls to this method.
     */
    @Override
    public void startElement(String uri, String name, String qName,
            Attributes amap) {// (String name,
        // AttributeList
        // amap) throws
        // SAXException {
        if (!translateModule)
        ...
        if (log4j.isDebugEnabled())
            log4j.info("Configuration: startElement is called: element name="
                    ...
                    + translationText);
        if (actualTag != null && isParseable(actualTag)
    ...
    /**
     * End of an element of the file. When the parser finds the end of an
     * element in the file, it calls to this method.
     */
    @Override
    ...
    /**
     * This method is called by the parser when it finds any content between the
     * start and end element's tags.
     */
    @Override
    ...
    private void translate(String ini, boolean isPartial) {
        ...
        if (!ini.equals("") && !ini.toLowerCase().startsWith("xx")
                && !isNumeric(ini)) {
            log4j.debug("Translating " + ini + " for file" + actualFile
            ...
            try {
                ...
                if (!aux) {
                    if (TranslateData.existsExpresionModFile(pool, ini,
                            actualFile, moduleLang) == 0
                            && TranslateData.existsExpresionModNoFile(pool,
                                    ini, moduleLang) == 0
                            && TranslateData.existsExpresionNoModFile(pool,
                                    ini, actualFile, moduleLang) == 0
                            && TranslateData.existsExpresionNoModNoFile(pool,
                                    ini, moduleLang) == 0) {

                        if (!TranslateData
                                .isInDevelopmentModule(pool, moduleID))
                            log4j
                                    .error("Module  is not in development, it will be inserted anyway");
                        if (log4j.isDebugEnabled())
    ...
    private static String replace(String strInicial, String strReplaceWhat,
        ...
        do {
            ...
            if (pos != -1) {
                strFinal.append(strInicial.substring(index, pos)
                        + strReplaceWith);
    ...
    private String tokenize(String ini, int indice, Vector<String> isTranslated) {
        ...
        try {
            ...
            TranslateData[] dataTranslated = TranslateData.select(pool, ini
                    .trim(), actualFile, actualLanguage);
            if (dataTranslated != null && dataTranslated.length > 0) {
                ...
                TranslateData
                        .update(pool, dataTranslated[0].adTextinterfacesId);
            }
            if (translated != null && translated.length() > 0) {
                fin.append(translated);
                return fin.toString();
            }
            final StringTokenizer st = new StringTokenizer(ini, tokens[indice],
                    false);
            while (st.hasMoreTokens()) {
                ...
                translated = null;
                dataTranslated = TranslateData.select(pool, token.trim(),
                        actualFile, actualLanguage);
                if (dataTranslated != null && dataTranslated.length > 0) {
                    translated = dataTranslated[0].tr;
                    // TranslateData.update(pool,
                    // dataTranslated[0].baseDictionaryEntryId, actualLanguage);
                    TranslateData.update(pool,
                            dataTranslated[0].adTextinterfacesId);
                }
