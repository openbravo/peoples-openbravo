...
public class OBScheduler {
  ...
  /**
   * Schedule a new process (bundle) to run immediately in the background 
   * with the default Openbravo Job implementation. 
   * 
  ...
  /**
   * Schedule a new process (bundle) to run immediately in the background
   *  with the specified Job implementation.
   * 
   * This will create a new record in AD_PROCESS_REQUEST.
   * 
  ...
  public void schedule(ProcessBundle bundle, Class<? extends Job> jobClass) 
    ...
    ProcessRequestData.insert(getConnection(), context.getOrganization(), 
        context.getClient(), context.getUser(), context.getUser(), requestId, 
        processId, context.getUser(), SCHEDULED, channel.toString(), 
        context.toString(), bundle.getParamsDefalated(), null, null, null);
    
    schedule(requestId, bundle, jobClass);
  }
  
  /**
   * Schedule a process (bundle) with the specified requestId and using the default
   * Openbravo Job implementation. The requestId is used in Quartz as the JobDetail's
   * name. The details must be saved to AD_PROCESS_REQUEST before reaching this method.
   * 
   * @param requestId   
   * @param bundle
   * @param jobClass
   ...
   * Job implementation. The requestId is used in Quartz as the JobDetail's
   * name. The details must be saved to AD_PROCESS_REQUEST before reaching this method.
   * 
   * @param requestId
   * @param bundle
  ...
  public void schedule(String requestId, ProcessBundle bundle, Class<? extends Job> jobClass) 
    ...
    JobDetail jobDetail = JobDetailProvider.newInstance(requestId, bundle, jobClass);
    Trigger trigger = TriggerProvider.newInstance(requestId, bundle, getConnection());
  ...
  public void initialize(Scheduler sched) throws SchedulerException {
    ...
    try {
       data = ProcessRequestData.selectByStatus(
          getConnection(), SCHEDULED);
    ...
    for (ProcessRequestData request : data) {
      ...
      try {
        ProcessBundle bundle = ProcessBundle.request(requestId, vars, getConnection());
        schedule(requestId, bundle);
  ...
  private static class JobDetailProvider {
    ...
    /**
     * Creates a new JobDetail with the specified name and job class. Inserts
     * the process bundle into the JobDetail's jobDataMap for retrieval when 
     * the job is executed.
     * 
  ...
  private static class TriggerProvider {
      ...
      try {
        ...
        if ("".equals(timingOption) || timingOption.equals(TIMING_OPTION_IMMEDIATE)) {
          ...
          if (data.frequency.equals(FREQUENCY_SECONDLY)) {
            trigger = makeIntervalTrigger(FREQUENCY_SECONDLY, data.secondlyInterval, data.secondlyRepetitions);
          
          } else if (data.frequency.equals(FREQUENCY_MINUTELY)) {
            trigger = makeIntervalTrigger(FREQUENCY_MINUTELY, data.minutelyInterval, data.minutelyRepetitions);
          
          } else if (data.frequency.equals(FREQUENCY_HOURLY)) {
            trigger = makeIntervalTrigger(FREQUENCY_HOURLY, data.hourlyInterval, data.hourlyRepetitions);
            ...
            if ("".equals(data.dailyOption)) {
              ...
              String cronExpression = second + " " + minute + " " + hour + " ? * MON-FRI";
              trigger = new CronTrigger(name, OB_GROUP, cronExpression);
              
            } else if (data.dailyOption.equals(WEEKENDS)) {
              String cronExpression = second + " " + minute + " " + hour + " ? * SAT,SUN";
              trigger = new CronTrigger(name, OB_GROUP, cronExpression);
            ...
            if (data.daySun.equals("Y")) sb.append("SUN");
            if (data.dayMon.equals("Y")) sb.append(sb.length() == 0 ? "MON" : ",MON");
            if (data.dayTue.equals("Y")) sb.append(sb.length() == 0 ? "TUE" : ",TUE");
            if (data.dayWed.equals("Y")) sb.append(sb.length() == 0 ? "WED" : ",WED");
            if (data.dayThu.equals("Y")) sb.append(sb.length() == 0 ? "THU" : ",THU");
            if (data.dayFri.equals("Y")) sb.append(sb.length() == 0 ? "FRI" : ",FRI");
            if (data.daySat.equals("Y")) sb.append(sb.length() == 0 ? "SAT" : ",SAT");
        ...
        String msg = Utility.messageBD(conn, "TRIG_INVALID_DATA", bundle.getContext().getLanguage());
        throw new ServletException(msg + " " + e.getMessage());
      } 
      
      if (trigger.getName() == null) trigger.setName(name);
      if (trigger.getGroup() == null) trigger.setGroup(OB_GROUP);
    ...
    private static Calendar timestamp(String date, String time, String dateTimeFormat) 
      ...
      Calendar cal = null;
      String dateFormat = dateTimeFormat.substring(0, dateTimeFormat.indexOf(' '));
