...
public class OBScheduler {
	...
	/**
	 * Schedule a new process (bundle) to run immediately in the background with the
	 * default Openbravo Job implementation.
	 * 
	...
	/**
	 * Schedule a new process (bundle) to run immediately in the background with the
	 * specified Job implementation.
	 * 
	 * This will create a new record in AD_PROCESS_REQUEST.
	 * 
	 * @param bundle   The bundle with all of the process' details
	 * @param jobClass The Quartz Job implementation that will execute when
	 * 
	...
	public void schedule(ProcessBundle bundle, Class<? extends Job> jobClass)
		...
		ProcessRequestData.insert(getConnection(), context.getOrganization(), context.getClient(), context.getUser(),
				context.getUser(), requestId, processId, context.getUser(), SCHEDULED, channel, context.toString(),
				bundle.getParamsDeflated(), null, null, null, null);

		if (bundle.getGroupInfo() != null) {
			// Is Part of a Group, update the info
			ProcessRequestData.updateGroup(getConnection(), bundle.getGroupInfo().getRequest().getId(), requestId);
		}
	 ...
	 * default Openbravo Job implementation. The requestId is used in Quartz as the
	 * JobDetail's name. The details must be saved to AD_PROCESS_REQUEST before
	 * reaching this method.
	 * 
	 * @param requestId the id of the request, the Quartz jobDetail name
	 * @param bundle    the context bundle
	 * @throws SchedulerException
	 */
	public void schedule(String requestId, ProcessBundle bundle) throws SchedulerException, ServletException {
		schedule(requestId, bundle, DefaultJob.class.asSubclass(Job.class));
	}

	/**
	 * Schedule a process (bundle) with the specified requestId and the specified
	 * Job implementation. The requestId is used in Quartz as the JobDetail's name.
	 * The details must be saved to AD_PROCESS_REQUEST before reaching this method.
	 * 
	 * @param requestId
	 * @param bundle
	 * @param jobClass
	...
	/**
	 * Returns whether current node is set with no-execute background policy, which
	 * should prevent any process scheduling.
	 */
	public static boolean isNoExecuteBackgroundPolicy() {
		String policy = OBPropertiesProvider.getInstance().getOpenbravoProperties().getProperty(BACKGROUND_POLICY,
				"default");
	 ...
	 */
	public void reschedule(String requestId, ProcessBundle bundle) throws SchedulerException, ServletException {
		try {
	...
	public void unschedule(String requestId, ProcessContext context) throws SchedulerException {
		try {
			sched.unscheduleJob(triggerKey(requestId, OB_GROUP));
			sched.deleteJob(jobKey(requestId, OB_GROUP));
			ProcessRequestData.update(getConnection(), UNSCHEDULED, null, sqlDateTimeFormat, format(new Date()),
					context.getUser(), requestId);
		...
		try {
			...
			for (final ProcessRequestData request : data) {
				...
				if ("Direct".equals(request.channel)
					...
					// do not re-schedule immediate and direct requests that were in execution last
					// time
					// Tomcat stopped
	...
	private void scheduleProcess(String requestId, VariablesSecureApp vars) throws SchedulerException {
		try {
	...
	private static class JobDetailProvider {
		...
		/**
		 * Creates a new JobDetail with the specified name and job class. Inserts the
		 * process bundle into the JobDetail's jobDataMap for retrieval when the job is
		 * executed.
		 * 
		 * @param name     The name of the JobDetail
		 * @param bundle   The Openbravo process bundle.
		 * @param jobClass The class to be executed when Job.execute is called.
		 * @throws SchedulerException
		 */
		private static JobDetail newInstance(String name, ProcessBundle bundle, Class<? extends Job> jobClass)
				throws SchedulerException {
			if (bundle == null) {
				throw new SchedulerException("Process bundle cannot be null.");
			}
	...
	private static class TriggerProvider {
		...
		/**
		 * Loads the trigger details from AD_PROCESS_REQUEST and converts them into a
		 * schedulable Quartz Trigger instance.
		 */
		private static Trigger newInstance(String name, ProcessBundle bundle, ConnectionProvider conn)
				...
				if ("".equals(timingOption) || timingOption.equals(TIMING_OPTION_IMMEDIATE)) {
					...
					if (data.frequency.equals(FREQUENCY_SECONDLY)) {
						...
						if (data.hourlyRepetitions.trim().equals("")) {
							...
							triggerBuilder = newTrigger().withSchedule(SimpleScheduleBuilder.repeatHourlyForTotalCount(
						...
						triggerBuilder = newTrigger().startNow()
								.withSchedule(cronSchedule(sb.toString()).withMisfireHandlingInstructionDoNothing());
					...
					final String msg = Utility.messageBD(conn, "TRIG_INVALID_DATA", bundle.getContext().getLanguage());
			...
			log.debug("Scheduled process {}. Start time:{}.", data.processName + " " + data.processGroupName,
		...
		private static Calendar timestamp(String date, String time) throws ParseException {
			...
			if (time != null && !time.equals("")) {
				...
				final int minute = Integer.parseInt(time.substring(time.indexOf(':') + 1, time.lastIndexOf(':')));
				final int second = Integer.parseInt(time.substring(time.lastIndexOf(':') + 1, time.length()));
