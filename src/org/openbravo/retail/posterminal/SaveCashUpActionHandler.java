...
public class SaveCashUpActionHandler extends BaseActionHandler {
  private static final Logger log = Logger.getLogger(SaveCashUpActionHandler.class);
    ...
    try {
      ...
      for (int i = 0; i < errorIds.length(); i++) {
        try {
          ...
          JSONObject jsonCashUp = new JSONObject(error.getJsoninfo());
          posTerminalId = error.getObposApplications().getId();
          ProcessCashClose loader = WeldUtils
    ...
    if (errorb) {
      ...
      try {
        result.put("message", Utility.messageBD(new DalConnectionProvider(false),
            "OBPOS_ErrorWhileSavingCashUp", RequestContext.get().getVariablesSecureApp()
                .getLanguage()));
      ...
      try {
        result.put("message", Utility.messageBD(new DalConnectionProvider(false),
            "OBPOS_CashUpSavedSuccessfully", RequestContext.get().getVariablesSecureApp()
                .getLanguage()));
      } catch (JSONException e) {
        // won't happen
      }
