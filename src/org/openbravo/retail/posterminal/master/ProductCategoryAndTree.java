...
public class ProductCategoryAndTree extends ProcessHQLQuery {
  ...
  public static final Logger log = Logger.getLogger(ProductCategoryAndTree.class);
  ...
  protected List<String> getQuery(JSONObject jsonsent) throws JSONException {
    ...
    if (isRemote) {
      hqlQueries
          .add("select"
              + regularProductsCategoriesAndTreesHQLProperties.getHqlSelect() //
              + "from OBRETCO_Productcategory aCat left outer join aCat.productCategory as pCat left outer join pCat.image as img, ADTreeNode as tn"
              + " where ( aCat.obretcoProductlist.id = :productListId ) and tn.node = aCat.productCategory.id and tn.tree.table.id = :productCategoryTableId"
              + " and aCat.$incrementalUpdateCriteria and aCat.ative = true and aCat.$naturalOrgCriteria and aCat.$readableSimpleClientCriteria "
              + " and tn.$incrementalUpdateCriteria and tn.active = true and tn.$naturalOrgCriteria and tn.$readableSimpleClientCriteria "
      ...
      hqlQueries
          .add("select"
              + regularProductsCategoriesAndTreesHQLProperties.getHqlSelect() //
              + "from ProductCategory as pCat left outer join pCat.image as img, ADTreeNode as tn "
              + " where (exists("
              ...
              + "AND tn.node = pCat.id and tn.tree.table.id = :productCategoryTableId "
              + "AND (pplv.id= :priceListVersionId) AND ("
              ...
              + "AND tn.$incrementalUpdateCriteria AND tn.$naturalOrgCriteria AND tn.$readableSimpleClientCriteria "
              + "order by pCat.name, pCat.id");
    }
    String promoNameTrl;
    if (OBContext.hasTranslationInstalled()) {
    ...
    hqlQueries.add("select pt.id as id, " + promoNameTrl + " as searchKey, " + promoNameTrl
        + " as name, " + promoNameTrl + " as _identifier, '' as parentId, '' as seqNo, "
