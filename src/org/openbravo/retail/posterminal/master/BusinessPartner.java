...
public class BusinessPartner extends ProcessHQLQuery {
  ...
  protected List<String> getQuery(JSONObject jsonsent) throws JSONException {
    ...
    String hql = "SELECT " + regularBusinessPartnerHQLProperties.getHqlSelect() //
        ...
        + "left outer join bpl.businessPartner.priceList AS plist " + "WHERE $filtersCriteria AND "
        ...
        + " and bpl.id in (select max(bpls.id) as bpLocId from BusinessPartnerLocation AS bpls where bpls.businessPartner.id=bpl.businessPartner.id and bpls.$readableSimpleClientCriteria AND "
    ...
    String hql2 = "SELECT" + regularBusinessPartnerHQLProperties.getHqlSelect() //
        ...
        + "(bpl.$incrementalUpdateCriteria" + operator
        + "bpl.businessPartner.$incrementalUpdateCriteria) "
        + " and bpl.id in (select max(bpls.id) as bpLocId from BusinessPartnerLocation AS bpls where bpls.active=true and bpls.businessPartner.id=bpl.businessPartner.id and bpls.$readableSimpleClientCriteria AND "
        + " bpls.$naturalOrgCriteria group by bpls.businessPartner.id)"
