...
public class RptC_Bpartner extends HttpSecureAppServlet {
    ...
    public void doPost(HttpServletRequest request, HttpServletResponse response)
        ...
        if (vars.commandIn("DEFAULT")) {
            String strcBpartnerId = vars
                    .getSessionValue("RptC_Bpartner.inpcBpartnerId_R");
            /*
             * String strcBpartnerId =
             * vars.getStringParameter("inpcBpartnerId");
             */
            if (strcBpartnerId.equals(""))
                strcBpartnerId = vars
                        .getSessionValue("RptC_Bpartner.inpcBpartnerId");
            printPageDataSheet(response, vars, strcBpartnerId);
        } else if (vars.commandIn("OPEN")) {
            String strcBpartnerId = vars
                    .getRequiredStringParameter("inpcBpartnerId");
            String strmProductTemplate = vars
                    .getRequiredStringParameter("inpProductTemplate");
            printPageAjaxResponse(response, vars, strcBpartnerId,
                    strmProductTemplate);
        } else if (vars.commandIn("OPENDOCUMENT")) {
            String strcBpartnerId = vars
                    .getRequiredStringParameter("inpcBpartnerId");
            String strmTypeDocument = vars
                    .getRequiredStringParameter("inpTypeDocument");
            printPageAjaxDocumentResponse(response, vars, strcBpartnerId,
                    strmTypeDocument);
        } else
            pageError(response);
    }
    ...
    void printPageDataSheet(HttpServletResponse response,
            VariablesSecureApp vars, String strcBpartnerId) throws IOException,
            ServletException {
        ...
        RptCBpartnerData[] dataPartner = RptCBpartnerData.select(this, vars
                .getLanguage(), strcBpartnerId);
        RptCBpartnerData[] dataAccount = RptCBpartnerData.selectAccount(this,
                strcBpartnerId);
        RptCBpartnerData[] dataShipper = RptCBpartnerData.selectShipper(this,
                strcBpartnerId);
        RptCBpartnerData[] dataTemplate = RptCBpartnerData.selectTemplate(this,
                vars.getLanguage(), strcBpartnerId);
        RptCBpartnerData[] dataDiscount = RptCBpartnerData.selectDiscount(this,
                strcBpartnerId);
        RptCBpartnerCustomerData[] dataCustomer = RptCBpartnerCustomerData
                .select(this, vars.getLanguage(), strcBpartnerId);
        RptCBpartnerVendorData[] dataVendor = RptCBpartnerVendorData.select(
                this, vars.getLanguage(), strcBpartnerId);
        RptCBpartnerlocationData[] dataLocation = RptCBpartnerlocationData
                .select(this, strcBpartnerId);
        RptCBpartnercontactData[] dataContact = RptCBpartnercontactData.select(
                this, strcBpartnerId);
        RptCBpartnerSalesData[] dataSales = RptCBpartnerSalesData.selectOrder(
                this, strcBpartnerId);
        RptCBpartnerSalesData[] dataInvoice = RptCBpartnerSalesData.select(
                this, strcBpartnerId);
        RptCBpartnerSalesData[] dataInout = RptCBpartnerSalesData.selectinout(
                this, strcBpartnerId);
        RptCBpartnerSalesData[] dataABC = RptCBpartnerSalesData.selectABC(this,
                DateTimeData.sysdateYear(this), DateTimeData.lastYear(this),
                strcBpartnerId);
        ...
        xmlDocument = xmlEngine.readXmlTemplate(
                "org/openbravo/erpReports/RptC_Bpartner", discard)
                .createXmlDocument();
    ...
    void printPageAjaxResponse(HttpServletResponse response,
            VariablesSecureApp vars, String strcBpartnerId,
            String strmProductTemplate) throws IOException, ServletException {
        if (log4j.isDebugEnabled())
            log4j.debug("Output: ajaxreponse");
        XmlDocument xmlDocument = null;

        RptCBpartnerData[] data = RptCBpartnerData.selectTemplateDetail(this,
                strcBpartnerId, strmProductTemplate);

        if (data == null || data.length == 0)
            data = RptCBpartnerData.set();

        xmlDocument = xmlEngine.readXmlTemplate(
                "org/openbravo/erpReports/RptC_BpartnerTemplateLines")
                .createXmlDocument();
    ...
    void printPageAjaxDocumentResponse(HttpServletResponse response,
            VariablesSecureApp vars, String strcBpartnerId,
            String strmTypeDocument) throws IOException, ServletException {
        if (log4j.isDebugEnabled())
            log4j.debug("Output: ajaxreponse");
        XmlDocument xmlDocument = null;

        RptCBpartnerSalesData[] data = null;
        RptCBpartnerSalesData[] dataPeriod = RptCBpartnerSalesData
                .selectperiod(this);

        if (strmTypeDocument.equals("INVOICE")) {
            data = RptCBpartnerSalesData.selectInvoiceperiod(this,
                    strcBpartnerId);
            xmlDocument = xmlEngine.readXmlTemplate(
                    "org/openbravo/erpReports/RptC_BpartnerPeriodInvoice")
                    .createXmlDocument();
        ...
        if (strmTypeDocument.equals("ORDER")) {
            data = RptCBpartnerSalesData
                    .selectOrderperiod(this, strcBpartnerId);
            xmlDocument = xmlEngine.readXmlTemplate(
                    "org/openbravo/erpReports/RptC_BpartnerPeriodSales")
                    .createXmlDocument();
            xmlDocument.setData("structurePeriod", dataPeriod);
            if (data == null || data.length == 0)
                data = RptCBpartnerSalesData.set();
        ...
        if (strmTypeDocument.equals("INOUT")) {
            data = RptCBpartnerSalesData
                    .selectInoutperiod(this, strcBpartnerId);
            xmlDocument = xmlEngine.readXmlTemplate(
                    "org/openbravo/erpReports/RptC_BpartnerPeriodInout")
                    .createXmlDocument();
            xmlDocument.setData("structurePeriod", dataPeriod);
            if (data == null || data.length == 0)
                data = RptCBpartnerSalesData.set();
        ...
        if (strmTypeDocument.equals("ABC")) {
            data = RptCBpartnerSalesData.selectABCactualdetail(this,
                    DateTimeData.sysdateYear(this), strcBpartnerId);
            xmlDocument = xmlEngine.readXmlTemplate(
                    "org/openbravo/erpReports/RptC_BpartnerABC")
                    .createXmlDocument();
        }
        if (strmTypeDocument.equals("ABCREF")) {
            data = RptCBpartnerSalesData.selectABCrefdetail(this, DateTimeData
                    .lastYear(this), strcBpartnerId);
            xmlDocument = xmlEngine.readXmlTemplate(
                    "org/openbravo/erpReports/RptC_BpartnerABCref")
                    .createXmlDocument();
        }
