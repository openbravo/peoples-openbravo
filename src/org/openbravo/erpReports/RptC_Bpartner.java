...
public class RptC_Bpartner extends HttpSecureAppServlet {
  ...
  public void doPost (HttpServletRequest request, HttpServletResponse response) throws IOException,ServletException {
    ...
    /*String strcBpartnerId = vars.getStringParameter("inpcBpartnerId");*/
      if (strcBpartnerId.equals("")) strcBpartnerId = vars.getSessionValue("RptC_Bpartner.inpcBpartnerId");
  ...
  void printPageDataSheet(HttpServletResponse response, VariablesSecureApp vars, String strcBpartnerId)
    ...
    RptCBpartnerSalesData[] dataABC = RptCBpartnerSalesData.selectABC(this, DateTimeData.sysdateYear(this), DateTimeData.lastYear(this), strcBpartnerId);
  ...
  void printPageAjaxResponse(HttpServletResponse response, VariablesSecureApp vars, String strcBpartnerId, String strmProductTemplate)
    throws IOException, ServletException {
    ...
    if (data == null || data.length == 0) data = RptCBpartnerData.set();

    xmlDocument = xmlEngine.readXmlTemplate("org/openbravo/erpReports/RptC_BpartnerTemplateLines").createXmlDocument();
	...
	void printPageAjaxDocumentResponse(HttpServletResponse response, VariablesSecureApp vars, String strcBpartnerId, String strmTypeDocument)
    throws IOException, ServletException {
    if (log4j.isDebugEnabled()) log4j.debug("Output: ajaxreponse");
    XmlDocument xmlDocument=null;
    ...
    if (strmTypeDocument.equals("INVOICE")) {
        ...
        xmlDocument = xmlEngine.readXmlTemplate("org/openbravo/erpReports/RptC_BpartnerPeriodInvoice").createXmlDocument();
		xmlDocument.setData("structurePeriod", dataPeriod);
		if (data == null || data.length == 0) data = RptCBpartnerSalesData.set();
    }
    if (strmTypeDocument.equals("ORDER")) {
        data = RptCBpartnerSalesData.selectOrderperiod(this, strcBpartnerId);
        xmlDocument = xmlEngine.readXmlTemplate("org/openbravo/erpReports/RptC_BpartnerPeriodSales").createXmlDocument();
		xmlDocument.setData("structurePeriod", dataPeriod);
		if (data == null || data.length == 0) data = RptCBpartnerSalesData.set();
    }
    if (strmTypeDocument.equals("INOUT")) {
        data = RptCBpartnerSalesData.selectInoutperiod(this, strcBpartnerId);
        xmlDocument = xmlEngine.readXmlTemplate("org/openbravo/erpReports/RptC_BpartnerPeriodInout").createXmlDocument();
		xmlDocument.setData("structurePeriod", dataPeriod);
		if (data == null || data.length == 0) data = RptCBpartnerSalesData.set();
    }
	if (strmTypeDocument.equals("ABC")) {
        data = RptCBpartnerSalesData.selectABCactualdetail(this, DateTimeData.sysdateYear(this), strcBpartnerId);
        xmlDocument = xmlEngine.readXmlTemplate("org/openbravo/erpReports/RptC_BpartnerABC").createXmlDocument();
    }
	if (strmTypeDocument.equals("ABCREF")) {
        ...
        xmlDocument = xmlEngine.readXmlTemplate("org/openbravo/erpReports/RptC_BpartnerABCref").createXmlDocument();
    }
