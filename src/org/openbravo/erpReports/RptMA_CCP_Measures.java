...
public class RptMA_CCP_Measures extends HttpSecureAppServlet {
    ...
    public void doPost(HttpServletRequest request, HttpServletResponse response)
        ...
        if (vars.commandIn("DEFAULT")) {
            String strmaMeasureShift = vars
                    .getSessionValue("RptMA_CCP_Measures.inpmaMeasureShift_R");
            if (strmaMeasureShift.equals(""))
                strmaMeasureShift = vars
                        .getSessionValue("RptMA_CCP_Measures.inpmaMeasureShiftId");
    ...
    void printPagePartePDF(HttpServletResponse response,
            VariablesSecureApp vars, String strmaMeasureShift)
            throws IOException, ServletException {
        ...
        RptMACCPMeasuresData[] data = RptMACCPMeasuresData.select(this,
                strmaMeasureShift);
        ...
        for (int i = 0; i < data.length; i++) {
            dataHours[i] = RptMACCPMeasuresHoursData.select(this,
                    data[i].groupid);
            if (dataHours[i] == null || dataHours[i].length == 0)
                dataHours[i] = new RptMACCPMeasuresHoursData[0];

            dataValues[i] = RptMACCPMeasuresValuesData.select(this,
                    data[i].groupid);
