...
public class RptMA_CCP_Measures extends HttpSecureAppServlet {
  ...
  public void doPost (HttpServletRequest request, HttpServletResponse response) throws IOException,ServletException {
    ...
    if (vars.commandIn("DEFAULT")) {
      ...
      if (strmaMeasureShift.equals("")) strmaMeasureShift = vars.getSessionValue("RptMA_CCP_Measures.inpmaMeasureShiftId");
      printPagePartePDF(response, vars, strmaMeasureShift);
    } else pageError(response);
  }


   void printPagePartePDF(HttpServletResponse response, VariablesSecureApp vars, String strmaMeasureShift) throws IOException,ServletException{
    if (log4j.isDebugEnabled()) log4j.debug("Output: pdf");
    XmlDocument xmlDocument = xmlEngine.readXmlTemplate("org/openbravo/erpReports/RptMA_CCP_Measures").createXmlDocument();
    ...
    if (data == null || data.length == 0) data = RptMACCPMeasuresData.set();
    ...
    for (int i=0; i<data.length; i++){
      ...
      if (dataHours[i] == null || dataHours[i].length == 0) dataHours[i] = new RptMACCPMeasuresHoursData[0];
      
      dataValues[i] = RptMACCPMeasuresValuesData.select(this, data[i].groupid);
      if (dataValues[i] == null || dataValues[i].length == 0) dataValues[i] = new RptMACCPMeasuresValuesData[0];
    }
