...
public class DatabaseValidator implements SystemValidator {
  ...
  private void checkTablesWithoutPrimaryKey(org.apache.ddlutils.model.Table[] dbTables,
      ...
      if (!property.isPrimitive() && !property.isOneToMany() && !property.isAuditInfo()) {
        ...
        if (!found) {
          final String fk = "ALTER TABLE "
              ...
              + " ADD CONSTRAINT "
              + (property.getEntity().getTableName() + "_" + property.getTargetEntity()
                  .getTableName()).toLowerCase() + " FOREIGN KEY ("
              + property.getColumnName().toLowerCase() + ")" + " REFERENCES "
              + property.getTargetEntity().getTableName().toLowerCase() + " ("
              + property.getReferencedProperty().getColumnName().toLowerCase() + ") MATCH SIMPLE"
