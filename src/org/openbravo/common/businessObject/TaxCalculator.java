...
public class TaxCalculator {
  ...
  public BigDecimal calculateTax(Currency ccy, BigDecimal amount, boolean isInclusiveOfTax,
    ...
    taxAmtsMap.clear();
    if (isInclusiveOfTax) {
      taxDivider = getTaxAmount(new BigDecimal(1), taxRate, isSale);
      ...
      taxAmt = getTaxAmount(amount, taxRate, isSale);
    return taxAmt;

  }

  private BigDecimal getTaxAmount(BigDecimal amount, TaxRate rate, boolean isSale) {
    ...
    if (rate.isSummaryLevel()) {
      ...
      for (TaxRate childRate : rateList) {
        amt = amt.add(getTaxAmount(amount, childRate, isSale));
      }
    } else {
