...
public class OrderLinePEHQLInjector extends HqlInserter {
  ...
  public String insertHql(Map<String, String> requestParameters,
    ...
    if (isSalesTransaction) {
      ...
      hql.append("   select 1 from OrderLine l left join l.procurementPOInvoiceMatchList as m ");
      hql.append("   where (m.id is null or m.invoiceLine.id is not null) ");
      hql.append("     and l.salesOrder.id = o.id");
      hql.append("   group by l.orderedQuantity, l.id");
      hql.append("   having (l.orderedQuantity - sum(coalesce(m.quantity,0)) <> 0)");
      hql.append(" )");
    }
