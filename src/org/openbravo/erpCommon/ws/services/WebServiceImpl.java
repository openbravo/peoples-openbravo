...
public class WebServiceImpl implements WebService {
    ...
    private boolean access(String username, String password) {
        try {
            return !WebServicesData.hasAccess(pool, username, password).equals(
                    "0");
        } catch (Exception e) {
            return false;
        }
    ...
    public Customer[] getCustomers(String clientId, String username,
        ...
        try {
            ...
            WebServicesCustomerData[] data = WebServicesCustomerData.select(
                    pool, clientId, username);
    ...
    public Customer getCustomer(String clientId, String customerId,
        ...
        try {
            ...
            WebServicesCustomerData[] data = WebServicesCustomerData
                    .selectCustomerById(pool, clientId, customerId, username);
    ...
    public Customer getCustomer(String clientId, String name, String searchKey,
        ...
        try {
            ...
            WebServicesCustomerData[] data = WebServicesCustomerData
                    .selectCustomer(pool, clientId, username,
                            (name == null ? "" : name), (searchKey == null ? ""
                                    : searchKey));
    ...
    public Boolean updateCustomer(BusinessPartner customer, String username,
        ...
        try {
            updated = WebServicesCustomerData.updateCustomer(pool, customer
                    .getName(), customer.getSearchKey(), String
                    .valueOf(customer.getClientId()), String.valueOf(customer
                    .getId()), username);
        } catch (Exception e) {
            log4j.error(e.getMessage());
        } finally {
    ...
    public int[] getCustomerAddresses(String clientId, String customerId,
        ...
        try {
            WebServicesAddressData[] data = WebServicesAddressData
                    .selectLocationList(pool, clientId, customerId);
            locationList = new int[data.length];
            for (int i = 0; i < data.length; i++) {
                locationList[i] = Integer.valueOf(data[i].cLocationId)
                        .intValue();
            }
        } catch (Exception e) {
            log4j.error(e.getMessage());
    ...
    public Location getCustomerLocation(String clientId, String customerId,
        ...
        try {
            WebServicesAddressData[] data = WebServicesAddressData.select(pool,
                    clientId, customerId, locationId);
            if (data.length > 0) {
    ...
    public Boolean updateAddress(Location addr, String username, String password) {
        int updated = 0;
        try {
            updated = WebServicesAddressData.updateAddress(pool, addr
                    .getAddress1(), addr.getAddress2(), addr.getCity(), addr
                    .getPostal(), String.valueOf(addr.getClientId()), String
                    .valueOf(addr.getId()));
        } catch (Exception e) {
            log4j.error(e.getMessage());
        } finally {
    ...
    public Contact getCustomerContact(String clientId, String customerId,
        ...
        try {
            WebServicesContactData[] data = WebServicesContactData.select(pool,
                    clientId, customerId, contactId);
            if (data.length > 0) {
    ...
    public Boolean updateContact(Contact contact, String username,
        ...
        int updated = 0;
        try {
            updated = WebServicesContactData.updateContact(pool, contact
                    .getFirstName(), contact.getLastName(), contact.getEmail(),
                    contact.getPhone(), contact.getPhone2(), contact.getFax(),
                    String.valueOf(contact.getClientId()), String
                            .valueOf(contact.getBusinessPartnerId()), String
