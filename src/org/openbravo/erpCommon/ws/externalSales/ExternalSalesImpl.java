...
public class ExternalSalesImpl implements ExternalSales {
    ...
    private boolean access(String username, String password) {
        try {
            return !ExternalSalesOrderData.access(pool, username, password)
                    .equals("0");
        } catch (Exception e) {
            return false;
        }
    ...
    public Product[] getProductsCatalog(String ClientID, String organizationId,
        ...
        if (log4j.isDebugEnabled()) {
            log4j.debug("getProductsCatalog" + " ClientID " + ClientID
                    + " organizationId " + organizationId + " salesChannel "
                    + salesChannel);
        }
        ...
        try {
            ...
            // Select
            ExternalSalesProductData[] data = ExternalSalesProductData.select(
                    pool, ClientID, salesChannel, organizationId);
            ...
            if (data != null && data.length > 0) {
                ...
                while (i < data.length) {
                    if (log4j.isDebugEnabled()) {
                        log4j.debug("getProductsCatalog data[i].id "
    ...
    public ProductPlus[] getProductsPlusCatalog(String ClientID,
    ...
    public boolean uploadOrders(String ClientID, String organizationId,
        ...
        try {
            ...
            if (log4j.isDebugEnabled())
                log4j.debug("uploadOrders" + " ClientID " + ClientID
                        + "organizationId " + organizationId + " sales channel"
                        + salesChannel + " order 1 "
                        + newOrders[0].getOrderId());
            initPool();

            // Reading default parameters
            ExternalSalesData[] externalPOS = ExternalSalesData.select(pool,
                    ClientID, organizationId, salesChannel);
            ...
            while (newOrders != null && i < newOrders.length) {
                ...
                if (newOrders[i].getOrderId() != null) {
                    data[0].orderReferenceno = newOrders[i].getOrderId()
                            .getDocumentNo();
                    SimpleDateFormat dateFormat = new SimpleDateFormat(
                            dateFormatStr);
                    data[0].dateordered = ""
                            + dateFormat.format(newOrders[i].getOrderId()
                                    .getDateNew());
                    data[0].dateTimeFormat = dateTimeFormatStr;
                }
                if (newOrders[i].getBusinessPartner() != null) {
                    data[0].bpartnervalue = newOrders[i].getBusinessPartner()
                            .getId();
                    data[0].name = newOrders[i].getBusinessPartner().getName();
                    data[0].countrycode = newOrders[i].getBusinessPartner()
                            .getCountry();
                    data[0].regionname = newOrders[i].getBusinessPartner()
                            .getRegion();
                    data[0].city = newOrders[i].getBusinessPartner().getCity();
                    data[0].postal = newOrders[i].getBusinessPartner()
                            .getPostal();
                    data[0].address1 = newOrders[i].getBusinessPartner()
                            .getAddress1();
                    data[0].address2 = newOrders[i].getBusinessPartner()
                            .getAddress2();
                }
                if ((data[0].bpartnervalue == null || data[0].bpartnervalue
                        .equals(""))
                ...
                if (newOrders[i].getLines() != null) {
                    // Insert lines
                    while (newOrders[i].getLines() != null
                        ...
                        if (log4j.isDebugEnabled())
                            log4j.debug("sequence" + data[0].iOrderId
                                    + " data[0].paymentamount1"
                                    + data[0].paymentamount1
                                    + " data[0].paymentrule1"
                                    + data[0].paymentrule1
                                    + " data[0].paymentamount2"
                                    + data[0].paymentamount2
                                    + " data[0].paymentrule2"
                                    + data[0].paymentrule2);
                        data[0].insert(pool);
    ...
    public Order[] getOrders(String ClientID, String organizationId,
        ...
        while (orderIds != null && cont < orderIds.length) {
            if (log4j.isDebugEnabled())
                log4j.debug("ClientID "
                        ...
                        + " - "
                        + dateFormat.format(orderIds[cont].getDateNew())
                                .toString());

            try {
                // Select

                ExternalSalesOrderData[] data = ExternalSalesOrderData.select(
                        pool, ClientID, dateFormat.format(
                                orderIds[cont].getDateNew()).toString(),
                        orderIds[cont].getDocumentNo());
                if ((data == null) || (data.length == 0))
                    data = ExternalSalesOrderData
                            .selectIOrder(pool, ClientID, dateFormat.format(
                                    orderIds[cont].getDateNew()).toString(),
                                    orderIds[cont].getDocumentNo());
                ...
                if (data != null && data.length > 0) {
                    ...
                    while (i < data.length) {
                        ...
                        orderIdentifier.setDocumentNo(orderIds[cont]
                                .getDocumentNo());
                        ...
                        if ((data[i].id != null) && (!data[i].id.equals(""))) {
                            dataLines = ExternalSalesOrderData.selectLines(
                                    pool, ClientID, data[i].id);
                        } else {
                            if (log4j.isDebugEnabled())
                                log4j.debug("ClientID "
                                        + ClientID
                                        ...
                                        + " - "
                                        + dateFormat.format(
                                                orderIds[cont].getDateNew())
                                                .toString());
                            dataLines = ExternalSalesOrderData
                                    .selectLinesIOrder(
                                            pool,
                                            ClientID,
                                            dateFormat
                        ...
                        if (dataLines != null && dataLines.length > 0) {
                            if (log4j.isDebugEnabled())
                                log4j.debug("getOrders dataLines.length "
                                        + dataLines.length);
                            ...
                            while (j < dataLines.length) {
                                ...
                                orderLine
                                        .setOrderLineId((!dataLines[j].orderLineId
                        ...
                        order.setState((Integer.valueOf(data[i].status))
                                .intValue());
                        BPartner bpartner = new org.openbravo.erpCommon.ws.externalSales.BPartner();
                        if (log4j.isDebugEnabled())
                            log4j.debug("data[i].bpartnervalue "
                                    + data[i].bpartnervalue
                                    + " data[i].cBpartnerName "
                                    + data[i].cBpartnerName);
                        ...
                        if ((data[i].id != null) && (!data[i].id.equals("")))
                            dataPayments = ExternalSalesOrderData
                                    .selectPayment(pool, ClientID, data[i].id);
                        else {
                            if (log4j.isDebugEnabled())
                                log4j.debug("ClientID "
                                        + ClientID
                        ...
                        if (dataPayments != null && dataPayments.length > 0) {
                            if (log4j.isDebugEnabled())
                                log4j.debug("getOrders dataPayments.length "
                                        + dataPayments.length);
