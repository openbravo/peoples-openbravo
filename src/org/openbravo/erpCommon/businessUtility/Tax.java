...
/*System.out.println("billC_BPartner_Location_ID: " + billC_BPartner_Location_ID);
    ...
    String billTo = td[0].billtocLocationId;
    String IsTaxExempt = (IsSOTrx?td[0].istaxexempt:"N");
    String IsSpecialTax = (IsSOTrx?td[0].isspecialtax:"N");
    String taxID="";
    if (IsTaxExempt.equalsIgnoreCase("Y")) {
      TaxData[] tdAux = TaxData.taxId(conn, AD_Org_ID);
      if (tdAux!=null && tdAux.length >0) taxID = tdAux[0].cTaxId;
    } else {
      String taxCategory = TaxData.taxCategory(conn, M_Product_ID);
      String shipFrom = TaxData.shipLocation(conn, M_Warehouse_ID);
      String shipTo = TaxData.shipTo(conn, shipC_BPartner_Location_ID);
System.out.println("taxCategory: " + taxCategory);
System.out.println("shipFrom: " + shipFrom);
System.out.println("shipTo: " + shipTo);
      if (!IsSOTrx) {
        String strAux = billTo;
        billTo = billFrom;
        billFrom = strAux;
        strAux = shipTo;
        shipTo = shipFrom;
...
System.out.println("IsSpecialTax: " + IsSpecialTax);
      TaxData[] tdAux = TaxData.select(conn, taxCategory, billFrom, shipTo, shipDate, IsSpecialTax);
      if (tdAux==null || tdAux.length==0) tdAux = TaxData.select2(conn, taxCategory, billFrom, shipDate, IsSpecialTax);
      if (tdAux==null || tdAux.length==0) tdAux = TaxData.select3(conn, billFrom, shipDate);
      if (tdAux!=null && tdAux.length!=0) taxID = tdAux[0].cTaxId;
    }
  ...
  public static String validateRoutingNo (String routingNo){
    ...
    if(longitud == 8 || longitud == 9){return "";}
    else{
...
public static String validateCreditCardNumber (String creditCardNumber, String creditCardType){
   ...
   do{
     if(!st.hasMoreTokens() || ccLengthOK) break;
   ...
   do{
     if(!st.hasMoreTokens() || ccIdentified) break;
