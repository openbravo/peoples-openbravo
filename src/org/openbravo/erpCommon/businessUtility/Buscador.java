...
public class Buscador extends HttpSecureAppServlet {
    ...
    public void doPost(HttpServletRequest request, HttpServletResponse response)
        ...
        if (vars.commandIn("DEFAULT")) {
            ...
            String strIsSOTrx = vars.getSessionValue(strWindowId
                    + "|issotrxtab");
            String strShowAudit = Utility.getContext(this, vars, "ShowAudit",
                    strWindowId);
            BuscadorData[] data;

            if (!BuscadorData.hasSelectionColumns(this, strTab).equals("0"))
                data = BuscadorData.select(this, vars.getLanguage(), strTab,
                        strShowAudit);
            else
                data = BuscadorData.selectIdentifiers(this, vars.getLanguage(),
                        strTab, strShowAudit);
            ...
            if (data == null || data.length == 0) {
                ...
                if (loadParameters(vars, data, strTab)) {
                    for (int i = 0; i < data.length; i++) {
                        if (data[i].reference.equals("10")
                                || data[i].reference.equals("14")
    ...
    BuscadorData[] removeParents(BuscadorData[] data, String strTab)
    ...
    boolean loadParameters(VariablesSecureApp vars, BuscadorData[] data,
            String strTab) throws ServletException {
        if (data == null || data.length == 0)
            return false;
    ...
    void printPage(HttpServletResponse response, VariablesSecureApp vars,
            String strTab, BuscadorData[] data, String strWindow,
            String strWindowId, String strIsSOTrx) throws IOException,
            ServletException {
        if (log4j.isDebugEnabled())
            log4j.debug("Output: Frame 1 of the attributes seeker");
        XmlDocument xmlDocument = xmlEngine.readXmlTemplate(
                "org/openbravo/erpCommon/businessUtility/Buscador")
                .createXmlDocument();
        ...
        xmlDocument.setParameter("data", generateHtml(vars, data, strTab,
                strWindowId, script, strIsSOTrx, vecScript));
        xmlDocument.setParameter("scripsJS", vecScript.elementAt(0).toString());
        xmlDocument.setParameter("script", (script.toString() + generateScript(
                data, strWindow, strTab)));
    ...
    String generateScript(BuscadorData[] fields, String strWindow, String strTab) {
        ...
        if (fields != null && fields.length != 0 && isHighVolume) {
            ...
            for (int i = 0; i < fields.length; i++) {
                ...
                paramsData.append("paramsData[count++] = new Array(\"inpParam")
                        .append(FormatUtilities.replace(fields[i].columnname))
                        .append("\" , ");
                params.append(", \"inpParam").append(
                ...
                if (fields[i].reference.equals("20")) {
                    paramsData.append("((radioValue(frm.inpParam").append(
                            FormatUtilities.replace(fields[i].columnname))
                            .append(")!=null)?");
                    paramsData.append("radioValue(frm.inpParam").append(
                            FormatUtilities.replace(fields[i].columnname))
                            .append("):");
                    ...
                    params.append("radioValue(frm.inpParam").append(
                            ...
                            .append("):");
                    params.append("\"\")");
                } else if (fields[i].reference.equals("17")
                        || fields[i].reference.equals("18")
                        || fields[i].reference.equals("19")) { // Select
                    paramsData.append("((frm.inpParam").append(
                            FormatUtilities.replace(fields[i].columnname))
                    ...
                    paramsData.append("\"\"));\n");
                    params.append("((frm.inpParam").append(
                            FormatUtilities.replace(fields[i].columnname))
                            .append(".selectedIndex!=-1)?");
                    params.append("frm.inpParam").append(
                            FormatUtilities.replace(fields[i].columnname))
                    ...
                    paramsData.append("frm.inpParam").append(
                            ...
                            .append(".value").append(");\n");
                    paramsData.append(
                            "paramsData[count++] = new Array(\"inpParam")
                            .append(
                                    FormatUtilities
                                            .replace(fields[i].columnname))
                            .append("_f\", ");
                    paramsData.append("frm.inpParam").append(
                            FormatUtilities.replace(fields[i].columnname))
                            .append("_f.value);\n");
                ...
                if (fields[i].reference.equals("17")
                        || fields[i].reference.equals("18")
                        || fields[i].reference.equals("19")) { // Select
                    strHtml.append("frm.inpParam").append(
                            FormatUtilities.replace(fields[i].columnname))
                            .append(".selectedIndex!=-1");
                } else if (Utility.isDecimalNumber(fields[i].reference)
                        || Utility.isIntegerNumber(fields[i].reference)
        ...
        strHtml.append("\n").append(paramsData);
        strHtml
                .append("  if (window.opener.selectFilters) window.opener.selectFilters(paramsData);\n");
        strHtml
                .append("  else window.opener.submitFormGetParams(\"SEARCH\", \"../"
                        + strWindow + "\"" + params.toString() + ");\n");
    ...
    String generateHtml(VariablesSecureApp vars, BuscadorData[] fields,
            String strTab, String strWindow, StringBuffer script,
            String strIsSOTrx, Vector<StringBuffer> vecScript)
            throws IOException, ServletException {
        ...
        for (int i = 0; i < fields.length; i++) {
            ...
            strHtml
                    .append("<tr><td class=\"TitleCell\"> <span class=\"LabelText\">");
            ...
            if (fields[i].reference.equals("17")
                ...
                strHtml.append("name=\"inpParam").append(
                        FormatUtilities.replace(fields[i].columnname)).append(
                        "\" ");
                ...
                try {
                    ComboTableData comboTableData = new ComboTableData(vars,
                            this, fields[i].reference, fields[i].columnname,
                            fields[i].referencevalue, fields[i].adValRuleId,
                            Utility.getContext(this, vars, "#User_Org",
                                    strWindow), Utility.getContext(this, vars,
                                    "#User_Client", strWindow), 0);
                    Utility.fillSQLParameters(this, vars, null, comboTableData,
                            strWindow, fields[i].value);
                    ...
                    for (int j = 0; j < data.length; j++) {
                        ...
                        strHtml.append("\" ");
                        if (data[j].getField("ID").equalsIgnoreCase(
                                fields[i].value))
                ...
                strHtml.append("</select>\n");
            } else if (fields[i].reference.equals("15")
                    || fields[i].reference.equals("16")) { // DATE || Date-time
                scriptCalendar = true;
                ...
                strHtml.append("displayFormat=\"").append(
                        vars.getSessionValue("#AD_SqlDateFormat"))
                        .append("\" ");
                strHtml.append("saveFormat=\"").append(
                        vars.getSessionValue("#AD_SqlDateFormat"))
                        .append("\" ");
                strHtml.append("name=\"inpParam").append(
                        FormatUtilities.replace(fields[i].columnname)).append(
                        "\" ");
                strHtml.append("maxlength=\"").append(
                        vars.getSessionValue("#AD_SqlDateFormat").length())
                        .append("\" ");
                strHtml.append("value=\"").append(fields[i].value)
                        .append("\" ");
                strHtml.append("id=\"inpParam").append(
                        FormatUtilities.replace(fields[i].columnname)).append(
                        "\" ");
                strHtml
                        .append("onkeyup=\"auto_complete_date(this.textbox);\"></input> ");
                strHtml.append("<script>djConfig.searchIds.push(\"").append(
                        "inpParam").append(
                        FormatUtilities.replace(fields[i].columnname)).append(
                        "\") </script>");
                strHtml.append("</td>\n");
                ...
                strHtml
                        .append("<td class=\"TextBox_btn_ContentCell\" colspan=\"3\">\n");
                strHtml
                        .append("<table border=\"0\" cellspacing=\"0\" cellpadding=\"0\" summary=\"\">\n");
                scriptSearch = true;
                strHtml.append("<tr>\n<td>\n");
                ...
                strHtml.append("value=\"").append(
                        (!fields[i].value.equals("") && !fields[i].value
                                .equals("%")) ? fields[i].value : "").append(
                        "\">");
                strHtml.append("</td>\n");
                strHtml.append("<td class=\"TextBox_ContentCell\">\n");
                if (Integer.valueOf(fields[i].fieldlength).intValue() < (MAX_TEXTBOX_LENGTH / 4)) {
                ...
                strHtml.append("id=\"inpParam").append(
                        FormatUtilities.replace(fields[i].columnname)).append(
                        "_DES\" ");
                strHtml.append("maxlength=\"").append(fields[i].fieldlength)
                        .append("\" ");

                if (!fields[i].value.equals("") && !fields[i].value.equals("%")) {
                    String strSearchTableName = BuscadorData
                            .selectSearchTableName(this,
                    ...
                    String strSearchName = BuscadorData.selectSearchName(this,
                            strSearchTableName, fields[i].value, vars
                                    .getLanguage());
                ...
                strHtml
                        .append((!fields[i].reference.equals("21") && !fields[i].reference
                                .equals("35")) ? "" : "readonly=\"true\" ");
                strHtml.append("><script>djConfig.searchIds.push(\"").append(
                        "inpParam").append(
                        FormatUtilities.replace(fields[i].columnname)).append(
                        "_DES\") </script></td>\n");
                ...
                if (fields[i].reference.equals("21")) {
                    ...
                    strMethod = searchsCommand(fields[i], false, strTab,
                            strWindow, strIsSOTrx);
                }

                strMethod = "new keyArrayItem(\"ENTER\", \"" + strMethod
                        + "\", \"inpParam"
                        + FormatUtilities.replace(fields[i].columnname)
                        + "_DES\", \"null\")";
                ...
                if (fields[i].reference.equals("21")) {
                    ...
                    strHtml.append(searchs(fields[i], strTab, strWindow,
                            strIsSOTrx));
                }
            } else if (Utility.isDecimalNumber(fields[i].reference)
                    || Utility.isIntegerNumber(fields[i].reference)) {
                ...
                strHtml
                        .append("class=\"dojoValidateValid TextBox_btn_OneCell_width\" ");
                strHtml.append("name=\"inpParam").append(
                        FormatUtilities.replace(fields[i].columnname)).append(
                        "\" ");
                strHtml.append("maxlength=\"").append(fields[i].fieldlength)
                ...
                if (Utility.isDecimalNumber(fields[i].reference)
                    ...
                    strHtml.append("inpParam").append(
                            FormatUtilities.replace(fields[i].columnname))
                            .append("', ");
                    strHtml.append("document.frmMain.inpParam").append(
                            FormatUtilities.replace(fields[i].columnname))
                            .append(".value, false);return false;\">\n");
                ...
                strHtml.append("<td class=\"TextBox_ContentCell\">");
                strHtml
                        .append("<input type=\"text\" class=\"dojoValidateValid TextBox_OneCell_width\" ");
                strHtml.append("name=\"inpParam").append(
                        FormatUtilities.replace(fields[i].columnname)).append(
                        "\" ");
                strHtml.append("maxlength=\"").append(fields[i].fieldlength)
                ...
                if (Utility.isDecimalNumber(fields[i].reference)) {
                    ...
                    scriptCalculator = true;
                    strHtml
                            .append("onkeydown=\"auto_complete_number(this, false, false);return true;\" ");
                }
                strHtml.append(">");
            ...
            if (Utility.isDecimalNumber(fields[i].reference)
                ...
                strHtml
                        .append("<td class=\"TitleCell\"> <span class=\"LabelText\">");
                strHtml.append(Utility
                        .messageBD(this, "To", vars.getLanguage()));
                strHtml.append("</span></td>\n");
                ...
                if (Utility.isDecimalNumber(fields[i].reference)
                    ...
                    strHtml.append("name=\"inpParam").append(
                            FormatUtilities.replace(fields[i].columnname))
                            .append("_f\" ");
                    strHtml.append("maxlength=\"")
                            .append(fields[i].fieldlength).append("\" ");
                    ...
                    strHtml.append("</td>");
                        ...
                        || fields[i].reference.equals("16")) { // DATE
                    strHtml.append("<td class=\"TextBox_btn_ContentCell\">\n");
                    strHtml
                            .append("<table border=\"0\" cellspacing=\"0\" cellpadding=\"0\" summary=\"\"  style=\"padding-top: 0px;\">\n");
                    strHtml.append("<tr>\n");
                    ...
                    strHtml
                            .append("onkeyup=\"auto_complete_date(this.textbox);\"></input> ");
                    strHtml.append("<script>djConfig.searchIds.push(\"")
                            .append("inpParam").append(
                                    FormatUtilities
                                            .replace(fields[i].columnname))
                            .append("_f\") </script>");
                    strHtml.append("</td>\n");
        ...
        vecKeys
                .addElement("new keyArrayItem(\"B\", \"aceptar()\", null, \"ctrlKey\")");
        vecKeys
                .addElement("new keyArrayItem(\"ESCAPE\", \"window.close()\", null, null)");
        if (vecKeys.size() > 0) {
            script.append("var keyArray = new Array();\n");
            script.append("function enableLocalShortcuts() {\n");
            script.append("\n");
            for (int i = 0; i < vecKeys.size(); i++) {
                script.append("keyArray[").append(i).append("] = ").append(
                        vecKeys.elementAt(i).toString()).append(";\n");
            }
        ...
        if (scriptKeyboard) {
            scrScr.append("<script language=\"JavaScript\" src=\"").append(
        ...
        if (scriptCalendar) {
            ...
            scrScr.append("<script language=\"JavaScript\" src=\"").append(
                    strReplaceWith).append("/js/jscalendar/lang/calendar-")
                    .append(vars.getLanguage().substring(0, 2)).append(
    ...
    public String locationCommands(BuscadorData efd) {
        ...
        html
                ...
                .append("', 'inpParam")
                .append(FormatUtilities.replace(efd.columnname))
                .append("_DES', document.frmMain.inpParam")
                .append(FormatUtilities.replace(efd.columnname))
                .append(
    ...
    public String location(BuscadorData efd) {
        ...
        html.append("<a href=\"#\"");
        html.append("onClick=\"").append(locationCommands(efd)).append(
                "return false;\" ");
        html
                .append(
                ...
                .append(efd.searchname.trim()).append("\" title=\"").append(
                        efd.searchname.trim()).append("\" ");
    ...
    public String searchsCommand(BuscadorData efd, boolean fromButton,
            String tabId, String windowId, String strIsSOTrx) {
        StringBuffer params = new StringBuffer();
        StringBuffer html = new StringBuffer();
        ...
        String searchName = (efd.reference.equals("25") ? "/info/Account"
                : ("/info/" + (efd.reference.equals("800011") ? "ProductComplete"
        ...
        if (efd.searchname.toUpperCase().startsWith("ATTRIBUTE")) {
            ...
            params.append(", document.frmMain.inpParam").append(
                    FormatUtilities.replace(efd.columnname)).append(".value");
            params.append(", 'inpwindowId'");
            params.append(", '").append(windowId).append("'");
            params.append(", 'inpProduct'");
            params.append(", document.frmMain.inpParam").append(
                    FormatUtilities.replace("M_Product_ID")).append(".value");
            params.append(", 'inpLocatorId'");
            params.append(", ((document.frmMain.inpParam").append(
                    FormatUtilities.replace("M_Locator_ID"));
            params.append("!=null)?document.frmMain.inpParam");
            params.append(FormatUtilities.replace("M_Locator_ID")).append(
                    ".value:'')");
        }
        html.append(strMethodName).append("(null, null, '..")
                .append(searchName).append(
                        "', null, false, 'frmMain', 'inpParam").append(
                        FormatUtilities.replace(efd.columnname)).append(
                        "', 'inpParam").append(
                        FormatUtilities.replace(efd.columnname)).append(
                        "_DES', document.frmMain.inpParam").append(
                        FormatUtilities.replace(efd.columnname)).append(
                        "_DES.value").append(params.toString()).append(");");
        return html.toString();
    }

    public String searchs(BuscadorData efd, String tabId, String windowId,
            String strIsSOTrx) {
        StringBuffer html = new StringBuffer();
        ...
        html.append("<td class=\"FieldButton_bg\"><a href=\"#\"");
        html.append("onClick=\"").append(
                searchsCommand(efd, true, tabId, windowId, strIsSOTrx)).append(
                "return false;\" ");
        html
                .append(
                        "onmouseout=\"this.className='FieldButton';window.status='';return true;\" onmouseover=\"this.className='FieldButton_hover';window.status='Search';return true;\" onmousedown=\"this.className='FieldButton_active';return true;\" onmouseup=\"this.className='FieldButton';return true;\">\n<img width=\"16\" height=\"16\" alt=\"")
    ...
    public String locatorCommands(BuscadorData efd, boolean fromButton,
            String windowId) {
        StringBuffer params = new StringBuffer();
        StringBuffer html = new StringBuffer();
        if (!fromButton) {
        ...
        html
                .append(
                        "openSearch(null, null, '../info/Locator.html', null, false, 'frmMain', 'inpParam")
    ...
    public String locator(BuscadorData efd, String windowId) {
        ...
        ;
        html.append("<img width=\"16\" height=\"16\" alt=\"").append(
                efd.searchname.trim()).append("\" title=\"").append(
                efd.searchname.trim()).append("\" ");
        html.append("class=\"FieldButton_Icon FieldButton_Icon_").append(
                FormatUtilities.replace(efd.searchname.trim())).append("\" ");
        ;
