...
public class TabFilter extends HttpSecureAppServlet {
      ...
      if (strOrderBy!=null && strOrderBy.length()>0) {
        strOrderBy = strOrderBy.substring(1, strOrderBy.length()-1);
        {
          ...
          while (st.hasMoreTokens()) {
            ...
            if (!strOrderBy.equals("")) strOrderBy += ",";
            if (token.startsWith("'")) strOrderBy += token.substring(1,token.length()-1).trim();
          }
        }
      }
      if (vars.commandIn("ORDERBY") && !strOrderBy.equals("")) vars.setSessionValue(strTab + "|orderby", strOrderBy);
      else vars.removeSessionValue(strTab + "|orderby");
      printPageClosePopUp(response, vars, strWindowPath);
    } else pageError(response);
  }


  void printPage(HttpServletResponse response, VariablesSecureApp vars, String strTab, String strWindow, String strWindowId, String strOrderBy) throws IOException, ServletException {
    ...
    if (vars.getLanguage().equals("en_US")) data = TabFilterData.select(this, strTab);
    else data = TabFilterData.selectTrl(this, vars.getLanguage(), strTab);
  ...
  private TabFilterData[] getShownFieldsData(TabFilterData[] data) {
    ...
    for (int i=0;i<data.length;i++) {
      if (mustBeCount(data[i].columnname)) {
        if (isHasDescriptionReference(data[i].adReferenceId) && data[i].isdisplayed.equals("Y")) contador++;
  ...
  private boolean mustBeCount(String columnname) {
    if (!columnname.equalsIgnoreCase("Created") && !columnname.equalsIgnoreCase("CreatedBy") && !columnname.equalsIgnoreCase("Updated") && !columnname.equalsIgnoreCase("UpdatedBy")) return true;
    else return false;
  }

  private TabFilterData[] getOrderByFields(TabFilterData[] data, String strOrderBy) {
    strOrderBy = strOrderBy.trim();
    if (strOrderBy.startsWith("(")) strOrderBy = strOrderBy.substring(1, strOrderBy.length()-1);
    if (log4j.isDebugEnabled()) log4j.debug("TabFilter - getOrderByFields() - String to parse: " + strOrderBy);
    ...
    while (orderBy.hasMoreTokens()) {
      ...
      if (log4j.isDebugEnabled()) log4j.debug("TabFilter - getOrderByFields() - token to parse: " + token + " - real token: " + realToken);
      for (int i=0;i<data.length;i++) {
  ...
  private TabFilterData[] getShownFieldsDataWithoutOrderBy(TabFilterData[] data, TabFilterData[] dataOrderBy) {
    Vector<Object> fields = new Vector<Object>();
    ...
    for (int i=0;i<data.length;i++) {
      ...
      if (!exists) fields.addElement(data[i]);
    }
    TabFilterData[] result = new TabFilterData[fields.size()];
    fields.copyInto(result);
    return result;
