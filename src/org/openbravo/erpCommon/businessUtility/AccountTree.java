...
public class AccountTree {
    ...
    public AccountTree(VariablesSecureApp _vars, ConnectionProvider _conn,
            AccountTreeData[] _elements, AccountTreeData[] _accounts,
            String _elementValueParent) throws ServletException {
        ...
        if (resultantAccounts != null && resultantAccounts.length > 0) {
            AccountTreeData[] forms = AccountTreeData.selectForms(conn, Utility
                    .getContext(conn, vars, "#User_Org", "AccountTree"),
                    Utility.getContext(conn, vars, "#User_Client",
                            "AccountTree"));
            resultantAccounts = calculateTree(forms, elementValueParent,
                    new Vector<Object>());
        }
    }
     ...
     *            example, first expenses then revenues) Objective tree.
     * @var forms: Array of accounts with its operands.
     * @throws ServletException
     */
    public AccountTree(VariablesSecureApp _vars, ConnectionProvider _conn,
        ...
        if (resultantAccounts != null && resultantAccounts.length > 0) {
            ...
            for (int i = 0; i < elementValueParent.length; i++) {
                r = calculateTree(forms, elementValueParent[i],
                        new Vector<Object>());
     ...
     */
    private AccountTreeData setDataQty(AccountTreeData element,
     ...
     * constructor to initializa the element's quantities. Also initializes the
     * level of each account
     * 
     * @param indice
     ...
     */
    private AccountTreeData[] updateTreeQuantitiesSign(String indice,
            int level, String isDebitCredit) {
        ...
        for (int i = 0; i < elements.length; i++) {
            if (elements[i].parentId.equals(indice)) {
                ...
                AccountTreeData[] dataChilds = updateTreeQuantitiesSign(
                        elements[i].nodeId, (level + 1), isDebitCredit);
     ...
     */
    private void formsCalculate(Vector<Object> vecAll, AccountTreeData[] forms,
            String indice, Vector<Object> vecTotal, boolean isExactValue) {
        if (isExactValue) {
            recursiveForms = true;
        } else {
        ...
        for (int i = 0; i < forms.length; i++) {
            if (forms[i].id.equals(indice)) {
                ...
                /* (a) */for (int j = 0; j < vecAll.size(); j++) {
                    AccountTreeData actual = (AccountTreeData) vecAll
                            .elementAt(j);
                    log4j.debug("AccountTree.formsCalculate - actual.nodeId: "
                            + actual.nodeId + " - forms[i].nodeId: "
                            + forms[i].nodeId);
     ...
     */
    private AccountTreeData[] calculateTree(AccountTreeData[] forms,
            String[] indice, Vector<Object> vecTotal) {
     ...
     * @param isExactValue
     *            Boolean auxiliar to use only for the calls from the forms
     *            calculating.
     * @return Array with the new calculated tree.
     */
    private AccountTreeData[] calculateTree(AccountTreeData[] forms,
            String indice, Vector<Object> vecTotal, boolean applysign,
    ...
    private AccountTreeData[] calculateTree(AccountTreeData[] forms,
        ...
        Vector<Object> vec = new Vector<Object>();
        if (log4j.isDebugEnabled())
            log4j.debug("AccountTree.calculateTree() - accounts: "
                    + resultantAccounts.length);
        if (vecTotal == null)
            vecTotal = new Vector<Object>();
        if (vecTotal.size() == 0) {
            vecTotal.addElement("0");
        ...
        for (int i = 0; i < resultantAccounts.length; i++) {
            if ((isExactValue && nodeIn(resultantAccounts[i].nodeId, indice))
                    || (!isExactValue && nodeIn(resultantAccounts[i].parentId,
                            indice))) { // modified by Eduardo Argal. For
                                                                 ...
                                                                 // why????
                {
                    ...
                    dataChilds = calculateTree(forms,
    ...
    /**
     * Method to make the level filter of the tree, to eliminate the levels that
     * shouldn't be shown in the report.
     * 
     * @param indice
     ...
     */
    private AccountTreeData[] levelFilter(String[] indice, boolean found,
            String strLevel) {
        if (resultantAccounts == null || resultantAccounts.length == 0
                || strLevel == null || strLevel.equals(""))
            return resultantAccounts;
        AccountTreeData[] result = null;
        Vector<Object> vec = new Vector<Object>();
        if (log4j.isDebugEnabled())
            log4j.debug("AccountTree.levelFilter() - accounts: "
                    + resultantAccounts.length);
        ...
        for (int i = 0; i < resultantAccounts.length; i++) {
            ...
            if (nodeIn(resultantAccounts[i].parentId, indice)
                    && (!found || resultantAccounts[i].elementlevel
                            .equalsIgnoreCase(strLevel))) {
                AccountTreeData[] dataChilds = levelFilter(
                        resultantAccounts[i].nodeId,
                        (found || resultantAccounts[i].elementlevel
                                .equals(strLevel)), strLevel);
                vec.addElement(resultantAccounts[i]);
                if (dataChilds != null && dataChilds.length > 0)
                    for (int j = 0; j < dataChilds.length; j++)
                        vec.addElement(dataChilds[j]);
    ...
    public AccountTreeData[] filterStructure(String[] indice,
            boolean notEmptyLines, String strLevel, boolean isLevel) {
        if (log4j.isDebugEnabled())
            log4j.debug("AccountTree.filterStructure() - accounts: "
                    + resultantAccounts.length);
        if (resultantAccounts == null || resultantAccounts.length == 0)
            return resultantAccounts;
        AccountTreeData[] result = null;
    ...
    public void filter(boolean notEmptyLines, String strLevel, boolean isLevel) {
        ...
        resultantAccounts = filterStructure(elementValueParent, notEmptyLines,
                strLevel, isLevel);
    }

    /**
     * Resets amounts of subaccounts which parents have been reset because of
    ...
    public void filterSVC() {
        if (log4j.isDebugEnabled())
            log4j.debug("AccountTree.filterShowValueCond() - accounts: "
                    + resultantAccounts.length);
        if (resultantAccounts == null || resultantAccounts.length == 0)
            return;
        ...
        for (int i = 0; i < resultantAccounts.length; i++) {
            ...
            if (level == levels[1]
                    && resultantAccounts[i].svcresetref.equals("N")) {
