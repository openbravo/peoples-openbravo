...
public class TabAttachments extends HttpSecureAppServlet {
    ...
    public void doPost(HttpServletRequest request, HttpServletResponse response)
        ...
        if (vars.getCommand().startsWith("SAVE_NEW")) {
            ...
            final String strDataType = vars
                    .getStringParameter("inpadDatatypeId");
            final TabAttachmentsData[] data = TabAttachmentsData.selectTabInfo(
                    this, strTab);
            String tableId = "";
            ...
            final OBError oberrInsert = insert(vars, strFileReference, tableId,
                    key, strDataType, strText);
            if (!oberrInsert.getType().equals("Success")) {
                ...
                if (vars.commandIn("SAVE_NEW_RELATION")) {
                    response
                            .sendRedirect(strDireccion
                                    + request.getServletPath()
                                    + "?Command=DEFAULT&inpcFileId="
                                    + strFileReference);
                    ...
                    response.sendRedirect(strDireccion
                            + request.getServletPath() + "?Command=NEW");
                }
            }
            ...
            final String strDataType = vars
                    .getStringParameter("inpadDatatypeId");
            final String strText = vars.getStringParameter("inptext");
            if (TabAttachmentsData.update(this, vars.getUser(), strDataType,
                    strText, strFileReference) == 0) {
                ...
                if (vars.commandIn("SAVE_EDIT_RELATION")) {
                    ...
                    response.sendRedirect(strDireccion
                            + request.getServletPath() + "?Command=NEW&inpKey="
                            + key);
                } else if (vars.commandIn("SAVE_EDIT_NEXT")) {
                    final TabAttachmentsData[] data = TabAttachmentsData
                            .selectTabInfo(this, strTab);
                    String tableId = "";
                    if (data == null || data.length == 0)
                        throw new ServletException("Tab not found: " + strTab);
                    else {
                    ...
                    final String strNewFile = TabAttachmentsData.selectNext(
                            this, Utility.getContext(this, vars,
                                    "#User_Client", strWindow), Utility
                                    .getContext(this, vars, "#User_Org",
                                            strWindow), strFileReference,
                            tableId, key);
            ...
            final String key = vars.getStringParameter("inpKey");
            vars.setSessionValue("TabAttachments.key", key);
            final String strFileReference = vars
                    .getStringParameter("inpcFileId");
            ...
            final String strTab = vars.getGlobalVariable("inpTabId",
                    "TabAttachments.tabId");
            final String strWindow = vars.getGlobalVariable("inpwindowId",
                    "TabAttachments.windowId");
            final String key = vars.getGlobalVariable("inpKey",
                    "TabAttachments.key");
            final boolean editable = vars.getGlobalVariable("inpEditable",
                    "TabAttachments.editable").equals("Y");
            ...
            final String key = vars.getGlobalVariable("inpKey",
                    "TabAttachments.key");
            final String strFileReference = vars
                    .getRequiredStringParameter("inpcFileId");
            ...
            final String strWindow = vars.getGlobalVariable("inpwindowId",
                    "TabAttachments.windowId");
            final String key = vars.getRequestGlobalVariable("inpKey",
                    "TabAttachments.key");
            printPageEdit(response, vars, strTab, strWindow, key, "");
        } else if (vars.commandIn("DISPLAY_DATA")) {
            final String strFileReference = vars
    ...
    OBError insert(VariablesSecureApp vars, String strFileReference,
            String tableId, String key, String strDataType, String strText)
            throws IOException, ServletException {
        ...
        try {
            ...
            TabAttachmentsData.insert(conn, this, strFileReference, vars
                    .getClient(), vars.getOrg(), vars.getUser(), tableId, key,
                    strDataType, strText, strName);
            try {
                // FIXME: Get the directory separator from Java runtime
                final File uploadedDir = new File(
                        globalParameters.strFTPDirectory + "/" + tableId + "-"
                                + key);
    ...
    OBError delete(VariablesSecureApp vars, String strFileReference)
            throws IOException, ServletException {
        OBError myMessage = null;
        myMessage = new OBError();
        myMessage.setTitle("");
    ...
    void printPageFS(HttpServletResponse response, VariablesSecureApp vars)
        ...
        final XmlDocument xmlDocument = xmlEngine.readXmlTemplate(
                "org/openbravo/erpCommon/businessUtility/TabAttachments_FS")
                .createXmlDocument();
    ...
    void printPage(HttpServletResponse response, VariablesSecureApp vars,
            String strTab, String strWindow, String key, boolean editable)
            throws IOException, ServletException {
        if (log4j.isDebugEnabled())
        ...
        if ((files == null) || (files.length == 0))
            discard[0] = "widthData";
        final XmlDocument xmlDocument = xmlEngine.readXmlTemplate(
                "org/openbravo/erpCommon/businessUtility/TabAttachments_F1",
        ...
        xmlDocument.setParameter("recordIdentifier", TabAttachmentsData
                .selectRecordIdentifier(this, key, vars.getLanguage(), strTab));
    ...
    void printPageEdit(HttpServletResponse response, VariablesSecureApp vars,
            String strTab, String strWindow, String key, String strFileReference)
            throws IOException, ServletException {
        if (log4j.isDebugEnabled())
        ...
        final XmlDocument xmlDocument = xmlEngine
                .readXmlTemplate(
                        "org/openbravo/erpCommon/businessUtility/TabAttachments_Edition",
                        discard).createXmlDocument();
        ...
        xmlDocument.setParameter("window", strWindow);
        xmlDocument.setParameter("key", key);
        xmlDocument.setParameter("save", (strFileReference.equals("") ? "NEW"
                : "EDIT"));
        xmlDocument.setParameter("recordIdentifier", TabAttachmentsData
                .selectRecordIdentifier(this, key, vars.getLanguage(), strTab));
        ...
        /*
         * String message = vars.getSessionValue("TabAttachments.message"); if
         * (!message.equals("")) message = "alert('" + message + "');";
         * xmlDocument.setParameter("body", message);
         */

        final TabAttachmentsData[] files = TabAttachmentsData.selectEdit(this,
                strFileReference);
        // FIXME: If we do not use this code, it should be removed
        /*
         * String viewButtons = "yes"; if
         * (strFileReference.equals("")||files==null||files.length==0)
         * viewButtons="none";
         * 
        ...
        xmlDocument
                .setData("structure1",
                        (strFileReference.equals("") ? TabAttachmentsData.set()
                                : files));
        xmlDocument.setData("reportAD_Datatype_ID_D", "liststructure",
                DataTypeComboData.select(this, Utility.getContext(this, vars,
                        "#User_Client", "TabAttachments"), Utility.getContext(
                        this, vars, "#User_Org", "TabAttachments")));
    ...
    void printPageFile(HttpServletResponse response, VariablesSecureApp vars,
        ...
        // FIXME: Get the directory separator from Java runtime
        final File file = new File(globalParameters.strFTPDirectory + "/"
                + data[0].adTableId + "-" + data[0].adRecordId, data[0].name);
        if (file.exists())
            f = new FileUtility(globalParameters.strFTPDirectory + "/"
                    + data[0].adTableId + "-" + data[0].adRecordId,
                    data[0].name, false, true);
        else
            f = new FileUtility(globalParameters.strFTPDirectory,
                    strFileReference, false, true);
    ...
    void printPageCheck(HttpServletResponse response, VariablesSecureApp vars,
            String strTab, String recordId) throws IOException,
            ServletException {
