...
public class EMail {
    ...
    public EMail(VariablesSecureApp vars, String smtpHost, String from,
            String to) {
        g_from = from;
        addTo(to);
        g_smtpHost = smtpHost;
    ...
    /*
     * never used private void dumpMessage() { if (g_smtpMsg == null) return;
     * try { Enumeration<?> e = g_smtpMsg.getAllHeaderLines (); while
     * (e.hasMoreElements ()) if (log4j.isDebugEnabled()) log4j.debug("- " +
     * e.nextElement ()); } catch (MessagingException ex) { if
    ...
    public String send() {
        ...
        try {
            ...
            g_smtpMsg.setNotifyOptions(SMTPMessage.NOTIFY_FAILURE
                    | SMTPMessage.NOTIFY_SUCCESS);
            ...
            if (log4j.isDebugEnabled())
                log4j.debug("send() - username and password set, username: "
    ...
    private void setContent() throws MessagingException, IOException {
        ...
        if (g_attachments == null || g_attachments.size() == 0) {
            ...
            if (g_messageHTML == null || g_messageHTML.length() == 0)
                ...
                mbp_1.setDataHandler(new DataHandler(new ByteArrayDataSource(
                        g_messageHTML, "text/html")));

            // Create Multipart and its parts to it
            Multipart mp = new MimeMultipart();
            mp.addBodyPart(mbp_1);
            if (log4j.isDebugEnabled())
                log4j.debug("setContent (multi) " + getSubject() + " - "
                        + mbp_1);
            ...
            for (int i = 0; i < g_attachments.size(); i++) {
                ...
                if (attachment instanceof File) {
                    ...
                    if (file.exists())
                        ...
                        log4j
                                .error("setContent - File does not exist: "
                                        + file);
                        continue;
                    }
                    ...
                    log4j.error("setContent - Attachement type unknown: "
                            + attachment);
                    continue;
                }
                // Attachment Part
                ...
                if (log4j.isDebugEnabled())
                    log4j.debug("setContent - Added Attachment " + ds.getName()
                            + " - " + mbp_2);
    ...
    public void addAttachment(byte[] data, String type, String name) {
        ByteArrayDataSource byteArray = new ByteArrayDataSource(data, type)
                .setName(name);
