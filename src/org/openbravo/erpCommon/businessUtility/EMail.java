...
public class EMail {
  ...
  public EMail(VariablesSecureApp vars, String smtpHost, String from, String to, String subject, String message) {
    g_from=from;
    addTo(to);
    g_subject=subject;
    g_message=message;
  ...
  public void setEMailUser(String username, String password) {
    if(username == null || password == null) log4j.warn("setEMailUser ignored - " + username + "/" + password);
    else {
      g_auth = new EMailAuthenticator(username, password);
      if (g_auth==null) g_valid = false;
    }
  }

  private void setEMailUser() {
    if(g_auth != null) return;
    String from = g_from;
    ...
    if(from.equals(email) && usr.length() > 0 && pwd.length() > 0) setEMailUser(usr, pwd);
  }
  ...
  private void setContent () throws MessagingException, IOException {
    ...
    if (g_attachments == null || g_attachments.size() == 0) {
      if (g_messageHTML == null || g_messageHTML.length () == 0) g_smtpMsg.setContent(getMessageCRLF(), "text/plain");
      else g_smtpMsg.setDataHandler(new DataHandler(new ByteArrayDataSource (g_messageHTML, "text/html")));
      
      if (log4j.isDebugEnabled()) log4j.debug("setContent(simple) " + getSubject());
    } else { // Multi part message
      //  First Part - Message
      MimeBodyPart mbp_1 = new MimeBodyPart();
      mbp_1.setText("");
      if (g_messageHTML == null || g_messageHTML.length () == 0) mbp_1.setContent (getMessageCRLF(), "text/plain");
      else mbp_1.setDataHandler (new DataHandler(new ByteArrayDataSource (g_messageHTML, "text/html")));

      // Create Multipart and its parts to it
      Multipart mp = new MimeMultipart();
      mp.addBodyPart(mbp_1);
      if (log4j.isDebugEnabled()) log4j.debug("setContent (multi) " + getSubject() + " - " + mbp_1);
      ...
      for (int i = 0; i < g_attachments.size(); i++) {
        ...
        if (attachment instanceof File) {
          ...
          if (file.exists()) ds = new FileDataSource (file);
          else {
        ...
        } else if (attachment instanceof DataSource) ds = (DataSource)attachment;
        else {
        ...
        if (log4j.isDebugEnabled()) log4j.debug("setContent - Added Attachment " + ds.getName() + " - " + mbp_2);
  ...
  public void setMessageHTML (String html) {
    if (html == null || html.length() == 0) g_valid = false;
    else {
      g_messageHTML = html;
  ...
  public boolean addTo (String newTo) {
    ...
    try {
      ...
      if (g_to == null) g_to = new ArrayList<InternetAddress>();
      g_to.add(ia);
    } catch (Exception e) {
  ...
  public InternetAddress getTo() {
    if (g_to == null || g_to.size() == 0) return null;
  ...
  public boolean addCc (String newCc) {
    if (newCc == null || newCc.length() == 0) return false;
    ...
    if (g_cc == null) g_cc = new ArrayList<InternetAddress>();
    g_cc.add (ia);
    return true;
  }

  public InternetAddress[] getCcs() {
    if (g_cc == null || g_cc.size() == 0) return null;
  ...
  public boolean addBcc (String newBcc) {
    if (newBcc == null || newBcc.length() == 0) return false;
    InternetAddress ia = null;
    try {
    ...
    if (g_bcc == null) g_bcc = new ArrayList<InternetAddress>();
    g_bcc.add (ia);
    return true;
  }

  public InternetAddress[] getBccs() {
    if (g_bcc == null || g_bcc.size() == 0) return null;
  ...
  public boolean setReplyTo (String newTo) {
    if (newTo == null || newTo.length() == 0) return false;
    InternetAddress ia = null;
    try {
  ...
  public void setSubject(String newSubject) {
    if (newSubject == null || newSubject.length() == 0) g_valid = false;
    else g_subject = newSubject;
  }
  ...
  public void setMessage(String newMessage) {
    if (newMessage == null || newMessage.length() == 0) g_valid = false;
    else {
  ...
  public String getMessageCRLF() {
    ...
    for (int i = 0; i < chars.length; i++) {
      ...
      if (c == '\n') {
        int previous = i-1;
        if (previous >= 0 && chars[previous] == '\r') sb.append(c);
        else sb.append("\r\n");
      } else sb.append(c);
    }
    return sb.toString();
  ...
  public void setSmtpHost(String newSmtpHost) {
    if (newSmtpHost == null || newSmtpHost.length() == 0) g_valid = false;
    else g_smtpHost = newSmtpHost;
  }
  ...
  public void addAttachment (File file) {
    if (file == null) return;
    if (g_attachments == null) g_attachments = new ArrayList<Object>();
