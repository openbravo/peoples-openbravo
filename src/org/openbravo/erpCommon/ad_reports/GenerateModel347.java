...
public class GenerateModel347 extends HttpSecureAppServlet {
    ...
    public void doPost(HttpServletRequest request, HttpServletResponse response)
        ...
        if (vars.commandIn("DEFAULT")) {
            ...
            String strmProductId = vars.getRequestInGlobalVariable(
                    "inpmProductId_IN", "GenerateModel347|product");
            printPageGenerate(response, vars, strDateFrom, strDateTo, strType,
                    strComplementar, strOrg, strHac, strID, strmProductId);
        } else
            pageError(response);
    }

    void printPageDataSheet(HttpServletResponse response,
            VariablesSecureApp vars, String strType, String strDateFrom,
            String strDateTo, String strmProductId) throws IOException,
            ServletException {
        ...
        ToolBar toolbar = new ToolBar(this, vars.getLanguage(),
                "GenerateModel347", false, "", "", "", false, "ad_reports",
        ...
        try {
            ...
            NavigationBar nav = new NavigationBar(this, vars.getLanguage(),
                    "GenerateModel347.html", classInfo.id, classInfo.type,
        ...
        xmlDocument.setData("reportMProductId_IN", "liststructure",
                GenerateModel347Data.selectMproduct(this, Utility.getContext(
                        this, vars, "#AccessibleOrgTree", ""), Utility
    ...
    void printPageGenerate(HttpServletResponse response,
            VariablesSecureApp vars, String strDateFrom, String strDateTo,
            String strType, String strComplementar, String strOrg,
            String strHac, String strID, String strmProductId)
            throws IOException, ServletException {
        if (log4j.isDebugEnabled())
            log4j.debug("Output: pageFind");
        GenerateModel347Data[] data = new GenerateModel347Data[0];
        if (strHac.equals("ES"))
            data = GenerateModel347Data.selectAEAT(this, strDateFrom
                    .substring(strDateFrom.length() - 4), strID, strType,
                    strComplementar, strDateFrom, DateTimeData.nDaysAfter(this,
                            strDateTo, "1"), Tree.getMembers(this, TreeData
                            .getTreeOrg(this, vars.getClient()), strOrg));
        else if (strHac.equals("NA"))
            data = GenerateModel347Data.select(this, strType, strComplementar,
        ...
        if (data.length > 0) {
            ...
            response.setHeader("Content-Disposition",
                    "attachment; filename=MODEL347.DAT");
            PrintWriter out = response.getWriter();
            ...
            if (strHac.equals("ES"))
                dataLines = GenerateModel347Data.selectAEATType2(this,
                        strDateFrom.substring(strDateFrom.length() - 4),
                        strDateFrom, DateTimeData.nDaysAfter(this, strDateTo,
                                "1"), Tree.getMembers(this, TreeData
                                .getTreeOrg(this, vars.getClient()), strOrg));
            else if (strHac.equals("NA"))
                dataLines = GenerateModel347Data.selectNavarraType2(this,
                        strDateFrom, DateTimeData.nDaysAfter(this, strDateTo,
                                "1"), Tree.getMembers(this, TreeData
                                .getTreeOrg(this, vars.getClient()), strOrg));

            GenerateModel347Data[] products = GenerateModel347Data
                    .selectMproduct(this, Utility.getContext(this, vars,
                            "#AccessibleOrgTree", ""), Utility.getContext(this,
                            vars, "#User_Client", ""), strmProductId);
            String productList = "";
            for (int i = 0; i < products.length; i++) {
                productList += "'" + products[i].id + "',";
            }
            productList = productList.substring(0, Math.max(0, productList
                    .length() - 1));
            ...
            for (int i = 0; i < dataLines.length; i++) {
                strLinea = dataLines[i].constant1 + dataLines[i].model
                        + dataLines[i].ejercicio + dataLines[i].nifDeclarante
                        + dataLines[i].nifDeclarado
                        + dataLines[i].nifRepresentante
                        + dataLines[i].nombreSocial;
                if (strHac.equals("ES")) {
                    arrendamientos = GenerateModel347Data.selectLeaseProducts(
                            this, (productList == "") ? "null" : productList,
                            strDateFrom, DateTimeData.nDaysAfter(this,
                                    strDateTo, "1"), Tree
                                    .getMembers(this, TreeData.getTreeOrg(this,
                                            vars.getClient()), strOrg),
                            dataLines[i].cBpartnerId);
                    if (arrendamientos.length > 0) {
                        strLineaArrendamientos = strLinea;
                        BigDecimal inmuebles = new BigDecimal(
                                arrendamientos[0].importesMetalicoEInmuebles);
                        numArrendamientos = numArrendamientos
                                .add(BigDecimal.ONE);
                        ...
                        strLineaArrendamientos = strLineaArrendamientos
                                + "I"
                                + "                       "
                                + new String("000000000000000").substring(0,
                                        15 - inmuebles.toString().length())
                                + inmuebles.toString();
                        if (arrendamientos[0].description != null)
                            strLineaArrendamientos = strLineaArrendamientos
                                    + arrendamientos[0].description
                                            .substring(
                                                    0,
                                                    Math
                                                            .min(
                                    ...
                                    + new String(
                                            ...
                                            .substring(
                                                    ...
                                                    Math
                                                            .max(
                                                                    0,
                                                                    146 - arrendamientos[0].description
                                                                            .length()));
                        else
                            strLineaArrendamientos = strLineaArrendamientos
                                    + "                                                                                                                                                  ";
                    ...
                    strLinea = strLinea + dataLines[i].tipoDeclaracion
                            + dataLines[i].codigoProvincia
                            + dataLines[i].codigoPais + dataLines[i].espacio
                            + dataLines[i].claveCodigo + dataLines[i].importe
                            + dataLines[i].operacionSeguro
                            + dataLines[i].arrendamiento
                            + dataLines[i].importesMetalicoEInmuebles
                            + dataLines[i].blancos
                            + dataLines[i].nifRepresentanteLegal
                            + dataLines[i].masBlancos;
                } else if (strHac.equals("NA"))
                    strLinea = strLinea + dataLines[i].importe
                            + dataLines[i].operacionSeguro
            ...
            strCabecera = data[0].constant1
                    ...
                    + data[0].importe
                    + new String("000000000").substring(0,
                            9 - numArrendamientos.toString().length())
                    + numArrendamientos.toString()
                    + new String("000000000000000").substring(0,
                            15 - totalArrendamientos.toString().length())
                    + totalArrendamientos.toString() + data[0].blancos
                    + data[0].nifRepresentanteLegal + data[0].masBlancos;
