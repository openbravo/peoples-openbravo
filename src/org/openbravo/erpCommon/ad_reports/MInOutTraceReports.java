...
public class MInOutTraceReports extends HttpSecureAppServlet {
  ...
  public void doPost (HttpServletRequest request, HttpServletResponse response) throws IOException,ServletException {
    ...
    if (log4j.isDebugEnabled()) log4j.debug("MInOutTraceReports doPost, commandIn: " + vars.getCommand());
  ...
  private MInOutTraceReportsData[] processData(VariablesSecureApp vars, MInOutTraceReportsData[] data, String strIn) throws ServletException {
    ...
    for (int i=0;i<data.length;i++) {
      data[i].html = processChilds(vars, data[i].mAttributesetinstanceId, data[i].mProductId, data[i].mLocatorId, strIn, true);
    }
    return data;
  }

  private String insertTabHtml(boolean border) {
    return "    <td width=\"20px\" class=\"DataGrid_Body_Cell\""+ (border?" style=\"border-bottom: 0px !important;\"":" style=\"border-top: 0px !important;\"") +">&nbsp;</td>\n";
  }

  private String insertHeaderHtml(boolean isClose, String border) {
    if (!isClose) return "<table border=\"" + border + "\" cellspacing=0 cellpadding=0 width=\"100%\" >\n";
    else return "</table>\n";
  }
  ...
  private String processChilds(VariablesSecureApp vars, String mAttributesetinstanceId, String mProductId, String mLocatorId, String strIn, boolean colorbg2) throws ServletException {
  ...
  private String processExternalChilds(VariablesSecureApp vars, MInOutTraceReportsData dataChild, String strIn, boolean colorbg) throws ServletException {
    StringBuffer strHtml = new StringBuffer();
    ...
    if (dataChild.movementtype.startsWith("P") && movementQty > 0) {
      ...
      if (dataProduction!=null && dataProduction.length>0) {
        ...
        for (int j=0;j<dataProduction.length;j++) {
          strHtml.append("  <tr style=\"background: ").append((colorbg?"#CCCCCC":"#AAAAAA")).append("\">\n");
          ...
          strHtml.append("    <td class=\"DataGrid_Body_Cell\" width=\"100\">").append(dataProduction[j].movementtypeName).append("</td>\n");
          strHtml.append("    <td class=\"DataGrid_Body_Cell\" width=\"100\">").append(dataProduction[j].locatorName).append("</td>\n");
          strHtml.append("    <td class=\"DataGrid_Body_Cell\" width=\"90\">").append(dataProduction[j].movementqty).append("&nbsp;").append(dataProduction[j].uomName).append("</td>\n");
          strHtml.append("    <td class=\"DataGrid_Body_Cell\" width=\"90\">").append(dataProduction[j].quantityorder).append("&nbsp;").append(dataProduction[j].productUomName).append("</td>\n");
          resultado2 = dataProduction[j].productName;
          strHtml.append("    <td class=\"DataGrid_Body_Cell\"><a href=\"#\" onclick=\"submitCommandForm('INVERSE', true, null, 'MInOutTraceReports.html?inpmProductId2=" +dataProduction[j].mProductId+"&inpmAttributeSetInstanceId2="+dataProduction[j].mAttributesetinstanceId+"&inpIn2="+(strIn.equals("Y")?"N":"Y")+ "', '_self');return true;\" class=\"LabelLink\">");
          ...
          if (!dataProduction[j].mAttributesetinstanceId.equals("0")) {
            ...
            if (isnull == null){
              String strPartial = processChilds(vars, dataProduction[j].mAttributesetinstanceId, dataProduction[j].mProductId, dataProduction[j].mLocatorId, strIn, !colorbg);
    ...
    if (dataChild.movementtype.startsWith("M") && movementQty > 0) {
      ...
      if (dataMovement!=null && dataMovement.length>0) {
        ...
        for (int j=0;j<dataMovement.length;j++) {
          ...
          strHtml.append("    <td class=\"DataGrid_Body_Cell\" width=\"90\">").append(dataMovement[j].movementqty).append("&nbsp;").append(dataMovement[j].uomName).append("</td>\n");
          ...
          if (!dataMovement[j].mAttributesetinstanceId.equals("0")) {
            ...
            if (!dataMovement[j].mProductId.equals(strmProductIdGlobal)) {
              ...
              if (calculated.get(strCalculate) == null){
                strPartial = processChilds(vars, dataMovement[j].mAttributesetinstanceId, dataMovement[j].mProductId, dataMovement[j].mLocatorId, strIn, !colorbg);
              }
            }
            if (!strPartial.equals("")) {
              strHtml.append("  <tr style=\"background: ").append((colorbg?"#CCCCCC":"#AAAAAA")).append("\">\n");
  ...
  private String getData(MInOutTraceReportsData data, String strClassName) throws ServletException {
    ...
    resultado.append("  <tr>\n");
    resultado.append("    <td class=\"DataGrid_Body_Cell\" width=\"70\">").append(data.movementdate).append("</td>\n");
    resultado.append("    <td class=\"DataGrid_Body_Cell\" width=\"100\">").append(data.movementtypeName).append("</td>\n");
    resultado.append("    <td class=\"DataGrid_Body_Cell\" width=\"100\">").append(data.locatorName).append("</td>\n");
    resultado.append("    <td class=\"DataGrid_Body_Cell\" width=\"90\">").append(data.movementqty).append("&nbsp;").append(data.uomName).append("</td>\n");
    if (!data.quantityorder.equals("")) {
      resultado.append("    <td class=\"DataGrid_Body_Cell\" width=\"90\">").append(data.quantityorder).append("&nbsp;").append(data.productUomName).append("</td>\n");
    }
