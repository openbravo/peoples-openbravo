...
public class MInOutTraceReports extends HttpSecureAppServlet {
    ...
    public void doPost(HttpServletRequest request, HttpServletResponse response)
        ...
        if (vars.commandIn("DEFAULT")) {
            ...
            strmAttributesetinstanceIdGlobal = vars.getGlobalVariable(
    ...
    void printPageDataSheet(HttpServletResponse response,
        ...
        if (strmProductIdGlobal.equals("")) {
            ...
            data = MInOutTraceReportsData.select(this, vars.getLanguage(),
                    strmProductIdGlobal, strmAttributesetinstanceIdGlobal,
                    Utility.getContext(this, vars, "#User_Org",
                            "MInOutTraceReports"), Utility.getContext(this,
                            vars, "#User_Client", "MInOutTraceReports"));
        }
        ...
        xmlDocument.setData("reportM_ATTRIBUTESETINSTANCE_ID", "liststructure",
                AttributeSetInstanceComboData.select(this, vars.getLanguage(),
                        strmProductIdGlobal, Utility.getContext(this, vars,
                                "#User_Client", "MInOutTraceReports"), Utility
                                .getContext(this, vars, "#User_Org",
                                        "MInOutTraceReports")));
        xmlDocument.setParameter("productDescription", MInOutTraceReportsData
                .selectMproduct(this, strmProductIdGlobal));
        ...
        if (log4j.isDebugEnabled())
            log4j.debug("****FIN: "/*
                                    * + ((data!=null &&
                                    * data.length>0)?data[0].html:"")
                                    */);

        ToolBar toolbar = new ToolBar(this, vars.getLanguage(),
                "MInOutTraceReports", false, "", "", "", false, "ad_reports",
        ...
        try {
            ...
            NavigationBar nav = new NavigationBar(this, vars.getLanguage(),
                    "MInOutTraceReports.html", classInfo.id, classInfo.type,
                    strReplaceWith, tabs.breadcrumb());
            xmlDocument.setParameter("navigationBar", nav.toString());
            LeftTabsBar lBar = new LeftTabsBar(this, vars.getLanguage(),
                    "MInOutTraceReports.html", strReplaceWith);
    ...
    private String insertTotal(String strTotal, String strUnit,
            String strTotalPedido, String strUnitPedido) {
        BigDecimal total, totalPedido;
        total = new BigDecimal(strTotal);
        totalPedido = (!strTotalPedido.equals("") ? new BigDecimal(
                strTotalPedido) : ZERO);
        ...
        if (totalPedido.intValue() != 0) {
            resultado
                    .append("<td class=\"DataGrid_Body_Cell\" width=\"80px\">\n");
            resultado.append(totalPedido.toString()).append(" ").append(
                    strUnitPedido);
            resultado.append("</td>\n");
        }
    ...
    private String processChilds(VariablesSecureApp vars,
        ...
        MInOutTraceReportsData[] dataChild = MInOutTraceReportsData
                .selectChilds(this, vars.getLanguage(),
                        mAttributesetinstanceId, mProductId, mLocatorId, strIn
                                .equals("Y") ? "plusQty" : "minusQty", strIn
                                .equals("N") ? "minusQty" : "plusQty");
        ...
        for (int i = 0; i < dataChild.length; i++) {
            ...
            strHtml.append(processExternalChilds(vars, dataChild[i], strIn,
                    colorbg2, strmProductIdGlobal, calculated, count));
        }
    ...
    private String processExternalChilds(VariablesSecureApp vars,
        ...
        if (dataChild.movementtype.startsWith("P")
            ...
            if (log4j.isDebugEnabled())
                log4j.debug("****PROCESSING PRODUCTIONLINE: " + strNewId + " "
                        + strIn);
            if (strIn.equals("Y")) {
                dataProduction = MInOutTraceReportsData.selectProductionOut(
                        this, vars.getLanguage(), strNewId);
            } else {
                dataProduction = MInOutTraceReportsData.selectProductionIn(
                        this, vars.getLanguage(), strNewId);
            }
            if (dataProduction != null && dataProduction.length > 0) {
                ...
                for (int j = 0; j < dataProduction.length; j++) {
                    ...
                    strHtml
                            .append(
                                    "    <td class=\"DataGrid_Body_Cell\" width=\"70\">")
                            .append(dataProduction[j].movementdate).append(
                                    "</td>\n");
                    strHtml
                            .append(
                                    "    <td class=\"DataGrid_Body_Cell\" width=\"100\">")
                            .append(dataProduction[j].movementtypeName).append(
                                    "</td>\n");
                    strHtml
                            .append(
                                    "    <td class=\"DataGrid_Body_Cell\" width=\"100\">")
                            .append(dataProduction[j].locatorName).append(
                                    "</td>\n");
                    strHtml
                            .append(
                                    "    <td class=\"DataGrid_Body_Cell\" width=\"90\">")
                            .append(dataProduction[j].movementqty).append(
                                    "&nbsp;").append(dataProduction[j].uomName)
                            .append("</td>\n");
                    strHtml
                            .append(
                                    "    <td class=\"DataGrid_Body_Cell\" width=\"90\">")
                            .append(dataProduction[j].quantityorder).append(
                                    "&nbsp;").append(
                                    dataProduction[j].productUomName).append(
                                    "</td>\n");
                    ...
                    strHtml
                            .append("    <td class=\"DataGrid_Body_Cell\" width=\"120\">");
                    if (!resultado2.equals(""))
                        strHtml.append(resultado2);
                    strHtml.append("&nbsp;</td>\n");
                    ...
                    if (!dataProduction[j].mAttributesetinstanceId.equals("0")) {
                        String strCalculate = dataProduction[j].mProductId
                                + "&"
                                + dataProduction[j].mAttributesetinstanceId
                                + "&" + dataProduction[j].mLocatorId;
                        if (log4j.isDebugEnabled())
                            log4j.debug("******** Hashtable.production: "
                                    + strCalculate);
                        if (log4j.isDebugEnabled())
                            log4j
                                    .debug("******** Production, hashtable calculated: "
        ...
        if (dataChild.movementtype.startsWith("M")
            ...
            if (log4j.isDebugEnabled())
                log4j.debug("****PROCESSING MOVEMENTLINE: " + strNewId + " "
                        + strIn);
            dataMovement = MInOutTraceReportsData.selectMovement(this, vars
                    .getLanguage(), strIn.equals("Y") ? "M+" : "M-", strNewId);
            if (dataMovement != null && dataMovement.length > 0) {
                ...
                for (int j = 0; j < dataMovement.length; j++) {
                    ...
                    strHtml
                            .append(
                                    "    <td class=\"DataGrid_Body_Cell\" width=\"70\">")
                            .append(dataMovement[j].movementdate).append(
                                    "</td>\n");
                    strHtml
                            .append(
                                    "    <td class=\"DataGrid_Body_Cell\" width=\"100\">")
                            .append(dataMovement[j].movementtypeName).append(
                                    "</td>\n");
                    strHtml
                            .append(
                                    "    <td class=\"DataGrid_Body_Cell\" width=\"100\">")
                            .append(dataMovement[j].locatorName).append(
                                    "</td>\n");
                    strHtml
                            .append(
                                    "    <td class=\"DataGrid_Body_Cell\" width=\"90\">")
                            .append(dataMovement[j].movementqty).append(
                    ...
                    strHtml
                            ...
                            .append(dataMovement[j].quantityorder).append(
                                    "&nbsp;").append(
                                    dataMovement[j].productUomName).append(
                                    "</td>\n");
                    ...
                    if (!dataMovement[j].mAttributesetinstanceId.equals("0")) {
                        String strPartial = "";
                        if (!dataMovement[j].mProductId
                                .equals(strmProductIdGlobal)) {
                            if (log4j.isDebugEnabled())
                                log4j
                                        .debug("******** hashtable.production: Prod: "
                            ...
                            if (log4j.isDebugEnabled())
                                log4j
                                        .debug("******** Movement, hashtable calculated: "
                                                + calculated.get(strCalculate));
                            if (calculated.get(strCalculate) == null) {
                                strPartial = processChilds(
                                        vars,
                                        dataMovement[j].mAttributesetinstanceId,
                                        dataMovement[j].mProductId,
    ...
    private String getData(MInOutTraceReportsData data, String strClassName)
        ...
        resultado.append("    <td class=\"DataGrid_Body_Cell\" width=\"90\">")
                .append(data.movementqty).append("&nbsp;").append(data.uomName)
                .append("</td>\n");
        if (!data.quantityorder.equals("")) {
            resultado.append(
                    "    <td class=\"DataGrid_Body_Cell\" width=\"90\">")
                    .append(data.quantityorder).append("&nbsp;").append(
                            data.productUomName).append("</td>\n");
        }
