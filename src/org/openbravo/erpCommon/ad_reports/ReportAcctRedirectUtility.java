...
public class ReportAcctRedirectUtility extends HttpSecureAppServlet {
  ...
  public void doPost (HttpServletRequest request, HttpServletResponse response) throws IOException,ServletException {
    ...
    if (vars.commandIn("DEFAULT")) {
      ...
      ReportAcctRedirectUtilityData[] data = ReportAcctRedirectUtilityData.select(this, strTableId, strDocBaseType);
      if (data==null || data.length==0) bdError(response, "RecordError", vars.getLanguage());
      else {
        String inputName = "inp" + Sqlc.TransformaNombreColumna(data[0].columnname);
        String ubicacion = "/" + FormatUtilities.replace(data[0].windowName) + "/" + FormatUtilities.replace(data[0].tabName) + "_Relation.html";
        response.sendRedirect(strDireccion + ubicacion + "?" + "Command=DIRECT&" + inputName + "=" + strRecordId);
      }
