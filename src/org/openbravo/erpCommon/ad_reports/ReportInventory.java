...
public class ReportInventory extends HttpSecureAppServlet {
  ...
  public void doPost (HttpServletRequest request, HttpServletResponse response) throws IOException,ServletException {
    ...
    if (!Utility.hasProcessAccess(this, vars, "", "ReportInventory")) {
  ...
  void printPageDataHtml(HttpServletResponse response, VariablesSecureApp vars, String strmWarehouseId, String strDetalle)
    throws IOException, ServletException {
    if (log4j.isDebugEnabled()) log4j.debug("Output: dataSheet");
    response.setContentType("text/html; charset=UTF-8");
    PrintWriter out = response.getWriter();
    XmlDocument xmlDocument=null;
    ReportInventoryData [] data = ReportInventoryData.selectSinDetalle(this, strmWarehouseId, Utility.getContext(this, vars, "#User_Client", "ReportInventory"), Utility.getContext(this, vars, "#User_Org", "ReportInventory"));


    if (data.length == 0) {
      String[] discard = {"sectionLocator"};
      xmlDocument = xmlEngine.readXmlTemplate("org/openbravo/erpCommon/ad_reports/ReportInventoryEdit", discard).createXmlDocument();
      ...
      xmlDocument = xmlEngine.readXmlTemplate("org/openbravo/erpCommon/ad_reports/ReportInventoryEdit").createXmlDocument();
      ...
      for (int i=0;i<data.length ;i++){
        existsAtt = false;
        if (!(strWarehouse.equals(data[i].mLocatorId))||!(strProduct.equals(data[i].mProductId))||!(strUom.equals(data[i].productUom))) {
          strWarehouse = data[i].mLocatorId;
          strProduct = data[i].mProductId;
          strUom = data[i].productUom;
          ReportInventoryData [] data2 = ReportInventoryData.select(this, data[i].mProductId, data[i].cUomId, data[i].mLocatorId, Utility.getContext(this, vars, "#User_Client", "ReportInventory"), Utility.getContext(this, vars, "#User_Org", "ReportInventory"));
  ...
  void printPageDataSheet(HttpServletResponse response, VariablesSecureApp vars, String strmWarehouseId, String strDetalle)
    throws IOException, ServletException {
    if (log4j.isDebugEnabled()) log4j.debug("Output: dataSheet");
    response.setContentType("text/html; charset=UTF-8");
    PrintWriter out = response.getWriter();
    XmlDocument xmlDocument = xmlEngine.readXmlTemplate("org/openbravo/erpCommon/ad_reports/ReportInventory").createXmlDocument();
    ...
    try {
      ...
      xmlDocument.setParameter("theme", vars.getTheme());
      NavigationBar nav = new NavigationBar(this, vars.getLanguage(), "ReportInventory.html", classInfo.id, classInfo.type, strReplaceWith, tabs.breadcrumb());
      xmlDocument.setParameter("navigationBar", nav.toString());
      LeftTabsBar lBar = new LeftTabsBar(this, vars.getLanguage(), "ReportInventory.html", strReplaceWith);
  ...
  //  xmlDocument.setData("structure", WarehouseComboData.selectFilter(this, Utility.getContext(this, vars, "#User_Client", "ReportInventory")));
   ...
   try {
      ...
      Utility.fillSQLParameters(this, vars, null, comboTableData, "ReportInventory", strmWarehouseId);
  ...
  /*void printPageDataSheet(HttpServletResponse response, VariablesSecureApp vars, String strmWarehouseId, String strDetalle)
    ...
    if (data.length == 0) {
      String[] discard = {"sectionLocator"};
      xmlDocument = xmlEngine.readXmlTemplate("org/openbravo/erpCommon/ad_reports/ReportInventory", discard).createXmlDocument();
      xmlDocument.setData("structure1", ReportInventoryData.set());
    } else if(!strDetalle.equals("-1")){
      String[] discard = {"reportDetalle"};
      xmlDocument = xmlEngine.readXmlTemplate("org/openbravo/erpCommon/ad_reports/ReportInventory", discard).createXmlDocument();
      ...
      xmlDocument = xmlEngine.readXmlTemplate("org/openbravo/erpCommon/ad_reports/ReportInventory").createXmlDocument();
