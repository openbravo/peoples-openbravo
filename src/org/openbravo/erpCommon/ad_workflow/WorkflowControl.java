...
public class WorkflowControl extends HttpSecureAppServlet {
    ...
    public void doPost(HttpServletRequest request, HttpServletResponse response)
        ...
        final String strAD_Workflow_ID = vars.getGlobalVariable(
                "inpadWorkflowId", "WorkflowControl|adWorkflowId");
        ...
        if (vars.commandIn("DEFAULT")) {
            ...
            final String strAction = vars
                    .getRequiredStringParameter("inpAction");
            final String strClave = vars.getRequiredStringParameter("inpClave");
            final String strPath = getUrlPath(vars.getLanguage(), strAction,
                    strClave);
    ...
    void printPageRedirect(HttpServletResponse response,
            VariablesSecureApp vars, String strPath) throws IOException,
            ServletException {
        if (log4j.isDebugEnabled())
            log4j.debug("Output: print page redirect");
        final XmlDocument xmlDocument = xmlEngine.readXmlTemplate(
                "org/openbravo/erpCommon/ad_workflow/WorkflowControl_Redirect")
                .createXmlDocument();
    ...
    public String getUrlPath(String language, String action, String clave)
        ...
        final MenuData[] menuData = MenuData.selectData(this, language,
                strWindow, strProcess, strForm, strTask, strWorkflow);
        if (menuData == null || menuData.length == 0)
            throw new ServletException(
                    "WorkflowControl.getUrlPath() - Error while getting data");

        return VerticalMenu.getUrlStringStatic(strDireccion, menuData[0].name,
                menuData[0].action, menuData[0].classname,
                menuData[0].mappingname, menuData[0].adWorkflowId,
    ...
    void printPage(HttpServletResponse response, VariablesSecureApp vars,
        ...
        final XmlDocument xmlDocument = xmlEngine.readXmlTemplate(
                "org/openbravo/erpCommon/ad_workflow/WorkflowControl_Response")
                .createXmlDocument();
    ...
    void printPageDataSheet(HttpServletResponse response,
            VariablesSecureApp vars, String strAD_Workflow_ID)
            throws IOException, ServletException {
        if (log4j.isDebugEnabled())
            log4j.debug("Output: dataSheet");
        WorkflowControlData[] workflowName = null;
        if (vars.getLanguage().equals("en_US")) {
            workflowName = WorkflowControlData.selectWorkflowName(this,
                    strAD_Workflow_ID);
        } else {
            workflowName = WorkflowControlData.selectWorkflowNameTrl(this, vars
                    .getLanguage(), strAD_Workflow_ID);
        }
        final String[] discard = { "" };
        if (workflowName == null || workflowName.length == 0
                || workflowName[0].help.equals(""))
            discard[0] = new String("fieldWorkflowHelp");
        final XmlDocument xmlDocument = xmlEngine.readXmlTemplate(
                "org/openbravo/erpCommon/ad_workflow/WorkflowControl", discard)
                .createXmlDocument();
    ...
    String buildHtml(VariablesSecureApp vars, String strAD_Workflow_ID)
            throws ServletException {
        final String firstNode = WorkflowControlData.selectFirstNode(this,
                strAD_Workflow_ID);
        if (firstNode.equals("")) {
            log4j
                    .warn("WorkflowControl.buildHtml() - There're no first node defined for workflow: "
                            + strAD_Workflow_ID);
            return "";
        }
        ...
        if (vars.getLanguage().equals("en_US"))
            ...
            name = WorkflowControlData.selectFirstNodeDataTrl(this, vars
                    .getLanguage(), firstNode);
    ...
    String buildLevel(VariablesSecureApp vars, String node)
            throws ServletException {
        WorkflowControlData[] data = null;
        if (vars.getLanguage().equals("en_US"))
            data = WorkflowControlData.select(this, Utility.getContext(this,
                    vars, "#User_Client", "WorkflowControl"), Utility
                    .getContext(this, vars, "#User_Org", "WorkflowControl"),
                    node);
        else
            data = WorkflowControlData.selectTrl(this, vars.getLanguage(),
                    Utility.getContext(this, vars, "#User_Client",
                            "WorkflowControl"), Utility.getContext(this, vars,
                            "#User_Org", "WorkflowControl"), node);
        ...
        for (int i = 0; i < data.length; i++) {
            if (data.length > 1) {
                ...
                for (int j = 0; j < 2; j++)
                    sb
                            .append("<td class=\"TableEdition_OneCell_width\"></td>\n");
                sb.append("  </tr>\n");
            }
            sb.append(line());
    ...
    String buildButton(VariablesSecureApp vars, WorkflowControlData data)
            throws ServletException {
        ...
        html.append(data.name).append(
                "';return true;\" onblur=\"this.hideFocus=false\" ");
        html.append(" onClick=\"this.hideFocus=true;callServlet('").append(
                data.action).append("', '").append(strClave).append(
                "');return false;\">\n");
        html.append("    <img src=\"").append(strReplaceWith).append(
                "/images/blank.gif\" class=\"").append(windowIcon(data.action))
                .append("\" border=\"0\" title=\"");
        html.append(data.name).append("\"></img></a>\n");
        html.append("</td>\n");
        html.append("<td class=\"Popup_Workflow_text_ContentCell\">\n");
        html.append("  <a href=\"#\" onclick=\"callServlet('").append(
                data.action).append("', '").append(strClave).append("');");
        html.append("return false;\" onmouseover=\"window.status='").append(
                data.name).append("';return true;\" ");
        html
                .append("onmouseout=\"window.status='';return true;\" class=\"Popup_Workflow_text\">");
    ...
    String line() {
        ...
        html
                ...
                .append(strReplaceWith).append(
                        "/images/blank.gif\" border=\"0\"></img></td>");
