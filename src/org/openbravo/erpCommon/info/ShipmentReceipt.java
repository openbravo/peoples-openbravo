...
public class ShipmentReceipt extends HttpSecureAppServlet {
    ...
    public void doPost(HttpServletRequest request, HttpServletResponse response)
        ...
        if (vars.commandIn("DEFAULT")) {
            ...
            if (!strNameValue.equals(""))
                vars
                        .setSessionValue("ShipmentReceipt.name", strNameValue
                                + "%");
            ...
            String strName = vars.getGlobalVariable("inpKey",
                    "ShipmentReceipt.name", "");
            String strSalesTransaction = vars.getGlobalVariable(
                    "inpSalesTransaction", "ShipmentReceipt.salestransaction",
                    "N");
            String strBpartnerId = vars.getGlobalVariable("inpBpartnerId",
                    "ShipmentReceipt.bpartnerid", "");
            String strDateFrom = vars.getGlobalVariable("inpDateFrom",
                    "ShipmentReceipt.datefrom", "");
            String strDateTo = vars.getGlobalVariable("inpDateTo",
                    "ShipmentReceipt.dateto", "");
    ...
    void printPage(HttpServletResponse response, VariablesSecureApp vars,
            String strNameValue, String strSOTrx) throws IOException,
            ServletException {
        if (log4j.isDebugEnabled())
            log4j.debug("Output: Frame 1 of the invoice seeker");
        XmlDocument xmlDocument = xmlEngine.readXmlTemplate(
                "org/openbravo/erpCommon/info/ShipmentReceipt")
                .createXmlDocument();
    ...
    String generateResult(ShipmentReceiptData[] data) throws IOException,
            ServletException {
        ...
        html.append("var text = \""
                + Replace.replace(data[0].documentno, "\"", "\\\"") + "\";\n");
    ...
    private SQLReturnObject[] getHeaders(VariablesSecureApp vars) {
        ...
        for (int i = 0; i < colNames.length; i++) {
            ...
            String name = Utility.messageBD(this, "SRS_"
                    + colNames[i].toUpperCase(), vars.getLanguage());
            dataAux.setData("name", (name.startsWith("SRS_") ? colNames[i]
                    : name));
        ...
        if (headers != null) {
            try {
                if (strNewFilter.equals("1") || strNewFilter.equals("")) { // New
                    ...
                    strNumRows = vars
                            .getSessionValue("ShipmentReceipt.numrows");
                }
