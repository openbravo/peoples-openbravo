...
public class Invoice extends HttpSecureAppServlet {
  ...
  public void doPost (HttpServletRequest request, HttpServletResponse response) throws IOException,ServletException {
    ...
    if (vars.commandIn("DEFAULT")) {
      ...
      InvoiceData[] data = InvoiceData.selectKey(this, vars.getSqlDateFormat(), Utility.getContext(this, vars, "#User_Client", "Invoice"), Utility.getSelectorOrgs(this, vars, strOrg), strSOTrx, strKeyValue + "%");
      if (data!=null && data.length==1) {
        printPageKey(response, vars, data);
      } else printPage(response, vars, strKeyValue, strWindowId);
  ...
  void printPage(HttpServletResponse response, VariablesSecureApp vars, String strNameValue, String strWindow) throws IOException, ServletException {
    if (log4j.isDebugEnabled()) log4j.debug("Output: business partners seeker Frame Set");
    XmlDocument xmlDocument = xmlEngine.readXmlTemplate("org/openbravo/erpCommon/info/Invoice").createXmlDocument();    
    ...
    total.append("new keyArrayItem(\"ENTER\", \"openSearch(null, null, '../Invoice.html', 'SELECTOR_INVOICE', false, 'frmMain', 'inpNewcInvoiceId', 'inpNewcInvoiceId_DES', document.frmMain.inpNewcInvoiceId_DES.value, 'Command', 'KEY', 'WindowID', '");
