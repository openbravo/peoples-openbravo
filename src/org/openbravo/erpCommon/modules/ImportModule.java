 ...
 * 
 * This is done in two steps: -First check if it is possible to install and
 * receive the latest installable modules -Second install them
 * 
...
public class ImportModule {
     ...
     * database model to be able to read the xml information within the obx
     * file.
     * 
     * @param obdir
     *            base directory for the application
     * @param _vars
     *            VariablesSecureApp that will be used to parse messages, if
     *            null they will not be parsed.
     */
    public ImportModule(ConnectionProvider conn, String obdir,
    ...
    public boolean checkDependenciesFile(InputStream file) throws Exception {
        if (installLocally) {
            ...
            for (final DynaBean module : modulesInObx) {
                if (ImportModuleData.moduleInstalled(pool, (String) module
                        .get("AD_MODULE_ID"))) {
            ...
            modulesToInstall = dyanaBeanToModules(dynModulesToInstall,
                    dependencies);
            modulesToUpdate = dyanaBeanToModules(dynModulesToUpdate,
                    dependencies);
            errors = new OBError();
            ...
            getModulesFromObx(dynModulesToInstall, dependencies,
                    new Vector<DynaBean>(), file);
            final String[] installableModules = new String[1];
            installableModules[0] = (String) dynModulesToInstall.get(0).get(
                    "AD_MODULE_ID");
        ...
        if (!checked) {
            try {
                ImportModuleData.insertLog(pool, (vars == null ? "0" : vars
                        .getUser()), "", "", "",
                        "Cannot perform installation correctly: "
                                + errors.getMessage()
    ...
    /**
     * Check the dependencies for a id. After checking dependencies
     * modulesToInstall and modulesToUpdate arrays of modules are completed,
     * thus it is possible to know wich are the modules that are needed to
     * install and/or update in order to complete the installation.
     * 
     */
    public boolean checkDependenciesId(String[] installableModules,
            String[] updateableModules) throws Exception {
        // just for remote usage
        errors = new OBError();
        VersionUtility.setPool(pool);
        final ModuleInstallDetail mid = VersionUtility.checkRemote(vars,
                installableModules, updateableModules, errors);
    ...
    /**
     * Executes the modules installation, first one of the checkDependencies
     * method should have been called in order to set the installable and
     * updateable modules.
     * 
        ...
        if (checked || force) {
            ...
            if (modulesToInstall != null) {
                for (int i = 0; i < modulesToInstall.length; i++) {
                    try {
                        final byte[] getMod = ws.getModule(modulesToInstall[i]
                                .getModuleVersionID());
                        ByteArrayInputStream obx = new ByteArrayInputStream(
                                getMod);
                        ...
                        rollback();
                        try {
                            ImportModuleData.insertLog(pool,
                                    (vars == null ? "0" : vars.getUser()), "",
            ...
            if (modulesToUpdate != null) {
                for (int i = 0; i < modulesToUpdate.length; i++) {
                    try {
                        final byte[] getMod = ws.getModule(modulesToUpdate[i]
                                .getModuleVersionID());
                        ...
                        getModulesFromObx(dynMod, dynDep, dynDBPrefix,
                                new ByteArrayInputStream(getMod));
                        insertDynaModulesInDB(dynMod, dynDep, dynDBPrefix);
                        ...
                        try {
                            ImportModuleData
                                    .insertLog(pool, (vars == null ? "0" : vars
                                            .getUser()), "", "", "",
                                            "Error getting module "
                                                    + modulesToUpdate[i]
                                                            .getName()
    ...
    /**
     * Prapares a backup for rollback of updates and removes the files within
     * the directories to update, making thus the update like an installation
     * from scratch.
     * 
     */
    private void initInstallation() {
        ...
        for (int i = 0; i < modulesToUpdate.length; i++) {
            ...
            if (modulesToUpdate[i].getModuleID().equals("0")) { // Updating core
                ...
                try {
                    Zip.zip(obDir + "/modules/"
                            + modulesToUpdate[i].getPackageName(), obDir
                            + "/backup_install/"
                            + modulesToUpdate[i].getPackageName() + "-"
    ...
    /**
     * Returns the list of modules to update. This list is set by one of the
     * checkDependencies methods.
     */
    ...
    /**
     * Returns the list of modules to install. This list is set by one of the
     * checkDependencies methods.
     */
    ...
    /**
     * Returns the list of errors. This list is set by one of the
     * checkDependencies methods. A list of errors is returned in case the
     * selected modules cannot be installed because dependencies are not
     * satisfied.
     * 
     */
    ...
    /**
     * Set the install locally variable, install locally means that no pull is
     * going to be done for the contents of the obx, it will be installed
     * directly from the obx file regardless better versions are available.
     * 
     */
    ...
    /**
     * Returns an OBError instance based on the log for the current ImportModule
     * instance
     * 
     */
    public OBError getOBError(ConnectionProvider conn) {
        if (log.length() != 0) {
            ...
            rt.setMessage(Utility.parseTranslation(conn, vars, lang, log
                    .toString()));
    ...
    private void rollback() {
        ...
        try {
            ImportModuleData.insertLog(pool, (vars == null ? "0" : vars
                    .getUser()), "", "", "", "Rollback installation", "E");
        } catch (final ServletException ex) {
            ex.printStackTrace();
        }
        if (modulesToInstall != null && modulesToInstall.length > 0) {
            for (int i = 0; i < modulesToInstall.length; i++) {
                try {
                    ...
                    ImportModuleData.setInDevelopment(pool, modulesToInstall[i]
                            .getModuleID());
                    ImportModuleData.deleteDependencies(pool,
                            modulesToInstall[i].getModuleID());
                    ImportModuleData.deleteDBPrefix(pool, modulesToInstall[i]
                            .getModuleID());
                    ImportModuleData.deleteModule(pool, modulesToInstall[i]
                            .getModuleID());
                } catch (final Exception e) {
                    e.printStackTrace();
                    addLog("Error deleting module "
                            + modulesToInstall[i].getName() + " from db. "
                            + e.getMessage(), MSG_ERROR);
                }

                final File f = new File(obDir + "/modules/"
                        + modulesToInstall[i].getPackageName());
                if (f.exists()) {
                    if (Utility.deleteDir(f))
                        addLog("@DeletedDirectory@ " + f.getAbsolutePath(),
                                MSG_ERROR);
                    else
                        addLog("@CouldntDeleteDirectory@ "
                                + f.getAbsolutePath(), MSG_ERROR);
                }
            }
        }
        ...
        if (modulesToUpdate != null && modulesToUpdate.length > 0) {
            for (int i = 0; i < modulesToUpdate.length; i++) {
                if (modulesToUpdate[i].getModuleID().equals("0")) { // restore
                    ...
                    Utility.deleteDir(core);
                    try {
                        Zip.unzip(obDir + "/backup_install/"
                                + modulesToUpdate[i].getPackageName() + "-"
                                + modulesToUpdate[i].getVersionNo() + ".zip",
                                obDir);
                    } catch (final Exception e) {
    ...
    private void printAntDependenciesLog() {
        if (!checked) {
            ...
            if (modulesToInstall != null && modulesToInstall.length > 0) {
                ...
                for (int i = 0; i < modulesToInstall.length; i++) {
                    log4j.info(modulesToInstall[i].getName() + " "
            ...
            if (modulesToUpdate != null && modulesToUpdate.length > 0) {
                ...
                for (int i = 0; i < modulesToUpdate.length; i++) {
                    log4j.info(modulesToUpdate[i].getName() + " "
                            + modulesToUpdate[i].getVersionNo());
                }
            }
        }
    ...
    private Module[] dyanaBeanToModules(Vector<DynaBean> dModulesToInstall,
        ...
        for (final DynaBean dynModule : dModulesToInstall) {
            ...
            rt[i].setDependencies(dyanaBeanToDependencies(dynDependencies,
                    rt[i].getModuleID()));
    ...
    /**
     * Returns the dependencies in Vector<DynaBean> dynDependencies for the
     * ad_module_id module as a ModuleDependency[], used by dyanaBeanToModules
     * method
     * 
     */
    private ModuleDependency[] dyanaBeanToDependencies(
            Vector<DynaBean> dynDependencies, String ad_module_id) {
        ...
        for (final DynaBean dynModule : dynDependencies) {
            if (((String) dynModule.get("AD_MODULE_ID")).equals(ad_module_id)) {
                ...
                md
                        .setModuleID((String) dynModule
                                .get("AD_DEPENDENT_MODULE_ID"));
    ...
    /**
     * Adds the classpath entries to .classpath file from the modules in the
     * Vector<DynaBean>
     * 
     */
    private void addDynaClasspathEntries(Vector<DynaBean> dModulesToInstall)
            throws Exception {
        ...
        for (final DynaBean module : dModulesToInstall) {
            final String dir = obDir + "/modules/"
                    + (String) module.get("JAVAPACKAGE") + "/src";
        ...
        final FileOutputStream fout = new FileOutputStream(obDir
                + "/.classpath");
     ...
     * @throws Exception
     */
    private ByteArrayInputStream getCurrentEntryStream(
            ZipInputStream obxInputStream) throws Exception {
        final ByteArrayOutputStream fout = new ByteArrayOutputStream();
        for (int c = obxInputStream.read(); c != -1; c = obxInputStream.read()) {
            fout.write(c);
        }
        fout.close();
        final ByteArrayInputStream ba = new ByteArrayInputStream(fout
                .toByteArray());
        return ba;
    }
    ...
    private void insertDynaModulesInDB(Vector<DynaBean> dModulesToInstall,
            Vector<DynaBean> dependencies1, Vector<DynaBean> dbPrefix)
            throws Exception {
        final Properties obProperties = new Properties();
        obProperties.load(new FileInputStream(obDir
        ...
        for (final DynaBean module : dModulesToInstall) {
            ...
            log4j
                    .info("Inserting in DB info for module: "
                            + module.get("NAME"));
            platform.updateinsert(conn, db, module);
            addLog("@ModuleInstalled@ " + module.get("NAME") + " - "
                    + module.get("VERSION"), MSG_SUCCESS);
        }
    ...
    /**
     * Returns all the modules and dependencies described within the obx file
     * (as InputStream)
     * 
     * @param dModulesToInstall
     * @param dDependencies
    ...
    private void getModulesFromObx(Vector<DynaBean> dModulesToInstall,
            Vector<DynaBean> dDependencies, Vector<DynaBean> dDBprefix,
            InputStream obx) throws Exception {
        ...
        while (((entry = obxInputStream.getNextEntry()) != null) && !foundAll) {
            ...
            if (entry.getName().endsWith(".obx")) { // If it is a new module
                ...
                getModulesFromObx(dModulesToInstall, dDependencies, dDBprefix,
                        ba);
                ...
                if (module != null && module.size() > 0) {
                    isPackage = !((String) module.get(0).get("TYPE"))
                            .equals("M");
                }
     ...
     * @throws Exception
     */
    private Vector<DynaBean> getEntryDynaBeans(ZipInputStream obxInputStream)
            throws Exception {
        final ByteArrayInputStream ba = getCurrentEntryStream(obxInputStream);
     ...
     * @throws Exception
     */
    private void installModule(InputStream obx, String moduleID)
            throws Exception {
        if (!(new File(obDir + "/modules").canWrite())) {
            addLog("@CannotWriteDirectory@ " + obDir + "/modules. ", MSG_ERROR);
            throw new PermissionException("Cannot write on directory: " + obDir
                    + "/modules");
        }
            ...
            if (entry.getName().endsWith(".obx")) { // If it is a new module
                ...
                if (installLocally) {
                    ...
                    final ByteArrayInputStream ba = new ByteArrayInputStream(
                            fout.toByteArray());
                    final ByteArrayInputStream ba1 = new ByteArrayInputStream(
                            fout.toByteArray());

                    // Obtain the ID for the module to be installed (this is
                    // done for core)
                    final Vector<DynaBean> obxModule = new Vector<DynaBean>();
                    getModulesFromObx(obxModule, new Vector<DynaBean>(),
                            new Vector<DynaBean>(), ba1);

                    installModule(ba, (String) obxModule.get(0).get(
                            "AD_MODULE_ID"));
                ...
                final String fileName = obDir
                        + (moduleID.equals("0") ? "/" : "/modules/")
    ...
    private void insertDBLog() {
        try {
            ...
            if (modulesToInstall != null && modulesToInstall.length > 0) {
                for (int i = 0; i < modulesToInstall.length; i++) {
                    ImportModuleData.insertLog(pool, user, modulesToInstall[i]
                            .getModuleID(), modulesToInstall[i]
                            .getModuleVersionID(), modulesToInstall[i]
                            .getName(), "Installed module "
                            + modulesToInstall[i].getName() + " - "
            ...
            if (modulesToUpdate != null && modulesToUpdate.length > 0) {
                for (int i = 0; i < modulesToUpdate.length; i++) {
                    ImportModuleData.insertLog(pool, user, modulesToUpdate[i]
                            .getModuleID(), modulesToUpdate[i]
                            .getModuleVersionID(),
                            modulesToUpdate[i].getName(), "Updated module "
                                    + modulesToUpdate[i].getName()
                                    + " to version "
    ...
    /**
     * Scans for updates for the existent modules and sets and returs the list
     ...
     * @return
     */
    public static HashMap<String, String> scanForUpdates(
            ConnectionProvider conn, VariablesSecureApp vars) {
        try {
            final HashMap<String, String> updateModules = new HashMap<String, String>();
            final String user = vars == null ? "0" : vars.getUser();
            ImportModuleData.insertLog(conn, user, "", "", "",
                    "Scanning For Updates", "S");
            WebServiceImplServiceLocator loc;
            WebServiceImpl ws = null;
                ...
                try {
                    ImportModuleData
                            .insertLog(
                                    ...
                                    "",
                                    "",
                                    "Scan for updates: Couldn't contact with webservice server",
                                    "E");
                } catch (final ServletException ex) {
                    ex.printStackTrace();
                }
            ...
            if (updates != null && updates.length > 0) {
                for (int i = 0; i < updates.length; i++) {
                    if (!ImportModuleData.existsVersion(conn, updates[i]
                            .getVersionNo())) {
                        ImportModuleData.updateNewVersionAvailable(conn,
                                updates[i].getVersionNo(), updates[i]
                                        .getModuleVersionID(), updates[i]
                                        .getModuleID());
                        ImportModuleData
                                .insertLog(conn, user,
                                        updates[i].getModuleID(), updates[i]
                                                .getModuleVersionID(),
                                        updates[i].getName(),
                                        "Found new version "
                                                + updates[i].getVersionNo()
                                                + " for module "
                                                + updates[i].getName(), "S");
            ...
            try {
                ImportModuleData.insertLog(conn, (vars == null ? "0" : vars
                        .getUser()), "", "", "", "Total: found "
                        + updateModules.size() + " updates", "S");
            } catch (final ServletException ex) {
                ex.printStackTrace();
            }
            ...
            try {
                ImportModuleData.insertLog(conn, (vars == null ? "0" : vars
                        .getUser()), "", "", "", "Scan for updates: Error: "
                        + e.toString(), "E");
            } catch (final ServletException ex) {
                ex.printStackTrace();
            }
    ...
    private static void addParentUpdates(HashMap<String, String> updates,
            ConnectionProvider conn) {
        final HashMap<String, String> iniUpdates = (HashMap<String, String>) updates
                .clone();
    ...
    private static void addParentNode(String node,
     ...
     */
    public static HashMap<String, String> getInstalledModules(
            ConnectionProvider conn) {
    ...
    /**
     * Returns the module with the ID that is in the module to install or
     * update.
     * 
     * @param moduleID
     * @return
