...
public class ModuleTree extends GenericTree {
    ...
    public void setRootTree() {
        try {
            data = ModuleTreeData.select(conn, (lang.equals("") ? "en_US"
                    : lang));
            addLinks();
            setLevel(0);
            setIcons();
    ...
    public void setSubTree(String nodeId, String level) {
        setIsSubTree(true);
        try {
            data = ModuleTreeData.selectSubTree(conn,
    ...
    public String getHTMLDescription(String node) {
        try {
            ...
            ModuleTreeData[] data = ModuleTreeData.selectDescription(conn,
                    lang, node);
            addLinks(data, true);
            String discard[] = { "" };
            ...
            if (data != null
                    && data.length > 0
                    && (data[0].updateAvailable == null || data[0].updateAvailable
                            .equals("")))
                discard[0] = "update";
    ...
    private void addLinks(ModuleTreeData[] modules, boolean showApplied) {
        ...
        for (int i = 0; i < modules.length; i++) {
            if (!modules[i].updateAvailable.equals("")) {
                modules[i].linkname = Utility.messageBD(conn,
                        "UpdateAvailable", lang);
                modules[i].linkclick = "gt_getUpdateDescription('"
                        + modules[i].nodeId + "'); return false;";
            }
            if (modules[i].status.equals("I")) {
                modules[i].linkname = Utility.messageBD(conn, "ApplyModules",
                        lang)
    ...
    /**
     * Selects the correct icons for the current set of nodes depending on the
     * module type (Module, Pack or Template), it also sets the secondary icon
     * in case the node have available updates.
     */
    ...
    private void setIcons(FieldProvider[] modules) {
        ...
        for (int i = 0; i < modules.length; i++) {
            if (modules[i].getField("type").equals("M"))
                FieldProviderFactory.setField(modules[i], "icon",
                        "Tree_Icon_Module");
            ...
            if (updateAvailable || updateAvailableInChildNode)
                FieldProviderFactory.setField(modules[i], "icon2",
                        "Tree_Icon_Update");
        }
    }

    /**
     * Returns true in case one of the descendant of the current node has an
     * update available
     * 
     * @param node
     * @return
    ...
    private boolean hasChildUpdate(String node) {
        try {
            ModuleTreeData data[] = ModuleTreeData
                    .selectSubTree(conn, "", node);
            if (data == null || data.length == 0)
                return false;
            for (int i = 0; i < data.length; i++) {
                if (data[i].updateAvailable != null
                        && !data[i].updateAvailable.equals(""))
                    return true;
