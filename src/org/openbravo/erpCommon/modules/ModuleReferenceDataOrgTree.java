...
public class ModuleReferenceDataOrgTree extends ModuleTree {
    ...
    public void setRootTree(String strClient, boolean bAddLinks) {
        try {
            data = ModuleReferenceDataOrgTreeData.select(conn,
                    (lang.equals("") ? "en_US" : lang), strClient);
            if (bAddLinks)
                addLinks();
            setLevel(0);
            setIcons();
    ...
    public void setRootTree(String strClient, String strOrg, boolean bAddLinks) {
        try {
            data = ModuleReferenceDataOrgTreeData.selectOrg(conn, (lang
                    .equals("") ? "en_US" : lang), strClient, strOrg);
            if (bAddLinks)
                addLinks();
            setLevel(0);
            setIcons();
    ...
    private void addLinks(ModuleReferenceDataOrgTreeData[] modules,
            boolean showApplied) {
        ...
        for (int i = 0; i < modules.length; i++) {
            if (modules[i].updateAvailable.equals("Y")) {
                ...
                modules[i].linkclick = "gt_submitUpdateData('OK','"
             ...
             * lang)+", "+Utility.messageBD(conn, "RebuildNow", lang);
             * modules[i].linkclick=
             ...
             * modules[i].linkname=Utility.messageBD(conn, "UninstalledModule",
             * lang);modules[i].linkclick=
             * "openServletNewWindow('DEFAULT', false, '../ad_process/ApplyModules.html', 'BUTTON', null, true, 600, 900);return false;"
             * ; }
             */
        }
    ...
    public String getHTMLDescription(String node) {
        try {
            ...
            ModuleReferenceDataOrgTreeData[] data = ModuleReferenceDataOrgTreeData
            ...
            if (data != null
                    ...
                    && (data[0].updateAvailable == null
                            || data[0].updateAvailable.equals("") || data[0].updateAvailable
                            .equals("N")))
                discard[0] = "update";
    ...
    public void setSubTree(String nodeId, String level) {
        setIsSubTree(true);
        try {
            data = ModuleReferenceDataOrgTreeData.selectSubTree(conn, (lang
