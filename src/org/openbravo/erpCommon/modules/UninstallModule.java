...
/**
 * UninstallModule class is able to uninstall a list of modules, uninstalling a
 * module consists on deleting all its contents directory and setting it in
 * ad_module table as uninstalled and inactive, afterwards a rebuild process is
 * required, this process will delete the uninstalled modules from database.
 * 
 * 
 */
public class UninstallModule {
     ...
     * @param _vars
     */
    public UninstallModule(ConnectionProvider conn, String obDir,
     ...
     * @param _vars
     */
    public UninstallModule(String xmlPoolFile, String obDir,
            VariablesSecureApp _vars) {
    ...
    /**
     * Executes the uninstall process for a comma separated list of modules, all
     * these modules will be uninstalled as well as all their contained modules.
     * 
     * @param moduleIdList
    ...
    public void execute(String moduleIdList) {
        ...
        try {
            ...
            final UninstallModuleData[] dependencies = UninstallModuleData
                    .selectDependencies(pool, moduleIdList);
            if (dependencies != null && dependencies.length > 0) {
                for (int i = 0; i < dependencies.length; i++) {
                    log4j
                            .error(dependencies[i].name
                                    + " cannot uninstall module because it is part of a dependency");
                    addLog(dependencies[i].name
            ...
            final StringTokenizer st = new StringTokenizer(moduleIdList, ",",
                    false);
            while (st.hasMoreTokens()) {
                ...
                UninstallModuleData.updateUninstall(pool, contents); // set as
                                                                     // uninstalled
                                                                     // in DB
                final UninstallModuleData data[] = UninstallModuleData
                        .selectDirectories(pool, contents); // delete
                                                            // directories
                if (data != null && data.length > 0) {
                    for (int i = 0; i < data.length; i++) {
                        UninstallModuleData.insertLog(pool, (vars == null ? "0"
                                : vars.getUser()), data[i].adModuleId,
                                data[i].version, data[i].name,
                                "Uninstalled module " + data[i].name + " - "
                                        + data[i].version, "D");
                        final File f = new File(modulesBaseDir + "/"
                                + data[i].javapackage);
                        if (f.exists()) {
                            if (!Utility.deleteDir(f)) {
                                addLog("@CannotRemoveModule@ " + data[i].name,
                                        MSG_ERROR);
                                log4j.error("Cannot remove module contents "
                                        + data[i].name);
                            } else {
                                addLog("@RemovedModule@ " + data[i].name,
                                        MSG_SUCCESS);
                                log4j.info("Removed contents for module "
                                        + data[i].name);
                            }
                        } else {
                            addLog("@ModuleHasNotDirectory@ " + data[i].name,
                                    MSG_WARN);
                            log4j.info(data[i].name
                                    + " has no contents directory");
                        }
    ...
    /**
     * Returns a list of comma separated ids of the modules contained in the
     * package or template. The passed module/package/template is also returned
    ...
    private String getContentList(String module) throws Exception {
        String rt = "'" + module + "'";
        final UninstallModuleData data[] = UninstallModuleData.selectContent(
                pool, module);
        if (data != null && data.length > 0) {
            for (int i = 0; i < data.length; i++) {
    ...
    public OBError getOBError() {
        if (log.length() != 0) {
            ...
            rt.setMessage(Utility.parseTranslation(pool, vars, lang, log
                    .toString()));
