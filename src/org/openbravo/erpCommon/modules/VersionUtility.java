...
public class VersionUtility {
    ...
    static private boolean checkVersion(String depParentMod, Dep dep, Mod mod,
        ...
        if (versionCompare(dep.minVer, mod.availableMaxVer, true) == 1) {
            errors.add(depParentMod + " @CR_DependensOnModule@ \""
                    + dep.modName + "\" @CR_InVersion@ \"" + dep.minVer
                    + "\", @CR_MaxAvailableVersion@ \"" + mod.availableMaxVer
                    + "\". ");
            return false;
        }
        if (versionCompare(mod.availableMinVer, dep.maxVer, true) == 1) {
            errors.add(depParentMod + " @CR_DependensOnModule@ \""
                    + dep.modName + "\" @CR_InVersion@ \"" + dep.maxVer
                    + "\", @CR_MaxAvailableVersion@ \"" + mod.availableMinVer
                    + "\". ");
            return false;
        }
        return true;
    ...
    static private boolean checkDependency(String strModVersion,
            HashMap<String, Mod> modulesMap,
            HashMap<String, Mod> modsToInstall,
            HashMap<String, Mod> modsToUpdate, Dep dependency,
        ...
        } else if (modsToInstall != null
                && modsToInstall.containsKey(dependency.modId)) {
            mod = modsToInstall.get(dependency.modId);
        } else if (modsToUpdate != null
                && modsToUpdate.containsKey(dependency.modId)) {
        ...
        if (foundModule && checkVersion(strModVersion, dependency, mod, errors))
            return true;
        errors.add(strModVersion + " @CR_DependensOnModule@ \""
                + dependency.modName + "\", @CR_ModuleNotInstalled@");
    ...
    static private boolean checkVersionDependency(String strModVersion,
            HashMap<String, Mod> modulesMap,
            HashMap<String, Mod> modsToInstall,
            HashMap<String, Mod> modsToUpdate, Ver version,
            Vector<String> errors, Vector<String> newModules) throws Exception {
        ...
        for (String depKey : depMap.keySet()) {
            /**
             * Check if in the whole modules versions map there is the module
             * version that satisfies the dependency and includes If no
             * available configuration is found an exception is throw. If new
             * modules or new updates are needed, they are added to they
             * correspondent list and added to errors vector. and the checked is
             * run again with the new configuration
             */
            if (!checkDependency(strModVersion, modulesMap, modsToInstall,
                    modsToUpdate, depMap.get(depKey), errors, newModules)) {
    ...
    static private boolean checkAllDependencies(
            HashMap<String, Mod> modulesMap,
            HashMap<String, Mod> modsToInstall,
            HashMap<String, Mod> modsToUpdate, Vector<String> errors)
            throws Exception {
        ...
        Vector<String> modKey = new Vector<String>(modsForCheckDependencies
                .keySet());
        ...
        for (int i = 0; i < modKey.size(); i++) {
            ...
            for (String verKey : verMap.keySet()) {
                ...
                if (!checkVersionDependency(strModVersion,
                        modulesInstalledLessToUpdate, modsToInstall,
                        modsToUpdate, ver, errors, newModules)) {
                    /**
                     * When any dependency fails, the process continue to found
                     * all dependency errors, but the configuration is marked as
                     * no valid
                     */
    ...
    static private HashMap<String, Mod> fillModules() throws ServletException {
        /**
         * fill Mod objects (include its versions and dependencies from
         * versions) from database for all Modules IDs
         */
    ...
    static private HashMap<String, Ver> fillVersions(VersionUtilityData data,
            Mod mod) throws ServletException {
    ...
    static private HashMap<String, Dep> fillDependencies(String modID)
    ...
    static private HashMap<String, Dep> fillIncludes(String modID)
            throws ServletException {
        /** fill Dep objects from database */
        VersionUtilityData[] data = VersionUtilityData
                .readIncludes(pool, modID);
        HashMap<String, Dep> hashDep = new HashMap<String, Dep>();
        for (int i = 0; i < data.length; i++) {
            Dep dep = new Dep();
            dep.depId = data[i].adModuleDependencyId;
    ...
    static private int versionCompare(String ver1, String ver2,
            boolean onlyMayorVersion) {
        if ((ver1 == null || ver1.equals(""))
                && (ver2 == null || ver2.equals("")))
            return 0;
        if ((ver1 == null || ver1.equals("")))
            return 1;
    ...
    static private HashMap<String, Mod> modules2mods(Module[] modules) {
        ...
        for (int i = 0; i < modules.length; i++) {
            ...
            if (mod.availableMinVer == null
                    || versionCompare(ver.version, mod.availableMinVer) == -1)
                mod.availableMinVer = ver.version;
            else if (mod.availableMaxVer == null
                    || versionCompare(ver.version, mod.availableMaxVer) == 1)
                mod.availableMaxVer = ver.version;
    ...
    static private boolean installModulesLocal(Module[] modulesToInstall,
            Module[] modulesToUpdate, Vector<String> vecErrors)
            throws Exception {
        ...
        try {
            ...
            checked = checkAllDependencies(modsInstalled, modsToInstall,
                    modsToUpdate, vecErrors);
        } catch (Exception e) {
            throw e;
        }
    ...
    static public boolean getOBError(OBError rt, ConnectionProvider conn,
            VariablesSecureApp vars, String[] errors) {
        if (errors.length != 0) {
            rt.setType("Error");
     ...
     *            In param. An String array with the modules version ids, that
     *            we want to install.
     ...
     * @return ModuleInstallDetail with modulesToInstall, modulesToUpdate and
     *         isValidConfiguration
     */
    static public ModuleInstallDetail checkRemote(VariablesSecureApp vars,
            String[] moduleVersionId, String[] moduleVersionToUpdateId,
        ...
        ModuleInstallDetail mid = ws.checkConsistency(ImportModule
                .getInstalledModules(pool), moduleVersionId,
                moduleVersionToUpdateId);
    ...
    /**
     * 
     * check the dependencies to install new modules locally, without connecting
     ...
     */
    static public boolean checkLocal(VariablesSecureApp vars,
            Module[] modulesToInstall, OBError obErrors) throws Exception {
