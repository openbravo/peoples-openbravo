...
public class VersionUtility {
  ...
  static private boolean checkVersion(String depParentMod, Dep dep, Mod mod, Vector<String> errors) {
    if(versionCompare(dep.minVer, mod.availableMaxVer, true) == 1){
      errors.add(depParentMod+" @CR_DependensOnModule@ \""+dep.modName+"\" @CR_InVersion@ \""+dep.minVer+"\", @CR_MaxAvailableVersion@ \""+mod.availableMaxVer+"\". ");
      return false;
    }
    if(versionCompare(mod.availableMinVer, dep.maxVer, true) == 1){
      errors.add(depParentMod+ " @CR_DependensOnModule@ \""+dep.modName+"\" @CR_InVersion@ \""+dep.maxVer+"\", @CR_MaxAvailableVersion@ \""+mod.availableMinVer+"\". ");
      return false;
    }
    return true;
  }
  
  static private boolean checkDependency(String strModVersion, HashMap<String,Mod> modulesMap, HashMap<String,Mod> modsToInstall, HashMap<String,Mod> modsToUpdate, Dep dependency, Vector<String> errors, Vector<String> newModules) throws Exception{
    ...
    if(foundModule && checkVersion(strModVersion, dependency, mod, errors)) return true;
    errors.add(strModVersion+" @CR_DependensOnModule@ \""+dependency.modName+"\", @CR_ModuleNotInstalled@");
    
    return false;
  }
  
  static private boolean checkVersionDependency(String strModVersion, HashMap<String,Mod> modulesMap, HashMap<String,Mod> modsToInstall, HashMap<String,Mod> modsToUpdate, Ver version, Vector<String> errors, Vector<String> newModules) throws Exception{
    ...
    for(String depKey : depMap.keySet()){
      /** Check if in the whole modules versions map there is the module version that satisfies the dependency and includes
        * If no available configuration is found an exception is throw.
        * If new modules or new updates are needed, they are added to they correspondent list
        * and added to errors vector.
        * and the checked is run again with the new configuration */
      if(!checkDependency(strModVersion, modulesMap, modsToInstall, modsToUpdate, depMap.get(depKey), errors, newModules) ){
        // If any dependency or include need a new module, it is added to modsToInstall or if needed an update is added to modsToUpdate 
        checked = false;
      }
    }
    return checked;
  ...
  static private boolean checkAllDependencies(HashMap<String,Mod> modulesMap, HashMap<String,Mod> modsToInstall, HashMap<String,Mod> modsToUpdate, Vector<String> errors) throws Exception{
    // New hashmap of installed modules less the modules that will be updated
    ...
    for(int i=0; i < modKey.size(); i++){
      ...
      for(String verKey : verMap.keySet()){
        ...
        if(!checkVersionDependency(strModVersion, modulesInstalledLessToUpdate, modsToInstall, modsToUpdate, ver, errors,  newModules)){
          /** When any dependency fails, the process continue to found all dependency errors,
           *  but the configuration is marked as no valid*/
  ...
  static public String toCommaString(String[] arr, boolean parenthesys){
    ...
    if(parenthesys) modNames.append("( ");
    for(int i=0; i<arr.length; i++){
      if(i!=0) modNames.append(", ");
  ...
  static private HashMap<String,Mod> fillModules() throws ServletException{
    /** fill Mod objects (include its versions and dependencies from versions) from database for all Modules IDs */
  ...
  static private HashMap<String,Ver> fillVersions(VersionUtilityData data, Mod mod) throws ServletException{
  ...
  static private int versionCompare(String ver1, String ver2, boolean onlyMayorVersion){
    if((ver1==null || ver1.equals("")) && (ver2 == null || ver2.equals(""))) return 0;
    if((ver1==null || ver1.equals(""))) return 1;
    if((ver2 == null || ver2.equals(""))) return -1;
    ...
    for(int i=0; i<3; i++){
      if(s1.length-1<i || s1[i] == null || s1[i].equals("")) n1[i]=0;
      else n1[i] = Integer.parseInt(s1[i]);
      
      if(s2.length-1<i || s2[i] == null || s2[i].equals("")) n2[i]=0;
      else n2[i] = Integer.parseInt(s2[i]);      
    }
  ...
  static private boolean installModulesLocal(Module[] modulesToInstall, Module[] modulesToUpdate,  Vector<String> vecErrors) throws Exception{
  ...
  static public boolean getOBError(OBError rt, ConnectionProvider conn, VariablesSecureApp vars, String[] errors){
    if (errors.length!=0) {
      rt.setType("Error");
   ...
   * 
   * @param moduleVersionId In param. An String array with the modules version ids, that we want to install.
   * @param modulesToInstall Out param. New modules that is necessary to install.
   * @param modulesToUpdate Out param. Modules that need an update.
   * @param obErrors Out param. Errors in dependencies. Null if no errors.
   * @return ModuleInstallDetail with modulesToInstall, modulesToUpdate and isValidConfiguration
   */
  static public ModuleInstallDetail checkRemote(VariablesSecureApp vars, String[] moduleVersionId, String[] moduleVersionToUpdateId, OBError obErrors) throws Exception{
    WebServiceImplServiceLocator loc = new WebServiceImplServiceLocator();
    WebServiceImpl ws = (WebServiceImpl) loc.getWebService();
    String[] errors = new String[0];
    
    
    ModuleInstallDetail mid = ws.checkConsistency(ImportModule.getInstalledModules(pool), moduleVersionId, moduleVersionToUpdateId);
   ...
   * 
   * @param modulesToInstall In param. New modules to install, with its dependencies.
   * @param obErrors Out param. Errors in dependencies. Null if no errors.
   * @return true if all dependencies can be resolved without need of download
   *              any package from central repository, false in other case.
   */
  static public boolean checkLocal(VariablesSecureApp vars, Module[] modulesToInstall, OBError obErrors) throws Exception{
