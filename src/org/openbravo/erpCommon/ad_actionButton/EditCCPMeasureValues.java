...
public class EditCCPMeasureValues extends HttpSecureAppServlet {
    ...
    public void doPost(HttpServletRequest request, HttpServletResponse response)
        ...
        if (vars.commandIn("DEFAULT")) {
            ...
            String strKey = vars
                    .getRequiredStringParameter("inpmaMeasureShiftId");
            ...
            printPageDataSheet(response, vars, strKey, strWindowId, strTabId,
                    strProcessId, strMeasureDate, strShift);
            ...
            OBError myMessage = updateValues(vars, strValueId, strGroupId,
                    strKey, strWindowId);
    ...
    void printPageDataSheet(HttpServletResponse response,
            ...
            String strTabId, String strProcessId, String strMeasureDate,
            String strShift) throws IOException, ServletException {
        ...
        if (vars.getLanguage().equals("en_US"))
            ...
            shift = EditCCPMeasureValuesData.selectShiftTrl(this, strShift,
                    vars.getLanguage());
        XmlDocument xmlDocument = xmlEngine.readXmlTemplate(
                "org/openbravo/erpCommon/ad_actionButton/EditCCPMeasureValues",
        ...
        if (log4j.isDebugEnabled())
            log4j.debug("Param: " + vars.getLanguage() + " " + strReplaceWith
                    + " " + strKey + " " + strWindowId + " " + strTabId + " "
                    + strMeasureDate + " shift: " + shift + " " + strShift);
        ...
        for (int i = 0; i < data.length; i++) {
            dataHours[i] = EditCCPMeasureValuesHoursData.select(this,
                    data[i].groupid);
            if (dataHours[i] == null || dataHours[i].length == 0)
                dataHours[i] = new EditCCPMeasureValuesHoursData[0];

            dataValues[i] = EditCCPMeasureValuesValuesData.select(this,
                    data[i].groupid);
    ...
    OBError updateValues(VariablesSecureApp vars, String[] strValueId,
            String[] strGroupId, String strKey, String strWindowId) {
        ...
        try {
            ...
            for (int i = 0; i < strValueId.length; i++) {
                ...
                if (!strValueId[i].equals("0")) {
                    String type = EditCCPMeasureValuesData.selectType(this,
                            strValueId[i]);
                    if (type.equals("N"))
                        numeric = vars.getStringParameter("strValue"
                                + strValueId[i]);
                    else if (type.equals("T"))
                        text = vars.getStringParameter("strValue"
                                + strValueId[i]);
                    else
                        check = vars.getStringParameter("strValue"
                                + strValueId[i]);
                    if (log4j.isDebugEnabled())
                        log4j.debug("Values to update: " + strValueId[i] + ", "
                                + numeric + ", " + text + ", " + check);
                    try {
                        total = EditCCPMeasureValuesData.update(conn, this,
                                numeric, text, check.equals("Y") ? "Y" : "N",
                                strValueId[i]);
