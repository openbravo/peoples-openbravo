...
/*        strBuf = strBuf.append("5676").append(Principio[0].nif).append("000").append(Lineas[i].nFactura).append(Lineas[i].direccion);
  ...
  void printPageFind34(HttpServletResponse response, VariablesSecureApp vars, String strKey) throws IOException, ServletException {
    ...
    */
    ...
    if (log4j.isDebugEnabled()) log4j.debug("Principio[0].acct = " + Principio[0].acct);
    if (log4j.isDebugEnabled()) log4j.debug("check1 ok");
    ...
    for (int i=0;i<Lineas.length;i++){
        //debugging lines
          ...
          if (log4j.isDebugEnabled()) log4j.debug("Lineas[i].acct=" + Lineas[i].acct);
          if (log4j.isDebugEnabled()) log4j.debug("Lineas[i].nom=" + Lineas[i].nom);
          if (log4j.isDebugEnabled()) log4j.debug("Lineas[i].dom=" + Lineas[i].dom);
          if (log4j.isDebugEnabled()) log4j.debug("Lineas[i].pla=" + Lineas[i].pla);
          if (log4j.isDebugEnabled()) log4j.debug("Lineas[i].prov=" + Lineas[i].prov);
