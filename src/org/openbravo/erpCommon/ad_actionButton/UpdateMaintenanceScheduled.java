...
public class UpdateMaintenanceScheduled extends HttpSecureAppServlet {
    ...
    public void doPost(HttpServletRequest request, HttpServletResponse response)
        ...
        if (vars.commandIn("DEFAULT")) {
            vars.getRequestGlobalVariable("inppartdate",
                    "UpdateMaintenanceScheduled|inppartdate");
            ...
            printPageDataSheet(response, vars, strKey, strWindowId, strTabId,
                    strPartDateFrom, strPartDateTo, strMaintType);
    ...
    void printPageDataSheet(HttpServletResponse response,
            ...
            String strTabId, String strPartDateFrom, String strPartDateTo,
        ...
        if (strPartDateTo == null)
            strPartDateTo = "";
        data = UpdateMaintenanceScheduledData.select(this, vars.getLanguage(),
                strPartDateFrom, strPartDateTo, strMaintType);

        XmlDocument xmlDocument = xmlEngine
                .readXmlTemplate(
                        "org/openbravo/erpCommon/ad_actionButton/UpdateMaintenanceScheduled",
        ...
        try {
            ComboTableData comboTableData = new ComboTableData(vars, this,
                    "LIST", "", "Maintenance type", "", Utility.getContext(
                            this, vars, "#User_Org",
                            "UpdateMaintenanceScheduled"), Utility.getContext(
                            this, vars, "#User_Client",
                            "UpdateMaintenanceScheduled"), 0);
            Utility.fillSQLParameters(this, vars, null, comboTableData,
                    "UpdateMaintenanceScheduled", strMaintType);
            xmlDocument.setData("reportMaintType", "liststructure",
    ...
    OBError updateValues(HttpServletRequest request, VariablesSecureApp vars,
            String strKey) {
        ...
        try {
            ...
            for (int i = 0; i < strValueId.length; i++) {
                ...
                String done = vars
                        .getStringParameter("strDone" + strValueId[i]);
                if (done == null)
                    done = "";
                String result = vars.getStringParameter("strResult"
                        + strValueId[i]);
                if (result == null)
                    result = "";
                String usedtime = vars.getStringParameter("strUsedTime"
                        + strValueId[i]);
                String observation = vars.getStringParameter("strObservation"
                        + strValueId[i]);
                if (done.equals("Y")) {
                    if (log4j.isDebugEnabled())
                        log4j
                                .debug("Values to update: " + strValueId[i]
                                        + ", " + result + ", " + usedtime
                                        + ", " + observation);
                    try {
                        UpdateMaintenanceScheduledData.update(conn, this,
                                result.equals("Y") ? "Y" : "N", usedtime,
                                observation, vars.getUser(), strKey,
                                strValueId[i]);
