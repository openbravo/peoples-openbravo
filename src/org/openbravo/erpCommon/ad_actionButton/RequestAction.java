...
public class RequestAction extends HttpSecureAppServlet {
  ...
  String processButton(VariablesSecureApp vars, String strKey, String windowId) {
    ...
    try {
      ...
      if(data[0].actiontype.equals("E")) {
        ...
        if (from.equals("")) {
          from = System.getProperty("user.name") + "@" + Utility.getContext(this, vars, "#AD_Client_Name", windowId) + ".com";
          from = from.trim().toLowerCase();
          for(int pos = from.indexOf(" "); pos != -1; pos = from.indexOf(" "))
            from = from.substring(0, pos) + from.substring(pos + 1);
        }
        String to = RequestActionData.selectEmailTo(conn, this, (data[0].adUserId.equals("0")?"":data[0].adUserId), data[0].cBpartnerId);
        String msg = "OK";
        if (log4j.isDebugEnabled()) log4j.debug("*************************************************************************************************");
        if (log4j.isDebugEnabled()) log4j.debug(smtpHost + "\n" + from + "\n" + to + "\n" + data[0].mailsubject + "\n" + data[0].mailtext);
        if (log4j.isDebugEnabled()) log4j.debug("*************************************************************************************************");
        EMail email = new EMail(vars, smtpHost, from, to, data[0].mailsubject, data[0].mailtext);
        msg = email.send();
        if("OK".equals(msg)) {
          RequestActionData.update(conn, this, Utility.messageBD(this, "RequestActionEMailOK", vars.getLanguage()), strKey);
        } else {
          RequestActionData.update(conn, this, Utility.messageBD(this, "RequestActionEMailError", vars.getLanguage()) + " - " + msg, strKey);
        }
        ...
        if (to.equals("")) {
          to = System.getProperty("user.name") + "@" + Utility.getContext(this, vars, "#AD_Client_Name", windowId) + ".com";
          to = to.trim().toLowerCase();
          for(int pos = to.indexOf(" "); pos != -1; pos = to.indexOf(" "))
            to = to.substring(0, pos) + to.substring(pos + 1);
        }
  ...
  void printPage(HttpServletResponse response, VariablesSecureApp vars, String strKey, String windowId, String strTab, String strProcessId)
    throws IOException, ServletException {
      ...
      XmlDocument xmlDocument = xmlEngine.readXmlTemplate("org/openbravo/erpCommon/ad_actionButton/RequestAction", discard).createXmlDocument();
