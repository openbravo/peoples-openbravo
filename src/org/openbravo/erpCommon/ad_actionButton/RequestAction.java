...
public class RequestAction extends HttpSecureAppServlet {
    ...
    public void doPost(HttpServletRequest request, HttpServletResponse response)
        ...
        if (vars.commandIn("DEFAULT")) {
            ...
            String strKey = vars.getRequiredGlobalVariable("inprRequestId",
                    strWindow + "|R_Request_ID");
            printPage(response, vars, strKey, strWindow, strTab, strProcessId);
        } else if (vars.commandIn("SAVE")) {
            String strWindow = vars.getStringParameter("inpwindowId");
            String strKey = vars.getRequestGlobalVariable("inprRequestId",
                    strWindow + "|R_Request_ID");
            String strTab = vars.getStringParameter("inpTabId");
            ...
            PInstanceProcessData.insertPInstance(this, pinstance, "158",
                ...
                if (!pinstanceData[0].errormsg.equals("")) {
                    ...
                    if (message.startsWith("@") && message.endsWith("@")) {
                        ...
                        if (message.indexOf("@") == -1)
                            messageResult = Utility.messageBD(this, message,
                                    vars.getLanguage());
                        else
                            messageResult = Utility.parseTranslation(this,
                                    vars, vars.getLanguage(), "@" + message
                                            + "@");
                    } else {
                        messageResult = Utility.parseTranslation(this, vars,
                                vars.getLanguage(), message);
                    }
                } else if (!pinstanceData[0].pMsg.equals("")) {
                    String message = pinstanceData[0].pMsg;
                    messageResult = Utility.parseTranslation(this, vars, vars
                            .getLanguage(), message);
                } else if (pinstanceData[0].result.equals("1")) {
                    messageResult = Utility.messageBD(this, "Success", vars
                            .getLanguage());
                } else {
                    messageResult = Utility.messageBD(this, "Error", vars
                            .getLanguage());
                }
            }
    ...
    String processButton(VariablesSecureApp vars, String strKey, String windowId) {
        ...
        try {
            ...
            RequestActionData[] data = RequestActionData.select(conn, this,
                    strKey);
            if (data == null || data.length == 0) {
                releaseRollbackConnection(conn);
                log4j.warn("Rollback in transaction");
                return Utility.messageBD(this, "ProcessRunError", vars
                        .getLanguage());
            }
            ...
            if (data[0].actiontype.equals("E")) {
                String smtpHost = RequestActionData.selectSMTPHost(conn, this,
                        data[0].adClientId);
                String from = "";
                RequestActionData[] mails = RequestActionData.selectEmail(conn,
                        this, data[0].salesrepId);
                if (mails == null || mails.length == 0) {
                    mails = RequestActionData.selectEmailRequest(conn, this,
                            data[0].adClientId);
                    if (mails != null && mails.length > 0) {
                        from = mails[0].email;
                        ...
                        for (int pos = from.indexOf(" "); pos != -1; pos = from
                            ...
                            from = from.substring(0, pos)
                                    + from.substring(pos + 1);
                        vars.setSessionValue("#Request_EMail", from);
                        vars.setSessionValue("#Request_EMailUser",
                                mails[0].emailuser);
                        vars.setSessionValue("#Request_EMailUserPw",
                                mails[0].emailuserpw);
                    }
                } else {
                    from = mails[0].email;
                    ...
                    vars.setSessionValue("#User_EMailUserPw",
                            mails[0].emailuserpw);
                }
                ...
                if (from.equals("")) {
                    from = System.getProperty("user.name")
                            + "@"
                            + Utility.getContext(this, vars, "#AD_Client_Name",
                                    windowId) + ".com";
                    from = from.trim().toLowerCase();
                    for (int pos = from.indexOf(" "); pos != -1; pos = from
                            .indexOf(" "))
                ...
                String to = RequestActionData.selectEmailTo(conn, this,
                        (data[0].adUserId.equals("0") ? "" : data[0].adUserId),
                        data[0].cBpartnerId);
                String msg = "OK";
                if (log4j.isDebugEnabled())
                    log4j
                ...
                if (log4j.isDebugEnabled())
                    log4j
                            .debug("*************************************************************************************************");
                EMail email = new EMail(vars, smtpHost, from, to,
                        data[0].mailsubject, data[0].mailtext);
                msg = email.send();
                if ("OK".equals(msg)) {
                    RequestActionData.update(conn, this, Utility.messageBD(
                            this, "RequestActionEMailOK", vars.getLanguage()),
                            strKey);
                } else {
                    RequestActionData.update(conn, this, Utility
                            .messageBD(this, "RequestActionEMailError", vars
                                    .getLanguage())
                            + " - " + msg, strKey);
                }
            } else if (data[0].actiontype.equals("T")) {
                String subject = Utility.messageBD(this,
                        "RequestActionTransfer", vars.getLanguage());

                RequestActionData.update(conn, this, subject, strKey);
                String smtpHost = RequestActionData.selectSMTPHost(conn, this,
                        data[0].adClientId);
                String to = "";
                RequestActionData[] dataTo = RequestActionData.selectEmail(
                        conn, this, data[0].adUserId);
                if (dataTo != null && dataTo.length > 0) {
                    to = dataTo[0].email;
                }
                to = to.trim().toLowerCase();
                for (int pos = to.indexOf(" "); pos != -1; pos = to
                        .indexOf(" "))
                ...
                if (log4j.isDebugEnabled())
                    log4j
                            .debug("*************************************************************************************************");
                EMail email = new EMail(vars, smtpHost, from, to, subject,
                        message);
                email.send();
            }
    ...
    void printPage(HttpServletResponse response, VariablesSecureApp vars,
        ...
        XmlDocument xmlDocument = xmlEngine.readXmlTemplate(
                "org/openbravo/erpCommon/ad_actionButton/RequestAction",
