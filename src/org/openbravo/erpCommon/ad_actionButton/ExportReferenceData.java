...
public class ExportReferenceData extends HttpSecureAppServlet {
	...
	public void doPost(HttpServletRequest request, HttpServletResponse response)
		...
		if (vars.commandIn("DEFAULT")) {
			vars.getGlobalVariable("inpProcessId",
					"ExportReferenceData|AD_Process_ID");
			vars.getGlobalVariable("inpwindowId",
					"ExportReferenceData|Window_ID");
			vars.getGlobalVariable("inpTabId", "ExportReferenceData|Tab_ID");
			String strDataSet = vars.getRequiredGlobalVariable(
					"inpadDatasetId", "ExportReferenceData|AD_DataSet_ID");
			...
			String strTab = vars.getGlobalVariable("inpTabId",
					"ExportReferenceData|Tab_ID");
			String strKey = vars.getRequestGlobalVariable("inpKey",
	...
	OBError processButton(VariablesSecureApp vars, String strKey) {
		...
		try {
			DataSet myDataset = (DataSet) OBDal.getInstance().get(
					DataSet.class, strKey);
			ExportReferenceDataData[] data = ExportReferenceDataData
					.selectDataset(this, strKey);
			if(data==null || data.length==0) return Utility.translateError(this, vars, vars.getLanguage(),
      "ProcessRunError");
			ExportReferenceDataData[] module = ExportReferenceDataData
					.selectModule(this, data[0].adModuleId);
			...
			File myFolder = new File(vars.getSessionValue("#sourcePath")
					+ "/modules/" + module[0].javapackage
					+ "/referencedata/standard");
			File myFile = new File(vars.getSessionValue("#sourcePath")
					+ "/modules/" + module[0].javapackage
					+ "/referencedata/standard/"
					+ Utility.wikifiedName(data[0].name) + ".xml");
			if (!myFolder.exists())
				myFolder.mkdirs();
	...
	void printPage(HttpServletResponse response, VariablesSecureApp vars,
			String strDataSetID) throws IOException, ServletException {
		if (log4j.isDebugEnabled())
			log4j.debug("Output: Button process Copy from Settlement");

		XmlDocument xmlDocument = xmlEngine.readXmlTemplate(
				"org/openbravo/erpCommon/ad_actionButton/ExportReferenceData")
				.createXmlDocument();
