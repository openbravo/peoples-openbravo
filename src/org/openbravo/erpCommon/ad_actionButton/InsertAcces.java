...
public class InsertAcces extends HttpSecureAppServlet {
    ...
    public void doPost(HttpServletRequest request, HttpServletResponse response)
        ...
        if (vars.commandIn("DEFAULT")) {
            ...
            final String strKey = vars.getGlobalVariable("inpadRoleId",
                    strWindow + "|AD_Role_ID");
            ...
            final ActionButtonDefaultData[] tab = ActionButtonDefaultData
                    .windowName(this, strTab);
            ...
            final OBError myMessage = getPrintPage(response, vars, strKey,
                    strModule, strType);
    ...
    void printPage(HttpServletResponse response, VariablesSecureApp vars,
            ...
            String strMessage, String strTab) throws IOException,
            ServletException {
        ...
        final XmlDocument xmlDocument = xmlEngine.readXmlTemplate(
                "org/openbravo/erpCommon/ad_actionButton/InsertAcces", discard)
                .createXmlDocument();
        ...
        xmlDocument.setParameter("language", "defaultLang=\""
                + vars.getLanguage() + "\";");
        xmlDocument.setData("reportModules_S", "liststructure", ModuleComboData
                .select(this));
    ...
    OBError getPrintPage(HttpServletResponse response, VariablesSecureApp vars,
            String strKey, String strModule, String strType)
            throws IOException, ServletException {
    ...
    public void generateAcces(VariablesSecureApp vars,
            InsertAccesData[] accesData, String roleid, String indice,
            String strType) throws ServletException {
        log4j.error("longitud accesdata: " + accesData.length + " indice: "
                + indice + " roleid: " + roleid);
        for (int i = 0; i < accesData.length; i++) {
            if (accesData[i].parentId.equals(indice)) {
                if (accesData[i].issummary.equals("Y"))
                    generateAcces(vars, accesData, roleid, accesData[i].nodeId,
                            strType);
                else {
                    if (accesData[i].action.equals("W")
                            && (InsertAccesData.selectWindow(this, roleid,
                                    accesData[i].adwindowid) == null || InsertAccesData
                                    .selectWindow(this, roleid,
                                            accesData[i].adwindowid).equals(""))
                            && (strType.equals("W") || strType.equals(""))) {
                        log4j.error("Action: " + accesData[i].action
                                + " window: " + accesData[i].adwindowid);
                        InsertAccesData.insertWindow(this,
                                accesData[i].adwindowid, roleid, vars
                                        .getClient(), "0", vars.getUser());
                    } else if (accesData[i].action.equals("P")
                            && (InsertAccesData.selectProcess(this, roleid,
                                    accesData[i].adprocessid) == null || InsertAccesData
                                    .selectProcess(this, roleid,
                                            accesData[i].adprocessid)
                                    .equals(""))
                            && (strType.equals("P") || strType.equals(""))) {
                        log4j.error("Action: " + accesData[i].action
                                + " process: " + accesData[i].adprocessid);
                        InsertAccesData.insertProcess(this,
                                accesData[i].adprocessid, roleid, vars
                                        .getClient(), "0", vars.getUser());
                        ...
                        log4j.error("Action: " + accesData[i].action
                                + " report: " + accesData[i].adprocessid);
                        InsertAccesData.insertProcess(this,
                                accesData[i].adprocessid, roleid, vars
                                        .getClient(), "0", vars.getUser());
                    } else if (accesData[i].action.equals("X")
                            && (InsertAccesData.selectForm(this, roleid,
                                    accesData[i].adformid) == null || InsertAccesData
                                    .selectForm(this, roleid,
                                            accesData[i].adformid).equals(""))
                            && (strType.equals("X") || strType.equals(""))) {
                        log4j.error("Action: " + accesData[i].action
                                + " form: " + accesData[i].adformid);
                        InsertAccesData.insertForm(this, accesData[i].adformid,
                                roleid, vars.getClient(), "0", vars.getUser());
                    } else if (accesData[i].action.equals("F")
                            && (InsertAccesData.selectWorkflow(this, roleid,
                                    accesData[i].adworkflowid) == null || InsertAccesData
                                    .selectWorkflow(this, roleid,
                                            accesData[i].adworkflowid).equals(
                                            ""))
                            && (strType.equals("F") || strType.equals(""))) {
                        log4j.error("Action: " + accesData[i].action
                                + " workflow: " + accesData[i].adworkflowid);
                        InsertAccesData.insertWorkflow(this,
                                accesData[i].adworkflowid, roleid, vars
                                        .getClient(), "0", vars.getUser());
                    } else if (accesData[i].action.equals("T")
                            && (InsertAccesData.selectTask(this, roleid,
                                    accesData[i].adtaskid) == null || InsertAccesData
                                    .selectTask(this, roleid,
                                            accesData[i].adtaskid) == null)
                            && (strType.equals("T") || strType.equals(""))) {
                        log4j.error("Action: " + accesData[i].action
                                + " task: " + accesData[i].adtaskid);
                        InsertAccesData.insertTask(this, accesData[i].adtaskid,
                                roleid, vars.getClient(), "0", vars.getUser());
                    }
                }
            }
