...
public class Posted extends HttpSecureAppServlet {
    ...
    public void doPost(HttpServletRequest request, HttpServletResponse response)
        ...
        if (vars.commandIn("DEFAULT")) {
            ...
            String strTableId = vars.getGlobalVariable("inpTableId",
                    "Posted|tableId");
            String strTabId = vars
                    .getGlobalVariable("inpTabId", "Posted|tabId");
            String strPosted = vars.getGlobalVariable("inpPosted",
                    "Posted|posted");
            ...
            String strWindowId = vars.getGlobalVariable("inpWindowId",
                    "Posted|windowId", "");
            String strTabName = vars.getGlobalVariable("inpTabName",
                    "Posted|tabName", "");

            printPage(response, vars, strKey, strWindowId, strTabId,
                    strProcessId, strTableId, strPath, strTabName, strPosted);
        } else if (vars.commandIn("SAVE")) {

            String strKey = vars.getRequiredGlobalVariable("inpKey",
                    "Posted|key");
            String strTableId = vars.getRequiredGlobalVariable("inpTableId",
                    "Posted|tableId");
            String strTabId = vars.getRequestGlobalVariable("inpTabId",
                    "Posted|tabId");
            String strPosted = vars.getRequiredGlobalVariable("inpPosted",
                    "Posted|posted");
            ...
            if (log4j.isDebugEnabled())
                log4j.debug("SAVE, strPosted: " + strPosted + " Elim "
                        + strEliminar);
            ...
            if (strPosted.equals("N")) {
                ...
                if (!messageResult.getType().equals("Success")) {
                    ...
                    PostedData[] data = PostedData.select(this, strKey,
                            strTableId);
                ...
                if (strEliminar.equals("N")) {
                    ...
                    if (data == null || data.length == 0
                        ...
                        // vars.getLanguage()));
                        vars.setMessage(strTabId, Utility.translateError(this,
                                vars, vars.getLanguage(), "NoFactAcct"));
                        printPageClosePopUp(response, vars);
                    } else {
                        printPageClosePopUp(
                                response,
                    ...
                    OBError myMessage = processButtonDelete(vars, strKey,
                            strTableId);
    ...
    OBError processButton(VariablesSecureApp vars, String strKey,
            String strTableId) throws ServletException {
        if (log4j.isDebugEnabled())
            log4j.debug("ProcessButton strKey: " + strKey + "strTableId: "
                    + strTableId);
        String strOrg;
        Connection con = null;
        OBError myMessage = null;

        strOrg = PostedData.selectDocOrg(this, PostedData.selectTableName(this,
                strTableId), strKey);
        if (strOrg == null)
            strOrg = "0";
    ...
    OBError processButtonDelete(VariablesSecureApp vars, String strKey,
            String strTableId) throws ServletException {
        OBError myMessage = null;

        try {

            String strClient = PostedData.selectClient(this, PostedData
                    .selectTableName(this, strTableId), strKey);
            String pinstance = SequenceIdData.getUUID();
            PInstanceProcessData.insertPInstance(this, pinstance, "176",
                    strKey, "N", vars.getUser(), vars.getClient(), vars
                            .getOrg());
            PInstanceProcessData.insertPInstanceParam(this, pinstance, "10",
                    "AD_Client_ID", strClient, vars.getClient(), vars.getOrg(),
                    vars.getUser());
            PInstanceProcessData.insertPInstanceParam(this, pinstance, "20",
                    "AD_Table_ID", strTableId, vars.getClient(), vars.getOrg(),
                    vars.getUser());
            PInstanceProcessData.insertPInstanceParam(this, pinstance, "30",
                    "DeletePosting", "Y", vars.getClient(), vars.getOrg(), vars
                            .getUser());
            if (log4j.isDebugEnabled())
    ...
    void printPage(HttpServletResponse response, VariablesSecureApp vars,
            ...
            String strTableId, String strPath, String strTabName,
        ...
        XmlDocument xmlDocument = xmlEngine.readXmlTemplate(
                "org/openbravo/erpCommon/ad_actionButton/Posted", discard)
                .createXmlDocument();
        ...
        /*
         * String message = vars.getSessionValue("Posted|message"); if
         * (!message.equals("")) message = "alert('" + message + "');";
