...
public class CreateVatRegisters extends HttpSecureAppServlet {
    ...
    public void doPost(HttpServletRequest request, HttpServletResponse response)
        ...
        if (vars.commandIn("DEFAULT")) {
            ...
            String strGeneratePayment = taxpayment[0]
                    .getField("GENERATEPAYMENT");
            String strProcessing = taxpayment[0].getField("PROCESSING");
            String strWindowId = vars.getStringParameter("inpWindowId");
            String strTabId = vars.getStringParameter("inpTabId");
            printPage(response, vars, strWindowId, strTabId, strDatefrom,
                    strDateto, strGeneratePayment, strProcessed, strProcessing,
                    strTaxpaymentID);
            ...
            String strGeneratePayment = vars
                    .getStringParameter("inpGeneratePayment");
            String strProcessing = vars.getStringParameter("inpProcessing");
            OBError myMessage = CreateRegisters(vars, strTaxpaymentID,
                    strDatefrom, strDateto, strProcessed, strGeneratePayment,
                    strProcessing);
            // try this
     ...
     * strDatefrom, strDateto, strGeneratepayment, strProcessed, strProcessing);
     * //alp vars.setMessage("CreateVatRegisters", myMessage);
    ...
    public OBError CreateRegisters(VariablesSecureApp vars,
            String strTaxpaymentID, String strDatefrom, String strDateto,
            String strProcessed, String strGeneratePayment, String strProcessing)
            throws IOException, ServletException {
        // Connection conn = getTransactionConnection();
        OBError myMessage = null;
        TaxPayment[] taxpayment = TaxPayment.select(this, strTaxpaymentID);
        String strUser = vars.getUser();
        log4j.info("strTaxpaymentID: " + strTaxpaymentID + "strDatefrom: "
                + strDatefrom + "strDateto: " + strDateto + "strProcessed: "
                + strProcessed + "strGeneratePayment: " + strGeneratePayment);
	    ...
	    BigDecimal CrossPeriodCount = new BigDecimal(TaxPayment
            ...
            TaxRegisterType[] taxregistertypes = TaxRegisterType.select(this,
                    vars.getClient());
            log4j.info("2strTaxpaymentID: " + strTaxpaymentID + "strDatefrom: "
                    + strDatefrom + "strDateto: " + strDateto
                    + "strProcessed: " + strProcessed + "strGeneratePayment: "
                    + strGeneratePayment);
            ...
            for (TaxRegisterType taxRegisterType : taxregistertypes) {
                ...
                try {
                    TaxRegister.insert(this, taxpayment[0].adClientId,
                            taxpayment[0].adOrgId, strSequence,
                            strTaxpaymentID,
                            taxRegisterType.cTaxregisterTypeId, "0",
            ...
            // TaxRegisterLine
            log4j.info("3strTaxpaymentID: " + strTaxpaymentID + "strDatefrom: "
                    + strDatefrom + "strDateto: " + strDateto
                    + "strProcessed: " + strProcessed + "strGeneratePayment: "
                    + strGeneratePayment);
            ...
            for (TaxRegister taxRegister : taxregisters) {
                CreateVatRegistersData[] invoices = CreateVatRegistersData
                        .select(this, strTaxpaymentID,
                ...
                for (CreateVatRegistersData myinvoice : invoices) {
                    ...
                    log4j.info("cTaxregisterTypeId: "
                            + taxRegister.cTaxregisterTypeId
                            + "strTaxpaymentID: " + strTaxpaymentID
                            + "strDatefrom: " + strDatefrom + "strDateto: "
                            + strDateto + "strGeneratePayment: "
                            + strGeneratePayment);
                    if ((myinvoice.istaxexempt.equals("N"))
                        ...
                        myMessage = Utility.translateError(this, vars, vars
                                .getLanguage(), Utility.messageBD(this,
                                "TaxCriteriaNotFound", vars.getLanguage()));
                        return myMessage;
                    }
                    if (myinvoice.docbasetype
				...
				.toPlainString();
                    ...
                    // Calculate totalamt
                    log4j.info("4strTaxpaymentID: " + strTaxpaymentID
                            + "strDatefrom: " + strDatefrom + "strDateto: "
			    ...
			    new BigDecimal(strNoVatAmt));
                    ...
                    try {
                        TaxRegister.insertLines(this, taxRegister.adClientId,
                                taxRegister.adOrgId, strSequence,
                                taxRegister.cTaxregisterId,
                                myinvoice.cInvoicetaxId, myinvoice.documentno,
                                myinvoice.cTaxId, strTaxBaseAmt, strTaxAmt,
                                strTaxUndeducAmt, strExemptAmt, strNoVatAmt,
                                strTotalAmt, myinvoice.taxdate, "RegisterLine",
                                strUser, strUser);
                    } catch (ServletException ex) {
                        myMessage = Utility.translateError(this, vars, vars
                                .getLanguage(), ex.getMessage());
                ...
                try {
                    TaxRegister.updateTaxTotalAmt(this,
                            taxRegister.cTaxregisterId);
                    TaxRegister.updateRegAccumAmt(this,
                            taxRegister.cTaxregisterId,
                            taxRegister.cTaxregisterTypeId, strDatefrom);
                } catch (ServletException ex) {
                    myMessage = Utility.translateError(this, vars, vars
            ...
            // }
            log4j.info("5strTaxpaymentID: " + strTaxpaymentID + "strDatefrom: "
                    + strDatefrom + "strDateto: " + strDateto
                    + "strProcessed: " + strProcessed + "strGeneratePayment: "
                    + strGeneratePayment);
			...
			strTaxpaymentID)).compareTo(BigDecimal.ZERO) > 0) {
                    ...
                    TaxPayment.updateGeneratePayment(this, "N", strUser,
                            strTaxpaymentID);
            ...
            myMessage = Utility.translateError(this, vars, vars.getLanguage(),
                    Utility.messageBD(this, "ProcessRunError", vars
                            .getLanguage()));
        return myMessage;
    ...
    void printPage(HttpServletResponse response, VariablesSecureApp vars,
            String strWindowId, String strTabId, String strDatefrom,
            String strDateto, String strGeneratePayment, String strProcessed,
            String strProcessing, String strTaxpaymentID) throws IOException,
            ServletException {
        if (log4j.isDebugEnabled())
            log4j.debug("Output: Print Vat Registers confirm window");
        /*
         * ActionButtonDefaultData[] data = null; String strHelp="",
         * strDescription=""; if (vars.getLanguage().equals("en_US")) data =
         * ActionButtonDefaultData.select(this, strProcessId); else data =
         * ActionButtonDefaultData.selectLanguage(this, vars.getLanguage(),
         * strProcessId);
         * 
         * if (data!=null && data.length!=0) { strDescription =
         * data[0].description; strHelp = data[0].help; }
         */
        String[] discard = { "" };
        // if (strHelp.equals("")) discard[0] = new String("helpDiscard");
        discard[0] = new String("helpDiscard");
        XmlDocument xmlDocument = xmlEngine.readXmlTemplate(
                "org/openbravo/erpCommon/ad_actionButton/CreateVatRegisters",
