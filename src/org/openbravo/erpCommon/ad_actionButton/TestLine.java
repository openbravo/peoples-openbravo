...
public class TestLine extends MultipartRequest implements FieldProvider {
    ...
    public TestLine(VariablesBase _vars, String _filename,
            boolean _firstLineHeads, String _currentWindow,
            String _currentFrame, boolean _generatedWindow,
    ...
    public TestLine(String _currentWindow, String _currentFrame,
            boolean _generatedWindow, ConnectionProvider _conn) {
        this.currentWindow = _currentWindow;
        this.currentFrame = _currentFrame;
        this.generatedWindow = _generatedWindow;
        this.conn = _conn;
    ...
    public FieldProvider lineSeparatorFormated(String linea) {
        if (linea.length() < 1)
            return null;
        TestLine TestLine = new TestLine(currentWindow, currentFrame,
                generatedWindow, conn);
        ...
        if (matcher.find()) {
            if (generatedWindow) {
                ...
                if (matcher.group(1).equals("user")
                        || matcher.group(1).equals("password"))
                    TestLine.comando = "800152";
                else if (isCheckTextField(currentTab,
                        argErase(matcher.group(1))))
        ...
        pattern = Pattern
                .compile("selectItem\\(\"report(.+?)_S\",\"(.+?)\",[0-9]{1,2}\\)");
        matcher = pattern.matcher("");
        matcher.reset(linea);
        if (matcher.find()) {
        ...
        pattern = Pattern
                .compile("clickImage\\(\"(command.+?)\",[0-9]{1,2}\\)");
        matcher = pattern.matcher("");
        matcher.reset(linea);
        if (matcher.find()) {
        ...
        pattern = Pattern
                .compile("clickImage\\(\"openbravoLogo\",[0-9]{1,2}\\)");
        matcher = pattern.matcher("");
        matcher.reset(linea);
        if (matcher.find()) {
        ...
        pattern = Pattern
                .compile("clickElement\\(\"tabtitle(.+?)\",[0-9]{1,2}\\)");
        matcher = pattern.matcher("");
        matcher.reset(linea);
        if (matcher.find()) {
        ...
        pattern = Pattern
                .compile("clickLink\\(\"childwindow(.+?)\",[0-9]{1,2}\\)");
        matcher = pattern.matcher("");
        matcher.reset(linea);
        if (matcher.find()) {
        ...
        pattern = Pattern
                .compile("clickLink\\(\"childprocess(.+?)\",[0-9]{1,2}\\)");
        matcher = pattern.matcher("");
        matcher.reset(linea);
        if (matcher.find()) {
        ...
        pattern = Pattern
                .compile("clickLink\\(\"childreport(.+?)\",[0-9]{1,2}\\)");
        matcher = pattern.matcher("");
        matcher.reset(linea);
        if (matcher.find()) {
        ...
        pattern = Pattern
                .compile("clickLink\\(\"childform(.+?)\",[0-9]{1,2}\\)");
        matcher = pattern.matcher("");
        matcher.reset(linea);
        if (matcher.find()) {
        ...
        pattern = Pattern
                .compile("clickLink\\(\"childtask(.+?)\",[0-9]{1,2}\\)");
        matcher = pattern.matcher("");
        matcher.reset(linea);
        if (matcher.find()) {
        ...
        pattern = Pattern
                .compile("clickLink\\(\"folderHref(.+?)\",[0-9]{1,2}\\)");
        matcher = pattern.matcher("");
        matcher.reset(linea);
        if (matcher.find()) {
        ...
        pattern = Pattern
                .compile("clickButton\\(\"(inp.+?)_?([0-9]{1,2})?\",[0-9]{1,2}\\)");
        matcher = pattern.matcher("");
        matcher.reset(linea);
        if (matcher.find()) {
        ...
        pattern = Pattern
                .compile("clickButton\\(\"param(.+?)_M_?([0-9]{1,2})?\",[0-9]{1,2}\\)");
        matcher = pattern.matcher("");
        matcher.reset(linea);
        if (matcher.find()) {
        ...
        // REPETITON
        pattern = Pattern
                .compile("clickButton\\(\"(.+?)_?([0-9]{1,2})?\",[0-9]{1,2}\\)");
        matcher = pattern.matcher("");
        matcher.reset(linea);
        if (matcher.find()) {
        ...
        pattern = Pattern
                .compile("setWindow\\(\\s\"(.+?)_([A-Z]+)_?([0-9]{1,2})?\",[0-9]{1,2}\\)");// gorka
        matcher = pattern.matcher("");
        matcher.reset(linea);
        if (matcher.find()) {
        ...
        pattern = Pattern
                .compile("setWindow\\(\\s\"(.+?)_([\\w]+?)_?([0-9]{1,2})?\",[0-9]{1,2}\\)");
        matcher = pattern.matcher("");
        matcher.reset(linea);
        if (matcher.find()) {
            int compWindow = matcher.group(1).compareTo(currentWindow);
