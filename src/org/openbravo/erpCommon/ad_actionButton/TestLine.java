...
public class TestLine extends MultipartRequest implements FieldProvider {
  ...
  public TestLine(VariablesBase _vars, String _filename, boolean _firstLineHeads, String _currentWindow, String _currentFrame, boolean _generatedWindow, ConnectionProvider _conn) throws IOException {
    super();
  ...
  public TestLine(String _currentWindow, String _currentFrame, boolean _generatedWindow, ConnectionProvider _conn) {
    this.currentWindow = _currentWindow;
    this.currentFrame = _currentFrame;
    this.generatedWindow= _generatedWindow;
    this.conn=_conn;
  ...
  public FieldProvider lineSeparatorFormated(String linea) {
    ...
    if (linea.substring(0,2).equals("//")) return TestLine;
    ...
    if (matcher.find()) {
      if (generatedWindow) {
        // To make it get all the login fields, because it's still unknown if it's generated
        if (matcher.group(1).equals("user")||matcher.group(1).equals("password")) TestLine.comando = "800152";
        else if(isCheckTextField(currentTab, argErase(matcher.group(1)))) TestLine.comando = "800108";
        else TestLine.comando = "800156";
      } else TestLine.comando = "800152";
      //if(!generatedWindow) TestLine.comando = "800162"; The textarea is not considered in non-generated
    ...
    if (matcher.find()) {
      ...
      if (generatedWindow) {
        try {currentTab = TestLineData.setTabId(conn, matcher.group(1));}
    ...
    //We capture the static-checkbox set's click. READY FOR THE "_" BY REPETITON
    ...
    if (matcher.find()) {
      ...
      if (compWindow!=0){
        ...
        if (!currentWindow.equals("Aplicación de gestión") && matcher.group(1).equals("Aplicación de gestión")) generatedWindow=generatedWindowPopup;
        else if (currentWindow.equals("Aplicación de gestión") && !matcher.group(1).equals("Aplicación de gestión")) {
  ...
  public boolean checkIsGenerated (String element,String generatedId) {
    ...
    if(element.equals("window")){
      try {comprobar = TestLineData.selectWindow(conn, generatedId);}
      catch (ServletException ex) {ex.printStackTrace();}
    ...
    if(element.equals("process")){
      try {comprobar = TestLineData.selectProcess(conn, generatedId);}
      catch (ServletException ex) {ex.printStackTrace();}
      if(comprobar.length==1){
        return true;
      }
    ...
    if(element.equals("windowtab")){
      try {comprobar = TestLineData.selectWindowTab(conn, generatedId);}
      catch (ServletException ex) {ex.printStackTrace();}
      if(comprobar.length==1){
        return true;
      }
  ...
  public boolean isCheckTextField (String tab,String column) {
    ...
    private String argErase(String arg) {
      if (arg.endsWith("_D")) arg = arg.substring(0, arg.lastIndexOf("_"));
      else if (arg.endsWith("_BTN")) arg = arg.substring(0, arg.lastIndexOf("_"));
