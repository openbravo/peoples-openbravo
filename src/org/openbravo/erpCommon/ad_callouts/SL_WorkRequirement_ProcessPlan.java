...
public class SL_WorkRequirement_ProcessPlan extends HttpSecureAppServlet {
  ...
  void printPage(HttpServletResponse response, VariablesSecureApp vars, String strProcessPlan) throws IOException, ServletException {
    if (log4j.isDebugEnabled()) log4j.debug("Output: dataSheet");
    XmlDocument xmlDocument = xmlEngine.readXmlTemplate("org/openbravo/erpCommon/ad_callouts/CallOut").createXmlDocument();
    SLWorkRequirementProcessPlanData[] data = SLWorkRequirementProcessPlanData.select(this, strProcessPlan);
    ...
    if (data!=null && data.length>0) {
      resultado.append("new Array(\"inpexplodephases\", \"" + FormatUtilities.replaceJS(data[0].explodephases) + "\"),");
      resultado.append("new Array(\"inpconversionrate\", \"" + FormatUtilities.replaceJS(data[0].conversionrate) + "\"),");
