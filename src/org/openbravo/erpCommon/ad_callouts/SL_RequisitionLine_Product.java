...
public class SL_RequisitionLine_Product extends HttpSecureAppServlet {
    ...
    public void doPost(HttpServletRequest request, HttpServletResponse response)
        ...
        if (vars.commandIn("DEFAULT")) {
            ...
            String strAttributeSetInstance = vars
                    .getStringParameter("inpmProductId_ATR");
            String strUOM = vars.getStringParameter("inpmProductId_UOM");
            try {
                printPage(response, vars, strMProductID, strWindowId, strTabId,
                        strAttributeSetInstance, strUOM, strRequisition,
                        strPriceListId, strChanged);
    ...
    void printPage(HttpServletResponse response, VariablesSecureApp vars,
            String strMProductID, String strWindowId, String strTabId,
            String strAttribute, String strUOM, String strRequisition,
            String strPriceListId, String strChanged) throws IOException,
            ServletException {
        if (log4j.isDebugEnabled())
        ...
        if (!strMProductID.equals("")) {
            String strDueDate = vars.getStringParameter("inpneedbydate",
                    DateTimeData.today(this));
            if (strPriceListId.equals(""))
                strPriceListId = SLRequisitionLineProductData.selectPriceList(
                        this, strRequisition);
            if (!strPriceListId.equals("")) {
                String strPriceListVersion = SLRequisitionLineProductData
                        .selectPriceListVersion(this, strPriceListId,
                                strDueDate);
                if (!strPriceListVersion.equals("")) {
                    SLRequisitionLineProductData[] prices = SLRequisitionLineProductData
                            .getPrices(this, strMProductID, strPriceListVersion);
                    if (prices != null
                            && prices.length > 0
                            && !(prices[0].pricelist.equals("0") && prices[0].pricestd
                                    .equals("0"))) {
                        String strPriceList = "";
                        String strPriceActual = "";
                        if (prices[0].pricelist.equals("0")
                                && prices[0].pricestd.equals("0"))
                            ...
                            if (strPriceList.startsWith("\""))
                                strPriceList = strPriceList.substring(1,
                                        strPriceList.length() - 1);
                            if (strPriceActual.startsWith("\""))
                                strPriceActual = strPriceActual.substring(1,
                                        strPriceActual.length() - 1);
                            BigDecimal priceList = (strPriceList.equals("") ? new BigDecimal(
                                    0.0)
                                    : new BigDecimal(strPriceList));
                            BigDecimal priceActual = (strPriceActual.equals("") ? new BigDecimal(
                                    0.0)
        ...
        if (strChanged.equals("inpmProductId")) {
            ...
            resultado.append("new Array(\"inpmAttributesetinstanceId_R\", \""
                    + FormatUtilities.replaceJS(SLRequisitionLineProductData
                            .attribute(this, strAttribute)) + "\"),\n");

            String strHasSecondaryUOM = SLRequisitionLineProductData
            ...
            try {
                ComboTableData comboTableData = new ComboTableData(vars, this,
                        "TABLEDIR", "M_Product_UOM_ID", "", "M_Product_UOM_ID",
                        Utility.getContext(this, vars, "#User_Org",
                                "SLRequisitionLineProduct"), Utility
