...
public class SL_Order_Amt extends HttpSecureAppServlet {
  ...
  public void doPost (HttpServletRequest request, HttpServletResponse response) throws IOException,ServletException {
    ...
    if (vars.commandIn("DEFAULT")) {
      ...
      try {
        printPage(response, vars, strChanged, strQtyOrdered, strPriceActual, strDiscount, strPriceLimit, strPriceList, strCOrderId, strProduct, strUOM, strAttribute, strTabId, strQty, strPriceStd);
  ...
  void printPage(HttpServletResponse response, VariablesSecureApp vars, String strChanged, String strQtyOrdered, String strPriceActual, String strDiscount, String strPriceLimit, String strPriceList, String strCOrderId, String strProduct, String strUOM, String strAttribute, String strTabId, String strQty, String strPriceStd) throws IOException, ServletException {
    if (log4j.isDebugEnabled()){ log4j.debug("Output: dataSheet"); log4j.debug("CHANGED:"+strChanged);}
    ...
    /*if (enforcedLimit) {
      String strPriceVersion = "";
      PriceListVersionComboData[] data1 = PriceListVersionComboData.selectActual(this, data[0].mPricelistId, DateTimeData.today(this));
      if (data1!=null && data1.length>0) strPriceVersion = data1[0].mPricelistVersionId;
      BigDecimal lineLimit = new BigDecimal(SLOrderAmtData.selectPriceLimit(this, strPriceVersion, strProduct));
      if (lineLimit.floatValue() >priceActual.floatValue()) isUnderLimit=true;
    }*/
    ...
    if (strChanged.equals("inppriceactual"))
    {
      ...
      priceStd = new BigDecimal(SLOrderProductData.getOffersStdPrice(this, dataOrder[0].cBpartnerId, strPriceActual.replace("\"", ""), strProduct, dataOrder[0].dateordered, strQty, dataOrder[0].mPricelistId, dataOrder[0].id));
     // priceList
      resultado.append("new Array(\"inppricestd\", \"" + priceStd.toString() + "\"),");
    }

/*
    if (strChanged.equals("inppricelist")||strChanged.equals("inpqtyordered"))
    {
      if (log4j.isDebugEnabled()) log4j.debug("priceList:" + Double.toString(priceList.doubleValue()));
      priceActual = new BigDecimal(SLOrderProductData.getOffersPrice(this, dataOrder[0].dateordered, dataOrder[0].cBpartnerId, strProduct, strPriceList.replace("\"", ""), strQty, dataOrder[0].mPricelistId, dataOrder[0].id));
...
*/
    ...
    if (strChanged.equals("inppricelist")||strChanged.equals("inppriceactual")) {
      if (priceList.doubleValue() == 0.0) discount = ZERO;
        ...
        if (log4j.isDebugEnabled()) log4j.debug("pricelist:" + Double.toString(priceList.doubleValue()));
        if (log4j.isDebugEnabled()) log4j.debug("priceActual:" + Double.toString(priceActual.doubleValue()));
        discount = new BigDecimal ((priceList.doubleValue()-priceStd.doubleValue()) / priceList.doubleValue() * 100.0);
      }
      ...
      priceActual = new BigDecimal(SLOrderProductData.getOffersPrice(this, dataOrder[0].dateordered, dataOrder[0].cBpartnerId, strProduct, priceStd.toString(), strQty, dataOrder[0].mPricelistId, dataOrder[0].id));
      ...
      if (priceList.doubleValue() != 0) discount1 = new BigDecimal ((priceList.doubleValue()-priceStd.doubleValue()) / priceList.doubleValue() * 100.0).setScale(StdPrecision, BigDecimal.ROUND_HALF_UP);
      ...
      {
        priceStd = new BigDecimal (priceList.doubleValue()-(priceList.doubleValue()*discount.doubleValue()/100) );
        priceActual = new BigDecimal(SLOrderProductData.getOffersPrice(this, dataOrder[0].dateordered, dataOrder[0].cBpartnerId, strProduct, priceStd.toString(), strQty, dataOrder[0].mPricelistId, dataOrder[0].id));
        if (priceStd.scale() > PricePrecision) priceStd = priceStd.setScale(PricePrecision, BigDecimal.ROUND_HALF_UP);
        if (priceActual.scale() > PricePrecision) priceActual = priceActual.setScale(PricePrecision, BigDecimal.ROUND_HALF_UP);
        resultado.append("new Array(\"inppriceactual\", \"" + priceActual.toString() + "\"),");
        resultado.append("new Array(\"inppricestd\", \"" + priceStd.toString() + "\"),");
      }     
      ...
      if (!strStockSecurity.equals("0")) {
          if (qtyOrdered.doubleValue() != 0.0) {
              if (strEnforceAttribute.equals("N")) {
                ...
                resultStock = new BigDecimal (stockNoAttribute.doubleValue() - qtyOrdered.doubleValue());
                if (stockSecurity.doubleValue() > resultStock.doubleValue()) {
                  resultado.append("new Array('MESSAGE', \"" + FormatUtilities.replaceJS(Utility.messageBD(this, "StockLimit", vars.getLanguage())) + "\"),");
                }
              } else {
                if (!strAttribute.equals("") && strAttribute != null) {
                  strStockAttribute = SLOrderStockData.totalStockAttribute(this, strProduct, strUOM, strAttribute);
                  stockAttribute = new BigDecimal (strStockAttribute);
                  resultStock = new BigDecimal (stockAttribute.doubleValue() - qtyOrdered.doubleValue());
                  if (stockSecurity.doubleValue() > resultStock.doubleValue()) {
                  resultado.append("new Array('MESSAGE', \"" + FormatUtilities.replaceJS(Utility.messageBD(this, "StockLimit", vars.getLanguage())) + "\"),");
                  }
                }
    ...
    if (log4j.isDebugEnabled()) log4j.debug(resultado.toString());
    ...
    if (lineNetAmt.scale() > StdPrecision) lineNetAmt = lineNetAmt.setScale(StdPrecision, BigDecimal.ROUND_HALF_UP);
