...
public class SL_Invoice_Product extends HttpSecureAppServlet {
    ...
    public void doPost(HttpServletRequest request, HttpServletResponse response)
        ...
        if (vars.commandIn("DEFAULT")) {
            ...
            String strPriceList = vars
                    .getStringParameter("inpmProductId_PLIST");
            String strPriceStd = vars.getStringParameter("inpmProductId_PSTD");
            String strPriceLimit = vars
                    .getStringParameter("inpmProductId_PLIM");
            ...
            try {
                printPage(response, vars, strUOM, strPriceList, strPriceStd,
                        strPriceLimit, strCurrency, strMProductID, strADOrgID,
                        strCInvoiceID, strIsSOTrx, strWharehouse, strTabId,
                        strQty);
    ...
    void printPage(HttpServletResponse response, VariablesSecureApp vars,
            String strUOM, String strPriceList, String strPriceStd,
            String strPriceLimit, String strCurrency, String strMProductID,
            String strADOrgID, String strCInvoiceID, String strIsSOTrx,
            String strWharehouse, String strTabId, String strQty)
            throws IOException, ServletException {
        if (log4j.isDebugEnabled())
        ...
        if (!strMProductID.equals("")) {
            SLOrderProductData[] dataInvoice = SLOrderProductData
                    .selectInvoice(this, strCInvoiceID);

            if (log4j.isDebugEnabled())
                log4j.debug("get Offers date: " + dataInvoice[0].dateinvoiced
                        + " partner:" + dataInvoice[0].cBpartnerId + " prod:"
                        + strMProductID + " std:"
                        + strPriceStd.replace("\"", ""));
            strPriceActual = SLOrderProductData.getOffersPriceInvoice(this,
                    dataInvoice[0].dateinvoiced, dataInvoice[0].cBpartnerId,
                    strMProductID, strPriceStd.replace("\"", ""), strQty,
        ...
        PAttributeSetData[] dataPAttr = PAttributeSetData.selectProductAttr(
                this, strMProductID);
        if (dataPAttr != null && dataPAttr.length > 0) {
            PAttributeSetData[] data2 = PAttributeSetData.select(this,
                    dataPAttr[0].mAttributesetId);
            if (PAttributeSet.isInstanceAttributeSet(data2)) {
                resultado
                        .append("new Array(\"inpmAttributesetinstanceId\", \"\"),");
                resultado
                        .append("new Array(\"inpmAttributesetinstanceId_R\", \"\"),");
            } else {
                ...
                resultado
                        .append("new Array(\"inpmAttributesetinstanceId_R\", \""
                                + FormatUtilities
                                        .replaceJS(dataPAttr[0].description)
                                + "\"),");
            }
        }
        ...
        if (data != null && data.length > 0) {
            String strCTaxID = Tax.get(this, strMProductID,
                    data[0].dateinvoiced, strADOrgID, strWharehouse,
                    data[0].cBpartnerLocationId, data[0].cBpartnerLocationId,
        ...
        try {
            ComboTableData comboTableData = new ComboTableData(vars, this,
                    "TABLE", "", "M_Product_UOM", "", Utility.getContext(this,
                            vars, "#User_Org", "SLOrderProduct"), Utility
