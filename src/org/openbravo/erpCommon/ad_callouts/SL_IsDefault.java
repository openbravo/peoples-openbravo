 ...
 * table (with organization) or, in case it is a tab with parent, it is the only
 * checked taking into account its parent.
 * 
 * If another one already exists an error message is raised and the checkbox is
 * unchecked.
 * 
 */
public class SL_IsDefault extends HttpSecureAppServlet {
    ...
    void printPage(HttpServletResponse response, VariablesSecureApp vars)
        ...
        String strValue = vars.getStringParameter(vars
        ...
        if (strValue.equals("Y")) {
            ...
            String currentKeyValue = vars.getStringParameter(vars
                    .getStringParameter("inpKeyName"));
            ...
            if (data != null && data.length != 0) {
                ...
                if (!currentKeyValue.equals(""))
                    currentClause = "AND " + currentColumnKey + " != '"
                            + currentKeyValue + "'";

                String strTotalDefaults = SLIsDefaultData.selectHasDefaults(
                        this, data[0].tablename, parentClause, currentClause,
                        strOrg);
                if (!strTotalDefaults.equals("0")) {
                    String msg = Utility.messageBD(this, "DuplicatedDefaults",
                            vars.getLanguage());
