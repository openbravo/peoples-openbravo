...
public class SL_RequisitionLine_Conversion extends HttpSecureAppServlet {
  ...
  void printPage(HttpServletResponse response, VariablesSecureApp vars, String strUOM, String strMProductUOMID,String strQuantityOrder, String strTabId) throws IOException, ServletException {
    if (log4j.isDebugEnabled()) log4j.debug("Output: dataSheet");
    XmlDocument xmlDocument = xmlEngine.readXmlTemplate("org/openbravo/erpCommon/ad_callouts/CallOut").createXmlDocument();
    if (strUOM.startsWith("\"")) strUOM=strUOM.substring(1,strUOM.length()-1);
    int stdPrecision = Integer.valueOf(SLRequisitionLineConversionData.stdPrecision(this, strUOM)).intValue();
    ...
    if (strMultiplyRate.equals("")) strMultiplyRate = SLRequisitionLineConversionData.divideRate (this, strUOM, strInitUOM);
    ...
    resultado.append("var calloutName='SL_RequisitionLine_Conversion';\n\n");
    if (strMultiplyRate.equals("0") || strQuantityOrder.equals("")) {
      ...
      movementQty = new BigDecimal (quantityOrder.doubleValue()*multiplyRate.doubleValue());
      if (movementQty.scale() > stdPrecision) movementQty = movementQty.setScale(stdPrecision, BigDecimal.ROUND_HALF_UP);
