...
public class SE_Order_BPartner extends HttpSecureAppServlet {
    ...
    public void doPost(HttpServletRequest request, HttpServletResponse response)
        ...
        if (vars.commandIn("DEFAULT")) {
            ...
            String strPaymentterm = vars
                    .getStringParameter("inpcPaymenttermId");
            String strInvoiceRule = vars.getStringParameter("inpinvoicerule");
            String strPriceList = vars.getStringParameter("inpmPricelistId");

            try {
                printPage(response, vars, strBPartner, strOrderType,
                        strIsSOTrx, strWindowId, strLocation, strContact,
                        strProjectId, strTabId, strDeliveryRule, strUserRep,
                        strPaymentrule, strPaymentterm, strInvoiceRule,
                        strPriceList);
    ...
    void printPage(HttpServletResponse response, VariablesSecureApp vars,
            String strBPartner, String strOrderType, String strIsSOTrx,
            String strWindowId, String strLocation, String strContact,
            String strProjectId, String strTabId, String strDeliveryRule0,
            String strUserRep0, String strPaymentrule0, String strPaymentterm0,
            String strInvoiceRule0, String strPriceList0) throws IOException,
            ServletException {
        if (log4j.isDebugEnabled())
        ...
        if (data != null && data.length > 0) {
            strDeliveryRule = data[0].deliveryrule.equals("") ? strDeliveryRule0
                    : data[0].deliveryrule;
            ...
            for (int i = 0; i < td.length; i++) {
                resultado
                        .append("new Array(\""
                                + td[i].getField("id")
                                + "\", \""
                                + FormatUtilities.replaceJS(td[i]
                                        .getField("name"))
                                + "\", \""
                                + (td[i].getField("id").equalsIgnoreCase(
                                        strMwarehouse) ? "true" : "false")
                                + "\")");
                if (i < td.length - 1)
                    resultado.append(",\n");
            }
        ...
        try {
            ComboTableData comboTableData = new ComboTableData(vars, this,
                    "TABLE", "", "AD_User SalesRep", "", Utility.getContext(
                            this, vars, "#User_Org", "SEOrderBPartner"),
            ...
            for (int i = 0; i < l.length; i++) {
                resultado
                        .append("new Array(\""
                                + l[i].getField("id")
                                + "\", \""
                                + FormatUtilities.replaceJS(l[i]
                                        .getField("name"))
                                + "\", \""
                                + (l[i].getField("id").equalsIgnoreCase(
                                        strInvoiceRule) ? "true" : "false")
                                + "\")");
                if (i < l.length - 1)
                    resultado.append(",\n");
            }
        ...
        resultado.append("new Array(\"inpcPaymenttermId\", \""
                + (strPaymentterm.equals("") ? SEOrderBPartnerData
                        .selectPaymentTerm(this, Utility.getContext(this, vars,
                                "#User_Client", strWindowId)) : strPaymentterm)
                + "\"),");
        resultado.append("new Array(\"inpdeliveryrule\", ");
        try {
            ComboTableData comboTableData = new ComboTableData(vars, this,
                    "LIST", "", "C_Order DeliveryRule", "", Utility.getContext(
                            this, vars, "#User_Org", "SEOrderBPartner"),
                    Utility.getContext(this, vars, "#User_Client",
                            "SEOrderBPartner"), 0);
            Utility.fillSQLParameters(this, vars, null, comboTableData,
        ...
        if (strIsSOTrx.equals("N")) {
            resultado
                    .append("\"")
                    .append(
                            (strDeliveryRule.equals("") ? ((l != null && l.length > 0) ? l[0]
                                    .getField("id")
                                    : "null")
                                    : strDeliveryRule)).append("\"");
        } else {
            if (l != null && l.length > 0) {
                resultado.append("new Array(");
                for (int i = 0; i < l.length; i++) {
