...
public class SL_GlobalUse_Product extends HttpSecureAppServlet {
  ...
  void printPage(HttpServletResponse response, VariablesSecureApp vars, String strTabId, String strMProductID, String strPAttr) throws IOException, ServletException {
...
//    String strProductUomId = SLGlobalUseProduct.selectProductUomId(this, strMProductID);

    resultado.append("new Array(\"inpmProductUomId\", ");
    String strmProductUOMId = SLGlobalUseProductData.strMProductUOMID(this,strMProductID,strProductUom);
	...
	try {
      ...
      if (tld!=null && tld.length>0) {
        ...
        for (int i=0;i<tld.length;i++) {
          resultado.append("new Array(\"" + tld[i].getField("id") + "\", \"" + FormatUtilities.replaceJS(tld[i].getField("name")) + "\", \"" + (tld[i].getField("id").equalsIgnoreCase(strmProductUOMId)?"true":"false") + "\")");
