...
/**
 * A callout used in the module dependency form. When the dependent module
 * changes then the minor version is set to the version of the dependent module.
 * 
 * @author mtaal
 */
public class SL_Module_Minor_Version extends HttpSecureAppServlet {
	...
	if (vars.commandIn("DEFAULT")) {
	    ...
	    String strFirstVersion = vars
		    .getStringParameter(MINOR_VERSION_FIELD);
	    try {
		printPage(response, vars, strChanged, strFirstVersion,
			strModule);
    ...
    private void printPage(HttpServletResponse response,
	    VariablesSecureApp vars, String strChanged, String firstVersion,
	    String strModule) throws IOException, ServletException {
	if (log4j.isDebugEnabled())
	...
	if (strChanged.equals(DEPENDENT_MODULE_FIELD)) {
	    ...
	    final Module dependsOnModule = OBDal.getInstance().get(
