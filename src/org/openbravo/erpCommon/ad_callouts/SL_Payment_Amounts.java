...
public class SL_Payment_Amounts extends HttpSecureAppServlet {
  ...
  public void doPost (HttpServletRequest request, HttpServletResponse response) throws IOException,ServletException {
    ...
    if (vars.commandIn("DEFAULT")) {
      ...
      try {
        printPage(response, vars, strChanged, strIsOverUnderPayment, strPayamt, strDiscountamt, strWriteoffamt, strOverunderamt, strcCurrencyId, strcPaymentId, strcInvoiceId, strTabId);
  ...
  void printPage(HttpServletResponse response, VariablesSecureApp vars, String strChanged, String strIsOverUnderPayment, String strPayamt, String strDiscountamt, String strWriteoffamt, String strOverunderamt, String strcCurrencyId, String strcPaymentId, String strcInvoiceId, String strTabId) throws IOException, ServletException {
    ...
    if (payamt == null) payamt = ZERO;  
    discountamt = new BigDecimal(strDiscountamt);
    if (discountamt == null) discountamt = ZERO;
    writeoffamt = new BigDecimal(strWriteoffamt);
    if (writeoffamt == null) writeoffamt = ZERO;
    overunderamt = new BigDecimal(strOverunderamt);
    if (overunderamt == null) overunderamt = ZERO;
    ...
    if (strChanged.equals("inpisoverunderpayment")){
      ...
      if (log4j.isDebugEnabled()) log4j.debug("LAST CURRENCY: "+strCurrency);
      if (strCurrency.equals("")){
        ...
        if (strCurrency.equals(strcCurrencyId)){
          ...
          String strconvert = SLPaymentAmountsData.selectConversion(this, strCurrency, strcCurrencyId, DateTimeData.today(this), null, vars.getClient(), vars.getOrg());
          if (strconvert.equals("")){
            resultado.append("new Array('MESSAGE', \"" + FormatUtilities.replaceJS(Utility.messageBD(this, "NoCurrencyConversion", vars.getLanguage())) + "\"),");
	FieldProvider [] tdd = null;
	try {
		ComboTableData comboTableData = new ComboTableData(vars, this, "TABLE", "", "C_Currency", "ID", Utility.getContext(this, vars, "#User_Org", "SLPaymentAmounts"), Utility.getContext(this, vars, "#User_Client", "SLPaymentAmounts"), 0);
		...
		throw new ServletException(ex);
            ...
            for (int i=0;i<tdd.length;i++) {
              resultado.append("new Array(\"" + tdd[i].getField("id") + "\", \"" + FormatUtilities.replaceJS(tdd[i].getField("name")) + "\", \"" + (tdd[i].getField("id").equalsIgnoreCase(strCurrency)?"true":"false") + "\")");
              if (i<tdd.length-1) resultado.append(",\n");
            }
            resultado.append("\n)");
      ...
      if (log4j.isDebugEnabled()) log4j.debug("PAYMENT: "+data[0].grand+"CURRENCYID: "+data[0].currencyid);
      invtotamt = new BigDecimal(data[0].grand);
      if (invtotamt == null) invtotamt = ZERO;
      if (strcCurrencyId.equals(data[0].currencyid)){
      }
      else {
         String strconvert = SLPaymentAmountsData.selectConversion(this, data[0].currencyid, strcCurrencyId, DateTimeData.today(this), null, vars.getClient(), vars.getOrg());
         convert = new BigDecimal(strconvert);
