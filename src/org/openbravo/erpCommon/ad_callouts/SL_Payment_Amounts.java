...
public class SL_Payment_Amounts extends HttpSecureAppServlet {
    ...
    public void doPost(HttpServletRequest request, HttpServletResponse response)
        ...
        if (vars.commandIn("DEFAULT")) {
            ...
            String strIsOverUnderPayment = vars
                    .getStringParameter("inpisoverunderpayment");
            ...
            try {
                printPage(response, vars, strChanged, strIsOverUnderPayment,
                        strPayamt, strDiscountamt, strWriteoffamt,
                        strOverunderamt, strcCurrencyId, strcPaymentId,
                        strcInvoiceId, strTabId);
    ...
    void printPage(HttpServletResponse response, VariablesSecureApp vars,
            String strChanged, String strIsOverUnderPayment, String strPayamt,
            String strDiscountamt, String strWriteoffamt,
            String strOverunderamt, String strcCurrencyId,
            String strcPaymentId, String strcInvoiceId, String strTabId)
            throws IOException, ServletException {
        if (log4j.isDebugEnabled())
        ...
        if (strChanged.equals("inpisoverunderpayment")) {
            ...
            if (overunderamt == null)
                overunderamt = ZERO;
            resultado.append("new Array(\"inpoverunderamt\", \""
                    + overunderamt.toString() + "\")");
        }
            ...
            if (strCurrency.equals("")) {
                ...
                if (strCurrency.equals(strcCurrencyId)) {
                    ...
                    String strconvert = SLPaymentAmountsData.selectConversion(
                            this, strCurrency, strcCurrencyId, DateTimeData
                                    .today(this), null, vars.getClient(), vars
                                    .getOrg());
                    if (strconvert.equals("")) {
                        resultado.append("new Array('MESSAGE', \""
                                + FormatUtilities.replaceJS(Utility.messageBD(
                                        this, "NoCurrencyConversion", vars
                                                .getLanguage())) + "\"),");
                        ...
                        try {
                            ComboTableData comboTableData = new ComboTableData(
                                    ...
                                    Utility.getContext(this, vars,
                                            "#User_Client", "SLPaymentAmounts"),
                                    0);
                            Utility.fillSQLParameters(this, vars, null,
                                    comboTableData, "SLPaymentAmounts", "");
                        ...
                        resultado.append("new Array(\"inppayamt\", \""
                                + payamt.toString() + "\"),");
                        discountamt = discountamt.multiply(convert).setScale(2,
                                4);
                        resultado.append("new Array(\"inpdiscountamt\", \""
                                + discountamt.toString() + "\"),");
                        writeoffamt = writeoffamt.multiply(convert).setScale(2,
                                4);
                        resultado.append("new Array(\"inpwriteoffamt\", \""
                                + writeoffamt.toString() + "\"),");
                        overunderamt = overunderamt.multiply(convert).setScale(
                                2, 4);
                        resultado.append("new Array(\"inpoverunderamt\", \""
                                + overunderamt.toString() + "\")");
                        vars.setSessionValue("Last.Currency", strcCurrencyId);
            ...
            SLPaymentAmountsData[] data = SLPaymentAmountsData.select(this,
                    strcInvoiceId);
            if (log4j.isDebugEnabled())
                log4j.debug("PAYMENT: " + data[0].grand + "CURRENCYID: "
                        + data[0].currencyid);
            ...
            if (strcCurrencyId.equals(data[0].currencyid)) {
                ...
                String strconvert = SLPaymentAmountsData.selectConversion(this,
                        data[0].currencyid, strcCurrencyId, DateTimeData
                                .today(this), null, vars.getClient(), vars
                                .getOrg());
                convert = new BigDecimal(strconvert);
                invtotamt = invtotamt.multiply(convert).setScale(2, 4);
            }
            writeoffamt = (invtotamt).subtract(payamt).subtract(discountamt)
            ...
            invtotamt = invtotamt.subtract(discountamt).subtract(writeoffamt)
                    .subtract(overunderamt);
            resultado.append("new Array(\"inppayamt\", \""
                    + invtotamt.toString() + "\")");
            ...
            invtotamt = invtotamt.subtract(writeoffamt).subtract(discountamt)
                    .subtract(overunderamt);
            resultado.append("new Array(\"inppayamt\", \""
                    + invtotamt.toString() + "\")");
            ...
            if (invtotamt == null)
                invtotamt = ZERO;
            invtotamt = invtotamt.subtract(overunderamt).subtract(writeoffamt)
