...
public class SL_WRPhaseProduct_Product extends HttpSecureAppServlet {
  ...
  void printPage(HttpServletResponse response, VariablesSecureApp vars, String strTabId, String strMProductID) throws IOException, ServletException {
    ...
    resultado.append("var calloutName='SL_WRPhaseProduct_Product';\n\n");
    resultado.append("var respuesta = new Array(");
    resultado.append("new Array(\"inpcUomId\", \"" + FormatUtilities.replaceJS((strProductUom.equals("")?"\"\"":strProductUom)) + "\"),\n");
...
//    String strProductUomId = SLProcessPlanProductData.selectProductUomId(this, strMProductID);

    resultado.append("new Array(\"inpmProductUomId\", ");
    String strmProductUOMId = SLWRPhaseProductProductData.strMProductUOMID(this,strMProductID,strProductUom);
	...
	try {
      ...
      if (tld!=null && tld.length>0) {
        ...
        for (int i=0;i<tld.length;i++) {
          resultado.append("new Array(\"" + tld[i].getField("id") + "\", \"" + tld[i].getField("name") + "\", \"" + (tld[i].getField("id").equalsIgnoreCase(strmProductUOMId)?"true":"false") + "\")");
