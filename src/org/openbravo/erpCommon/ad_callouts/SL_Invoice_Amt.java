...
public class SL_Invoice_Amt extends HttpSecureAppServlet {
    ...
    public void doPost(HttpServletRequest request, HttpServletResponse response)
        ...
        if (vars.commandIn("DEFAULT")) {
            ...
            try {
                printPage(response, vars, strChanged, strQtyInvoice,
                        strPriceActual, strInvoiceId, strProduct,
                        strPriceLimit, strTabId, strPriceList, strPriceStd);
    ...
    void printPage(HttpServletResponse response, VariablesSecureApp vars,
            String strChanged, String strQtyInvoice, String strPriceActual,
            String strInvoiceId, String strProduct, String strPriceLimit,
            String strTabId, String strPriceList, String strPriceStd)
            throws IOException, ServletException {
        if (log4j.isDebugEnabled())
        ...
        if (data != null && data.length > 0) {
            strPrecision = data[0].stdprecision.equals("") ? "0"
                    : data[0].stdprecision;
            strPricePrecision = data[0].priceprecision.equals("") ? "0"
                    : data[0].priceprecision;
            enforcedLimit = (data[0].enforcepricelimit.equals("Y") ? true
                    : false);
        }
        ...
        qtyInvoice = (strQtyInvoice.equals("") ? ZERO : new BigDecimal(
                strQtyInvoice));
        ...
        SLOrderProductData[] dataInvoice = SLOrderProductData.selectInvoice(
                this, strInvoiceId);
        ...
        if (strChanged.equals("inppriceactual")) {
            ...
            priceStd = new BigDecimal(SLOrderProductData
                    .getOffersStdPriceInvoice(this, dataInvoice[0].cBpartnerId,
                            strPriceActual.replace("\"", ""), strProduct,
                            dataInvoice[0].dateinvoiced, strQtyInvoice,
        ...
        if (strChanged.equals("inpqtyinvoiced")) {
            if (log4j.isDebugEnabled())
                log4j.debug("strPriceList: " + strPriceList.replace("\"", "")
                        + " product:" + strProduct + " qty:" + strQtyInvoice);
            priceActual = new BigDecimal(SLOrderProductData
                    .getOffersPriceInvoice(this, dataInvoice[0].dateinvoiced,
                            dataInvoice[0].cBpartnerId, strProduct, priceStd
                                    .toString(), strQtyInvoice,
                            dataInvoice[0].mPricelistId, dataInvoice[0].id));
            ...
            if (priceLimit.doubleValue() != 0.0
                ...
                resultado.append("new Array('MESSAGE', \""
                        + FormatUtilities.replaceJS(Utility.messageBD(this,
                                "UnderLimitPrice", vars.getLanguage()))
                        + "\"), ");
        }
        resultado.append("new Array(\"inplinenetamt\", \""
                + LineNetAmt.toString() + "\"),");
