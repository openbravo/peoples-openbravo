...
public abstract class ImportProcess {
    ...
    public final OBError startProcess(VariablesSecureApp vars)
    ...
    protected void process(VariablesSecureApp vars) {
        ...
        try {
            ...
            m_error.setTitle(Utility.messageBD(m_conn, "Error", vars
                    .getLanguage()));
            m_error.setMessage(Utility.messageBD(m_conn, "ProcessRunError",
                    vars.getLanguage()));
        }
    }

    protected abstract OBError doIt(VariablesSecureApp vars)
            throws ServletException;

    protected abstract void createInstanceParams(VariablesSecureApp vars)
            throws ServletException;
    ...
    private void createInstance(VariablesSecureApp vars)
            throws ServletException {
        try {
            ...
            ImportProcessData.insertInstance(getConnection(),
                    m_AD_PInstance_ID, getAD_Process_ID(), getRecord_ID(),
                    getAD_User_ID(), getAD_Client_ID(), getAD_Org_ID());
        } catch (ServletException ex) {
            log4j.error("Unable to create instance - " + ex);
            throw new ServletException("Unable to create instance - "
                    + ex.getMessage());
        }
    }

    public void addParameterString(String parametername, String seqno,
            String value) throws ServletException {
        ImportProcessData.insertPInstanceParam(getConnection(),
                getAD_PInstance_ID(), seqno, parametername, value,
                getAD_Client_ID(), getAD_Org_ID(), getAD_User_ID());
    }

    public void addParameterNumber(String parametername, String seqno,
            String value) throws ServletException {
        ImportProcessData.insertPInstanceParamNumber(getConnection(),
                getAD_PInstance_ID(), seqno, parametername, value,
                getAD_Client_ID(), getAD_Org_ID(), getAD_User_ID());
    }

    public void addParameterDate(String parametername, String seqno,
            String value) throws ServletException {
        ImportProcessData.insertPInstanceParamDate(getConnection(),
                getAD_PInstance_ID(), seqno, parametername, value,
                getAD_Client_ID(), getAD_Org_ID(), getAD_User_ID());
    }

    public void addParameterInfo(String parametername, String seqno,
            String value) throws ServletException {
        ImportProcessData.insertPInstanceParamInfo(getConnection(),
                getAD_PInstance_ID(), seqno, parametername, value,
                getAD_Client_ID(), getAD_Org_ID(), getAD_User_ID());
    }
    ...
    private void lock() throws ServletException {
        if (log4j.isDebugEnabled())
            log4j.debug("lock");
        try {
            ImportProcessData.lockInstance(getConnection(),
                    getAD_PInstance_ID());
        } catch (ServletException ex) {
            log4j.error("Unable to lock instance - " + ex);
            throw new ServletException("Unable to lock instance - "
                    + ex.getMessage());
        }
    }
    ...
    private void unlock() throws ServletException {
        try {
            ImportProcessData.unlockInstance(getConnection(), (m_error
                    .getType().equals("Error") ? "0" : "1"), m_msg.toString(),
                    getAD_PInstance_ID());
        } catch (ServletException ex) {
            log4j.error("Unable to unlock instance - " + ex);
            throw new ServletException("Unable to unlock instance - "
                    + ex.getMessage());
        }
        if (log4j.isDebugEnabled())
            log4j.debug("unlock - Error=" + m_error.toString() + " - "
                    + m_msg.toString());
    }

    private void fillInstanceParams() throws ServletException {
        ImportProcessData[] data = ImportProcessData.select(getConnection(),
                getAD_PInstance_ID());
