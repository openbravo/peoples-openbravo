...
public class ChangeOrderOrg extends HttpSecureAppServlet {
    ...
    public void doPost(HttpServletRequest request, HttpServletResponse response)
        ...
        if (vars.commandIn("DEFAULT")) {
            ...
            String strOrg = vars.getGlobalVariable("inpadOrgId",
                    "ChangeOrderOrg.adOrgId", "");
            vars.getGlobalVariable("inpadShipperpathId",
                    "ChangeOrderOrg.adShipperpathId", "");
            String strPayment = vars.getGlobalVariable("inppaymentrule",
            ...
            String strBPartner = vars.getRequestGlobalVariable(
                    "inpcBpartnerId", "ChangeOrderOrg.cBpartnerId");
            String strOrg = vars.getRequestGlobalVariable("inpadOrgId",
                    "ChangeOrderOrg.adOrgId");
            String strShipperpath = vars.getRequestGlobalVariable(
                    "inpadShipperpathId", "ChangeOrderOrg.adShipperpathId");
            ...
            printPage(response, vars, strBPartner, strOrg, strShipperpath,
                    strPayment, strNewOrg, strTax);
    ...
    OBError processSave(VariablesSecureApp vars, String strOrder,
            String strNewOrg, String strTax) {
        ...
        try {
            ...
            if (!strOrder.equals("")) {
                ...
                while (st.hasMoreTokens()) {
                    ...
                    String StrDocBaseType = ChangeOrderOrgData
                            .selectDocbasetype(this, strOrderId);
                    String StrDocSubTypeSo = ChangeOrderOrgData
                            .selectDocsubtypeso(this, strOrderId);
                    String StrDocTypeID = ChangeOrderOrgData
                            .selectDoctypeneworg(this, StrDocBaseType,
                                    StrDocSubTypeSo, strNewOrg);
                    String StrExistsShipment = ChangeOrderOrgData
                            .selectExistsShipment(this, strOrderId);
                    if (ChangeOrderOrgData.checkRestrictions(conn, this,
                            strOrderId)) {
                    ...
                    boolean shouldDelete = ChangeOrderOrgData
                            .shouldDeleteCashLine(conn, this, strOrderId);
                    ChangeOrderOrgData.resetOrder(conn, this, strOrderId);
                    try {
                        ChangeOrderOrgData.updateDocAction(conn, this, vars
                                .getUser(), strOrderId);
                    ...
                    try {
                        ChangeOrderOrgData.insertPInstance(conn, this,
                                pinstance, "104", strOrderId, vars.getUser(),
                    ...
                    try {
                        ChangeOrderOrgData.updateLines(conn, this, strTax, vars
                                .getUser(), strNewOrg, strOrderId);
                        ChangeOrderOrgData.update(conn, this, vars.getUser(),
                                strNewOrg, StrDocTypeID, StrDocTypeID,
                                strOrderId);
                    } catch (ServletException ex) {
                        myMessage = Utility.translateError(this, vars, vars
                                .getLanguage(), ex.getMessage());
                    ...
                    if (!StrExistsShipment.equals("0")) {
                        ...
                        for (i = 0; i < shipmentsData.length; i++) {
                            ChangeOrderOrgData.updateProcessed(conn, this, vars
                                    .getUser(), shipmentsData[i].mInoutId);
                            String StrDocTypeIDShipment = ChangeOrderOrgData
                                    .cDocttypeshipment(this, StrDocTypeID);
                            String DocumentnoShipment = Utility.getDocumentNo(
                                    this, vars, "169", "M_Inout",
                                    StrDocTypeIDShipment, StrDocTypeIDShipment,
                                    false, true);
                            try {
                                ChangeOrderOrgData.updateShipment(conn, this,
                                        vars.getUser(), DocumentnoShipment,
                                        StrDocTypeIDShipment, strNewOrg,
                                        shipmentsData[i].mInoutId);
                                ChangeOrderOrgData.updateShipmentlines(conn,
                                        this, vars.getUser(), strNewOrg,
                                        shipmentsData[i].mInoutId);
                                ChangeOrderOrgData.updateMtransaction(conn,
                                        this, strNewOrg, vars.getUser(),
                                        shipmentsData[i].mInoutId);
                            } catch (ServletException ex) {
                    ...
                    try {
                        if (shouldDelete)
                            ChangeOrderOrgData.deleteCashLine(conn, this,
                                    strOrderId);
                    } catch (ServletException ex) {
                        myMessage = Utility.translateError(this, vars, vars
                                .getLanguage(), ex.getMessage());
    ...
    void printPage(HttpServletResponse response, VariablesSecureApp vars,
            String strBPartner, String strOrg, String strShipperpath,
            String strPayment, String strNewOrg, String strTax)
            throws IOException, ServletException {
        if (log4j.isDebugEnabled())
        ...
        ToolBar toolbar = new ToolBar(this, vars.getLanguage(),
                "ChangeOrderOrg", false, "", "", "", false, "ad_process",
        ...
        try {
            ...
            NavigationBar nav = new NavigationBar(this, vars.getLanguage(),
                    "ChangeOrderOrg.html", classInfo.id, classInfo.type,
                    strReplaceWith, tabs.breadcrumb());
            xmlDocument.setParameter("navigationBar", nav.toString());
            LeftTabsBar lBar = new LeftTabsBar(this, vars.getLanguage(),
                    "ChangeOrderOrg.html", strReplaceWith);
        ...
        xmlDocument.setParameter("bpartnerDes", ChangeOrderOrgData
        ...
        xmlDocument.setData("reportAD_Org_IDNew", "liststructure",
                OrganizationComboData.selectCombo(this, vars.getRole()));

        try {
            ComboTableData comboTableData = new ComboTableData(vars, this,
                    "TABLEDIR", "C_Tax_ID", "", "", Utility.getContext(this,
                            vars, "#User_Org", "ChangeOrderOrg"), Utility
                            .getContext(this, vars, "#User_Client",
                                    "ChangeOrderOrg"), 0);
            Utility.fillSQLParameters(this, vars, null, comboTableData,
                    "ChangeOrderOrg", strTax);
        ...
        try {
            ComboTableData comboTableData = new ComboTableData(vars, this,
                    "TABLEDIR", "M_Warehouse_ID", "", "", Utility.getContext(
                            this, vars, "#User_Org", "ChangeOrderOrg"), Utility
                            .getContext(this, vars, "#User_Client",
                                    "ChangeOrderOrg"), 0);
            Utility.fillSQLParameters(this, vars, null, comboTableData,
                    "ChangeOrderOrg", strShipperpath);
        ...
        if (vars.commandIn("DEFAULT") && strBPartner.equals("")
                && strOrg.equals("")) {
            xmlDocument.setData("structure1", new ChangeOrderOrgData[0]);
        } else {
            xmlDocument.setData("structure1", ChangeOrderOrgData.select(this,
                    vars.getLanguage(), Utility.getContext(this, vars,
                            "#User_Client", "ChangeOrderOrg"), Utility
                            .getContext(this, vars, "#User_Org",
                                    "ChangeOrderOrg"), strBPartner, strOrg,
                    strPayment, strShipperpath));
        }
