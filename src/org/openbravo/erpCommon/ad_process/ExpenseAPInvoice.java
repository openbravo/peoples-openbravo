...
public class ExpenseAPInvoice extends HttpSecureAppServlet {
    ...
    public void doPost(HttpServletRequest request, HttpServletResponse response)
        ...
        if (vars.commandIn("DEFAULT")) {
            ...
            String strDatereportFrom = vars.getGlobalVariable(
                    "inpReportDateFrom", "ExpenseAPInvoice.reportdatefrom", "");
            ...
            printPage(response, vars, strcBpartnerId, strDatereportFrom,
                    strDatereportTo, strDateInvoiced);
        } else if (vars.commandIn("SAVE")) {
            String strcBpartnerId = vars.getStringParameter("inpcBpartnerId",
                    "");
            String strDatereportFrom = vars.getRequestGlobalVariable(
                    "inpReportDateFrom", "ExpenseAPInvoice.reportdatefrom");
            String strDatereportTo = vars.getRequestGlobalVariable(
                    "inpReportDateTo", "ExpenseAPInvoice.reportdateto");
            String strDateInvoiced = vars.getRequestGlobalVariable(
                    "inpDateinvoiced", "ExpenseAPInvocie.dateinvoiced");
            OBError myMessage = processExpense(vars, strcBpartnerId,
                    strDatereportFrom, strDatereportTo, strDateInvoiced);
    ...
    OBError processExpense(VariablesSecureApp vars, String strcBpartnerId,
            String strDatereportFrom, String strDatereportTo,
        ...
        try {
            ...
            ExpenseAPInvoiceData[] data = ExpenseAPInvoiceData.select(this,
                    Utility.getContext(this, vars, "#User_Client",
                            "ExpenseAPInvoice"), Utility.getContext(this, vars,
                            "#User_Org", "ExpenseAPInvoice"),
                    strDatereportFrom, DateTimeData.nDaysAfter(this,
                            strDatereportTo, "1"), strcBpartnerId);
            for (int i = 0; i < data.length; i++) {
                String docTargetType = ExpenseAPInvoiceData.cDoctypeTarget(
                        this, data[i].adClientId, data[i].adOrgId);
                ...
                if (data[i].cProjectId.equals("")) {
                    strcInvoiceIdOld = ExpenseAPInvoiceData
                            .selectInvoiceHeaderNoProject(conn, this,
                                    data[i].adClientId, data[i].adOrgId,
                                    strDateInvoiced, data[i].cBpartnerId,
                                    strBPCCurrencyId, data[i].cActivityId,
                                    data[i].cCampaignId);
                } else {
                    strcInvoiceIdOld = ExpenseAPInvoiceData
                            .selectInvoiceHeader(conn, this,
                                    data[i].adClientId, data[i].adOrgId,
                                    strDateInvoiced, data[i].cBpartnerId,
                                    strBPCCurrencyId, data[i].cProjectId,
                                    data[i].cActivityId, data[i].cCampaignId);
                }
                ...
                if (strcInvoiceIdOld.equals("")) {
                    ...
                    strPricelistId = ExpenseAPInvoiceData.pricelistId(this,
                            data[i].cBpartnerId);
                    if (strPricelistId.equals("")) {
                        ...
                        strBPCCurrencyId = ExpenseAPInvoiceData.selectCurrency(
                                this, strPricelistId);
                    }

                    strcBpartnerLocationId = ExpenseAPInvoiceData
                            .bPartnerLocation(this, data[i].cBpartnerId);
                    if (strcBpartnerLocationId.equals(""))
                        throw new Exception("ShiptoNotdefined");

                    strPaymentRule = ExpenseAPInvoiceData.paymentrule(this,
                            data[i].cBpartnerId);
                    if (strPaymentRule.equals(""))
                        throw new Exception("FormofPaymentNotdefined");

                    strPaymentterm = ExpenseAPInvoiceData.paymentterm(this,
                            data[i].cBpartnerId);
                    ...
                    String strDocumentno = Utility.getDocumentNo(this, vars,
                            "", "C_Invoice", docTargetType, docTargetType,
                            false, true);
                    String strDocType = ExpenseAPInvoiceData.cDoctypeTarget(
                            this, data[i].adClientId, data[i].adOrgId);

                    // Catch database error message
                    try {
                        ExpenseAPInvoiceData.insert(conn, this, strcInvoiceId,
                                "N", "", "N", "N", "N", "N", "N",
                                data[i].adClientId, data[i].adOrgId, "", "",
                                strDocumentno, "", "", "Y", docTargetType,
                                strDateInvoiced, strDateInvoiced,
                                data[i].cBpartnerId, strcBpartnerLocationId,
                                "", strPricelistId, strBPCCurrencyId,
                                strSalesrepId, "N", "", "", strPaymentRule,
                                strPaymentterm, "N", "N", data[i].cProjectId,
                                data[i].cActivityId, data[i].cCampaignId, vars
                                        .getOrg(), "", "", "0", "0", "DR",
                                strDocType, "N", "CO", "N", vars.getUser(),
                                vars.getUser());
                    ...
                    textoMensaje.append(
                            Utility.messageBD(this,
                                    "PurchaseInvoiceDocumentno", vars
                                            .getLanguage())).append(" ")
                            .append(strDocumentno).append(" ").append(
                                    Utility.messageBD(this, "beenCreated", vars
                                            .getLanguage())).append("<br>");
                    total++;
                } else
                    strcInvoiceId = strcInvoiceIdOld;

                // Determines the date of the expense (strDateExpense)
                strDateexpense = data[i].dateexpense.equals("") ? data[i].datereport
                        : data[i].dateexpense;
                strDateexpense = strDateexpense.equals("") ? DateTimeData
                        .today(this) : strDateexpense;

                String bpartnerLocationShip = ExpenseAPInvoiceData.shipto(this,
                        data[i].cBpartnerId);
                if (bpartnerLocationShip.equals(""))
                    throw new Exception("ShiptoNotdefined");

                String strmProductUomId = ExpenseAPInvoiceData.mProductUomId(
                        this, data[i].mProductId);

                // Gets the tax for the purchase invoice line
                strcTaxID = Tax.get(this, data[i].mProductId, strDateInvoiced,
                        data[i].adOrgId, vars.getWarehouse(),
                        strcBpartnerLocationId, bpartnerLocationShip,
                        data[i].cProjectId, false);
                if (strcTaxID.equals(""))
                ...
                ExpenseAPInvoiceData[] dataPrice = ExpenseAPInvoiceData
                        .selectPrice(this, data[i].mProductId, strPricelistId,
                                strDateInvoiced);
                ...
                // necessary
                if (data[i].invoiceprice != null
                        && !data[i].invoiceprice.equals("")) {
                    ...
                    if (!strCCurrencyId.equals(strBPCCurrencyId)) {
                        ...
                        strPricestd = ExpenseAPInvoiceData.selectConvertedAmt(
                                this, strPricestd, strCCurrencyId,
                                strBPCCurrencyId, strDateexpense, vars
                                        .getClient(), vars.getOrg());
                        strPricelist = ExpenseAPInvoiceData.selectConvertedAmt(
                                this, strPricelist, strCCurrencyId,
                                strBPCCurrencyId, strDateexpense, vars
                                        .getClient(), vars.getOrg());
                        strPricelimit = ExpenseAPInvoiceData
                                .selectConvertedAmt(this, strPricelimit,
                                        strCCurrencyId, strBPCCurrencyId,
                                        strDateexpense, vars.getClient(), vars
                                                .getOrg());
                    }
                }
                ...
                int PricePrecision = Integer.valueOf(strPricePrecision)
                        .intValue();
                priceActual = (strPricestd.equals("") ? ZERO : (new BigDecimal(
                        strPricestd)));
                priceActual = priceActual.setScale(PricePrecision,
                        BigDecimal.ROUND_HALF_UP);
                priceList = (strPricelist.equals("") ? ZERO : (new BigDecimal(
                        strPricelist)));
                priceList = priceList.setScale(PricePrecision,
                        BigDecimal.ROUND_HALF_UP);
                priceLimit = (strPricelimit.equals("") ? ZERO
                        : (new BigDecimal(strPricelimit)));
                priceLimit = priceLimit.setScale(PricePrecision,
                        BigDecimal.ROUND_HALF_UP);
                ...
                // current invoice
                ExpenseAPInvoiceData[] dataInvoiceline = ExpenseAPInvoiceData
                        .selectInvoiceLine(conn, this, strcInvoiceId,
                                data[i].adClientId, data[i].adOrgId,
                                data[i].mProductId, data[i].cUomId,
                                strPricestd, strPricelist, strPricelimit,
                                data[i].description, strcTaxID);
                ...
                if (dataInvoiceline == null || dataInvoiceline.length == 0) {
                    ...
                    if (log4j.isDebugEnabled())
                        log4j
                                .debug("*****************+client: "
                                        + (data[i].invoiceprice.equals("") ? dataPrice[0].pricestd
                                                : data[i].invoiceprice));
                    // Catch database error message
                    try {
                        ExpenseAPInvoiceData.insertLine(conn, this,
                                data[i].adClientId, data[i].adOrgId,
                ...
                if (!data[i].cProjectId.equals("")) {
                    ...
                    // requirements
                    ExpenseAPInvoiceData[] dataAcctdimension = ExpenseAPInvoiceData
                            .selectAcctdimension(conn, this,
                                    data[i].adClientId, data[i].adOrgId,
                                    strcInvoiceLineId, data[i].cProjectId,
                                    data[i].cCampaignId);
                    if (dataAcctdimension == null
                            || dataAcctdimension.length == 0) {
                        String strcInvoicelineAcctdimension = SequenceIdData
                                .getUUID();
                        // Catch database error message
                        try {
                            ExpenseAPInvoiceData
                                    .insertInvoicelineAcctdimension(
                                            conn,
                                            this,
                                            strcInvoicelineAcctdimension,
                                            data[i].adClientId,
                                            data[i].adOrgId,
                                            "Y",
                                            vars.getUser(),
                ...
                try {
                    ...
                    ExpenseAPInvoiceData.updateExpense(conn, this,
            ...
            myMessage.setMessage(Utility.messageBD(this,
                    "PriceListVersionNotFound", vars.getLanguage())
                    + ' '
                    + Utility.messageBD(this, "ForProduct", vars.getLanguage())
                    + ' '
                    + strProd
                    + ' '
                    + Utility.messageBD(this, "And", vars.getLanguage())
                    + ' '
                    + Utility.messageBD(this, "Employee", vars.getLanguage())
                    + ' ' + strEmpl);
            return myMessage;
        } catch (Exception e) {
            try {
            ...
            if (e.getMessage().equals("PricelistNotdefined")) {
                myMessage.setMessage(Utility.messageBD(this, "TheEmployee",
                        vars.getLanguage())
                        + ' '
                        + strEmpl
                        + ' '
                        + Utility.messageBD(this, "PricelistNotdefined", vars
                                .getLanguage()));
                ...
                myMessage.setMessage(Utility.messageBD(this, "TheEmployee",
                        ...
                        + strEmpl
                        + ' '
                        + Utility.messageBD(this, "FormofPaymentNotdefined",
                                vars.getLanguage()));
                ...
                myMessage.setMessage(Utility.messageBD(this, "TheEmployee",
                        ...
                        + strEmpl
                        + ' '
                        + Utility.messageBD(this, "PaymenttermNotdefined", vars
                                .getLanguage()));
                ...
                myMessage.setMessage(Utility.messageBD(this, "TheEmployee",
                        ...
                        + strEmpl
                        + ' '
                        + Utility.messageBD(this, "ShiptoNotdefined", vars
                                .getLanguage()));
            } else if (e.getMessage().equals("TaxNotFound")) {
                myMessage.setMessage(Utility.messageBD(this, "TaxNotFound",
                        vars.getLanguage()));
            } else {
    ...
    void printPage(HttpServletResponse response, VariablesSecureApp vars,
            String strcBpartnerId, String strDatereportFrom,
            String strDatereportTo, String strDateInvoiced) throws IOException,
            ServletException {
        if (log4j.isDebugEnabled())
        ...
        ToolBar toolbar = new ToolBar(this, vars.getLanguage(),
                "ExpenseAPInvoice", false, "", "", "", false, "ad_process",
        ...
        try {
            ComboTableData comboTableData = new ComboTableData(vars, this,
                    ...
                    "", Utility.getContext(this, vars, "#User_Org",
                            "ExpenseAPInvoice"), Utility.getContext(this, vars,
                            "#User_Client", "ExpenseAPInvoice"), 0);
            Utility.fillSQLParameters(this, vars, null, comboTableData,
                    "ExpenseAPInvoice", "");
            xmlDocument.setData("reportC_BPARTNERID", "liststructure",
        ...
        xmlDocument.setParameter("dateTosaveFormat", vars
                .getSessionValue("#AD_SqlDateFormat"));
        xmlDocument.setParameter("dateInvdisplayFormat", vars
                .getSessionValue("#AD_SqlDateFormat"));
        xmlDocument.setParameter("dateInvsaveFormat", vars
                .getSessionValue("#AD_SqlDateFormat"));
        ...
        try {
            ...
            NavigationBar nav = new NavigationBar(this, vars.getLanguage(),
                    "ExpenseAPInvoice.html", classInfo.id, classInfo.type,
                    strReplaceWith, tabs.breadcrumb());
            xmlDocument.setParameter("navigationBar", nav.toString());
            LeftTabsBar lBar = new LeftTabsBar(this, vars.getLanguage(),
                    "ExpenseAPInvoice.html", strReplaceWith);
