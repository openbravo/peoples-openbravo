...
public class ImportBPartner extends ImportProcess {
    ...
    public ImportBPartner(ConnectionProvider conn, String AD_Process_ID,
            String recordId, boolean deleteOld) {
        super(conn);
    ...
    protected OBError doIt(VariablesSecureApp vars) throws ServletException {
        ...
        try {
            ...
            // Set BPGroup
            no = ImportBPartnerData.updateBPGroupDefault(con, conn,
                    getAD_Client_ID());
            if (log4j.isDebugEnabled())
                log4j.debug("ImportBPartner BPGroup = " + no);
            no = ImportBPartnerData.updateBPGroupId(con, conn,
                    getAD_Client_ID());
            if (log4j.isDebugEnabled())
                log4j.debug("ImportBPartner BPGroupId = " + no);
            no = ImportBPartnerData.updateGroupError(con, conn,
                    getAD_Client_ID());
            if (log4j.isDebugEnabled())
                log4j.debug("Invalid BPartner group = " + no);

            // Country
            no = ImportBPartnerData.updateCountryCodeDefault(con, conn,
                    getAD_Client_ID());
            if (log4j.isDebugEnabled())
                log4j.debug("ImportBPartner CountryCode = " + no);
            no = ImportBPartnerData.updateCountryId(con, conn,
                    getAD_Client_ID());
            if (log4j.isDebugEnabled())
                log4j.debug("ImportBPartner CountryId = " + no);
            no = ImportBPartnerData.updateCountryError(con, conn,
                    getAD_Client_ID());
            if (log4j.isDebugEnabled())
                log4j.debug("Invalid Country = " + no);

            // Region
            no = ImportBPartnerData.updateRegionDefault(con, conn,
                    getAD_Client_ID());
            if (log4j.isDebugEnabled())
                log4j.debug("ImportBPartner Region = " + no);
            no = ImportBPartnerData
                    .updateRegionId(con, conn, getAD_Client_ID());
            if (log4j.isDebugEnabled())
                log4j.debug("ImportBPartner RegionId = " + no);
            no = ImportBPartnerData.updateRegionError(con, conn,
                    getAD_Client_ID());
            if (log4j.isDebugEnabled())
                log4j.debug("Invalid Region = " + no);

            // Greeting
            no = ImportBPartnerData.updateGreetingId(con, conn,
                    getAD_Client_ID());
            if (log4j.isDebugEnabled())
                log4j.debug("ImportBPartner GreetingId = " + no);
            no = ImportBPartnerData.updateGreetingError(con, conn,
                    getAD_Client_ID());
            if (log4j.isDebugEnabled())
                log4j.debug("Invalid Greeting = " + no);

            // BPartner
            no = ImportBPartnerData.updateBPartnerId(con, conn,
                    getAD_Client_ID());
            if (log4j.isDebugEnabled())
            ...
            // ADUser
            no = ImportBPartnerData
                    .updateADUserId(con, conn, getAD_Client_ID());
            if (log4j.isDebugEnabled())
                log4j.debug("ImportBPartner ADUserId = " + no);

            // Location
            no = ImportBPartnerData.updateLocationId(con, conn,
                    getAD_Client_ID());
            if (log4j.isDebugEnabled())
            ...
            se.printStackTrace();
            addLog(Utility.messageBD(conn, "ProcessRunError", vars
                    .getLanguage()));
            myError.setType("Error");
            myError.setTitle(Utility.messageBD(conn, "Error", vars
                    .getLanguage()));
            myError.setMessage(Utility.messageBD(conn, "ProcessRunError", vars
                    .getLanguage()));
            return myError;
        }
        ...
        try {
            // Go through Records
            ImportBPartnerData[] data = ImportBPartnerData.select(conn,
                    getAD_Client_ID());
            if (log4j.isDebugEnabled())
            ...
            for (int i = 0; i < data.length; i++) {
                ...
                boolean newBPartner = (C_BPartner_ID == null || C_BPartner_ID
                        .equals(""));
                ...
                if (newBPartner) { // Insert new BPartner
                    ...
                    try {
                        no = ImportBPartnerData.insertBPartner(con, conn,
                                C_BPartner_ID, I_BPartner_ID);
                        if (log4j.isDebugEnabled())
                        ...
                        conn.releaseRollbackConnection(con);
                        no = ImportBPartnerData.insertBPartnerError(conn, ex
                                .toString(), I_BPartner_ID);
                        continue;
                    }
                } else { // Update existing BPartner
                    try {
                        no = ImportBPartnerData.updateBPartner(con, conn,
                                I_BPartner_ID, C_BPartner_ID);
                        if (log4j.isDebugEnabled())
                        ...
                        conn.releaseRollbackConnection(con);
                        no = ImportBPartnerData.updateBPartnerError(conn, ex
                                .toString(), I_BPartner_ID);
                        continue;
                    }
                }
                ...
                if (!C_BPartner_Location_ID.equals("")) { // Update Location
                    ...
                    try {
                        no = ImportBPartnerData.insertLocation(con, conn,
                                C_Location_ID, I_BPartner_ID);
                        if (log4j.isDebugEnabled())
                        ...
                        noInsert--;
                        conn.releaseRollbackConnection(con);
                        no = ImportBPartnerData.insertLocationError(conn, ex
                                .toString(), I_BPartner_ID);
                        continue;
                    }
                    ...
                    try {
                        ...
                        no = ImportBPartnerData.insertBPLocation(con, conn,
                                C_BPartner_Location_ID, locationName,
                                C_BPartner_ID, C_Location_ID, I_BPartner_ID);
                        if (log4j.isDebugEnabled())
                            log4j.debug("Insert BP Location = " + no);
                    } catch (ServletException ex) {
                        if (log4j.isDebugEnabled())
                            log4j
                                    .debug("Insert BP Location - "
                                            + ex.toString());
                        noInsert--;
                        conn.releaseRollbackConnection(con);
                        no = ImportBPartnerData.insertBPLocationError(conn, ex
                                .toString(), I_BPartner_ID);
                        continue;
                    }
                }
                ...
                if (!AD_User_ID.equals("")) {
                    try {
                        ...
                        no = ImportBPartnerData.updateBPContact(con, conn,
                                I_BPartner_ID, AD_User_ID);
                        if (log4j.isDebugEnabled())
                        ...
                        noUpdate--;
                        conn.releaseRollbackConnection(con);
                        no = ImportBPartnerData.updateBPContactError(conn, ex
                                .toString(), I_BPartner_ID);
                        continue;
                    }
                    ...
                    try {
                        if (data[i].contactname != null
                            ...
                            no = ImportBPartnerData
                                    .insertBPContact(
                                            ...
                                            AD_User_ID,
                                            C_BPartner_ID,
                                            (C_BPartner_Location_ID.equals("0")) ? "NULL"
                        ...
                        noInsert--;
                        conn.releaseRollbackConnection(con);
                        no = ImportBPartnerData.insertBPContactError(conn, ex
                                .toString(), I_BPartner_ID);
                        continue;
                    }
                }
                ...
                // Update I_BPARTNER
                try {
                    no = ImportBPartnerData.setImported(con, conn,
                            C_BPartner_ID,
                            (C_BPartner_Location_ID.equals("0")) ? ""
                                    : C_BPartner_Location_ID, (AD_User_ID
                                    .equals("0")) ? "" : AD_User_ID,
                            I_BPartner_ID);
                    conn.releaseCommitConnection(con);
                } catch (ServletException ex) {
                    ...
                    noInsert--;
                    conn.releaseRollbackConnection(con);
                    no = ImportBPartnerData.updateSetImportedError(conn, ex
                            .toString(), I_BPartner_ID);
                    continue;
                }
            }
            ...
            noBPartnerError = ImportBPartnerData.updateNotImported(conn,
                    getAD_Client_ID());
        } catch (Exception se) {
            se.printStackTrace();
            addLog(Utility.messageBD(conn, "ProcessRunError", vars
        ...
        addLog(Utility.messageBD(conn, "Business partners not imported", vars
                .getLanguage())
                + ": " + noBPartnerError + "; ");
        ...
        if (noBPartnerError == 0) {
            ...
            myError.setTitle(Utility.messageBD(conn,
                    "Some business partners could not be imported", vars
                            .getLanguage()));
        } else {
            myError.setType("Error");
            myError.setTitle(Utility.messageBD(conn,
                    " No business partners could be imported", vars
                            .getLanguage()));
        }
    ...
    private String parseAddressName(ImportBPartnerData addressData) {
        ...
        } else if (!addressData.regionname.equals("")
                && addressData.regionname != null) {
        ...
        else if (!addressData.address2.equals("")
