    ...
    ConnectionProvider conn = null;
		...
		try {
			...
			String[] strFieldsPO = new String[] {"UPC",
				"PriceEffective","VendorProductNo","VendorCategory","Manufacturer","Discontinued","DiscontinuedBy"};
			...
			for (int i = 0; i < numFieldsPO.length; i++){
				no = ImportProductData.updateProductNumFieldPO(con, conn, numFieldsPO[i], getAD_Client_ID());
				if (no != 0)
			...
			if (log4j.isDebugEnabled()) log4j.debug("ImportProduct Set Product Category Default =" + no);
			//
			no = ImportProductData.updateProductCategory(con, conn, getAD_Client_ID());
			if (log4j.isDebugEnabled()) log4j.debug("ImportProduct Set Product Category =" + no);
			//
			no = ImportProductData.updateInvalidCategory(con, conn, getAD_Client_ID());
			if (log4j.isDebugEnabled()) log4j.debug("ImportProduct Invalid Product Category =" + no);
			...
			try {
				...
				for(int i =0;i<data.length;i++){
					...
					if (!C_BPartner_ID.equals("")){
						...
						if (!newProduct){
							try{
								no = ImportProductData.updateProductPOImport(con, conn, I_Product_ID, M_Product_ID, C_BPartner_ID);
					...
					if (log4j.isDebugEnabled()) log4j.debug("after update: " + M_Product_ID + " -- " + I_Product_ID);
				} //  for all I_Product
			} catch (Exception se) {
				se.printStackTrace();
				addLog(Utility.messageBD(conn, "ProcessRunError", vars.getLanguage()));
