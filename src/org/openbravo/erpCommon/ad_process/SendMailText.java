...
public class SendMailText extends HttpSecureAppServlet {
    ...
    public void doPost(HttpServletRequest request, HttpServletResponse response)
        ...
        if (vars.commandIn("DEFAULT")) {
            ...
            String strBPGroup = vars.getGlobalVariable("inpBPGroup",
                    "SendMailText|bpGroup", "");
            String strUser = vars.getGlobalVariable("inpUser",
                    "SendMailText|user", "");
            printPage(response, vars, strMailTemplate, strInterestArea,
                    strBPGroup, strUser);
        } else if (vars.commandIn("SEND")) {
            String strMailTemplate = vars.getRequiredGlobalVariable(
                    "inpMailTemplate", "SendMailText|mailTemplate");
            String strInterestArea = vars.getRequestGlobalVariable(
                    "inpInterestArea", "SendMailText|interestArea");
            String strBPGroup = vars.getRequestGlobalVariable("inpBPGroup",
                    "SendMailText|bpGroup");
            String strUser = vars.getRequestGlobalVariable("inpUser",
                    "SendMailText|user");

            vars.removeMessage("SendMailText");
            String strMessage = processSend(vars, strMailTemplate,
                    strInterestArea, strBPGroup, strUser);
            // New message system
    ...
    void printPage(HttpServletResponse response, VariablesSecureApp vars,
            String strMailTemplate, String strInterestArea, String strBPGroup,
        ...
        XmlDocument xmlDocument = xmlEngine.readXmlTemplate(
                "org/openbravo/erpCommon/ad_process/SendMailText", discard)
                .createXmlDocument();

        ToolBar toolbar = new ToolBar(this, vars.getLanguage(), "SendMailText",
        ...
        try {
            ...
            NavigationBar nav = new NavigationBar(this, vars.getLanguage(),
                    "SendMailText.html", classInfo.id, classInfo.type,
                    strReplaceWith, tabs.breadcrumb());
            xmlDocument.setParameter("navigationBar", nav.toString());
            LeftTabsBar lBar = new LeftTabsBar(this, vars.getLanguage(),
                    "SendMailText.html", strReplaceWith);
        ...
        try {
            ComboTableData comboTableData = new ComboTableData(vars, this,
                    "TABLEDIR", "R_MailText_ID", "", "", Utility.getContext(
                            this, vars, "#User_Org", "SendMailText"), Utility
                            .getContext(this, vars, "#User_Client",
                                    "SendMailText"), 0);
            Utility.fillSQLParameters(this, vars, null, comboTableData,
                    "SendMailText", strMailTemplate);
            xmlDocument.setData("reportMailTemplate", "liststructure",
        ...
        try {
            ComboTableData comboTableData = new ComboTableData(
                    ...
                    "",
                    "",
                    Utility.getContext(this, vars, "#User_Org", "SendMailText"),
                    Utility.getContext(this, vars, "#User_Client",
                            "SendMailText"), 0);
            Utility.fillSQLParameters(this, vars, null, comboTableData,
                    "SendMailText", strInterestArea);
            xmlDocument.setData("reportInterestArea", "liststructure",
                    comboTableData.select(false));
            comboTableData = null;
        } catch (Exception ex) {
            throw new ServletException(ex);
            ...
            ComboTableData comboTableData = new ComboTableData(vars, this,
                    "TABLEDIR", "C_BP_Group_ID", "", "", Utility.getContext(
                            ...
                            .getContext(this, vars, "#User_Client",
                                    "SendMailText"), 0);
            Utility.fillSQLParameters(this, vars, null, comboTableData,
                    "SendMailText", strBPGroup);
            xmlDocument.setData("reportBPGroup", "liststructure",
                    comboTableData.select(false));
            comboTableData = null;
        } catch (Exception ex) {
            throw new ServletException(ex);
    ...
    protected String processSend(VariablesSecureApp vars,
            String strMailTemplate, String strInterestArea, String strBPGroup,
            String strUser) throws IOException, ServletException {
        ...
        try {
            ...
            if (strMailTemplate.equals(""))
                return (Utility.messageBD(this, "NotFound", language) + " " + Utility
                        .messageBD(this, "R_MailText_ID", language));
            if (log4j.isDebugEnabled())
                log4j.debug("processSend - AD_Client_ID=" + client);
            if (client.equals("0"))
                return (Utility.messageBD(this, "NotFound", language) + " - " + Utility
                        .messageBD(this, "AD_Client_ID", language));
            String smtpHost = RequestActionData.selectSMTPHost(conn, this,
                    client);
            if (log4j.isDebugEnabled())
                log4j.debug("processSend - SMTP Host=" + smtpHost);
            if (smtpHost.equals(""))
                return (Utility.messageBD(this, "NotFound", language) + " " + Utility
                        .messageBD(this, "SMTPHost", language));
            String from = "";
            String fromID = "";
            String fromPW = "";
            SendMailTextData[] mailTextData = SendMailTextData.selectMailData(
                    conn, this, strMailTemplate);
            if (mailTextData == null || mailTextData.length == 0)
                return (Utility.messageBD(this, "NotFound", language) + " "
                        + Utility.messageBD(this, "R_MailText_ID", language)
                        + ": " + strMailTemplate);
            ...
            if (!strUser.equals("")) {
                ...
                if (mails == null || mails.length == 0)
                    return ("From EMail not complete - " + from + "(" + fromID
                            + "/" + fromPW + ")");
                from = mails[0].email;
                from = from.trim().toLowerCase();
                for (int pos = from.indexOf(" "); pos != -1; pos = from
                        .indexOf(" ")) {
                    from = from.substring(0, pos) + from.substring(pos + 1);
                }
                fromID = mails[0].emailuser;
                fromPW = FormatUtilities.encryptDecrypt(mails[0].emailuserpw,
                        false);
            } else {
                mails = RequestActionData
                        .selectEmailRequest(conn, this, client);
                if (mails == null || mails.length == 0)
                    return ("From EMail not complete - " + from + "(" + fromID
                            + "/" + fromPW + ")" + " - selectEmailRequest");
                from = mails[0].email;
                from = from.trim().toLowerCase();
                for (int pos = from.indexOf(" "); pos != -1; pos = from
                        .indexOf(" ")) {
            ...
            if (strBPGroup != null && !strBPGroup.equals("")) {
                ...
                mailData = SendMailTextData.selectBPGroup(conn, this,
                        strBPGroup);
                if (mailData != null && mailData.length > 0) {
                    for (int i = 0; i < mailData.length; i++) {
                        if (sendIndividualMail(conn, vars, smtpHost, from,
