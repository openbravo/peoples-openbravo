...
public class PriceListCreateAll extends HttpSecureAppServlet {
  ...
  public void doPost (HttpServletRequest request, HttpServletResponse response) throws IOException,ServletException {
    ...
    if (vars.commandIn("DEFAULT")) {
      ...
      //vars.setSessionValue("PriceListCreateAll.message", messageResult);
      vars.setMessage("PriceListCreateAll", myMessage);
      response.sendRedirect(strDireccion + request.getServletPath());
    } else pageErrorPopUp(response);
  }

  OBError processButton(VariablesSecureApp vars, String strPriceListVersion, String windowId) throws ServletException {
    ...
    try {
      ...
      PriceListCreateAllData[] data = PriceListCreateAllData.selectParent(conn, this, strPriceListVersion, Utility.getContext(this, vars, "#User_Org", windowId), Utility.getContext(this, vars, "#User_Client", windowId));
      if (data==null || data.length==0) {
        ...
        return myMessage;
        //return Utility.messageBD(this, "SearchNothing", vars.getLanguage());
      }
      ...
      if (!strMessage.equals("")) {
        ...
        return myMessage;
        //return strMessage;
      } else strMessage = Utility.messageBD(this, "ProcessOK", vars.getLanguage());
  ...
  String processHijos(Connection conn, VariablesSecureApp vars, String windowId, String strPriceListVersion) throws ServletException {
    String strMessage="";
    PriceListCreateAllData[] data = PriceListCreateAllData.select(conn, this, strPriceListVersion, Utility.getContext(this, vars, "#User_Org", windowId), Utility.getContext(this, vars, "#User_Client", windowId));
    if (data==null || data.length==0) return "";
    for (int i=0;i<data.length;i++) {
      strMessage = processPL(conn, vars, data[i]);
      if (!strMessage.equals("")) break;
  ...
  String processPL(Connection conn, VariablesSecureApp vars, PriceListCreateAllData data) throws ServletException {
    ...
    if (!messageResult.equals("")) if (log4j.isDebugEnabled()) log4j.debug(messageResult);
    return messageResult;
  }

  void printPage(HttpServletResponse response, VariablesSecureApp vars) throws IOException, ServletException {
      if (log4j.isDebugEnabled()) log4j.debug("Output: Button process Create Pricelist");

      ActionButtonDefaultData[] data = null;
      //String strMessage = vars.getSessionValue("PriceListCreateAll.message");
      ...
      XmlDocument xmlDocument = xmlEngine.readXmlTemplate("org/openbravo/erpCommon/ad_process/PriceListCreateAll", discard).createXmlDocument();
      
      ToolBar toolbar = new ToolBar(this, vars.getLanguage(), "PriceListCreateAll", false, "", "", "",false, "ad_process",  strReplaceWith, false,  true);
      ...
      try {
        ...
        NavigationBar nav = new NavigationBar(this, vars.getLanguage(), "PriceListCreateAll.html", classInfo.id, classInfo.type, strReplaceWith, tabs.breadcrumb());
        xmlDocument.setParameter("navigationBar", nav.toString());
        LeftTabsBar lBar = new LeftTabsBar(this, vars.getLanguage(), "PriceListCreateAll.html", strReplaceWith);
	...
	try {
		ComboTableData comboTableData = new ComboTableData(vars, this, "TABLEDIR", "M_PriceList_Version_ID", "", "", Utility.getContext(this, vars, "#User_Org", "PriceListCreateAll"), Utility.getContext(this, vars, "#User_Client", "PriceListCreateAll"), 0);
