...
public class HeartbeatProcess implements Process {
    ...
    public void execute(ProcessBundle bundle) throws Exception {
        ...
        if (!isHeartbeatActive()) {
            msg = Utility.messageBD(connection, "HB_INACTIVE", ctx
                    .getLanguage());
            logger.logln(msg);
            return;
        }
        if (!isInternetAvailable(connection)) {
            msg = Utility.messageBD(connection, "HB_INTERNET_UNAVAILABLE", ctx
                    .getLanguage());
            logger.logln(msg);
    ...
    public boolean isHeartbeatActive() {
        String isheartbeatactive = SystemInfo
                .get(SystemInfo.Item.ISHEARTBEATACTIVE);
     ...
     */
    public static boolean isInternetAvailable(ConnectionProvider connection)
            throws ServletException {
        log.info("Checking for internet connection...");
        String isproxyrequired = SystemInfo
                .get(SystemInfo.Item.ISPROXYREQUIRED);
        if (isproxyrequired != null && isproxyrequired.equals("Y")) {
            String proxyServer = HeartbeatProcessData
                    .selectProxyServer(connection);
     ...
     * @throws ServletException
     */
    public Properties getSystemInfo(ConnectionProvider connection)
            throws ServletException {
    ...
    /**
     * Sends a query string to the heartbeat server. Returns the https response
     * as a string.
     * 
     * @param queryStr
     * @return
    ...
    public void logSystemInfo(ConnectionProvider connection,
            Properties systemInfo) throws ServletException {
        ...
        String servletContainerVersion = systemInfo
                .getProperty("servletContainerVersion");
        ...
        String numRegisteredUsers = systemInfo
                .getProperty("numRegisteredUsers");

        HeartbeatProcessData.insertHeartbeatLog(connection, id, "0", "0",
                systemIdentifier, isHeartbeatActive, isProxyRequired,
                proxyServer, proxyPort, activityRate, complexityRate, os,
                osVersion, db, dbVersion, servletContainer,
                servletContainerVersion, webserver, webserverVersion,
                obVersion, obInstallMode, codeRevision, numRegisteredUsers,
                javaVersion, antVersion);
    }
     ...
     * @param updates
     */
    public void saveUpdateAlerts(ConnectionProvider connection,
            List<Alert> updates) {
        if (updates == null) {
