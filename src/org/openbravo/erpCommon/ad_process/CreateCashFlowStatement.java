...
void printPage(HttpServletResponse response, VariablesSecureApp vars, String strProcessId) throws IOException, ServletException {
      ...
      XmlDocument xmlDocument = xmlEngine.readXmlTemplate("org/openbravo/erpCommon/ad_process/CreateCashFlowStatement").createXmlDocument();
      ...
      ToolBar toolbar = new ToolBar(this, vars.getLanguage(), "CreateCashFlowStatement", false, "", "", "",false, "ad_process",  strReplaceWith, false,  true);
      toolbar.prepareSimpleToolBarTemplate();
      xmlDocument.setParameter("toolbar", toolbar.toString());
      
            // New interface paramenters
      try {
        KeyMap key = new KeyMap(this, vars, "CreateCashFlowStatement.html");
        xmlDocument.setParameter("keyMap", key.getActionButtonKeyMaps());
      } catch (Exception ex) {
        throw new ServletException(ex);
      }
      try {
        WindowTabs tabs = new WindowTabs(this, vars, "org.openbravo.erpCommon.ad_process.CreateCashFlowStatement");

        xmlDocument.setParameter("parentTabContainer", tabs.parentTabs());
        xmlDocument.setParameter("mainTabContainer", tabs.mainTabs());
        xmlDocument.setParameter("childTabContainer", tabs.childTabs());
        xmlDocument.setParameter("theme", vars.getTheme());
        NavigationBar nav = new NavigationBar(this, vars.getLanguage(), "CreateCashFlowStatement.html", classInfo.id, classInfo.type, strReplaceWith, tabs.breadcrumb());
        xmlDocument.setParameter("navigationBar", nav.toString());
        LeftTabsBar lBar = new LeftTabsBar(this, vars.getLanguage(), "CreateCashFlowStatement.html", strReplaceWith);
  ...
  void insertCFS (Connection conn, HttpServletResponse response, VariablesSecureApp vars, String strPaymentId, String strFactAcctId, String strAmount, String strAccount, int level) throws ServletException {
        if (log4j.isDebugEnabled()) log4j.debug("Output: CreateCashFlowStatement");
        if (log4j.isDebugEnabled()) log4j.debug("strPaymentId - " + strPaymentId + " - strFactAcctId - " + strFactAcctId + " - strAmount - " + strAmount);
        CreateCashFlowStatementData[] data = CreateCashFlowStatementData.selectPaymentInfo(this, strPaymentId);
        ...
        if(data==null || data.length==0){
          ...
          if(!(data[0].cInvoiceId).equals("")){
            if (log4j.isDebugEnabled()) log4j.debug("CreateCashFlowStatement - INVOICE - " + data[0].cInvoiceId);
            CreateCashFlowStatementData[] statements = CreateCashFlowStatementData.selectStatements(this, "318", data[0].cInvoiceId);
            String strTotal = CreateCashFlowStatementData.selectSumStatements(this, "318", data[0].cInvoiceId);
            if (log4j.isDebugEnabled()) log4j.debug("strTotal - " + strTotal + " - strAmount - " + strAmount);
            if(strTotal==null || strTotal.equals("")){
              ...
              if (log4j.isDebugEnabled()) log4j.debug("CreateCashFlowStatement - POSTED INVOICE");
              ...
              if (log4j.isDebugEnabled()) log4j.debug("CreateCashFlowStatement - SETTLEMENT_GENERATE");
              if((data[0].ismanual).equals("Y")){
                  if (log4j.isDebugEnabled()) log4j.debug("CreateCashFlowStatement - SETTLEMENT_GENERATE MANUAL");
                  if (log4j.isDebugEnabled()) log4j.debug("CreateCashFlowStatement - SETTLEMENT_GENERATE - " + data[0].cSettlementGenerateId + " - SETTLEMENT_CANCEL - " + data[0].cSettlementCancelId);
                  CreateCashFlowStatementData[] glItems = CreateCashFlowStatementData.selectGLItemsSettlementGenerate(this, data[0].cSettlementGenerateId, strPaymentId);
                  String strTotal = CreateCashFlowStatementData.selectSumGLItemsGenerate(this, data[0].cSettlementGenerateId, strPaymentId);
                  if(glItems==null || glItems.length==0){
                    glItems = CreateCashFlowStatementData.selectGLItemsSettlementCancel(this, data[0].cSettlementCancelId, strPaymentId);
                    strTotal = CreateCashFlowStatementData.selectSumGLItemsCancel(this, data[0].cSettlementCancelId, strPaymentId);
                  }
                  ...
                  if (log4j.isDebugEnabled()) log4j.debug("CreateCashFlowStatement - SETTLEMENT_GENERATE NOT MANUAL");
                  ...
                  if(canceledPayments == null || canceledPayments.length == 0){
                    ...
                    String strTotal = CreateCashFlowStatementData.selectSumGeneratedPayments(this, data[0].cSettlementGenerateId);
                    for(int j=0; j<canceledPayments.length;j++){
                        String strNewAmount = multiply(calculateRatio(strTotal,strAmount),canceledPayments[j].amount);
                        if (log4j.isDebugEnabled()) log4j.debug("CreateCashFlowStatement - Rellamada - strNewAmount - " + strNewAmount + " - Payment - " + canceledPayments[j].id + " - strFactAcctId - " + strFactAcctId);
                        insertCFS(conn, response, vars, canceledPayments[j].id, strFactAcctId, strNewAmount, strAccount, (level+1));
                    }
                  }
              }
          }
        ...
        if(level==0){ //Compensamos la diferencia con el valor mas alto en terminos absolutos
          ...
          if(records!=null && records.length > 0){
            if (log4j.isDebugEnabled()) log4j.debug("CreateCashFlowStatement - updateDifference - strDifference - " + strDifference + " - records[0].id - " + records[0].id);
  ...
  public String calculateRatio(String strTotal, String strAmt) {
    if (log4j.isDebugEnabled()) log4j.debug("CreateCashFlowStatement - calculateRatio - strTotal - " + strTotal + " - strAmt - " + strAmt);
    if(strTotal == null || strAmt == null ||strTotal.equals("0") || strAmt.equals("0") || strTotal.equals("") || strAmt.equals("")) return "0";
    BigDecimal total = new BigDecimal(strTotal);
    BigDecimal amt = new BigDecimal(strAmt);
    if (log4j.isDebugEnabled()) log4j.debug("CreateCashFlowStatement - calculateRatio - strTotal - " + strTotal + " - strAmt - " + strAmt);
    if (log4j.isDebugEnabled()) log4j.debug("CreateCashFlowStatement - calculateRatio - total - " + total.doubleValue() + " - amt - " + amt.doubleValue());
    String strRatio = "";
    try{
    ...
    if (log4j.isDebugEnabled()) log4j.debug("CreateCashFlowStatement - calculateRatio - strRatio - " + strRatio);
    return strRatio;
