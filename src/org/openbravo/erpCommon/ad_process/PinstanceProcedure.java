...
public class PinstanceProcedure implements Process {
    ...
    public void execute(ProcessBundle bundle) throws Exception {
        ...
        if (connection.getRDBMS().equalsIgnoreCase("ORACLE")) {
            ...
            try {
                RDBMSIndependent.getCallableResult(null, connection, sql,
                        parametersData, parametersTypes, 0);
    ...
    private void log(String pinstanceId, ConnectionProvider conn,
        ...
        OBError msg;
        try {
            final PInstanceProcessData[] data = PInstanceProcessData.select(
                    conn, pinstanceId);
            msg = Utility.getProcessInstanceMessage(conn, obContext.toVars(),
                    data);
            logger.log(msg.getType() + " " + msg.getTitle() + " "
                    + msg.getMessage());

        } catch (final Exception e) {
            e.printStackTrace();
            msg = Utility.translateError(conn, obContext.toVars(), obContext
