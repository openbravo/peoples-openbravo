...
public class CreateTaxReport extends HttpSecureAppServlet {
    ...
    public void doPost(HttpServletRequest request, HttpServletResponse response)
        ...
        if (vars.commandIn("DEFAULT")) {
            String strTaxReportId = vars.getGlobalVariable("inpTaxReportId",
            ...
            printPage(response, vars, strTaxReportId, strDateFrom, strDateTo,
                    strOrg, process);
        } else if (vars.commandIn("FIND")) {
            String strTaxReportId = vars.getRequestGlobalVariable(
                    "inpTaxReportId", "CreateTaxReport|taxReport");
            String strDateFrom = vars.getGlobalVariable("inpDateFrom",
                    "CreateTaxReport|dateFrom", "");
            String strDateTo = vars.getGlobalVariable("inpDateTo",
                    "CreateTaxReport|dateTo", "");
            String strOrg = vars.getGlobalVariable("inpadOrgId",
                    "CreateTaxReport|orgId", "0");
            printPagePopUp(response, vars, strTaxReportId, strDateFrom,
                    strDateTo, strOrg);
        } else
    ...
    void printPage(HttpServletResponse response, VariablesSecureApp vars,
            String strTaxReportId, String strDateFrom, String strDateTo,
            String strOrg, String strProcessId) throws IOException,
            ServletException {
        ...
        ToolBar toolbar = new ToolBar(this, vars.getLanguage(),
                "CreateTaxReport", false, "", "", "", false, "ad_process",
        ...
        try {
            ...
            NavigationBar nav = new NavigationBar(this, vars.getLanguage(),
                    "CreateTaxReport.html", classInfo.id, classInfo.type,
                    strReplaceWith, tabs.breadcrumb());
            xmlDocument.setParameter("navigationBar", nav.toString());
            LeftTabsBar lBar = new LeftTabsBar(this, vars.getLanguage(),
                    "CreateTaxReport.html", strReplaceWith);
        ...
        try {
            ComboTableData comboTableData = new ComboTableData(vars, this,
                    "TABLEDIR", "AD_Org_ID", "", "", Utility.getContext(this,
                            vars, "#User_Org", "CreateTaxReport"), Utility
        ...
        xmlDocument.setData("reportC_TAX_REPORT", "liststructure",
                CreateTaxReportData.selectC_TAX_REPORT(this,
                        Utility.getContext(this, vars, "#User_Org",
                                "CreateTaxReport"), Utility.getContext(this,
                                vars, "#User_Client", "CreateTaxReport"), ""));
    ...
    void printPagePopUp(HttpServletResponse response, VariablesSecureApp vars,
        ...
        XmlDocument xmlDocument = xmlEngine.readXmlTemplate(
                "org/openbravo/erpCommon/ad_process/CreateTaxReportPopUp")
                .createXmlDocument();
        int level = 0;
     ...
     * String result = ""; CreateTaxReportData[] data =
     * CreateTaxReportData.selectC_TAX_REPORT(this, Utility.getContext(this,
     * vars, "#User_Org", "CreateTaxReport"), Utility.getContext(this, vars,
     * "#User_Client", "CreateTaxReport"), ""); if (data == null || data.length
     * == 0) { result = "var array = null;"; } else { result =
     * "var array = new Array(\n"; for (int i = 0;i<data.length;i++) { result +=
     * "new Array(\"" + data[i].id + "\",\"" + data[i].filteredbyorganization +
     * "\",\"" + data[i].temporaryfiltertype + "\")"; if (i<data.length-1)
    ...
    void childData(VariablesSecureApp vars, Vector<Object> vectorArray,
            String strTaxReportId, String strPeriodFrom, String strPeriodTo,
            String strOrg, int level, String strParent, int rownum)
            throws IOException, ServletException {
        if (log4j.isDebugEnabled())
            log4j.debug("**********************strTaxReportId: "
                    + strTaxReportId);
        if (log4j.isDebugEnabled())
            log4j
                    .debug("**********************strPeriodFrom: "
        ...
        CreateTaxReportData[] data = CreateTaxReportData.select(this, String
                .valueOf(rownum), strParent, String.valueOf(level), Utility
                .getContext(this, vars, "#User_Client", "CreateTaxReport"),
                Utility.stringList(strOrg), strPeriodFrom, DateTimeData
                        .nDaysAfter(this, strPeriodTo, "1"), strTaxReportId);
        if (data == null || data.length == 0)
        ...
        CreateTaxReportData[] dataAux = CreateTaxReportData.selectChild(this,
                Utility.getContext(this, vars, "#User_Client",
                        "CreateTaxReport"), Utility.getContext(this, vars,
                        "#User_Org", "CreateTaxReport"), data[0].id);
        if (log4j.isDebugEnabled())
            log4j.debug("**********************dataAux.length: "
                    + dataAux.length);
        for (int i = 0; i < dataAux.length; i++) {
            // if (dataAux[i].issummary.equals("Y")){
            childData(vars, vectorArray, dataAux[i].id, strPeriodFrom,
                    strPeriodTo, strOrg, level + 1, data[0].id, rownum);
            rownum++;
            // }
        }
    }
    ...
    CreateTaxReportData[] convertVector(Vector<Object> vectorArray)
        ...
        if (log4j.isDebugEnabled())
            log4j.debug("***************************data.length: "
                    + data.length);
        for (int i = data.length - 1; i >= 0; i--) {
            if (log4j.isDebugEnabled())
                log4j.debug("*******************dataissummary: " + data[i].name
                        + " " + data[i].issummary + " " + data[i].levelTax);
            if (data[i].issummary.equals("Y")) {
                for (int j = i + 1; j < data.length; j++) {
                    if (log4j.isDebugEnabled())
                        log4j
                                .debug("******************************data[j].name: "
                                        ...
                                        + data[j].levelTax
                                        + " " + data[i].levelTax);
                    if (Integer.valueOf(data[j].levelTax).intValue() > Integer
                            .valueOf(data[i].levelTax).intValue()
                    ...
                    /*
                     * String strLevelSecundary = data[j].levelTax; if
                     * (!strLevelSecundary.equals("")){ if
                     * (Integer.valueOf(strLevelSecundary).intValue() ==
                     * Integer.valueOf(strLevel).intValue()+1) { String total =
                     * data[j].total; count +=
