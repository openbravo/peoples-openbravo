...
public class Register extends HttpSecureAppServlet {
    ...
    public void doPost(HttpServletRequest request, HttpServletResponse response)
        ...
        try {
            if (!HeartbeatProcess.isInternetAvailable(this)) {
                String message = Utility.messageBD(myPool,
                        "HB_INTERNET_UNAVAILABLE", vars.getLanguage());
                log4j.error(message);
        ...
        if (data.length > 0) {
            ...
            if (rd.isregistrationactive == null
                    || rd.isregistrationactive.equals("")) {
            ...
            try {
                ...
                if ((!rd.registrationId.equals("") && rd.registrationId != null)
                        && (rd.isregistrationactive.equals("")
                                || rd.isregistrationactive == null || rd.isregistrationactive
                                .equals("N")))
                    message = Utility.messageBD(myPool, "REG_UNREGISTER", vars
                            .getLanguage());
                else
                    message = Utility.messageBD(myPool, "REG_SUCCESS", vars
                            .getLanguage());

                adviseRegistrationConfirm(response, vars, "SUCCESS",
                        "Registration", message);
            } catch (IOException e) {
                if (e instanceof SSLHandshakeException) {
                    message = Utility.messageBD(myPool,
                            "HB_SECURE_CONNECTION_ERROR", vars.getLanguage());
                    advisePopUp(response, "ERROR", "Registration", message);
                } else {
                    message = Utility.messageBD(myPool, "HB_SEND_ERROR", vars
                            .getLanguage());
                    advisePopUp(response, "ERROR", "Registration", message);
                }
            } catch (GeneralSecurityException e) {
                message = Utility.messageBD(myPool, "HB_CERTIFICATE_ERROR",
                        vars.getLanguage());
                advisePopUp(response, "ERROR", "Registration", message);
            }
        }
    ...
    public void adviseRegistrationConfirm(HttpServletResponse response,
            VariablesSecureApp vars, String strTipo, String strTitulo,
    ...
    public String register(String encodedQueryStr)
        ...
        return HttpsUtils.sendSecure(url, encodedQueryStr,
    ...
    private String createQueryString(RegisterData data) {
        ...
        sb.append("isregistrationactive="
        ...
        sb.append("contactjobtitle="
                + (contactjobtitle == null ? "" : contactjobtitle) + "&");
        ...
        try {
            sb.append("companycountry="
                    + (companycountry == null ? "" : PaisComboData.selectName(
                            this, companycountry)
                            + "&"));
        } catch (Exception e) {
            log4j.error("Error building registration request: "
                    + e.getMessage());
            sb.append("companycountry="
                    + (companycountry == null ? "" : companycountry + "&"));
        }
        sb.append("companyindustry="
                + (companyindustry == null ? "" : companyindustry) + "&");
        sb.append("companynumEmployees="
                + (companynumEmployees == null ? "" : companynumEmployees)
                + "&");
        sb.append("comments=" + (comments == null ? "" : comments) + "&");
        sb.append("contactme=" + (contactme == null ? "" : contactme) + "&");
        sb.append("wanttopartner="
                + (wanttopartner == null ? "" : wanttopartner) + "&");
        ...
        sb.append("development=" + (development == null ? "" : development)
                + "&");
        sb.append("developmentannounce="
                + (developmentAnnounce == null ? "" : developmentAnnounce)
                + "&");
