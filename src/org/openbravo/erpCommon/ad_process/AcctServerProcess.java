...
public class AcctServerProcess implements Process {
    ...
    public void execute(ProcessBundle bundle) throws Exception {
        ...
        for (int i = 0; i < tables.length; i++) {
            ...
            strTableDesc = AcctServerProcessData.selectDescription(connection,
                    ctx.getLanguage(), acct.AD_Table_ID);
            int total = 0;
            while (acct.checkDocuments()) {
                ...
                if (total == 0) {
                    ...
                    if (isDirect)
                        addLog("@DL_COUNTED@ " + total + " - " + strTableDesc,
                                false);
                    else
    ...
    public void addLog(String msg, boolean generalLog) {
        ...
        if (isDirect) {
            ...
            if (generalLog) {
                this.message.append(tmp.toString()).append(" - ").append(msg)
                        .append("<br>");
            }
            lastLog.append("<span>").append(tmp.toString()).append(" - ")
                    .append(msg).append("</span><br>");
        }
    }
