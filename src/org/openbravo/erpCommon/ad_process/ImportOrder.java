...
public class ImportOrder extends ImportProcess {
    ...
    public ImportOrder(ConnectionProvider conn, String AD_Process_ID,
            String recordId, boolean deleteOld, boolean processOrders) {
        super(conn);
    ...
    protected OBError doIt(VariablesSecureApp vars) throws ServletException {
        ...
        try {
            ...
            no = ImportOrderData.updateInvalidVendor(con, conn, getAD_Org_ID(),
                    getAD_Client_ID());
            if (log4j.isDebugEnabled())
                log4j.debug("Invalid vendor errors = " + no);
            // Invalid organization
            no = ImportOrderData.updateRecordsError(con, conn,
                    getAD_Client_ID());
            if (log4j.isDebugEnabled())
            ...
            no = ImportOrderData.updateDocTypeError(con, conn,
                    getAD_Client_ID());
            if (log4j.isDebugEnabled())
                log4j.debug("Invalid DocType errors = " + no);
            // DocType Default
            no = ImportOrderData.updateDocTypePODefault(con, conn,
                    getAD_Client_ID());
            if (log4j.isDebugEnabled())
                log4j.debug("Updated DocTypePO default = " + no);
            no = ImportOrderData.updateDocTypeSODefault(con, conn,
                    getAD_Client_ID());
            if (log4j.isDebugEnabled())
                log4j.debug("Updated DocTypeSO default = " + no);
            no = ImportOrderData.updateDocTypeDefault(con, conn,
                    getAD_Client_ID());
            if (log4j.isDebugEnabled())
                log4j.debug("Updated DocType default = " + no);
            no = ImportOrderData.updateDocTypeDefaultError(con, conn,
                    getAD_Client_ID());
            if (log4j.isDebugEnabled())
            ...
            // Price List
            no = ImportOrderData.updatePriceListCurrencyDefault(con, conn,
                    getAD_Client_ID());
            if (log4j.isDebugEnabled())
                log4j.debug("Updated PriceList by currency (default) = " + no);
            no = ImportOrderData.updatePriceListNullCurrencyDefault(con, conn,
                    getAD_Client_ID());
            if (log4j.isDebugEnabled())
                log4j.debug("Updated PriceList with null currency (default) = "
                        + no);
            no = ImportOrderData.updatePriceListCurrency(con, conn,
                    getAD_Client_ID());
            if (log4j.isDebugEnabled())
                log4j.debug("Updated PriceList by currency = " + no);
            no = ImportOrderData.updatePriceListNullCurrency(con, conn,
                    getAD_Client_ID());
            if (log4j.isDebugEnabled())
                log4j.debug("Updated PriceList with null currency = " + no);
            no = ImportOrderData.updatePriceListError(con, conn,
                    getAD_Client_ID());
            if (log4j.isDebugEnabled())
                log4j.debug("Invalid PriceList errors = " + no);
            // Set Currency
            no = ImportOrderData.updateCurrencyDefaultFromPriceList(con, conn,
                    getAD_Client_ID());
            if (log4j.isDebugEnabled())
                log4j.debug("ImportOrder Set Currency Default =" + no);
            no = ImportOrderData.updateInvalidCurrency(con, conn,
                    getAD_Client_ID());
            if (log4j.isDebugEnabled())
                log4j.debug("ImportOrder Invalid Currency =" + no);
            // Payment Term
            no = ImportOrderData
                    .updatePaymentTerm(con, conn, getAD_Client_ID());
            if (log4j.isDebugEnabled())
                log4j.debug("Updated PaymentTerm = " + no);
            no = ImportOrderData.updatePaymentTermDefault(con, conn,
                    getAD_Client_ID());
            if (log4j.isDebugEnabled())
                log4j.debug("Updated PaymentTerm default = " + no);
            no = ImportOrderData.updatePaymentTermError(con, conn,
                    getAD_Client_ID());
            if (log4j.isDebugEnabled())
            ...
            no = ImportOrderData.updateWarehouseOther(con, conn,
                    getAD_Client_ID());
            if (log4j.isDebugEnabled())
                log4j.debug("Updated Warehouse (other) = " + no);
            no = ImportOrderData.updateWarehouseError(con, conn,
                    getAD_Client_ID());
            if (log4j.isDebugEnabled())
                log4j.debug("Invalid Warehouse errors = " + no);
            // BusinessPartner
            // import depending on the external value
            no = ImportOrderData.updateBPartnerFromValue(con, conn,
                    getAD_Client_ID());
            if (log4j.isDebugEnabled())
                log4j.debug("Updated BPartner from value = " + no);
            no = ImportOrderData.updateBPartnerFromEmail(con, conn,
                    getAD_Client_ID());
            if (log4j.isDebugEnabled())
                log4j.debug("Updated BPartner from email = " + no);
            no = ImportOrderData.updateBPartnerFromContact(con, conn,
                    getAD_Client_ID());
            if (log4j.isDebugEnabled())
                log4j.debug("Updated BPartner from contact = " + no);
            no = ImportOrderData.updateBPartnerFromName(con, conn,
                    getAD_Client_ID());
            if (log4j.isDebugEnabled())
                log4j.debug("Updated BPartner from name = " + no);
            no = ImportOrderData.updateBPartnerFromUPC(con, conn,
                    getAD_Client_ID());
            if (log4j.isDebugEnabled())
                log4j.debug("Updated BPartner from upc = " + no);
            no = ImportOrderData.updateBPartnerDefault(con, conn,
                    getAD_Client_ID());
            if (log4j.isDebugEnabled())
                log4j.debug("Updated BPartner (default) = " + no);
            // update PriceList and PaymentTerm according to BPartner
            no = ImportOrderData.updatePriceListFromBPartner(con, conn,
                    getAD_Client_ID());
            if (log4j.isDebugEnabled())
                log4j.debug("Updated PriceList from BPartner = " + no);
            no = ImportOrderData.updatePOPriceListFromBPartner(con, conn,
                    getAD_Client_ID());
            if (log4j.isDebugEnabled())
                log4j.debug("Updated POPriceList from BPartner = " + no);
            no = ImportOrderData.updatePaymentTermFromBPartner(con, conn,
                    getAD_Client_ID());
            if (log4j.isDebugEnabled())
                log4j.debug("Updated PaymentTerm from BPartner = " + no);
            no = ImportOrderData.updatePOPaymentTermFromBPartner(con, conn,
                    getAD_Client_ID());
            if (log4j.isDebugEnabled())
                log4j.debug("Updated POPaymentTerm from BPartner = " + no);
            // Location
            no = ImportOrderData.updateLocationByUPC(con, conn,
                    getAD_Client_ID());
            if (log4j.isDebugEnabled())
                log4j.debug("Updated Location by UPC = " + no);
            no = ImportOrderData
                    .updateBilltoByUPC(con, conn, getAD_Client_ID());
            if (log4j.isDebugEnabled())
                log4j.debug("Updated bill location by UPC = " + no);
            no = ImportOrderData.updateLocationExisting(con, conn,
                    getAD_Client_ID());
            if (log4j.isDebugEnabled())
                log4j.debug("Updated Location (existing) = " + no);
            no = ImportOrderData.updateBillLocation(con, conn,
                    getAD_Client_ID());
            if (log4j.isDebugEnabled())
            ...
            no = ImportOrderData.updateLocationError(con, conn,
                    getAD_Client_ID());
            if (log4j.isDebugEnabled())
                log4j.debug("Invalid Location errors = " + no);
            // Country
            no = ImportOrderData.updateCountryDefault(con, conn,
                    getAD_Client_ID());
            if (log4j.isDebugEnabled())
            ...
            no = ImportOrderData.updateCountryError(con, conn,
                    getAD_Client_ID());
            if (log4j.isDebugEnabled())
                log4j.debug("Invalid Country errors = " + no);
            // Set Region
            no = ImportOrderData.updateRegionDefault(con, conn,
                    getAD_Client_ID());
            if (log4j.isDebugEnabled())
            ...
            no = ImportOrderData
                    .updateRegionError(con, conn, getAD_Client_ID());
            if (log4j.isDebugEnabled())
                log4j.debug("Invalid Region errors = " + no);
            // Product
            no = ImportOrderData.updateProductFromValue(con, conn,
                    getAD_Client_ID());
            if (log4j.isDebugEnabled())
                log4j.debug("Updated Product from value = " + no);
            no = ImportOrderData.updateProductFromUpc(con, conn,
                    getAD_Client_ID());
            if (log4j.isDebugEnabled())
                log4j.debug("Updated Product from UPC = " + no);
            no = ImportOrderData.updateProductFromSku(con, conn,
                    getAD_Client_ID());
            if (log4j.isDebugEnabled())
                log4j.debug("Updated Product from SKU = " + no);
            no = ImportOrderData.updateProductError(con, conn,
                    getAD_Client_ID());
            if (log4j.isDebugEnabled())
            ...
            // New BPartner
            ImportOrderData[] data = ImportOrderData.selectWithoutBP(conn,
                    getAD_Client_ID());
            if (log4j.isDebugEnabled())
            ...
            for (int i = 0; i < data.length; i++) {
                ...
                if (data[i].bpartnervalue == null
                        || data[i].bpartnervalue.equals("")) {
                ...
                if (data[i].contactname != null || data[i].email != null
                        || data[i].phone != null) {
            ...
            con = conn.getTransactionConnection();
            no = ImportOrderData.updateBPartnerError(con, conn,
                    getAD_Client_ID());
            if (log4j.isDebugEnabled())
            ...
            for (int i = 0; i < data.length; i++) {
                ...
                if (!order_documentno.equals(data[i].documentno)) {
                    ...
                    ImportOrderData[] orderInserted = ImportOrderData
                            .selectOrderInserted(conn, getAD_Client_ID(),
                                    data[i].adOrgId, data[i].cDoctypeId,
                                    data[i].documentno, data[i].dateordered
                                            .equals("") ? DateTimeData
                                            .today(conn) : data[i].dateordered);
                    if (orderInserted != null && orderInserted.length == 0) {
                        corder = new COrderData();
                        corder.cOrderId = SequenceIdData.getUUID();
                        if (log4j.isDebugEnabled())
                            log4j.debug("Creating new order with id = "
                                    + corder.cOrderId);
                        ...
                        corder.cDoctypeId = "0";
                        if (log4j.isDebugEnabled())
                            log4j.debug("data[i].cBpartnerLocationId: "
                                    + data[i].cBpartnerLocationId);
                        corder.cBpartnerLocationId = data[i].cBpartnerLocationId;
                        if (log4j.isDebugEnabled())
                            log4j.debug("data[i].description: "
                                    + data[i].description);
                        corder.description = data[i].description;
                        if (data[i].description != null
                                && !data[i].description.equals("")) {
                            String location_name = ImportOrderData
                                    .selectLocationName(con, conn,
                                            data[i].description);
                            if (log4j.isDebugEnabled())
                            ...
                            if (location_name != null
                                ...
                                if (log4j.isDebugEnabled())
                                    log4j.debug("corder.description: "
                                            + corder.description);
                            }
                        }
                        corder.issotrx = data[i].issotrx;
                        if (data[i].documentno != null
                                && !data[i].documentno.equals("")) {
                            corder.documentno = data[i].documentno;
                        } else {
                            String docTargetType = ImportOrderData
                                    .cDoctypeTarget(con, conn, vars.getUser(),
                                            vars.getOrg());
                            corder.documentno = Utility.getDocumentNo(conn,
                                    vars, "", "C_Order", docTargetType,
                        ...
                        if (!data[i].adUserId.equals("")
                                && Integer.parseInt(data[i].adUserId) != 0)
                        ...
                        if (!data[i].mShipperId.equals("")
                                && Integer.parseInt(data[i].mShipperId) != 0)
                            corder.mShipperId = data[i].mShipperId;
                        if (!data[i].salesrepId.equals("")
                                && Integer.parseInt(data[i].salesrepId) != 0)
                            corder.salesrepId = data[i].salesrepId;
                        if (!data[i].adOrgtrxId.equals("")
                                && Integer.parseInt(data[i].adOrgtrxId) != 0)
                            corder.adOrgtrxId = data[i].adOrgtrxId;
                        if (!data[i].cActivityId.equals("")
                                && Integer.parseInt(data[i].cActivityId) != 0)
                            corder.cActivityId = data[i].cActivityId;
                        if (!data[i].cCampaignId.equals("")
                                && Integer.parseInt(data[i].cCampaignId) != 0)
                            corder.cCampaignId = data[i].cCampaignId;
                        if (!data[i].cProjectId.equals("")
                                && Integer.parseInt(data[i].cProjectId) != 0)
                            corder.cProjectId = data[i].cProjectId;
                        if (data[i].dateordered != null
                                && !data[i].dateordered.equals(""))
                            corder.dateordered = data[i].dateordered;
                        if (data[i].datepromised != null
                                && !data[i].datepromised.equals(""))
                            corder.datepromised = data[i].datepromised;
                        if (log4j.isDebugEnabled())
                            log4j.debug("getting bp values as default");
                        SEOrderBPartnerData[] data1 = SEOrderBPartnerData
                                .select(conn, data[i].cBpartnerId);
                        String tmpCurrency = COrderData.selectCurrency(conn,
                                vars.getUser(), data[i].cBpartnerId);
                        ...
                        corder.paymentrule = data1[0].paymentrule.equals("") ? ""
                                : data1[0].paymentrule;
                        if (data1[0].paymentrule != null
                                && !data1[0].paymentrule.equals("")) {
                            corder.paymentrule = data1[0].paymentrule;
                        } else {
                            String defaultPaymentRule = ImportOrderData
                                    .defaultValue(con, conn, "C_Order",
                                            "PaymentRule");
                            corder.paymentrule = (defaultPaymentRule == null || defaultPaymentRule
                                    .equals("")) ? "P" : defaultPaymentRule; // P
                            // =
                            // on
                            // credit
                        }
                        if (log4j.isDebugEnabled())
                            log4j.debug("corder.paymentrule = "
                                    + corder.paymentrule);
                        if (data1[0].invoicerule != null
                                && !data1[0].invoicerule.equals("")) {
                            corder.invoicerule = data1[0].invoicerule;
                        } else {
                            String defaultInvoiceRule = ImportOrderData
                                    .defaultValue(con, conn, "C_Order",
                                            "InvoiceRule");
                            corder.invoicerule = (defaultInvoiceRule == null || defaultInvoiceRule
                                    .equals("")) ? "I" : defaultInvoiceRule; // I
                            // =
                            // immediate
                        }
                        if (log4j.isDebugEnabled())
                            log4j.debug("corder.invoicerule = "
                                    + corder.invoicerule);
                        if (data1[0].deliveryrule != null
                                && !data1[0].deliveryrule.equals("")) {
                            corder.deliveryrule = data1[0].deliveryrule;
                        } else {
                            String defaultDeliveryRule = ImportOrderData
                                    .defaultValue(con, conn, "C_Order",
                                            "DeliveryRule");
                            corder.deliveryrule = (defaultDeliveryRule == null || defaultDeliveryRule
                                    .equals("")) ? "A" : defaultDeliveryRule; // A
                            // =
                            // availability
                        }
                        if (log4j.isDebugEnabled())
                            log4j.debug("corder.deliveryrule = "
                                    + corder.deliveryrule);
                        if (data1[0].deliveryviarule != null
                                && !data1[0].deliveryviarule.equals("")) {
                            corder.deliveryviarule = data1[0].deliveryviarule;
                        } else {
                            String defaultDeliveryViaRule = ImportOrderData
                                    .defaultValue(con, conn, "C_Order",
                                            "DeliveryViaRule");
                        ...
                        if (log4j.isDebugEnabled())
                            log4j.debug("corder.deliveryviarule = "
                                    + corder.deliveryviarule);
                        corder.freightcostrule = "I"; // I = included
                        if (log4j.isDebugEnabled())
                            log4j.debug("corder.freightcostrule = "
                                    + corder.freightcostrule);
                        if (data1[0].cPaymenttermId != null
                                && !data1[0].cPaymenttermId.equals("")) {
                            corder.cPaymenttermId = data1[0].cPaymenttermId;
                        }
                        if (log4j.isDebugEnabled())
                            log4j.debug("corder.cPaymenttermId = "
                                    + corder.cPaymenttermId);
                        if (data1[0].paymentrule != null
                                && !data1[0].paymentrule.equals("")) {
                            corder.paymentrule = data1[0].paymentrule;
                        } else {
                        ...
                        if (log4j.isDebugEnabled())
                            log4j.debug("corder.paymentrule = "
                                    + corder.paymentrule);
                        if (data1[0].salesrepId != null
                                && !data1[0].salesrepId.equals("")) {
                            String salesrep = ImportOrderData.selectSalesRep(
                                    con, conn, data[i].cBpartnerId);
                            corder.salesrepId = salesrep;
                        }
                        if (log4j.isDebugEnabled())
                            log4j.debug("corder.salesrepId = "
                                    + corder.salesrepId);
                        String user = ImportOrderData.defaultUser(con, conn,
                                data[i].cBpartnerId);
                        ...
                        try {
                            ...
                            if (log4j.isDebugEnabled())
                                log4j.debug("Insert Order - " + ex.toString());
                            conn.releaseRollbackConnection(con);
                            ImportOrderData.importOrderError(conn, ex
                                    .toString(), I_Order_ID);
                            continue;
                        }
                        noInsert++;
                        ...
                        lineNo = Integer
                                .parseInt(orderInserted[0].linedescription);
                    }
                }
                ...
                lineNo += 10;
                if (data[i].mProductId != null
                        && !data[i].mProductId.equals("")
                ...
                if (line.mProductId != null && !line.mProductId.equals("")) {
                    ProductPriceData[] pprice = ProductPriceData.selectPL(conn,
                            line.mProductId, corder_mpricelistid);
                    if (pprice.length > 0) {
                        line.pricestd = pprice[0].pricestd;
                        line.priceactual = pprice[0].pricestd;
                        line.pricelist = (pprice[0].pricelist == null && pprice[0].pricelist
                ...
                if (data[i].cTaxId != null && !data[i].cTaxId.equals("")
                    ...
                    try {
                        line.cTaxId = Tax.get(conn, data[i].mProductId,
                                DateTimeData.today(conn), data[i].adOrgId,
                                data[i].mWarehouseId.equals("") ? vars
                                        .getWarehouse() : data[i].mWarehouseId,
                                ExpenseSOrderData.cBPartnerLocationId(conn,
                                        data[i].cBpartnerId), ExpenseSOrderData
                                        .cBPartnerLocationId(conn,
                                                data[i].cBpartnerId),
                                data[i].cProjectId, true);
                ...
                if (line.cTaxId == null || line.cTaxId.equals(""))
                    line.cTaxId = ProductPriceData.selectCTaxId(conn, vars
                            .getClient());
                data[i].cTaxId = line.cTaxId;

                line.dateordered = data[i].dateordered;
                line.mWarehouseId = (data[i].mWarehouseId == null || data[i].mWarehouseId
                        .equals("")) ? vars.getWarehouse()
                        : data[i].mWarehouseId;
                if (line.cUomId == null || line.cUomId.equals(""))
                    line.cUomId = ProductPriceData.selectCUomIdByProduct(conn,
                            line.mProductId);
                if (line.cUomId == null || line.cUomId.equals(""))
                    line.cUomId = ProductPriceData.selectCUomIdDefault(conn);
                line.cCurrencyId = (data[i].cCurrencyId == null || data[i].cCurrencyId
                        .equals("")) ? corder_ccurrencyid : data[i].cCurrencyId;
                try {
                ...
                data[i].processed = "Y";
                data[i].update(con, conn);
                String[] arrayPayment = { data[i].paymentrule1,
                        data[i].paymentrule2 };
                ...
                for (int k = 0; k < arrayPayment.length; k++) {
                    if (!arrayPayment[k].equals("")) {
                        ...
                        cdebtpayment.dateplanned = data[i].dateordered
                                .equals("") ? DateTimeData.today(conn)
                                : data[i].dateordered;
                        ...
                        ImportOrderData[] paymentInserted = ImportOrderData
                                .selectPaymentInserted(conn,
                                        cdebtpayment.adClientId,
                                        cdebtpayment.adOrgId,
                                        cdebtpayment.createdby,
                                        cdebtpayment.updatedby,
                                        cdebtpayment.cBpartnerId,
                                        cdebtpayment.cCurrencyId,
                                        cdebtpayment.paymentrule,
                                        cdebtpayment.amount,
                                        cdebtpayment.ispaid,
                                        cdebtpayment.ismanual,
                                        cdebtpayment.isvalid,
                                        cdebtpayment.changesettlementcancel,
                                        cdebtpayment.cancelProcessed,
                                        cdebtpayment.generateProcessed,
                                        cdebtpayment.glitemamt,
                                        cdebtpayment.isdirectposting,
                                        cdebtpayment.status,
                                        cdebtpayment.statusInitial,
                                        cdebtpayment.cOrderId);
                        if (paymentInserted != null
                                && paymentInserted.length == 0) {
                            try {
                                cdebtpayment.cDebtPaymentId = SequenceIdData
                                        .getUUID();
                                cdebtpayment.insert(con, conn);
                            } catch (ServletException ex) {
                                if (log4j.isDebugEnabled())
                                    log4j.debug("Insert Order - "
                                            + ex.toString());
                                conn.releaseRollbackConnection(con);
                                ImportOrderData.importOrderError(conn, ex
                                        .toString(), I_Order_ID);
                ...
                try {
                    if (data[i].performPost.equals("Y") || m_processOrders) {
                        if (i != data.length - 1) {
                            if (!order_documentno
                                    .equals(data[i + 1].documentno))
                                strPostMessage += cOrderPost(con, conn, vars,
                                        data[i].cOrderId, order_documentno)
                                        + ", ";
                        } else {
                            strPostMessage += cOrderPost(con, conn, vars,
                                    data[i].cOrderId, order_documentno)
                    ...
                    ImportOrderData.updatePostError(con, conn, strPostMessage,
                            data[i].orderReferenceno, getAD_Client_ID());
                }
                noInsertLine++;
                conn.releaseCommitConnection(con);
            }
            con = conn.getTransactionConnection();
            noOrderError = ImportOrderData.updateNotImported(con, conn,
                    getAD_Client_ID());

            addLog(Utility.messageBD(conn, "Orders not imported", vars
                    .getLanguage())
                    + ": " + noOrderError + "; ");
            addLog("Orders inserted: " + noInsert + "; ");
            addLog("Orders line inserted: " + noInsertLine + "; " + "<br>");
            addLog("Orders processed: " + Integer.toString(totalProcessed)
                    + "; " + "<br>");
            ...
            if (noOrderError == 0) {
                ...
                myError.setTitle(Utility
                        .messageBD(conn, "Some orders could not be imported",
                                vars.getLanguage()));
            } else {
                myError.setType("Error");
                myError.setTitle(Utility.messageBD(conn,
                        " No orders could be imported", vars.getLanguage()));
            }
            myError.setMessage(Utility.messageBD(conn, getLog(), vars
                    .getLanguage()));
    ...
    String cOrderPost(Connection con, ConnectionProvider conn,
            VariablesSecureApp vars, String strcOrderId, String order_documentno)
            throws IOException, ServletException {
        String pinstance = SequenceIdData.getUUID();
        PInstanceProcessData.insertPInstance(conn, pinstance, "104",
        ...
        if (myMessage.getType().equals("Success")
                || myMessage.getType().equals("Warning")) {
            totalProcessed = totalProcessed + 1;
        }
        /*
         * if (pinstanceData!=null && pinstanceData.length>0) { if
         * (!pinstanceData[0].errormsg.equals("")) { String message =
         * pinstanceData[0].errormsg; if (message.startsWith("@") &&
         * message.endsWith("@")) { message = message.substring(1,
         * message.length()-1); if (message.indexOf("@")==-1) messageResult =
         * Utility.messageBD(conn, message, vars.getLanguage()); else
         * messageResult = Utility.parseTranslation(conn, vars,
         * vars.getLanguage(), "@" + message + "@"); } else { messageResult =
         * Utility.parseTranslation(conn, vars, vars.getLanguage(), message); }
         * } else if (!pinstanceData[0].pMsg.equals("")) { String message =
         * pinstanceData[0].pMsg; messageResult = Utility.parseTranslation(conn,
         * vars, vars.getLanguage(), message); total += 1; } else if
         * (pinstanceData[0].result.equals("1")) { messageResult =
         * Utility.messageBD(conn, "Success", vars.getLanguage()); total += 1; }
         * else { messageResult = Utility.messageBD(conn, "PostOrderError",
         * vars.getLanguage()); ImportOrderData.updatePostError(con, conn,
