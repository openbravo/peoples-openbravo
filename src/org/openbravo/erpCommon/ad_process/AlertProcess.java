...
public class AlertProcess implements Process {
    ...
    public void execute(ProcessBundle bundle) throws Exception {
        ...
        logger
                .log("Starting Alert Backgrouond Process. Loop " + counter
                        + "\n");

        try {
            AlertProcessData[] alertRule = AlertProcessData
    ...
    private void processAlert(AlertProcessData alertRule,
            ConnectionProvider connection) throws Exception {
        ...
        if (alert != null && alert.length != 0) {
            ...
            for (int i = 0; i < alert.length; i++) {
                if (AlertProcessData.existsReference(connection,
                        alertRule.adAlertruleId, alert[i].referencekeyId)
                        .equals("0")) {
                    ...
                    AlertProcessData.InsertAlert(connection, adAlertId,
                            alert[i].adClientId, alert[i].adOrgId,
                            alert[i].created, alert[i].createdby,
                            alertRule.adAlertruleId, alert[i].recordId,
            ...
            if (insertions > 0) {
                // Send mail
                AlertProcessData[] mail = AlertProcessData.prepareMails(
                        connection, alertRule.adAlertruleId);

                if (mail != null) {
                    for (int i = 0; i < mail.length; i++) {
                        String head = Utility.messageBD(connection,
                                "AlertMailHead", mail[i].adLanguage)
                                + "\n";
                        EMail email = new EMail(null, mail[i].smtphost,
                                mail[i].mailfrom, mail[i].mailto, "[OB Alert] "
                                        + alertRule.name, head + msg);
                        String pwd = FormatUtilities.encryptDecrypt(
                                mail[i].requestuserpw, false);
        ...
        if (!alertRule.sql.equals("")) {
            try {
                Integer count = AlertProcessData.updateAlert(connection,
                        alertRule.adAlertruleId, alertRule.sql);
