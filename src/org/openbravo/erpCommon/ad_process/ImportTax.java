  ...
  protected boolean doIt(VariablesSecureApp vars) throws ServletException {
    ...
    Connection con = null;
      ...
      /* This cannot be done like this, because it can be referred by data inside the file, it has to be done line per line
			//  Tax Category ID
			no = ImportTaxData.updateTaxCategoryId(con, conn, getAD_Client_ID());
			if (log4j.isDebugEnabled()) log4j.debug("Tax Category Id = " + no);
      
      // BP Tax Category ID
      no = ImportTaxData.updateBPTaxCategoryId(con, conn, getAD_Client_ID());
			if (log4j.isDebugEnabled()) log4j.debug("BP Tax Category Id = " + no);
      
      // Tax ID
      no = ImportTaxData.updateTaxId(con, conn, getAD_Client_ID());
			if (log4j.isDebugEnabled()) log4j.debug("BP Tax Category Id = " + no);*/
      
      // Set Country From
      no = ImportTaxData.updateCountryFromId(con, conn, getAD_Client_ID());
			if (log4j.isDebugEnabled()) log4j.debug("Set Country From =" + no);
			
			no = ImportTaxData.updateCountryFromError(con, conn, getAD_Client_ID());
			if (log4j.isDebugEnabled()) log4j.debug("Invalid Country From =" + no);
      
      // Set Country To
      no = ImportTaxData.updateCountryToId(con, conn, getAD_Client_ID());
			if (log4j.isDebugEnabled()) log4j.debug("Set Country to =" + no);
			
			no = ImportTaxData.updateCountryToError(con, conn, getAD_Client_ID());
			if (log4j.isDebugEnabled()) log4j.debug("Invalid Country to =" + no);
      
      // Set Region From
      no = ImportTaxData.updateRegionFromId(con, conn, getAD_Client_ID());
			if (log4j.isDebugEnabled()) log4j.debug("Set Region From =" + no);
			
			no = ImportTaxData.updateRegionFromError(con, conn, getAD_Client_ID());
			if (log4j.isDebugEnabled()) log4j.debug("Invalid Region From =" + no);
      
      // Set Region From
      no = ImportTaxData.updateRegionToId(con, conn, getAD_Client_ID());
			if (log4j.isDebugEnabled()) log4j.debug("Set Region To =" + no);
			
			no = ImportTaxData.updateRegionToError(con, conn, getAD_Client_ID());
			if (log4j.isDebugEnabled()) log4j.debug("Invalid Region To =" + no);
    ...
    int noBPTCUpdate = 0;
        ...
        String ParentTax_ID = data[i].parentTaxId;
				boolean newTaxCategory = C_TaxCategory_ID == "";
        boolean newBPTaxCategory = C_BPTaxCategory_ID == "";
        boolean newTax = C_Tax_ID == ""; 
				con = conn.getTransactionConnection();
          ...
          if (newTaxCategory) {	//	Insert new TaxCategory
            ...
            try {
              ...
              if (log4j.isDebugEnabled()) log4j.debug(err);
            ...
            try	{
              ...
              if (log4j.isDebugEnabled()) log4j.debug("Update TaxCategory = " + no);
              noTCUpdate++;
            }	catch (ServletException ex)	{
        ...
        if (!data[i].bptcName.equals("") || !newBPTaxCategory){
          ...
          if (newTaxCategory) {	//	Insert new BPTaxCategory
            ...
            try	{
              ...
              if (log4j.isDebugEnabled()) log4j.debug("Update BPTaxCategory = " + no);
              noBPTCUpdate++;
            }	catch (ServletException ex)	{
        ...
        if (!data[i].tName.equals("") || !newTax) {  //Update Create Tax
           ...
           if (newTax) C_Tax_ID = ImportTaxData.selectTaxId(conn, data[i].tName, getAD_Client_ID());
           ...
           if (newTax) {	//	Insert new Tax
            ...
            try	{
              no = ImportTaxData.updateTax(con, conn, I_Tax_ID, C_TaxCategory_ID, C_BPTaxCategory_ID, ParentTax_ID, C_Tax_ID);
              if (log4j.isDebugEnabled()) log4j.debug("Update Tax = " + no);
              noTaxUpdate++;
            }	catch (ServletException ex)	{
          ...
          if ((!data[i].cCountryId.equals("")||!data[i].cRegionId.equals("")||!data[i].toCountryId.equals("0")||!data[i].toRegionId.equals("0"))
            ...
            if (ImportTaxData.hasLocation(con, conn, C_Tax_ID).equals("0")) {  //Update Location in c_tax
              try {
                no = ImportTaxData.updateTaxRegion(con, conn, I_Tax_ID, C_Tax_ID);
                if (log4j.isDebugEnabled()) log4j.debug("Update Tax Location (in c_tax) = " + no);
              } catch (ServletException ex) {
