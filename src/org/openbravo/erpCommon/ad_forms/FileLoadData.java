...
public class FileLoadData extends MultipartRequest {
    ...
    public FileLoadData(VariablesBase _vars, String _filename,
            boolean _firstLineHeads, String _format, FieldProvider[] _data)
            throws IOException {
        ...
        while (siguiente < linea.length()) {
                ...
                if (texto.charAt(0) == '"') {
                    ...
                    if (texto.charAt(texto.length() - 1) == '"') {
                        ...
                        if (siguiente == -1)
                            siguiente = linea.length();
                        texto = texto + format
                                + linea.substring(anterior, siguiente);
            ...
            try {
                if (log4j.isDebugEnabled())
                    log4j.debug("FileLoadData - setFieldProvider - text = "
                            + texto);
