...
public class RemittanceCancel extends HttpSecureAppServlet {
    ...
    public void doPost(HttpServletRequest request, HttpServletResponse response)
        ...
        if (vars.commandIn("DEFAULT")) {
            String strDesde = vars.getGlobalVariable("inpDesde",
                    "RemittanceCancel.inpDesde", "");
            String strHasta = vars.getGlobalVariable("inpHasta",
                    "RemittanceCancel.inpHasta", "");
            String strRemittanceId = vars.getGlobalVariable("inpRemittanceId",
                    "RemittanceCancel.inpRemittanceId", "");
            if (log4j.isDebugEnabled())
                log4j.debug("FRAME1--inpDesde:" + strDesde + " inpHasta:"
                        + strHasta + "inpRemittanceId:" + strRemittanceId);
            ...
            if (!strRemittanceId.equals(""))
                vars.setSessionValue("RemittanceCancel.inpRemittanceId",
                        strRemittanceId);
            else
                vars.removeSessionValue("RemittanceCancel.inpRemittanceId");
            if (log4j.isDebugEnabled())
                log4j.debug("FIND--inpDesde:" + strDesde + " inpHasta:"
                        + strHasta + "inpRemittanceId:" + strRemittanceId);
            printPage(response, vars, strDesde, strHasta, strRemittanceId);
            ...
            OBError myMessage = processCancel(vars, strRemittanceLineId,
                    strDateAcct);
            ...
            String strRemittanceLineId = vars.getInStringParameter("inpcRLId");
            String strDateAcct = vars.getRequiredStringParameter("inpDateAcct");
            OBError myMessage = processReturn(vars, strRemittanceLineId,
                    strDateAcct);
            vars.setMessage("RemittanceCancel", myMessage);
            String strDesde = vars.getGlobalVariable("inpDesde",
                    "RemittanceCancel.inpDesde", "");
    ...
    OBError processReturn(VariablesSecureApp vars, String strRemittanceLineId,
            String strDateAcct) {
        if (log4j.isDebugEnabled())
        ...
        try {
            ...
            if (strRemittanceLineId.startsWith("("))
                strRemittanceLineId = strRemittanceLineId.substring(1,
                        strRemittanceLineId.length() - 1);
            strRemittanceLineId = Replace.replace(strRemittanceLineId, "'", "");
            StringTokenizer st = new StringTokenizer(strRemittanceLineId, ",",
                    false);
            ...
            while (st.hasMoreTokens()) {
                ...
                if (strDPMID.equals("")) {
                    ...
                    line = new Integer(RemittanceCancelData
                            .getLineDPMConnection(conn, this, strDPMID));
                    String strDocumentNo = Utility.getDocumentNoConnection(
                            conn, this, vars.getClient(), "C_DP_Management",
                            true);
                    try {
                        RemittanceCancelData.insertDPManagement(conn, this,
                                strDPMID, vars.getClient(), vars.getOrg(), vars
                                        .getUser(), strDocumentNo, strDateAcct,
                                strRLId);
                ...
                RemittanceCancelData.returnDPOriginal(conn, this, strLID, vars
                        .getClient(), vars.getOrg(), vars.getUser(), strDPMID,
                        strRLId, line.toString());
            ...
            if (!strDPMID.equals("")) {
                ...
                try {
                    PInstanceProcessData.insertPInstance(conn, this, pinstance,
                            "800140", strDPMID, "N", vars.getUser(), vars
                ...
                PInstanceProcessData[] pinstanceData = PInstanceProcessData
                        .selectConnection(conn, this, pinstance);
    ...
    OBError processCancel(VariablesSecureApp vars, String strRemittanceLineId,
            String strDateAcct) {
        if (log4j.isDebugEnabled())
            ...
            if (!strRemittanceLineId.equals("")) {
                ...
                while (st.hasMoreTokens()) {
                    ...
                    if (strSettlementID.equals("")) {
                        String strOrg = RemittanceCancelData.selectOrg(this,
                                strRLId);
                        strSettlementID = SequenceIdData.getUUID();
                        strDocumentNo = Utility.getDocumentNo(this, vars
                                .getClient(), "C_Settlement", true);
                        try {
                            RemittanceCancelData.insertSettlement(conn, this,
                                    strSettlementID, vars.getClient(), strOrg,
                                    vars.getUser(), strDocumentNo, strDateAcct,
                                    strRLId);
                        } catch (ServletException ex) {
                            myMessage = Utility.translateError(this, vars, vars
                                    .getLanguage(), ex.getMessage());
                        ...
                        if (log4j.isDebugEnabled())
                            log4j.debug("Inserted Settlement "
                                    + strSettlementID);
                    }
                    RemittanceCancelData.cancel(conn, this, strSettlementID,
                            strRLId);
                    if (log4j.isDebugEnabled())
                ...
                if (!strSettlementID.equals("")) {
                    String pinstance = SequenceIdData.getUUID();
                    if (log4j.isDebugEnabled())
                        log4j.debug("call c_settlement_post pinstnce "
                                + pinstance);
                    RemittanceCancelData.process800025(conn, this,
                            strSettlementID);
    ...
    void printPage(HttpServletResponse response, VariablesSecureApp vars,
            String strDesde, String strHasta, String strRemittanceId)
            throws IOException, ServletException {
        if (log4j.isDebugEnabled())
            log4j.debug("Output: Frame 1 of the CancelRemittance seeker");
        XmlDocument xmlDocument;
        RemittanceCancelData data[] = RemittanceCancelData.select(this, vars
                .getLanguage(), Utility.getContext(this, vars, "#User_Client",
                "RemittanceCancel"), Utility.getContext(this, vars,
                "#User_Org", "RemittanceCancel"), strRemittanceId, strDesde,
                strHasta);
        ...
        ToolBar toolbar = new ToolBar(this, vars.getLanguage(),
                "RemittanceCancel", false, "", "", "", false, "ad_forms",
                strReplaceWith, false, true, true);
        ...
        xmlDocument.setParameter("dateTosaveFormat", vars
                .getSessionValue("#AD_SqlDateFormat"));
        xmlDocument.setParameter("dateAcctdisplayFormat", vars
                .getSessionValue("#AD_SqlDateFormat"));
        xmlDocument.setParameter("dateAcctsaveFormat", vars
                .getSessionValue("#AD_SqlDateFormat"));
        ...
        try {
            ...
            NavigationBar nav = new NavigationBar(this, vars.getLanguage(),
                    "RemittanceCancel.html", classInfo.id, classInfo.type,
                    strReplaceWith, tabs.breadcrumb());
            xmlDocument.setParameter("navigationBar", nav.toString());
            LeftTabsBar lBar = new LeftTabsBar(this, vars.getLanguage(),
                    "RemittanceCancel.html", strReplaceWith);
        ...
        xmlDocument.setData("reportRemittance_ID", "liststructure",
                RemittanceComboData.selectNoCanceled(this, vars.getLanguage(),
                        vars.getUserClient(), vars.getUserOrg()));
        if (!strDesde.equals(""))
