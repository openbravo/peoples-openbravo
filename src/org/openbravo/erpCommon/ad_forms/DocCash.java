...
public class DocCash extends AcctServer {
     ...
     *            AD_Client_ID
     */
    public DocCash(String AD_Client_ID, String AD_Org_ID,
    ...
    public void loadObjectFieldProvider(ConnectionProvider conn,
        ...
        log4jDocCash
                .debug("************************** DocCash - loadObjectFieldProvider - ID - "
                        + Id);
        setObjectFieldProvider(DocCashData.selectRegistro(conn, AD_Client_ID,
                Id));
    }
    ...
    public boolean loadDocumentDetails(FieldProvider[] data,
        ...
        log4jDocCash.debug("data.length = " + data.length
                + " - DocumentType = " + DocumentType);
        DateDoc = data[0].getField("StatementDate");

        // Amounts
        Amounts[AcctServer.AMTTYPE_Gross] = data[0]
                .getField("StatementDifference");
    ...
    private void setCashBookInfo() {
        ...
        log4jDocCash.debug("setCashBookInfo - C_Currency_ID = " + C_Currency_ID
                + " - AD_Org_ID = " + AD_Org_ID + " - C_CashBook_ID = "
                + C_CashBook_ID);
    } // setCashBookInfo
    ...
    private DocLine[] loadLines(ConnectionProvider conn) {
        ...
        try {
            ...
            for (int i = 0; data != null && i < data.length; i++) {
                ...
                DocLine_Cash docLine = new DocLine_Cash(DocumentType,
                        Record_ID, t_Line_ID);
                ...
                docLine.m_C_BPartner_ID = DocLineCashData.selectDebtBPartner(
                        connectionProvider, docLine.m_C_Debt_Payment_Id);
                docLine.m_C_Glitem_ID = data[i].cGlitemId;
                docLine.setReference(data[i].cOrderId, data[i].cDebtPaymentId,
                        conn);
                docLine.setAmount(data[i].amount, data[i].discountamt,
                        data[i].writeoffamt);
                list.add(docLine);
            }
        } catch (ServletException e) {
    ...
    public BigDecimal getBalance() {
        ...
        for (int i = 0; i < p_lines.length; i++) {
            retValue = retValue
                    .subtract(new BigDecimal(p_lines[i].getAmount()));
    ...
    public Fact createFact(AcctSchema as, ConnectionProvider conn,
        ...
        for (int i = 0; p_lines != null && i < p_lines.length; i++) {
            ...
            log4jDocCash.debug("antes del creteline, line.getAmount(): "
                    + line.getAmount() + " - CashType: " + CashType);
            if (CashType.equals(DocLine_Cash.CASHTYPE_EXPENSE)) {
                ...
                log4jDocCash
                        .debug("********** DocCash - factAcct - account - "
                                + getAccount(AcctServer.ACCTTYPE_CashExpense,
                                        as, conn).C_ValidCombination_ID);
                fact.createLine(line, getAccount(
                        AcctServer.ACCTTYPE_CashExpense, as, conn),
                        C_Currency_ID, amount.negate().toString(), "",
                ...
                log4jDocCash
                        .debug("********** DocCash - factAcct - account - "
                                + getAccount(AcctServer.ACCTTYPE_CashReceipt,
                                        as, conn).C_ValidCombination_ID);
                fact.createLine(line, getAccount(
                        AcctServer.ACCTTYPE_CashReceipt, as, conn),
                        C_Currency_ID, "", amount.toString(),
                        Fact_Acct_Group_ID, nextSeqNo(SeqNo), DocumentType,
                        conn);
                ...
                log4jDocCash
                        .debug("********** DocCash - factAcct - account - "
                                + line.getChargeAccount(as, new BigDecimal(
                                        getAmount()), conn).C_ValidCombination_ID);
                fact.createLine(line, line.getChargeAccount(as, new BigDecimal(
                        getAmount()), conn), C_Currency_ID, amount.negate()
                ...
                log4jDocCash
                        .debug("********** DocCash - factAcct - account - "
                                + line.getGlitemAccount(as, new BigDecimal(
                                        getAmount()), conn).C_ValidCombination_ID);
                fact.createLine(line, line.getGlitemAccount(as, new BigDecimal(
                        getAmount()), conn), C_Currency_ID, amount.negate()
                        .toString(), "", Fact_Acct_Group_ID, nextSeqNo(SeqNo),
                        DocumentType, conn);
                assetAmt = assetAmt.subtract(amount.negate());
                ...
                log4jDocCash.debug("********** DocCash - factAcct - account - "
                        + getAccount(AcctServer.ACCTTYPE_CashDifference, as,
                                conn).C_ValidCombination_ID);
                fact.createLine(line, getAccount(
                        AcctServer.ACCTTYPE_CashDifference, as, conn),
                        C_Currency_ID, amount.negate().toString(),
                        Fact_Acct_Group_ID, nextSeqNo(SeqNo), DocumentType,
                        conn);
                // fact.createLine(line,
                ...
                if (line.getC_Currency_ID(conn) == C_Currency_ID)
                    ...
                    fact.createLine(null, getAccount(
                            AcctServer.ACCTTYPE_CashAsset, as, conn), line
                            .getC_Currency_ID(conn), amount.toString(),
                            Fact_Acct_Group_ID, nextSeqNo(SeqNo), DocumentType,
                            conn);
                fact.createLine(line, getAccount(
                        AcctServer.ACCTTYPE_CashTransfer, as, conn), line
                        .getC_Currency_ID(conn), amount.negate().toString(),
                        Fact_Acct_Group_ID, nextSeqNo(SeqNo), DocumentType,
                        conn);
                ...
                if (amount.signum() == 1) {
                    log4jDocCash
                            .debug("********** DocCash - factAcct - amount - "
                                    + amount.toString() + " - debit");
                    log4jDocCash
                            .debug("********** DocCash - factAcct - account - "
                                    + getAccount(
                                            AcctServer.ACCTTYPE_BankInTransitDefault,
                                            as, conn).C_ValidCombination_ID);
                    ...
                    fact.createLine(line,
                            getAccount(
                                    AcctServer.ACCTTYPE_BankInTransitDefault,
                                    as, conn), line.getC_Currency_ID(conn), "",
                            amount.toString(), Fact_Acct_Group_ID,
                            nextSeqNo(SeqNo), DocumentType, conn);
                    fact.createLine(null, getAccount(
                            AcctServer.ACCTTYPE_CashAsset, as, conn),
                            C_Currency_ID, amount.toString(), "",
                            Fact_Acct_Group_ID, nextSeqNo(SeqNo), DocumentType,
                            conn);
              ...
              * log4jDocCash.debug("********************* pasa por aqui " +
              * CashType); String BPartner = ""; String isSOTrx = "";
              * DocCashData [] data = null; try{ data =
              * DocCashData.selectBPartner(conn, line.Line_ID); } catch
              * (ServletException e){ log4jDocCash.warn(e); } if (data!=null &&
              * data.length > 0){ BPartner = data[0].cBpartnerId; isSOTrx =
              * data[0].issotrx; }
              ...
              * conn),line.getC_Currency_ID(conn), "", amount.toString(),
              * Fact_Acct_Group_ID, nextSeqNo(SeqNo), DocumentType, conn);
              * //fact
              * .createLine(line,getAccount(AcctServer.ACCTTYPE_CashReceipt, as,
              * conn),line.getC_Currency_ID(conn), "",
              * amount.negate().toString(), conn); assetAmt =
              * assetAmt.add(amount); }else{
              * //fact.createLine(line,getAccount(AcctServer
              * .ACCTTYPE_CashExpense, as, conn),line.getC_Currency_ID(conn),
              * "", amount.toString(), conn);
              * log4jDocCash.debug("********** DocCash - factAcct - account - "
              * + getAccountBPartner(false,BPartner, as,
              * conn).C_ValidCombination_ID);
              * fact.createLine(line,getAccountBPartner(false,BPartner, as,
              * conn),line.getC_Currency_ID(conn), amount.negate().toString(),
              * "", Fact_Acct_Group_ID, nextSeqNo(SeqNo), DocumentType, conn);
              * assetAmt = assetAmt.subtract(amount.negate()); } }
              */else if (CashType.equals(DocLine_Cash.CASHTYPE_TRANSFER)) {
                ...
                log4jDocCash.debug("********** DocCash - factAcct - account - "
                        + getAccount(AcctServer.ACCTTYPE_BankInTransit, as,
                                conn).C_ValidCombination_ID);
                fact.createLine(line, getAccountBankInTransit(C_BankAccount_ID,
                        as, conn), line.getC_Currency_ID(conn), amount.negate()
                        .toString(), Fact_Acct_Group_ID, nextSeqNo(SeqNo),
                        DocumentType, conn);
        ...
        // Cash Asset
        log4jDocCash
                .debug("********** DocCash - factAcct - account2 - "
                        + getAccount(AcctServer.ACCTTYPE_CashAsset, as, conn).C_ValidCombination_ID);
        if (!assetAmt.toString().equals("0"))
            fact.createLine(null, getAccount(AcctServer.ACCTTYPE_CashAsset, as,
                    conn), C_Currency_ID, assetAmt.toString(),
                    Fact_Acct_Group_ID, nextSeqNo(SeqNo), DocumentType, conn);
        SeqNo = "0";
        return fact;
    } // createFact
     ...
     * @return Account
     * 
     *         public final Account getAccountCashInTransit(String
     *         strcCashlineId, AcctSchema as, ConnectionProvider conn){
     *         DocCashData [] data=null; try{ data =
     *         DocCashData.selectCashLineAcct(conn, strcCashlineId,
     *         as.getC_AcctSchema_ID()); }catch(ServletException e){
     *         log4j.warn(e); } // Get Acct String Account_ID = ""; if (data !=
     *         null && data.length!=0){ Account_ID = data[0].accountId; }else
     *         return null; // No account if (Account_ID.equals("")){
     *         log4j.warn(
     *         "DocCash - getAccountCashInTransit - NO account CashLine=" +
     *         strcCashlineId + ", Record=" + Record_ID); return null; } //
     *         Return Account Account acct = null; try{ acct =
     *         Account.getAccount(conn, Account_ID); }catch(ServletException e){
     *         log4j.warn(e); } return acct; } // getAccount
     */
    ...
    public final Account getAccountBankInTransit(String strcBankAccountId,
            AcctSchema as, ConnectionProvider conn) {
        DocCashData[] data = null;
        try {
            data = DocCashData.selectBankInTransitAcct(conn, strcBankAccountId,
        ...
        if (Account_ID.equals("")) {
            log4j
                    .warn("DocCash - getAccountBankInTransit - NO account strcBankAccountId="
