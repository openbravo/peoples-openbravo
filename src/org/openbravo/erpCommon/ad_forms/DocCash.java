...
private void setCashBookInfo(){
    ...
    if (data!=null && data.length!=0){
        ...
        if (AD_Org_ID==null || AD_Org_ID.equals("")) AD_Org_ID = data[0].adOrgId;
        C_Currency_ID = data[0].cCurrencyId;
    }
    log4jDocCash.debug("setCashBookInfo - C_Currency_ID = " + C_Currency_ID + " - AD_Org_ID = " + AD_Org_ID + " - C_CashBook_ID = " + C_CashBook_ID);
}   //  setCashBookInfo
...
jarenor
    ...
    public Fact createFact (AcctSchema as,ConnectionProvider conn,Connection con,VariablesSecureApp vars) throws ServletException{
        ...
        for (int i = 0;p_lines!=null && i < p_lines.length; i++){
            ...
            log4jDocCash.debug("antes del creteline, line.getAmount(): " + line.getAmount() + " - CashType: " + CashType);
            if (CashType.equals(DocLine_Cash.CASHTYPE_EXPENSE)){
                ...
                fact.createLine(line, getAccount(AcctServer.ACCTTYPE_CashExpense, as, conn),C_Currency_ID, amount.negate().toString(), "", Fact_Acct_Group_ID, nextSeqNo(SeqNo), DocumentType, conn);
            //  fact.createLine(line, getAccount(AcctServer.ACCTTYPE_CashAsset, as),
          ...
          //      p_vo.C_Currency_ID, null, line.getAmount().negate());
              ...
              assetAmt = assetAmt.subtract(amount.negate());
                ...
                fact.createLine(line, getAccount(AcctServer.ACCTTYPE_CashDifference, as, conn),C_Currency_ID, amount.negate().toString(), Fact_Acct_Group_ID, nextSeqNo(SeqNo), DocumentType, conn);
            //  fact.createLine(line, getAccount(AcctServer.ACCTTYPE_CashAsset, as),
            //      p_vo.C_Currency_ID, line.getAmount());
                ...
                fact.createLine(line,getAccount(AcctServer.ACCTTYPE_CashTransfer, as, conn),line.getC_Currency_ID(conn), amount.negate().toString(), Fact_Acct_Group_ID, nextSeqNo(SeqNo), DocumentType, conn);
            }else if (CashType.equals(DocLine_Cash.CASHTYPE_DEBTPAYMENT) || CashType.equals(DocLine_Cash.CASHTYPE_ORDER)){
                if (amount.signum() == 1){
...
//                    fact.createLine(line,getAccountCashInTransit(line.m_TrxLine_ID, as, conn),line.getC_Currency_ID(conn), amount.negate().toString(), "", Fact_Acct_Group_ID, nextSeqNo(SeqNo), DocumentType, conn);
                ...
                log4jDocCash.debug("********************* pasa por aqui " + CashType);
                String BPartner = "";
                String isSOTrx = "";
                DocCashData [] data = null;
                try{
                    data = DocCashData.selectBPartner(conn, line.Line_ID);
                }
                catch (ServletException e){
                    log4jDocCash.warn(e);
                }
                if (data!=null && data.length > 0){
                  BPartner = data[0].cBpartnerId;
                  isSOTrx = data[0].issotrx;
                }
                log4jDocCash.debug("DocCash CASHTYPE_ORDER - C_CURRENCY_ID = " + line.getC_Currency_ID(conn));
                if (isSOTrx.equals("Y")){
                    fact.createLine(line,getAccountBPartner(true,BPartner, as, conn),line.getC_Currency_ID(conn), "", amount.toString(), Fact_Acct_Group_ID, nextSeqNo(SeqNo), DocumentType, conn);
                    //fact.createLine(line,getAccount(AcctServer.ACCTTYPE_CashReceipt, as, conn),line.getC_Currency_ID(conn), "", amount.negate().toString(), conn);
                    assetAmt = assetAmt.add(amount);
                }else{
                    //fact.createLine(line,getAccount(AcctServer.ACCTTYPE_CashExpense, as, conn),line.getC_Currency_ID(conn), "", amount.toString(), conn);
                log4jDocCash.debug("********** DocCash - factAcct - account - " + getAccountBPartner(false,BPartner, as, conn).C_ValidCombination_ID);
                    fact.createLine(line,getAccountBPartner(false,BPartner, as, conn),line.getC_Currency_ID(conn), amount.negate().toString(), "", Fact_Acct_Group_ID, nextSeqNo(SeqNo), DocumentType, conn);
                    assetAmt = assetAmt.subtract(amount.negate());
                }
        ...
        if(!assetAmt.toString().equals("0"))fact.createLine(null, getAccount(AcctServer.ACCTTYPE_CashAsset, as, conn),C_Currency_ID, assetAmt.toString(), Fact_Acct_Group_ID, nextSeqNo(SeqNo), DocumentType, conn);
        SeqNo = "0";
        return fact;
    }   //  createFact
