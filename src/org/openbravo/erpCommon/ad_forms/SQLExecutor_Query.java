...
public class SQLExecutor_Query implements FieldProvider {
  ...
  public static SQLExecutor_Query[] select(ConnectionProvider connectionProvider, String strSQL) throws ServletException {
    return select(connectionProvider, strSQL, 0, 0);
  }

  public static SQLExecutor_Query[] select(ConnectionProvider connectionProvider, String strSQL, int firstRegister, int numberRegisters) throws ServletException {
    ...
    try {
      if (log4j.isDebugEnabled()) log4j.debug("select - Preparing Statement\n");
      st = connectionProvider.getPreparedStatement(strSQL);
      if (log4j.isDebugEnabled()) log4j.debug("select - Statement Prepared\n");
      if (log4j.isDebugEnabled()) log4j.debug("select - Executing Statement\n");
      result = st.executeQuery();
      if (log4j.isDebugEnabled()) log4j.debug("select - Statement Executed\n");
      long countRecord = 0;
      ...
      if (log4j.isDebugEnabled()) log4j.debug("select - Making data\n");
      while(continueResult && result.next()) {
        ...
        for (int i=1;i<=numColumns;i++) {
          ...
          if (countRecord>1) {
            ...
            if (auxType.equals("NUMBER") && auxName.toUpperCase().endsWith("_ID")) auxType="ID";
      ...
      if (log4j.isDebugEnabled()) log4j.debug("select - Closing resultset\n");
      result.close();
    ...
    if (log4j.isDebugEnabled()) log4j.debug("select - returning data\n");
  ...
  public static String transformSQLType(int sql_type) {
    ...
    switch (sql_type) {
      ...
      case Types.SMALLINT:
      case Types.TINYINT:
      case Types.BIGINT: strType = "INTEGER";
            break;
      case Types.CLOB:
      case Types.BLOB:
      case Types.BINARY: strType = "FILE";
            break;
      case Types.DECIMAL:
      case Types.DOUBLE:
      case Types.FLOAT:
      ...
      case Types.NUMERIC:
      case Types.REAL:
      case Types.BIT: strType = "NUMBER";
            break;
      case Types.BOOLEAN: strType = "BOOLEAN";
            break;
      case Types.DATE: strType = "DATE";
            break;
      case Types.TIME: strType = "TIME";
            break;
      case Types.TIMESTAMP: strType = "DATETIME";
            break;
      default: strType = "STRING";
    }
    return strType;
  }
}
