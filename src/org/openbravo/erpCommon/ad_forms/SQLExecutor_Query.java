...
public class SQLExecutor_Query implements FieldProvider {
    ...
    public static SQLExecutor_Query[] select(
            ConnectionProvider connectionProvider, String strSQL)
    ...
    public static SQLExecutor_Query[] select(
        ...
        try {
            ...
            while (continueResult && result.next()) {
                ...
                for (int i = 1; i <= numColumns; i++) {
                    ...
                    if (countRecord > 1) {
                        ...
                        String auxType = transformSQLType(rmeta
                                .getColumnType(i));
                        String auxName = rmeta.getColumnName(i);
                        if (auxType.equals("NUMBER")
                                && auxName.toUpperCase().endsWith("_ID"))
                            auxType = "ID";
            ...
            log4j.error("No connection available error in query: " + strSQL
                    + "Exception:" + ex);
        ...
        SQLExecutor_Query objectSQLExecutor_Query[] = new SQLExecutor_Query[vector
                .size()];
