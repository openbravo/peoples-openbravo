...
public class SQLExecutor extends HttpSecureAppServlet {
  ...
  public void doPost (HttpServletRequest request, HttpServletResponse response) throws IOException,ServletException {
    ...
    if (vars.commandIn("DEFAULT")) {
      ...
      try {
        ...
        //vars.setSessionValue("SQLExecutor|message", Utility.messageBD(this, "Error", vars.getLanguage()));
      }
  ...
  private String lastRange(VariablesSecureApp vars, String strSQL, String strRecordRange) {
    ...
    initRecord -= Integer.parseInt(strRecordRange);
    if (initRecord<0) initRecord = 0;
    return Integer.toString(initRecord);

  }
  private void printExcel(HttpServletResponse response, VariablesSecureApp vars, String strSQL, SQLExecutor_Query[] data) throws IOException, ServletException{
    log4j.info("print page");
    if (log4j.isDebugEnabled()) log4j.debug("printPage - Reading xml\n");
    XmlDocument xmlDocument = xmlEngine.readXmlTemplate("org/openbravo/erpCommon/ad_forms/SQLExecutor_Excel").createXmlDocument();
    ...
    if (log4j.isDebugEnabled()) log4j.debug("printPage - Printing document\n");
    out.println(xmlDocument.print());
    out.close();
    if (log4j.isDebugEnabled()) log4j.debug("printPage - End printing document\n");
  }
  ...
  private void printPage(HttpServletResponse response, VariablesSecureApp vars, String strSQL, SQLExecutor_Query[] data, String strInitRecord, int initRecordNumber, int intRecordRange) throws IOException, ServletException{
    ...
    if (log4j.isDebugEnabled()) log4j.debug("printPage - Making grid data\n");
    int selectedIndex = 0;
    if (data!=null && data.length!=0) {
      for (int countHeads=0;countHeads<data[0].type.size();countHeads++) {
        strHeader.append("obj.addHeader(new Header(\"").append(data[0].name.elementAt(countHeads)).append("\", \"").append(data[0].type.elementAt(countHeads)).append("\", 100, 20, true));\n");
      }
      for (int contadorData=0;contadorData<data.length;contadorData++) {
        strData.append("new Array(");
        for (int countHeads=0;countHeads<data[0].data.size();countHeads++) {
          if (countHeads>0) strData.append(",");
          strData.append("\"").append(Replace.replace(Replace.replace(Replace.replace(data[contadorData].getField(Integer.toString(countHeads)), "\r", ""), "\n", "<br>"), "\"", "\\\"")).append("\"");
        }
        strData.append(")");
        if (contadorData < (data.length-1)) strData.append(",");
        strData.append("\n");
      }
    } else {
      strInitRecord = "0";
    ...
    strData.append(");");
    String strGrid = "obj.setSelectedRow(" + Integer.toString(selectedIndex) + ");\n";
    strGrid += "obj.setInitRowNum(" + (strInitRecord.equals("0")?"1":strInitRecord) + ");\n";
    xmlDocument.setParameter("grid", strGrid);
    xmlDocument.setParameter("header", strHeader.toString());
    ...
    ToolBar toolbar = new ToolBar(this, vars.getLanguage(), "SQLExecutor", false, "document.frmMain.inpKey", "myGrid", null, false, "ad_forms", strReplaceWith, false, true);
    toolbar.prepareQueryTemplate((initRecordNumber>1), (data!=null && data.length!=0 && data.length>=intRecordRange), vars.getSessionValue("#ShowTest", "N").equals("Y"));
    ...
    try {
      ...
      log4j.debug("theme");
      NavigationBar nav = new NavigationBar(this, vars.getLanguage(), "SQLExecutor.html", classInfo.id, classInfo.type, strReplaceWith, tabs.breadcrumb());
      xmlDocument.setParameter("navigationBar", nav.toString());
      log4j.debug("navigationBar");
      LeftTabsBar lBar = new LeftTabsBar(this, vars.getLanguage(), "SQLExecutor.html", strReplaceWith);
