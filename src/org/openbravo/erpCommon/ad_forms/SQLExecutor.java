...
public class SQLExecutor extends HttpSecureAppServlet {
    ...
    public void doPost(HttpServletRequest request, HttpServletResponse response)
        ...
        if (vars.commandIn("DEFAULT")) {
            ...
            try {
                ...
                data = SQLExecutor_Query.select(this, strSQL, initRecordNumber,
                        intRecordRange);
            } catch (Exception ignored) {
            }
            printPage(response, vars, strSQL, data, strInitRecord,
                    initRecordNumber, intRecordRange);
        } else if (vars.commandIn("FIND")) {
            String strSQL = vars.getRequestGlobalVariable("inpSQL",
                    "SQLExecutor|sql");
            SQLExecutor_Query[] data = null;
            String strRecordRange = Utility.getContext(this, vars,
                    "#RecordRange", "SQLExecutor");
            ...
            printPage(response, vars, strSQL, data, "0", initRecordNumber,
                    intRecordRange);
            ...
            if (strInitRecord.equals("") || strInitRecord.equals("0")) {
                ...
                strInitRecord = ((initRecord < 0) ? "0" : Integer
                        .toString(initRecord));
                vars.setSessionValue("SQLExecutor|initRecordNumber",
                        strInitRecord);
            }
            response.sendRedirect(strDireccion + request.getServletPath()
                    + "?Command=DEFAULT");
    ...
    private String lastRange(VariablesSecureApp vars, String strSQL,
    ...
    private void printExcel(HttpServletResponse response,
            VariablesSecureApp vars, String strSQL, SQLExecutor_Query[] data)
            throws IOException, ServletException {
        log4j.info("print page");
        if (log4j.isDebugEnabled())
            log4j.debug("printPage - Reading xml\n");
        XmlDocument xmlDocument = xmlEngine.readXmlTemplate(
                "org/openbravo/erpCommon/ad_forms/SQLExecutor_Excel")
                .createXmlDocument();
            ...
            for (int j = 0; j < data.length; j++) {
                if (j != 0) {
                    ...
                    for (int k = 0; k < data[0].name.size(); k++) {
                        ...
                        dataBuffer
                                .append(data[j].getField(Integer.toString(k)));
                        dataBuffer.append("</td>\n");
                    }
                    dataBuffer.append("</tr>\n");
                }
    ...
    private void printPage(HttpServletResponse response,
            VariablesSecureApp vars, String strSQL, SQLExecutor_Query[] data,
            String strInitRecord, int initRecordNumber, int intRecordRange)
            throws IOException, ServletException {
        log4j.info("print page");
        if (log4j.isDebugEnabled())
            log4j.debug("printPage - Reading xml\n");
        XmlDocument xmlDocument = xmlEngine.readXmlTemplate(
                "org/openbravo/erpCommon/ad_forms/SQLExecutor")
                .createXmlDocument();
        ...
        if (data != null && data.length != 0) {
            ...
            for (int j = 0; j < data.length; j++) {
                if (j != 0) {
                    int evenOdd = j % 2;
                    dataBuffer.append("<tr class=\"TableDetailRow"
                            + String.valueOf(evenOdd) + "\">\n");
                    for (int k = 0; k < data[0].name.size(); k++) {
                        dataBuffer.append("<td>");
                        dataBuffer
                                .append(data[j].getField(Integer.toString(k)));
         ...
         * StringBuffer strHeader = new StringBuffer(); if
         * (log4j.isDebugEnabled())
         * log4j.debug("printPage - Making grid data\n"); int selectedIndex = 0;
         * if (data!=null && data.length!=0) { for (int
         * countHeads=0;countHeads<data[0].type.size();countHeads++) {
         * strHeader.
         * append("obj.addHeader(new Header(\"").append(data[0].name.elementAt
         * (countHeads
         * )).append("\", \"").append(data[0].type.elementAt(countHeads
         * )).append("\", 100, 20, true));\n"); } for (int
         * contadorData=0;contadorData<data.length;contadorData++) {
         * strData.append("new Array("); for (int
         * countHeads=0;countHeads<data[0].data.size();countHeads++) { if
         * (countHeads>0) strData.append(",");
         * strData.append("\"").append(Replace
         * .replace(Replace.replace(Replace.replace
         * (data[contadorData].getField(Integer.toString(countHeads)), "\r",
         * ""), "\n", "<br>"), "\"", "\\\"")).append("\""); }
         * strData.append(")"); if (contadorData < (data.length-1))
         * strData.append(","); strData.append("\n"); } } else { strInitRecord =
         * "0"; } strData.append(");"); String strGrid = "obj.setSelectedRow(" +
         * Integer.toString(selectedIndex) + ");\n"; strGrid +=
         * "obj.setInitRowNum(" + (strInitRecord.equals("0")?"1":strInitRecord)
        ...
        ToolBar toolbar = new ToolBar(this, vars.getLanguage(), "SQLExecutor",
                false, "document.frmMain.inpKey", "myGrid", null, false,
                "ad_forms", strReplaceWith, false, true);
        toolbar.prepareQueryTemplate((initRecordNumber > 1), (data != null
                && data.length != 0 && data.length >= intRecordRange), vars
        ...
        try {
            ...
            NavigationBar nav = new NavigationBar(this, vars.getLanguage(),
                    "SQLExecutor.html", classInfo.id, classInfo.type,
                    strReplaceWith, tabs.breadcrumb());
            xmlDocument.setParameter("navigationBar", nav.toString());
            log4j.debug("navigationBar");
            LeftTabsBar lBar = new LeftTabsBar(this, vars.getLanguage(),
                    "SQLExecutor.html", strReplaceWith);
