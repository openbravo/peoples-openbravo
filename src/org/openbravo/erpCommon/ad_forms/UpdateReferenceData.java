...
public class UpdateReferenceData extends HttpSecureAppServlet {
  ...
  private void printPage(HttpServletResponse response, VariablesSecureApp vars, String strOrganization) throws IOException, ServletException{
    ModuleReferenceDataOrgTree tree = new ModuleReferenceDataOrgTree(this, vars.getClient(), strOrganization, true);
    ...
    try {
      WindowTabs tabs = new WindowTabs(this, vars, "org.openbravo.erpCommon.ad_forms.UpdateReferenceData");
      xmlDocument.setParameter("parentTabContainer", tabs.parentTabs());
      xmlDocument.setParameter("mainTabContainer", tabs.mainTabs());
      xmlDocument.setParameter("childTabContainer", tabs.childTabs());
      xmlDocument.setParameter("theme", vars.getTheme());
      NavigationBar nav = new NavigationBar(this, vars.getLanguage(), "UpdateReferenceData.html", classInfo.id, classInfo.type, strReplaceWith, tabs.breadcrumb());
      xmlDocument.setParameter("navigationBar", nav.toString());
      LeftTabsBar lBar = new LeftTabsBar(this, vars.getLanguage(), "UpdateReferenceData.html", strReplaceWith);
    ...
    xmlDocument.setData("reportAD_Org_ID", "liststructure", UpdateReferenceDataData.selectOrganization(this, vars.getRole(), vars.getUserOrg()));
  ...
  private void printPageResult(HttpServletResponse response, VariablesSecureApp vars, String strResultado) throws IOException, ServletException{
    ...
    myMessage.setTitle("");
    if(log4j.isDebugEnabled()) log4j.debug("UpdateReferenceData - before setMessage");
    if(strError!=null && !strError.equals("")) {
      myMessage = Utility.translateError(this, vars, vars.getLanguage(), strError);
    }
    if(log4j.isDebugEnabled()) log4j.debug("UpdateReferenceData - isOK: " + strResultado.equals(""));
    if(strResultado.equals("")) {
      myMessage.setType("Success");
    }
    else myMessage.setType("Error");
    myMessage.setMessage(Utility.messageBD(this, strResultado.equals("")?"Success":strResultado, vars.getLanguage()));
    if(log4j.isDebugEnabled()) log4j.debug("UpdateReferenceData - Message Type: " + myMessage.getType());
    vars.setMessage("UpdateReferenceData", myMessage);
    if(log4j.isDebugEnabled()) log4j.debug("UpdateReferenceData - after setMessage");
    if (myMessage!=null) {
      xmlDocument.setParameter("messageType", myMessage.getType());
      xmlDocument.setParameter("messageTitle", myMessage.getTitle());
      xmlDocument.setParameter("messageMessage", myMessage.getMessage());
  ...
  public String updateReferenceData(HttpServletRequest request, HttpServletResponse response, VariablesSecureApp vars) throws IOException, ServletException {
      ...
      if(data!=null && data.length!=0) {
        ...
        for(int i=0;i<data.length;i++){
          ...
          if (!strError.toString().equals("")) return strError.toString();
            ...
            if(UpdateReferenceDataData.selectRegister(this, data[i].adModuleId, strOrganization).equals("0"))InitialOrgSetupData.insertOrgModule(this, vars.getClient(), strOrganization, vars.getUser(), data[i].adModuleId, data[i].version);
            else UpdateReferenceDataData.updateOrgModule(this, data[i].version, vars.getUser(), vars.getClient(), strOrganization, data[i].adModuleId);
