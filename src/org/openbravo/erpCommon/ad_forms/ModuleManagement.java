...
/**
 * This servlet is in charge of showing the Module Manager Console which have
 * three tabs: *Installed modules *Add Modules *Installation history
 * 
 * 
 */
public class ModuleManagement extends HttpSecureAppServlet {
    ...
    public void doPost(HttpServletRequest request, HttpServletResponse response)
        ...
        if (vars.commandIn("DEFAULT")) {
            ...
            final String record = vars.getStringParameter("inpcRecordId");
            printPageInstall1(response, vars, record, false, null,
                    new String[0]);
            ...
            if (!fi.getName().toUpperCase().endsWith(".OBX")) {
                ...
                message.setTitle(Utility.messageBD(this, message.getType(),
                        vars.getLanguage()));
                message.setMessage(Utility.messageBD(this, "MOD_OBX", vars
                        .getLanguage()));
                printSearchFile(response, vars, message);
            } else {
                vars.setSessionObject("ModuleManagementInstall|File", vars
                        .getMultiFile("inpFile"));
                printPageInstall1(response, vars, null, true, fi
                        .getInputStream(), new String[0]);
            }
        } else if (vars.commandIn("UNINSTALL")) {
            final String modules = vars.getInStringParameter("inpNodes");
            final UninstallModule um = new UninstallModule(this, vars
                    .getSessionValue("#sourcePath"), vars);
            um.execute(modules);
            ...
            printPageInstall1(response, vars, null, false, null,
                    modulesToUpdate);
        } else
            pageError(response);
    }
    ...
    /**
     * Show the tab for installed modules, where it is possible to look for
     * updates, uninstall and apply changes-
     * 
     * @param response
     * @param vars
     * @throws IOException
     ...
     */
    void printPageInstalled(HttpServletResponse response,
        ...
        final XmlDocument xmlDocument = xmlEngine.readXmlTemplate(
                "org/openbravo/erpCommon/ad_forms/ModuleManagementInstalled")
                .createXmlDocument();
        ...
        final ToolBar toolbar = new ToolBar(this, vars.getLanguage(),
                "ModuleManagement", false, "", "", "", false, "ad_forms",
        ...
        try {
            ...
            final NavigationBar nav = new NavigationBar(this, vars
                    .getLanguage(), "ModuleManagement.html", classInfo.id,
                    classInfo.type, strReplaceWith, tabs.breadcrumb());
            xmlDocument.setParameter("navigationBar", nav.toString());
            final LeftTabsBar lBar = new LeftTabsBar(this, vars.getLanguage(),
                    "ModuleManagement.html", strReplaceWith);
            xmlDocument.setParameter("leftTabs", lBar.manualTemplate());
        } catch (final Exception ex) {
            throw new ServletException(ex);
        }
        {
            final OBError myMessage = vars
                    .getMessage("ModuleManagement|message");
     ...
     * @throws ServletException
     */
    void printPageApply(HttpServletResponse response, VariablesSecureApp vars)
            throws IOException, ServletException {
        try {
            final XmlDocument xmlDocument = xmlEngine.readXmlTemplate(
                    "org/openbravo/erpCommon/ad_forms/ApplyModule")
                    .createXmlDocument();
            final PrintWriter out = response.getWriter();
            response.setContentType("text/html; charset=UTF-8");
            out.println(xmlDocument.print());
    ...
    /**
     * Returns an HTML with the available notifications which can be: *Unapplied
     * changes: rebuild system *Available updates: install them
     * 
     * @param lang
     * @return
     */
        ...
        try {
            ...
            if (!total.equals("0")) {
                rt += total
                        ...
                        + Utility.messageBD(this, "InstallUpdatesNow", lang)
                        + "</a>";
            }
        } catch (final Exception e) {
            e.printStackTrace();
    ...
    /**
     * Displays the second tab: Add modules where it is possible to search and
     * install modules remotely or locally
     * 
     * @param request
     * @param response
     * @param vars
    ...
    void printPageAdd(HttpServletRequest request, HttpServletResponse response,
        ...
        final PrintWriter out = response.getWriter();
        final XmlDocument xmlDocument = xmlEngine.readXmlTemplate(
                "org/openbravo/erpCommon/ad_forms/ModuleManagementAdd")
                .createXmlDocument();
        xmlDocument.setParameter("directory", "var baseDirectory = \""
                + strReplaceWith + "/\";\n");
        xmlDocument.setParameter("language", "defaultLang=\""
    ...
    /**
     * Displays the third tab "Installation History" with a log of all
     ...
     * @throws ServletException
     */
    void printPageHistory(HttpServletResponse response,
        ...
        final PrintWriter out = response.getWriter();
        final XmlDocument xmlDocument = xmlEngine.readXmlTemplate(
                "org/openbravo/erpCommon/ad_forms/ModuleManagementHistory")
                .createXmlDocument();
        xmlDocument.setParameter("directory", "var baseDirectory = \""
                + strReplaceWith + "/\";\n");
        xmlDocument.setParameter("language", "defaultLang=\""
        ...
        try {
            ComboTableData comboTableData = new ComboTableData(vars, this,
                    "18", "AD_User_ID", "110", "", Utility.getContext(this,
                            vars, "#User_Org", "ModuleManagement"), Utility
                            .getContext(this, vars, "#User_Client",
                                    "ModuleManagement"), 0);
            Utility.fillSQLParameters(this, vars, null, comboTableData,
                    "ModuleManagement", strUser);
            xmlDocument.setData("reportUser", "liststructure", comboTableData
                    .select(false));
            comboTableData = null;
        } catch (final Exception ex) {
            throw new ServletException(ex);
        ...
        final ModuleManagementData data[] = ModuleManagementData.selectLog(
                this, vars.getLanguage(), strUser, strDateFrom, strDateTo);
    ...
    void printPageDetail(HttpServletResponse response, VariablesSecureApp vars,
        ...
        if (!local) {
            ...
            final ImportModule im = (ImportModule) vars
                    .getSessionObject("InstallModule|ImportModule");
        ...
        final XmlDocument xmlDocument = xmlEngine.readXmlTemplate(
                "org/openbravo/erpCommon/ad_forms/ModuleManagementDetails",
                discard).createXmlDocument();
        xmlDocument.setParameter("language", "defaultLang=\""
                + vars.getLanguage() + "\";");
        xmlDocument.setParameter("theme", vars.getTheme());
        xmlDocument.setParameter("key", recordId);
        xmlDocument.setParameter("type", (module.getType() == null ? "M"
                : module.getType()).equals("M") ? "Module" : module.getType()
                .equals("T") ? "Template" : "Pack");
        ...
        if (dependencies != null && dependencies.length > 0)
            xmlDocument.setData("dependencies", FieldProviderFactory
                    .getFieldProviderArray(dependencies));

        if (includes != null && includes.length > 0)
            xmlDocument.setData("includes", FieldProviderFactory
                    .getFieldProviderArray(includes));
    ...
    /**
     * Shows the first pop-up for the installation process, where it is
     * displayed the modules to install/update and an error message in case it
     * is not possible to install the selected one or a warning message in case
     * the selected version is not installable but it is possible to install
     * another one.
     * 
     * @param response
     * @param vars
     ...
     * @throws ServletException
     */
    void printPageInstall1(HttpServletResponse response,
            VariablesSecureApp vars, String recordId, boolean islocal,
            InputStream obx, String[] updateModules) throws IOException,
            ServletException {
        final String discard[] = { "", "", "", "", "", "" };
        Module module = null;
        ...
        final ImportModule im = new ImportModule(this, vars
                .getSessionValue("#sourcePath"), vars);
        im.setInstallLocal(islocal);
        try {
            // check the dependenies and obtain the modules to install/update
            if (!islocal) {
                final String[] installableModules = { module != null ? module
                        .getModuleVersionID() : "" };
                check = im.checkDependenciesId(installableModules,
                        updateModules);
            } else {
                ...
                if (installOrig == null || installOrig.length == 0)
                    ...
                    if (!islocal && module != null) {
                        ...
                        for (int i = 0; i < installOrig.length; i++) {
                            found = installOrig[i].getModuleID().equals(
                                    module.getModuleID());
                            if (found
                                    && !module
                                            .getModuleVersionID()
                                ...
                                message.setTitle(Utility.messageBD(this,
                                        message.getType(), vars.getLanguage()));
                                message
                                        .setMessage(module.getName()
                                                + " "
                                                + module.getVersionNo()
                                                + " "
                                                + Utility
            ...
            if ((upd == null || upd.length == 0)
                    && (inst == null || inst.length == 0) && (module == null)) {
            ...
            message = new OBError();
            message.setType("Error");
            message.setTitle(Utility.messageBD(this, message.getType(), vars
                    .getLanguage()));
            message.setMessage(e.toString());
        }

        final XmlDocument xmlDocument = xmlEngine.readXmlTemplate(
                "org/openbravo/erpCommon/ad_forms/ModuleManagement_InstallP1",
                discard).createXmlDocument();
        xmlDocument.setParameter("directory", "var baseDirectory = \""
                + strReplaceWith + "/\";\n");
        xmlDocument.setParameter("language", "defaultLang=\""
        ...
        xmlDocument.setParameter("theme", vars.getTheme());
        if (inst != null && inst.length > 0)
            xmlDocument.setData("installs", FieldProviderFactory
                    .getFieldProviderArray(inst));

        if (upd != null && upd.length > 0)
            xmlDocument.setData("updates", FieldProviderFactory
        ...
        }
        {
            if (message != null) {
                ...
                xmlDocument
                        .setParameter("messageMessage", message.getMessage());
            }
        }
        response.setContentType("text/html; charset=UTF-8");
        final PrintWriter out = response.getWriter();
    ...
    /**
     * Shows the second installation pup-up with all the license agreements for
     * the modules to install/update
     * 
     * @param response
     * @param vars
     * @throws IOException
     * @throws ServletException
     */
    void printPageInstall2(HttpServletResponse response, VariablesSecureApp vars)
            throws IOException, ServletException {
        Module[] inst = null;
        Module[] selected;
        ...
        if (!islocal) {
            ...
            int j = 0;
            for (int i = 0; i < installOrig.length; i++) {
                final boolean found = installOrig[i].getModuleID().equals(
                        adModuleId);
                if (found) {
        ...
        final XmlDocument xmlDocument = xmlEngine.readXmlTemplate(
                "org/openbravo/erpCommon/ad_forms/ModuleManagement_InstallP2",
                discard).createXmlDocument();
        ...
        int position = 1;
        if (selected != null && selected.length > 0) {
            final FieldProvider[] fp = FieldProviderFactory
                    .getFieldProviderArray(selected);
            for (int i = 0; i < fp.length; i++)
                FieldProviderFactory.setField(fp[i], "position", new Integer(
                        position++).toString());
    ...
    /**
     * Shows the third pup-up for the installation process, in this popup the
     * installation is executed and afterwards a message is displayed with the
     * success or fail information.
     * 
     * @param response
     * @param vars
     * @throws IOException
     * @throws ServletException
     */
    void printPageInstall3(HttpServletResponse response, VariablesSecureApp vars)
            throws IOException, ServletException {
        final ImportModule im = (ImportModule) vars
                .getSessionObject("InstallModule|ImportModule");
        final XmlDocument xmlDocument = xmlEngine.readXmlTemplate(
                "org/openbravo/erpCommon/ad_forms/ModuleManagement_InstallP4")
                .createXmlDocument();
        xmlDocument.setParameter("directory", "var baseDirectory = \""
                + strReplaceWith + "/\";\n");
        xmlDocument.setParameter("language", "defaultLang=\""
        ...
        OBError message;
        if (im.getIsLocal())
            im.execute(((FileItem) vars
                    .getSessionObject("ModuleManagementInstall|File"))
                    .getInputStream(), ((FileItem) vars
                    .getSessionObject("ModuleManagementInstall|File"))
                    .getInputStream());
        else
            im.execute();
    ...
    /**
     * Executes a search query in the web service and returns a HTML with the
     * list of modules retrieved from the query. This list is HTML with styles.
     * 
     * @param request
     * @param response
     * @param vars
    ...
    private String getSearchResults(HttpServletRequest request,
            HttpServletResponse response, VariablesSecureApp vars, String text) {
        ...
        final XmlDocument xmlDocument = xmlEngine.readXmlTemplate(
                "org/openbravo/erpCommon/modules/ModuleBox")
                .createXmlDocument();
        xmlDocument.setData("structureBox", FieldProviderFactory
                .getFieldProviderArray(modules));
     ...
     * search in the webservice and not to obtain in the list the already
     * installed ones.
     * 
     * @return
     */
    private String[] getInstalledModules() {
        try {
            final ModuleManagementData data[] = ModuleManagementData
                    .selectInstalled(this);
     ...
     * @throws ServletException
     */
    void printLicenseAgreement(HttpServletResponse response,
            VariablesSecureApp vars, String record) throws IOException,
            ServletException {
        if (log4j.isDebugEnabled())
    ...
    /**
     * Displays the pop-up for the search locally file in order to look for an
     * obx file and to install it locally.
     * 
     * @param response
     * @param vars
     * @throws IOException
     * @throws ServletException
     */
    void printSearchFile(HttpServletResponse response, VariablesSecureApp vars,
            OBError message) throws IOException, ServletException {
        final XmlDocument xmlDocument = xmlEngine
                .readXmlTemplate(
                        "org/openbravo/erpCommon/ad_forms/ModuleManagement_InstallLocal")
                .createXmlDocument();
        xmlDocument.setParameter("directory", "var baseDirectory = \""
                + strReplaceWith + "/\";\n");
        xmlDocument.setParameter("language", "defaultLang=\""
    ...
    void printScan(HttpServletResponse response, VariablesSecureApp vars)
            throws IOException, ServletException {
        if (log4j.isDebugEnabled())
            log4j.debug("Output: ajaxreponse");

        final HashMap<String, String> updates = ImportModule.scanForUpdates(
                this, vars);
        String up = "";
        ...
        if (notifications.equals(""))
            notifications = Utility.messageBD(this, "NoUpdatesAvailable", vars
                    .getLanguage());
        up = notifications + "|" + up + "|";
        response.setContentType("text/plain; charset=UTF-8");
        response.setHeader("Cache-Control", "no-cache");
