...
public void doPost (HttpServletRequest request, HttpServletResponse response) throws IOException,ServletException {
  ...
  /**
   * Show the tab for installed modules, where it is possible to look for updates, uninstall and apply changes-
   * 
   * @param response
   * @param vars
   * @throws IOException
  ...
  /**
   * Returns an HTML with the available notifications which can be:
   *   *Unapplied changes: rebuild system
   *   *Available updates: install them
   * @param lang
   * @return
   */
  private String getNotificationsHTML(String lang) {
    ...
    try {
      ...
      if (!total.equals("0")) {
        rt += total + "&nbsp;"+Utility.messageBD(this, "UpdateAvailable", lang)
  ...
  /**
   * Displays the second tab: Add modules where it is possible to search and install modules remotely or locally
   * 
   * @param request
   * @param response
   * @param vars
  ...
  void printPageAdd(HttpServletRequest request, HttpServletResponse response, VariablesSecureApp vars, String searchText, boolean displaySearch) throws IOException, ServletException {
    ...
    if (displaySearch)
      xmlDocument.setParameter("searchResults", getSearchResults(request, response, vars, searchText));
  ...
  void printPageHistory(HttpServletResponse response, VariablesSecureApp vars, String strDateFrom, String strDateTo, String strUser) throws IOException, ServletException {
    ...
    try {
      ComboTableData comboTableData = new ComboTableData(vars, this, "18", "AD_User_ID", "110", "", Utility.getContext(this, vars, "#User_Org", "ModuleManagement"), Utility.getContext(this, vars, "#User_Client", "ModuleManagement"), 0);
   ...
   * @throws ServletException
   */
  void printPageDetail(HttpServletResponse response, VariablesSecureApp vars, String recordId, boolean local) throws IOException, ServletException {
    Module module = null;
    if (!local) {
      try {
    ...
    xmlDocument.setParameter("type"            , (module.getType()==null?"M":module.getType()).equals("M")?"Module":module.getType().equals("T")?"Template":"Pack");
  ...
  /**
   * Shows the first pop-up for the installation process, where it is displayed the modules to install/update and an
   * error message in case it is not possible to install the selected one or a warning message in case the selected
   * version is not installable but it is possible to install another one.
   * 
   * @param response
   * @param vars
   * @param recordId
  ...
  void printPageInstall1(HttpServletResponse response, VariablesSecureApp vars, String recordId, boolean islocal, InputStream obx, String[] updateModules) throws IOException, ServletException {
    final String discard[]={"","", "", "", "", ""};
     ...
     try {
       ...
       if (check) { //dependencies are statisfied, show modules to install
         ...
         if (installOrig==null || installOrig.length==0) discard[0] = "modulesToinstall";
           ...
           if (!islocal && module!=null) {
             inst = new Module[installOrig.length-1]; //to remove the module itself
             ...
             for (int i=0; i<installOrig.length; i++){
               ...
               if (found && !module.getModuleVersionID().equals(installOrig[i].getModuleVersionID())) {
                 ...
                 message.setMessage(module.getName()+" "+module.getVersionNo()+" "+Utility.messageBD(this,"OtherModuleVersionToinstallOrigall",vars.getLanguage())+ " " +installOrig[i].getVersionNo());
               }
               if (found) {
         ...
         // after all the checks, save the ImportModule object in session to take it in next steps
       ...
       if (upd==null || upd.length==0) discard[1] = "updateModules";
       if (inst==null || inst.length==0) discard[2] = "installModules";
       if ((upd==null || upd.length==0) && (inst==null || inst.length==0) && (module == null)) { 
  ...
  /**
   * Shows the second installation pup-up with all the license agreements for the modules to install/update
   * 
   * @param response
   * @param vars
   * @throws IOException
   * @throws ServletException
   */
  void printPageInstall2(HttpServletResponse response, VariablesSecureApp vars) throws IOException, ServletException {
    Module[] inst=null;
    Module[] selected;
    ...
    final Module[] upd=im.getModulesToUpdate();
     ...
     if (!islocal) {
       ...
       inst = new Module[installOrig.length==0?0:adModuleId.equals("")?installOrig.length:installOrig.length-1]; //to remove the module itself
       //check if the version for the selected module is the selected one
       int j =0;
       for (int i=0; i<installOrig.length; i++){
  ...
  void printPageInstall3(HttpServletResponse response, VariablesSecureApp vars) throws IOException, ServletException {
    ...
    OBError message;
    if (im.getIsLocal())
      im.execute(((FileItem)vars.getSessionObject("ModuleManagementInstall|File")).getInputStream(), ((FileItem)vars.getSessionObject("ModuleManagementInstall|File")).getInputStream());
    else
      im.execute();
    ...
    try {
     ...
     if (modules !=null && modules.length>0) {
      ...
      for (int i=0; i<modules.length; i++) {
        ...
        icon = (icon==null?"M":icon).equals("M")?"Module":icon.equals("T")?"Template":"Pack";
   ...
   * @throws ServletException
   */
  void printLicenseAgreement(HttpServletResponse response, VariablesSecureApp vars, String record) throws IOException, ServletException {
    ...
    while (!found && inst!=null && i<inst.length) {
      if (found=inst[i].getModuleID().equals(record)) agreement = inst[i].getLicenseAgreement();
      i++;
    }
    i=0;
    while (!found && upd!=null && i<upd.length) {
      if (found=upd[i].getModuleID().equals(record)) agreement = upd[i].getLicenseAgreement();
      i++;
    }
  ...
  /**
   * Displays the pop-up for the search locally file in order to look for an obx file and to
   * install it locally.
   * 
   * @param response
   * @param vars
