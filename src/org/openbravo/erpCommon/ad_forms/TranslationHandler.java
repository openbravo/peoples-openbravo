...
public class TranslationHandler extends DefaultHandler {
    ...
    public void startElement(String uri, String localName, String qName,
        ...
        if (qName.equals(Translation.XML_TAG)) {
            m_AD_Language = attributes
                    .getValue(Translation.XML_ATTRIBUTE_LANGUAGE);
            ...
            if (log4j.isDebugEnabled())
                log4j.debug("AD_Language=" + m_AD_Language + ", TableName="
                        + m_TableName);
            ...
            m_curColumnName = attributes
                    .getValue(Translation.XML_VALUE_ATTRIBUTE_COLUMN);
    ...
    public void endElement(String uri, String localName, String qName)
        ...
        if (qName.equals(Translation.XML_TAG)) {
            ...
            // Where section
            m_sql.append(" WHERE ").append(m_TableName).append("_ID='").append(
                    m_curID).append("'");
            m_sql.append(" AND AD_Language='").append(m_AD_Language)
                    .append("'");
            if (m_AD_Client_ID >= 0)
            ...
            if (m_sql.length() > 0)
                m_sql.append(",");
            m_sql.append(m_curColumnName).append("=").append(
                    TO_STRING(m_curValue.toString()));
        } else if (qName.equals(Translation.XML_CONTRIB)) {
            if (log4j.isDebugEnabled())
                log4j.debug("Contibutors:" + TO_STRING(m_curValue.toString()));
            try {
                TranslationData.insertContrib(DB, m_curValue.toString(),
                        m_AD_Language);
            } catch (Exception e) {
