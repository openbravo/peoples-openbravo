...
public class TranslationHandler extends DefaultHandler
{
  ...
  if (log4j.isDebugEnabled()) log4j.debug("endelement "+qName);
    ...
    //  
      ...
      } catch (Exception e){log4j.error("183:"+m_sql.toString()+e.toString());}
      finally {
        try {
          if (st != null) DB.releaseStatement(st);
      ...
      if (log4j.isDebugEnabled()) log4j.debug("Contibutors:"+TO_STRING(m_curValue.toString()));
      try {
