...
public class InvoiceVendorMultiline_Lines extends HttpSecureAppServlet {
  ...
  void printPageCallOut(HttpServletResponse response, VariablesSecureApp vars) throws IOException, ServletException {
    ...
    InvoiceVendorMultilineData[] data = InvoiceVendorMultilineData.select(this, strDateFormat, vars.getLanguage(), strcInvoiceId);
    if (data!=null && data.length>0) {
      out.print(Tax.get(this, strMProductID, data[0].dateinvoiced, data[0].adOrgId, strWarehouse, data[0].cBpartnerLocationId, data[0].cBpartnerLocationId, data[0].cProjectId, data[0].issotrx.equals("Y")));
    } else {
      out.print("");
    }
    out.close();
  ...
  void printPageDataSheet(HttpServletResponse response, VariablesSecureApp vars, String strInvoice)
    ...
    InvoiceVendorMultilineLinesData[] data = InvoiceVendorMultilineLinesData.select(this, vars.getLanguage(), strInvoice);
    if (!strInvoice.equals("") && (data==null || data.length==0)) data = InvoiceVendorMultilineLinesData.set(InvoiceVendorMultilineLinesData.selectNextLine(this, strInvoice));

    XmlDocument xmlDocument = xmlEngine.readXmlTemplate("org/openbravo/erpCommon/ad_forms/InvoiceVendorMultiline_Lines").createXmlDocument();
    ...
    String strTax = "";
    if (data!=null && data.length>0) strTax = data[0].cTaxId;

    try {
      ComboTableData comboTableData = new ComboTableData(vars, this, "TABLE", "C_Tax_ID", "C_Tax", "", Utility.getContext(this, vars, "#User_Org", windowId), Utility.getContext(this, vars, "#User_Client", windowId), 0);
  ...
  InvoiceVendorMultilineLinesData getEditVariables(VariablesSecureApp vars) throws IOException,ServletException {
    ...
    data.qtyinvoiced = vars.getStringParameter("inpqtyinvoiced");
    data.pricelist = vars.getStringParameter("inppricelist", "1");
    data.priceactual = vars.getStringParameter("inppriceactual", "1");
    data.pricelimit = vars.getStringParameter("inppricelimit", "1");
    data.pricestd = vars.getStringParameter("inppricestd", "1");
    data.cChargeId = vars.getStringParameter("inpcChargeId");
    data.chargeamt = vars.getStringParameter("inpchargeamt", "0");
    ...
    data.quantityorder = vars.getStringParameter("inpquantityorder");
    ...
    if (data.cUomId.equals("") && !data.mProductId.equals("")) data.cUomId = InvoiceVendorMultilineLinesData.selectUOM(this, data.mProductId);

    if (log4j.isDebugEnabled()) log4j.debug("C_InvoiceLine_ID: " + data.cInvoicelineId);
    if (log4j.isDebugEnabled()) log4j.debug("C_Invoice_ID: " + data.cInvoiceId);
    if (log4j.isDebugEnabled()) log4j.debug("AD_Client_ID: " + data.adClientId);
    if (log4j.isDebugEnabled()) log4j.debug("AD_Org_ID: " + data.adOrgId);
    if (log4j.isDebugEnabled()) log4j.debug("IsActive: " + data.isactive);
    if (log4j.isDebugEnabled()) log4j.debug("C_OrderLine_ID: " + data.cOrderlineId);
    if (log4j.isDebugEnabled()) log4j.debug("M_InoutLine_ID: " + data.mInoutlineId);
    if (log4j.isDebugEnabled()) log4j.debug("Line: " + data.line);
    if (log4j.isDebugEnabled()) log4j.debug("Description: " + data.description);
    if (log4j.isDebugEnabled()) log4j.debug("M_Product_ID: " + data.mProductId);
    if (log4j.isDebugEnabled()) log4j.debug("QtyInvoiced: " + data.qtyinvoiced);
    if (log4j.isDebugEnabled()) log4j.debug("PriceList: " + data.pricelist);
    if (log4j.isDebugEnabled()) log4j.debug("PriceActual: " + data.priceactual);
    if (log4j.isDebugEnabled()) log4j.debug("PriceLimit: " + data.pricelimit);
    if (log4j.isDebugEnabled()) log4j.debug("C_Charge_ID: " + data.cChargeId);
    if (log4j.isDebugEnabled()) log4j.debug("ChargeAmt: " + data.chargeamt);
    ...
    if (log4j.isDebugEnabled()) log4j.debug("QuantityOrder: " + data.quantityorder);
    if (log4j.isDebugEnabled()) log4j.debug("M_Product_UOM_ID: " + data.mProductUomId);
    if (log4j.isDebugEnabled()) log4j.debug("C_Invoice_Discount_ID: " + data.cInvoiceDiscountId);

    return data;
  }

  void printPageHidden(HttpServletResponse response, VariablesSecureApp vars) throws IOException, ServletException {
    if (log4j.isDebugEnabled()) log4j.debug("Output: hidden");
    ...
    try {
      if (vars.commandIn("DELETE")) {
        if (data.cInvoicelineId.equals("")) strMensaje = Utility.messageBD(this, "ProcessError", vars.getLanguage());
        else {
          Connection conn = getTransactionConnection();
          try {
            if (data.delete(conn, this)==0) strMensaje = Utility.messageBD(this, "ProcessError", vars.getLanguage());
        ...
        if (data.cInvoicelineId.equals("")) {
          ...
          if (data.update(this)==0) strMensaje = Utility.messageBD(this, "ProcessError", vars.getLanguage());
    ...
    XmlDocument xmlDocument = xmlEngine.readXmlTemplate("org/openbravo/erpCommon/utility/FrameOcultoMultilinea").createXmlDocument();
    ...
    if (!vars.commandIn("HIDDEN")) {
      if (!strMensaje.equals("")) {
        ...
        datos.append("new Array(\"MENSAJE_ERROR\", \"").append(Utility.messageBD(this, strMensaje, vars.getLanguage())).append("\"),\n");
      } else {
        ...
        datos.append("new Array(\"inpcInvoicelineIdNew\", \"").append(data.cInvoicelineId.equals("")?strIDNew:data.cInvoicelineId).append("\"),\n");
      }
    ...
    if (strMensaje.equals("") && vars.commandIn("SAVE_NEW", "DELETE")) {
      datos.append("new Array(\"LineNo\", \"").append(InvoiceVendorMultilineLinesData.selectNextLine(this, data.cInvoiceId)).append("\")\n");
      FieldProvider [] fp = null;
      try {
        ComboTableData comboTableData = new ComboTableData(vars, this, "TABLE", "", "C_Tax", "", Utility.getContext(this, vars, "#User_Org", windowId), Utility.getContext(this, vars, "#User_Client", windowId), 0);
      ...
      if (fp!=null && fp.length>0) {
        ...
        for (int i=0;i<fp.length;i++) {
          datos.append("new Array(\"").append(fp[i].getField("id")).append("\", \"").append(fp[i].getField("name")).append("\")\n");
          if (i<fp.length-1) datos.append(", ");
        }
        datos.append(")\n");
        datos.append(")\n");
      }
