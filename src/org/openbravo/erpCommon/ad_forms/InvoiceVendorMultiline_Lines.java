...
public class InvoiceVendorMultiline_Lines extends HttpSecureAppServlet {
    ...
    public void doPost(HttpServletRequest request, HttpServletResponse response)
...

        {
            ...
            if (data == null || data.length == 0) {
                throw new ServletException(formClassName
    ...
    void printPageCallOut(HttpServletResponse response, VariablesSecureApp vars)
        ...
        String strWarehouse = Utility.getContext(this, vars, "#M_Warehouse_ID",
                windowId);
        String strDateFormat = vars.getSessionValue("#AD_SqlDateFormat");
        InvoiceVendorMultilineData[] data = InvoiceVendorMultilineData.select(
                this, strDateFormat, vars.getLanguage(), strcInvoiceId);
        if (data != null && data.length > 0) {
            out.print(Tax.get(this, strMProductID, data[0].dateinvoiced,
                    data[0].adOrgId, strWarehouse, data[0].cBpartnerLocationId,
                    data[0].cBpartnerLocationId, data[0].cProjectId,
    ...
    void printPageDataSheet(HttpServletResponse response,
        ...
        InvoiceVendorMultilineLinesData[] data = InvoiceVendorMultilineLinesData
                .select(this, vars.getLanguage(), strInvoice);
        if (!strInvoice.equals("") && (data == null || data.length == 0))
            data = InvoiceVendorMultilineLinesData
                    .set(InvoiceVendorMultilineLinesData.selectNextLine(this,
                            strInvoice));

        XmlDocument xmlDocument = xmlEngine
                .readXmlTemplate(
                        "org/openbravo/erpCommon/ad_forms/InvoiceVendorMultiline_Lines")
                .createXmlDocument();
        ...
        try {
            ComboTableData comboTableData = new ComboTableData(vars, this,
                    "TABLE", "C_Tax_ID", "C_Tax", "", Utility.getContext(this,
                            vars, "#User_Org", windowId), Utility.getContext(
                            this, vars, "#User_Client", windowId), 0);
            Utility.fillSQLParameters(this, vars, null, comboTableData,
                    windowId, strTax);
            xmlDocument.setData("reportC_Tax_ID", "liststructure",
    ...
    InvoiceVendorMultilineLinesData getEditVariables(VariablesSecureApp vars)
        ...
        data.cInvoiceId = vars.getRequestGlobalVariable("inpcInvoiceId",
                windowId + "|C_Invoice_ID");
        data.adClientId = vars.getGlobalVariable("inpadClientId", windowId
                + "|AD_Client_ID", vars.getClient());
        data.adOrgId = vars.getGlobalVariable("inpadOrgId", windowId
                + "|AD_Org_ID", vars.getOrg());
        ...
        data.mProductId = vars.getStringParameter("inpmProductId"
                + data.cInvoicelineId);
        ...
        data.sResourceassignmentId = vars
                .getStringParameter("inpsResourceassignmentId");
        data.taxamt = vars.getStringParameter("inptaxamt", "0");
        data.mAttributesetinstanceId = vars
        ...
        data.cInvoiceDiscountId = vars
                .getStringParameter("inpcInvoiceDiscountId");

        data.createdby = vars.getUser();
        data.updatedby = vars.getUser();

        if (data.cUomId.equals("") && !data.mProductId.equals(""))
            data.cUomId = InvoiceVendorMultilineLinesData.selectUOM(this,
                    data.mProductId);
        ...
        if (log4j.isDebugEnabled())
            log4j.debug("S_ResourceAssignment_ID: "
                    + data.sResourceassignmentId);
        if (log4j.isDebugEnabled())
            log4j.debug("TaxAmt: " + data.taxamt);
        if (log4j.isDebugEnabled())
            log4j.debug("M_AttributeSetInstance_ID: "
                    + data.mAttributesetinstanceId);
        if (log4j.isDebugEnabled())
    ...
    void printPageHidden(HttpServletResponse response, VariablesSecureApp vars)
        ...
        try {
            if (vars.commandIn("DELETE")) {
                if (data.cInvoicelineId.equals(""))
                    strMensaje = Utility.messageBD(this, "ProcessError", vars
                            .getLanguage());
                else {
                    Connection conn = getTransactionConnection();
                    try {
                        if (data.delete(conn, this) == 0)
                            strMensaje = Utility.messageBD(this,
                                    "ProcessError", vars.getLanguage());
                        ...
                        ex1.printStackTrace();
                        log4j
                                .error("Failed delete on expense Invoice lines - ROLLBACK");
                        strMensaje = Utility.messageBD(this, "ProcessError",
                                vars.getLanguage());
                    }
                }
                ...
                if (data.cInvoicelineId.equals("")) {
                    ...
                    if (data.insert(this) == 0)
                        strMensaje = Utility.messageBD(this, "ProcessError",
                                vars.getLanguage());
                    strLineNo = InvoiceVendorMultilineLinesData.selectLineNo(
                            this, strIDNew);
                    data.cInvoicelineId = "";
                } else {
        ...
        XmlDocument xmlDocument = xmlEngine.readXmlTemplate(
                "org/openbravo/erpCommon/utility/FrameOcultoMultilinea")
                .createXmlDocument();
        ...
        if (!vars.commandIn("HIDDEN")) {
            if (!strMensaje.equals("")) {
                ...
                datos.append("new Array(\"inpcInvoicelineId\", \"").append(
                        data.cInvoicelineId).append("\"),\n");
                datos.append("new Array(\"MENSAJE_ERROR\", \"")
                        .append(
                                Utility.messageBD(this, strMensaje, vars
                                        .getLanguage())).append("\"),\n");
            } else {
                datos.append("new Array(\"Command\", \"").append(
                        vars.getCommand()).append("\"),\n");
                datos.append("new Array(\"inpcInvoicelineId\", \"").append(
                        data.cInvoicelineId).append("\"),\n");
                datos.append("new Array(\"inpmProductId\", \"").append(
                        data.mProductId).append("\"),\n");
                datos.append("new Array(\"inplineno\", \"").append(strLineNo)
                        .append("\"),\n");
                datos.append("new Array(\"inppriceactual\", \"").append(
                        data.priceactual).append("\"),\n");
                datos.append("new Array(\"inpqtyinvoiced\", \"").append(
                        data.qtyinvoiced).append("\"),\n");
                datos.append("new Array(\"inpcTaxId\", \"").append(data.cTaxId)
                        .append("\"),\n");
                datos.append("new Array(\"inpcInvoicelineIdNew\", \"").append(
                        data.cInvoicelineId.equals("") ? strIDNew
                                : data.cInvoicelineId).append("\"),\n");
            }
        ...
        if (strMensaje.equals("") && vars.commandIn("SAVE_NEW", "DELETE")) {
            datos.append("new Array(\"LineNo\", \"").append(
                    InvoiceVendorMultilineLinesData.selectNextLine(this,
                            data.cInvoiceId)).append("\")\n");
            FieldProvider[] fp = null;
            try {
                ComboTableData comboTableData = new ComboTableData(vars, this,
                        "TABLE", "", "C_Tax", "", Utility.getContext(this,
            ...
            if (fp != null && fp.length > 0) {
                ...
                for (int i = 0; i < fp.length; i++) {
                    datos.append("new Array(\"").append(fp[i].getField("id"))
                            .append("\", \"").append(fp[i].getField("name"))
                            .append("\")\n");
                    if (i < fp.length - 1)
                        datos.append(", ");
                }
        ...
        xmlDocument.setParameter("targetFrame", vars
                .getStringParameter("target"));
