...
public class Role extends HttpSecureAppServlet {
  ...
  void changePassword(VariablesSecureApp vars, String strClaveOld, String strClaveNew) throws IOException, ServletException {
    if (log4j.isDebugEnabled()) log4j.debug("Output: Login change process change");
    if (log4j.isDebugEnabled()) log4j.debug("Output: Old password: "+strClaveOld+", "+FormatUtilities.sha1Base64(strClaveOld)+" - user:"+vars.getUser());
    if (log4j.isDebugEnabled()) log4j.debug("pwd: "+RoleData.getPassword(this, vars.getUser())+" - encr: "+FormatUtilities.sha1Base64(strClaveOld));
  ...
  private String getDescriptionFromArray(ClientData[] data, String clave) {
    ...
    for (int i=0;i<data.length;i++) {
      if (data[i].id.equalsIgnoreCase(clave)) return data[i].name;
    }
    return "";
  }
}
