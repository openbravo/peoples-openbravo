...
public class Role extends HttpSecureAppServlet {
    ...
    public void doPost(HttpServletRequest request, HttpServletResponse response)
        ...
        if (vars.commandIn("DEFAULT")) {
            ...
            if (myMessage == null) {
                myMessage = Utility.translateError(this, vars, vars
                        .getLanguage(), "@CODE=PasswordChanged");
                myMessage.setTitle(Utility.messageBD(this, "Success", vars
                        .getLanguage()));
                vars.setMessage("Role", myMessage);
            }
            bdErrorAjax(response, myMessage.getType(), myMessage.getTitle(),
            ...
            if (saveDefaultOptions(vars, strSetDefault, request)) {
                ...
                myMessage.setTitle(Utility.messageBD(this, "Error", vars
                        .getLanguage()));
                myMessage.setMessage(Utility.messageBD(this, "LoginError", vars
                        .getLanguage()));
                vars.setMessage("Role", myMessage);
                response.sendRedirect(strDireccion + request.getServletPath());
            }
    ...
    void changePassword(VariablesSecureApp vars, String strClaveOld,
            String strClaveNew) throws IOException, ServletException {
        if (log4j.isDebugEnabled())
            log4j.debug("Output: Login change process change");
        if (log4j.isDebugEnabled())
            log4j.debug("Output: Old password: " + strClaveOld + ", "
                    + FormatUtilities.sha1Base64(strClaveOld) + " - user:"
                    + vars.getUser());
        if (log4j.isDebugEnabled())
            log4j.debug("pwd: " + RoleData.getPassword(this, vars.getUser())
                    + " - encr: " + FormatUtilities.sha1Base64(strClaveOld));

        if (!RoleData.getPassword(this, vars.getUser()).equals(
                FormatUtilities.sha1Base64(strClaveOld))) {
            log4j.error("Invalid password");
            throw new ServletException("@CODE=PasswordIncorrect");
        }
        if (RoleData.update(this, FormatUtilities.sha1Base64(strClaveNew), vars
                .getUser()) == 0)
            throw new ServletException("@CODE=ProcessError");
    }

    private boolean saveDefaultOptions(VariablesSecureApp vars,
            String strSetDefault, HttpServletRequest req)
        ...
        if (strSetDefault.equals("Y"))
            DefaultOptionsData.saveDefaultOptions(this, strLanguage, strRol,
                    strClient, strOrg, strWarehouse, strUserAuth);
        ...
        String sessionUser = (String) req.getSession(true).getAttribute(
                "#Authenticated_user");
        ...
        boolean result = LoginUtils.fillSessionArguments(this, vars,
                strUserAuth, strLanguage, strIsRTL, strRol, strClient, strOrg,
                strWarehouse);
        if (!result)
            return false;
    ...
    private void printPage(HttpServletResponse response,
        ...
        xmlDocument.setParameter("TextDirection",
                ((strIsRTL.equals("Y") ? "RTL" : "LTR")));
        xmlDocument.setParameter("roleInfo", Utility.messageBD(this,
                "RoleInfo", vars.getLanguage()));
        xmlDocument.setParameter("role", Utility.messageBD(this, "AD_Role_ID",
                vars.getLanguage()));
        xmlDocument.setParameter("client", Utility.messageBD(this,
                "AD_Client_ID", vars.getLanguage()));
        xmlDocument.setParameter("org", Utility.messageBD(this, "AD_Org_ID",
                vars.getLanguage()));
        xmlDocument.setParameter("user", RoleData.nombreUsuario(this, vars
                .getUser()));
        ...
        if (plaindataclient != null) {
            ...
            for (int i = 0; i < datarole.length; i++) {
                ...
                while (st.hasMoreTokens()) {
                    ...
                    auxClient.id = token;
                    auxClient.name = getDescriptionFromArray(plaindataclient,
                            token);
