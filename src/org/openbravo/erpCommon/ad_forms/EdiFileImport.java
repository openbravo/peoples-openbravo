...
public class EdiFileImport extends HttpSecureAppServlet {
  ...
  public void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
    ...
    if (vars.commandIn("DEFAULT")) {
      ...
      if (strMessage.equals("OK")) strMessage = Utility.messageBD(this, "Success", vars.getLanguage());
      else strMessage = Utility.messageBD(this, "ProcessRunError", vars.getLanguage());
      if (!strMessage.equals("")) vars.setSessionValue("EdiFileImport.message", strMessage);
      response.sendRedirect(strDireccion + request.getServletPath());
    } else pageError(response);
  }

  public String importarFichero(VariablesSecureApp vars, FieldProvider[] data2, HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
    Connection con = null;
      ...
      for (int i=0;i<data2.length;i++){
				...
				if (codigo.equals("UNH")) {
					...
					if (!formato.equals("102") && !formato.equals("203"))
						log4j.error("Dates must be supplied in YYYYMMDD format (102) or in YYYMMDDHHMM format (203). Found: " + formato);
  ...
  void printPage(HttpServletResponse response, VariablesSecureApp vars) throws IOException, ServletException {
    if (log4j.isDebugEnabled()) log4j.debug("Output: edi files importing Frame Set");
    ...
    try {
      ...
      NavigationBar nav = new NavigationBar(this, vars.getLanguage(), "EdiFileImport.html", classInfo.id, classInfo.type, strReplaceWith, tabs.breadcrumb());
      xmlDocument.setParameter("navigationBar", nav.toString());
      LeftTabsBar lBar = new LeftTabsBar(this, vars.getLanguage(), "EdiFileImport.html", strReplaceWith);
