...
public class EdiFileImport extends HttpSecureAppServlet {
    ...
    public void doPost(HttpServletRequest request, HttpServletResponse response)
        ...
        if (vars.commandIn("DEFAULT")) {
            ...
            String strMessage = importarFichero(vars, fieldsData
                    .getFieldProvider(), request, response);
            if (strMessage.equals("OK"))
                strMessage = Utility.messageBD(this, "Success", vars
                        .getLanguage());
            else
                strMessage = Utility.messageBD(this, "ProcessRunError", vars
                        .getLanguage());
    ...
    public String importarFichero(VariablesSecureApp vars,
        ...
        try {
            ...
            for (int i = 0; i < data2.length; i++) {
                ...
                if (codigo.equals("UNH")) {
                    if (!data2[i].getField("2").equals(
                            "ORDERS:D:96A:UN:EAN008'"))
                        log4j
                                .error("The file's format is not recognized. Found: "
                                        + data2[i].getField("2"));
                    ...
                    if (!tipo.equals("220"))
                        ...
                        log4j.error("Only EAN orders accepted. Found: "
                                + formato);
                    else
                    ...
                    if (!formato.equals("9") && !formato.equals("92"))
                        log4j
                                .error("NAD fields are only supported in EAN format. Found: "
                                        + formato);
                    ...
                    if (!formato.equals("EN"))
                        log4j
                                .error("Products are only supported in EAN format (EN). Found: "
                                        + formato);
                    else
                        data.upc = valor;
                    ...
                    if (!tipo.equals("F"))
                        log4j
                                .error("Description only allowed in free form (F). Found: "
                                        + tipo);
                    else
                    ...
                    if (!tipo.equals("PD"))
                        log4j
                                .error("Only physical dimensions (PD) allowed. Found: "
                                        + tipo);
                    else if (!dimension.equals("ULY"))
                        log4j
                                .error("Only units per layer (ULY) allowed. Found: "
                                        + dimension);
                    else if (!exact.equals("4"))
                        log4j.error("Only exact form (4) allowed. Found: "
                                + exact);
                    else if (!unidad.equals("PCE"))
                        log4j.error("Only units (PCE) allowed. Found: "
                                + unidad);
                    else
                        ;
    ...
    EdiFileImportData insert(ConnectionProvider conn, VariablesSecureApp vars,
    ...
    void printPage(HttpServletResponse response, VariablesSecureApp vars)
        ...
        XmlDocument xmlDocument = xmlEngine.readXmlTemplate(
                "org/openbravo/erpCommon/ad_forms/EdiFileImport")
                .createXmlDocument();

        ToolBar toolbar = new ToolBar(this, vars.getLanguage(),
                "EdiFileImport", false, "", "", "", false, "ad_forms",
        ...
        try {
            ...
            NavigationBar nav = new NavigationBar(this, vars.getLanguage(),
                    "EdiFileImport.html", classInfo.id, classInfo.type,
                    strReplaceWith, tabs.breadcrumb());
            xmlDocument.setParameter("navigationBar", nav.toString());
            LeftTabsBar lBar = new LeftTabsBar(this, vars.getLanguage(),
                    "EdiFileImport.html", strReplaceWith);
        ...
        }
        {
            ...
            /*
             * String strMessage =
             * vars.getSessionValue("EdiFileImport.message").
             * equals("")?"xx();":"alert('" +
             * vars.getSessionValue("EdiFileImport.message") + "');";
