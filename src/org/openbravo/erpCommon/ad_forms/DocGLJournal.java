...
public class DocGLJournal extends AcctServer {
    ...
    public BigDecimal getBalance() {
        ...
        sb.append("]");
        //
        log4jDocGLJournal.debug(toString() + " Balance=" + retValue
                + sb.toString());
        return retValue;
    } // getBalance

    /**
     * Create Facts (the accounting logic) for GLJ. (only for the accounting
     * scheme, it was created)
     * 
     * <pre>
     *      account     DR          CR
     * </pre>
    ...
    public Fact createFact(AcctSchema as, ConnectionProvider conn,
        ...
        if (DocumentType.equals(AcctServer.DOCTYPE_GLJournal)) {
            // account DR CR
            for (int i = 0; i < p_lines.length; i++) {
                if (p_lines[i].getC_AcctSchema_ID().equals(
                        as.getC_AcctSchema_ID())) {
                    fact.createLine(p_lines[i], p_lines[i].getAccount(),
                            C_Currency_ID, p_lines[i].getAmtSourceDr(),
                            p_lines[i].getAmtSourceCr(), Fact_Acct_Group_ID,
                            nextSeqNo(SeqNo), DocumentType, conn);
                }
            } // for all lines
        } else {
            log4jDocGLJournal.warn("createFact - " + "DocumentType unknown: "
                    + DocumentType);
            fact = null;
        }
        SeqNo = "0";
