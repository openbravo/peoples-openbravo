...
public class DocDPManagement extends AcctServer {
    ...
    public void loadObjectFieldProvider(ConnectionProvider conn,
        ...
        setObjectFieldProvider(DocDPManagementData.selectRegistro(conn,
                AD_Client_ID, Id));
    }
    ...
    private DocLine[] loadLines(ConnectionProvider conn) {
        ...
        for (int i = 0; data != null && i < data.length; i++) {
            ...
            DocLine_DPManagement docLine = new DocLine_DPManagement(
                    DocumentType, Record_ID, Line_ID);
        ...
        for (int i = 0; p_lines != null && i < p_lines.length; i++) {
            ...
            if (line.Isreceipt.equals("Y")) {
                fact.createLine(line, getAccount(line.Isreceipt,
                        line.m_C_BPartner_ID, as, line.StatusTo, conn),
                        line.m_C_Currency_ID, line.Amount, "",
                        Fact_Acct_Group_ID, nextSeqNo(SeqNo), DocumentType,
                        conn);
                fact.createLine(line, getAccount(line.Isreceipt,
                        line.m_C_BPartner_ID, as, line.StatusFrom, conn),
                        line.m_C_Currency_ID, "", line.Amount,
                        Fact_Acct_Group_ID, nextSeqNo(SeqNo), DocumentType,
                        conn);
     ...
     */
    public Account getAccount(String Isreceipt, String partnerID,
            AcctSchema as, String status, ConnectionProvider conn) {
        /*
        ...
        /*
         * if (A_Asset_ID.equals("")) return getAccountDefault(AcctType, as,
         * conn);
         */
        ...
        try {
            ...
            if (Isreceipt.equals("Y")) {
                data = DocDPManagementData.selectReceiptAcct(conn, partnerID,
                        as.getC_AcctSchema_ID(), status);
                validCombination_ID = data[0].acct;
            } else {
                data = DocDPManagementData.selectNoReceiptAcct(conn, partnerID,
                        as.getC_AcctSchema_ID(), status);
                validCombination_ID = data[0].acct;
            }
