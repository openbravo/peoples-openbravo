  ...
  public Account getAccount(String AcctType, AcctSchema as,ConnectionProvider conn){
    ...
    try{
      ...
      switch (Integer.parseInt(AcctType)){
        case 1: validCombination_ID = data[0].revenue;
            break;
        case 2: validCombination_ID = data[0].expense;
            break;
        case 3: validCombination_ID = data[0].asset;
            break;
        case 4: validCombination_ID = data[0].cogs;
            break;
        case 5: validCombination_ID = data[0].purchasepricevariance;
            break;
        case 6: validCombination_ID = data[0].invoicepricevariance;
            break;
        case 7: validCombination_ID = data[0].discountrec;
            break;
        case 8: validCombination_ID = data[0].discountgrant;
            break;
      }
 ...
 */
public static String getConvertedQty (String qty,String C_UOM_From_ID, String C_UOM_To_ID, String StdPrecision, ConnectionProvider conn){
  //  Nothing to do
...
public String getProductCosts (String date, String strQty, AcctSchema as, ConnectionProvider conn, Connection con){
  ...
  } else return cost.multiply(new BigDecimal(strQty)).toString();

}   //  getProductCosts

public String getProductItemCost(String date, AcctSchema as, String costType, ConnectionProvider conn, Connection con){
  String cost = "";
  log4jProductInfo.debug("getProductItemCost - m_M_Product_ID(" + m_M_Product_ID + ") - date(" + date + ")");
  try{
...
private String getPOCost (AcctSchema as, ConnectionProvider conn){
  ...
  if (!cost.equals("") && !cost.equals(ZERO.toString()))
    cost = AcctServer.getConvertedAmt(cost, C_Currency_ID, as.getC_Currency_ID(), m_AD_Client_ID, m_AD_Org_ID, conn);
  return cost;
}   //  getPOCost
...
private String getPriceList (AcctSchema as, boolean onlyPOPriceList, ConnectionProvider conn){
  ...
  ProductInfoData [] data = null;
  try{
    data = ProductInfoData.selectPriceList(conn, m_M_Product_ID, onlyPOPriceList?"onlyPOPriceList":"");
  }catch(ServletException e){
    log4jProductInfo.warn(e);
  }
  if (data.length==1){
  ...
  //  Convert
  if (!price.equals("") && !price.equals(ZERO.toString()))
    price = AcctServer.getConvertedAmt(price, C_Currency_ID, as.m_C_Currency_ID, as.m_AD_Client_ID, "", conn);
  return price;
}   //  getPOPrice
