...
public class MaterialReceiptPending extends HttpSecureAppServlet {
    ...
    public void doPost(HttpServletRequest request, HttpServletResponse response)
        ...
        if (vars.commandIn("DEFAULT")) {
            ...
            printPageDataSheet(response, vars, strC_BPartner_ID, strAD_Org_ID,
                    strDateFrom, strDateTo, strDocumentNo);
            ...
            String strDocumentNo = vars.getRequestGlobalVariable(
                    "inpDocumentNo", "MaterialReceiptPending|DocumentNo");
            String strC_BPartner_ID = vars.getRequestGlobalVariable(
                    "inpcBpartnerId", "MaterialReceiptPending|C_BPartner_ID");
            String strAD_Org_ID = vars.getGlobalVariable("inpadOrgId",
                    "MaterialReceiptPending|AD_Org_ID");
            printPageDataSheet(response, vars, strC_BPartner_ID, strAD_Org_ID,
                    strDateFrom, strDateTo, strDocumentNo);
        } else if (vars.commandIn("GENERATE")) {
            String strcOrderLineId = vars
                    .getRequiredInStringParameter("inpOrder");
    ...
    void printPageDataSheet(HttpServletResponse response,
        ...
        String strTreeOrg = MaterialReceiptPendingData.treeOrg(this, vars
                .getClient());
        if (strC_BPartner_ID.equals("") && strAD_Org_ID.equals("")) {
            ...
            data = MaterialReceiptPendingData.selectLines(this, strDateFormat,
                    vars.getLanguage(), Utility.getContext(this, vars,
                            "#User_Client", "MaterialReceiptPending"), Tree
                            .getMembers(this, strTreeOrg, strAD_Org_ID),
                    strDateFrom, DateTimeData.nDaysAfter(this, strDateTo, "1"),
                    strC_BPartner_ID, strDocumentNo);
        }

        ToolBar toolbar = new ToolBar(this, vars.getLanguage(),
                "MaterialReceiptPending", false, "", "", "", false, "ad_forms",
        ...
        try {
            ...
            NavigationBar nav = new NavigationBar(this, vars.getLanguage(),
                    "MaterialReceiptPending.html", classInfo.id,
                    classInfo.type, strReplaceWith, tabs.breadcrumb());
            xmlDocument.setParameter("navigationBar", nav.toString());
            LeftTabsBar lBar = new LeftTabsBar(this, vars.getLanguage(),
                    "MaterialReceiptPending.html", strReplaceWith);
        ...
        try {
            ComboTableData comboTableData = new ComboTableData(vars, this,
                    ...
                    Utility.getContext(this, vars, "#User_Org",
                            "MaterialReceiptPending"), Utility.getContext(this,
                            vars, "#User_Client", "MaterialReceiptPending"), 0);
            Utility.fillSQLParameters(this, vars, null, comboTableData,
                    "MaterialReceiptPending", strAD_Org_ID);
    ...
    OBError processPurchaseOrder(VariablesSecureApp vars, String strcOrderLineId)
        ...
        try {
            ...
            if (strcOrderLineId.startsWith("("))
                strcOrderLineId = strcOrderLineId.substring(1, strcOrderLineId
                        .length() - 1);
            if (!strcOrderLineId.equals("")) {
                strcOrderLineId = Replace.replace(strcOrderLineId, "'", "");
                StringTokenizer st = new StringTokenizer(strcOrderLineId, ",",
                        false);
                String strmInoutId = "";
                ...
                while (st.hasMoreTokens()) {
                    ...
                    MaterialReceiptPendingData[] data = MaterialReceiptPendingData
                            .select(this, strOrderlineId);
                    if (!strLastBpartnerId.equals(data[0].cBpartnerId)
                        ...
                        if (!strmInoutId.equals("")) {
                            ...
                            strMessageResult += strWindowName + " "
                                    + strDocumentno + ": "
                        ...
                        docTargetType = MaterialReceiptPendingData
                                .cDoctypeTarget(this, vars.getClient(),
                                        data[0].adOrgId);
                        docType = MaterialReceiptPendingData.cDoctypeId(this,
                                docTargetType);
                        strDocumentno = Utility.getDocumentNo(this, vars, "",
                                "M_InOut", docTargetType, docType, false, true);
                        strDateReceipt = vars
                                .getStringParameter("inpDateReceipt"
                                        + data[0].cBpartnerId);
                        ...
                        if (strDateReceipt.equals("")) {
                            ...
                            myMessage.setMessage(Utility.messageBD(this,
                                    "DateReceipt", vars.getLanguage())
                                    + " "
                                    + MaterialReceiptPendingData
                                            .bPartnerDescription(this,
                                                    data[0].cBpartnerId));
                            return myMessage;
                        }
                        try {
                            MaterialReceiptPendingData.insert(conn, this,
                                    strmInoutId, vars.getClient(),
                                    data[0].adOrgId, "Y", vars.getUser(), vars
                                            .getUser(), "N", strDocumentno,
                                    "CO", "DR", "N", "N", "N", docTargetType,
                                    data[0].description, data[0].cOrderId,
                                    data[0].dateordered, "N", "V+",
                                    strDateReceipt, strDateReceipt,
                                    data[0].cBpartnerId,
                                    data[0].cBpartnerLocationId,
                                    data[0].mWarehouseId, data[0].poreference,
                                    data[0].deliveryrule,
                                    data[0].freightcostrule,
                                    data[0].freightamt,
                                    data[0].deliveryviarule,
                                    data[0].mShipperId, data[0].cChargeId,
                                    data[0].chargeamt, data[0].priorityrule,
                                    "N", "N", data[0].adUserId,
                                    data[0].salesrepId, data[0].adOrgtrxId,
                                    data[0].cProjectId, data[0].cCampaignId,
                                    data[0].cActivityId, data[0].user1Id,
                                    data[0].user2Id, "N", "N", "N");
                    ...
                    String strQtyordered = vars
                            .getStringParameter("inpQtyordered"
                                    + strOrderlineId);
                    String strLocator = vars.getStringParameter("inpmLocatorId"
                            + strOrderlineId);
                    String strSequenceLine = SequenceIdData.getUUID();
                    MaterialReceiptPendingLinesData[] dataLine = MaterialReceiptPendingLinesData
                            .select(this, strOrderlineId);
                    try {
                        MaterialReceiptPendingLinesData.insert(conn, this,
                                strSequenceLine, vars.getClient(),
                                data[0].adOrgId, "Y", vars.getUser(), vars
                                        .getUser(), String.valueOf(line),
                                dataLine[0].description, strmInoutId,
                                strOrderlineId, strLocator,
    ...
    OBError mInoutPost(Connection conn, VariablesSecureApp vars,
        ...
        myMessage = new OBError();
        try {
            PInstanceProcessData.insertPInstance(this, pinstance, "109",
                    strmInoutId, "N", vars.getUser(), vars.getClient(), vars
                            .getOrg());
        } catch (ServletException ex) {
            myMessage = Utility.translateError(this, vars, vars.getLanguage(),
                    ex.getMessage());
        ...
        Pattern patern = Pattern
                .compile("^@ERROR=@Inline@\\s+[0-9]+\\s+@productWithoutAttributeSet@");
