...
import java.io.*;
  ...
  public class AccountingValueData extends MultipartRequest implements FieldProvider {
    ...
    public AccountingValueData(VariablesBase _vars, String _filename, boolean _firstLineHeads, String _format) throws IOException {
      super(_vars, _filename, _firstLineHeads, _format, null);
    }

    public AccountingValueData(VariablesBase _vars, InputStream _in, boolean _firstLineHeads, String _format) throws IOException {
    ...
    public String getField(String fieldName) {
      if (fieldName.equalsIgnoreCase("ACCOUNT_VALUE") || fieldName.equals("accountValue"))
        ...
        return accountName;
      else if (fieldName.equalsIgnoreCase("ACCOUNT_DESCRIPTION") || fieldName.equals("accountDescription"))
      ...
      else if (fieldName.equalsIgnoreCase("US_1120_BALANCE_SHEET") || fieldName.equals("uS1120BalanceSheet"))
        return uS1120BalanceSheet;
      else if (fieldName.equalsIgnoreCase("US_1120_BALANCE_SHEET_NAME") || fieldName.equals("uS1120BalanceSheetName"))
        return uS1120BalanceSheetName;
      else if (fieldName.equalsIgnoreCase("PROFIT_AND_LOSS") || fieldName.equals("profitAndLoss"))
        return profitAndLoss;
      else if (fieldName.equalsIgnoreCase("PROFIT_AND_LOSS_NAME") || fieldName.equals("profitAndLossName"))
        return profitAndLossName;
      else if (fieldName.equalsIgnoreCase("US_1120_INCOME_STATEMENT") || fieldName.equals("uS1120IncomeStatement"))
        return uS1120IncomeStatement;
      else if (fieldName.equalsIgnoreCase("US_1120_INCOME_STATEMENT_NAME") || fieldName.equals("uS1120IncomeStatementName"))
        ...
        return cashFlowName;
      else if (fieldName.equalsIgnoreCase("C_ELEMENT_VALUE_ID") || fieldName.equalsIgnoreCase("CELEMENTVALUEID"))
        return cElementValueId;
      else {
        if (log4j.isDebugEnabled()) log4j.debug("AccountingValueData - getField - Field does not exist: " + fieldName);
        return null;
      }
    }
    ...
    public FieldProvider lineSeparatorFormated(String linea) {
      ...
      for (int i=0;i<21;i++) {
        ...
        if ((anterior+1)<linea.length() && linea.substring(anterior, anterior+1).equals("\"")) {
          ...
          if (aux!=-1) siguiente = aux;
        }
        siguiente=linea.indexOf(",",siguiente+1);
        if (siguiente==-1) siguiente=linea.length();
        texto = linea.substring(anterior,siguiente);
        //if (anterior==siguiente || anterior==(siguiente-1)) texto=""; Comentado por que no sabemos para que sirve
        if (texto.length()>0) {
          if (texto.charAt(0)=='"') texto = texto.substring(1);
          if (texto.charAt(texto.length()-1)=='"') texto=texto.substring(0,texto.length()-1);
        }
        if (log4j.isDebugEnabled()) log4j.debug("AccountingValueData - lineSeparatorFormated - i: " + i);
        if (log4j.isDebugEnabled()) log4j.debug("AccountingValueData - lineSeparatorFormated - texto: " + texto);
        switch (i) {
          case 0: AccountingValueData.accountValue = texto;
                        break;
          case 1: AccountingValueData.accountName = texto;
                        break;
          case 2: AccountingValueData.accountDescription = texto;
                        break;
          case 3: AccountingValueData.accountType = texto;
                        break;
          case 4: AccountingValueData.accountSign = texto;
                        break;
          case 5: AccountingValueData.accountDocument = texto;
                        break;
          case 6: AccountingValueData.accountSummary = texto;
                        break;
          case 7: AccountingValueData.defaultAccount = texto;
                        break;
          case 8: AccountingValueData.accountParent = texto;
                        break;
