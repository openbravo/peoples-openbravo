...
public class DocBank extends AcctServer {
     ...
     *            AD_Client_ID
     */
    public DocBank(String AD_Client_ID, String AD_Org_ID,
    ...
    public void loadObjectFieldProvider(ConnectionProvider conn,
        ...
        setObjectFieldProvider(DocBankData.selectRegistro(conn, AD_Client_ID,
                Id));
    }
    ...
    public boolean loadDocumentDetails(FieldProvider[] data,
        ...
        if (!strCount.equals("0"))
            ...
            log4jDocBank.debug("Record_ID = " + Record_ID + " - Lines="
                    + p_lines.length);
        }
        return true;
    } // loadDocumentDetails
    ...
    private DocLine[] loadLines(ConnectionProvider conn) {
        ...
        for (int i = 0; data != null && i < data.length; i++) {
            ...
            DocLine_Bank docLine = new DocLine_Bank(DocumentType, Record_ID,
                    Line_ID);
    ...
    public BigDecimal getBalance() {
        ...
        for (int i = 0; i < p_lines.length; i++) {
            BigDecimal lineBalance = new BigDecimal(
                    ((DocLine_Bank) p_lines[i]).m_StmtAmt);
    ...
    public Fact createFact(AcctSchema as, ConnectionProvider conn,
        ...
        for (int i = 0; p_lines != null && i < p_lines.length; i++) {
            ...
            // BankInTransit DR CR (Payment)
            /*
             * if (!line.m_C_Payment_ID.equals("") &&
             * !line.isManual.equals("Y"))
             * fact.createLine(line,getAccountBPartner(line.m_C_BPartner_ID,
             * as,conn, (TrxAmt.negate().compareTo(new
             * BigDecimal("0"))>0?false:true)),line.m_C_Currency_ID,
             * TrxAmt.negate().toString(), conn);
             */
            if (line.m_C_Payment_ID.equals("")
                    && !line.m_C_GLItem_ID.equals(""))
                fact.createLine(line, line.getGlitemAccount(as, new BigDecimal(
                        getAmount()), conn), line.m_C_Currency_ID, TrxAmt
                        .signum() == -1 ? TrxAmt.negate().toString() : "",
                        TrxAmt.signum() == -1 ? "" : TrxAmt.toString(),
                        Fact_Acct_Group_ID, nextSeqNo(SeqNo), DocumentType,
                        conn);
            else
                fact.createLine(line, getAccountBankInTransit(
                        line.m_TrxLine_ID, as, conn), line.m_C_Currency_ID,
                        TrxAmt.negate().toString(), Fact_Acct_Group_ID,
                        nextSeqNo(SeqNo), DocumentType, conn);
            // Charge DR (Charge)
            fact.createLine(new DocLine_Bank(DocumentType, Record_ID,
                    line.m_TrxLine_ID), new Account(conn, DocLineBankData
                    .selectChargeAccount(conn, C_BankAccount_ID,
                            as.m_C_AcctSchema_ID)), line.m_C_Currency_ID,
                    ChargeAmt.toString(), "", Fact_Acct_Group_ID,
                    nextSeqNo(SeqNo), DocumentType, conn);
            // Interest DR CR (Interest)
            /*
             * if (InterestAmt.signum() < 0)
             * fact.createLine(line,getAccount(AcctServer.ACCTTYPE_InterestExp,
             * as, conn), getAccount(AcctServer.ACCTTYPE_InterestExp, as,
             * conn),line.m_C_Currency_ID, InterestAmt.negate().toString(),
             * conn); else
             * fact.createLine(line,getAccount(AcctServer.ACCTTYPE_InterestExp,
             * as, conn), getAccount(AcctServer.ACCTTYPE_InterestRev, as,
             * conn),line.m_C_Currency_ID, InterestAmt.negate().toString(),
             * conn);
             */
            //
            if (ConvertChargeAmt.signum() > 0) // >0 loss
                ...
                fact.createLine(new DocLine_Bank(DocumentType, Record_ID,
                        ...
                        line.m_C_Currency_ID, "", ConvertChargeAmt.negate()
     ...
     * @return Account
     */
    public final Account getAccountBankInTransit(
            String strcBankstatementlineId, AcctSchema as,
            ConnectionProvider conn) {
        DocBankData[] data = null;
        try {
            data = DocBankData.selectBankInTransitAcct(conn,
        ...
        if (Account_ID.equals("")) {
            log4j
                    .warn("DocBank - getAccountBankStatementLine - NO account BankStatementLine="
    ...
    public boolean getDocumentConfirmation(ConnectionProvider conn,
        ...
        String strCount = "";
        try {
            strCount = DocLineBankData.selectLinesPeriodClosed(conn,
                    strRecordId);
        } catch (ServletException e) {
            e.printStackTrace();
            return false;
