...
public class DocFINPayment extends AcctServer {
  ...
  private DocLine[] loadLines() {
    ...
    for (int i = 0; i < data.length; i++) {
      ...
      docLine.setInvoiceAccountingDate(data[i].getField("invoiceAccountingDate"));
      list.add(docLine);
    }
    // Return Array
        ...
        if ("".equals(line.getC_GLItem_ID())) {
          fact
              .createLine(line,
                  ...
                  line.getInvoiceAccountingDate(), conn);
        } else {
  ...
  public String convertAmount(String amount, boolean isReceipt, String dateAcct,
      String conversionDate, String fromCurrencyId, String toCurrencyID, DocLine line,
      AcctSchema as, Fact fact, String factAcctGroupId, ConnectionProvider conn)
      throws ServletException {
    if (log4j.isDebugEnabled()) {
      log4j.debug("Amount:" + amount + " curr from:" + fromCurrencyId + " Curr to:"
          + toCurrencyID + " convDate:" + conversionDate + " DateAcct:" + dateAcct);
    }
    ...
    if (fromCurrencyId.equals(toCurrencyID))
      ...
      MultiCurrency = true;
    String Amt = getConvertedAmt(amount, fromCurrencyId, toCurrencyID, conversionDate, "",
        AD_Client_ID, AD_Org_ID, conn);
    if (log4j.isDebugEnabled())
      log4j.debug("Amt:" + Amt);

    String AmtTo = getConvertedAmt(amount, fromCurrencyId, toCurrencyID, dateAcct, "",
        AD_Client_ID, AD_Org_ID, conn);
    if (log4j.isDebugEnabled())
    ...
    if (log4j.isDebugEnabled()) {
      log4j.debug("curr from:" + fromCurrencyId + " Curr to:" + toCurrencyID + " convDate:"
    ...
    if ((isReceipt && AmtDiff.compareTo(new BigDecimal("0.00")) == 1)
      ...
      fact.createLine(line, getAccount(AcctServer.ACCTTYPE_ConvertGainDefaultAmt, as, conn),
          toCurrencyID, "", AmtDiff.abs().toString(), factAcctGroupId, nextSeqNo(SeqNo),
          DocumentType, conn);
    } else {
