...
public class InvoiceVendorMultiline extends HttpSecureAppServlet {
  ...
  public void doPost (HttpServletRequest request, HttpServletResponse response) throws IOException,ServletException {
    ...
    if (vars.commandIn("DEFAULT", "EDIT")) {
      ...
      if (strC_Invoice_ID.equals("")) response.sendRedirect(strDireccion + "/" + FormatUtilities.replace(windowNameEnUS) + "/" + FormatUtilities.replace(tabNameEnUS) + "_Relation.html?Command=RELATION");
      else printPageDataSheet(response, vars, strC_Invoice_ID);
      ...
      if (data.insert(this)==0) {
        ...
        else response.sendRedirect(strDireccion + "/" + FormatUtilities.replace(windowNameEnUS) + "/" + FormatUtilities.replace(tabNameEnUS) + "_Relation.html?Command=RELATION");
      }
      ...
      vars.removeSessionValue(windowId + "|C_Invoice_ID");
      response.sendRedirect(strDireccion + "/" + FormatUtilities.replace(windowNameEnUS) + "/" + FormatUtilities.replace(tabNameEnUS) + "_Relation.html?Command=RELATION");
  ...
  InvoiceVendorMultilineData getEditVariables(VariablesSecureApp vars) throws IOException,ServletException {
    ...
    data.cBpartnerId = vars.getRequiredGlobalVariable("inpcBpartnerId", windowId + "|C_BPartner_ID");
    ...
    if (data.cDoctypetargetId.equals("")) data.cDoctypetargetId = InvoiceVendorMultilineData.selectDocTypeTarget(this, Utility.getContext(this, vars, "#User_Org", windowId), Utility.getContext(this, vars, "#User_Client", windowId), Utility.getContext(this, vars, "#AD_Client_ID", windowId), data.issotrx);

    if (data.cDoctypeId.equals("")) data.cDoctypeId = InvoiceVendorMultilineData.selectDocType(this, Utility.getContext(this, vars, "#User_Org", windowId), Utility.getContext(this, vars, "#User_Client", windowId));

    if (data.mPricelistId.equals("")) data.mPricelistId = InvoiceVendorMultilineData.selectPriceList(this, Utility.getContext(this, vars, "#User_Org", windowId), Utility.getContext(this, vars, "#User_Client", windowId));

    if (data.cCurrencyId.equals("")) data.cCurrencyId = InvoiceVendorMultilineData.selectCurrency(this, Utility.getContext(this, vars, "#User_Org", windowId), Utility.getContext(this, vars, "#User_Client", windowId));

    if (data.documentno.startsWith("<")) data.documentno = Utility.getDocumentNo(this, vars, windowId, "C_Invoice", data.cDoctypetargetId, data.cDoctypeId, false, true);

    return data;
  }

  void printPageDataSheet(HttpServletResponse response, VariablesSecureApp vars, String strC_Invoice_ID) throws IOException, ServletException {
    if (log4j.isDebugEnabled()) log4j.debug("Output: dataSheet");
    String[] discard = {"newDiscard",""};
    String strCommand = "EDIT";
    String strDateFormat = vars.getSessionValue("#AD_SqlDateFormat");
    InvoiceVendorMultilineData[] data = InvoiceVendorMultilineData.select(this, strDateFormat, vars.getLanguage(), strC_Invoice_ID);
    if (data==null || data.length==0) {
      discard[0] = new String("editDiscard");
      strCommand = "NEW";
      data = InvoiceVendorMultilineData.set(vars.getOrg(), vars.getClient(), DateTimeData.today(this), "DR", "CO", InvoiceVendorMultilineData.selectDocAction(this, vars.getLanguage(), "CO"), "0", "0");
    }
    ...
    try {
      ...
      NavigationBar nav = new NavigationBar(this, vars.getLanguage(), "../ExpenseInvoice/ExpenseInvoice_Relation.html", classInfo.id, classInfo.type, strReplaceWith, tabs.breadcrumb());
      xmlDocument.setParameter("navigationBar", nav.toString());
      LeftTabsBar lBar = new LeftTabsBar(this, vars.getLanguage(), "../ExpenseInvoice/ExpenseInvoice_Relation.html", strReplaceWith);
    ...
    try {
      ComboTableData comboTableData = new ComboTableData(vars, this, "LIST", "", "All_Payment Rule", "", Utility.getContext(this, vars, "#User_Org", windowId), Utility.getContext(this, vars, "#User_Client", windowId), 0);
    ...
    try {
      ComboTableData comboTableData = new ComboTableData(vars, this, "TABLEDIR", "AD_Org_ID", "", "AD_Org Trx Security validation", Utility.getContext(this, vars, "#User_Org", windowId), Utility.getContext(this, vars, "#User_Client", windowId), 0);
    ...
    try {
      ...
      try {
        ComboTableData comboTableData = new ComboTableData(vars, this, "TABLEDIR", "C_PaymentTerm_ID", "", "", Utility.getContext(this, vars, "#User_Org", windowId), Utility.getContext(this, vars, "#User_Client", windowId), 0);
      ...
      try {
        ComboTableData comboTableData = new ComboTableData(vars, this, "TABLEDIR", "C_BPartner_Location_ID", "", "C_BPartner Location - Bill To", Utility.getContext(this, vars, "#User_Org", windowId), Utility.getContext(this, vars, "#User_Client", windowId), 0);
        Utility.fillSQLParameters(this, vars, null, comboTableData, windowId, data[0].cBpartnerId);
        xmlDocument.setData("reportC_BPartner_Location_ID","liststructure", comboTableData.select(false));
        comboTableData = null;
      } catch (Exception ex) {
        throw new ServletException(ex);
      }
