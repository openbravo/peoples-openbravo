...
public class InvoiceVendorMultiline extends HttpSecureAppServlet {
    ...
    public void doPost(HttpServletRequest request, HttpServletResponse response)
        ...
        if (vars.commandIn("DEFAULT", "EDIT")) {
            ...
            if (data.insert(this) == 0) {
                ...
                if (vars.commandIn("SAVE_NEW_NEW"))
                    ...
                    response.sendRedirect(strDireccion
                            + request.getServletPath() + "?Command=EDIT");
                else
                    response.sendRedirect(strDireccion + "/"
                            + FormatUtilities.replace(windowNameEnUS) + "/"
                            + FormatUtilities.replace(tabNameEnUS)
        ...
        } else if (vars.commandIn("SAVE_EDIT_RELATION", "SAVE_EDIT_NEW",
                "SAVE_EDIT_EDIT")) {
            ...
            String strC_Invoice_ID = vars
                    .getRequiredStringParameter("inpcInvoiceId");
    ...
    InvoiceVendorMultilineData getEditVariables(VariablesSecureApp vars)
        ...
        data.issotrx = vars.getRequiredInputGlobalVariable("inpissotrx",
                windowId + "|IsSOTrx", "N");
        ...
        data.adOrgId = vars.getRequiredGlobalVariable("inpadOrgId", windowId
                + "|AD_Org_ID");
        ...
        data.cBpartnerId = vars.getRequiredGlobalVariable("inpcBpartnerId",
                windowId + "|C_BPartner_ID");
        data.cBpartnerLocationId = vars
        ...
        data.cPaymenttermId = vars
                .getRequiredStringParameter("inpcPaymenttermId");
        ...
        data.mPricelistId = vars.getGlobalVariable("inpmPricelistId", windowId
                + "|M_PriceList_ID", "");
        data.cCurrencyId = vars.getGlobalVariable("inpcCurrencyId", windowId
                + "|C_Currency_ID", "");
        data.cDoctypeId = vars.getGlobalVariable("inpcDoctypeId", windowId
                + "|C_DocType_ID", "");
        ...
        if (data.cDoctypetargetId.equals(""))
            data.cDoctypetargetId = InvoiceVendorMultilineData
                    .selectDocTypeTarget(this, Utility.getContext(this, vars,
                            "#User_Org", windowId), Utility.getContext(this,
                            vars, "#User_Client", windowId), Utility
                            .getContext(this, vars, "#AD_Client_ID", windowId),
                            data.issotrx);

        if (data.cDoctypeId.equals(""))
            data.cDoctypeId = InvoiceVendorMultilineData.selectDocType(this,
        ...
        if (data.mPricelistId.equals(""))
            data.mPricelistId = InvoiceVendorMultilineData.selectPriceList(
                    this,
                    Utility.getContext(this, vars, "#User_Org", windowId),
                    Utility.getContext(this, vars, "#User_Client", windowId));

        if (data.cCurrencyId.equals(""))
            data.cCurrencyId = InvoiceVendorMultilineData.selectCurrency(this,
                    Utility.getContext(this, vars, "#User_Org", windowId),
                    Utility.getContext(this, vars, "#User_Client", windowId));

        if (data.documentno.startsWith("<"))
            data.documentno = Utility.getDocumentNo(this, vars, windowId,
                    "C_Invoice", data.cDoctypetargetId, data.cDoctypeId, false,
                    true);
    ...
    void printPageDataSheet(HttpServletResponse response,
        ...
        InvoiceVendorMultilineData[] data = InvoiceVendorMultilineData.select(
                this, strDateFormat, vars.getLanguage(), strC_Invoice_ID);
        if (data == null || data.length == 0) {
            discard[0] = new String("editDiscard");
            strCommand = "NEW";
            data = InvoiceVendorMultilineData.set(vars.getOrg(), vars
                    .getClient(), DateTimeData.today(this), "DR", "CO",
                    InvoiceVendorMultilineData.selectDocAction(this, vars
                            .getLanguage(), "CO"), "0", "0");
        }

        XmlDocument xmlDocument = xmlEngine.readXmlTemplate(
                "org/openbravo/erpCommon/ad_forms/InvoiceVendorMultiline",
        ...
        xmlDocument.setParameter("windowPath", FormatUtilities
                .replace(windowNameEnUS)
                + "/" + FormatUtilities.replace(tabNameEnUS) + "_Edition.html");
        ...
        ToolBar toolbar = new ToolBar(
                ...
                "document.frmMain.inpcInvoiceId", "", "", "".equals("Y"),
                "ExpenseInvoice", strReplaceWith, true);
        toolbar.prepareEditionTemplate("N".equals("Y"), false, vars
                .getSessionValue("#ShowTest", "N").equals("Y"),
                "N".equals("Y"), false);
        ...
        try {
            ...
            NavigationBar nav = new NavigationBar(this, vars.getLanguage(),
                    "../ExpenseInvoice/ExpenseInvoice_Relation.html",
        ...
        try {
            ComboTableData comboTableData = new ComboTableData(vars, this,
                    "LIST", "", "All_Payment Rule", "", Utility.getContext(
                            this, vars, "#User_Org", windowId), Utility
                            .getContext(this, vars, "#User_Client", windowId),
                    0);
            Utility.fillSQLParameters(this, vars, null, comboTableData,
                    windowId, data[0].paymentrule);
        ...
        try {
            ComboTableData comboTableData = new ComboTableData(vars, this,
                    "TABLEDIR", "C_PaymentTerm_ID", "", "", Utility.getContext(
                            this, vars, "#User_Org", windowId), Utility
                            .getContext(this, vars, "#User_Client", windowId),
                    0);
            Utility.fillSQLParameters(this, vars, null, comboTableData,
                    windowId, data[0].cPaymenttermId);
            xmlDocument.setData("reportC_PaymentTerm_ID", "liststructure",
                    comboTableData.select(false));
            comboTableData = null;
        } catch (Exception ex) {
            throw new ServletException(ex);
        ...
        try {
            ComboTableData comboTableData = new ComboTableData(vars, this,
                    ...
                    "C_BPartner Location - Bill To", Utility.getContext(this,
                            vars, "#User_Org", windowId), Utility.getContext(
                            this, vars, "#User_Client", windowId), 0);
            Utility.fillSQLParameters(this, vars, null, comboTableData,
                    windowId, data[0].cBpartnerId);
            xmlDocument.setData("reportC_BPartner_Location_ID",
                    "liststructure", comboTableData.select(false));
            comboTableData = null;
        } catch (Exception ex) {
            throw new ServletException(ex);
        }
