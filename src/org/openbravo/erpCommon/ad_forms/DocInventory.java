...
public class DocInventory extends AcctServer {
    ...
    public Fact createFact(AcctSchema as, ConnectionProvider conn,
        ...
        for (int i = 0; i < p_lines.length; i++) {
            ...
            if (invDiff == null)
                invDiff = getAccount(AcctServer.ACCTTYPE_InvDifferences, as,
                        conn);
            log4jDocInventory.debug("CreateFact - after getAccount - invDiff; "
                    + invDiff);
            cr = fact.createLine(line, invDiff, as.getC_Currency_ID(), (b_Costs
                    .negate()).toString(), Fact_Acct_Group_ID,
                    nextSeqNo(SeqNo), DocumentType, conn);
