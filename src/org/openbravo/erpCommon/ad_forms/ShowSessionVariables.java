...
public class ShowSessionVariables extends HttpSecureAppServlet {
  ...
  public void doPost (HttpServletRequest request, HttpServletResponse response) throws IOException,ServletException {
    ...
    if (vars.commandIn("REMOVE")) {
      String preferences = vars.getRequestGlobalVariable("inpPreference", "ShowSessionVariables|preferences");
      String global = vars.getRequestGlobalVariable("inpGlobal", "ShowSessionVariables|global");
      String accounting = vars.getRequestGlobalVariable("inpAccounting", "ShowSessionVariables|accounting");
      String windowG = vars.getRequestGlobalVariable("inpWindowGlobal", "ShowSessionVariables|windowGlobal");
      ...
      String preferences = vars.getGlobalVariable("inpPreference", "ShowSessionVariables|preferences", "Y");
      String global = vars.getGlobalVariable("inpGlobal", "ShowSessionVariables|global", "Y");
      String accounting = vars.getGlobalVariable("inpAccounting", "ShowSessionVariables|accounting", "Y");
      String windowG = vars.getGlobalVariable("inpWindowGlobal", "ShowSessionVariables|windowGlobal", "Y");
  ...
  ShowSessionVariablesStructureData[] orderStructure(ShowSessionVariablesStructureData[] data, ShowSessionVariablesData[] windows, boolean preferences, boolean global, boolean accounting, boolean windowGlobal, String window) {
        ...
        for (int i=0;i<data.length;i++) {
          if (data[i].isPreference && (data[i].window==null || data[i].window.equals(""))) {
            ...
            for (int j=0;j<vecPreferences.size() && !insertado;j++) {
            	ShowSessionVariablesStructureData element = (ShowSessionVariablesStructureData) vecPreferences.elementAt(j);
        ...
        for (int i=0;i<data.length;i++) {
          if (data[i].isPreference && (data[i].window!=null && !data[i].window.equals("") && (data[i].window.equals(window) || window.equals("0")))) {
            ...
            for (int j=0;j<vecPreferencesW.size() && !insertado;j++) {
            	ShowSessionVariablesStructureData element = (ShowSessionVariablesStructureData) vecPreferencesW.elementAt(j);
        ...
        for (int i=0;i<data.length;i++) {
          if (data[i].isGlobal) {
            ...
            for (int j=0;j<vecGlobal.size() && !insertado;j++) {
            	ShowSessionVariablesStructureData element = (ShowSessionVariablesStructureData) vecGlobal.elementAt(j);
              if (element.name.compareTo(data[i].name)>=0) {
        ...
        for (int i=0;i<data.length;i++) {
          if (data[i].isAccounting) {
            ...
            for (int j=0;j<vecAccounting.size() && !insertado;j++) {
            	ShowSessionVariablesStructureData element = (ShowSessionVariablesStructureData) vecAccounting.elementAt(j);
              if (element.name.compareTo(data[i].name)>=0) {
        ...
        for (int i=0;i<data.length;i++) {
          if (!data[i].isAccounting && !data[i].isGlobal && !data[i].isPreference && (data[i].window==null || data[i].window.equals(""))) {
            ...
            for (int j=0;j<vecWindowG.size() && !insertado;j++) {
            	ShowSessionVariablesStructureData element = (ShowSessionVariablesStructureData) vecWindowG.elementAt(j);
              if (element.name.compareTo(data[i].name)>=0) {
        ...
        for (int i=0;i<data.length;i++) {
          if (!data[i].isAccounting && !data[i].isGlobal && !data[i].isPreference && (data[i].window!=null && !data[i].window.equals("") && (data[i].window.equals(window) || window.equals("0")))) {
            ...
            for (int j=0;j<vecWindow.size() && !insertado;j++) {
            	ShowSessionVariablesStructureData element = (ShowSessionVariablesStructureData) vecWindow.elementAt(j);
              if (element.windowName.compareTo(data[i].windowName)>0) {
  ...
  ShowSessionVariablesStructureData[] compoundSession(HttpServletRequest request, VariablesSecureApp vars, Vector<Object> windows) {
  ...
  void printPageDataSheet(HttpServletRequest request, HttpServletResponse response, VariablesSecureApp vars, String preferences, String global, String accounting, String windowG, String window) throws IOException, ServletException {
    ...
    if (data==null || data.length==0) {
      ...
      xmlDocument = xmlEngine.readXmlTemplate("org/openbravo/erpCommon/ad_forms/ShowSessionVariables", discard).createXmlDocument();
    } else {
      xmlDocument = xmlEngine.readXmlTemplate("org/openbravo/erpCommon/ad_forms/ShowSessionVariables").createXmlDocument();
    }
    ...
    ToolBar toolbar = new ToolBar(this, vars.getLanguage(), "ShowSessionVariables", false, "", "", "",false, "ad_forms",  strReplaceWith, false,  true);
    ...
    try {
      WindowTabs tabs = new WindowTabs(this, vars, "org.openbravo.erpCommon.ad_forms.ShowSessionVariables");
      xmlDocument.setParameter("theme", vars.getTheme());
      NavigationBar nav = new NavigationBar(this, vars.getLanguage(), "ShowSessionVariables.html", classInfo.id, classInfo.type, strReplaceWith, tabs.breadcrumb());
      xmlDocument.setParameter("navigationBar", nav.toString());
      LeftTabsBar lBar = new LeftTabsBar(this, vars.getLanguage(), "ShowSessionVariables.html", strReplaceWith);
    ...
    ShowSessionVariablesData[] windowsData = (vars.getLanguage().equals("en_US")?ShowSessionVariablesData.select(this, strWindows.toString()):ShowSessionVariablesData.selectTrl(this, strWindows.toString(), vars.getLanguage()));
    {
