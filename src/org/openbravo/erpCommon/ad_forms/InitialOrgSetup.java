...
public class InitialOrgSetup extends HttpSecureAppServlet {
  ...
  public void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
    ...
    if (vars.commandIn("DEFAULT")) {
      ...
      String strResult = process(request, response, vars, strModules);
  ...
  private void printPage(HttpServletResponse response, VariablesSecureApp vars) throws IOException, ServletException{
    ModuleReferenceDataOrgTree tree = new ModuleReferenceDataOrgTree(this, vars.getClient(), false, true);
    XmlDocument xmlDocument = null;
    String [] discard = {"selEliminar"};
    if(tree.getData()==null || tree.getData().length==0)
      xmlDocument = xmlEngine.readXmlTemplate("org/openbravo/erpCommon/ad_forms/InitialOrgSetup").createXmlDocument();
    else xmlDocument = xmlEngine.readXmlTemplate("org/openbravo/erpCommon/ad_forms/InitialOrgSetup", discard).createXmlDocument();
    ...
    try {
      WindowTabs tabs = new WindowTabs(this, vars, "org.openbravo.erpCommon.ad_forms.InitialOrgSetup");
      xmlDocument.setParameter("parentTabContainer", tabs.parentTabs());
      xmlDocument.setParameter("mainTabContainer", tabs.mainTabs());
      xmlDocument.setParameter("childTabContainer", tabs.childTabs());
      xmlDocument.setParameter("theme", vars.getTheme());
      NavigationBar nav = new NavigationBar(this, vars.getLanguage(), "InitialOrgSetup.html", classInfo.id, classInfo.type, strReplaceWith, tabs.breadcrumb());
      xmlDocument.setParameter("navigationBar", nav.toString());
      LeftTabsBar lBar = new LeftTabsBar(this, vars.getLanguage(), "InitialOrgSetup.html", strReplaceWith);
    ...
    }
    {
      ...
      OBError myMessage = vars.getMessage("InitialOrgSetup");
      if (myMessage!=null) {
      ...
      xmlDocument.setData("reportParentOrg","liststructure", InitialOrgSetupData.selectParentOrg(this, vars.getLanguage(), vars.getClient()));
      try {
  ...
  private void printPageResult(HttpServletResponse response, VariablesSecureApp vars, String strResult) throws IOException, ServletException{
    ...
    myMessage.setTitle("");
    if(log4j.isDebugEnabled()) log4j.debug("InitialOrgSetup - before setMessage");
    if(strError!=null && !strError.equals("")) {
      myMessage = Utility.translateError(this, vars, vars.getLanguage(), strError);
    }
    if(log4j.isDebugEnabled()) log4j.debug("InitialOrgSetup - isOK: " + isOK);
    if(isOK) myMessage.setType("Success");
    else myMessage.setType("Error");
    if(log4j.isDebugEnabled()) log4j.debug("InitialOrgSetup - Message Type: " + myMessage.getType());
    vars.setMessage("InitialOrgSetup", myMessage);
    if(log4j.isDebugEnabled()) log4j.debug("InitialOrgSetup - after setMessage");
    if (myMessage!=null) {
      xmlDocument.setParameter("messageType", myMessage.getType());
      xmlDocument.setParameter("messageTitle", myMessage.getTitle());
      xmlDocument.setParameter("messageMessage", myMessage.getMessage());
  ...
  public String process(HttpServletRequest request, HttpServletResponse response, VariablesSecureApp vars, String strModules) throws IOException {
    ...
    try {
      conn = this.getTransactionConnection();
      String strIsLE = InitialOrgSetupData.isLE(this, strOrgType);
    ...
    try {
      ...
      m_info.append(SALTO_LINEA).append(Utility.messageBD(this, "StartingOrg", vars.getLanguage())).append(SALTO_LINEA);
      if (!createOrg(vars, strOrganization, strOrgType, strParentOrg, strOrgUser)){
        releaseRollbackConnection(conn);
        m_info.append(SALTO_LINEA).append(Utility.messageBD(this, "createOrgFailed", vars.getLanguage())).append(SALTO_LINEA);
        strSummary.append(SALTO_LINEA).append(Utility.messageBD(this, "createOrgFailed", vars.getLanguage())).append(SALTO_LINEA);
        isOK = false;
        return m_info.toString();
      }
    } catch (Exception err){
  ...
  //==========================================================================================================================================
      ...
      if(strModules!=null && !strModules.equals("")) {
        try {
          ...
          String strReferenceData = createReferenceData(vars, strOrganization, AD_Client_ID, strModules, bProduct, bBPartner, bProject, bCampaign, bSalesRegion, strCreateAccounting);
  ...
  public boolean createOrg(VariablesSecureApp vars, String orgName, String strOrgType, String strParentOrg, String userOrg) throws ServletException{
  ...
  //     *  Create Organization
      ...
      if (log4j.isDebugEnabled()) log4j.debug("InitialOrgSetup - createOrg - CLIENT_ID: " + AD_Client_ID);
...
//     *  Create Org
      ...
      if (InitialOrgSetupData.insertOrg(conn ,this, AD_Client_ID, AD_Org_ID, name, strOrgType) != 1) {
        String err = "InitialOrgSetup - createOrg - Org NOT inserted";
        log4j.warn(err);
        m_info.append(err).append(SALTO_LINEA);
        releaseRollbackConnection(conn);
        return false;
      }
      String strTree = InitialOrgSetupData.selectOrgTree(this, vars.getClient());
      ...
      if (log4j.isDebugEnabled()) log4j.debug("InitialOrgSetup - createOrg - m_info: " + m_info.toString());
...
//     *  - Org
      ...
      AD_User_U_Name = name;
      if (log4j.isDebugEnabled()) log4j.debug("InitialOrgSetup - createOrg - AD_User_U_Name : " + AD_User_U_Name);
      if (InitialOrgSetupData.insertUser(conn ,this, AD_Client_ID,AD_User_U_ID , name, FormatUtilities.sha1Base64(name), vars.getLanguage()) != 1) {
        String err = "InitialOrgSetup - createOrg - Org User A NOT inserted";
        log4j.warn(err);
        m_info.append(err).append(SALTO_LINEA);
        releaseRollbackConnection(conn);
        return false;
      }
      if (log4j.isDebugEnabled()) log4j.debug("InitialOrgSetup - createOrg - USER INSERTED " + name);
      //  Info
      ...
      releaseCommitConnection(conn);
      conn = this.getTransactionConnection();
      vars.setSessionValue("#USER_ORG", vars.getSessionValue("#USER_ORG") + ", '" + AD_Org_ID + "'");
      vars.setSessionValue("#ORG_CLIENT", vars.getSessionValue("#ORG_CLIENT") + ", '" + AD_Org_ID + "'");
      vars.setSessionValue("#AccessibleOrgTree", vars.getSessionValue("#AccessibleOrgTree") + ", '" + AD_Org_ID + "'");
      OBContext.getOBContext().addWritableOrganisation(AD_Org_ID);
//     *  Create User-Role
    ...
    m_info.append(SALTO_LINEA).append(Utility.messageBD(this, "CreateOrgSuccess", vars.getLanguage())).append(SALTO_LINEA);
  ...
  public boolean save (Connection conn, VariablesSecureApp vars, String AD_Client_ID, String AD_Org_ID, String C_Element_ID, AccountingValueData[] data) throws ServletException {
    String strAccountTree = InitialOrgSetupData.selectTree(this, AD_Client_ID);
    for (int i=0;i<data.length;i++) {
      if (log4j.isDebugEnabled()) log4j.debug("InitialOrgSetup - save - DATA LENGTH : " + data.length + ", POSICION : " + i + ", DEFAULT_ACCT: " + data[i].defaultAccount);
      ...
      String accountType="";
      String accountSign="";
      if (log4j.isDebugEnabled()) log4j.debug("InitialOrgSetup - save - AccountType debug");
      if (!data[i].accountType.equals("")){
        ...
        if (log4j.isDebugEnabled()) log4j.debug("InitialOrgSetup - save - Not is account type");
      } else {
        accountType = "E";
        if (log4j.isDebugEnabled()) log4j.debug("InitialOrgSetup - save - Is account type");
      }
      if (!data[i].accountSign.equals("")){
        ...
        if (log4j.isDebugEnabled()) log4j.debug("InitialOrgSetup - save - ACCOUNT VALUE : " + data[i].accountValue + " ACCOUNT NAME : " + data[i].accountName + " DEFAULT_ACCOUNT: " + data[i].defaultAccount);
        if (log4j.isDebugEnabled()) log4j.debug("InitialOrgSetup - save - C_ElementValue_ID: " + C_ElementValue_ID );
        if (log4j.isDebugEnabled()) log4j.debug("InitialOrgSetup - save - C_Element_ID: " + C_Element_ID);
        if (log4j.isDebugEnabled()) log4j.debug("InitialOrgSetup - save - data[i].accountValue: " + data[i].accountValue);
        if (log4j.isDebugEnabled()) log4j.debug("InitialOrgSetup - save - data[i].accountName: " + data[i].accountName);
        if (log4j.isDebugEnabled()) log4j.debug("InitialOrgSetup - save - data[i].accountDescription: " + data[i].accountDescription);
        if (log4j.isDebugEnabled()) log4j.debug("InitialOrgSetup - save - accountType: " + accountType);

      if (!data[i].accountValue.equals("")) {
        try {
          if (InitialOrgSetupData.insertElementValue(conn ,this, C_ElementValue_ID, C_Element_ID, AD_Client_ID, AD_Org_ID, data[i].accountValue, data[i].accountName, data[i].accountDescription, accountType, accountSign, IsDocControlled, IsSummary, data[i].elementLevel) != 1) {
            log4j.warn("InitialOrgSetup - save - Natural Account not added");
            data[i].cElementValueId = "";
            return false;
            } else {
              String strParent = InitialOrgSetupData.selectParent(conn ,this, data[i].accountParent, AD_Client_ID);
              if(strParent!=null && !strParent.equals(""))InitialOrgSetupData.updateTreeNode(conn ,this, strParent, strAccountTree, C_ElementValue_ID, AD_Client_ID);
            }
    ...
    if (log4j.isDebugEnabled()) log4j.debug("InitialOrgSetup - save - NATURAL ACCOUNT ADDED");
    if (log4j.isDebugEnabled()) log4j.debug("InitialOrgSetup - save - m_info last: " + m_info.toString());
...
//    strResult = filterArray(strResult);
 ...
 public String nextSeqNo(String oldSeqNo){
  ...
  public AccountingValueData[] parseData (FieldProvider[] data) throws ServletException {
    ...
    if (log4j.isDebugEnabled()) log4j.debug("InitialOrgSetup - dataAux.accountValue: " + dataAux.accountValue);
      dataAux.accountName = data[i].getField("accountName");
    if (log4j.isDebugEnabled()) log4j.debug("InitialOrgSetup - dataAux.accountName: " + dataAux.accountName);
      dataAux.accountDescription = data[i].getField("accountDescription");
    if (log4j.isDebugEnabled()) log4j.debug("InitialOrgSetup - dataAux.accountDescription: " + dataAux.accountDescription);
      dataAux.accountType = data[i].getField("accountType");
    if (log4j.isDebugEnabled()) log4j.debug("InitialOrgSetup - dataAux.accountType: " + dataAux.accountType);
      dataAux.accountSign = data[i].getField("accountSign");
    if (log4j.isDebugEnabled()) log4j.debug("InitialOrgSetup - dataAux.accountSign: " + dataAux.accountSign);
      dataAux.accountDocument = data[i].getField("accountDocument");
    if (log4j.isDebugEnabled()) log4j.debug("InitialOrgSetup - dataAux.accountDocument: " + dataAux.accountDocument);
      dataAux.accountSummary = data[i].getField("accountSummary");
    if (log4j.isDebugEnabled()) log4j.debug("InitialOrgSetup - dataAux.accountSummary: " + dataAux.accountSummary);
      dataAux.defaultAccount = data[i].getField("defaultAccount");
    if (log4j.isDebugEnabled()) log4j.debug("InitialOrgSetup - dataAux.defaultAccount: " + dataAux.defaultAccount);
      dataAux.accountParent = data[i].getField("accountParent");
    if (log4j.isDebugEnabled()) log4j.debug("InitialOrgSetup - dataAux.accountParent: " + dataAux.accountParent);
      dataAux.elementLevel = data[i].getField("elementLevel");
    if (log4j.isDebugEnabled()) log4j.debug("InitialOrgSetup - dataAux.elementLevel: " + dataAux.elementLevel);
      dataAux.balanceSheet = data[i].getField("balanceSheet");
    if (log4j.isDebugEnabled()) log4j.debug("InitialOrgSetup - dataAux.operands: " + dataAux.operands);
      dataAux.operands = data[i].getField("operands");
    if (log4j.isDebugEnabled()) log4j.debug("InitialOrgSetup - dataAux.balanceSheet: " + dataAux.balanceSheet);
      dataAux.balanceSheetName = data[i].getField("balanceSheetName");
    if (log4j.isDebugEnabled()) log4j.debug("InitialOrgSetup - dataAux.balanceSheetName: " + dataAux.balanceSheetName);
      dataAux.uS1120BalanceSheet = data[i].getField("uS1120BalanceSheet");
    if (log4j.isDebugEnabled()) log4j.debug("InitialOrgSetup - dataAux.uS1120BalanceSheet: " + dataAux.uS1120BalanceSheet);
      dataAux.uS1120BalanceSheetName = data[i].getField("uS1120BalanceSheetName");
    if (log4j.isDebugEnabled()) log4j.debug("InitialOrgSetup - dataAux.uS1120BalanceSheetName: " + dataAux.uS1120BalanceSheetName);
      dataAux.profitAndLoss = data[i].getField("profitAndLoss");
    if (log4j.isDebugEnabled()) log4j.debug("InitialOrgSetup - dataAux.profitAndLoss: " + dataAux.profitAndLoss);
      dataAux.profitAndLossName = data[i].getField("profitAndLossName");
    if (log4j.isDebugEnabled()) log4j.debug("InitialOrgSetup - dataAux.profitAndLossName: " + dataAux.profitAndLossName);
      dataAux.uS1120IncomeStatement = data[i].getField("uS1120IncomeStatement");
    if (log4j.isDebugEnabled()) log4j.debug("InitialOrgSetup - dataAux.uS1120IncomeStatement: " + dataAux.uS1120IncomeStatement);
      dataAux.uS1120IncomeStatementName = data[i].getField("uS1120IncomeStatementName");
    if (log4j.isDebugEnabled()) log4j.debug("InitialOrgSetup - dataAux.uS1120IncomeStatementName: " + dataAux.uS1120IncomeStatementName);
      dataAux.cashFlow = data[i].getField("cashFlow");
    if (log4j.isDebugEnabled()) log4j.debug("InitialOrgSetup - dataAux.cashFlow: " + dataAux.cashFlow);
      dataAux.cashFlowName = data[i].getField("cashFlowName");
    if (log4j.isDebugEnabled()) log4j.debug("InitialOrgSetup - dataAux.cashFlowName: " + dataAux.cashFlowName);
      dataAux.cElementValueId = data[i].getField("cElementValueId");
    if (log4j.isDebugEnabled()) log4j.debug("InitialOrgSetup - dataAux.cElementValueId: " + dataAux.cElementValueId);
  ...
  public boolean createAccounting(VariablesSecureApp vars, String strOrganization, String newC_Currency_ID, String curName, boolean hasProduct, boolean hasBPartner, boolean hasProject,
    ...
    try {
      ...
      if (InitialOrgSetupData.insertElement(conn ,this, AD_Client_ID, AD_Org_ID, C_Element_ID, name, AD_Tree_Account_ID) != 1) {
        String err = "InitialOrgSetup - createAccounting - Acct Element NOT inserted";
        log4j.warn(err);
        m_info.append(err).append(SALTO_LINEA);
        releaseRollbackConnection(conn);
        return false;
      ...
      if (!errMsg) {
        releaseRollbackConnection(conn);
        String err = "InitialOrgSetup - createAccounting - Acct Element Values NOT inserted";
        log4j.warn(err);
        m_info.append(err).append(SALTO_LINEA);
        return false;
      }
  ...
  //     *  Create AccountingSchema
      ...
      //
      if (InitialOrgSetupData.insertAcctSchema(conn ,this, AD_Client_ID, AD_Org_ID, C_AcctSchema_ID, name, GAAP, CostingMethod, C_Currency_ID) != 1){
        String err = "InitialOrgSetup - createAccounting - AcctSchema NOT inserted";
        log4j.warn(err);
        m_info.append(err).append(SALTO_LINEA);
        releaseRollbackConnection(conn);
        return false;
        }
      if (log4j.isDebugEnabled()) log4j.debug("InitialOrgSetup - createAccounting - ACCT SCHEMA INSERTED");
      //  Info
...
//     *  Create AccountingSchema Elements (Structure)
    ...
    try {
      ComboTableData comboTableData = new ComboTableData(vars, this, "LIST", "", "C_AcctSchema ElementType", "", Utility.getContext(this, vars, "#User_Org", "InitialOrgSetup"), Utility.getContext(this, vars, "#User_Client", "InitialOrgSetup"), 0);
    ...
    try {
      for (int i=0; i< data1.length;i++) {
        ...
        if (log4j.isDebugEnabled()) log4j.debug("InitialOrgSetup - createAccounting - C_ElementValue_ID: " + C_ElementValue_ID);

        if (!IsMandatory.equals("")){
          if (InitialOrgSetupData.insertAcctSchemaElement(conn, this, AD_Client_ID, AD_Org_ID, C_AcctSchema_Element_ID,C_AcctSchema_ID, ElementType, name, SeqNo, IsMandatory, IsBalanced) == 1)
          ...
          if (ElementType.equals("OO")){
            if (InitialOrgSetupData.updateAcctSchemaElement(conn, this, AD_Org_ID, C_AcctSchema_Element_ID) != 1){
              ...
              m_info.append("InitialOrgSetup - createAccounting - Default Org in AcctSchamaElement NOT updated").append(SALTO_LINEA);
            }
          }
          if (ElementType.equals("AC")){
            if (InitialOrgSetupData.updateAcctSchemaElement2(conn, this, C_ElementValue_ID, C_Element_ID, C_AcctSchema_Element_ID) != 1){
              log4j.warn("InitialOrgSetup - createAccounting - Default Account in AcctSchamaElement NOT updated");
              m_info.append("InitialOrgSetup - createAccounting - Default Account in AcctSchamaElement NOT updated").append(SALTO_LINEA);
            }
          }
        }
      }
    ...
    if (log4j.isDebugEnabled()) log4j.debug("InitialOrgSetup - createAccounting - ACCT SCHEMA ELEMENTS INSERTED");
    if (log4j.isDebugEnabled()) log4j.debug("InitialOrgSetup - createAccounting - m_info last: " + m_info.toString());
    try {
      //  Create AcctSchema
      releaseCommitConnection(conn);
...
getAcct(conn, data, "A_DEPRECIATION_ACCT"),getAcct(conn, data, "A_ACCUMDEPRECIATION_ACCT"),getAcct(conn, data, "A_DISPOSAL_LOSS"),getAcct(conn, data, "A_DISPOSAL_GAIN"))  != 1) {
      ...
      if(data!=null && data.length!=0) {
        ...
        for(int i=0;i<data.length;i++){
          if(data[i].haschartofaccounts.equals("Y") && !strCreateAccounting.equals("")){ 
            ...
            createAccounting(vars, strOrganization, C_Currency_ID, InitialOrgSetupData.currency(this, C_Currency_ID), hasProduct, hasBPartner, hasProject, hasMCampaign, hasSRegion, av.getFieldProvider());
          }
          ...
          for(int j=0;j<myFiles.length;j++){
            ...
            if (myResult.getWarningMessages()!=null && !myResult.getWarningMessages().equals("") && !myResult.getWarningMessages().equals("null")){
            ...
            if (myResult.getErrorMessages()!=null && !myResult.getErrorMessages().equals("") && !myResult.getErrorMessages().equals("null")){
            ...
            if(myResult.getErrorMessages()!=null && !myResult.getErrorMessages().equals("") && !myResult.getErrorMessages().equals("null")) strError = strError.append(myResult.getErrorMessages());
          }
          if (!strError.toString().equals("")) return strError.toString();
          else {
            InitialOrgSetupData.insertOrgModule(this, strClient, AD_Org_ID, vars.getUser(), data[i].adModuleId, data[i].version);
  ...
  private InitialOrgSetupData[] orderModuleByDependency(InitialOrgSetupData[] modules) {
    ...
    InitialOrgSetupData[] rt = new InitialOrgSetupData[orderList.size()];
  ...
  private String getAcct(Connection conn, AccountingValueData [] data, String key) throws ServletException {
      ...
      String C_ElementValue_ID = getC_ElementValue_ID(data, key);
       if (log4j.isDebugEnabled()) log4j.debug("InitialOrgSetup - getAcct - C_ElementValue_ID: " + C_ElementValue_ID);
