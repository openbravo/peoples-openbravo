...
public class DocPayment extends AcctServer {
     ...
     *            AD_Client_ID
     */
    public DocPayment(String AD_Client_ID, String AD_Org_ID,
    ...
    public void loadObjectFieldProvider(ConnectionProvider conn,
        ...
        setObjectFieldProvider(DocPaymentData.selectRegistro(conn,
                AD_Client_ID, Id));
    }
    ...
    public boolean loadDocumentDetails(FieldProvider[] data,
        ...
        if (log4j.isDebugEnabled())
            log4j.debug("DocPayment - loadDocumentDetails - Lines="
                    + p_lines.length);
        return false;
    } // loadDocumentDetails
    ...
    public Fact createFact(AcctSchema as, ConnectionProvider conn,
        ...
        if (log4j.isDebugEnabled())
            log4j.debug("DocPayment - createFact - p_lines.length - "
                    + p_lines.length);
        ...
        for (int i = 0; p_lines != null && i < p_lines.length; i++) {
            ...
            if (log4j.isDebugEnabled())
                log4j.debug("DocPayment - createFact - line.conversionDate - "
                        + line.conversionDate);
            // 1* Amount is calculated and if there is currency conversion
            // variations between dates this change is accounted
            String convertedAmt = convertAmount(line.Amount, line.isReceipt
                    .equals("Y"), DateAcct, line.conversionDate,
                    line.C_Currency_ID_From, C_Currency_ID, line, as, fact,
                    Fact_Acct_Group_ID, conn);
            String convertWithHold = convertAmount(line.WithHoldAmt,
            ...
            if (line.isManual.equals("N")) { // 2* Normal debt-payments
                if (!line.C_Settlement_Generate_ID.equals(Record_ID)) { // 2.1*
                    ...
                    fact.createLine(line, getAccountBPartner(
                            line.m_C_BPartner_ID, as, line.isReceipt
                                    .equals("Y"), line.dpStatus, conn),
                            C_Currency_ID, (line.isReceipt.equals("Y") ? ""
                                    : convertTotal.toString()),
                            (line.isReceipt.equals("Y") ? convertTotal
                                    .toString() : ""), Fact_Acct_Group_ID,
                            nextSeqNo(SeqNo), DocumentType, conn);
                } else { // 2.2* Generated DP
                    if (log4j.isDebugEnabled())
                        log4j.debug("Genenarted DP");
                    if (!line.isPaid.equals("Y")
                            || !(line.C_Settlement_Cancel_ID == null || line.C_Settlement_Cancel_ID
                                    .equals(""))) {
                        if (log4j.isDebugEnabled())
                ...
                if (log4j.isDebugEnabled())
                    log4j
                            .debug("DocPayment - createFact - No manual  - isReceipt: "
                                    + line.isReceipt);
            } else {// 3* MANUAL debt-payments (generated in a Manual stt)
                if (log4j.isDebugEnabled())
                    log4j.debug("Manual DP - DirectPosting: "
                            + line.IsDirectPosting + " - SettType:"
                            + SettlementType);
                if (line.IsDirectPosting.equals("Y")) { // Direct posting:
                                                        // transitory Account
                    BigDecimal amount = ZERO;
                    DocPaymentData[] data = DocPaymentData.selectDirectManual(
                            conn, as.m_C_AcctSchema_ID, line.Line_ID);
                    if (log4j.isDebugEnabled())
                        log4j.debug("data[0].amount:" + data[0].amount
                    ...
                    if (log4j.isDebugEnabled())
                        log4j.debug("Manual DP - amount:" + amount
                                + " - transitoryAmount:" + transitoryAmount
                                + " - Receipt:" + line.isReceipt);
                    // Depending on the stt type and the signum of DP it will be
                    // posted on credit or debit
                    if (amount.signum() == 1) {
                        fact.createLine(line, new Account(conn, (line.isReceipt
                                .equals("Y") ? data[0].creditAcct
                                : data[0].debitAcct)), C_Currency_ID,
                                (line.isReceipt.equals("Y") ? transitoryAmount
                                        .abs().toString() : "0"),
                                (line.isReceipt.equals("Y") ? "0"
                                        : transitoryAmount.abs().toString()),
                                Fact_Acct_Group_ID, nextSeqNo(SeqNo),
                                DocumentType, conn);
                        if ((!changeGenerate && line.isReceipt.equals("N"))
                                || (changeGenerate && line.isReceipt
                                        .equals("Y")))
                            amount = amount.negate();
                    } else {
                        fact.createLine(line, new Account(conn, (line.isReceipt
                ...
                // conceptos contables
                if (SettlementType.equals("I")
                        || line.IsDirectPosting.equals("N")) {
                    DocPaymentData[] data = DocPaymentData.selectManual(conn,
                            as.m_C_AcctSchema_ID, line.Line_ID);
                    for (int j = 0; data != null && j < data.length; j++) {
                        String amountdebit = getConvertedAmt(
                                data[j].amountdebit, line.C_Currency_ID_From,
                                C_Currency_ID, DateAcct, "", AD_Client_ID,
                                AD_Org_ID, conn);
                        String amountcredit = getConvertedAmt(
                                data[j].amountcredit, line.C_Currency_ID_From,
                                C_Currency_ID, DateAcct, "", AD_Client_ID,
                                AD_Org_ID, conn);
                        if (log4j.isDebugEnabled())
                            log4j
                                    .debug("DocPayment - createFact - Conceptos - AmountDebit: "
                                            + amountdebit
                                            + " - AmountCredit: "
                                            + amountcredit);
                        fact.createLine(line, new Account(conn, (line.isReceipt
                                .equals("Y") ? data[j].creditAcct
                                : data[j].debitAcct)), C_Currency_ID,
                                (amountdebit.equals("0") ? "" : amountdebit),
                                (amountcredit.equals("0") ? "" : amountcredit),
                                Fact_Acct_Group_ID, nextSeqNo(SeqNo),
                                DocumentType, conn);
                    }
                }
            ...
            if (line.C_Settlement_Cancel_ID.equals(Record_ID)) { // Cancelled
                ...
                if (line.WriteOffAmt != null && !line.WriteOffAmt.equals("")
                    ...
                    fact
                            .createLine(line, getAccount(
                                    AcctServer.ACCTTYPE_WriteOffDefault, as,
                                    conn), C_Currency_ID, (line.isReceipt
                                    .equals("Y") ? line.WriteOffAmt : ""),
                                    (line.isReceipt.equals("Y") ? ""
                                            : line.WriteOffAmt),
                                    Fact_Acct_Group_ID, nextSeqNo(SeqNo),
                                    DocumentType, conn);
            ...
            if (line.isPaid.equals("Y")
                    && ((line.C_Settlement_Cancel_ID == null || line.C_Settlement_Cancel_ID
                            .equals("")) || (line.C_Settlement_Cancel_ID
                            .equals(Record_ID)))) {
                ...
                String IdAccount = WithholdingManualData.select_accounts(conn,
                        line.C_WITHHOLDING_ID, idSchema);
                //
                ...
                if (line.WriteOffAmt != null && !line.WriteOffAmt.equals("")
                        && !line.WriteOffAmt.equals("0"))
                    finalLineAmt = finalLineAmt.subtract(new BigDecimal(
                            line.WriteOffAmt));
                String finalAmtTo = getConvertedAmt(finalLineAmt.toString(),
                        line.C_Currency_ID_From, C_Currency_ID, DateAcct, "",
                ...
                if (finalLineAmt.compareTo(new BigDecimal("0.00")) != 0) {
                    if (line.C_BANKSTATEMENTLINE_ID != null
                        ...
                        fact.createLine(line, getAccountBankStatementLine(
                                line.C_BANKSTATEMENTLINE_ID, as, conn),
                                C_Currency_ID,
                                (line.isReceipt.equals("Y") ? finalAmtTo : ""),
                                (line.isReceipt.equals("Y") ? "" : finalAmtTo),
                                Fact_Acct_Group_ID, "999999", DocumentType,
                                conn);
                     ...
                     // Fact_Acct_Group_ID, "999999", DocumentType,conn);
                        ...
                        fact.createLine(line, getAccount(
                                AcctServer.ACCTTYPE_BankInTransitDefault, as,
                                conn), C_Currency_ID, (line.isReceipt
                                .equals("Y") ? finalAmtTo : ""),
                                (line.isReceipt.equals("Y") ? "" : finalAmtTo),
                                Fact_Acct_Group_ID, "999999", DocumentType,
    ...
    public String convertAmount(String Amount, boolean isReceipt,
            String DateAcct, String conversionDate, String C_Currency_ID_From,
            String C_Currency_ID, DocLine line, AcctSchema as, Fact fact,
            String Fact_Acct_Group_ID, ConnectionProvider conn)
            throws ServletException {
        if (log4j.isDebugEnabled())
            log4j.debug("Amount:" + Amount + " curr from:" + C_Currency_ID_From
                    + " Curr to:" + C_Currency_ID + " convDate:"
                    + conversionDate + " DateAcct:" + DateAcct);
        if (Amount == null || Amount.equals(""))
            Amount = "0";
        String Amt = getConvertedAmt(Amount, C_Currency_ID_From, C_Currency_ID,
        ...
        BigDecimal AmtDiff = (new BigDecimal(AmtTo)).subtract(new BigDecimal(
                Amt));
        if (log4j.isDebugEnabled())
        ...
        if (log4j.isDebugEnabled()) {
            log4j.debug("curr from:" + C_Currency_ID_From + " Curr to:"
                    + C_Currency_ID + " convDate:" + conversionDate
                    + " DateAcct:" + DateAcct);
        ...
        if ((isReceipt && AmtDiff.compareTo(new BigDecimal("0.00")) == 1)
            ...
            fact.createLine(line, getAccount(
                    AcctServer.ACCTTYPE_ConvertGainDefaultAmt, as, conn),
                    C_Currency_ID, "", AmtDiff.abs().toString(),
                    Fact_Acct_Group_ID, nextSeqNo(SeqNo), DocumentType, conn);
        } else {
            fact.createLine(line, getAccount(
                    AcctServer.ACCTTYPE_ConvertChargeDefaultAmt, as, conn),
                    C_Currency_ID, AmtDiff.abs().toString(), "",
                    Fact_Acct_Group_ID, nextSeqNo(SeqNo), DocumentType, conn);
        }
    ...
    public final Account getAccountBPartner(String cBPartnerId, AcctSchema as,
        ...
        try {
            if (log4j.isDebugEnabled())
                log4j.debug("DocPayment - getAccountBPartner - DocumentType = "
                        + DocumentType);
            if (isReceipt) {
     ...
     * @return Account
     */
    public final Account getAccountBankStatementLine(
            String strcBankstatementlineId, AcctSchema as,
            ConnectionProvider conn) {
        DocPaymentData[] data = null;
        try {
            data = DocPaymentData.selectBankStatementLineAcct(conn,
                    strcBankstatementlineId, as.getC_AcctSchema_ID());
        } catch (ServletException e) {
            log4j.warn(e);
        }
        // Get Acct
    ...
    public final Account getAccountCashLine(String strcCashlineId,
        ...
        DocPaymentData[] data = null;
        try {
            data = DocPaymentData.selectCashLineAcct(conn, strcCashlineId, as
                    .getC_AcctSchema_ID());
        } catch (ServletException e) {
            log4j.warn(e);
        }
