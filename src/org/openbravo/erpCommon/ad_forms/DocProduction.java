...
public class DocProduction extends AcctServer {
     ...
     *            AD_Client_ID
     */
    public DocProduction(String AD_Client_ID, String AD_Org_ID,
    ...
    public void loadObjectFieldProvider(ConnectionProvider conn,
        ...
        setObjectFieldProvider(DocProductionData.selectRegistro(conn,
                AD_Client_ID, Id));
    }
    ...
    public Fact createFact(AcctSchema as, ConnectionProvider conn,
        ...
        for (int i = 0; p_lines != null && i < p_lines.length; i++) {
            ...
            log4j.debug("DocProduction - createFact - line.m_Productiontype - "
            ...
            if (line.m_Productiontype.equals("+")) {
                ...
                fact.createLine(line, getAccountWarehouse(
                        line.m_M_Warehouse_ID, as, conn),
                        as.getC_Currency_ID(), "", costs, Fact_Acct_Group_ID,
                        nextSeqNo(SeqNo), DocumentType, conn);
            } else {
                fact.createLine(line, line.getAccount(
                        ProductInfo.ACCTTYPE_P_Asset, as, conn), as
     ...
     * @return Account
     */
    public final Account getAccountWarehouse(String M_Warehouse_ID,
            AcctSchema as, ConnectionProvider conn) throws ServletException {
        AcctServerData[] data = null;
        data = AcctServerData.selectWDifferencesAcct(conn, M_Warehouse_ID, as
                .getC_AcctSchema_ID());
        // Get Acct
        String Account_ID = "";
