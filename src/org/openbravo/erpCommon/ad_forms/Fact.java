...
public class Fact {
     ...
     */
    public Fact(AcctServer document, AcctSchema acctSchema,
        ...
        log4jFact.debug("Fact[" + m_doc.DocumentNo + "," + "AcctSchema["
                + m_acctSchema.m_C_AcctSchema_ID + "-" + m_acctSchema.m_Name
	...
	if (strNegate.equals("N")
        ...
        //
        log4jFact.debug("createLine - Fact_Acct_Group_ID = "
                + Fact_Acct_Group_ID);
        FactLine line = new FactLine(m_doc.AD_Table_ID, m_doc.Record_ID,
                docLine == null ? "" : docLine.m_TrxLine_ID,
        ...
        log4jFact.debug("C_Currency_ID: " + m_acctSchema.getC_Currency_ID()
                + " - DateAcct: " + m_doc.DateAcct + " - CurrencyRateType: "
        ...
        log4jFact
                .debug("********************* Fact - createLine - DocumentNo - "
                        + m_doc.DocumentNo
                        + " -  m_lines.size() - "
                        + m_lines.size());
        m_lines.add(line);
     ...
     * @return FactLine
     */
    public FactLine createLine(DocLine docLine, Account accountDr,
            Account accountCr, String C_Currency_ID, String Amt,
            String Fact_Acct_Group_ID, String SeqNo, String DocBaseType,
            ConnectionProvider conn) {
        ...
        if (m_Amt.compareTo(ZERO) < 0)
            return createLine(docLine, accountCr, C_Currency_ID, "", m_Amt
                    .abs().toString(), Fact_Acct_Group_ID, SeqNo, DocBaseType,
                    conn);
        else
            return createLine(docLine, accountDr, C_Currency_ID, m_Amt
                    .toString(), "", Fact_Acct_Group_ID, SeqNo, DocBaseType,
                    conn);
    } // createLine
     ...
     */
    public FactLine createLine(DocLine docLine, Account account,
            String C_Currency_ID, String Amt, String Fact_Acct_Group_ID,
            String SeqNo, String DocBaseType, ConnectionProvider conn) {
        BigDecimal m_Amt = ZERO;
        try {
            if (!Amt.equals(""))
                m_Amt = new BigDecimal(Amt);
        ...
        if (m_Amt.compareTo(ZERO) < 0)
            return createLine(docLine, account, C_Currency_ID, "", m_Amt.abs()
                    .toString(), Fact_Acct_Group_ID, SeqNo, DocBaseType, conn);
        else
            return createLine(docLine, account, C_Currency_ID, Amt.toString(),
     ...
     * enabled and not a multi-currency document (double check as otherwise the
     * rule should not have fired) If not balanced create balancing entry in
    ...
    public FactLine balanceSource(ConnectionProvider conn) {
        ...
        // new line
        FactLine line = new FactLine(m_doc.AD_Table_ID, m_doc.Record_ID, "",
                fl.m_Fact_Acct_Group_ID, fl.m_SeqNo, fl.m_DocBaseType);// antes
                                                                       // "0".
        ...
        if (diff.compareTo(ZERO) < 0) // negative balance => DR
            line.setAmtSource(m_doc.C_Currency_ID, diff.abs().toString(), ZERO
                    .toString());
        else
            // positive balance => CR
            line.setAmtSource(m_doc.C_Currency_ID, ZERO.toString(), diff
                    .toString());
        // Convert
        line.convert(m_acctSchema.getC_Currency_ID(), m_doc.DateAcct,
                m_acctSchema.getCurrencyRateType(), conn);
        ...
        log4jFact
                .debug("****************** fact - balancesource -  m_lines.size() - "
                        + m_lines.size());
        m_lines.add(line);
        return line;
    } // balancingSource
    ...
    public boolean isSegmentBalanced(ConnectionProvider conn) {
        ...
        for (int i = 0; i < size; i++) {
            ...
            if (ase.m_balanced.equals("Y")
                    && !isSegmentBalanced(ase.m_segmentType, conn))
                return false;
        }
        return true;
    } // isSegmentBalanced
     ...
     * 
     * @param segmentType
     *            - see AcctSchemaElement.SEGMENT_* Implemented only for Org
        ...
        if (segmentType.equals(AcctSchemaElement.SEGMENT_Org)) {
            ...
            while (values.hasNext()) {
                ...
                if (bal.compareTo(ZERO) != 0) {
                    map.clear();
                    log4jFact.warn("isSegmentBalanced (" + segmentType
                            + ") NO - " + toString() + ", Balance=" + bal);
                    return false;
                }
            }
            map.clear();
            log4jFact.debug("isSegmentBalanced (" + segmentType + ") - "
                    + toString());
            return true;
        }
        log4jFact.debug("isSegmentBalanced (" + segmentType
                + ") (not checked) - " + toString());
        return true;
    } // isSegmentBalanced

    /**
     * Balance all segments. - For all balancing segments - For all segment
     * values - If balance <> 0 create dueTo/dueFrom line overwriting the
     * segment value
     */
        ...
        if (segmentType.equals(AcctSchemaElement.SEGMENT_Org)) {
            ...
            while (keys.hasNext()) {
                ...
                if (diff.compareTo(ZERO) != 0) {
                    ...
                    FactLine fl = (FactLine) m_lines.get(0);
                    FactLine line = new FactLine(m_doc.AD_Table_ID,
                            m_doc.Record_ID, "", fl.m_Fact_Acct_Group_ID,
                    ...
                    // Amount & Account
                    if (diff.compareTo(ZERO) < 0) {
                        line.setAmtSource(m_doc.C_Currency_ID, diff.abs()
                                .toString(), ZERO.toString());
                        line.setAccount(m_acctSchema,
                                m_acctSchema.m_DueFrom_Acct);
                    } else {
                        line.setAmtSource(m_doc.C_Currency_ID, ZERO.toString(),
                                diff.abs().toString());
                        line
                                .setAccount(m_acctSchema,
                                        m_acctSchema.m_DueTo_Acct);
                    }
                    line.convert(m_acctSchema.getC_Currency_ID(),
                            m_doc.DateAcct, m_acctSchema.getCurrencyRateType(),
                            conn);
                    line.setAD_Org_ID(key.toString());
                    log4jFact.debug("balanceSegment (" + segmentType + ") - ");
                    log4jFact
                            .debug("************* fact - balanceSegment - m_lines.size() - "
                                    + m_lines.size()
                                    + " - line.ad_org_id - "
                                    + line.getAD_Org_ID(conn));
                    m_lines.add(line);
                }
            }
            map.clear();
    ...
    /**
     * Balance Accounting Currency. If the accounting currency is not balanced,
     * if Currency balancing is enabled create a new line using the currency
     * balancing account with zero source balance or adjust the line with the
     * largest balance sheet account or if no balance sheet account exist, the
     * line with the largest amount
     * 
     * @return FactLine
     */
    public FactLine balanceAccounting(ConnectionProvider conn) {
        ...
        if (m_acctSchema.isCurrencyBalancing()) {
            ...
            FactLine fl = (FactLine) m_lines.get(0);
            line = new FactLine(m_doc.AD_Table_ID, m_doc.Record_ID, "",
            ...
            // Amount
            line.setAmtSource(m_doc.C_Currency_ID, ZERO.toString(), ZERO
                    .toString());
            line.convert(m_acctSchema.getC_Currency_ID(), m_doc.DateAcct,
                    m_acctSchema.getCurrencyRateType(), conn);
            if (diff.compareTo(ZERO) < 0)
            ...
            line.setAccount(m_acctSchema, m_acctSchema
                    .getCurrencyBalancing_Acct());
            log4jFact.debug("balanceAccounting - " + line.toString());
            log4jFact
                    .debug("************* fact - balanceAccounting - m_lines.size() - "
                            + m_lines.size());
            m_lines.add(line);
            ...
            if (line == null)
                ...
                log4jFact.debug("Adjusting Amt=" + diff.toString() + "; Line="
                        + line.toString());
