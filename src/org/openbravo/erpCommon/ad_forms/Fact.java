...
public class Fact {
   ...
   */
  /*
   * public FactLine createLine(DocLine docLine, Account account, String C_Currency_ID, String
   * debitAmt, String creditAmt, String Fact_Acct_Group_ID, String SeqNo, String DocBaseType, String
   ...
   * m_acctSchema.m_C_AcctSchema_ID, DocBaseType); if (strNegate.equals("")) strNegate =
   * AcctServerData.selectDefaultNegate(conn, m_acctSchema.m_C_AcctSchema_ID); } catch
   * (ServletException e) { } if (strNegate.equals("")) strNegate = "Y"; BigDecimal DebitAmt = new
   * BigDecimal(debitAmt.equals("") ? "0.00" : debitAmt); BigDecimal CreditAmt = new
   * BigDecimal(creditAmt.equals("") ? "0.00" : creditAmt); if (DebitAmt.compareTo(BigDecimal.ZERO)
   * == 0 && CreditAmt.compareTo(BigDecimal.ZERO) == 0) { return null; } if (strNegate.equals("N")
   * && (DebitAmt.compareTo(ZERO) < 0 || CreditAmt.compareTo(ZERO) < 0)) { return
   * createLine(docLine, account, C_Currency_ID, CreditAmt.abs().toString(), DebitAmt.abs()
   * .toString(), Fact_Acct_Group_ID, SeqNo, DocBaseType, conn); }
   * 
   * log4jFact.debug("createLine - " + account + " - Dr=" + debitAmt + ", Cr=" + creditAmt);
   * log4jFact.debug("Starting createline"); // Data Check if (account == null) {
   * log4jFact.debug("end of create line"); m_doc.setStatus(AcctServer.STATUS_InvalidAccount);
   * return null; } // log4jFact.debug("createLine - Fact_Acct_Group_ID = " + Fact_Acct_Group_ID);
   ...
   * line.setAD_Org_ID(docLine.m_AD_Org_ID); log4jFact.debug("document info set");
   ...
   * " - creditAmt: " + creditAmt); // Amounts - one needs to be both not zero if
   * (!line.setAmtSource(C_Currency_ID, debitAmt, creditAmt)) return null; if (conversionDate ==
   ...
   * conversionDate + " - CurrencyRateType: " + m_acctSchema.getCurrencyRateType()); // Convert if
   * (conversionRate != null) { line.convertByRate(m_acctSchema.getC_Currency_ID(), conversionRate);
   * } else { line.convert(m_acctSchema.getC_Currency_ID(), conversionDate,
   * m_acctSchema.getCurrencyRateType(), conn); } // Optionally overwrite Acct Amount if (docLine !=
   * null && !docLine.m_AmtAcctDr.equals("") && !docLine.m_AmtAcctCr.equals(""))
   * line.setAmtAcct(docLine.m_AmtAcctDr, docLine.m_AmtAcctCr); // Info
   * line.setJournalInfo(m_doc.GL_Category_ID); line.setPostingType(m_postingType); // Set Info
   * line.setDocumentInfo(m_doc, docLine); // log4jFact.debug("createLine - " + m_doc.DocumentNo);
   * log4jFact.debug("********************* Fact - createLine - DocumentNo - " + m_doc.DocumentNo +
   * " -  m_lines.size() - " + m_lines.size()); m_lines.add(line); return line; } // createLine
   */
  /**
   * Create and convert Fact Line using a specified conversion date. Used to create a DR and/or CR
   * entry
  ...
  public FactLine createLine(DocLine docLine, Account account, String C_Currency_ID,
      String debitAmt, String creditAmt, String Fact_Acct_Group_ID, String SeqNo,
      String DocBaseType, String conversionDate, BigDecimal conversionRate, String AD_Table_ID,
