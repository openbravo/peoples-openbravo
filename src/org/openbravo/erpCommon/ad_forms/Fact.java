...
public  class Fact {
  static Logger log4jFact = Logger.getLogger(Fact.class);
    ...
    public FactLine createLine (DocLine docLine, Account account,
        String C_Currency_ID, String debitAmt, String creditAmt, String Fact_Acct_Group_ID, String SeqNo, String DocBaseType,ConnectionProvider conn){
        ...
        log4jFact.debug("C_Currency_ID: " + C_Currency_ID + " - debitAmt: " + debitAmt + " - creditAmt: " + creditAmt);
        //  Amounts - one needs to be both not zero
        if (!line.setAmtSource(C_Currency_ID, debitAmt, creditAmt))
            return null;
        log4jFact.debug("C_Currency_ID: " + m_acctSchema.getC_Currency_ID() + " - DateAcct: " + m_doc.DateAcct + " - CurrencyRateType: " + m_acctSchema.getCurrencyRateType());
        //  Convert
        ...
        log4jFact.debug("********************* Fact - createLine - DocumentNo - " + m_doc.DocumentNo + " -  m_lines.size() - " + m_lines.size());
        m_lines.add(line);
        return line;
    }   //  createLine
    ...
    public FactLine createLine (DocLine docLine, Account accountDr, Account accountCr,String C_Currency_ID, String Amt, String Fact_Acct_Group_ID, String SeqNo, String DocBaseType, ConnectionProvider conn){
        ...
        if (m_Amt.compareTo(ZERO) < 0)
            ...
            return createLine (docLine, accountDr, C_Currency_ID, m_Amt.toString(), "", Fact_Acct_Group_ID, SeqNo, DocBaseType,conn);
    }   //  createLine
    ...
    public FactLine createLine (DocLine docLine, Account account,String C_Currency_ID, String Amt, String Fact_Acct_Group_ID, String SeqNo, String DocBaseType, ConnectionProvider conn){
        ...
        if (m_Amt.compareTo(ZERO) < 0)
            ...
            return createLine (docLine, account, C_Currency_ID, Amt.toString(), "", Fact_Acct_Group_ID, SeqNo, DocBaseType,conn);
    }   //  createLine
        ...
        if (segmentType.equals(AcctSchemaElement.SEGMENT_Org)){
            ...
            for (int i = 0; i < m_lines.size(); i++){
                ...
                if (oldBal != null)
                    bal = bal.add(oldBal);
                map.put(key, bal);
            //  log4jFact.debug("Add Key=" + key + ", Bal=" + bal + " <- " + line);
            }
    ...
    public void balanceSegments(ConnectionProvider conn){
        ...
        for (int i = 0; i < size; i++){
            AcctSchemaElement ase = (AcctSchemaElement)elementList.get(i);
            if (ase.m_balanced.equals("Y")) balanceSegment (ase.m_segmentType, conn);
        }
    }   //  balanceSegments
        ...
        if (segmentType.equals(AcctSchemaElement.SEGMENT_Org)){
            ...
            while (keys.hasNext()){
                ...
                if (diff.compareTo(ZERO) != 0){
                    ...
                    log4jFact .debug ("balanceSegment (" + segmentType + ") - ");
                    log4jFact.debug("************* fact - balanceSegment - m_lines.size() - " + m_lines.size() + " - line.ad_org_id - " + line.getAD_Org_ID(conn));
                    m_lines.add(line);
                }
            }
            map.clear();
    ...
    public FactLine balanceAccounting(ConnectionProvider conn){
        ...
        if (m_acctSchema.isCurrencyBalancing()){
            ...
            line.convert(m_acctSchema.getC_Currency_ID(), m_doc.DateAcct, m_acctSchema.getCurrencyRateType(),conn);
            if (diff.compareTo(ZERO) < 0)   line.setAmtAcct(diff.abs().toString(), ZERO.toString());
            else    line.setAmtAcct(ZERO.toString(), diff.abs().toString());
            ...
            if (line == null)   log4jFact.error ("balanceAccounting - No Line found");
            else{
