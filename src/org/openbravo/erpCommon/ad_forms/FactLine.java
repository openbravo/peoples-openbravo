...
public class FactLine {
     ...
     */
    public boolean setAmtSource(String C_Currency_ID, String AmtSourceDr,
            String AmtSourceCr) {
     ...
     */
    public boolean convert(String Acct_Currency_ID, String ConversionDate,
            String CurrencyRateType, ConnectionProvider conn) {
        ...
        if (m_C_Currency_ID == null
                || m_C_Currency_ID.equals(AcctServer.NO_CURRENCY))
        ...
        m_AmtAcctDr = AcctServer
                .getConvertedAmt(m_AmtSourceDr, m_C_Currency_ID,
                        Acct_Currency_ID, ConversionDate, CurrencyRateType,
                        m_docVO.AD_Client_ID, m_docVO.AD_Org_ID, conn);
        if (m_AmtAcctDr == null || m_AmtAcctDr.equals(""))
            return false;
        m_AmtAcctCr = AcctServer
                .getConvertedAmt(m_AmtSourceCr, m_C_Currency_ID,
                        Acct_Currency_ID, ConversionDate, CurrencyRateType,
                        m_docVO.AD_Client_ID, m_docVO.AD_Org_ID, conn);
        return true;
    } // convert
     ...
     *            from
     */
    public void setLocationFromOrg(String AD_Org_ID, boolean isFrom,
    ...
    /**
     * Get AD_Org_ID (balancing segment). (if not set directly - from document
     * line, document, account, locator)
     * <p>
     * Note that Locator needs to be set before - otherwise segment balancing
    ...
    public String getAD_Org_ID(ConnectionProvider conn) {
        ...
        if (m_M_Locator_ID != null && !m_M_Locator_ID.equals("")) {
            FactLineData[] data = null;
            try {
                data = FactLineData.selectOrg(conn, m_M_Locator_ID,
                        m_docVO.AD_Client_ID);
            } catch (ServletException e) {
                log4jFactLine.warn(e);
            }
            if (data != null && data.length > 0) {
                m_AD_Org_ID = data[0].org;
                log4jFactLine.debug("setAD_Org_ID=" + m_AD_Org_ID
                        + " (1 from M_Locator_ID=" + m_M_Locator_ID + ")");
            } else
                log4jFactLine.warn("getAD_Org_ID - Did not find M_Locator_ID="
                        + m_M_Locator_ID);
        } // M_Locator_ID != 0

        // Prio 2 - get from doc line - if exists (document context overwrites)
        if ((m_AD_Org_ID == null || m_AD_Org_ID.equals("") || m_AD_Org_ID
                .equals("0"))
                && m_docLine != null) {
    ...
    public void setM_Locator_ID(String M_Locator_ID) {
        ...
        if (!m_AD_Org_ID.equals(""))
            log4jFactLine
                    .warn("setM_Locator_ID - Organization already calculated");
    } // setM_Locator_ID
    ...
    public void setLocationFromBPartner(String C_BPartner_Location_ID,
            boolean isFrom, ConnectionProvider conn) {
     ...
     *            from
     */
    public void setLocationFromLocator(String M_Locator_ID, boolean isFrom,
            ConnectionProvider conn) {
    ...
    /**
     * Save to Disk. Get Info from this, doc-line, document, account Optionally
     * create Revenue Recognition Plan
     * 
     * @param con
     *            connection
     * @return true if saved
     */
    public boolean save(Connection con, ConnectionProvider conn,
        ...
        log4jFactLine.debug("FactLine - save - before Record_ID2 "
                + m_Record_ID);
        String Record_ID2 = "";
        if (m_docLine != null)
            Record_ID2 = m_docLine.m_Record_Id2;
        log4jFactLine.debug("FactLine - save - after Record_ID2 = "
                + Record_ID2);
        String C_Period_ID = "";
        if (m_docLine != null)
        ...
        description = getDescription(conn, C_BPartner_ID, m_C_AcctSchema_ID,
        ...
        try {
             ...
             * Save to DB
             */
            log4jFactLine.debug("FactLine - save - m_Record_ID = "
                    + m_Record_ID + " - Account_ID = " + Account_ID
                    + " - m_Fact_Acct_Group_ID = " + m_Fact_Acct_Group_ID
                    + " - m_SeqNo = " + m_SeqNo);
            FactLineData[] cuenta = FactLineData.selectAccountValue(conn,
                    Account_ID);
            log4jFactLine
                    .debug("FactLine - After selectAccountValue - cuenta.length - "
                            + cuenta.length);
            BigDecimal zero = new BigDecimal("0.0");
            log4jFactLine.debug("FactLine - m_AmtSourceDr " + m_AmtSourceDr
                    + " - m_AmtSourceDr " + m_AmtSourceDr);
            log4jFactLine.debug("FactLine - m_AmtAcctDr " + m_AmtAcctDr
                    + " - m_AmtSourceDr " + m_AmtAcctCr);
            if (zero.compareTo(new BigDecimal(m_AmtSourceDr)) == 0
                ...
                log4jFactLine.debug("FactLine - m_Fact_Acct_ID "
                        + m_Fact_Acct_ID + " - AD_Client_ID " + AD_Client_ID);
                log4jFactLine.debug("FactLine - m_C_AcctSchema_ID "
                        + m_C_AcctSchema_ID + " - Account_ID " + Account_ID);
                log4jFactLine.debug("FactLine - cuenta[0].value "
                        + cuenta[0].value + " - cuenta[0].description "
                        + cuenta[0].description);
                log4jFactLine.debug("FactLine - DateDoc " + DateDoc
                        + " - DateAcct " + DateAcct);
                log4jFactLine.debug("FactLine - C_Period_ID " + C_Period_ID
                        + " - m_AD_Table_ID " + m_AD_Table_ID);
                log4jFactLine.debug("FactLine - m_Record_ID " + m_Record_ID
                        + " - m_Line_ID " + m_Line_ID);
                log4jFactLine.debug("FactLine - m_GL_Category_ID "
                        + m_GL_Category_ID);
                log4jFactLine.debug("FactLine - C_Tax_ID " + C_Tax_ID
                        + " - m_PostingType " + m_PostingType);
                log4jFactLine
                        .debug("FactLine - m_C_Currency_ID " + m_C_Currency_ID
                                + " - m_AmtSourceDr " + m_AmtSourceDr);
                log4jFactLine.debug("FactLine - m_AmtSourceCr " + m_AmtSourceCr
                        + " - m_AmtAcctDr " + m_AmtAcctDr);
                log4jFactLine.debug("FactLine - m_AmtAcctCr " + m_AmtAcctCr
                        + " - C_UOM_ID " + C_UOM_ID);
                log4jFactLine.debug("FactLine - Qty " + Qty
                        + " - m_M_Locator_ID " + m_M_Locator_ID);
                log4jFactLine.debug("FactLine - M_Product_ID " + M_Product_ID
                        + " - C_BPartner_ID " + C_BPartner_ID);
                log4jFactLine.debug("FactLine - AD_OrgTrx_ID " + AD_OrgTrx_ID
                        + " - C_LocFrom_ID " + C_LocFrom_ID);
                log4jFactLine.debug("FactLine - C_LocTo_ID " + C_LocTo_ID
                        + " - C_SalesRegion_ID " + C_SalesRegion_ID);
                log4jFactLine.debug("FactLine - C_Project_ID " + C_Project_ID
                        + " - C_Campaign_ID " + C_Campaign_ID);
                log4jFactLine.debug("FactLine - C_Activity_ID " + C_Activity_ID
                        + " - User1_ID " + User1_ID);
                log4jFactLine.debug("FactLine - User2_ID " + User2_ID
                        + " - description " + description.toString());
                log4jFactLine.debug("FactLine - m_Fact_Acct_Group_ID "
                        + m_Fact_Acct_Group_ID + " - m_SeqNo " + m_SeqNo);
                ...
                no = FactLineData.insertFactAct(con, conn, m_Fact_Acct_ID,
                        AD_Client_ID, AD_Org_ID, m_C_AcctSchema_ID, Account_ID,
                        cuenta[0].value, cuenta[0].description, DateDoc,
                        DateAcct, C_Period_ID, m_AD_Table_ID, m_Record_ID,
                        m_Line_ID, m_GL_Category_ID, C_Tax_ID, m_PostingType,
                        m_C_Currency_ID, m_AmtSourceDr, m_AmtSourceCr,
                        m_AmtAcctDr, m_AmtAcctCr, C_UOM_ID, Qty,
                        m_M_Locator_ID, M_Product_ID, C_BPartner_ID,
                        AD_OrgTrx_ID, C_LocFrom_ID, C_LocTo_ID,
                        C_SalesRegion_ID, C_Project_ID, C_Campaign_ID,
                        C_Activity_ID, User1_ID, User2_ID, description
                                .toString(), m_Fact_Acct_Group_ID, m_SeqNo,
                        m_DocBaseType, Record_ID2,
                        (m_docLine != null) ? m_docLine.m_A_Asset_ID : "");
                log4jFactLine.debug("FactLine - After insertFactAct");
            }
            if (m_docVO.m_IsOpening.equals("Y"))
                FactLineData.updateFactAcct(con, conn, m_AD_Table_ID,
                        m_Record_ID);
        } catch (ServletException e) {
            log4jFactLine.warn(e);
        }
    ...
    private String getC_SalesRegion_ID(ConnectionProvider conn) {
        ...
        if (C_SalesRegion_ID == null)
            C_SalesRegion_ID = "";
        if (C_SalesRegion_ID.equals("")
                && !m_docVO.BP_C_SalesRegion_ID.equals(""))
            C_SalesRegion_ID = m_docVO.BP_C_SalesRegion_ID;
        if (C_SalesRegion_ID == null)
            C_SalesRegion_ID = "";
        // derive SalesRegion if AcctSegment
        if (C_SalesRegion_ID.equals("")
                && !m_docVO.C_BPartner_Location_ID.equals("")
                && m_docVO.BP_C_SalesRegion_ID.equals("")// never tried
                && m_acctSchema
                        .isAcctSchemaElement(AcctSchemaElement.SEGMENT_SalesRegion)) {
            FactLineData[] data = null;
            try {
            ...
            log4jFactLine.debug("getC_SalesRegion_ID=" + C_SalesRegion_ID
                    + " (from BPL)");
        }
        if (C_SalesRegion_ID == null || C_SalesRegion_ID.equals(""))
    ...
    public void currencyCorrect(BigDecimal deltaAmount) {
        ...
        log4jFactLine.debug("currencyCorrect - New-AcctDr=" + m_AmtAcctDr
                + ",AcctCr=" + m_AmtAcctCr);
    } // currencyCorrect

    public String setC_Period_ID(AcctServer m_docVO, String strDateAcct,
            ConnectionProvider conn) {
    ...
    public StringBuffer getDescription(ConnectionProvider connectionProvider,
            String strC_Bpartner_ID, String strC_AcctSchema_ID,
            String strAD_Table_ID, String strRecord_ID, String strLine)
            throws ServletException {
        StringBuffer description = new StringBuffer();
        String strSql = AcctServerData.selectDescription(connectionProvider,
                strAD_Table_ID, strC_AcctSchema_ID);
        try {
            if (!strSql.equals("")/* && strLine!=null && !strLine.equals("") */) {
                ...
                try {
                    ...
                    log4jFactLine.error("SQL error in query: " + strSql
                            + "Exception:" + e);
                    throw new ServletException(Integer.toString(e
                            .getErrorCode()));
                } finally {
                    try {
            ...
            if (description.length() == 0) {
                ...
                if (!strC_Bpartner_ID.equals(""))
                    description.append(" # ").append(
                            AcctServerData.selectBpartnerName(
                                    connectionProvider, strC_Bpartner_ID));
                // ... line
                if (m_docLine != null) {
                    if (m_docLine.m_Line != null
                            && !m_docLine.m_Line.equals(""))
                        description.append(" # ").append(m_docLine.m_Line);
                    if (m_docLine.m_description != null
                            && !m_docLine.m_description.equals(""))
                        description.append(" (")
                                .append(m_docLine.m_description).append(")");
                }
