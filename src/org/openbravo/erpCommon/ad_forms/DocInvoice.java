...
public class DocInvoice extends AcctServer {
     ...
     *            AD_Client_ID
     */
    public DocInvoice(String AD_Client_ID, String AD_Org_ID,
    ...
    public void loadObjectFieldProvider(ConnectionProvider conn,
        ...
        setObjectFieldProvider(DocInvoiceData.selectRegistro(conn,
                AD_Client_ID, Id));
    }
    ...
    private DocLine[] loadLines() {
        ...
        try {
            ...
            if (groupLines.equals("Y"))
                data = DocLineInvoiceData.selectTotal(connectionProvider,
                        Record_ID);
            else
        ...
        for (int i = 0; i < data.length; i++) {
            ...
            DocLine_Invoice docLine = new DocLine_Invoice(DocumentType,
                    Record_ID, Line_ID);
    ...
    public Fact createFact(AcctSchema as, ConnectionProvider conn,
        ...
        if (DocumentType.equals(AcctServer.DOCTYPE_ARInvoice)
            ...
            fact.createLine(null, getAccount(AcctServer.ACCTTYPE_Charge, as,
                    conn), C_Currency_ID, "",
                    getAmount(AcctServer.AMTTYPE_Charge), Fact_Acct_Group_ID,
                    nextSeqNo(SeqNo), DocumentType, conn);
            // TaxDue CR
            log4jDocInvoice.debug("m_taxes.length: " + m_taxes);
            for (int i = 0; m_taxes != null && i < m_taxes.length; i++) {
                fact.createLine(null, m_taxes[i].getAccount(
                        DocTax.ACCTTYPE_TaxDue, as, conn), C_Currency_ID, "",
                        m_taxes[i].m_amount, Fact_Acct_Group_ID,
                        nextSeqNo(SeqNo), DocumentType, conn);
            }
            // Revenue CR
            if (p_lines != null && p_lines.length > 0) {
                for (int i = 0; i < p_lines.length; i++)
                    fact.createLine(p_lines[i], ((DocLine_Invoice) p_lines[i])
                            .getAccount(ProductInfo.ACCTTYPE_P_Revenue, as,
                                    conn), this.C_Currency_ID, "", p_lines[i]
                            .getAmount(), Fact_Acct_Group_ID, nextSeqNo(SeqNo),
                            DocumentType, conn);
            }
            // Set Locations
            ...
            for (int i = 0; i < fLines.length; i++) {
                if (fLines[i] != null) {
                    fLines[i].setLocationFromOrg(fLines[i].getAD_Org_ID(conn),
                            true, conn); // from Loc
                    fLines[i].setLocationFromBPartner(C_BPartner_Location_ID,
                            false, conn); // to Loc
                }
            }
        }
        // ARC
            ...
            for (int i = 0; m_taxes != null && i < m_taxes.length; i++)
                fact.createLine(null, m_taxes[i].getAccount(
                        DocTax.ACCTTYPE_TaxDue, as, conn), this.C_Currency_ID,
                        m_taxes[i].getAmount(), "", Fact_Acct_Group_ID,
                        nextSeqNo(SeqNo), DocumentType, conn);
            // Revenue CR
            for (int i = 0; p_lines != null && i < p_lines.length; i++)
                fact.createLine(p_lines[i], ((DocLine_Invoice) p_lines[i])
                        .getAccount(ProductInfo.ACCTTYPE_P_Revenue, as, conn),
                        this.C_Currency_ID, p_lines[i].getAmount(), "",
                        Fact_Acct_Group_ID, nextSeqNo(SeqNo), DocumentType,
                        conn);
            // Set Locations
            FactLine[] fLines = fact.getLines();
        ...
        // API
            ...
            for (int i = 0; m_taxes != null && i < m_taxes.length; i++)
                fact.createLine(null, m_taxes[i].getAccount(
                        DocTax.ACCTTYPE_TaxCredit, as, conn),
        ...
        // APC
            ...
            log4jDocInvoice.warn("Doc_Invoice - DocumentType unknown: "
                    + this.DocumentType);
            fact = null;
        }
        SeqNo = "0";
    ...
    private void updateProductInfo(String C_AcctSchema_ID,
        ...
        log4jDocInvoice.debug("updateProductInfo - C_Invoice_ID="
                + this.Record_ID);
    ...
    public BigDecimal getBalance() {
        ...
        for (int i = 0; i < m_taxes.length; i++) {
            retValue = retValue
                    .subtract(new BigDecimal(m_taxes[i].getAmount()));
    ...
    public final Account getAccountBPartner(String cBPartnerId, AcctSchema as,
            boolean isReceipt, String dpStatus, ConnectionProvider conn) {
        DocPaymentData[] data = null;
        try {
            if (log4j.isDebugEnabled())
                log4j.debug("DocInvoice - getAccountBPartner - DocumentType = "
                        + DocumentType);
            if (isReceipt) {
                data = DocPaymentData.selectBPartnerCustomerAcct(conn,
                        cBPartnerId, as.getC_AcctSchema_ID(), dpStatus);
            } else {
                data = DocPaymentData.selectBPartnerVendorAcct(conn,
                        cBPartnerId, as.getC_AcctSchema_ID(), dpStatus);
            }
        } catch (ServletException e) {
            log4j.warn(e);
        ...
        if (Account_ID.equals("")) {
            log4j.warn("DocInvoice - getAccountBPartner - NO account BPartner="
                    + cBPartnerId + ", Record=" + Record_ID + ", status "
                    + dpStatus);
            return null;
        }
        // Return Account
