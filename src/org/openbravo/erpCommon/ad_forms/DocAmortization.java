...
public class DocAmortization extends AcctServer {
    ...
    static Logger log4jDocAmortization = Logger
            .getLogger(DocAmortization.class);
    ...
    public void loadObjectFieldProvider(ConnectionProvider conn,
        ...
        setObjectFieldProvider(DocAmortizationData.selectRegistro(conn,
                AD_Client_ID, Id));
    }
    ...
    public boolean loadDocumentDetails(FieldProvider[] data,
        ...
        log4jDocAmortization.debug("Record_ID = " + Record_ID + " - Lines="
                + p_lines.length);
        return false;
    } // loadDocumentDetails
    ...
    private DocLine[] loadLines(ConnectionProvider conn) {
        ...
        if (p_lines != null) {
            ...
            log4jDocAmortization.debug("Record_ID = " + Record_ID
                    + " - Lines= **p_lines is null**");
        }
        //
        for (int i = 0; data != null && i < data.length; i++) {
            String Line_ID = data[i].getField("A_AMORTIZATIONLINE_ID");
            DocLine_Amortization docLine = new DocLine_Amortization(
    ...
    public Fact createFact(AcctSchema as, ConnectionProvider conn,
        ...
        log4jDocAmortization.debug("createFact - p_lines.length - "
                + p_lines.length);
        // Lines
        fact = new Fact(this, as, Fact.POST_Actual);
        for (int i = 0; p_lines != null && i < p_lines.length; i++) {
            DocLine_Amortization line = (DocLine_Amortization) p_lines[i];
            fact.createLine(line, getAccount(ACCTTYPE_Depreciation,
                    line.m_A_Asset_ID, as, conn), line.m_C_Currency_ID,
                    line.Amount, "", Fact_Acct_Group_ID, nextSeqNo(SeqNo),
                    DocumentType, conn);
            fact.createLine(line, getAccount(ACCTTYPE_AccumDepreciation,
                    line.m_A_Asset_ID, as, conn), line.m_C_Currency_ID, "",
                    line.Amount, Fact_Acct_Group_ID, nextSeqNo(SeqNo),
                    DocumentType, conn);
        }
     ...
     */
    public Account getAccount(String AcctType, String A_Asset_ID,
        ...
        Account acc = null;
        try {
            data = DocAmortizationData.selectAssetAcct(conn, A_Asset_ID, as
                    .getC_AcctSchema_ID());
