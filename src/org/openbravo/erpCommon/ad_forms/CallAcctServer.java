...
//      printPageAjax(response,vars, strMessage);
      ...
      String strTableId = vars.getStringParameter("inpadTableId");
      String strAdOrgId = vars.getStringParameter("inpadOrgId");
      if (strAdOrgId==null || strAdOrgId.equals("")) strAdOrgId="0";
      if (log4j.isDebugEnabled()) log4j.debug(strTableId);
      runProcess(response, vars, strTableId, strAdOrgId, adProcessId);
    } else pageError(response);
  }

  private void printPage(HttpServletResponse response, VariablesSecureApp vars, String strTableId, String strOrgId, String strMessage) throws IOException, ServletException{
    ...
    if (log4j.isDebugEnabled()) log4j.debug("select organizations:"+Utility.getContext(this, vars, "#User_Org", "")+", Client:"+Utility.getContext(this, vars, "#User_Client", "")); 
    if (log4j.isDebugEnabled()) log4j.debug("lenght:"+data.length);
  ...
  private void runProcess(HttpServletResponse response, VariablesSecureApp vars, String strTableId, String strOrgId, String adProcessId) throws IOException, ServletException {
  ...
  private void runProcess(HttpServletResponse response, VariablesSecureApp vars, String strTableId, String strOrgId, String adProcessId) throws IOException, ServletException {
	  ...
	  try {
			String adPinstanceId = SequenceIdData.getUUID();
		  PInstanceProcessData.insertPInstance(this, adPinstanceId, adProcessId, adOrgId, "N", vars.getUser(), vars.getClient(), vars.getOrg());
		  PInstanceProcessData.insertPInstanceParam(this, adPinstanceId, "10", "AD_Table_ID", strTableId, vars.getClient(), vars.getOrg(), vars.getUser());
		  PInstanceProcessData.insertPInstanceParam(this, adPinstanceId, "20", "AD_Org_ID", strOrgId, vars.getClient(), vars.getOrg(), vars.getUser());
