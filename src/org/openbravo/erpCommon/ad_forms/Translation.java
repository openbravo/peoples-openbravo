 ...
 * 
 * Example: /opt/attachments/ en_US/ <trl tables from core> module1/ <trl tables
 * from module1>
 * 
 */
public class Translation extends HttpSecureAppServlet {
    ...
    public void doPost(HttpServletRequest request, HttpServletResponse response)
        ...
        if (vars.commandIn("DEFAULT")) {
            ...
            final String strLang = vars.getRequestGlobalVariable("language",
                    "translation.lang");
            final String strClient = vars.getRequestGlobalVariable("client",
                    "translation.client");
            ...
            final String directory = globalParameters.strFTPDirectory
                    + "/lang/" + strLang + "/";
            final OBError myMessage = importTrlDirectory(directory, strLang,
                    strClient, vars);
            if (log4j.isDebugEnabled())
                log4j.debug("message:" + myMessage.getMessage());
            vars.setMessage("Translation", myMessage);
     ...
     * or trl tables that refers to tables with a parent table with ad_module_id
     * column
     * 
     * For example: If a record from ad_process is in module "core", the records
     * from ad_process_trl and ad_process_para_trl are exported in "core" module
     * 
     * @param strLang
     *            Language to export.
     * @param strClient
     ...
     */
    private OBError exportTrl(String strLang, String strClient,
        ...
        if (new File(strFTPDirectory).canWrite()) {
            ...
            myMessage.setMessage(Utility.messageBD(this,
                    "CannotWriteDirectory", vars.getLanguage())
                    + " " + strFTPDirectory);
            return myMessage;
        }
        ...
        try {
            final TranslationData[] modulesTables = TranslationData
                    .trlModulesTables(this);

            for (int i = 0; i < modulesTables.length; i++) {
                exportModulesTrl(rootDirectory, AD_Client_ID, AD_Language,
                        modulesTables[i].c);
            }
            ...
            myMessage.setType("Error");
            myMessage.setMessage(Utility.messageBD(this, "Error", vars
                    .getLanguage()));
            return myMessage;
        }
    ...
    /**
     * 
     * The import process insert in database all the translations found in the
     * folder of the defined language RECURSIVELY. It don't take into account if
     * a module is marked o no as isInDevelopment. Only search for trl's xml
     * files corresponding with trl's tables in database.
     * 
     * 
     * @param directory
     ...
     * @return Message with the error or with the success
     */
    public static OBError importTrlDirectory(String directory, String strLang,
            String strClient, VariablesSecureApp vars) {
        final String AD_Language = strLang;
        ...
        for (int f = 0; f < list.length; f++) {
            if (list[f].isDirectory()) {
                final OBError subDirError = importTrlDirectory(list[f]
                        .toString()
                        + "/", strLang, strClient, vars);
    ...
    private void exportContibutors(String directory, String AD_Language) {
        final File out = new File(directory, CONTRIBUTORS_FILENAME + "_"
                + AD_Language + ".xml");
        try {
    ...
    private void exportReferenceData(String rootDirectory, String AD_Language) {
        try {
            ...
            final TranslationData[] referenceTrlData = TranslationData
                    .referenceDataTrl(this);
    ...
    private String exportModulesTrl(String rootDirectory, int AD_Client_ID,
     ...
     * @param exportAll
     *            In case it is reference data if it should be exported all data
     *            or just imported
     * @param table
     *            Base table
     * @param tableID
    ...
    private void exportTable(String AD_Language, boolean exportReferenceData,
        ...
        try {
            ...
            for (int i = 0; i < trlColumns.length; i++) {
                sql.append(", t.").append(trlColumns[i].c).append(",o.")
                        .append(trlColumns[i].c).append(" AS ").append(
                                trlColumns[i].c).append("O");
            }

            sql.append(" FROM ").append(trlTable).append(" t").append(", ")
                    .append(table).append(" o");
            ...
            /*
             * AdClient !=0 not supported if (AD_Client_ID >= 0){
             * sql.append(" AND "
            ...
            if (!exportReferenceData) {
                ...
                final TranslationData[] parentTable = TranslationData
                        .parentTable(this, trlTable);
                if (parentTable.length > 0) {
                    strParentTable = parentTable[0].tablename;
                }
                if (strParentTable == null) {
                    sql.append(" AND ").append(" o.ad_module_id='").append(
                            moduleId).append("'");
                } else {
                    /** Search for ad_module_id in the parent table */
                    sql.append(" AND ");
                    sql.append(" exists ( select 1 from ").append(
                            strParentTable).append(" p ");
                    sql.append("   where p.").append(strParentTable + "_ID")
                            .append("=").append("o." + strParentTable + "_ID");
                    sql.append("   and p.ad_module_id='").append(moduleId)
                            .append("')");
                }
            }
            if (exportReferenceData && !exportAll) {
                sql.append(" AND DL.GENERIC_ID = o.").append(keyColumn).append(
                        " AND DL.AD_TABLE_ID = '").append(tableID).append("'")
                        .append(" AND DL.AD_MODULE_ID = '").append(moduleId)
                        .append("'");
            }
            ...
            if (!exportReferenceData) { // when not exporting rd create always
                ...
                root.setAttribute(XML_ATTRIBUTE_VERSION, TranslationData
                        .version(this));
                document.appendChild(root);
                String directory = "";
                ...
                final String fileName = directory + trlTable + "_"
                        + AD_Language + ".xml";
            ...
            while (rs.next()) {
                if (!hasRows && !exportReferenceData) { // Create file only in
                    ...
                    root.setAttribute(XML_ATTRIBUTE_LANGUAGE, AD_Language);
                    root.setAttribute(XML_ATTRIBUTE_TABLE, table);
                    root.setAttribute(XML_ATTRIBUTE_BASE_LANGUAGE,
                            moduleLanguage);
                    root.setAttribute(XML_ATTRIBUTE_VERSION, TranslationData
                            .version(this));
                    document.appendChild(root);
                ...
                row.setAttribute(XML_ROW_ATTRIBUTE_ID, String.valueOf(rs
                        .getString(2))); // KeyColumn
                ...
                for (int i = 0; i < trlColumns.length; i++) {
                    ...
                    if (valueString == null)
                        valueString = "";
                    value
                            .setAttribute(XML_VALUE_ATTRIBUTE_ORIGINAL,
                                    origString);
            ...
            log4j.info("exportTrl - Records=" + rows + ", DTD="
                    + document.getDoctype());
            ...
            final OutputStreamWriter osw = new OutputStreamWriter(
                    new FileOutputStream(out));// , "ISO-8859-1");
    ...
    private static String importContributors(String directory,
            String AD_Language) {
        final String fileName = directory + File.separator
                + CONTRIBUTORS_FILENAME + "_" + AD_Language + ".xml";
    ...
    private static String importTrlFile(String directory, int AD_Client_ID,
            String AD_Language, String Trl_Table) {
        final String fileName = directory + File.separator + Trl_Table + "_"
                + AD_Language + ".xml";
        ...
        try {
            ...
            final SAXParser parser = factory.newSAXParser();
            parser.parse(in, handler);
            translationlog4j.info("importTrl - Updated="
                    + handler.getUpdateCount() + " - from file " + fileName);
    ...
    void printPageDataSheet(HttpServletResponse response,
        ...
        final ToolBar toolbar = new ToolBar(this, vars.getLanguage(),
                "Translation", false, "", "", "", false, "ad_forms",
        ...
        }
        {
            ...
            xmlDocument.setParameter("paramLanguage", "defaultLang=\""
                    + vars.getLanguage() + "\";");
            xmlDocument.setParameter("paramSelLanguage", vars
                    .getSessionValue("translation.lang"));
            xmlDocument.setParameter("paramSystem", vars
                    .getSessionValue("translation.client"));
            xmlDocument.setData("structure1", LanguageComboData.select(this));

            xmlDocument.setData("structureClient", ClientComboData
