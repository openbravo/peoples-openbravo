...
public class Task extends Thread {
  ...
  public Task(String cmd) {
    ...
    m_outReader = new Thread() {
      public void run() {
        if (log4j.isDebugEnabled()) log4j.debug("Task.outReader.run");
        try {
          int c;
        ...
        if (log4j.isDebugEnabled()) log4j.debug("Task.outReader.run - done");
      }
    };
    m_errReader = new Thread() {
      public void run() {
        if (log4j.isDebugEnabled()) log4j.debug("Task.errReader.run");
        try {
          int c;
        ...
        if (log4j.isDebugEnabled()) log4j.debug("Task.errReader.run - done");
      }
    };
    m_cmd = cmd;
  }
  ...
  public void run() {
    if (log4j.isDebugEnabled()) log4j.debug("Task.run");
    try {
    ...
    try {
      if(m_child != null) if (log4j.isDebugEnabled()) log4j.debug("Task.run - ExitValue=" + m_child.exitValue());
    } catch(Exception e) { }
    if (log4j.isDebugEnabled()) log4j.debug("Task.run - done");
    return;
  }

  private boolean checkInterrupted() {
    if(isInterrupted()) {
      if (log4j.isDebugEnabled()) log4j.debug("Task.checkInterrupted - true");
