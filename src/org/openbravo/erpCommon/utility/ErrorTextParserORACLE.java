 ...
 * the error parsing for ORACLE RDBMS.
 */
public class ErrorTextParserORACLE extends ErrorTextParser {
  ...
  public OBError parse() throws Exception {
    ...
    if (pos!=-1) {
      ...
      try {
        errorCode = Integer.valueOf(myMessage.substring(pos+4, myMessage.indexOf(":", pos+4))).intValue();
        errorCodeText = myMessage.substring(pos, myMessage.indexOf(":", pos));
      } catch (Exception ignored) {
        errorCode = 0;
        errorCodeText = "";
      }
      if (log4j.isDebugEnabled()) log4j.debug("Error code: " + Integer.toString(errorCode));
      if (errorCode!=0) {
        FieldProvider fldMessage = Utility.locateMessage(getConnection(), Integer.toString(errorCode), getLanguage());
        if (fldMessage!=null) {
          ...
          String messageAux = Utility.parseTranslation(getConnection(), getVars(), getLanguage(), myMessage.replace(errorCodeText + ": ", ""));
          if (log4j.isDebugEnabled()) log4j.debug("Message parsed: " + messageAux);
      ...
      if (pos != -1) {
        ...
        ErrorTextParserORACLEData[] constraintData = ErrorTextParserORACLEData.select(getConnection(), objectName);
        ...
        if (constraintData!=null && constraintData.length>0) {
          ...
          FieldProvider fldMessage = Utility.locateMessage(getConnection(), myMessage.substring(0, myMessage.indexOf("ORA-")), getLanguage());
          if (fldMessage!=null) {
            myError = new OBError();
            myError.setType((fldMessage.getField("msgtype").equals("E")?"Error":"Warning"));
            myError.setMessage(fldMessage.getField("msgtext"));
