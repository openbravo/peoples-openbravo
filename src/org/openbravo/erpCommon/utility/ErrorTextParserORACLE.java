 ...
 * 
 *         Instance of the Abstract class, ErrorTextParser, that implements the
 *         error parsing for ORACLE RDBMS.
 */
public class ErrorTextParserORACLE extends ErrorTextParser {
    ...
    public OBError parse() throws Exception {
        ...
        if (pos != -1) {
            ...
            try {
                errorCode = Integer.valueOf(
                        myMessage.substring(pos + 4, myMessage.indexOf(":",
                                pos + 4))).intValue();
                errorCodeText = myMessage.substring(pos, myMessage.indexOf(":",
                        pos));
            ...
            if (errorCode != 0) {
                FieldProvider fldMessage = Utility.locateMessage(
                        getConnection(), Integer.toString(errorCode),
                        getLanguage());
                if (fldMessage != null) {
                    myCodeError = new OBError();
                    myCodeError.setType((fldMessage.getField("msgtype").equals(
                            "E") ? "Error" : "Warning"));
                    ...
                    String messageAux = Utility.parseTranslation(
                            getConnection(), getVars(), getLanguage(),
            ...
            if (pos != -1) {
                ...
                ErrorTextParserORACLEData[] constraintData = ErrorTextParserORACLEData
                        .select(getConnection(), objectName);
                ...
                if (constraintData != null && constraintData.length > 0) {
                    ...
                    FieldProvider fldMessage = Utility.locateMessage(
                            getConnection(), constraintData[0].constraintName,
                            getLanguage());
                    if (fldMessage != null) {
                        myError = new OBError();
                        myError.setType((fldMessage.getField("msgtype").equals(
                                "E") ? "Error" : "Warning"));
                    ...
                    if (constraintData[0].constraintType.equalsIgnoreCase("C")
                        ...
                        fldMessage = Utility.locateMessage(getConnection(),
                                constraintData[0].searchCondition,
                                getLanguage());
                        if (fldMessage != null) {
                            myError = new OBError();
                            myError.setType((fldMessage.getField("msgtype")
                                    .equals("E") ? "Error" : "Warning"));
                            myError.setMessage(fldMessage.getField("msgtext"));
                            return myError;
                        } else if (!constraintData[0].searchCondition.trim()
                                .equals("")) {
                            String searchCond = constraintData[0].searchCondition
                                    .trim().toUpperCase();
                            if (searchCond.endsWith(" IS NOT NULL")) {
                                String columnName = searchCond.substring(0,
                                        searchCond.lastIndexOf(" IS NOT NULL"))
                                        .trim();
                                columnName = Utility.messageBD(getConnection(),
                                        columnName, getLanguage());
                                String tableName = Utility.messageBD(
                                        getConnection(),
                                        constraintData[0].tableName,
                                        getLanguage());
                                myError = new OBError();
                                myError.setType("Error");
                                myError
                                        .setMessage(Utility.messageBD(
                                                getConnection(),
                                                "NotNullError", getLanguage())
                                                + ": "
                                                + tableName
                                                + " - "
                                                + columnName);
                                return myError;
                                    ...
                                    || searchCond.endsWith(" IN ('N', 'Y')")) {
                                String columnName = searchCond.substring(0,
                                        searchCond.lastIndexOf(" IN (")).trim();
                                columnName = Utility.messageBD(getConnection(),
                                        columnName, getLanguage());
                                String tableName = Utility.messageBD(
                                        getConnection(),
                ...
                } else if (ErrorTextParserORACLEData.isTrigger(getConnection(),
                        objectName)) {
                    FieldProvider fldMessage = Utility.locateMessage(
                            getConnection(), myMessage.substring(0, myMessage
                                    .indexOf("ORA-")), getLanguage());
                    if (fldMessage != null) {
                        myError = new OBError();
                        myError.setType((fldMessage.getField("msgtype").equals(
                                "E") ? "Error" : "Warning"));
                    ...
                    myError = new OBError();
                    myError.setType("Error");
                    myError.setMessage(Utility.messageBD(getConnection(),
                            "NotNullError", getLanguage())
