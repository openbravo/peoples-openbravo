 ...
 * 
 *         Implements the needed methods to execute the differents kinds of
...
public class ExecuteQuery {
     ...
     * @throws Exception
     */
    public ExecuteQuery(ConnectionProvider _conn, String _sql,
            Vector<String> _parameters) throws Exception {
        setPool(_conn);
        setSQL(_sql);
    ...
    /**
     * Executes the query and tests the result list via the specified type. This
     * way it is possible to directly search for the next value (in relation to
     * the oldValue parameter) in the result. This next value is returned and
     * the search is stopped leading to a speedup depending on the position
     ...
     */
    public String selectAndSearch(SearchType searchType, String oldValue,
            String idFieldName) throws ServletException {
        ...
        try {
            ...
            if (params != null) {
                for (int iParameter = 0; iParameter < params.size(); iParameter++) {
                    ...
                    UtilSql.setValue(st, iParameter + 1, 12, null,
                            getParameter(iParameter));
                }
            }
            ...
            log4j.error("SQL error in query: " + strSQL.toString()
                    + "Exception:" + e);
            throw new ServletException("@CODE="
                    + Integer.toString(e.getErrorCode()) + "@" + e.getMessage());
        } catch (Exception ex) {
            log4j.error("Exception in query: " + strSQL.toString()
                    + "Exception:" + ex);
        ...
        try {
            ...
            while (result.next()) {
                ...
                for (int i = 0; i < numColumns; i++) {
                    sqlReturnObject.setData(names.elementAt(i), UtilSql
                            .getValue(result, names.elementAt(i)));
                }
