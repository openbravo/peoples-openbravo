...
public class ReferencedLink extends HttpSecureAppServlet {
  ...
  public void doPost (HttpServletRequest request, HttpServletResponse response) throws IOException,ServletException {
    ...
    if (vars.commandIn("DEFAULT")) {
      ...
      //String strKeyReferenceName = vars.getRequiredStringParameter("inpKeyReferenceName");
      ...
      if (log4j.isDebugEnabled()) log4j.debug("strKeyReferenceColumnName:"+strKeyReferenceColumnName+" strTableReferenceId:"+strTableReferenceId+" strKeyReferenceId:"+strKeyReferenceId+" strWindowId:"+strWindowId+" strTableName:"+strTableName);
      {
        ReferencedTables ref = new ReferencedTables(this, strTableReferenceId, strKeyReferenceColumnName, strKeyReferenceId);
        if (!ref.hasSOTrx()) isSOTrx = (Utility.getContext(this, vars, "IsSOTrx", strWindowId).equals("N")?false:true);
        else isSOTrx = ref.isSOTrx();
        ref=null;
      }
      {
        ...
        if (strTableReferenceId.equals("800018")) { //DP
          if (ReferencedTablesData.selectKeyId(this, "C_INVOICE_ID", strTableName, strKeyReferenceColumnName, strKeyReferenceId).equals("")) {
            if (!ReferencedTablesData.selectKeyId(this, "C_ORDER_ID", strTableName, strKeyReferenceColumnName, strKeyReferenceId).equals("")) {
      ...
      if (data == null || data.length==0) throw new ServletException("Window not found: " + strWindowId);
      ...
      if (strKeyReferenceId.equals("")) {
        ...
        if (data == null || data.length==0) throw new ServletException("Window parent not found: " + strWindowId);
        windowName = data[0].windowname;
        tabName = data[0].tabname;
      }
      StringBuffer cadena = new StringBuffer();
      cadena.append(strDireccion).append("/").append(FormatUtilities.replace(windowName)).append("/").append(FormatUtilities.replace(tabName));
      cadena.append("_Edition.html?Command=").append((strKeyReferenceId.equals("")?"DEFAULT":"DIRECT")).append("&");
      cadena.append("inpDirectKey").append("=").append(strKeyReferenceId);
      if (log4j.isDebugEnabled()) log4j.debug(cadena.toString());
