...
public class ExecuteTask extends HttpSecureAppServlet {
  ...
  void printPageDefault(HttpServletResponse response, VariablesSecureApp vars, String strTaskId) throws IOException, ServletException {
    if (log4j.isDebugEnabled()) log4j.debug("Output: Default");
    response.setContentType("text/html; charset=UTF-8");
    PrintWriter out = response.getWriter();
    XmlDocument xmlDocument = xmlEngine.readXmlTemplate("org/openbravo/erpCommon/utility/TaskDefault").createXmlDocument();
  ...
  void executeCommand(HttpServletResponse response, VariablesSecureApp vars, String Tittle, String command) throws IOException, ServletException {
    if (log4j.isDebugEnabled()) log4j.debug("Output: Default");
    response.setContentType("text/html; charset=UTF-8");
    PrintWriter out = response.getWriter();
    ...
    out.println("    <img class=\"Popup_WindowLogo_Icon Popup_WindowLogo_Icon_task\" src=\"" + strDireccion + "/web/images/blank.gif\" border=0/></img>");
    out.println("  </div>");
    out.println("</div>");
    out.println("");
    ...
    out.println("  <tr>");
    out.println("    <td>");
    out.println("      <table cellspacing=\"0\" cellpadding=\"0\" class=\"Popup_ContentPane_NavBar\">");
    ...
    out.println("  <tr>");
    out.println("    <td>");
    out.println("      <table cellspacing=\"0\" cellpadding=\"0\" class=\"Popup_ContentPane_SeparatorBar\">");
    out.println("        <tr>");
    ...
    out.println("            <table>");
    out.println("              <tr>");
    out.println("                <td class=\"Popup_InfoBar_text\" id=\"processMessage\">" + Utility.messageBD(this, "Processing", vars.getLanguage()) + "</td>");
    out.println("              </tr>");
    out.println("            </table>");
    out.println("          </td>");
    out.println("        </tr>");
    ...
    out.println("        <table cellspacing=\"0\" cellpadding=\"0\" class=\"Popup_Client_TablePopup\">");
    out.println("");
    out.println("          <tr>");
    out.println("");
    ...
    out.println("  </tr>");
    out.println("</table>");
    out.println("<script language=\"JavaScript\" type=\"text/javascript\">layer('processMessage', '" + Utility.messageBD(this, "ProcessOK", vars.getLanguage()) + "', true, false);</script>");
    out.println("</form>");
    out.println("</body>");
    out.println("</html>");
    out.close();
  ...
  public void executeCommandTask(PrintWriter out, VariablesSecureApp vars, String cmd) throws IOException, ServletException {
    if (log4j.isDebugEnabled()) log4j.debug("ExecuteTask.executeCommand: " + cmd);
    ...
    do {
      ...
      if (!m_task.getOut().toString().equals("")) out.println("<span>" + m_task.getOut().toString() + "</span><br>");
      if (!m_task.getErr().toString().equals("")) out.println("<span class=\"error\">" + m_task.getErr().toString() + "</span><br>");
      out.flush();
    } while(m_task.isAlive());
    if (log4j.isDebugEnabled()) log4j.debug("ExecuteTask.executeCommand - done");
  }
