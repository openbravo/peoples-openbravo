...
public class ExecuteTask extends HttpSecureAppServlet {
    ...
    public void doPost(HttpServletRequest request, HttpServletResponse response)
        ...
        if (vars.commandIn("DEFAULT")) {
            String strTaskId = vars.getGlobalVariable("inpadTaskId",
                    "ExecuteTask|taskId");
            ...
            if (vars.getLanguage().equals("en_US")) {
                ...
                data = ExecuteTaskData.selectTrl(this, vars.getLanguage(),
                        strTaskId);
            }
            ...
            String command = Utility.parseTranslation(this, vars, vars
                    .getLanguage(), data[0].osCommand);
            String taskinstance = SequenceIdData.getUUID();
            ExecuteTaskData.insert(this, taskinstance, vars.getClient(), vars
                    .getOrg(), vars.getUser(), strTaskId);
            executeCommand(response, vars, data[0].name, command);
        } else if (vars.commandIn("EXECUTE_CMD")) {
            String command = vars.getGlobalVariable("inpCommandText",
                    "ExecuteTask|commandtext");
            // The permissions of the tasks executor must be checked
            command = Utility.parseTranslation(this, vars, vars.getLanguage(),
                    command);
    ...
    void printPageDefault(HttpServletResponse response,
            VariablesSecureApp vars, String strTaskId) throws IOException,
            ServletException {
        ...
        XmlDocument xmlDocument = xmlEngine.readXmlTemplate(
                "org/openbravo/erpCommon/utility/TaskDefault")
                .createXmlDocument();
    ...
    void executeCommand(HttpServletResponse response, VariablesSecureApp vars,
            String Tittle, String command) throws IOException, ServletException {
        if (log4j.isDebugEnabled())
            log4j.debug("Output: Default");
        response.setContentType("text/html; charset=UTF-8");
        PrintWriter out = response.getWriter();
        ...
        out.println("<link REL=\"stylesheet\" TYPE=\"text/css\" HREF=\""
                + strDireccion + "/web/skins/" + vars.getTheme()
                + "/Popup/Popup.css\" TITLE=\"Style\">");
        out
                .println("<script language=\"JavaScript\" type=\"text/javascript\" src=\""
                        + strDireccion + "/web/js/mensaje.js\"></script>");
        out
                .println("<script language=\"JavaScript\" type=\"text/javascript\" src=\""
                        + strDireccion + "/web/js/utils.js\"></script>");
        out
                .println("<script language=\"JavaScript\" type=\"text/javascript\">  defaultLang = \""
                        + vars.getLanguage() + "\";</script>");
        out.println("</head>");
        out.println("<body>");
        out
                .println("<form name=\"form1\" method=\"post\" action=\"ExecuteTask.html\">");
        out.println("<div class=\"Popup_ContentPane_CircleLogo\">");
        out.println("  <div class=\"Popup_WindowLogo\">");
        out
                .println("    <img class=\"Popup_WindowLogo_Icon Popup_WindowLogo_Icon_task\" src=\""
                        + strDireccion
                        + "/web/images/blank.gif\" border=0/></img>");
        out.println("  </div>");
        out.println("</div>");
        out.println("");
        out
                .println("<table cellspacing=\"0\" cellpadding=\"0\" width=\"100%\">");
        out.println("");
        out.println("  <tr>");
        out.println("    <td>");
        out
        ...
        out
                .println("          <td class=\"Popup_NavBar_separator_cell\"></td>");
        ...
        out
                .println("          <td class=\"Popup_NavBar_Popup_title_cell\"><span>"
                        + Tittle + "</span></td>");
        out
                .println("          <td class=\"Popup_NavBar_separator_cell\"></td>");
        out.println("        </tr>");
        ...
        out.println("        <tr>");
        out
                .println("          <td class=\"Popup_InfoBar_Icon_cell\"><img src=\""
                        + strDireccion
        ...
        out.println("              <tr>");
        out
                .println("                <td class=\"Popup_InfoBar_text\" id=\"processMessage\">"
                        + Utility.messageBD(this, "Processing", vars
                                .getLanguage()) + "</td>");
        out.println("              </tr>");
        out.println("            </table>");
        out.println("          </td>");
        ...
        out.println("    <td>");
        out
                .println("      <div class=\"Popup_ContentPane_Client\" style=\"overflow: auto;\">");
        out
        ...
        out
                .println("<script language=\"JavaScript\" type=\"text/javascript\">layer('processMessage', '"
                        + Utility.messageBD(this, "ProcessOK", vars
                                .getLanguage()) + "', true, false);</script>");
        out.println("</form>");
        out.println("</body>");
        out.println("</html>");
        out.close();
    ...
    public void executeCommandTask(PrintWriter out, VariablesSecureApp vars,
            String cmd) throws IOException, ServletException {
        if (log4j.isDebugEnabled())
            log4j.debug("ExecuteTask.executeCommand: " + cmd);
        if (cmd == null || cmd.equals("")) {
            out.println(Utility.messageBD(this, "EmptyCommand", vars
                    .getLanguage()));
            return;
        }
        ...
        do {
            ...
            if (!m_task.getOut().toString().equals(""))
                out.println("<span>" + m_task.getOut().toString()
                        + "</span><br>");
            if (!m_task.getErr().toString().equals(""))
                out.println("<span class=\"error\">"
                        + m_task.getErr().toString() + "</span><br>");
            out.flush();
