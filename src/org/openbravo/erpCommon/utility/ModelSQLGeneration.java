...
public class ModelSQLGeneration {
     ...
     * @throws Exception
     */
    private static Vector<String> getOrderBy(VariablesSecureApp vars,
            TableSQLData tableSQL) throws Exception {
        ...
        if (sortCols != null && sortCols.length() > 0) {
            ...
            while (datas.hasMoreTokens()) {
                ...
                if (!token.equals("")) {
                    vOrderBy.addElement(tableSQL.getTableName() + "." + token
                            + " " + tokenDir);
                    if (!orderBy.toString().equals(""))
                        orderBy.append(", ");
                    orderBy.append(tableSQL.getTableName()).append(".").append(
                            token).append(" ").append(tokenDir);
                }
            }
        }
        vars.setSessionValue(tableSQL.getTabID() + "|orderby", orderBy
                .toString());
        vars.setSessionValue(tableSQL.getTabID() + "|orderbySimple", orderBy
                .toString());
        return vOrderBy;
    }
     ...
     * @throws Exception
     */
    private static SQLReturnObject getFilter(VariablesSecureApp vars,
            TableSQLData tableSQL, Vector<String> filter,
            Vector<String> filterParams) throws Exception {
        SQLReturnObject result = new SQLReturnObject();
        if (tableSQL == null)
            return result;
        boolean isNewFilter = !vars.getStringParameter("newFilter").equals("");
        Vector<Properties> filters = tableSQL.getFilteredStructure(
                "IsSelectionColumn", "Y");
        ...
        if (isNewFilter) {
            for (Enumeration<Properties> e = filters.elements(); e
                    .hasMoreElements();) {
                Properties prop = e.nextElement();
                String aux = vars.getRequestGlobalVariable("inpParam"
                        + prop.getProperty("ColumnName"), tableSQL.getTabID()
                ...
                if (!aux.equals("")) {
                    if (!aux.equals("%")
                            || (!prop.getProperty("AD_Reference_ID").equals(
                                    "10")
                                    && !prop.getProperty("AD_Reference_ID")
                                            .equals("14") && !prop.getProperty(
                                    "AD_Reference_ID").equals("34"))) {
                        filter.addElement(formatFilter(tableSQL.getTableName(),
                                prop.getProperty("ColumnName"), prop
                                        .getProperty("AD_Reference_ID"), true));
                        filterParams.addElement("Param"
                                + prop.getProperty("ColumnName"));
                        result.setData(
                                "Param" + prop.getProperty("ColumnName"), aux);
                    } else {
                ...
                if (Utility.isDecimalNumber(adRefId)
                    ...
                    aux = vars.getRequestGlobalVariable("inpParam"
                            ...
                            .getTabID()
                            + "|param" + prop.getProperty("ColumnName") + "_f");
                    if (!aux.equals("")) {
                        filter
                                .addElement(formatFilter(tableSQL
                                        .getTableName(), prop
                                        .getProperty("ColumnName"), prop
                                        .getProperty("AD_Reference_ID"), false));
                        filterParams.addElement("Param"
                                + prop.getProperty("ColumnName") + "_f");
                        result.setData("Param" + prop.getProperty("ColumnName")
                                + "_f", aux);
                    }
                }
            }
            ...
            for (Enumeration<Properties> e = filters.elements(); e
                    .hasMoreElements();) {
                Properties prop = e.nextElement();
                String aux = vars.getSessionValue(tableSQL.getTabID()
                        + "|param" + prop.getProperty("ColumnName"));
                // The filter is not applied if the parameter value is null or
                // parameter value is '%' for string references.
                if (!aux.equals("")) {
    ...
    /**
     * Formats the filter to get the correct output (adds TO_DATE,
     * TO_NUMBER...).
     * 
     * @param tablename
     ...
     */
    private static String formatFilter(String tablename, String columnname,
            String reference, boolean first) {
        if (columnname == null || columnname.equals("") || tablename == null
                || tablename.equals("") || reference == null
                || reference.equals(""))
            return "";
        StringBuffer text = new StringBuffer();
        if (reference.equals("15") || reference.equals("16")
                || reference.equals("24")) {
            text.append("TO_DATE(").append(tablename).append(".").append(
                    columnname).append(
            ...
            text.append("TO_DATE(?").append(
                    (reference.equals("24") ? ", 'HH24:MI:SS'" : "")).append(
                    ")");
            if (!first && !reference.equals("24"))
                text.append("+1");
        } else if (reference.equals("11") || reference.equals("12")
                || reference.equals("13") || reference.equals("22")
                || reference.equals("29") || reference.equals("800008")
            ...
            text.append(tablename).append(".").append(columnname)
                    .append(" = ?");
        }
     ...
     * @throws Exception
     */
    private static void setSessionOrderBy(VariablesSecureApp vars,
            TableSQLData tableSQL) throws Exception {
        ...
        vars.setSessionValue(tableSQL.getTabID() + "|orderby", txtAux
                .toString());
        ...
        if (txtAuxSimple.toString().equals(""))
            vars.setSessionValue(tableSQL.getTabID() + "|orderbySimple",
                    txtAuxSimple.toString());
        ...
        if (positions != null) {
            for (int i = 0; i < positions.size(); i++) {
                ...
                auxStructure = Integer.toString((Integer.valueOf(auxStructure)
                        .intValue() + 1));
                if (!txtAux.toString().equals(""))
                    txtAux.append(",");
        ...
        vars.setSessionValue(tableSQL.getTabID() + "|orderbyPositions", txtAux
                .toString());
        ...
        vars.setSessionValue(tableSQL.getTabID() + "|orderbyDirections", txtAux
                .toString());
    }
     ...
     */
    public static String generateSQL(ConnectionProvider conn,
            VariablesSecureApp vars, TableSQLData tableSQL,
            String selectFields, Vector<String> filter,
            Vector<String> filterParams, int offset, int pageSize)
            throws Exception {
        return generateSQL(conn, vars, tableSQL, selectFields, filter,
                filterParams, offset, pageSize, true, false);
    }

    /**
     * Special case of the method which specifies that only the id column of the
     * base table should be returned. No extra left joins (besides possible sort
     * columns) are needed.
     */
    public static String generateSQLonlyId(ConnectionProvider conn,
            VariablesSecureApp vars, TableSQLData tableSQL,
            String selectFields, Vector<String> filter,
            Vector<String> filterParams, int offset, int pageSize)
            throws Exception {
        return generateSQL(conn, vars, tableSQL, selectFields, filter,
                filterParams, offset, pageSize, true, true);
    }

    /**
     * Generates the query for this tab. This method adds to the standard query
     * defined in the TableSQLData (from dictionary) the user filter parameters
     * and order by defined by UI
     * 
     * @param conn
    ...
    public static String generateSQL(ConnectionProvider conn,
            ...
            String selectFields, Vector<String> filter,
            Vector<String> filterParams, int offset, int pageSize,
            boolean sorted, boolean onlyId) throws Exception {
        ...
        Vector<String> orderBySimple = new Vector<String>(); // Maintains
                                                             ...
                                                             // just with column
                                                             // names
        String loadSessionOrder = vars.getSessionValue(tableSQL.getTabID()
                + "|newOrder");
        if (loadSessionOrder == null
                || loadSessionOrder.equals("")
                || vars.getSessionValue(tableSQL.getTabID() + "|orderbySimple")
                        .equals("")) {
            orderBy = getOrderBy(vars, tableSQL);
            orderBySimple = (Vector<String>) orderBy.clone();
        } else {
            String auxOrder = vars.getSessionValue(tableSQL.getTabID()
                    + "|orderby");
            String auxOrderSimple = vars.getSessionValue(tableSQL.getTabID()
                    + "|orderbySimple");
            if (!auxOrder.equals(""))
        ...
        SQLReturnObject parametersData = getFilter(vars, tableSQL, filter,
                filterParams);
        String parentKey = tableSQL.getParentColumnName();
        if (parentKey != null && !parentKey.equals("")) {
            String aux = vars.getGlobalVariable("inpParentKey", tableSQL
                    .getWindowID()
                    + "|" + parentKey);
            if (!aux.equals("")) {
        ...
        String strSQL = tableSQL.getSQL(filter, filterParams, orderBy, null,
                selectFields, orderBySimple, offset, pageSize, sorted, onlyId);
        setSessionOrderBy(vars, tableSQL);
        Utility.fillTableSQLParameters(conn, vars, parametersData, tableSQL,
                tableSQL.getWindowID());
        return strSQL;
    }
}
