...
public class WindowTreeUtility {
    ...
    public static String getTreeType(String keyColumnName) {
        if (log4j.isDebugEnabled())
            log4j.debug("WindowTreeUtility.getTreeID() - key Column: "
                    + keyColumnName);
        ...
        if (TreeType.equals(""))
            log4j.error("WindowTreeUtility.getTreeID() - Could not map "
                    + keyColumnName);
        return TreeType;
    }
     ...
     */
    public static WindowTreeData[] getTree(ConnectionProvider conn,
            VariablesSecureApp vars, String TreeType, String TreeID,
            boolean editable, String strParentID, String strNodeId,
        ...
        if (TreeType.equals("MM")) {
            if (isBaseLanguage)
                data = WindowTreeData.select(conn, vars.getUser(), strEditable,
                        strParentID, strNodeId, TreeID);
            else
                data = WindowTreeData.selectTrl(conn, vars.getUser(),
                        strEditable, strParentID, strNodeId, TreeID, vars
                                .getLanguage());
        } else if (TreeType.equals("OO"))
            data = WindowTreeData.selectOrg(conn, vars.getUser(), strEditable,
                    strParentID, strNodeId, TreeID);
        else if (TreeType.equals("PR"))
            data = WindowTreeData.selectProduct(conn, vars.getUser(),
                    strEditable, strParentID, strNodeId, TreeID);
        else if (TreeType.equals("PC"))
            data = WindowTreeData.selectProductCategory(conn, vars.getUser(),
                    strEditable, strParentID, strNodeId, TreeID);
        else if (TreeType.equals("BB"))
            data = WindowTreeData.selectBOM(conn, vars.getUser(), strEditable,
                    strParentID, strNodeId, TreeID);
        else if (TreeType.equals("EV")) {
            String strElementId = vars.getSessionValue(Utility.getWindowID(
                    conn, strTabID)
                    + "|C_Element_ID");
            data = WindowTreeData.selectElementValue(conn, vars.getLanguage(),
                    vars.getUser(), strEditable, strParentID, strNodeId,
                    TreeID, strElementId);
        } else if (TreeType.equals("BP"))
            data = WindowTreeData.selectBPartner(conn, vars.getUser(),
                    strEditable, strParentID, strNodeId, TreeID);
        else if (TreeType.equals("MC"))
            data = WindowTreeData.selectCampaign(conn, vars.getUser(),
                    strEditable, strParentID, strNodeId, TreeID);
        else if (TreeType.equals("PJ"))
            data = WindowTreeData.selectProject(conn, vars.getUser(),
                    strEditable, strParentID, strNodeId, TreeID);
        else if (TreeType.equals("AY"))
            data = WindowTreeData.selectActivity(conn, vars.getUser(),
                    strEditable, strParentID, strNodeId, TreeID);
        else if (TreeType.equals("SR"))
            data = WindowTreeData.selectSalesRegion(conn, vars.getUser(),
                    strEditable, strParentID, strNodeId, TreeID);
        else if (TreeType.equals("AR"))
            data = WindowTreeData.selectAccountingReport(conn, vars.getUser(),
                    strEditable, strParentID, strNodeId, TreeID);
        else if (TreeType.equals("TR"))
            data = WindowTreeData.selectTaxReport(conn, vars.getUser(),
                    strEditable, strParentID, strNodeId, TreeID);
        else {
            FieldProvider[] fData = WindowTreeUtilityClient.getTree(conn, vars,
                    TreeType, TreeID, editable, strParentID, strNodeId,
                    strTabID);
            if (fData != null) {
                data = new WindowTreeData[fData.length];
                for (int i = 0; i < fData.length; i++) {
                    data[i] = WindowTreeUtility
                            .transformFieldProvider(fData[i]);
                }
            }
            fData = null;
        }
    ...
    /**
     * Auxiliar method to transform a FieldProvider into a WindowTreeData
     * object.
     * 
     * @param data
     ...
     * @throws ServletException
     */
    public static WindowTreeData transformFieldProvider(FieldProvider data)
            throws ServletException {
    ...
    /**
     * Adds an html node structure. Used by the WindowTree java to build the
     * html tree.
     * 
     * @param name
     *            Text to display.
     ...
     */
    public static String addNodeElement(String name, String description,
            String target, boolean isSummary, String windowType,
            String strDirection, String strOnClick, String strOnDblClick,
            boolean hasChilds, String nodeId, String action) {
        if (log4j.isDebugEnabled())
            ...
            element
                    ...
                    .append(
                            (nodeId.equals("0") ? " noDrag=\"true\" noSiblings=\"true\" noDelete=\"true\" noRename=\"true\" "
                                    : "")).append(">\n");
            element.append("<a href=\"#\" id=\"folderHref").append(
                    (hasChilds ? "" : "NoChilds")).append("__").append(nodeId)
                    .append("\"");

            if (strOnDblClick != null && !strOnDblClick.equals("")) {
                element.append(" onclick=\"").append(strOnDblClick).append(
                        "return true;\"");
            }
            element.append(">").append(name).append("</a>\n");
            // element.append("</li>\n");
        } else {
            element.append("<li id=\"").append(windowTypeNico(action)).append(
                    "__").append(nodeId).append("\" isElement=\"true\"")
                    .append(" noChildren=\"true\"").append(">\n");
            element.append("<a href=\"#\" id=\"child").append(
                    windowTypeNico(action)).append("__").append(nodeId).append(
                    "\"");
     ...
     * @throws ServletException
     */
    public static void setNode(ConnectionProvider conn,
            VariablesSecureApp vars, String TreeType, String TreeID,
            String strParentID, String strLink, String strSeqNo)
            throws ServletException {
        if (log4j.isDebugEnabled())
            log4j.debug("WindowTreeUtility.setNode() - TreeID: " + TreeID);

        if (TreeType.equals("PR"))
            WindowTreeData.updatePR(conn, vars.getUser(), strParentID,
                    strSeqNo, TreeID, strLink);
        else if (TreeType.equals("BP"))
            WindowTreeData.updateBP(conn, vars.getUser(), strParentID,
                    strSeqNo, TreeID, strLink);
        else
            WindowTreeData.update(conn, vars.getUser(), strParentID, strSeqNo,
                    TreeID, strLink);
    }
