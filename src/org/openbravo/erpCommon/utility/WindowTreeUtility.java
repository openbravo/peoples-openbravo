...
public class WindowTreeUtility {
  ...
  public static String getTreeType(String keyColumnName) {
    ...
    String TreeType = "";
    if(keyColumnName.equals("AD_Menu_ID")) TreeType = "MM";
    else if(keyColumnName.equals("C_ElementValue_ID")) TreeType = "EV";
    else if(keyColumnName.equals("M_Product_ID")) TreeType = "PR";
    ...
    else if(keyColumnName.equals("C_Activity_ID")) TreeType = "AY";
    else if (keyColumnName.equals("AD_Accountingrpt_Element_ID")) TreeType = "AR";
    else if (keyColumnName.equals("C_Tax_Report_ID")) TreeType = "TR";
    else TreeType = WindowTreeUtilityClient.getTreeType(keyColumnName);
    if (TreeType.equals("")) log4j.error("WindowTreeUtility.getTreeID() - Could not map " + keyColumnName);
    return TreeType;
  }
  ...
  public static WindowTreeData[] getTree(ConnectionProvider conn, VariablesSecureApp vars, String TreeType, String TreeID, boolean editable, String strParentID, String strNodeId, String strTabID) throws ServletException {
    ...
    if(TreeType.equals("MM")) {
      if (isBaseLanguage) data = WindowTreeData.select(conn, vars.getUser(), strEditable, strParentID, strNodeId, TreeID);
      else data = WindowTreeData.selectTrl(conn, vars.getUser(), strEditable, strParentID, strNodeId, TreeID, vars.getLanguage());
    } else if (TreeType.equals("OO")) data = WindowTreeData.selectOrg(conn, vars.getUser(), strEditable, strParentID, strNodeId, TreeID);
    else if (TreeType.equals("PR")) data = WindowTreeData.selectProduct(conn, vars.getUser(), strEditable, strParentID, strNodeId, TreeID);
    else if (TreeType.equals("PC")) data = WindowTreeData.selectProductCategory(conn, vars.getUser(), strEditable, strParentID, strNodeId, TreeID);
    ...
    else if (TreeType.equals("AY")) data = WindowTreeData.selectActivity(conn, vars.getUser(), strEditable, strParentID, strNodeId, TreeID);
    else if (TreeType.equals("SR")) data = WindowTreeData.selectSalesRegion(conn, vars.getUser(), strEditable, strParentID, strNodeId, TreeID);
    else if (TreeType.equals("AR")) data = WindowTreeData.selectAccountingReport(conn, vars.getUser(), strEditable, strParentID, strNodeId, TreeID);
    else if (TreeType.equals("TR")) data = WindowTreeData.selectTaxReport(conn, vars.getUser(), strEditable, strParentID, strNodeId, TreeID);
    else {
      FieldProvider[] fData = WindowTreeUtilityClient.getTree(conn, vars, TreeType, TreeID, editable, strParentID, strNodeId, strTabID);
      if (fData!=null) {
  ...
  public static String windowType(String type) {
    ...
    else if (type.equals("W")) return "Ventanas";
    else if (type.equals("X")) return "Formularios";
    else if (type.equals("P")) return "Procesos";
    else if (type.equals("T")) return "Tareas";
    else if (type.equals("R")) return "Informes";
    else if (type.equals("F")) return "FlujoTrabajo";
    else if (type.equals("B")) return "BancoTrabajo";
  ...
  public static String windowTypeNico(String tipo) {
    ...
    if (tipo==null) return "";
    else if (tipo.equals("W")) return "window";
  ...
  public static void setNode(ConnectionProvider conn, VariablesSecureApp vars, String TreeType, String TreeID, String strParentID, String strLink, String strSeqNo) throws ServletException {
    ...
    if(TreeType.equals("PR")) WindowTreeData.updatePR(conn, vars.getUser(), strParentID, strSeqNo, TreeID, strLink);
    else if (TreeType.equals("BP")) WindowTreeData.updateBP(conn, vars.getUser(), strParentID, strSeqNo, TreeID, strLink);
    else WindowTreeData.update(conn, vars.getUser(), strParentID, strSeqNo, TreeID, strLink);
  }
