...
public class VerticalMenu extends HttpSecureAppServlet {
    ...
    void printPageAlert(HttpServletResponse response, VariablesSecureApp vars)
            throws IOException, ServletException {

        Integer alertCount = 0;

        final VerticalMenuData[] data = VerticalMenuData.selectAlertRules(this,
                vars.getUser(), vars.getRole());
        if (data != null && data.length != 0) {
            for (int i = 0; i < data.length; i++) {
                final String strWhere = new UsedByLink().getWhereClause(vars,
                        "", data[i].filterclause);
                try {
                    final Integer count = new Integer(VerticalMenuData
                            .selectCountActiveAlerts(this, Utility.getContext(
                                    this, vars, "#User_Client", ""), Utility
                                    .getContext(this, vars, "#User_Org", ""),
                                    data[i].adAlertruleId, strWhere))
                            .intValue();
                    alertCount += count;
                } catch (final Exception ex) {
                    log4j
                            .error("Error in Alert Query, alertRule="
                                    + data[i].adAlertruleId + " error:"
                                    + ex.toString());
                }
            }
        }
    ...
    void printPageDataSheet(HttpServletResponse response,
            VariablesSecureApp vars, String strCliente, boolean open)
            throws IOException, ServletException {
        ...
        final XmlDocument xmlDocument = xmlEngine.readXmlTemplate(
                "org/openbravo/erpCommon/utility/VerticalMenu", discard)
                .createXmlDocument();
        ...
        dataMenu = MenuData.select(this, vars.getLanguage(), vars.getRole(),
                data[0].parentId);
        xmlDocument.setParameter("directory", "var baseDirectory = \""
                + strReplaceWith + "/\";");
        xmlDocument.setParameter("autosave", "var autosave = "
                + (vars.getSessionValue("#Autosave").equals("")
                        || vars.getSessionValue("#Autosave").equalsIgnoreCase(
                                "N") ? "false" : "true") + ";");
        ...
        xmlDocument.setParameter("userName", MenuData.getUserName(this, vars
                .getUser()));
    ...
    public void printPageLoadingMenu(HttpServletResponse response,
            VariablesSecureApp vars) throws IOException, ServletException {
        final XmlDocument xmlDocument = xmlEngine.readXmlTemplate(
                "org/openbravo/erpCommon/utility/VerticalMenuLoading")
                .createXmlDocument();
        xmlDocument.setParameter("directory", "var baseDirectory = \""
                + strReplaceWith + "/\";\n");
        xmlDocument.setParameter("language", "defaultLang=\""
    ...
    public String generarMenuVertical(MenuData[] menuData, String strDireccion,
            String indice, boolean open) {
        ...
        for (int i = 0; i < menuData.length; i++) {
            if (menuData[i].parentId.equals(indice)) {
                // haveData=true;
                final String strHijos = generarMenuVertical(menuData,
                        strDireccion, menuData[i].nodeId, open);
                String strID = "";
                if (!strHijos.equals("") || menuData[i].issummary.equals("N")) {
                    ...
                    if (menuData[i].issummary.equals("N")) {
                        strText.append(" id=\"").append(
                                tipoVentanaNico(menuData[i].action)).append(
                                menuData[i].nodeId).append("\"");
                        strID = tipoVentanaNico(menuData[i].action)
                                + menuData[i].nodeId;
                    } else
                        strText.append(" id=\"folder").append(
                                menuData[i].nodeId).append("\"");
                    strText.append(" onmouseover=\"window.status='");
                    strText.append(FormatUtilities
                            .replaceJS(menuData[i].description));
                    ...
                    if (!open || !menuData[i].issummary.equals("Y"))
                        strText.append("NOT_");
                    strText
                            .append("Opened NOT_Hover NOT_Selected NOT_Pressed NOT_Focused");
                    strText.append("\"");
                    if (menuData[i].issummary.equals("N")) {
                        strText.append(" id=\"child").append(strID)
                                .append("\"");
                        strText.append(" onclick=\"checkSelected('child")
                                .append(strID).append(
                                        "');submitCommandForm('DEFAULT', "
                                                + (menuData[i].action
                                                        ...
                                                        : "true")
                                                + ", getForm(),'");
                        if (menuData[i].action.equals("L")
                                || menuData[i].action.equals("I"))
                            strText.append(menuData[i].url);
                        else {
                            strText.append(getUrlString(strDireccion,
                                    menuData[i].name, menuData[i].action,
                                    menuData[i].classname,
                                    menuData[i].mappingname,
                                    menuData[i].adWorkflowId,
                                    menuData[i].adTaskId,
                                    menuData[i].adProcessId,
                                    menuData[i].isexternalservice,
                                    menuData[i].serviceType));
                        }
                        strText.append("', '");
                        if (menuData[i].action.equals("F")
                                || menuData[i].action.equals("T")
                                || (menuData[i].action.equals("P")
                                        && menuData[i].mappingname.equals("") && !(menuData[i].isexternalservice
                                        .equals("Y") && menuData[i].serviceType
                                        .equals("PS"))))
                        ...
                        strText.append(", false, "
                                + (menuData[i].action.equals("F") ? "false"
                                        : "true") + ");return false;\"");
                    } else {
                        strText.append(" id=\"child")
                    ...
                    if (menuData[i].issummary.equals("Y"))
                        strText.append(" id=\"folderCell1_").append(
                                menuData[i].nodeId).append("\"");
                    strText.append(">");
                    strText
                            .append("<img src=\"")
                            ...
                            .append(
                                    "/images/blank.gif\" class=\"Menu_Client_Button_")
                            .append((indice.equals("0") ? "Big" : "")).append(
                                    "Icon");
                    if (menuData[i].issummary.equals("N")) {
                        if (menuData[i].action.equals("F"))
                            strText
                                    .append(" Menu_Client_Button_Icon_childWorkFlow");
                        else if (menuData[i].action.equals("T"))
                            strText
                                    .append(" Menu_Client_Button_Icon_childTasks");
                        else if (menuData[i].action.equals("B"))
                            strText
                                    .append(" Menu_Client_Button_Icon_childWorkBench");
                        else if (menuData[i].action.equals("P"))
                            strText
                                    .append(" Menu_Client_Button_Icon_childProcesses");
                        else if (menuData[i].action.equals("R"))
                            strText
                                    .append(" Menu_Client_Button_Icon_childReports");
                        else if (menuData[i].action.equals("X"))
                            strText
                                    .append(" Menu_Client_Button_Icon_childForms");
                        else if (menuData[i].action.equals("L")
                                || menuData[i].action.equals("I"))
                            strText
                                    .append(" Menu_Client_Button_Icon_childExternalLink");
                        else
                            strText
                                    .append(" Menu_Client_Button_Icon_childWindows");
                    } else {
                    ...
                    strText.append("\"");
                    if (menuData[i].issummary.equals("Y"))
                        strText.append(" id=\"folderImg").append(
                                menuData[i].nodeId).append("\"");
                    strText.append(">");
                    strText.append("</td>\n");
                    strText.append("        <td nowrap=\"\"");
                    if (menuData[i].issummary.equals("Y"))
                        strText.append(" id=\"folderCell2_").append(
                                menuData[i].nodeId).append("\"");
                    strText.append(">");
                    ...
                    strText.append("  <td");
                    if (strHijos.equals("")) {
                        strText.append(" style=\"").append("display: none;")
                                .append("\" id=\"parent").append(
                                        menuData[i].nodeId).append("\">\n");
                    } else {
                        strText.append(" style=\"").append(
                                (!open ? "display: none;" : "")).append(
                                "\" id=\"parent").append(menuData[i].nodeId)
                                .append("\">\n");
                    }
                    strText
    ...
    public static String getUrlStringStatic(String strDireccionBase,
            String name, String action, String classname, String mappingname,
            String adWorkflowId, String adTaskId, String adProcessId,
            String isExternalService, String externalType) {
        return new VerticalMenu().getUrlString(strDireccionBase, name, action,
                classname, mappingname, adWorkflowId, adTaskId, adProcessId,
                isExternalService, externalType);
    }

    public String getUrlString(String strDireccionBase, String name,
            String action, String classname, String mappingname,
            String adWorkflowId, String adTaskId, String adProcessId,
            String isExternalService, String externalType) {
        ...
        if (mappingname.equals("")) {
            if (action.equals("F")) {
                strResultado.append(
                        "/ad_workflow/WorkflowControl.html?inpadWorkflowId=")
                        .append(adWorkflowId);
            } else if (action.equals("T")) {
                strResultado.append("/utility/ExecuteTask.html?inpadTaskId=")
                        .append(adTaskId);
            } else if (action.equals("P")) {
                if (isExternalService.equals("Y") && externalType.equals("PS"))
                    strResultado.append(
                            "/utility/OpenPentaho.html?inpadProcessId=")
                            .append(adProcessId);
                else {
                    try {
                        if (MenuData.isGenericJavaProcess(this, adProcessId))
                            strResultado
                                    .append(
                                            "/ad_actionButton/ActionButtonJava_Responser.html?inpadProcessId=")
                                    .append(adProcessId);
                        else
                            strResultado
                                    .append(
                                            "/ad_actionButton/ActionButton_Responser.html?inpadProcessId=")
                                    .append(adProcessId);
                ...
                strResultado.append("/ad_forms/").append(
                        FormatUtilities.replace(name)).append(".html");
            } else if (action.equals("R")) {
                strResultado.append("ad_reports/").append(
                        FormatUtilities.replace(name)).append(".html");
            }
        } else {
    ...
    /**
     * Generates Search folder and entries in case there is at least one
     * accessible entrie, other case it returns an empty string
     * 
     * @param vars
     * @param open
     * @return
     ...
     */
    public String generateMenuSearchs(VariablesSecureApp vars, boolean open)
            throws ServletException {
        final StringBuffer menu = new StringBuffer();
        final MenuData[] data = MenuData
                .selectSearchs(this, vars.getLanguage());
        if (data != null && data.length > 0) {
            final String entries = generateMenuSearchEntries(vars,
                    strDireccion, open, data);
            if (entries.length() > 0) {
                menu.append("<tr>\n");
                menu.append("  <td>\n");
                menu
                        .append("    <table cellspacing=\"0\" cellpadding=\"0\" onmouseover=\"window.status='");
                menu.append(Utility.messageBD(this, "Information", vars
                        .getLanguage()));
                ...
                menu
                        .append(
                                "        <td width=\"5px\" id=\"folderCell1_Information\"><img src=\"")
                        .append(strReplaceWith)
                        .append(
                ...
                menu
                        .append("        <td nowrap=\"\" id=\"folderCell2_Information\">");
                menu.append(Utility.messageBD(this, "Information", vars
                        .getLanguage()));
                menu.append("        </td>\n");
                menu.append("      </tr>\n");
                ...
                menu.append("  <td");
                menu.append(" style=\"")
                        .append((!open ? "display: none;" : "")).append(
     ...
     * @throws ServletException
     */
    public String generateMenuSearchEntries(VariablesSecureApp vars,
            String direccion, boolean open, MenuData[] data)
            throws ServletException {
        final StringBuffer result = new StringBuffer();
        if (data == null || data.length == 0)
            return "";

        final AccessData[] accessData = AccessData.selectAccessSearchMultiple(
                this, vars.getRole());
        ...
        for (int i = 0; i < data.length; i++) {
            ...
            if ((res != null) && (!res.equals("0"))) {
                ...
                result.append(" id=\"info").append(
                        FormatUtilities.replace(data[i].name)).append("\"");
                result.append(">\n");
                result.append("      <tr");
                result
                        .append(" class=\"Normal NOT_Opened NOT_Hover NOT_Selected NOT_Pressed NOT_Focused\"");
                result.append(" id=\"childinfo").append(
                        FormatUtilities.replace(data[i].name)).append("\"");
    ...
    void decidePopups(XmlDocument xmlDocument, VariablesSecureApp vars)
            throws ServletException {

        if (vars.getRole() != null && vars.getRole().equals("0")) {
            // Check if the heartbeat popup needs to be displayed
            final HeartbeatProcessData[] hbData = HeartbeatProcessData
                    .selectSystemProperties(myPool);
            if (hbData.length > 0) {
                ...
                if (isheartbeatactive == null || isheartbeatactive.equals("")) {
                    if (postponeDate == null || postponeDate.equals("")) {
                        ...
                        Date date = null;
                        try {
                            date = new SimpleDateFormat(vars
                                    .getJavaDateFormat()).parse(postponeDate);
            ...
            if (rData.length > 0) {
                ...
                if (isregistrationactive == null
                        || isregistrationactive.equals("")) {
                    if (rPostponeDate == null || rPostponeDate.equals("")) {
                        xmlDocument
                                .setParameter("popup", "openRegistration();");
                        return;
                    } else {
                        Date date = null;
                        try {
                            date = new SimpleDateFormat(vars
                                    .getJavaDateFormat()).parse(rPostponeDate);
                            if (date.before(new Date())) {
                                xmlDocument.setParameter("popup",
                                        "openRegistration();");
                                return;
