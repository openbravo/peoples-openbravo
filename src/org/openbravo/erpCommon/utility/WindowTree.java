...
public class WindowTree extends HttpSecureAppServlet {
    ...
    public void doPost(HttpServletRequest request, HttpServletResponse response)
        ...
        if (vars.commandIn("DEFAULT", "TAB")) {
            ...
            String strResult = WindowTreeChecks.checkChanges(this, vars,
                    strTabId, strTop, strLink, strChild.equals("Y"));
            if (strResult.equals(""))
     ...
     */
    public String loadNodes(VariablesSecureApp vars, String key,
            boolean editable, String strTabId) throws ServletException {
        String TreeType = WindowTreeUtility.getTreeType(key);
        String TreeID = "";
        String TreeName = "";
        String TreeDescription = "";
        WindowTreeData[] data = WindowTreeData.selectTreeID(this, Utility
                .getContext(this, vars, "#User_Client", ""), TreeType);
        StringBuffer menu = new StringBuffer();
        if (data == null || data.length == 0) {
            log4j.error("WindowTree.loadNodes() - Unknown TreeNode: TreeType "
                    + TreeType + " - TreeKey " + key);
            throw new ServletException(
                    "WindowTree.loadNodes() - Unknown TreeNode");
        } else {
            TreeID = data[0].id;
        ...
        menu.append(WindowTreeUtility.addNodeElement(TreeName, TreeDescription,
                CHILD_SHEETS, true, "", strDireccion, "clickItem(0, '"
                        + Replace.replace(TreeName, "'", "\\'") + "', 'N');",
        ...
        // "', 'N');", "", true, "0", ""));
        menu.append(generateTree(WindowTreeUtility.getTree(this, vars,
                TreeType, TreeID, editable, "", "", strTabId), strDireccion,
                "0", true));
        menu.append("\n</ul>\n");
     ...
     * @throws ServletException
     */
    public String loadChildNodes(VariablesSecureApp vars, String key,
            String strTreeID, String strParentID, boolean editable,
        ...
        if (!script.equals("")) {
            ...
            if (strParentID.equals("0"))
                data = WindowTreeData.selectTreeID(this, Utility.getContext(
                        this, vars, "#User_Client", ""), TreeType);
            else
                data = WindowTreeUtility.getTree(this, vars, TreeType,
                        strTreeID, editable, "", strParentID, strTabId);
            if (data == null || data.length == 0) {
     ...
     * @return String html with the tree.
     */
    public String generateTree(WindowTreeData[] data, String strDireccion,
            String indice, boolean isFirst) {
        if (data == null || data.length == 0)
            return "";
        if (log4j.isDebugEnabled())
        ...
        for (int i = 0; i < data.length; i++) {
            if (data[i].parentId.equals(indice)) {
                hayDatos = true;
                String strHijos = generateTree(data, strDireccion,
                        data[i].nodeId, isFirst);
                strResultado.append(WindowTreeUtility.addNodeElement(
                        data[i].name, data[i].description, CHILD_SHEETS,
                        data[i].issummary.equals("Y"), WindowTreeUtility
                                .windowType(data[i].action), strDireccion,
                        "clickItem('" + data[i].nodeId + "', '"
                                + Replace.replace(data[i].name, "'", "\\'")
                                + "', '" + data[i].issummary + "');",
                        "dblClickItem('" + data[i].nodeId + "');", !strHijos
                                .equals(""), data[i].nodeId, data[i].action));
    ...
    void printPageDataSheet(HttpServletResponse response,
            VariablesSecureApp vars, String TabId) throws IOException,
            ServletException {
        if (log4j.isDebugEnabled())
            log4j.debug("Output: Tree's screen for the tab: " + TabId);
        OBError defaultInfo = new OBError();
        defaultInfo.setType("INFO");
        defaultInfo.setTitle(Utility
                .messageBD(this, "Info", vars.getLanguage()));
        defaultInfo.setMessage(Utility.messageBD(this, "TreeInfo", vars
                .getLanguage()));
        vars.setMessage("WindowTree", defaultInfo);

        XmlDocument xmlDocument = xmlEngine.readXmlTemplate(
                "org/openbravo/erpCommon/utility/WindowTree")
                .createXmlDocument();
        ...
        xmlDocument.setParameter("page", "../" + windowName + "/" + tabName
                + "_Edition.html");
     ...
     * @param strChild
     *            String indicating if is a child or not of the parent node
     *            (Y|N).
     * @throws ServletException
     */
    void changeNode(VariablesSecureApp vars, String strTabId, String strTop,
            String strLink, String strChild) throws ServletException {
        String key = WindowTreeData.selectKey(this, strTabId);
        String TreeType = WindowTreeUtility.getTreeType(key);
        String TreeID = "";
        String strParent = strTop;
        boolean editable = WindowTreeData.selectEditable(this, strTabId)
                .equals("Y");
        ...
        if (!strTop.equals("0")) {
            WindowTreeData[] data = WindowTreeUtility.getTree(this, vars,
                    TreeType, TreeID, editable, "", strTop, strTabId);
            if (data == null || data.length == 0) {
                log4j.error("WindowTree.loadNodes() - Unknown Top Node");
                throw new ServletException(
                        "WindowTree.loadNodes() - Unknown Top Node");
            }
        ...
        WindowTreeData[] data = WindowTreeUtility.getTree(this, vars, TreeType,
                TreeID, editable, strParent, "", strTabId);
        int seqNo = 0;
        int add = 10;
        try {
            if (data == null || data.length == 0) {
                WindowTreeUtility.setNode(this, vars, TreeType, TreeID,
                        strParent, strLink, Integer.toString(seqNo));
            } else {
                    ...
                    if (!data[i].nodeId.equals(strLink)) {
                        ...
                        if (updated && !finish) { // update only elements after
                            ...
                            if (data[i].isindevelopment == null
                                ...
                                seqNo += add;
                                WindowTreeUtility
                                        .setNode(this, vars, TreeType, TreeID,
                                                data[i].parentId,
                                                data[i].nodeId, Integer
                                                        .toString(seqNo));
                            } else {
                        ...
                        if (!updated && data[i].nodeId.equals(strTop)) {
                            ...
                            for (j = i + 1; j < data.length
                                    && (data[j].isindevelopment == null
                                            || data[j].isindevelopment
                                                    .equals("") || data[j].isindevelopment
                                            .equals("Y")); j++)
                                ;
                            if (j == data.length)
                            ...
                            if (i == 0)
                                ...
                                seqNo = new Integer(data[i].seqno).intValue()
                                        + add;
                            WindowTreeUtility.setNode(this, vars, TreeType,
                                    TreeID, strParent, strLink, Integer
                                            .toString(seqNo));
                            updated = true;
                        }
                    }
            ...
            log4j.error("WindowTree.changeNode() - Couldn't change the node: "
                    + strLink);
