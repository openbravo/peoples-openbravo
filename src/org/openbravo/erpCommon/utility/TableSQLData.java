...
public class TableSQLData {
     ...
     * @throws Exception
     */
    public TableSQLData(VariablesSecureApp _vars, ConnectionProvider _conn,
            String _adTabId, String _orgList, String _clientList,
            boolean showAudit) throws Exception {
        if (_vars != null)
            setVars(_vars);
        setPool(_conn);
        setTabID(_adTabId);
    ...
    public TableSQLData(VariablesSecureApp _vars, ConnectionProvider _conn,
            String _adTabId, String _orgList, String _clientList)
            throws Exception {
        this(_vars, _conn, _adTabId, _orgList, _clientList, _vars
                .getSessionValue("#ShowAudit").equals("Y"));
    }
    ...
    /**
     * Gets the parameter's values defined, which are needed for onlyId queries.
     * These correspond to the sql generated by the getSql-function with
     * onlyId=true
     * 
     * @return Vector with the values.
     */
    ...
    public void setWindowDefinition() throws Exception {
        TableSQLQueryData[] data = TableSQLQueryData.selectWindowDefinition(
                getPool(), getVars().getLanguage(), getTabID());
        ...
        setParameter(internalPrefix + "AD_ColumnSortOrder_ID",
                data[0].adColumnsortorderId);
        setParameter(internalPrefix + "AD_ColumnSortYesNo_ID",
                data[0].adColumnsortyesnoId);
        ...
        if (!getTabLevel().equals("0")) {
            TableSQLQueryData[] parentFields = TableSQLQueryData
                    .parentsColumnName(getPool(), getTabID());
            if (parentFields != null && parentFields.length > 0) {
                if (log4j.isDebugEnabled())
                    log4j.debug("Parent key: " + parentFields[0].columnname);
                setParameter(internalPrefix + "ParentColumnName",
                        parentFields[0].columnname);
            } else {
                parentFields = TableSQLQueryData.parentsColumnNameKey(
                        getPool(), getTabID());
                if (parentFields != null && parentFields.length > 0) {
                    if (log4j.isDebugEnabled())
                        log4j
     ...
     */
    public Vector<Properties> getFilteredStructure(String propertyName,
            String propertyValue) {
        Vector<Properties> vAux = new Vector<Properties>();
        if (this.structure == null)
            return vAux;
        for (Enumeration<Properties> e = this.structure.elements(); e
                .hasMoreElements();) {
    ...
    public void addAuditFields(Vector<Properties> properties) {
        ...
        for (Enumeration<Properties> e = this.structure.elements(); e
            ...
            if (prop.getProperty("ColumnName").equalsIgnoreCase("Created")
                    || prop.getProperty("ColumnName").equalsIgnoreCase(
                            "CreatedBy")
                    || prop.getProperty("ColumnName").equalsIgnoreCase(
                            "Updated")
                    || prop.getProperty("ColumnName").equalsIgnoreCase(
    ...
    public void addSelectField(String _field, String _alias, boolean _new) {
        ...
        if (this.select == null)
            ...
            if (pOld != null) {
                p = new QueryFieldStructure(pOld.getField() + FIELD_CONCAT
                        + _field, " AS ", _alias, "SELECT");
    ...
    /**
     * Adds fields to the internal filter clause. The internal filter clause is
     * the one defined by the tab in the dictionary.
     * 
     * @param _field
     *            String with the field.
     * @param _type
    ...
    /**
     * Tokenizes vector elements and inserts it as new vector elements and
    ...
    public Vector<String> cleanVector(Vector<String> v) {
        ...
        if (v != null) {
            for (int i = 0; i < v.size(); i++) {
                StringTokenizer element = new StringTokenizer(v.elementAt(i),
                        ",", false);
    ...
    /**
     * Adds new field to the internal order by clause. The internal order by
     * clause is the one defined in the application dictionary for the tab.
     * 
     * @param _field
     *            String with the field.
     */
     ...
     *            String with the field of this parameter.
     */
    public void addSubQueryParameter(String _parameter, String _fieldName,
            String _realName) {
        if (this.paramSubquery == null)
            this.paramSubquery = new Vector<QueryParameterStructure>();
        QueryParameterStructure aux = new QueryParameterStructure(_parameter,
                _fieldName, "SUB", _realName);
     ...
     *            String with the type of parameter.
     */
    public void addFilterParameter(String _parameter, String _fieldName,
            String _type) {
     ...
     *            String with the type of parameter.
     */
    public void addWrapperParameter(String _parameter, String _fieldName,
            String _type) {
    ...
    /**
     * Adds new parameter to the internal filter clause (the one defined in the
     * dictionary).
     * 
     * @param _parameter
     *            String with the parameter.
     ...
     *            String with the type of parameter.
     */
    public void addInternalFilterParameter(String _parameter,
            String _fieldName, String _type) {
    ...
    /**
     * Adds new parameter to the internal order by clause (the one defined in
     * the dictionary).
     * 
     * @param _parameter
     *            String with the parameter.
        ...
        while (i != -1) {
            ...
            if (defStr.equals("?")) {
                if (type.equalsIgnoreCase("WHERE"))
                    addWhereParameter(token, type.toUpperCase(), type
                            .toUpperCase());
                else if (type.equalsIgnoreCase("FILTER"))
                    addInternalFilterParameter(token, type.toUpperCase(), type
                            .toUpperCase());
    ...
    private void generateStructure(boolean showAudit) throws Exception {
        ...
        TableSQLQueryData[] data = TableSQLQueryData.selectRelationStructure(
                getPool(), getVars().getLanguage(), getTabID());
        if (data == null || data.length == 0)
        ...
        for (int i = 0; i < data.length; i++) {
            ...
            if (primaryKey.equals("")
                    && data[i].iskey.equals("Y")
                    && (getIsSameTable() || !data[i].columnname
                            .equalsIgnoreCase(parentKey))) {
                primaryKey = data[i].columnname;
            } else if (secondaryKey.equals("")
                    && data[i].issecondarykey.equals("Y")
                    && (getIsSameTable() || !data[i].columnname
                            .equalsIgnoreCase(parentKey))) {
        ...
        if (showAudit) {
            TableSQLQueryData[] dataAudit = TableSQLQueryData
                    .selectRelationStructureAudit(getPool(), getVars()
    ...
    public void generateSQL() throws Exception {
        ...
        for (Enumeration<Properties> e = headers.elements(); e
                .hasMoreElements();) {
            Properties prop = e.nextElement();
            switch (Integer.valueOf(prop.getProperty("AD_Reference_ID"))
                    .intValue()) {
            case 17: // List
            case 18: // Table
            case 19: // TableDir
            ...
            case 800011: // Product Search
                ...
                break;
            default:
                identifier(getTableName(), prop,
                        prop.getProperty("ColumnName"), getTableName() + "."
                                + prop.getProperty("ColumnName"));
                break;
            }
        }
    ...
    /**
     * Auxiliar method to build the query with the recursive process to build
     * the foreigns to the references. It adds fields to the select clause and,
     * if needed, to the from clause.
     * 
     ...
     * @param field
     *            String with the list of properties of the field to prepare
     *            identifier.
     * @param identifierName
     *            String with the identifier name.
     * @param realName
     *            String identifying tableName.fieldName, this is maintained
     *            through recursivity
     * @throws Exception
     */
    public void identifier(String parentTableName, Properties field,
        ...
        case 17: // List
            setListQuery(parentTableName, field.getProperty("ColumnName"),
                    field.getProperty("AD_Reference_Value_ID"), identifierName,
                    realName);
            break;
        case 18: // Table
            setTableQuery(parentTableName, field.getProperty("ColumnName"),
                    field.getProperty("AD_Reference_Value_ID"), identifierName,
                    realName);
            break;
        case 19: // TableDir
            setTableDirQuery(parentTableName, field
                    .getProperty("ColumnNameSearch"), field
                    .getProperty("ColumnName"), field
                    .getProperty("AD_Reference_Value_ID"), identifierName,
                    realName);
            break;
        case 35: // PAttribute
        case 30: // Search
            ...
            break;
        case 32: // Image
            setImageQuery(parentTableName, field
                    .getProperty("ColumnNameSearch"), identifierName, realName);
            break;
        case 28: // Button
            if (field.getProperty("AD_Reference_Value_ID") != null
                    && !field.getProperty("AD_Reference_Value_ID").equals("")) {
                setListQuery(parentTableName, field.getProperty("ColumnName"),
                        field.getProperty("AD_Reference_Value_ID"),
                        identifierName, realName);
            } else
                addSelectField(formatField((parentTableName + "." + field
                        .getProperty("ColumnName")), reference), identifierName);
            break;
        default:
            if (!checkTableTranslation(parentTableName, field, reference,
                    identifierName, realName)) {
                addSelectField(formatField((parentTableName + "." + field
                        .getProperty("ColumnName")), reference), identifierName);
            }
     ...
     * @throws Exception
     */
    private boolean checkTableTranslation(String tableName, Properties field,
            String reference, String identifierName, String realName)
            throws Exception {
        if (tableName == null || tableName.equals("") || field == null)
            return false;
        ComboTableQueryData[] data = ComboTableQueryData
                .selectTranslatedColumn(getPool(), field
                        .getProperty("TableName"), field
                        .getProperty("ColumnName"));
        if (data == null || data.length == 0)
            return false;
        ...
        addSelectField("(CASE WHEN td_trl"
                ...
                + data[0].columnname
                + " IS NULL THEN "
                + formatField((tableName + "." + field
                        .getProperty("ColumnName")), reference)
                + " ELSE "
                + formatField(("td_trl" + myIndex + "." + data[0].columnname),
                        reference) + " END)", identifierName);
        addFromField("(SELECT AD_Language, " + data[0].reference + ", "
                + data[0].columnname + " FROM " + data[0].tablename
                + ") td_trl" + myIndex + " on " + tableName + "."
                + data[0].reference + " = td_trl" + myIndex + "."
    ...
    private Properties fieldToProperties(FieldProvider field) throws Exception {
        ...
        if (field != null) {
            ...
            aux.setProperty("AD_Reference_Value_ID", field
     ...
     * @throws Exception
     */
    private void setImageQuery(String tableName, String fieldName,
            String identifierName, String realName) throws Exception {
        int myIndex = this.index++;
        addSelectField("((CASE td" + myIndex + ".isActive WHEN 'N' THEN '"
                + INACTIVE_DATA + "' ELSE '' END) || td" + myIndex
                + ".imageURL)", identifierName);
        String tables = "(select IsActive, AD_Image_ID, ImageURL from AD_Image) td"
                + myIndex;
    ...
    private void setListQuery(String tableName, String fieldName,
            String referenceValue, String identifierName, String realName)
            throws Exception {
        int myIndex = this.index++;
        addSelectField("((CASE td" + myIndex + ".isActive WHEN 'N' THEN '"
        ...
        if (fieldName.equalsIgnoreCase("DocAction"))
            tables += "(CASE " + tableName + "." + fieldName
                    + " WHEN '--' THEN 'CL' ELSE TO_CHAR(" + tableName + "."
                    + fieldName + ") END)";
        else
            tables += tableName + "." + fieldName;
        tables += " = td" + myIndex + ".value AND td" + myIndex
                + ".ad_reference_id = ?";
        addFromField(tables, "td" + myIndex, realName);
        addFromParameter("TD" + myIndex + ".AD_REFERENCE_ID", "KEY", realName);
        setParameter("TD" + myIndex + ".AD_REFERENCE_ID", referenceValue);
        addFromField(
                "(SELECT ad_language, name, ad_ref_list_id from ad_ref_list_trl) td_trl"
                        + myIndex + " on td" + myIndex
    ...
    private void setTableDirQuery(String tableName, String fieldName,
        ...
        addFromField(tables, "td" + myIndex, realName);
        for (int i = 0; i < trd.length; i++)
            identifier("td" + myIndex, fieldToProperties(trd[i]),
                    identifierName, realName);
    }

    /**
     * Search for the close char inside a text. This method bear in mind that in
     * the string can be another open chars with their own close char.
     * 
     * @param text
     ...
     */
    private int findCloseTarget(String text, int pos, String openChar,
            String closeChar) {
    ...
    /**
     * Sets the filters for the window, including the filters by granted clients
     * and organizations.
     * 
     * @throws Exception
     */
    private void setWindowFilters() throws Exception {
        ...
        if (!getTableName().toUpperCase().endsWith("_ACCESS")) {
            addWhereField(getTableName() + ".AD_Client_ID IN ("
                    + getClientList() + ")", "WHERE");
            addWhereField(getTableName() + ".AD_Org_ID IN (" + getOrgList()
                    + ")", "WHERE");
        }
        String parentKey = getParentColumnName();
        ...
        if (strOrderByClause == null || strOrderByClause.equals(""))
            ...
            if (strOrderByClause.indexOf("@") != -1)
                strOrderByClause = parseContext(strOrderByClause,
                        "INTERNAL_ORDERBY");
            ...
            if (vecOrdersAux != null && vecOrdersAux.size() > 0) {
                for (int i = 0; i < vecOrdersAux.size(); i++)
                    addInternalOrderByField(getRealOrderByColumn(vecOrdersAux
                            .elementAt(i)));
            } else
        ...
        if (getWindowType().equalsIgnoreCase("T") && getTabLevel().equals("0")) {
            /*
             * using now() as a function on oracle so that it is evaluated many
             * times in a query is very inefficient. Special case it for
             * oracle/postgres
             */
            if (getPool().getRDBMS().equalsIgnoreCase("ORACLE")) {
                addInternalFilterField("(COALESCE(" + getTableName()
                        + ".Processed, 'N')='N' OR " + getTableName()
                        + ".Updated > sysdate-TO_NUMBER(?))", "FILTER");
            } else {
                addInternalFilterField("(COALESCE(" + getTableName()
                        + ".Processed, 'N')='N' OR " + getTableName()
                        + ".Updated > now()-TO_NUMBER(?))", "FILTER");
            }
            addInternalFilterParameter("#Transactional$Range", "FILTER",
                    "FILTER");
        }
    }
    ...
    private void getFieldsOrderByClause() throws Exception {
        TableSQLQueryData[] orderByData = TableSQLQueryData
                .selectOrderByFields(getPool(), getTabID());
        if (orderByData == null)
            return;
        for (int i = 0; i < orderByData.length; i++) {
            addInternalOrderByField(getTableName()
                    + "."
                    + orderByData[i].columnname
                    + " "
                    + (orderByData[i].columnname.equalsIgnoreCase("DocumentNo") ? "DESC"
                            : "ASC"));
        }
    }

    /**
     * Returns the actual order by columns. The columns names inside the
     * handler.
     * 
     * @param data
    ...
    private String getRealOrderByColumn(String data) {
        ...
        if (!_alias.equals("") && this.structure != null) {
            ...
            if (myProps != null) {
                position = Integer.valueOf(myProps.getProperty("Position"))
                        .intValue();
    ...
    /**
     * Returns the columns defined for this tab, including the ones derived from
     * the references.
     * 
     ...
     *            Boolean to indicate if the identifier columns must be removed
     *            from the list.
     * @param useFieldLength
     *            Boolean to indicate if the column width should be calculated
     *            based on the FieldLength instead of DisplayLength.
     * @return Array with the columns.
     */
    public SQLReturnObject[] getHeaders(boolean withoutIdentifiers,
            boolean useFieldLength) {
        SQLReturnObject[] data = null;
        Vector<SQLReturnObject> vAux = new Vector<SQLReturnObject>();
        Vector<Properties> structure = getStructure();
        for (Enumeration<Properties> e = structure.elements(); e
                .hasMoreElements();) {
            Properties prop = e.nextElement();
            if (prop.getProperty("IsKey").equals("Y")
                    ...
                    || (prop.getProperty("IsDisplayed").equals("Y")
                            ...
                            && prop.getProperty("IsEncrypted").equals("N") && !prop
                            .getProperty("ColumnName").equals(
                                    getParentColumnName()))) {
                SQLReturnObject dataAux = new SQLReturnObject();

                boolean cloneRecord = (!withoutIdentifiers
                        && (prop.getProperty("IsDisplayed").equals("Y") && prop
                                .getProperty("ShowInRelation").equals("Y"))
                        && prop.getProperty("ColumnName")
                                .equals(getKeyColumn()) && !getSelectField(
                        prop.getProperty("ColumnName") + "_R").equals(""));

                dataAux.setData("columnname", prop.getProperty("ColumnName"));
                dataAux.setData("gridcolumnname", prop
                        .getProperty("ColumnName"));
                dataAux.setData("adReferenceId", prop
                        .getProperty("AD_Reference_ID"));
                dataAux.setData("adReferenceValueId", prop
                        .getProperty("AD_Reference_Value_ID"));
                dataAux.setData("isidentifier", (prop.getProperty(
                        "IsIdentifier").equals("Y") ? "true" : "false"));
                dataAux.setData("iskey", (prop.getProperty("ColumnName")
                        .equals(getKeyColumn()) && !cloneRecord) ? "true"
                        : "false");
                dataAux.setData("isvisible", ((prop.getProperty("IsDisplayed")
                        .equals("Y") && prop.getProperty("ShowInRelation")
                        .equals("Y")) ? "true" : "false"));
    ...
    /**
     * Auxiliar method to clone the column object. It's used to add key column
     * when the key column must be a reference one.
     * 
     * @param data
    ...
    public SQLReturnObject getClone(SQLReturnObject data) {
        ...
        dataAux.setData("adReferenceValueId", data
                .getData("adReferenceValueId"));
    ...
    public Properties getColumnPosition(String _name) {
        ...
        for (int i = 0; i < vAux.length; i++) {
            if (vAux[i].getData("columnname").equalsIgnoreCase(_name)
                    || (getTableName() + "." + vAux[i].getData("columnname"))
                            .equalsIgnoreCase(_name)) {
                _prop.setProperty("AD_Reference_ID", vAux[i]
                        .getData("adReferenceId"));
    ...
    public String getSelectFieldAlias(String _data) {
        ...
        for (int i = 0; i < this.select.size(); i++) {
            ...
            if (p.getField().equalsIgnoreCase(_data))
                ...
                return p.getAlias();
            else if ((getTableName() + "." + p.getAlias())
                    .equalsIgnoreCase(_data.trim()))
                return p.getAlias();
        }
        return "";
    }
        ...
        if (_fields == null || _fields.size() == 0) {
            if (this.internalOrderBy != null) {
                ...
                if (this.paramInternalOrderBy != null) {
                    for (int i = 0; i < this.paramInternalOrderBy.size(); i++) {
                        QueryParameterStructure aux = this.paramInternalOrderBy
                                .elementAt(i);
            ...
            for (int i = 0; i < _fields.size(); i++) {
                ...
                if (vecOrdersAux != null && vecOrdersAux.size() > 0) {
                    for (int j = 0; j < vecOrdersAux.size(); j++) {
                        addOrderByField(getRealOrderByColumn(vecOrdersAux
                                .elementAt(j)));
            ...
            if (_params != null)
                for (int i = 0; i < _params.size(); i++)
                    addOrderByParameter(_params.elementAt(i), _params
                            .elementAt(i));
        }
    }
    ...
    public void setFilter(Vector<String> _fields, Vector<String> _params) {
        ...
        if (_fields == null || _fields.size() == 0) {
            if (this.internalFilter != null) {
                ...
                if (this.paramInternalFilter != null) {
                    for (int i = 0; i < this.paramInternalFilter.size(); i++) {
                        QueryParameterStructure aux = this.paramInternalFilter
                                .elementAt(i);
                        addFilterParameter(aux.getName(), aux.getField(), aux
                                .getType());
                    }
                }
            }
            ...
            if (_params != null)
                for (int i = 0; i < _params.size(); i++)
                    addFilterParameter(_params.elementAt(i), _params
                            .elementAt(i), "FILTER");
        }
    }
     ...
     * @return String with the sql
     */
    public String getSQL(Vector<String> _FilterFields,
            Vector<String> _FilterParams, Vector<String> _OrderFields,
            Vector<String> _OrderParams, String selectFields,
            Vector<String> _OrderSimple, int startPosition, int rangeLength) {
        return getSQL(_FilterFields, _FilterParams, _OrderFields, _OrderParams,
                selectFields, _OrderSimple, startPosition, rangeLength, true,
                false);
    }
     ...
     * @param _OrderFields
     *            Vector with specific order by fields. It can contain
     *            tablename.field or SQL clause
     * @param _OrderParams
     *            Vector with parameters for the specific order by fields.
     * @param selectFields
     *            String with the fields for the select clause.
     * @param _OrderSimple
     *            Vector with specific order by fields. It always contains
     *            tablename.field, never SQL clause
     * @param startPosition
    ...
    public String getSQL(Vector<String> _FilterFields,
        ...
        String keyColumns = getKeyColumns(getPool().getRDBMS()
                .equalsIgnoreCase("ORACLE") ? "" : getTableName());
        String keyColumnsTable = getKeyColumns(getTableName());
        txtAuxWhere.append("(").append(keyColumnsTable).append(") IN (SELECT ")
                .append(keyColumns).append("\n");
        ...
        if (_OrderSimple != null) {
            ...
            for (int i = 0; i < _OrderSimple.size(); i++) {
                ...
                if (auxStructure.indexOf(" ") != -1)
                    orderField = auxStructure.substring(0, auxStructure
                            .indexOf(" ")); // tablename.fieldname
                if (auxFrom != null) {
                    for (int j = 0; j < auxFrom.size(); j++) {
                        QueryFieldStructure auxStructureFrom = auxFrom
                                .elementAt(j);
                        if ((!auxStructureFrom.getRealName().equals(
                                getTableName()))
                                && (auxStructureFrom.getRealName().equals(
                                        orderField) || (keyColumnsTable
                                        .indexOf(auxStructureFrom.getRealName()) != -1))) {
                            txtAuxWhere.append("left join ");
                            txtAuxWhere.append(auxStructureFrom.toString())
                                    .append(" \n");
                        }
                    }
                }
                if (auxParam != null) {
                    for (int j = 0; j < auxParam.size(); j++) {
                        QueryParameterStructure auxParamFrom = auxParam
                                .elementAt(j);
                        if ((!auxParamFrom.getRealName().equals(getTableName()))
                                && (auxParamFrom.getRealName().equals(
                                        orderField) || (keyColumnsTable
                                        .indexOf(auxParamFrom.getRealName()) != -1))) {
                            addSubQueryParameter(auxParamFrom.getName(),
                                    auxParamFrom.getField(), "JOIN2"
        ...
        if (sorted) {
            if (aux != null) {
                ...
                if (!hasOrder)
                    ...
                    txtAuxOrderBy.append(", ");
                hasOrder = true;
                txtAuxOrderBy.append(getTableName()).append(".").append(
                        getKeyColumn());
            }
        }
        if (hasWhere) {
            ...
            if (hasRange || onlyId)
                txtAuxWhere.append(txtAuxOrderBy.toString()); // Internal order
                                                              ...
                                                              // onlyId case
            if ((!hasRange)
                    || (getPool().getRDBMS().equalsIgnoreCase("ORACLE")))
                txtAuxWhere.append(")\n");
            if (hasRange) {
                // wrap end SQL
                if (getPool().getRDBMS().equalsIgnoreCase("ORACLE")) {
                ...
                try {
                    if (getPool().getRDBMS().equalsIgnoreCase("ORACLE")) {
                        ...
                        setParameter("ORAstartPosition", Integer
                                .toString(startPosition + 1));
                        if (hasRangeLimit) {
                            addWrapperParameter("ORArangeLimit", "RANGE", "");
                            setParameter("ORArangeLimit", Integer
                                    .toString(startPosition + rangeLength));
                        }
                    } else {
                        if (hasRangeLimit) {
                            addWrapperParameter("PGrangeLength", "RANGE", "");
                            setParameter("PGrangeLength", Integer
                                    .toString(rangeLength));
                            addWrapperParameter("PGstart", "RANGE", "");
                            setParameter("PGstart", Integer
            ...
            /*
             * When only interested in ordered sequence of id values we need
             * only the inner subselect of the previously assembled query. This
             * block massages the inner select into an complete executable sql
             * statement and uses this instead of the bigger one.
             */
            if (onlyId) {
