...
public class EmailManager
{
	 ...
	 * 
	 */
	public static Session newMailSession( ConnectionProvider connectionProvider, String clientId ) throws PocException
	{
		PocConfigurationData configurations[];
		try
		{
			configurations = PocConfigurationData.getSmtpDetails( connectionProvider, clientId );
		}
		...
		if( configurations.length > 0 )
		{
			...
			if (log4j.isDebugEnabled()) log4j.debug( "Crm configuration, smtp server: " + configuration.smtpserver );
			if (log4j.isDebugEnabled()) log4j.debug( "Crm configuration, smtp server auth: " + configuration.issmtpauthorization );			
			if (log4j.isDebugEnabled()) log4j.debug( "Crm configuration, smtp server account: " + configuration.smtpserveraccount );
			if (log4j.isDebugEnabled()) log4j.debug( "Crm configuration, smtp server password: " + configuration.smtpserverpassword );
		}
		else
		{
			//throw new ServletException( "No CRM configuration found for this client." );
			if (log4j.isDebugEnabled()) log4j.debug( "No CRM configuration found for this client." );
		}
		...
		{
			authenticator = new ClientAuthenticator( configuration.smtpserveraccount, configuration.smtpserverpassword );
		}
	 ...
	 * 
	 */
	public void sendSimpleEmail( Session session, String from, String to, String bcc, String subject, String body, String attachmentFileLocations ) throws PocException
	{	    
		try
		{
	...
	private InternetAddress[] getAddressesFrom( String[] textualAddresses )
	{
		...
		for (int index = 0; index < textualAddresses.length; index++ )
		{
			...
			{
				if (log4j.isDebugEnabled()) log4j.debug( "Could not create a valid email for: " + textualAddresses[index] + ". Address ignored" );
			}
		}
