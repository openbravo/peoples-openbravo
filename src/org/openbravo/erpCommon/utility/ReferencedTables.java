...
public class ReferencedTables {
    ...
    public ReferencedTables(ConnectionProvider _conn, String _adTableId,
            String _keyName, String _keyId) throws ServletException {
        if (_adTableId == null || _adTableId.equals("") || _keyName == null
                || _keyName.equals("")) {
    ...
    private void process() throws ServletException {
        String tableName = ReferencedTablesData
                .selectTableName(conn, adTableId);
        if (keyId != null && !keyId.equals("")) {
            if (adTableId.equals("800018")) { // C_Debt_Payment
                if (log4j.isDebugEnabled())
                    log4j.debug("DP");
                String invoiceId = ReferencedTablesData.selectKeyId(conn,
                        "C_INVOICE_ID", tableName, keyName, keyId);
                if (!invoiceId.equals("")) {
                    if (log4j.isDebugEnabled())
                        log4j.debug("InvoiceId: " + invoiceId);
                    String newAdTableId = ReferencedTablesData.selectTableId(
                            conn, "C_Invoice");
                    ReferencedTables ref = new ReferencedTables(conn,
                            newAdTableId, "C_Invoice_ID", invoiceId);
                    hassotrx = ref.hasSOTrx();
                    sotrx = ref.isSOTrx();
                    ref = null;
                } else {
                    String orderId = ReferencedTablesData.selectKeyId(conn,
                            "C_ORDER_ID", tableName, keyName, keyId);
                    if (!orderId.equals("")) {
                        if (log4j.isDebugEnabled())
                            log4j.debug("OrderId: " + orderId);
                        String newAdTableId = ReferencedTablesData
                                .selectTableId(conn, "C_Order");
                        ReferencedTables ref = new ReferencedTables(conn,
                                newAdTableId, "C_Order_ID", orderId);
                        hassotrx = ref.hasSOTrx();
                        sotrx = ref.isSOTrx();
                        ref = null;
                    } else {
                ...
                sotrx = ReferencedTablesData.selectSOTrx(conn, tableName,
                        keyName, keyId);
            } else
    ...
    private void checkParent(String tableName, String filterField)
            throws ServletException {
        ReferencedTablesData[] data = ReferencedTablesData.select(conn,
                filterField, adTableId);
        if (keyName.equalsIgnoreCase("C_DocTypeTarget_ID"))
            keyName = "C_DocType_ID";
        else if (keyName.equalsIgnoreCase("PO_Window_ID"))
            keyName = "AD_Window_ID";
        ...
        if (data != null && data.length > 0) {
            for (int i = 0; i < data.length; i++) {
                Vector<Object> vecReference = getTableNameReferenced(
                        data[i].columnname, data[i].adReferenceId,
                        data[i].adReferenceValueId);
                String newKeyId = ReferencedTablesData.selectKeyId(conn,
                        data[i].columnname, tableName, keyName, keyId);
                if (vecReference != null && vecReference.size() > 0) {
                    String newAdTableId = ReferencedTablesData.selectTableId(
                            conn, ((String) vecReference.elementAt(0)));
                    try {
                        ReferencedTables ref = new ReferencedTables(conn,
                                newAdTableId, ((String) vecReference
    ...
    private Vector<Object> getTableNameReferenced(String columnname,
            String adReferenceId, String adReferenceValueId)
            throws ServletException {
        ...
        case 800013:
            ...
            break;
        case 18:
            ReferencedTablesData[] data = ReferencedTablesData.selectRefTable(
                    conn, adReferenceValueId);
            if (data != null && data.length > 0) {
