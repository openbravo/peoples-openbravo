...
public class ReferencedTables {
  ...
  public ReferencedTables(ConnectionProvider _conn, String _adTableId, String _keyName, String _keyId) throws ServletException {
  ...
  private void process() throws ServletException {
    ...
    if (keyId!=null && !keyId.equals("")) {
      if (adTableId.equals("800018")) { //C_Debt_Payment
        ...
        String invoiceId = ReferencedTablesData.selectKeyId(conn, "C_INVOICE_ID", tableName, keyName, keyId);
        if (!invoiceId.equals("")) {
          if (log4j.isDebugEnabled()) log4j.debug("InvoiceId: "+invoiceId);
          ...
          String orderId = ReferencedTablesData.selectKeyId(conn, "C_ORDER_ID", tableName, keyName, keyId);
          if (!orderId.equals("")) {
            if (log4j.isDebugEnabled()) log4j.debug("OrderId: "+orderId);
            ...
            if (log4j.isDebugEnabled()) log4j.debug("Settlement");
        ...
        hassotrx = true;
        if (keyName.equalsIgnoreCase("C_DocTypeTarget_ID")) keyName = "C_DocType_ID";
        else if (keyName.equalsIgnoreCase("PO_Window_ID")) keyName = "AD_Window_ID";
        else if (keyName.equalsIgnoreCase("BillTo_ID")) keyName = "C_BPartner_Location_ID";
        sotrx = ReferencedTablesData.selectSOTrx(conn, tableName, keyName, keyId);
      } else checkParent(tableName, "");
    }
  }
  ...
  private void checkParent(String tableName, String filterField) throws ServletException {
    ...
    else if ((keyName.equalsIgnoreCase("SO_Bankaccount_ID")) || (keyName.equalsIgnoreCase("PO_Bankaccount_ID"))) keyName="C_BankAccount_ID";
    if (data!=null && data.length>0) {
      for (int i=0;i<data.length;i++) {
        Vector<Object> vecReference = getTableNameReferenced(data[i].columnname, data[i].adReferenceId, data[i].adReferenceValueId);
        String newKeyId = ReferencedTablesData.selectKeyId(conn, data[i].columnname, tableName, keyName, keyId);
        if (vecReference!=null && vecReference.size()>0) {
          String newAdTableId = ReferencedTablesData.selectTableId(conn, ((String)vecReference.elementAt(0)));
          try {
            ReferencedTables ref = new ReferencedTables(conn, newAdTableId, ((String)vecReference.elementAt(1)), newKeyId);
            if (ref.hasSOTrx()) {
              hassotrx = ref.hasSOTrx();
              sotrx = ref.isSOTrx();
              break;
  ...
  private Vector<Object> getTableNameReferenced(String columnname, String adReferenceId, String adReferenceValueId) throws ServletException {
