 ...
 * are needed.
 * 
 * It is able to show a HTML component with the tree, to expand nodes Ajax calls
 * are used, these calls are managed by GenericTreeServlet.
 * 
 * <br>
 * Usage</br> This class cannot be directly used, instead a subclass
 * implementing the abstract method must be used.
 * 
 * The purpose is to set the toHml directly in the HTML where the tree is going
 * to be displayed, additionally genericTree.js must be imported in that page.
 * These java classes will show the tree and manage the ajax requests.
 * 
 * <br>
 * To take into account</br>
 * 
 * -data is a FieldProvider that must have the following fields: -nodeID : It is
 * the unique identifier for the node -name : It is the short name for the node
 * (it will be displayed in the tree) -display : It indicates whether the node
 * has child elements in order to show or not the expand/collapse button, the
 * values it can contain are: -block: To show the button -none : Not to show it
 * -linkname : In case this element is passed a link will appear next to the
 * node name, linkname is the text to display in this link. -linkclick: Is the
 * onclick action that will have the link. -levelno : It is the deep level for
 * the current node -icon : It is the css class to display the main icon for the
 * node -icon2 : It is another css class to display another icon over the main
 * icon Additionally to these fields that must be populated by the subclasses,
 * some helper methods that can be passed as empty String and because will be
 * caculated by the super class are needed, they are: -leveltree: It is a set of
 * divs to move the current node depending on its deep. -endline: Is a div at
 * the end of the dotted line -position: It is the node position relative to the
 * tree in order to allow keyboard navigation -It is necessary to implement in
 * subclasses apart of the abstract method a constructor without parameters,
 * this constructor is used by GenericTreeServlet.
 * 
 */
public abstract class GenericTree {
    ...
    /**
     * This method will assign to data the child nodes (just one level) for the
     * nodeId
     * 
     * @param nodeId
    ...
    /**
     * Default constructor without parameters. It is needed to be able to create
     * instances by GenericTreeServlet, it must be implemented also by
     * subclases.
     */
    public GenericTree() {
    }
    ...
    /**
     * This constructor receives a HttpBaseServlet object to set the
    ...
    /**
     * Sets the infrastructure parameters from the HttpBaseServlet object
     * passed.
     * 
     * @param base
    ...
    /**
     * Sets the isSubTree variable to the passed value. Subtrees have different
     * treatment in interface.
     * 
     * @param value
     */
    ...
    public String toHtml() {
        ...
        if (!isSubTree) {
            ...
            xmlDocument = xmlEngine.readXmlTemplate(
                    "org/openbravo/erpCommon/utility/GenericTreeHeader",
            ...
            String[] discard = { "inpNodes_xx" }; // remove check-box for
                                                  // subtree nodes
            xmlDocument = xmlEngine.readXmlTemplate(
                    "org/openbravo/erpCommon/utility/GenericTree", discard)
                    .createXmlDocument();
    ...
    public String descriptionToHtml() {
        XmlDocument xmlDocument = xmlEngine.readXmlTemplate(
                "org/openbravo/erpCommon/utility/GenericTreeDescription")
                .createXmlDocument();
        xmlDocument.setParameter("genericDescriptionSize",
                genericDescriptionSize);
        return xmlDocument.print();
    }
    ...
    protected void setLevel(int l) {
        ...
        for (int i = 0; i < data.length; i++) {
            ...
            if (!isSubTree && i == 0) {
                FieldProviderFactory.setField(data[i], "endline",
            ...
            FieldProviderFactory.setField(data[i], "levelno", (new Integer(l)
                    .toString()));
            ...
            for (int j = l - 1; j >= 0; j--) {
                ...
                String divClass = "Tree_"
                        + (isLastLevelNode(parentID) ? "LastParent"
                                : "MiddleParent") + "_Spots";
                FieldProviderFactory.setField(data[i], "leveltree",
                        "<div class=\"" + divClass + "\"></div>"
                                + data[i].getField("leveltree"));
            }
            ...
            for (int j = l; j >= 0; j--) {
                ...
                position = getNodePosition(parentID)
    ...
    private boolean isLastLevelNode(String nodeID) {
        try {
            ...
            if (parentNodeID.equals(""))
                tree = ModuleTreeData.select(conn, (lang.equals("") ? "en_US"
                        : lang)); // Root
            else
                tree = ModuleTreeData.selectSubTree(conn,
                        (lang.equals("") ? "en_US" : lang), parentNodeID); // Subtree
