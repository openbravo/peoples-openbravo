 ...
 * 
 * It is able to show a HTML component with the tree, to expand nodes Ajax calls are used, 
 * these calls are managed by GenericTreeServlet.
 * 
 * <br>Usage</br>
 * This class cannot be directly used, instead a subclass implementing the abstract method must
 * be used.
 * 
 ...
 * 
 * <br>To take into account</br>
 *
 * -data is a FieldProvider that must have the following fields:
 *   -nodeID   : It is the unique identifier for the node
 *   -name     : It is the short name for the node (it will be displayed in the tree)
 *   -display  : It indicates whether the node has child elements in order to show or not the 
 *               expand/collapse button, the values it can contain are:
 *                -block: To show the button
 *                -none : Not to show it
 *   -linkname : In case this element is passed a link will appear next to the node name, linkname
 *               is the text to display in this link.
 *   -linkclick: Is the onclick action that will have the link.
 *   -levelno  : It is the deep level for the current node
 *   -icon     : It is the css class to display the main icon for the node
 *   -icon2    : It is another css class to display another icon over the main icon 
 *   Additionally to these fields that must be populated by the subclasses, some helper methods that
 *   can be passed as empty String and because will be caculated by the super class are needed, they
 *   are:
 *    -leveltree: It is a set of divs to move the current node depending on its deep. 
 *    -endline:   Is a div at the end of the dotted line
 *    -position:  It is the node position relative to the tree in order to allow keyboard navigation
 * -It is necessary to implement in subclasses apart of the abstract method a constructor without 
 *  parameters, this constructor is used by GenericTreeServlet.
 *  
 */
public abstract class GenericTree {
   ...
   * @param nodeId Root node for the subtree
   */
   ...
   * @param base HttpBaseServlet object to obtain the parameters from
   */  
  ...
  /**
   * Sets the isSubTree variable to the passed value.
   * Subtrees have different treatment in interface.
   * 
   * @param value
   */
  ...
  public String toHtml(){
    ...
    if (!isSubTree) {
      String [] discard={""};
      if (!showNotifications) discard[0]="notifArea";
      xmlDocument=xmlEngine.readXmlTemplate("org/openbravo/erpCommon/utility/GenericTreeHeader", discard).createXmlDocument();
      XmlDocument xmlDocumentTree=xmlEngine.readXmlTemplate("org/openbravo/erpCommon/utility/GenericTree").createXmlDocument();
      ...
      String[] discard={"inpNodes_xx"};  //remove check-box for subtree nodes
      xmlDocument=xmlEngine.readXmlTemplate("org/openbravo/erpCommon/utility/GenericTree",discard).createXmlDocument();
  ...
  public String descriptionToHtml(){
    XmlDocument xmlDocument=xmlEngine.readXmlTemplate("org/openbravo/erpCommon/utility/GenericTreeDescription").createXmlDocument();
  ...
  protected void setLevel(int l){
    ...
    for (int i=0; i<data.length; i++) {
      ...
      String parentID = data[i].getField("nodeId"); //set the current node to look for its parents
      for (int j=l-1; j>=0; j--) {
        parentID = getParent(parentID);
        String divClass = "Tree_"+(isLastLevelNode(parentID)?"LastParent":"MiddleParent")+"_Spots";
        FieldProviderFactory.setField(data[i],"leveltree","<div class=\""+divClass+"\"></div>"+ data[i].getField("leveltree"));
      }
      
      //Position: this is in order to be able to use keyboard to navigate in the tree UI
      String position = "";
      parentID = data[i].getField("nodeId"); //set the current node to look for its parents
      for (int j=l; j>=0; j--) {
   ...
   * 
   * @param notifications HTML string with the text and link
   */
