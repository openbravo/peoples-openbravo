 ...
 *         error parsing for POSTGRESQL RDBMS.
 */
        ...
        if (pos != -1) {
            ...
            if (pos != -1) {
                ...
                if (pos != -1) {
                    ...
                    ErrorTextParserPOSTGREData[] constraintData = ErrorTextParserPOSTGREData
                    ...
                    if (constraintData != null && constraintData.length > 0) {
                        ...
                        FieldProvider fldMessage = Utility
                                .locateMessage(getConnection(),
                                        constraintData[0].constraintName,
                                        getLanguage());
                        if (fldMessage != null) {
                            myError = new OBError();
                        ...
                        if (constraintData[0].constraintType
                                .equalsIgnoreCase("C")
                            ...
                            if (fldMessage != null) {
                                ...
                                myError.setType((fldMessage.getField("msgtype")
                                        .equals("E") ? "Error" : "Warning"));
                                myError.setMessage(fldMessage
                                        .getField("msgtext"));
                                return myError;
                            } else if (!constraintData[0].searchCondition
                                    .trim().equals("")) {
                                String searchCond = constraintData[0].searchCondition
                                        .trim().toUpperCase();
                                if (searchCond.endsWith(" IS NOT NULL")) {
                                    String columnName = searchCond
                                            .substring(
                                                    0,
                                                    searchCond
                                                            .lastIndexOf(" IS NOT NULL"))
                                            .trim();
                                    columnName = Utility.messageBD(
                                            getConnection(), columnName,
                                            getLanguage());
                                    ...
                                    myError.setMessage(Utility.messageBD(
                                            getConnection(), "NotNullError",
                                            getLanguage())
                                            + ": "
                                            + tableName
                                            + " - "
