...
public class MessagesJS extends HttpSecureAppServlet {
  ...
  private void printPageDataSheet(HttpServletResponse response, VariablesSecureApp vars) throws IOException, ServletException {
    ...
    try {
      ...
      log4j.error("Error in MessagesJS.printPageDataSheet(): " + myError.getTitle() + " - " + myError.getMessage());
      return;
    }
    ...
    if (data!=null && data.length!=0) {
      for (int i=0;i<data.length;i++) {
        ...
        arrayType.append("new messageType(\"").append(num).append("\",").append(data[i].msgtype.equals("C")?"1":"0").append(")");
        array.append("new messagesTexts(\"").append(vars.getLanguage()).append("\", \"");
        array.append(num).append("\", \"").append(FormatUtilities.replaceJS(data[i].msgtext)).append("\", null)");
      }
    ...
    if (log4j.isDebugEnabled()) log4j.debug(arrayType.toString() + array.toString());
