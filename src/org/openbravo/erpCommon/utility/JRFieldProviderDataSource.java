            ...
            try {
                if (clazz.equals(java.lang.Boolean.class)) {
                    if (value != null && !value.equals("")) {
                        objValue = value.equals("Y") ? Boolean.TRUE
                                : Boolean.FALSE;
                    } else
                        objValue = null;
                ...
                /*
                 * else if (clazz.equals(java.io.InputStream.class)) { byte[]
                 * bytes = readBytes(columnIndex);
                 * 
                 * if(bytes == null) { objValue = null; } else { objValue = new
                 * ByteArrayInputStream(bytes); } }
                 */
                /*
                 * else if (clazz.equals(java.lang.Long.class)) { objValue = new
                 * Long(resultSet.getLong(columnIndex.intValue()));
                 * if(resultSet.wasNull()) { objValue = null; } }
                 */
                /*
                 * else if (clazz.equals(java.lang.Short.class)) { objValue =
                 * new Short(resultSet.getShort(columnIndex.intValue()));
                 * if(resultSet.wasNull()) { objValue = null; } }
                 */
                 ...
                 * resultSet.getClob(columnIndex.intValue());
                 * if(resultSet.wasNull()) { objValue = null; } }
                 */
                /*
                 * else if (clazz.equals(Reader.class)) { Reader reader = null;
                 * long size = -1;
                 * 
                 * int columnType =
                 * resultSet.getMetaData().getColumnType(columnIndex
                 * .intValue()); switch (columnType) { case Types.CLOB: Clob
                 * clob = resultSet.getClob(columnIndex.intValue()); if
                 * (!resultSet.wasNull()) { reader = clob.getCharacterStream();
                 * size = clob.length(); } break;
                 * 
                 * default: reader =
                 * resultSet.getCharacterStream(columnIndex.intValue()); if
                 * (resultSet.wasNull()) { reader = null; } }
                 * 
                 * if (reader == null) { objValue = null; } else { objValue =
                 * getArrayReader(reader, size); } }
                 */
                /*
                 * else if (clazz.equals(Blob.class)) { objValue =
                 * resultSet.getBlob(columnIndex.intValue());
                 * if(resultSet.wasNull()) { objValue = null; } }
                 */
                /*
                 * else if (clazz.equals(Image.class)) { byte[] bytes =
                 * readBytes(columnIndex);
                 * 
                 * if(bytes == null) { objValue = null; } else { objValue =
                 * JRImageLoader.loadImage(bytes); } }
                 */
                else {
                    if (value != null && !value.equals("")) {
                        objValue = new String(value);
