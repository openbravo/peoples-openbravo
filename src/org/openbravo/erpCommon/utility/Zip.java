...
public class Zip {
    ...
    private static void zip(File[] list, ZipOutputStream dest,
        ...
        for (int i = 0; i < list.length; i++) {
            if (list[i].isDirectory()) {
                ...
                dest.putNextEntry(new ZipEntry(list[i].toString().replace(
                        relativeDir, "")));
    ...
    private static void zip(File orig, ZipOutputStream dest, String relativeDir)
            throws Exception {
    ...
    public static void zip(File[] list, String dest, String relativeDir)
            throws Exception {
        ZipOutputStream destZip = new ZipOutputStream(
                new FileOutputStream(dest));
        relativeDir = (relativeDir + (relativeDir.endsWith("/") ? "" : "/"))
    ...
    public static void zip(String orig, String dest, String relativeDir)
            throws Exception {
