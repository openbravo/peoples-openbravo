...
public class PrintController extends HttpSecureAppServlet {
  ...
  protected void post(HttpServletRequest request, HttpServletResponse response, VariablesSecureApp vars, DocumentType documentType, String sessionValuePrefix, String strDocumentId) throws IOException, ServletException {
    ...
    final ReportManager reportManager = new ReportManager(this, globalParameters.strFTPDirectory, strReplaceWithFull, globalParameters.strBaseDesignPath, globalParameters.strDefaultDesignPath, globalParameters.prefix, classInfo);
    ...
    if (vars.commandIn("PRINT") || vars.commandIn("ARCHIVE") || (!request.getRequestURI().endsWith(".html"))) {
      ...
      if (!report.isDraft() && !report.isAttached() && vars.commandIn("ARCHIVE")) {
        ...
        if (log4j.isDebugEnabled())
          log4j.debug("Table " + report.getDocumentType().getTableName() + " has table id: " + tableId);
      ...
      if (vars.commandIn("DEFAULT")) {
        ...
        for (int index = 0; index < documentIds.length; index++) {
          ...
          try {
            final Report report = new Report(this, documentType, documentId, vars.getLanguage(), "default");
            ...
            String senderAddress = PocConfigurationData.getSenderAddress(this, vars.getClient(), report.getOrgId());
            if (moreThanOnesalesRep) {
              if ("".equals(senderAddress) || senderAddress == null) {
                final OBError on = new OBError();
                on.setMessage(Utility.messageBD(this, "No sender defined: Please go to client configuration to complete de email configuration", vars.getLanguage()));
        ...
        if (request.getServletPath().toLowerCase().indexOf("print.html") != -1)
          createPrintOptionsPage(request, response, vars, documentType, getComaSeparatedString(documentIds), reports);
        else
          createEmailOptionsPage(request, response, vars, documentType, getComaSeparatedString(documentIds), reports);
        ...
        final Vector<java.lang.Object> vector = (Vector<java.lang.Object>) request.getSession().getAttribute("files");
        ...
        for (final PocData documentData : pocData) {
          ...
          if (differentDocTypes.size() == 1) {
            ...
            try {
              final TemplateInfo usedTemplateInfo = new TemplateInfo(this, report.getDocTypeId(), report.getOrgId(), vars.getLanguage(), templateId);
          ...
          if (!report.isDraft()) {
            ...
            String senderAddress = PocConfigurationData.getSenderAddress(this, vars.getClient(), report.getOrgId());
            sendDocumentEmail(report, vars, request.getSession().getAttribute("files"), documentData, senderAddress);
            nrOfEmailsSend++;
          }
        }
  ...
  protected void sendDocumentEmail(Report report, VariablesSecureApp vars, Object object, PocData documentData, String senderAddess) throws IOException, ServletException {
    ...
    try {
      ...
      try {
        ...
        EmailData.insertEmail(conn, this, newEmailId, clientId, organizationId, userId, EmailType.OUTGOING.getStringValue(), from, to, cc, bcc, dateOfEmail, subject, body, bPartnerId);
  ...
  void createPrintOptionsPage(HttpServletRequest request, HttpServletResponse response, VariablesSecureApp vars, DocumentType documentType, String strDocumentId, Map<String, Report> reports) throws IOException, ServletException {
  ...
  void createEmailOptionsPage(HttpServletRequest request, HttpServletResponse response, VariablesSecureApp vars, DocumentType documentType, String strDocumentId, Map<String, Report> reports) throws IOException, ServletException {
    XmlDocument xmlDocument = null;
    ...
    if (hiddenTags != null) {
      xmlDocument = xmlEngine.readXmlTemplate("org/openbravo/erpCommon/utility/reporting/printing/EmailOptions", hiddenTags).createXmlDocument();
    } else {
    ...
    for (final PocData documentData : pocData) {
      ...
      if (customer == null || customer.length() == 0) {
        final OBError on = new OBError();
        on.setMessage(Utility.messageBD(this, "There is at least one document with no contact", vars.getLanguage()));
      ...
      if (moreThanOnesalesRep) {
        if (salesRep == null || salesRep.length() == 0) {
          final OBError on = new OBError();
          on.setMessage(Utility.messageBD(this, "There is at least one document with no sales rep.", vars.getLanguage()));
          on.setTitle(Utility.messageBD(this, "Info", vars.getLanguage()));
          on.setType("info");
          String tabId = vars.getSessionValue("inpTabId");
          vars.getStringParameter("tab");
    ...
    if (!onlyOneAttachedDoc && isTheFirstEntry) {
      if (numberOfCustomers > 1) {
        attachedContent.setDocName(String.valueOf(reports.size() + " Documents to " + String.valueOf(numberOfCustomers) + " Customers"));
        attachedContent.setVisible("checkbox");
  ...
  void createPrintStatusPage(HttpServletResponse response, VariablesSecureApp vars, int nrOfEmailsSend) throws IOException, ServletException {
    XmlDocument xmlDocument = null;
