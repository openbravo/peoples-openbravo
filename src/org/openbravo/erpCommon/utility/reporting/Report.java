...
import org.openbravo.erpCommon.utility.reporting.ReportData;
...
public class Report
{
  ...
  public Report( ConnectionProvider connectionProvider, DocumentType documentType, String documentId , String strLanguage ) throws ReportingException, ServletException
  {
    ...
    {
      String orgId =  reportData[0].getField("ad_Org_Id");
      String docTypeId =  reportData[0].getField("docTypeTargetId");
      ...
      _TemplateInfo = new TemplateInfo( connectionProvider, docTypeId, orgId, strLanguage );
      
      _Filename =  generateReportFileName();
      _targetDirectory =  null;       
    }
    else
      throw new ReportingException( Utility.messageBD(connectionProvider, "NoDataReport", strLanguage) + documentId );
  ...
  private String generateReportFileName()
  {
    ...
    String dateStamp = Utility.formatDate( new Date(), "yyyyMMdd-HHmmss");
    ...
    if (log4j.isDebugEnabled()) log4j.debug( "target report filename: " + reportFilename );   
