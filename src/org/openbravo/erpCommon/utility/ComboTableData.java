 ...
 * 
 *         This class builds the queries for populating the different kind of
 *         combos in the application.
 */
public class ComboTableData {
     ...
     * @throws Exception
     */
    public ComboTableData(ConnectionProvider _conn, String _referenceType,
            String _name, String _objectReference, String _validation,
            String _orgList, String _clientList, int _index) throws Exception {
        this(null, _conn, _referenceType, _name, _objectReference, _validation,
                _orgList, _clientList, _index);
    }
     ...
     * @throws Exception
     */
    public ComboTableData(VariablesSecureApp _vars, ConnectionProvider _conn,
            String _referenceType, String _name, String _objectReference,
            String _validation, String _orgList, String _clientList, int _index)
            throws Exception {
        if (_vars != null)
            setVars(_vars);
        setPool(_conn);
    ...
    public void setObjectReference(String _reference) throws Exception {
        if (_reference != null && !_reference.equals("")) {
            try {
                ...
                if (!Utility.isUUIDString(_reference))
                    _reference = ComboTableQueryData.getReferenceID(getPool(),
                            _reference, (getReferenceType().equals("17") ? "L"
                                    : "T"));
            }
        }
    ...
    public void setValidation(String _reference) throws Exception {
        if (_reference != null && !_reference.equals("")) {
            try {
                ...
                if (!Utility.isUUIDString(_reference))
                    _reference = ComboTableQueryData.getValidationID(getPool(),
                            _reference);
            }
        }
    ...
    public void addSelectField(String _field, String _alias) {
        QueryFieldStructure p = new QueryFieldStructure(_field, " AS ", _alias,
                "SELECT");
        if (this.select == null)
    ...
    public void addFromField(String _field, String _alias) {
        QueryFieldStructure p = new QueryFieldStructure(_field, " ", _alias,
                "FROM");
        if (this.from == null)
    ...
    public void addOrderByField(String _field) {
        QueryFieldStructure p = new QueryFieldStructure(_field, "", "",
                "ORDERBY");
    ...
    public void addSelectParameter(String _parameter, String _fieldName) {
        ...
        QueryParameterStructure aux = new QueryParameterStructure(_parameter,
                _fieldName, "SELECT");
     ...
     */
    public void addWhereParameter(String _parameter, String _fieldName,
            String _type) {
     ...
     * @throws Exception
     */
    private void setListQuery(String tableName, String fieldName,
            String referenceValue) throws Exception {
        int myIndex = this.index++;
        addSelectField("td" + myIndex + ".value", "id");
        addSelectField("((CASE td" + myIndex + ".isActive WHEN 'N' THEN '"
                + INACTIVE_DATA + "' ELSE '' END) || (CASE WHEN td_trl"
                + myIndex + ".name IS NULL THEN td" + myIndex
                + ".name ELSE td_trl" + myIndex + ".name END))", "NAME");
        addSelectField("(CASE WHEN td_trl" + myIndex
                + ".description IS NULL THEN td" + myIndex
                + ".description ELSE td_trl" + myIndex + ".description END)",
                "DESCRIPTION");
        String tables = "ad_ref_list td" + myIndex;
        if (tableName != null && tableName.length() != 0 && fieldName != null
                && fieldName.length() != 0)
            tables += " on " + tableName + "." + fieldName + " = td" + myIndex
                    + ".value ";
        addFromField(tables, "td" + myIndex);
        addFromField("ad_ref_list_trl td_trl" + myIndex + " on td" + myIndex
                + ".ad_ref_list_id = td_trl" + myIndex
                + ".ad_ref_list_id AND td_trl" + myIndex + ".ad_language = ?",
                "td_trl" + myIndex);
        ...
        if (tableName == null || tableName.length() == 0) {
            parseValidation();
            addWhereField("(td" + myIndex + ".isActive = 'Y' OR td" + myIndex
                    + ".Value = ? )", "ISACTIVE");
     ...
     * @throws Exception
     */
    private void setTableQuery(String tableName, String fieldName,
            String referenceValue) throws Exception {
        int myIndex = this.index++;
        ComboTableQueryData trd[] = ComboTableQueryData.selectRefTable(
                getPool(), ((referenceValue != null && !referenceValue
                        .equals("")) ? referenceValue : getObjectReference()));
        ...
        if (tableName != null && !tableName.equals("") && fieldName != null
            ...
            tables += " on " + tableName + "." + fieldName + " = td" + myIndex
                    + "." + trd[0].keyname + " \n";
            tables += "AND td" + myIndex + ".AD_Client_ID IN ("
                    + getClientList() + ") \n";
            tables += "AND td" + myIndex + ".AD_Org_ID IN (" + getOrgList()
                    + ")";
        } else {
            addWhereField("td" + myIndex + ".AD_Client_ID IN ("
                    + getClientList() + ")", "CLIENT_LIST");
            if (getOrgList() != null)
                addWhereField("td" + myIndex + ".AD_Org_ID IN (" + getOrgList()
                        + ")", "ORG_LIST");
        }
        addFromField(tables, "td" + myIndex);
        ...
        String orderByAux = (trd[0].orderbyclause.equals("") ? "2"
                : trd[0].orderbyclause);
     ...
     * @throws Exception
     */
    private void setTableDirQuery(String tableName, String fieldName,
        ...
        if (tableName != null && !tableName.equals("")
                && parentFieldName != null && !parentFieldName.equals("")) {
            tables += " on " + tableName + "." + parentFieldName + " = td"
                    + myIndex + "." + name + "\n";
            tables += "AND td" + myIndex + ".AD_Client_ID IN ("
                    + getClientList() + ") \n";
            tables += "AND td" + myIndex + ".AD_Org_ID IN (" + getOrgList()
    ...
    /**
     * Method to fix the names of the fields. Searchs all the fields in the
     * where clause and order by clause to change the names with correct
     * aliases. This intends to fix the problem of the names in the
     * whereclauses, filterclauses and orderbyclauses fields of the tab's table,
     * where the user doesn´t know the alias of the referenced field.
     */
            ...
            for (int i = 0; i < where.size(); i++) {
                ...
                if (auxStructure.getType().equalsIgnoreCase("FILTER")) {
                    ...
                    for (int j = 0; j < tables.size(); j++) {
                        ...
                        strAux = replaceIgnoreCase(strAux, strTable + ".",
                                auxTable.getAlias() + ".");
                    }
        ...
        if (orderBy != null && orderBy.size() > 0) {
            for (int i = 0; i < orderBy.size(); i++) {
                ...
                for (int j = 0; j < tables.size(); j++) {
                    ...
                    if (p != -1)
                        strTable = strTable.substring(0, p).trim();
                    strAux = replaceIgnoreCase(strAux, strTable + ".", auxTable
                            .getAlias()
                            + ".");
                }
                if (!strAux.equalsIgnoreCase(auxStructure.getField())) {
     ...
     */
    private String replaceIgnoreCase(String data, String replaceWhat,
            String replaceWith) {
        if (data == null || data.equals(""))
            return "";
        if (log4j.isDebugEnabled())
            log4j.debug("parsing data: " + data + " - replace: " + replaceWhat
                    + " - with: " + replaceWith);
    ...
    /**
     * Parse the validation string searching the @ elements and replacing them
     * with the correct values, adding the needed parameters.
     * 
     * @throws Exception
     */
    private void parseValidation() throws Exception {
        ...
        String val = ComboTableQueryData.getValidation(getPool(),
                getValidation());
        if (log4j.isDebugEnabled())
     ...
     * @throws Exception
     */
    public void identifier(String tableName, FieldProvider field)
            throws Exception {
        String reference;
        if (field == null)
        ...
        case 17: // List
            setListQuery(tableName, ((field == null) ? "" : field
                    .getField("name")), ((field == null) ? "" : field
                    .getField("referencevalue")));
            break;
        case 18: // Table
            setTableQuery(tableName, ((field == null) ? "" : field
                    .getField("name")), ((field == null) ? "" : field
                    .getField("referencevalue")));
            break;
        case 19: // TableDir
            setTableDirQuery(tableName, ((field == null) ? "" : field
                    .getField("name")), ((field == null) ? "" : field
                    .getField("name")));
            break;
        case 30: // Search
        ...
        case 31: // Locator
            setTableDirQuery(
                    tableName,
                    "M_Locator_ID",
                    ((field == null) ? getObjectName() : field.getField("name")));
            break;
        case 35:
            setTableDirQuery(tableName, ((field == null) ? "" : field
                    .getField("name")), ((field == null) ? "" : field
        ...
        default:
            if (!checkTableTranslation(tableName, field, reference)) {
                addSelectField(formatField((((tableName != null && tableName
                        .length() != 0) ? (tableName + ".") : "") + field
    ...
    /**
     * Checks if the table has a translated table, making the joins to the
     * translated one.
     * 
     * @param tableName
     *            Name of the table.
     ...
     * @throws Exception
     */
    private boolean checkTableTranslation(String tableName,
            FieldProvider field, String reference) throws Exception {
        if (tableName == null || tableName.equals("") || field == null)
            return false;
        ComboTableQueryData[] data = ComboTableQueryData
                .selectTranslatedColumn(getPool(), field.getField("tablename"),
                        field.getField("name"));
        if (data == null || data.length == 0)
            return false;
        int myIndex = this.index++;
        addSelectField("(CASE WHEN td_trl"
                + myIndex
                + "."
                + data[0].columnname
                + " IS NULL THEN "
                + formatField((tableName + "." + field.getField("name")),
                        reference)
                + " ELSE "
                + formatField(("td_trl" + myIndex + "." + data[0].columnname),
                        reference) + " END)", "NAME");
        addFromField(data[0].tablename + " td_trl" + myIndex + " on "
                + tableName + "." + data[0].reference + " = td_trl" + myIndex
                + "." + data[0].reference + " AND td_trl" + myIndex
    ...
    /**
     * Formating method to add the correct functions to the fields for the
     * presentation. It depends on the type of field (DATE, NUMBER...).
     * 
     * @param field
     *            Name of the field.
     * @param reference
     ...
     * 
     * @param onlyId
     *            Boolean to indicate if the select clause must have only the
     *            key field.
     * @param discard
        ...
        if (aux != null) {
            ...
            for (int i = 0; i < aux.size(); i++) {
                ...
                if (!isInArray(discard, auxStructure.getType())) {
                    if (auxStructure.getData("alias").equalsIgnoreCase("ID")) {
                    ...
                    } else if (auxStructure.getData("alias").equalsIgnoreCase(
                            "DESCRIPTION")) {
                        ...
                        name.append("COALESCE(TO_CHAR(").append(
                                auxStructure.toString()).append("),'')");
                    }
                }
            }
            if (!name.toString().equals(""))
    ...
    private boolean isInArray(String[] data, String element) {
        if (data == null || data.length == 0 || element == null
                || element.equals(""))
            return false;
     ...
     */
    private int setSQLParameters(PreparedStatement st, int iParameter,
            String[] discard) {
        Vector<QueryParameterStructure> vAux = getSelectParameters();
        if (vAux != null) {
            for (int i = 0; i < vAux.size(); i++) {
                ...
                if (!isInArray(discard, aux.getType())) {
                    String strAux = getParameter(aux.getName());
                    if (log4j.isDebugEnabled())
                        log4j.debug("Parameter - " + iParameter + " - "
                                + aux.getName() + ": " + strAux);
     ...
     * 
     * @param includeActual
     *            Boolean that indicates if the actual selected value must be
     *            included in the result, even if it doesn´t exists in the new
     *            query.
    ...
    public FieldProvider[] select(boolean includeActual) throws Exception {
        ...
        try {
            ...
            while (result.next()) {
                ...
                sqlReturnObject.setData("NAME", UtilSql
            ...
            if (includeActual && actual != null && !actual.equals("")
                ...
                String[] discard = { "filter", "orderBy", "CLIENT_LIST",
                        "ORG_LIST" };
                ...
                while (result.next()) {
                    SQLReturnObject sqlReturnObject = new SQLReturnObject();
                    sqlReturnObject.setData("ID", UtilSql
                            .getValue(result, "ID"));
                ...
                if (!idFound) {
                    ...
                    sqlReturnObject.setData("NAME", INACTIVE_DATA
                            + Utility.messageBD(getPool(), "NotFound",
