	...
	if (model == null) {
		...
		for (final Table t : tablesByTableName.values()) {
			...
			if (!c.isPrimitiveType()) {
				...
				thisProp.getEntity().getProperties().remove(
					thisProp);
				if (thisProp.getEntity().getIdProperties()
					.remove(thisProp)) {
				    Check
				...
				thisProp.getEntity().getIdentifierProperties()
					.remove(thisProp);
				continue;
			    }
			    // targetentity is set within setReferencedProperty
			    final Property thatProperty = thatColumn
				    .getProperty();
