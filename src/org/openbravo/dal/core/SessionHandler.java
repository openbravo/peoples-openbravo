...
public class SessionHandler {
  ...
  /**
   * Sets the session handler at rollback so that the controller can rollback at
   * the end
   */
   ...
   * Session.get method can not handle class names if the entity was mapped with
   * entitynames.
   */
  @SuppressWarnings("unchecked")
  public <T extends Object> T find(Class<T> clazz, Object id) {
    if (Identifiable.class.isAssignableFrom(clazz)) {
      return (T) find(ModelUtil.getEntityName(clazz), id);
    }
