...
public class DalMappingGenerator implements OBSingleton {
    private static final Logger log = Logger
            .getLogger(DalMappingGenerator.class);
    ...
    private String generateReferenceMapping(Property p) {
        if (p.getTargetEntity() == null) {
            return "<!-- Unsupported reference type " + p.getName()
                    + " of entity " + p.getEntity().getName() + "-->" + NL;
        }
        final StringBuffer sb = new StringBuffer();
        if (p.isOneToOne()) {
            final String name = p.getSimpleTypeName().substring(0, 1)
                    .toLowerCase()
            ...
            sb.append(TAB2 + "<many-to-one name=\"" + p.getName()
                    + "\" column=\"" + p.getColumnName() + "\""); // cascade=\
            // "save-update\"
            if (p.isMandatory()) {
                sb.append(" not-null=\"true\"");
            }
    ...
    private String generateOneToMany(Property p) {
        ...
        if (p.isOneToMany()) {
            if (p.getTargetEntity().getOrderByProperties().size() > 0) {
                ...
                for (final Property po : p.getTargetEntity()
            ...
            sb.append(TAB3 + "<key column=\""
                    + p.getReferencedProperty().getColumnName() + "\"/>" + NL);
    ...
    private String generateStandardID(Entity entity) {
        ...
        final StringBuffer sb = new StringBuffer();
        sb.append(TAB2 + "<id name=\"" + p.getName() + "\" type=\""
                + p.getPrimitiveType().getName() + "\" "
                + getAccessorAttribute() + " column=\"" + p.getColumnName()
                + "\">" + NL);
    ...
    private String generateCompositeID(Entity e) {
        Check
                .isTrue(e.hasCompositeId(),
                        "Method can only handle primary keys with more than one column");
        final StringBuffer sb = new StringBuffer();
        sb.append(TAB2 + "<composite-id name=\"id\" class=\""
                + e.getClassName() + "$Id\"" + getAccessorAttribute() + ">"
                + NL);
        final Property compId = e.getIdProperties().get(0);
        Check.isTrue(compId.isCompositeId(), "Property " + compId
                + " is expected to be a composite Id");
        for (final Property p : compId.getIdParts()) {
            if (p.isPrimitive()) {
                String type = p.getPrimitiveType().getName();
                if (boolean.class.isAssignableFrom(p.getPrimitiveType()
                        .getClass())
                        || Boolean.class == p.getPrimitiveType()) {
                    type = "yes_no";
                }
                sb.append(TAB3 + "<key-property name=\"" + p.getName()
                        + "\" column=\"" + p.getColumnName() + "\" type=\""
                        + type + "\"/>" + NL);
            } else {
                sb.append(TAB3 + "<key-many-to-one name=\"" + p.getName()
    ...
    private String readFile(String fileName) {
        try {
            final InputStreamReader fr = new InputStreamReader(getClass()
