...
public class FIN_Utility {
   ...
   * any FinAccPaymentMethod related to paymentMethodId FIN_PaymentMethod ordered by default field.
   * FinAccPaymentMethod must have pay in/out active and must be compatible with currencyId Currency
   * if currencyId is not null. If the organization parameter is not null the FinAccPaymentMethod
   * must be in the organization tree.
   */
  public static FinAccPaymentMethod getFinancialAccountPaymentMethodByOrganization(
      String paymentMethodId, String financialAccountId, boolean issotrx, String currencyId,
      String organizationId) {
    final OrganizationStructureProvider osp = OBContext.getOBContext()
        .getOrganizationStructureProvider(OBContext.getOBContext().getCurrentClient().getId());
    Set<String> orgTree = osp.getNaturalTree(organizationId);
    StringBuffer where = new StringBuffer();
    where.append(" select fapm.id from " + FinAccPaymentMethod.ENTITY_NAME + " as fapm");
    where.append(" join fapm." + FinAccPaymentMethod.PROPERTY_ACCOUNT + " as fa");
    where.append(" where fapm." + FinAccPaymentMethod.PROPERTY_PAYMENTMETHOD + " = :paymentMethod");
    where.append(" and fa." + FIN_FinancialAccount.PROPERTY_ACTIVE + " = true");
    if (issotrx) {
    ...
    if (!StringUtils.isEmpty(organizationId)) {
      where.append(" and (fa." + FIN_FinancialAccount.PROPERTY_ORGANIZATION
          + ".id in (:organizationTree))");
    }
    where.append(" and fapm.active = true ");
    where.append(" order by fapm." + FinAccPaymentMethod.PROPERTY_DEFAULT + " desc");

    final Query qry = OBDal.getInstance().getSession().createQuery(where.toString());
    qry.setParameter("paymentMethod",
        OBDal.getInstance().get(FIN_PaymentMethod.class, paymentMethodId));
    if (!StringUtils.isEmpty(financialAccountId)) {
      qry.setParameter("financialAccount",
          OBDal.getInstance().get(FIN_FinancialAccount.class, financialAccountId));
    }
    if (!StringUtils.isEmpty(currencyId)) {
      qry.setParameter("currency", OBDal.getInstance().get(Currency.class, currencyId));
    }
    if (!StringUtils.isEmpty(currencyId)) {
      qry.setParameterList("organizationTree", orgTree);
    }
    qry.setMaxResults(1);

    final FinAccPaymentMethod finAccPaymentMethod = OBDal.getInstance().get(
        FinAccPaymentMethod.class, qry.uniqueResult());

    return finAccPaymentMethod;
  }
