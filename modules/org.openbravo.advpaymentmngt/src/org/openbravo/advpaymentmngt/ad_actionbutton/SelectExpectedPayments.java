...
public class SelectExpectedPayments extends HttpSecureAppServlet {
  ...
  public void doPost(HttpServletRequest request, HttpServletResponse response)
    ...
    if (vars.commandIn("DEFAULT")) {
      ...
      String strDifferenceAmount = vars.getRequiredStringParameter("inpDifference");
    ...
    } else if (vars.commandIn("PAYMENTMETHODLIST")) {
      ...
      String strFinancialAccountId = vars.getRequiredStringParameter("inpFinancialAccount",
          IsIDFilter.instance);
      String strOrgId = vars.getRequiredStringParameter("inpadOrgId", IsIDFilter.instance);
      String paymentMethods = FIN_Utility.getPaymentMethodList(strPaymentMethodId,
          strFinancialAccountId, strOrgId, true, true);

      printList(response, paymentMethods);
    } else if (vars.commandIn("FINANCIALACCOUNTLIST")) {
      String strPaymentMethodId = vars.getRequiredStringParameter("inpPaymentMethod",
          IsIDFilter.instance);
      String strFinancialAccountId = vars.getRequiredStringParameter("inpFinancialAccount",
          IsIDFilter.instance);
      String strCurrencyId = vars.getRequiredStringParameter("inpCurrencyId", IsIDFilter.instance);
      String strOrgId = vars.getRequiredStringParameter("inpadOrgId", IsIDFilter.instance);
      String financialAccounts = FIN_Utility.getFinancialAccountList(strPaymentMethodId,
          strFinancialAccountId, strOrgId, true, strCurrencyId);

      printList(response, financialAccounts);
    }
  }

  private void printList(HttpServletResponse response, String options) throws IOException {
    response.setContentType("text/plain; charset=UTF-8");
    response.setHeader("Cache-Control", "no-cache");
    PrintWriter out = response.getWriter();
    out.print(options.replaceAll("\"", "\\'"));
    out.close();
  ...
  private void saveAndClose(HttpServletResponse response, VariablesSecureApp vars,
    ...
    try {
      ...
      List<FIN_PaymentScheduleDetail> selectedPaymentScheduleDetails = FIN_Utility.getOBObjectList(
      ...
      FIN_AddPayment.savePaymentProposal(paymentProposal, newPaymentAmount,
          selectedPaymentScheduleDetails, selectedPaymentScheduleDetailAmounts, strDifferenceAction
              .equals("writeoff") ? new BigDecimal(strDifferenceAmount) : null);
