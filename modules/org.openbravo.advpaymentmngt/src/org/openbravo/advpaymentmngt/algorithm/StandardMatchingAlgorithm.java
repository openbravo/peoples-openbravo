...
public class StandardMatchingAlgorithm implements FIN_MatchingAlgorithm {
  ...
  public FIN_MatchedTransaction match(FIN_BankStatementLine line) throws ServletException {
    List<FIN_FinaccTransaction> transactions = MatchTransactionDao.getMatchingFinancialTransaction(
        line.getBankStatement().getAccount().getId(), line.getTransactionDate(), line
            .getReferenceNo(), (line.getDramount().subtract(line.getCramount())), line
            .getBpartnername());
    ...
    transactions = MatchTransactionDao.getMatchingFinancialTransaction(line.getBankStatement()
        .getAccount().getId(), line.getTransactionDate(), line.getDramount().subtract(
        line.getCramount()));
    if (!transactions.isEmpty())
