...
public class FIN_AddPayment {
  ...
  public static FIN_Payment savePayment(FIN_Payment _payment, boolean isReceipt,
      ...
      Date paymentDate, Organization organization,
    ...
    if (_payment != null)
      ...
      payment = dao.getNewPayment(isReceipt, organization, docType, strPaymentDocumentNo,
          businessPartner, paymentMethod, finAccount, strPaymentAmount, paymentDate);
    ...
    try {
      ...
      if (refundAmount != null) {
        ...
        dao.getNewPaymentDetail(payment, refundScheduleDetail, refundAmount, null, true, null);
      }
    } finally {
  ...
  public static void removeGLItem(FIN_Payment payment, FIN_PaymentDetail paymentDetail) {
    ...
    try {
      ...
      if (paymentDetail != null) {
        ...
        payment.setAmount(payment.getAmount().subtract(paymentDetail.getAmount()));
  ...
  public static HashMap<String, BigDecimal> getSelectedPaymentDetailsAndAmount(
    ...
    for (FIN_PaymentScheduleDetail paymentScheduleDetail : selectedPaymentScheduleDetails) {
      selectedPaymentScheduleDetailsAmounts.put(paymentScheduleDetail.getId(), new BigDecimal(vars
          .getStringParameter("inpPaymentAmount" + paymentScheduleDetail.getId(), "")));
    }
    ...
    try {
      ...
      for (int i = 0; i < data.length; i++) {
        ...
        if (firstLoad && (selectedScheduledPaymentDetails.contains(FIN_PaymentScheduleDetails[i]))
          ...
          strPaymentAmt = vars.getStringParameter("inpPaymentAmount"
              + FIN_PaymentScheduleDetails[i].getId(), "");
        if (!"".equals(strPaymentAmt))
          strDifference = FIN_PaymentScheduleDetails[i].getAmount().subtract(
              new BigDecimal(strPaymentAmt)).toString();
  ...
  private static void updateInvoicePaymentMonitor(Invoice invoice, Date dueDate, BigDecimal amount,
    ...
    if (firstDueDate != null)
      invoice.setDaysTillDue(getDaysToDue(firstDueDate));
    else
   ...
   */
  private static Long getDaysToDue(Date date) {
