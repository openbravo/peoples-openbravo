...
public class FIN_PaymentProposalProcess implements org.openbravo.scheduling.Process {
      ...
      if (strAction.equals("GSP")) {
        ...
        try {
          ...
          for (FIN_PaymentPropDetail paymentProposalDetail : selectedPaymentProposalDetails) {
            ...
            if (!strBusinessPartner_old.equals(strBusinessPartner)
              ...
              FIN_Payment payment = FIN_AddPayment.savePayment(null, isReceipt, dao.getObject(
                  ...
                  selectedPaymentDetails, selectedPaymentDetailsAmounts, isWriteOff, null);
              ...
              strMessageResult.append(" (").append(payment.getBusinessPartner().getName()).append(
                  "): ");
              strMessageResult.append(message.getMessage());
          ...
          FIN_Payment payment = FIN_AddPayment.savePayment(null, isReceipt, dao.getObject(
              ...
              paymentTotal.toString(), paymentDate, orgId, selectedPaymentDetails,
              selectedPaymentDetailsAmounts, isWriteOff, null);
          ...
          else if (!"".equals(message.getTitle()))
            strMessageResult.append(": ").append(message.getTitle());
          strMessageResult.append("<br>");
        ...
        if (isAutomaticStepEnabled(paymentProposal))
  ...
  boolean isAutomaticStepEnabled(FIN_PaymentProposal paymentProposal) {
    OBCriteria<FinAccPaymentMethod> financialAccountPaymentMethods = OBDal.getInstance()
        .createCriteria(FinAccPaymentMethod.class);
    financialAccountPaymentMethods.add(Expression.eq(FinAccPaymentMethod.PROPERTY_PAYMENTMETHOD,
        paymentProposal.getPaymentMethod()));
    financialAccountPaymentMethods.add(Expression.eq(FinAccPaymentMethod.PROPERTY_ACCOUNT,
        paymentProposal.getAccount()));
    financialAccountPaymentMethods.add(Expression.eq(
        FIN_FinancialAccountAccounting.PROPERTY_ACTIVE, true));
    financialAccountPaymentMethods.setFilterOnReadableClients(false);
    financialAccountPaymentMethods.setFilterOnReadableOrganization(false);
    List<FinAccPaymentMethod> list = financialAccountPaymentMethods.list();
    if (list == null || list.size() == 0)
      return false;
    return paymentProposal.getDocumentType().getDocumentCategory().equals("APPP") ? list.get(0)
        .isAutomaticWithdrawn() : list.get(0).isAutomaticDeposit();
  }
