...
public class AlertsForWrongInvoices extends ModuleScript {
  ...
  private void createAlert(ConnectionProvider cp, AlertsForWrongInvoicesData wrongInvoice)
    ...
    String oldAlertRuleId = AlertsForWrongInvoicesData.getAlertRuleId(cp, ALERT_RULE, wrongInvoice.adClientId);
    if (!AlertsForWrongInvoicesData.existsAlert(cp, oldAlertRuleId, wrongInvoice.cInvoiceId)) {
      if (!AlertsForWrongInvoicesData.existsAlertRule(cp, ALERT_RULE)) {
        ...
        AlertsForWrongInvoicesData[] roles = AlertsForWrongInvoicesData.getRoleId(cp, WindowInvoiceId, wrongInvoice.adClientId);
        for(AlertsForWrongInvoicesData role : roles){
          AlertsForWrongInvoicesData.insertAlertRecipient(cp, wrongInvoice.adClientId, wrongInvoice.adOrgId, AlertsForWrongInvoicesData.getAlertRuleId(cp, ALERT_RULE, wrongInvoice.adClientId),
            role.adRoleId);
        }
      }
      String alertRuleId = AlertsForWrongInvoicesData.getAlertRuleId(cp, ALERT_RULE, wrongInvoice.adClientId);
      AlertsForWrongInvoicesData.insertAlert(cp, wrongInvoice.adClientId, wrongInvoice.adOrgId, strName, alertRuleId,
