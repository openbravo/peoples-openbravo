...
public class CreateAccountingConfiguration extends ModuleScript{
  ...
  //4) update Table for Document Types (ARR accounts receivables recept, APP accounts payable payments)
  public void execute() {
    try {
  ...
  void createAcctSchemaTables(ConnectionProvider cp) throws Exception{
    ...
    //Tables to be added hardcoded due to problem when executing module script. The tables sometimes do not exist yet
    String tables[] = {"4D8C3B3C31D1410DA046140C9F024D17", "B1B7075C46934F0A9FD4C4D0F1457B42", "D1A97202E832470285C9B1EB026D54E2"};
    for (int i = 0; i < data.length; i++) {
      for (int j = 0; j < tables.length; j++) {
        boolean exist = CreateAccountingConfigurationData.selectTables(cp, data[i].cAcctschemaId, tables[j]);
        if(!exist){
        CreateAccountingConfigurationData.insertAcctSchemaTable(cp.getConnection(), cp, data[i].cAcctschemaId, tables[j], data[i].adClientId);
        }
      }
    }
  }
  ...
  void createPeriodControl(ConnectionProvider cp) throws Exception{
    ...
    for (int i = 0; i < data.length; i++) {
      CreateAccountingConfigurationData[] docBaseTypes = CreateAccountingConfigurationData.selectDocBaseTypes(cp);
      for (int j = 0; j < docBaseTypes.length; j++) {
        log4j.debug("Evaluating period: " + data[i].cPeriodId + " - DocBaseType: " + docBaseTypes[j].value);
        boolean exist = CreateAccountingConfigurationData.selectPeriodControl(cp, data[i].cPeriodId, data[i].adOrgId, docBaseTypes[j].value);
        if(!exist){
          log4j.debug("Period control does not exist for period: " + data[i].cPeriodId + " and DocBaseType: " + docBaseTypes[j].value);
          CreateAccountingConfigurationData.insertPeriodControl(cp.getConnection(), cp, data[i].adClientId, data[i].adOrgId, data[i].cPeriodId, docBaseTypes[j].value, data[i].status);
        }
      }
    }
  }
