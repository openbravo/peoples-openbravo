...
OB.APRM.validateModifyPaymentPlanAmounts = function (item, validator, value, record) {
  var indRow, allRows = item.grid.data.allRows,
      ...
      invoiceOutstanding = new BigDecimal(String(item.grid.view.parentWindow.activeView.getContextInfo(false, true, true, true).inpoutstandingamt));
...
OB.APRM.addNew = function (grid) {
  ...
  returnObject.outstanding = Number(new BigDecimal(String(grid.view.parentWindow.activeView.getContextInfo(false, true, true, true).inpoutstandingamt)).subtract(totalOutstanding));
