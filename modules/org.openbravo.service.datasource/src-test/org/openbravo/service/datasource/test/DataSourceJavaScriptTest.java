...
public class DataSourceJavaScriptTest extends BaseTest {
  ...
  private static void initializeStatics() {
    ...
    TEMPLATE_LANGUAGE = new FreemarkerTemplateProcessor().getTemplateLanguage();
  ...
  protected void setUp() throws Exception {
    ...
    if (!TemplateProcessorRegistry.getInstance().isTemplateProcessorPresent(TEMPLATE_LANGUAGE)) {
  ...
  public void testJavaScriptGenerationDataSources() throws Exception {
    ...
    final TemplateProcessor templateProcessor = TemplateProcessorRegistry.getInstance()
        .getTemplateProcessor(TEMPLATE_LANGUAGE);

    int i = 0;
    for (DataSource dataSource : OBDal.getInstance().createQuery(DataSource.class, "").list()) {
      final DataSourceService dataSourceService = DataSourceServiceProvider.getInstance()
          .getDataSource(dataSource.getId());
      final DataSourceComponent component = new DataSourceComponent();
