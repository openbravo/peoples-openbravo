...
public class DataSourceJavaScriptCreator extends BaseWebServiceServlet {
  ...
  private String getFieldFromProperty(Property property) {
    ...
    if (property.getAllowedValues() != null && property.getAllowedValues().size() > 0) {
      ...
      final Map<String, String> allowedValues = DataSourceProperty.createValueMap(property
          .getAllowedValues(), property.getDomainType().getReference().getId());
      for (String key : allowedValues.keySet()) {
        final String label = allowedValues.get(key);

        if (multipleValue) {
          sb.append(", ");
        }
        sb.append("\"" + escape(key) + "\": \"" + escape(label) + "\"");
        multipleValue = true;
      }
      sb.append("}");
    }
