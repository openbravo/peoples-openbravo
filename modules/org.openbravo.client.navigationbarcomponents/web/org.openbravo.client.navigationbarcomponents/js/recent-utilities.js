...
//
// Retrieves the most recent choices which are stored under the name specified by
// by the parameter      
//
// Parameters:
...
//
  ...
  //** {{{ RecentUtilities.addRecent(/*String*/ propertyName, /*Object*/ choiceObject) }}} **
  //
  // Add a new recent entry to the array of recent entries. There are never stored more than {{{recentNum}}}
  ...
  //
    addRecent: function (/*String*/ propertyName, /*Object*/ choiceObject) {
      ...
      for (var i = 0; i < currentRecentValue.length; i++) {
        if (currentRecentValue[i] && currentRecentValue[i].id === choiceObject.id) {
          // found it 
          currentIndex = i;
        }
      }
