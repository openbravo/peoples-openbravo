...
public class MainLayoutComponent extends BaseTemplateComponent {
  ...
  protected Template getComponentTemplate() {
    return OBDal.getInstance().get(Template.class, MainLayoutConstants.MAIN_LAYOUT_TEMPLATE_ID);
  }
  ...
  public Collection<NBComponent> getNavigationBarComponents() {
    ...
    for (NavBarComponent nbc : obc.list()) {
      ...
      String jsCode = "";
      try {
        final BaseTemplateComponent component = (BaseTemplateComponent) OBClassLoader.getInstance()
            .loadClass(nbc.getJavaClassName()).newInstance();
  ...
  public String getStartPage() {
    try {
      return Preferences.getPreferenceValue(getContextUrl()
          + MainLayoutConstants.START_PAGE_PROPERTY, true, OBContext.getOBContext()
  ...
  public String getLoginPage() {
    try {
      return Preferences.getPreferenceValue(getContextUrl()
          + MainLayoutConstants.LOGIN_PAGE_PROPERTY, true, OBContext.getOBContext()
          .getCurrentClient(), OBContext.getOBContext().getCurrentOrganization(), OBContext
          .getOBContext().getUser(), OBContext.getOBContext().getRole(), null);
    } catch (PropertyException e) {
      return "";
