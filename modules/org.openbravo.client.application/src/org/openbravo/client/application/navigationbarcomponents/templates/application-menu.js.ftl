...
isc.MenuButton.create(OB.ApplicationMenuButtonStylingProperties, {
    ...
    placeNear: function(left, top) {
        ...
        return this.Super('placeNear', [parentLeft, top]);
    },
    ...
    show: function() {
        ...
        if (!this.selectedHideLayout) {
            this.selectedHideLayout = isc.Layout.create({styleName: 'OBNavBarComponentHideLine', 
              ...
              top: layoutContainer.getPageTop() + layoutContainer.getVisibleHeight(), 
    ...
    itemClick: function(item, colNum) {
        ...
        if (item.viewId) {
            if (isTestEnvironment && item.viewId === 'OBClassicWindow') {
              goto = {viewId: '_' + item.windowId, windowId: item.windowId, tabId: item.tabId, id: item.tabId, command: 'DEFAULT', tabTitle: item.title};
            } else {
              goto = item;
            }
            ...
            if (!goto.viewId) {
                goto.viewId = 'OBClassicWindow'; 
            }
        } else if (item.tabId) {
            if (isTestEnvironment) {
              goto = {viewId: '_' + item.windowId, windowId: item.windowId, tabId: item.tabId, id: item.tabId, command: 'DEFAULT', tabTitle: item.title};
            } else {
              goto = {viewId: 'OBClassicWindow', windowId: item.windowId, tabId: item.tabId, id: item.tabId, command: 'DEFAULT', tabTitle: item.title};
            }
        } else if (item.manualUrl) {
            if (item.manualProcessId) {
                goto = {viewId: 'OBClassicWindow', obManualURL: item.manualUrl, processId: item.manualProcessId, id: item.manualProcessId, command: 'DEFAULT', tabTitle: item.title};
            } else if (item.processId) {
                goto = {viewId: 'OBPopupClassicWindow', obManualURL: item.manualUrl, processId: item.processId, id: item.processId, command: 'BUTTON' + item.processId, tabTitle: item.title};
            } else if (item.formId) { 
                goto = {viewId: 'OBClassicWindow', obManualURL: item.manualUrl, id: item.manualUrl, formId: item.formId, command: 'DEFAULT', tabTitle: item.title};
            } else {
                goto = {viewId: 'OBClassicWindow', obManualURL: item.manualUrl, id: item.manualUrl, command: 'DEFAULT', tabTitle: item.title};
            }
        } else if (item.externalUrl) {
            goto = {viewId: 'OBExternalPage', contentsURL: item.externalUrl, id: item.externalUrl, command: 'DEFAULT', tabTitle: item.title};
        }
