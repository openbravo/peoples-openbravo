  ...
  validators: {
    condition: function (item) {
      ...
      for (i = 0; i < recordsToValidate.length; i++) {
        record = grid.getEditedRecord(grid.getRecordIndex(recordsToValidate[i]));
        for (j = 0; j < fields.length; j++) {
          if (fields[j].required && (record[fields[j].name] === null || record[fields[j].name] === undefined || record[fields[j].name] === '')) {
            hasErrors = true;
            if (!lineNumbers) {
              lineNumbers = grid.getRecordIndex(recordsToValidate[i]).toString();
            } else {
              lineNumbers = lineNumbers + ',' + grid.getRecordIndex(recordsToValidate[i]).toString();
            }
          }
        }
      }
