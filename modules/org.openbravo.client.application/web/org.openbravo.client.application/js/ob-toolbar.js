...
isc.OBToolbar.addProperties({
  initWidget: function() {
    ...
    newMembers[j] = isc.HLayout.create({width: this.leftMargin, height: 1});
    j++;
    ...
    if (this.leftMembers) {
      for (i=0; i<this.leftMembers.length; i++) {
        ...
        j++;
        newMembers[j] = isc.HLayout.create({width: this.leftMembersMargin, height: 1});
        j++;
      }
    }

    newMembers[j] = isc.HLayout.create({width: '100%', height: 1});
    j++;
    newMembers[j] = isc.HLayout.create({width: 40, height: 1});
    j++;
    ...
    if (this.rightMembers) {
      for (i=0; i<this.rightMembers.length; i++) {
        ...
        j++;
        newMembers[j] = isc.HLayout.create({width: this.rightMembersMargin, height: 1});
        j++;
      }
    }

    newMembers[j] = isc.HLayout.create({width: this.rightMargin, height: 1});
    j++;
  ...
  // Given a numerical index or a left member ID, return a pointer to the appropriate left member.
  ...
  getLeftMember: function(member) {
    ...
    if (typeof member === 'number') {
      ...
      for (i=0; i<this.leftMembers.length; i++) {
        if (this.leftMembers[i].ID === member) {
  ...
  addLeftMembers: function(newMembers, position) {
    ...
    for (i=0; i<newMembers.length; i++) {
      ...
      position = position + 1;
      this.Super('addMembers', [isc.HLayout.create({width: this.leftMembersMargin, height: 1}), position]);
      position = position + 1;
    }
  },
  ...
  addRightMembers: function(newMembers, position) {
    ...
    for (i=0; i<newMembers.length; i++) {
      this.Super('addMembers', [newMembers[i], position]);
      position = position + 1;
      this.Super('addMembers', [isc.HLayout.create({width: this.rightMembersMargin, height: 1}), position]);
      position = position + 1;
    }
  },
