...
isc.OBDateItem.addProperties({
  ...
  checkOBDateItemValue: function(){
    ...
    if (isValid === false) {
      this.form.addFieldErrors(this.name, OB.I18N.getLabel('OBUIAPP_InvalidValue'), false);
      this.form.markForRedraw();
    } else if (isRequired === true &&
    (value === null || value === '' || typeof value === 'undefined')) {
      this.form.addFieldErrors(this.name, OB.I18N.getLabel('OBUISC_Validator.requiredField'), false);
      this.form.markForRedraw();
    } else {
...
isc.OBNumberItem.addProperties({
  ...
  focusNumberInput: function(maskNumeric, decSeparator, groupSeparator, groupInterval){
    ...
    var oldCaretPosition = this.getSelectionRange()[0];
  ...
  manageDecPoint: function(keyCode, decSeparator){
    ...
    var caretPosition = this.getSelectionRange()[0];
    /*
  ...
  validateOBNumberItem: function(){
    ...
    if (isValid === false) {
      this.form.setFieldErrors(this.name, OB.I18N.getLabel('OBUIAPP_InvalidValue'), false);
      this.form.markForRedraw();
    } else if (isRequired === true &&
    (value === null || value === '' || typeof value === 'undefined')) {
      this.form.setFieldErrors(this.name, OB.I18N.getLabel('OBUISC_Validator.requiredField'), false);
      this.form.markForRedraw();
    } else {
      this.form.clearFieldErrors(this.name, false);
      this.form.markForRedraw();
