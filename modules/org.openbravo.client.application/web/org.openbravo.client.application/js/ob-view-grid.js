...
isc.OBViewGrid.addProperties({
  ...
  createRecordComponent: function(record, colNum){
    ...
    if (this.isEditLinkColumn(colNum)) {
      ...
      layout = isc.OBEditOpenGridComponent.create({
        record: record,
        grid: this,
        rowNum: rowNum
      });
  ...
  initWidget: function(){
    ...
    if (this.editGrid) {
      ...
      this.fields.unshift(this.editLinkFieldProperties);
    }
  ...
  convertCriteria: function(criteria){
    ...
    if (this.view.targetRecordId) {
      criteria.targetRecordId = this.view.targetRecordId;
      this.view.targetRecordId = null;
    }
  ...
  makeCellContextItems: function(record, rowNum, colNum){
    ...
    if (field.foreignKeyField) {
      menuItems.add({
        ...
        click: function(){
          OB.EntityToWindowMapping.openView(field.targetEntity, record[field.name]);
        }
      });
    }
...
// = OBEditOpenGridComponent =
// The component which is used to create the contents of the 
// edit open column in the grid
isc.ClassFactory.defineClass('OBEditOpenGridComponent', isc.HLayout);

isc.OBEditOpenGridComponent.addProperties({
  height: 1,
  width: '100%',
  visible: 'overflow',
  align: 'center',
  ...
  initWidget: function(){
    var me = this;
    this.editOpenLayout = isc.HLayout.create({
      width: 1,
      height: 1,
      overflow: 'visible',
      membersMargin: 5
    });
    this.editOpenLayout.addMember(this.createLabel(OB.I18N.getLabel('OBUIAPP_Edit'), function(){
      me.doEdit();
    }));
    this.editOpenLayout.addMember(isc.Label.create({
      baseStyle: 'OBSeparator',
      width: 1,
      height: 1,
      overflow: 'visible'
    }));
    this.editOpenLayout.addMember(this.createLabel(OB.I18N.getLabel('OBUIAPP_Open'), function(){
      me.doOpen();
    }));
    
    this.saveCancelLayout = isc.HLayout.create({
      width: 1,
      height: 1,
      overflow: 'visible',
      membersMargin: 5
    });
    this.saveCancelLayout.addMember(this.createLabel(OB.I18N.getLabel('UINAVBA_Save'), function(){
      me.doSave();
    }));
    this.saveCancelLayout.addMember(isc.Label.create({
      baseStyle: 'separator',
      width: 1,
      height: 1,
      overflow: 'visible'
    }));
    this.saveCancelLayout.addMember(this.createLabel(OB.I18N.getLabel('UINAVBA_Cancel'), function(){
      me.doCancel();
    }));
    
    this.addMember(this.editOpenLayout);
  },
  
  createLabel: function(title, clickFunction){
    return isc.Label.create({
      baseStyle: 'OBLabelLink',
      width: 1,
      height: 1,
      overflow: 'visible',
      contents: title,
  ...
  showEditOpen: function(){
    this.removeMembers(this.saveCancelLayout);
    this.addMembers(this.editOpenLayout);
    this.grid.currentEditColumnLayout = null;
  },
  
  showSaveCancel: function(){
    this.removeMembers(this.editOpenLayout);
    this.addMembers(this.saveCancelLayout);
