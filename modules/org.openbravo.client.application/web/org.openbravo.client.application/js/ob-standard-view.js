...
isc.OBStandardWindow.addProperties({
  ...
  isEqualParams: function(params){
    ...
    var equalView = params.viewId && params.viewId === this.getClassName();
    return equalTab && equalView;
  },
...
isc.OBStandardView.addProperties({
  ...
  doHandleDoubleClick: function(){
    ...
    if (this.previousState === isc.OBStandardView.STATE_BOTTOM_MAX) {
      ...
      alert(this.previousState + ' not supported ');
    }
  ...
  setDataSource: function(ds){
    ...
    if (this.targetRecordId) {
      this.dataSource.fetchRecord(this.targetRecordId, this.selectDirectRecord, {
        view: this
      });
    }
  },
  
  selectDirectRecord: function(response, data, request){
    if (data && data.length && data.length > 0) {
      request.view.editRecord(data[0]);
    }
  },
