...
// = OBQuickRun =
// The OBQuickRun widget creates a button with a layout which is displayed below it
...
// ** {{{ currentQuickRun }}} **
// The current OBQuickRun widget which is expanded (or null if none is expanded).
...
isc.OBQuickRun.addProperties({
  ...
  // ** {{{ baseStyle }}} **
  // The base style for the quick run launch button. All other styles are derived
  ...
  draw : function() {
    ...
    var ksAction = function() {
      me.getLayoutContainer().setStyleName('navBarComponentSelected');
      if (!me.showing) {
...
//    }
    ...
    layoutContainer.setStyleName('navBarComponentSelected');
  ...
  // ** {{{ beforeShow }}} **
  // Intended to be overridden, is called just before the layout.show() method
  // is called.
  beforeShow : function() {},
  ...
  doHide : function() {
    ...
    this.getLayoutContainer().setStyleName('navBarComponent');
