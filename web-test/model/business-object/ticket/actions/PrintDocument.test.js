...
require('../../../../../web/org.openbravo.retail.posterminal/app/model/business-object/ticket/actions/PrintDocument');
...
describe('Print Document action', () => {
  it('Expected state after invoking the action', () => {
    const state = { Messages: [], Ticket: { dummy: 'state' } };
    const payload = { data: '<xml></xml>', device: 0 };
    deepfreeze(state);
    const newState = OB.App.StateAPI.Global.printDocument(state, payload);
    const expectedState = {
      Messages: [
        {
