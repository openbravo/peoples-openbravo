CREATE OR REPLACE RULE C_DEBT_PAYMENT_GENERATE_INS AS ON INSERT TO C_DEBT_PAYMENT_GENERATE DO INSTEAD  
    INSERT INTO C_DEBT_PAYMENT (C_DEBT_PAYMENT_ID, AD_CLIENT_ID, AD_ORG_ID, 
        ISACTIVE, CREATED, CREATEDBY, UPDATED, UPDATEDBY, ISRECEIPT, C_SETTLEMENT_GENERATE_ID, DESCRIPTION, C_INVOICE_ID, 
        C_BPARTNER_ID, C_CURRENCY_ID, C_CASHLINE_ID, C_BANKSTATEMENTLINE_ID, C_BANKACCOUNT_ID, C_CASHBOOK_ID, PAYMENTRULE, 
        ISPAID, AMOUNT, WRITEOFFAMT, DATEPLANNED, ISMANUAL, ISVALID, GENERATE_PROCESSED, GLITEMAMT, C_SETTLEMENT_CANCEL_ID, 
        C_GLITEM_ID, ISDIRECTPOSTING, STATUS_INITIAL, C_PROJECT_ID) 
    VALUES (new.c_debt_payment_id, new.ad_client_id, new.ad_org_id, new.isactive, new.created, new.createdby, new.updated, new.updatedby, 
        new.isreceipt, new.c_settlement_id, new.description, new.c_invoice_id, new.c_bpartner_id, new.c_currency_id, 
        new.c_cashline_id, new.c_bankstatementline_id, new.c_bankaccount_id, new.c_cashbook_id, new.paymentrule, 
        new.ispaid, new.amount, new.writeoffamt, new.dateplanned, new.ismanual, new.isvalid, new.generate_processed, 
        new.glitemamt, new.c_settlement_cancel_id, new.c_glitem_id, new.isdirectposting, new.status, new.c_project_id);
/-- END

 CREATE OR REPLACE RULE C_DEBT_PAYMENT_GENERATE_UPD AS ON UPDATE TO C_DEBT_PAYMENT_GENERATE DO INSTEAD  
        UPDATE C_DEBT_PAYMENT SET 
          C_DEBT_PAYMENT_ID = NEW.C_DEBT_PAYMENT_ID, 
          ...
          ISRECEIPT = NEW.ISRECEIPT, 
          C_SETTLEMENT_GENERATE_ID = NEW.C_SETTLEMENT_ID, 
          ...
          C_BANKSTATEMENTLINE_ID = NEW.C_BANKSTATEMENTLINE_ID, 
          C_BANKACCOUNT_ID = NEW.C_BANKACCOUNT_ID, 
          C_CASHBOOK_ID = NEW.C_CASHBOOK_ID, 
          PAYMENTRULE = NEW.PAYMENTRULE, 
          ISPAID = NEW.ISPAID,
          AMOUNT = NEW.AMOUNT, 
          WRITEOFFAMT = NEW.WRITEOFFAMT, 
          DATEPLANNED = NEW.DATEPLANNED, 
          ISMANUAL = NEW.ISMANUAL, 
          ISVALID = NEW.ISVALID, 
          GENERATE_PROCESSED = NEW.GENERATE_PROCESSED, 
          GLITEMAMT = NEW.GLITEMAMT, 
          C_SETTLEMENT_CANCEL_ID = NEW.C_SETTLEMENT_CANCEL_ID, 
          C_GLITEM_ID = NEW.C_GLITEM_ID, 
          ISDIRECTPOSTING = NEW.ISDIRECTPOSTING,
          status_initial = NEW.STATUS, 
          C_PROJECT_ID = NEW.C_PROJECT_ID 
        WHERE C_DEBT_PAYMENT_ID = NEW.C_DEBT_PAYMENT_ID;
/-- END

 CREATE OR REPLACE RULE C_DEBT_PAYMENT_GENERATE_DEL AS ON DELETE TO C_DEBT_PAYMENT_GENERATE DO INSTEAD  
