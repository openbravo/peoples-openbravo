...
public class WadUtility {
    ...
    public static String columnIdentifier(ConnectionProvider conn,
            String tableName, boolean required, FieldsData fields, int ilist,
            int itable, boolean translated, Vector<Object> vecFields,
            Vector<Object> vecTable, Vector<Object> vecWhere,
            Vector<Object> vecParameters, Vector<Object> vecTableParameters,
        ...
        if (fields.reference.equals("17")) { // List
            ...
            if (tableName != null && tableName.length() != 0) {
                vecTable.addElement("left join ad_ref_list_v list" + ilist
                        + " on (" + tableName + "." + fields.name + " = list"
                        + ilist + ".value and list" + ilist
                        + ".ad_reference_id = '" + fields.referencevalue
                        + "' and list" + ilist + ".ad_language = ?) ");
                vecTableParameters
                        .addElement("<Parameter name=\"paramLanguage\"/>");
            } else {
                vecTable.addElement("ad_ref_list_v list" + ilist);
                vecWhere.addElement(fields.referencevalue + " = " + "list"
                        + ilist + ".ad_reference_id");
            ...
            itable++;
            TableRelationData trd[] = TableRelationData.selectRefTable(conn,
            ...
            vecTable
                    .addElement("left join (select "
                            ...
                            + (!trd[0].keyname.equalsIgnoreCase(trd[0].name) ? (", " + trd[0].name)
                                    : "") + " from " + trd[0].tablename
                            + ") table" + itable + " on (" + tableName + "."
                            + fields.name + " = table" + itable + "."
                            + trd[0].keyname + ") ");
            ...
            texto
                    .append(columnIdentifier(conn, "table" + itable, required,
                            fieldsAux, ilist, itable, translated, vecFields,
                            vecTable, vecWhere, vecParameters,
            ...
            vecTable.addElement("left join " + tableDirName + " on ("
                    + tableName + "." + fields.name + " = " + tableDirName
                    + "." + fields.name + ") ");
            for (int i = 0; i < fdi.length; i++) {
                if (i > 0)
                    texto.append(" || ' - ' || ");
                texto.append(columnIdentifier(conn, tableDirName, required,
                        fdi[i], ilist, itable, translated, vecFields, vecTable,
                        vecWhere, vecParameters, vecTableParameters,
                        sqlDateFormat));
            }
        } else if (fields.reference.equals("30")) {
            itable++;
            EditionFieldsData[] dataSearchs = EditionFieldsData.selectSearchs(
                    conn, "", fields.referencevalue);
            ...
            for (int i = 0; i < fdi.length; i++) {
                ...
                || fields.reference.equals("800011")) { // Search y Locator
            itable++;
            int ilength = fields.name.length();
            String tableDirName = "";
            if (fields.reference.equals("25"))
            ...
            if (fields.istranslated.equals("Y")
                    && TableRelationData.existsTableColumn(conn,
                            fields.tablename + "_TRL", fields.name)) {
                FieldsData fdi[] = FieldsData.tableKeyColumnName(conn,
                        fields.tablename);
                if (fdi == null || fdi.length == 0) {
                    vecFields.addElement(applyFormat(tableName + "."
                            + fields.name, fields.reference, sqlDateFormat));
                    texto.append(applyFormat(tableName + "." + fields.name,
                    ...
                    vecTable
                            .addElement("left join (select "
                                    ...
                                    + ", AD_Language"
                                    + (!fdi[0].name
                                            .equalsIgnoreCase(fields.name) ? (", " + fields.name)
                                            : "") + " from " + fields.tablename
                                    + "_TRL) tableTRL" + itable + " on ("
                                    + tableName + "." + fdi[0].name
                                    + " = tableTRL" + itable + "."
                                    + fdi[0].name + " and tableTRL" + itable
                                    + ".AD_Language = ?) ");
                    vecTableParameters
                            .addElement("<Parameter name=\"paramLanguage\"/>");
                    vecFields.addElement(applyFormat("(CASE WHEN tableTRL"
                            + itable + "." + fields.name
                            + " IS NULL THEN TO_CHAR(" + tableName + "."
                            + fields.name + ") ELSE TO_CHAR(tableTRL" + itable
                            + "." + fields.name + ") END)", fields.reference,
                            sqlDateFormat));
                    texto.append(applyFormat("(CASE WHEN tableTRL" + itable
                            + "." + fields.name + " IS NULL THEN TO_CHAR("
                            + tableName + "." + fields.name
                            + ") ELSE TO_CHAR(tableTRL" + itable + "."
                            + fields.name + ") END)", fields.reference,
                            sqlDateFormat));
                }
            } else {
                vecFields.addElement(applyFormat(tableName + "." + fields.name,
                        fields.reference, sqlDateFormat));
                texto.append(applyFormat(tableName + "." + fields.name,
                        fields.reference, sqlDateFormat));
            }
    ...
    public static String applyFormat(String text, String reference,
    ...
    public static String columnIdentifier(ConnectionProvider conn,
            String tableName, boolean required, FieldsData fields,
            Vector<Object> vecCounters, boolean translated,
            Vector<Object> vecFields, Vector<Object> vecTable,
            Vector<Object> vecWhere, Vector<Object> vecParameters,
            Vector<Object> vecTableParameters, String sqlDateFormat)
            throws ServletException {
        if (fields == null)
            return "";
        StringBuffer texto = new StringBuffer();
        int ilist = Integer.valueOf(vecCounters.elementAt(1).toString())
                .intValue();
        int itable = Integer.valueOf(vecCounters.elementAt(0).toString())
                .intValue();
        if (fields.reference.equals("13")) { // ID
            ...
            for (int i = 0; i < fdi.length; i++) {
                ...
                texto.append(columnIdentifier(conn, tableName, required,
                        fdi[i], vecCounters, translated, vecFields, vecTable,
                        vecWhere, vecParameters, vecTableParameters,
                        sqlDateFormat));
                ilist = Integer.valueOf(vecCounters.elementAt(1).toString())
                        .intValue();
                itable = Integer.valueOf(vecCounters.elementAt(0).toString())
                        .intValue();
            }
            if (texto.toString().equals("")) {
                vecFields
                        .addElement(((tableName != null && tableName.length() != 0) ? (tableName + ".")
                                ...
                                + fields.name);
                texto
                        .append(((tableName != null && tableName.length() != 0) ? (tableName + ".")
                                : "")
                                + fields.name);
            }
                ...
                vecTable
                        .addElement("left join (select "
                                ...
                                + ((trd[0].isvaluedisplayed.equals("Y") && !trd[0].keyname
                                        ...
                                        : "")
                                + (!trd[0].keyname
                                        .equalsIgnoreCase(trd[0].name) ? (", " + trd[0].name)
                                        : "") + " from " + trd[0].tablename
                                + ") table" + itable + " on (" + tableName
                                + "." + fields.name + " = " + " table" + itable
                                + "." + trd[0].keyname + ")");
            ...
            texto
                    .append(columnIdentifier(conn, "table" + itable, required,
                            fieldsAux, vecCounters, translated, vecFields,
                            vecTable, vecWhere, vecParameters,
                            vecTableParameters, sqlDateFormat));
            ilist = Integer.valueOf(vecCounters.elementAt(1).toString())
                    .intValue();
            ...
            if (tableName != null && tableName.length() != 0) {
                vecTable
                        .addElement("left join (select AD_Image_ID, ImageURL from AD_Image) list"
                                + ilist
                                + " on ("
                                + tableName
                                + "."
            ...
            vecCounters.set(0, Integer.toString(itable));
            vecCounters.set(1, Integer.toString(ilist));
        } else if (fields.reference.equals("19")
                || fields.reference.equals("30")
                || fields.reference.equals("31")
                || fields.reference.equals("35")
                || fields.reference.equals("25")
            ...
            if (fields.reference.equals("30"))
                dataSearchs = EditionFieldsData.selectSearchs(conn, "",
                        fields.referencevalue);
            String tableDirName = "", fieldId = "";
            if (dataSearchs == null || dataSearchs.length == 0) {
                int ilength = fields.name.length();
            ...
            if (tableName != null && tableName.length() != 0) {
                ...
                vecTable.addElement("left join (select " + fieldId
                        + fieldsAux.toString() + " from " + tableDirName
                        + ") table" + itable + " on (" + tableName + "."
                        + fields.name + " = table" + itable + "." + fieldId
                        + ")");
            } else {
            ...
            for (int i = 0; i < fdi.length; i++) {
                ...
                vecCounters.set(0, Integer.toString(itable));
                vecCounters.set(1, Integer.toString(ilist));
                texto.append(columnIdentifier(conn, "table" + tableId,
                        required, fdi[i], vecCounters, translated, vecFields,
                    ...
                    vecFields
                            .addElement(applyFormat(
                                    ((tableName != null && tableName.length() != 0) ? (tableName + ".")
                                            : "")
                                            + fields.name, fields.reference,
                                    sqlDateFormat));
                    texto.append(applyFormat(((tableName != null && tableName
                            .length() != 0) ? (tableName + ".") : "")
                            + fields.name, fields.reference, sqlDateFormat));
                } else {
                    vecTable
    ...
    /**
     * Establece el tipo de class que le corresponde a un campo determinado de
     * la edición
     * 
     * @param efd
     *            - Estructura de tipo EditionFieldsData
     * @return Devuelve un String con el parámetro class completo o un String
     *         vacío en caso de no tener class asociado para el campo indicado.
     */
    public static String classRequiredUpdateable(EditionFieldsData efd,
            boolean isupdateable, boolean tabIsReadOnly) {
        StringBuffer htmltext = new StringBuffer();
        String strAux = "";
        try {
            if (isDecimalNumber(efd.reference) || isPriceNumber(efd.reference)
                    || isIntegerNumber(efd.reference)
                    || isGeneralNumber(efd.reference)
            ...
            if (efd.required.equals("Y")
                    && !efd.columnname.equalsIgnoreCase("Value")) {
                if (efd.isreadonly.equals("Y") || tabIsReadOnly) {
                    htmltext.append(" class=\"").append(strType).append(
                            " required ").append(classType).append(
                ...
                htmltext.append(" class=\"").append(strType).append(" ")
                        .append(classType).append("_OneCell_width").append(
                                strAux).append(" readonly\" ");
            } else if (!isupdateable) {
                htmltext.append(" class=\"").append(strType).append(" ")
                        .append(classType).append("_OneCell_width").append(
                                strAux).append(" readonly\" ");
    ...
    public static boolean columnRelationFormat(FieldsData data, boolean header,
            int maxColSize) throws IOException, ServletException {
        if (data == null)
            return false;
        ...
        if (maxColSize > 0) {
            ...
            if (header
                    && data.name.length() > Integer.valueOf(data.displaylength)
                            .intValue()) {
                data.name = data.name.substring(0, Integer.valueOf(
                        data.displaylength).intValue());
            }
        }
        return true;
    }
    ...
    public static String xmlFields(FieldsData fd, String completeName,
            int maxTextboxLength, boolean forcedAttribute, boolean tabIsReadOnly) {
        ...
        if (forcedAttribute) {
        ...
        } else if (fd.reference.equals("17") || fd.reference.equals("18")
                || fd.reference.equals("19")) { // List
            html.append(xmlFields(fd, completeName, maxTextboxLength, true,
                    tabIsReadOnly));
            if (fd.isdisplayed.equals("Y")) {
                html.append("\n<SUBREPORT id=\"report" + fd.name + completeName
                        + "\" name=\"report" + fd.name + completeName + "\"");
                html.append(" report=\"org" + strSystemSeparator + "openbravo"
                        + strSystemSeparator + "erpCommon" + strSystemSeparator
                        + "reference" + strSystemSeparator + "List\">\n");
                html
                        .append("  <ARGUMENT name=\"parameterListSelected\" withId=\""
                                + fd.name + completeName + "\"/>\n");
                html.append("</SUBREPORT>\n");
            }
            if (fd.isreadonly.equals("Y") || !fd.isupdateable.equals("Y")
                    || tabIsReadOnly) {
                html.append("<FIELD ");
            ...
            html
                    .append(xmlFields(fd, "", maxTextboxLength, true,
                            tabIsReadOnly));
            if (fd.isdisplayed.equals("Y")) {
                html.append("<FIELD id=\"" + fd.name + completeName
                        + "\" name=\"" + fd.name + completeName
            ...
            html.append(fd.name + completeName + "</FIELD>");
                ...
                && !fd.referencevalue.equals("")
                && !fd.columnname.equals("ChangeProjectStatus")) { // Button
            html.append("<FIELD ");
            html.append("id=\"" + fd.name + completeName + "\" ");
            html.append("replaceCharacters=\"htmlPreformated\" ");
            html.append(" attribute=\"value\">");
    ...
    public static String htmlFields(EditionFieldsData efd, String completeName,
            String completeID, boolean isupdateable, int maxTextboxLength,
            boolean forcedAttribute, boolean isdesigne,
            Vector<Object> vecCallOuts, String tabName,
            Vector<Object> vecDisplayLogic, Vector<Object> vecReloads,
            boolean tabIsReadOnly, int textareaLength) {
        StringBuffer html = new StringBuffer();
        String onChange = ((isInVector(vecDisplayLogic, efd.columnname) && !tabIsReadOnly) ? "displayLogic();"
                : "");
        String logChanges = "logChanges(this);";
        if (vecReloads != null && vecReloads.size() > 0
                && efd.calloutname.equals("") && !tabIsReadOnly
        ...
        if (!forcedAttribute && efd.isdisplayed.equals("Y")
                && !efd.displaylogic.equals("")) {
            html.append("<span id=\"" + efd.columnname + "_inp\">");
        }
        if (forcedAttribute) {
            html.append("<input type=\"hidden\"");
            html.append(" name=\"inp"
            ...
            if (isDecimalNumber(efd.reference) || isPriceNumber(efd.reference)
                    ...
                    || isQtyNumber(efd.reference))
                html
                        .append("\" onkeydown=\"auto_complete_number(this, true, true);return true;");
            else if (isIntegerNumber(efd.reference))
                html
                        .append("\" onkeydown=\"auto_complete_number(this, false, true);return true;");
            html.append("\"");
            html.append(classRequiredUpdateable(efd, isupdateable,
                    tabIsReadOnly));
            html.append(" ></input>");
            ...
            html.append("<select name=\"inp"
                    + Sqlc.TransformaNombreColumna(efd.columnnameinp)
                    + completeName + "\"");
            onChange = callouts(efd, vecCallOuts, maxTextboxLength) + onChange;
            // if (efd.isparent.equals("Y") || !isupdateable || tabIsReadOnly) {
            if (log4j.isDebugEnabled())
                log4j.debug("column: " + efd.columnname + "isUpdateable: "
                        + isupdateable + " tabReadOnly: " + tabIsReadOnly
                        + " isUpdateable field: " + efd.isupdateable);
            if (!isupdateable || tabIsReadOnly) {
                html.append(" readonly=\"true\"");
                onChange = "selectCombo(this, 'xx');";
            }
            html.append(" onchange=\"").append(logChanges).append(onChange)
                    .append("return true;\"");
            html.append(classRequiredUpdateable(efd, isupdateable,
                    tabIsReadOnly));
            ...
            html.append("\"");
            if (!tabIsReadOnly)
                onChange = callouts(efd, vecCallOuts, maxTextboxLength)
                        + onChange;
            if (!onChange.equals(""))
            ...
            if (!tabIsReadOnly && isupdateable) {
                ...
                html
                        .append("openSearch(null, null, '../info/ImageInfo.html', null, false, 'frmMain', 'inp"
                                + Sqlc
                                        .TransformaNombreColumna(efd.columnnameinp)
                                + "', 'inp"
                                + Sqlc
                                        .TransformaNombreColumna(efd.columnnameinp)
                                + "_R', document.frmMain.inp"
                                + Sqlc
                                        .TransformaNombreColumna(efd.columnnameinp)
                                + ".value);");
                html
                        .append("return false;\" onmouseover=\"window.status='")
                        .append(efd.referenceName)
                        .append(
                                "';return true;\" onmouseout=\"window.status='';return true;\">");
            }
            html
                    .append("<img src=\"../../../../../web/images/xx\" border=\"0\"");
            html
                    .append(" width=\"")
                    .append(
                            (efd.displaysize.equals("") || IMAGE_EDITION_WIDTH < Integer
                                    .valueOf(efd.displaysize).intValue()) ? Integer
                                    .toString(IMAGE_EDITION_WIDTH)
                                    : efd.displaysize).append("\"");
            html
                    .append(" height=\"")
                    .append(
                            (efd.displaysize.equals("") || IMAGE_EDITION_HEIGHT < Integer
                                    .valueOf(efd.displaysize).intValue()) ? Integer
                                    .toString(IMAGE_EDITION_HEIGHT)
                                    : efd.displaysize).append("\" ");
            html.append(classRequiredUpdateable(efd, isupdateable,
                    tabIsReadOnly));
            html.append(" name=\"inp").append(
                    Sqlc.TransformaNombreColumna(efd.columnnameinp)).append(
                    completeName);
            html.append("\" id=\"").append(efd.columnname).append(completeID)
                    .append("\" alt=\"").append(efd.name).append("\" title=\"")
                    .append(efd.name).append("\"");
            if (!tabIsReadOnly)
                onChange = callouts(efd, vecCallOuts, maxTextboxLength)
                        + onChange;
            if (!onChange.equals(""))
            ...
            if (rowLength < 3.0)
                rowLength = 3.0;
            html.append("<textarea cols=\"").append(textareaLength).append(
                    "\" rows=\"").append(Double.toString(rowLength)).append(
                    "\" name=\"inp").append(
                    Sqlc.TransformaNombreColumna(efd.columnnameinp));
            html.append(completeName).append("\" id=\"").append(efd.columnname)
                    .append(completeID).append("\" ");
            onChange = callouts(efd, vecCallOuts, maxTextboxLength) + onChange;
            html.append(" onclick=\"").append(logChanges).append(onChange)
                    .append("return true;\"");
            html.append(classRequiredUpdateable(efd, isupdateable,
                    tabIsReadOnly));
            if (!isupdateable || tabIsReadOnly) {
            ...
            html.append(" onkeypress=\"return handleFieldMaxLength(this, ")
                    .append(efd.fieldlength).append(");").append("\"");
            html.append(">");
            if (!isdesigne)
                html.append("xxV");
            ...
            html
                    .append(htmlFields(efd, completeName, completeID,
                            isupdateable, maxTextboxLength, true, isdesigne,
                            vecCallOuts, tabName, vecDisplayLogic, vecReloads,
                            tabIsReadOnly, textareaLength));
            html
                    .append("<span id=\"" + efd.columnname + completeID
                            + "_BTN\">");
            html.append((efd.referencevalue.equals("") || efd.columnname
                    .equals("ChangeProjectStatus")) ? efd.name : "xx");
            html.append("</span>&nbsp;</td>");
            ...
            if (efd.isdisplayed.equals("N")) {
                ...
                if (efd.isencrypted.equals("Y")
                        || efd.iscolumnencrypted.equals("Y"))
                    html.append("password\"");
                else
                    html.append("text\"");
                if (isDecimalNumber(efd.reference)
                        || isPriceNumber(efd.reference)
                        || isGeneralNumber(efd.reference)
                        || isQtyNumber(efd.reference))
                    html
                            .append(" onkeydown=\"auto_complete_number(this, true, true);return true;\"");
            ...
            if (efd.reference.equals("14")
                    && Integer.valueOf(efd.fieldlength).intValue() > maxTextboxLength) {
                html
                        .append((!onChange.equals("") ? (" onblur=\""
                                + onChange + "\"") : ""));
            } else if (efd.reference.equals("20")) {
                if (!isupdateable || tabIsReadOnly)
                    onChange = "return false;";
                html
                        .append((!onChange.equals("") ? (" onclick=\""
                                + onChange + "\"") : ""));
            } else {
                html
                        .append((!onChange.equals("") ? (" onchange=\""
                                + onChange + "\"") : ""));
            }
    ...
    public static void setLabel(ConnectionProvider conn, WADControl auxControl,
            boolean isSOTrx, String keyName) throws ServletException {
        if (log4j.isDebugEnabled())
            log4j.debug("processing WadUtility.setLabel() - field name: "
                    + auxControl.getData("Name"));
        String strTableID = "", strColumnName = "", strTableName = "";
        if (auxControl.getData("AD_Reference_ID").equals("18")) {
            strTableID = TableLinkData.tableId(conn, auxControl
                    .getData("AD_Reference_Value_ID"));
            strColumnName = TableLinkData.columnName(conn, auxControl
                    .getData("AD_Reference_Value_ID"));
            ...
            if (auxControl.getData("AD_Reference_ID").equals("30"))
                dataSearchs = EditionFieldsData.selectSearchs(conn, "",
                        auxControl.getData("AD_Reference_Value_ID"));
            if (auxControl.getData("AD_Reference_ID").equals("800011")) {
                ...
                strTableName = strTableName.substring(0,
                        (strTableName.length() - 3));
                strColumnName = TableLinkData.keyColumnName(conn, strTableName);
            }
        ...
        if ((strTableID.equals("") || strColumnName.equals(""))
                && !(auxControl.getData("ColumnName").equalsIgnoreCase(
                        "updatedBy") || auxControl.getData("ColumnName")
                        .equalsIgnoreCase("createdBy"))) {
            log4j.warn("There're no table name or column name for: "
                    + auxControl.getData("ColumnName") + " - TABLE_NAME: "
                    + strTableName + " - COLUMN_NAME: " + strColumnName);
        }
        ...
        TableLinkData[] data = TableLinkData.select(conn, strWindowId,
                strTableID);
    ...
    public static void comboReloadScript(EditionFieldsData efd,
            Vector<Object> vecCallOuts, Vector<Object> vecReloads,
            int maxTextboxLength, String strTab) {
        if (vecReloads == null)
            return;
        if (isInVector(vecReloads, efd.columnnameinp)) {
            if (efd.isdisplayed.equals("Y")
                    && (efd.reference.equals("21")
                            || efd.reference.equals("30")
                            || efd.reference.equals("31")
                            || efd.reference.equals("35")
                            || efd.reference.equals("25") || efd.reference
                            .equals("800011")) && efd.calloutname.equals("")) {
    ...
    public static String callouts(EditionFieldsData efd,
            Vector<Object> vecCallOuts, int maxTextboxLength) {
        return callouts(efd, vecCallOuts, maxTextboxLength, false);
    }

    public static String callouts(EditionFieldsData efd,
            Vector<Object> vecCallOuts, int maxTextboxLength, boolean isReload) {
        StringBuffer html = new StringBuffer();
        ...
        if (!efd.calloutname.equals("")) {
            ...
            if (efd.reference.equals("30") || efd.reference.equals("31")
                    || efd.reference.equals("35") || efd.reference.equals("25")
                    || efd.reference.equals("800011")) {
                boolean existDebug = false;
                int i;
                for (i = 0; i < vecCallOuts.size(); i++) {
                    CallOutsStructure data = (CallOutsStructure) vecCallOuts
                            .elementAt(i);
                ...
                if (existDebug) {
                    script.append("  if (keyField==\"inp"
                            + Sqlc.TransformaNombreColumna(efd.columnnameinp)
                            + "\") {\n");
                    /*
                     * if (isReload) { script.append("    submitCommandForm('" +
                     * efd.columnname +
                     * "', false, null, '../ad_callouts/ComboReloads' + document.frmMain.inpTabId.value + '.html', 'hiddenFrame');"
                     * ); }
                     */
                    script.append("    " + (isReload ? "reload" : "callout")
                            + calloutName + "(keyField);\n");
                    script.append("  }\n");
                    CallOutsStructure data = (CallOutsStructure) vecCallOuts
                            .elementAt(i);
                    if (data.method.indexOf(script.toString()) == -1) {
                        ...
                        complete.append(data.method.substring(0, init
                                + header.length()));
                        complete.append(script.toString());
                        complete.append(data.method.substring(init
                                + header.length(), data.method.length()));
                ...
                if (isDateField(efd.reference)) { // Calendar
                    ...
                    if (existDebug) {
                        script
                                .append("  if (keyField==\"inp"
                                        + Sqlc
                                                .TransformaNombreColumna(efd.columnnameinp)
                                        + "\") {\n");
                        script.append("    "
                                + (isReload ? "reload" : "callout")
                                + calloutName + "(keyField);\n");
                        script.append("  }\n");
                        CallOutsStructure data = (CallOutsStructure) vecCallOuts
                                .elementAt(i);
                ...
                } else if (isDecimalNumber(efd.reference)
                        || isPriceNumber(efd.reference)
                        || isIntegerNumber(efd.reference)
                        || isGeneralNumber(efd.reference)
                ...
                html.append((isReload ? "reload" : "callout") + calloutName
                        + "(this.name);");
            }
            ...
            if (!existCallOut) {
                ...
                if (isReload) {
                    ...
                    strCallOut.append("\nfunction callout" + calloutName
                            + "(changedField) {\n");
                    strCallOut
                            .append(
                                    "submitCommandFormParameter('DEFAULT', frmMain.inpLastFieldChanged, changedField, false, null, '..")
                            .append(efd.mappingnameCallout).append(
                                    "', 'hiddenFrame', null, null, true);\n");
                }
    ...
    public static String findField(ConnectionProvider conn,
            EditionFieldsData[] fields, EditionFieldsData[] auxiliars,
            String fieldName) {
        if (fields == null)
            return "";
    ...
    public static String searchsCommand(EditionFieldsData efd,
            boolean fromButton, String tabId, ConnectionProvider conn,
            String windowId, EditionFieldsData[] fieldsData,
        ...
        if (!field.equals("")) {
            ...
            params.append(", document.frmMain.inp"
                    + Sqlc.TransformaNombreColumna(field) + ".value");
        }
        String searchName = (efd.reference.equals("25") ? "/info/Account"
                : ("/info/" + FormatUtilities.replace(efd.searchname.trim())))
                + "_FS.html";
        EditionFieldsData[] fieldsSearch = null;
        try {
            fieldsSearch = EditionFieldsData.selectSearchs(conn, "I",
                    efd.referencevalue);
        ...
        if (fieldsSearch != null && fieldsSearch.length > 0) {
            ...
            if (!fieldsSearch[0].referencevalue.equals("")) {
                for (int i = 0; i < fieldsSearch.length; i++) {
                    field = findField(conn, fieldsData, auxiliarsData,
                            fieldsSearch[i].referencevalue);
                    if (!field.equals("")) {
                        params.append(", 'inp").append(
                                fieldsSearch[i].columnnameinp).append("'");
                        params.append(", document.frmMain.inp"
            ...
            if (!field.equals("")) {
                ...
                params.append(", inputValue(document.frmMain.inp"
                        + Sqlc.TransformaNombreColumna(field) + ")");
            }
        }
        /*
         ...
         * if (!field.equals("")) { params.append(", 'inpPriceList'");
         * params.append(", document.frmMain.inp" +
         * Sqlc.TransformaNombreColumna(field) + ".value"); }
         * field=findField(conn, fieldsData, auxiliarsData, "m_warehouse_id");
         * if (!field.equals("")) { params.append(", 'inpWarehouse'");
         * params.append(", document.frmMain.inp" +
         * Sqlc.TransformaNombreColumna(field) + ".value"); }
         * field=findField(conn, fieldsData, auxiliarsData, "dateordered"); if
         * (!field.equals("")) { params.append(", 'inpDate'");
         * params.append(", document.frmMain.inp" +
         * Sqlc.TransformaNombreColumna(field) + ".value"); } } else if
         * (efd.searchname.equalsIgnoreCase("PROJECT")) { field=findField(conn,
         * fieldsData, auxiliarsData, "c_bpartner_id"); if (!field.equals("")) {
         * params.append(", 'inpBPartner'");
         * params.append(", document.frmMain.inp" +
         * Sqlc.TransformaNombreColumna(field) + ".value"); } } else if
         * (efd.searchname.equalsIgnoreCase("PRODUCT COMPLETE")) {
         * field=findField(conn, fieldsData, auxiliarsData, "m_warehouse_id");
         * if (!field.equals("")) { params.append(", 'inpWarehouse'");
         * params.append(", document.frmMain.inp" +
         * Sqlc.TransformaNombreColumna(field) + ".value"); }
         * field=findField(conn, fieldsData, auxiliarsData, "c_bpartner_id"); if
         * (!field.equals("")) { params.append(", 'inpBPartner'");
         * params.append(", document.frmMain.inp" +
         * Sqlc.TransformaNombreColumna(field) + ".value"); } } else if
         * (efd.searchname.equalsIgnoreCase("SALES ORDER LINE")) {
         * field=findField(conn, fieldsData, auxiliarsData, "c_bpartner_id"); if
         * (!field.equals("")) { params.append(", 'inpBPartner'");
         * params.append(", document.frmMain.inp" +
         * Sqlc.TransformaNombreColumna(field) + ".value"); }
         * field=findField(conn, fieldsData, auxiliarsData, "m_product_id"); if
         * (!field.equals("")) { params.append(", 'inpProduct'");
         * params.append(", document.frmMain.inp" +
         * Sqlc.TransformaNombreColumna(field) + ".value"); } } else if
         * (efd.searchname.equalsIgnoreCase("SHIPMENT/RECEPIT LINE")) {
         * field=findField(conn, fieldsData, auxiliarsData, "c_bpartner_id"); if
         * (!field.equals("")) { params.append(", 'inpBPartner'");
         * params.append(", document.frmMain.inp" +
         * Sqlc.TransformaNombreColumna(field) + ".value"); }
         ...
         * params.append(", document.frmMain.inp" +
         * Sqlc.TransformaNombreColumna(field) + ".value"); } } else if
         * (efd.searchname.toUpperCase().startsWith("ATTRIBUTE")) {
         * strMethodName = "openPAttribute"; params.append(", 'inpKeyValue'");
         * params.append(", document.frmMain.inp" +
         ...
         * params.append(", document.frmMain.inp" +
         * Sqlc.TransformaNombreColumna(field) + ".value"); }
         * field=findField(conn, fieldsData, auxiliarsData, "m_locator_id"); if
         * (!field.equals("")) { params.append(", 'inpLocatorId'");
         * params.append(", document.frmMain.inp" +
         * Sqlc.TransformaNombreColumna(field) + ".value"); } } else if
         * (efd.reference.equals("25")) { field=findField(conn, fieldsData,
         * auxiliarsData, "c_acctschema_id"); if (!field.equals("")) {
         * params.append(", 'inpAcctSchema'");
         * params.append(", inputValue(document.frmMain.inp" +
         * Sqlc.TransformaNombreColumna(field) + ")"); } }
         */
        html.append(strMethodName + "(null, null, '.." + searchName
                + "', null, "
                + ((efd.calloutname.equals("")) ? "false" : "true")
                + ", 'frmMain', 'inp"
    ...
    public static String productSearch(EditionFieldsData efd,
            int maxTextboxLength, Vector<Object> vecCallOuts, String tabId,
            ConnectionProvider conn, String windowId,
            EditionFieldsData[] fieldsData, EditionFieldsData[] auxiliarsData) {
        StringBuffer html = new StringBuffer();
        Vector<Object> vec = new Vector<Object>();
        efd.searchname = "Product Complete";
        html.append(htmlFields(efd, "_LOC", "_LOC", true, maxTextboxLength,
                true, true, vecCallOuts, "", vec, null, false, 0));
        html.append(htmlFields(efd, "_ATR", "_ATR", true, maxTextboxLength,
                true, true, vecCallOuts, "", vec, null, false, 0));
        html.append(htmlFields(efd, "_PQTY", "_PQTY", true, maxTextboxLength,
                true, true, vecCallOuts, "", vec, null, false, 0));
        html.append(htmlFields(efd, "_PUOM", "_PUOM", true, maxTextboxLength,
                true, true, vecCallOuts, "", vec, null, false, 0));
        html.append(htmlFields(efd, "_QTY", "_QTY", true, maxTextboxLength,
                true, true, vecCallOuts, "", vec, null, false, 0));
        html.append(htmlFields(efd, "_UOM", "_UOM", true, maxTextboxLength,
                true, true, vecCallOuts, "", vec, null, false, 0));
        html.append(htmlFields(efd, "_PLIST", "_PLIST", true, maxTextboxLength,
                true, true, vecCallOuts, "", vec, null, false, 0));
        html.append(htmlFields(efd, "_PSTD", "_PSTD", true, maxTextboxLength,
                true, true, vecCallOuts, "", vec, null, false, 0));
        html.append(htmlFields(efd, "_PLIM", "_PLIM", true, maxTextboxLength,
                true, true, vecCallOuts, "", vec, null, false, 0));
        html.append(htmlFields(efd, "_CURR", "_CURR", true, maxTextboxLength,
                true, true, vecCallOuts, "", vec, null, false, 0));
        ...
        html.append("<a href=\"#\"");
        html.append("onClick=\""
                + searchsCommand(efd, true, tabId, conn, windowId, fieldsData,
                        auxiliarsData) + "return false;\" ");
        html.append("onMouseOut=\"window.status='';return true;\"");
        html.append("onMouseOver=\"window.status='").append(efd.referenceName)
                .append("';return true;\" class=\"windowbutton\"><img alt=\"")
                .append(efd.name).append("\" title=\"").append(efd.name)
                .append("\"");
        html.append(" width=\"").append(IMAGE_BUTTON_WIDTH).append(
                "\" height=\"").append(IMAGE_BUTTON_HEIGHT).append("\" ");
        html.append("border=\"0\" src=\"../../../../../web/images/"
                + FormatUtilities.replace(efd.searchname.trim())
                + ".jpg\" id=\"button"
    ...
    public static String searchs(EditionFieldsData efd, int maxTextboxLength,
            Vector<Object> vecCallOuts, String tabId, ConnectionProvider conn,
            String windowId, EditionFieldsData[] fieldsData,
            EditionFieldsData[] auxiliarsData) {
        StringBuffer html = new StringBuffer();
        Vector<Object> vec = new Vector<Object>();
        EditionFieldsData[] fieldsSearch = null;
        try {
            fieldsSearch = EditionFieldsData.selectSearchs(conn, "O",
                    efd.referencevalue);
        } catch (ServletException ex) {
            ex.printStackTrace();
        }
        if (fieldsSearch != null && fieldsSearch.length > 0) {
            if (!fieldsSearch[0].columnnameinp.equals("")) {
                ...
                for (int i = 0; i < fieldsSearch.length; i++) {
                    ...
                    html.append(htmlFields(efd, fieldsSearch[i].columnnameEnd,
                            fieldsSearch[i].columnnameEnd, true,
                            maxTextboxLength, true, true, vecCallOuts, "", vec,
                            null, false, 0));
                }
         ...
         * html.append(htmlFields(efd, "_LOC", "_LOC", true, maxTextboxLength,
         * true, true, vecCallOuts, "", vec, null, false, 0));
         * html.append(htmlFields(efd, "_CON", "_CON", true, maxTextboxLength,
         * true, true, vecCallOuts, "", vec, null, false, 0)); } else if
         * (efd.searchname.equalsIgnoreCase("PRODUCT")) {
         * html.append(htmlFields(efd, "_LOC", "_LOC", true, maxTextboxLength,
         * true, true, vecCallOuts, "", vec, null, false, 0));
         * html.append(htmlFields(efd, "_ATR", "_ATR", true, maxTextboxLength,
         * true, true, vecCallOuts, "", vec, null, false, 0));
         ...
         * html.append(htmlFields(efd, "_QTY", "_QTY", true, maxTextboxLength,
         * true, true, vecCallOuts, "", vec, null, false, 0));
         * html.append(htmlFields(efd, "_PLIST", "_PLIST", true,
        ...
        html.append(" width=\"").append(IMAGE_BUTTON_WIDTH).append(
                "\" height=\"").append(IMAGE_BUTTON_HEIGHT).append("\" ");
        html.append("border=\"0\" src=\"../../../../../web/images/"
                + (efd.reference.equals("25") ? "Account" : FormatUtilities
                        .replace(efd.searchname.trim())) + ".jpg\" id=\"button"
                + FormatUtilities.replace(efd.searchname.trim()) + "\"></a>");
        if (!efd.displaylogic.equals("")) {
            html.append("</span>");
        }
    ...
    public static String locatorCommands(EditionFieldsData efd,
            boolean fromButton, String tabId, ConnectionProvider conn,
            String windowId, EditionFieldsData[] fieldsData,
            EditionFieldsData[] auxiliarsData) {
        StringBuffer params = new StringBuffer();
        ...
        params.append(", 'WindowID'");
        params.append(", '" + windowId + "'");
        String field = findField(conn, fieldsData, auxiliarsData,
                "m_warehouse_id");
        if (!field.equals("")) {
        ...
        html.append("openSearch(null, null, '../info/Locator").append(
                (efd.reference.equals("800013") ? "_Detail" : "")).append(
                "_FS.html', null, "
                        + ((efd.calloutname.equals("")) ? "false" : "true")
                        + ", 'frmMain', 'inp"
                        + Sqlc.TransformaNombreColumna(efd.columnnameinp)
    ...
    public static String locator(EditionFieldsData efd, int maxTextboxLength,
        ...
        html.append("<a href=\"#\"");
        html.append("onClick=\""
                + locatorCommands(efd, true, tabId, conn, windowId, fieldsData,
                        auxiliarsData) + "return false;\" ");
        html.append("onMouseOut=\"window.status='';return true;\"");
        html.append("onMouseOver=\"window.status='").append(efd.referenceName)
                .append("';return true;\" class=\"windowbutton\"><img alt=\"")
    ...
    public static String locationCommands(EditionFieldsData efd) {
        ...
        html
                .append("openLocation(null, null, '../info/Location_FS.html', null, "
                        + ((efd.calloutname.equals("")) ? "false" : "true")
                        + ", 'frmMain', 'inp"
                        + Sqlc.TransformaNombreColumna(efd.columnnameinp)
                        + "', 'inp"
    ...
    public static String buttonsCommand(EditionFieldsData efd,
            String servletName) {
        StringBuffer html = new StringBuffer();
        if (efd.javaClassName.equals("")) {
            html
                    .append("openServletNewWindow('BUTTON"
                            + FormatUtilities.replace(efd.columnname)
                            + efd.adProcessId
                            + "', false, '"
                            + servletName
                            + "', 'BUTTON', null, true"
                            + (efd.columnname.equalsIgnoreCase("CreateFrom") ? ",600, 900"
                                    : "") + ");return false;");
        } else {
            html.append("openServletNewWindow('DEFAULT', false, '.."
                    + (efd.javaClassName.startsWith("/") ? "" : "/")
                    + efd.javaClassName + "', 'BUTTON', '" + efd.adProcessId
                    + "', true" + ",600, 900" + ");return false;");
        }
        return html.toString();
    }

    public static String getSQLWadContext(String code,
        ...
        while (i != -1) {
            if (strValue.length() > (i + 5)
                ...
                if (strAux.substring(strAux.length() - 1).equals("("))
                    strAux = strAux.substring(0, strAux.length() - 1)
                            .toUpperCase().trim();
                if (strAux.substring(strAux.length() - 3, strAux.length())
                        .equals(" IN")) {
                    strAux = " type=\"replace\" optional=\"true\" after=\""
                            + strAux1 + "\" text=\"'" + i + "'\"";
                } else {
                    strAux = "";
                }
                ...
                if (token.substring(0, 1).indexOf("#") > -1
                        || token.substring(0, 1).indexOf("$") > -1)
                    token = token.substring(1, token.length());
                if (strAux.equals(""))
                    strOut.append("?");
                else
                    strOut.append("'" + i + "'");
                vecParameters.addElement("<Parameter name=\"" + token + "\""
                        + strAux + "/>");
                strValue = strValue.substring(j + 1, strValue.length());
                strAux = strValue.trim();
                if (strAux.length() > 0
                        && strAux.substring(0, 1).indexOf("'") > -1)
    ...
    public static String getWadContext(String code, Vector<Object> vecFields,
            Vector<Object> vecAuxiliarFields, FieldsData[] parentsFieldsData,
            boolean isDefaultValue, String isSOTrx, String windowId) {
        if (code == null || code.trim().equals(""))
            return "";
        String token;
        String strValue = code;
        ...
        while (i != -1) {
            if (strValue.length() > (i + 5)
                ...
                token = strValue.substring(0, j);
                strAux = getWadContextTranslate(token, vecFields,
                        vecAuxiliarFields, parentsFieldsData, isDefaultValue,
                        isSOTrx, windowId, true);
                if (!strAux.trim().equals("")
                        && strOut.toString().indexOf(strAux) == -1)
                    strOut.append(", " + strAux);
    ...
    public static String getWadComboReloadContextTranslate(String token,
            String isSOTrx) {
        String result = "";
        if (token.substring(0, 1).indexOf("#") > -1
                || token.substring(0, 1).indexOf("$") > -1) {
            ...
            // else result = "vars.getSessionValue(\"" + token + "\")";
                ...
                result = "Utility.getContext(this, vars, \"" + token
                        + "\", windowId)";
        } else {
    ...
    public static String getTextWadContext(String code,
            Vector<Object> vecFields, Vector<Object> vecAuxiliarFields,
            FieldsData[] parentsFieldsData, boolean isDefaultValue,
        ...
        if (h != -1) {
            ...
            String strFirstElement = getTextWadContext(
                    strValue.substring(0, h), vecFields, vecAuxiliarFields,
            ...
            if (strValue.substring(h + 1).indexOf("@") == -1)
                total.append("\"");
            total.append(getTextWadContext(strValue.substring(h + 1),
                    vecFields, vecAuxiliarFields, parentsFieldsData,
                    isDefaultValue, isSOTrx, windowId));
            if (strValue.substring(h + 1).indexOf("@") == -1)
                total.append("\"");
            total.append("):(");
            if (strValue.substring(0, h).indexOf("@") == -1)
        ...
        while (i != -1) {
            ...
            token = strValue.substring(0, j);
            strOut.append(getWadContextTranslate(token, vecFields,
                    vecAuxiliarFields, parentsFieldsData, isDefaultValue,
                    isSOTrx, windowId, true));
    ...
    public static String getWadContextTranslate(String token,
            Vector<Object> vecFields, Vector<Object> vecAuxiliarFields,
            FieldsData[] parentsFieldsData, boolean isDefaultValue,
        ...
        if (token.substring(0, 1).indexOf("#") > -1
            ...
            // else return "vars.getSessionValue(\"" + token + "\")";
                ...
                return "Utility.getContext(this, vars, \"" + token
                        + "\", windowId)";
        } else {
            String aux = Sqlc.TransformaNombreColumna(token);
            if (token.equalsIgnoreCase("ISSOTRX"))
            ...
            if (!isDefaultValue) {
                if (vecFields != null && findField(vecFields, token)) {
                    return (dataMultiple ? "((dataField!=null)?dataField.getField(\""
                            ...
                            : "((data==null)?\"\":data.")
                            + aux + "))";
                } else if (vecAuxiliarFields != null
                        && findField(vecAuxiliarFields, token)) {
                    return "str" + token;
                }
            }
            return "Utility.getContext(this, vars, \"" + token + "\", \""
    ...
    public static String getWadDefaultValue(FieldsData fd) {
        if (fd == null)
            return "";
        if (fd.referencevalue.equals("28")
                && !fd.name.toUpperCase().endsWith("_ID"))
            return "N"; // Button
        else if (fd.referencevalue.equals("20"))
            return "N"; // YesNo
        else if (fd.required.equals("Y")) {
            if (isDecimalNumber(fd.referencevalue)
                    || isPriceNumber(fd.referencevalue)
                    || isIntegerNumber(fd.referencevalue)
                    || isGeneralNumber(fd.referencevalue)
    ...
    public static String displayLogic(String code, Vector<Object> vecDL,
            FieldsData[] parentsFieldsData, Vector<Object> vecAuxiliar,
            Vector<Object> vecFields, String windowId, Vector<Object> vecContext) {
        if (code == null || code.trim().equals(""))
            return "";
        String token, token2;
        String strValue = code;
        ...
        while (st.hasMoreTokens()) {
            ...
            if (pos[0] >= 0) {
                ...
                token2 = strAux.substring(pos[0]
                        + comparations[pos[1]][0].length(), strAux.length());
                strAux = strAux.substring(0, pos[0])
                        + comparations[pos[1]][1]
                        + strAux.substring(pos[0]
                                + comparations[pos[1]][0].length(), strAux
                                .length());
            }

            strOut.append(getDisplayLogicText(token, vecFields,
                    parentsFieldsData, vecAuxiliar, vecDL, windowId,
                    vecContext, true));
            if (pos[0] >= 0)
                strOut.append(comparations[pos[1]][1]);
            strOut.append(getDisplayLogicText(token2, vecFields,
                    parentsFieldsData, vecAuxiliar, vecDL, windowId,
                    vecContext, false));
        }
        return strOut.toString();
    ...
    public static String getComboReloadText(String token,
            Vector<Object> vecFields, FieldsData[] parentsFieldsData,
            Vector<Object> vecComboReload, String prefix) {
        return getComboReloadText(token, vecFields, parentsFieldsData,
                vecComboReload, prefix, "");
    }

    public static String getComboReloadText(String token,
            Vector<Object> vecFields, FieldsData[] parentsFieldsData,
            Vector<Object> vecComboReload, String prefix, String columnname) {
        StringBuffer strOut = new StringBuffer();
        int i = token.indexOf("@");
        while (i != -1) {
            ...
            if (!token.startsWith("SQL")) {
                ...
                if (i != -1) {
                    ...
                    if (!strOut.toString().trim().equals(""))
                        strOut.append(", ");
                    strOut.append(getComboReloadTextTranslate(strAux,
                            vecFields, parentsFieldsData, vecComboReload,
                            prefix, columnname));
                }
            }
            i = token.indexOf("@");
    ...
    public static String getComboReloadTextTranslate(String token,
            Vector<Object> vecFields, FieldsData[] parentsFieldsData,
            Vector<Object> vecComboReload, String prefix, String columnname) {
            ...
            for (int i = 0; i < parentsFieldsData.length; i++) {
                if (parentsFieldsData[i].name.equalsIgnoreCase(token))
                    return ((prefix.equals("")) ? ("\""
                            + parentsFieldsData[i].name + "\"")
                            : ("\""
                                    + prefix
                                    + Sqlc
                                            .TransformaNombreColumna(parentsFieldsData[i].name) + "\""));
            }
        }
        if (vecFields != null && findField(vecFields, token)) {
    ...
    public static String getDisplayLogicText(String token,
            Vector<Object> vecFields, FieldsData[] parentsFieldsData,
            Vector<Object> vecAuxiliar, Vector<Object> vecDisplayLogic,
        ...
        while (i != -1) {
            ...
            if (i != -1) {
                String strAux = token.substring(0, i);
                token = token.substring(i + 1);
                strOut.append(getDisplayLogicTextTranslate(strAux, vecFields,
                        parentsFieldsData, vecAuxiliar, vecDisplayLogic,
                        windowId, vecContext, save));
            }
            i = token.indexOf("@");
        }
        strOut.append(token);
    ...
    public static String getDisplayLogicTextTranslate(String token,
            Vector<Object> vecFields, FieldsData[] parentsFieldsData,
            Vector<Object> vecAuxiliar, Vector<Object> vecDisplayLogic,
            String windowId, Vector<Object> vecContext, boolean save) {
        if (token == null || token.trim().equals(""))
    ...
    public static String sqlCasting(ConnectionProvider conn, String reference,
        ...
        if (reference == null || reference.equals(""))
            ...
            return "";
        else if (isIntegerNumber(reference) || isPriceNumber(reference)
                || isQtyNumber(reference) || isGeneralNumber(reference)
                || isDecimalNumber(reference))
            return "TO_NUMBER";
            ...
            if (referencevalue == null)
                return "";
            try {
                TableRelationData trd[] = TableRelationData.selectRefTable(
                        conn, referencevalue);
                if (trd == null || trd.length == 0)
                    return "";
                return sqlCasting(conn, trd[0].referencekey,
                        trd[0].referencevaluekey);
            } catch (ServletException ex) {
    ...
    public static void setPropertyValue(Properties _prop, FieldProvider _field,
            String _name, String _fieldName, String _defaultValue)
            throws Exception {
        String aux = "";
        try {
    ...
    public static WADControl getControl(ConnectionProvider conn,
            FieldProvider field, boolean isreadonly, String tabName,
            String adLanguage, XmlEngine xmlEngine, boolean isDisplayLogic,
            boolean isReloadObject, boolean isReadOnlyLogic,
            boolean hasParentsFields) throws Exception {
        return getControl(conn, field, isreadonly, tabName, adLanguage,
                xmlEngine, isDisplayLogic, isReloadObject, isReadOnlyLogic,
    ...
    public static WADControl getControl(ConnectionProvider conn,
            ...
            String adLanguage, XmlEngine xmlEngine, boolean isDisplayLogic,
            boolean isReloadObject, boolean isReadOnlyLogic,
            boolean hasParentsFields, boolean isReadOnlyDefinedTab)
            throws Exception {
        if (field == null)
            return null;
        Properties prop = new Properties();
        setPropertyValue(prop, field, "ColumnName", "columnname", null);
        prop.setProperty("ColumnNameInp", Sqlc.TransformaNombreColumna(field
                .getField("columnname")));
        ...
        setPropertyValue(prop, field, "ReferenceNameTrl", "referenceNameTrl",
                "");
        setPropertyValue(prop, field, "AD_Reference_Value_ID",
                "referencevalue", "");
        ...
        prop.setProperty("isReadOnlyDefinedTab", (isReadOnlyDefinedTab ? "Y"
                : "N"));
    ...
    public static boolean isNewGroup(WADControl control, String strFieldGroup) {
        ...
        return (control.getData("IsDisplayed").equals("Y")
                && fieldgroup != null && !fieldgroup.equals("") && !fieldgroup
                .equals(strFieldGroup));
    }

    public static String getReadOnlyLogic(WADControl auxControl,
            Vector<Object> vecDL, FieldsData[] parentsFieldsData,
            Vector<Object> vecAuxiliar, Vector<Object> vecFields,
        ...
        _displayLogic.append("  readOnlyLogicElement('").append(element)
                .append("', (").append(
                        displayLogic(code, vecDL, parentsFieldsData,
                                vecAuxiliar, vecFields, windowId, vecContext))
                .append("));\n");
    ...
    public static String getbuttonShortcuts(HashMap<String, String> sc) {
        ...
        while (ik.hasNext() && iv.hasNext()) {
            ...
            shortcuts.append("keyArray[keyArray.length] = new keyArrayItem(\"")
                    .append(ik.next()).append("\", \"").append(iv.next())
                    .append("\", null, \"altKey\", false, \"onkeydown\");\n");
        }
        return shortcuts.toString();
    }

    public static String getDisplayLogicForGroups(String strFieldGroup,
            StringBuffer code) {
        if ((code == null) || (code.length() == 0))
            return "";
        StringBuffer _displayLogic = new StringBuffer();
        _displayLogic.append("if ").append(code).append("{\n");
        _displayLogic.append("  displayLogicElement('fldgrp").append(
                strFieldGroup).append("', true);\n");
        _displayLogic.append("} else {\n");
        _displayLogic.append("  displayLogicElement('fldgrp").append(
                strFieldGroup).append("', false);\n");
        _displayLogic.append("}\n");
        return _displayLogic.toString();
    }

    public static String getDisplayLogic(WADControl auxControl,
            Vector<Object> vecDL, FieldsData[] parentsFieldsData,
            Vector<Object> vecAuxiliar, Vector<Object> vecFields,
            String windowId, Vector<Object> vecContext, boolean isreadonly) {
        ...
        _displayLogic.append(displayLogic(code, vecDL, parentsFieldsData,
                vecAuxiliar, vecFields, windowId, vecContext));
        _displayLogic.append(") {\n");
    ...
    public static void writeFile(File path, String filename, String text)
            throws IOException {
        File fileData = new File(path, filename);
        FileOutputStream fileWriterData = new FileOutputStream(fileData);
        OutputStreamWriter printWriterData = new OutputStreamWriter(
                fileWriterData, "UTF-8");
