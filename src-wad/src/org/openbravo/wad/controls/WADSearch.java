...
public class WADSearch extends WADControl {
        ...
        if (getData("IsMandatory").equals("Y")) {
            ...
            if (getData("IsDisplayed").equals("Y"))
                validation.append("    setWindowElementFocus(frm.inp").append(
                        getData("ColumnNameInp")).append("_R);\n");
        ...
        if (!getData("IsReadOnly").equals("Y")
            ...
            try {
                if (!getData("AD_Reference_Value_ID").equals("")) {
                    WADSearchData[] data = WADSearchData.select(
                            getConnection(), getData("AD_Language"),
                    ...
                    if (data != null && data.length > 0) {
                        ...
                        for (int i = 0; i < data.length; i++) {
                            if (data[i].columntype.equals("I")) {
                                columnsScript.append(", 'inp").append(
                                        data[i].name).append("'");
                                columnsScript
                                        .append(
                                                ", inputValue(document.frmMain.inp")
                                        .append(
                                                Sqlc
                                ...
                                hiddenScript
                                        ...
                                        .append(
                                                Sqlc
                                                        .TransformaNombreColumna(data[i].name));
                                hiddenScript.append(data[i].columnSuffix)
                                        .append("\" value=\"\" ");
                                hiddenScript.append("id=\"").append(
                                        data[i].columnname).append(
                                        data[i].columnSuffix).append("\"/>\n");
                            }
                        }
                    }
                }
            ...
            commandScript.append("openSearch(null, null, '..").append(
                    servletName).append("', ");
            commandScript.append("null, false, 'frmMain', 'inp").append(
                    getData("ColumnNameInp")).append("', ");
            commandScript.append("'inp").append(getData("ColumnNameInp"))
                    .append("_R', ");
            commandScript.append("inputValue(document.frmMain.inp").append(
                    getData("ColumnNameInp")).append("_R), ");
            commandScript.append(
                    "'inpIDValue', inputValue(document.frmMain.inp").append(
                    getData("ColumnNameInp")).append("), ");
            commandScript
                    .append("'WindowID', inputValue(document.frmMain.inpwindowId)");
            commandScript.append(columnsScript);
            text.append(commandScript).append(", 'Command', 'KEY'");
            commandScript.append(");");
            text.append(");");
            setOnLoad("keyArray[keyArray.length] = new keyArrayItem(\"ENTER\", \""
                    + text.toString()
                    + "\", \"inp"
    ...
    public String editMode() {
        ...
        XmlDocument xmlDocument = getReportEngine().readXmlTemplate(
                "org/openbravo/wad/controls/WADSearch", discard)
                .createXmlDocument();
    ...
    public String toXml() {
        ...
        XmlDocument xmlDocument = getReportEngine().readXmlTemplate(
                "org/openbravo/wad/controls/WADSearchXML", discard)
                .createXmlDocument();
