...
public class WADString extends WADControl {
    ...
    public void initialize() {
        generateJSCode();
        this.button = new WADFieldButton("Password", getData("ColumnName"),
                getData("ColumnNameInp"), getData("Name"),
                "openServletNewWindow('BUTTON" + getData("ColumnName")
                        + "', false, '" + getData("TabName")
                        + "_Edition.html', 'BUTTON', null, true, 300, 600);");
    }
    ...
    public String editMode() {
        ...
        if (getData("IsEncrypted").equals("Y")) {
            ...
            xmlDocument = getReportEngine().readXmlTemplate(
                    "org/openbravo/wad/controls/WADStringEncrypted", discard)
                    .createXmlDocument();
            xmlDocument.setParameter("hasButton",
                    (textButton.equals("") ? "TextButton_ContentCell" : ""));
            xmlDocument.setParameter("buttonClass", buttonClass
                    + "_ContentCell");
            xmlDocument.setParameter("button", textButton);
            isDisabled = true;
        } else {
            xmlDocument = getReportEngine().readXmlTemplate(
                    "org/openbravo/wad/controls/WADString", discard)
                    .createXmlDocument();
        }
        ...
        if (!isDisabled)
            isDisabled = (getData("IsReadOnly").equals("Y")
    ...
    public String newMode() {
        ...
        if (!isDisabled)
            isDisabled = (getData("IsReadOnly").equals("Y") || getData(
