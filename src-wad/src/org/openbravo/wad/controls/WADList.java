...
public class WADList extends WADControl {
  ...
  private void generateJSCode() {
    ...
    setOnLoad("if (inputValue(key)==null || inputValue(key)==\"\") updateOnChange(frm.inp" + getData("ColumnNameInp") + ");");
    setCalloutJS();
  }
  ...
  public String editMode() {
    String[] discard = {""};
    if (getData("IsMandatory").equals("Y")) discard[0] = "fieldBlankSection";
    XmlDocument xmlDocument = getReportEngine().readXmlTemplate("org/openbravo/wad/controls/WADList", discard).createXmlDocument();

    xmlDocument.setParameter("columnName", getData("ColumnName"));
    xmlDocument.setParameter("columnNameInp", getData("ColumnNameInp"));
    String length = getData("DisplayLength");
    if (!length.endsWith("%")) length += "px";
    xmlDocument.setParameter("size", getData("CssSize"));
    
    String auxClassName = "";
    if (getData("IsMandatory").equals("Y")) auxClassName += "Key";
    if (getData("IsReadOnly").equals("Y") || getData("IsReadOnlyTab").equals("Y")) auxClassName += "ReadOnly";
    else if (getData("IsUpdateable").equals("N")) auxClassName += "NoUpdatable";
    xmlDocument.setParameter("myClass", auxClassName);

    if (getData("IsReadOnly").equals("Y") || getData("IsReadOnlyTab").equals("Y") || getData("IsUpdateable").equals("N")) xmlDocument.setParameter("disabled", "Y");
    else xmlDocument.setParameter("disabled", "N");
    //if (getData("IsMandatory").equals("Y")) xmlDocument.setParameter("required", "Y");
    //else xmlDocument.setParameter("required", "N");

    StringBuffer text = new StringBuffer();
    if (getData("IsReadOnly").equals("Y") || getData("IsReadOnlyTab").equals("Y") || getData("IsUpdateable").equals("N")) text.append("selectCombo(this, 'xx');");
  ...
  public String newMode() {
    ...
    if (getData("IsReadOnly").equals("Y") || getData("IsReadOnlyTab").equals("Y")) xmlDocument.setParameter("disabled", "Y");
    else xmlDocument.setParameter("disabled", "N");
    //if (getData("IsMandatory").equals("Y")) xmlDocument.setParameter("required", "Y");
    //else xmlDocument.setParameter("required", "N");

    StringBuffer text = new StringBuffer();
    if (getData("IsReadOnly").equals("Y") || getData("IsReadOnlyTab").equals("Y")) text.append("selectCombo(this, 'xx');");
    text.append(getOnChangeCode());
    xmlDocument.setParameter("callout", text.toString());
  ...
  public String toXml() {
    ...
    if (getData("IsParameter").equals("Y")) {
      ...
      if (getData("IsReadOnly").equals("Y") || getData("IsReadOnlyTab").equals("Y") || getData("IsUpdateable").equals("N")) {
        ...
        text.append("_S\" name=\"report").append(getData("ColumnName")).append("_S\" attribute=\"onchange\" replace=\"xx\"/>");
      }
    } else {
    ...
    if (getData("IsDisplayed").equals("Y")) {
      ...
      text.append("  <ARGUMENT name=\"parameterListSelected\" withId=\"").append(getData("ColumnName")).append("\"/>\n");
  ...
  public String toJava() {
    ...
    if (getData("IsDisplayed").equals("Y")) {
      ...
      text.append("xmlDocument.setData(\"report").append(getData("ColumnName")).append("\",\"liststructure\", ");
