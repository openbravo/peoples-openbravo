...
public class WADList extends WADControl {
    ...
    public String editMode() {
        ...
        XmlDocument xmlDocument = getReportEngine().readXmlTemplate(
                "org/openbravo/wad/controls/WADList", discard)
                .createXmlDocument();
    ...
    public String toXml() {
        ...
        if (getData("IsParameter").equals("Y")) {
            ...
            text.append("\" name=\"").append(getData("ColumnName")).append(
                    "\" attribute=\"value\"/>");
            if (getData("IsReadOnly").equals("Y")
                    || getData("IsReadOnlyTab").equals("Y")
                    || getData("IsUpdateable").equals("N")) {
                text.append("\n<PARAMETER id=\"report").append(
                        getData("ColumnName"));
        ...
        if (getData("IsDisplayed").equals("Y")) {
            text.append("\n<SUBREPORT id=\"report").append(
                    getData("ColumnName"));
            text.append("\" name=\"report").append(getData("ColumnName"));
            text
                    .append("\" report=\"org/openbravo/erpCommon/reference/List\">\n");
    ...
    public String toJava() {
        ...
        if (getData("IsDisplayed").equals("Y")) {
            ...
            if (getData("ColumnName").equalsIgnoreCase("AD_Client_ID"))
                ...
                text
                        .append("Utility.getContext(this, vars, \"#User_Client\", windowId), 0);\n");
            ...
            text.append(getData("ColumnNameInp")).append(
                    "\"):dataField.getField(\"");
            text.append(getData("ColumnNameInp")).append("\")));\n");
            text.append("xmlDocument.setData(\"report").append(
                    getData("ColumnName")).append("\",\"liststructure\", ");
            text
                    .append("comboTableData.select(!strCommand.equals(\"NEW\")));\n");
