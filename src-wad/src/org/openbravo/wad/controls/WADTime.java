...
public class WADTime extends WADControl {
    ...
    private void generateJSCode() {
        ...
        if (getData("IsMandatory").equals("Y")) {
            validation.append("  if (inputValue(frm.inp").append(
                    getData("ColumnNameInp")).append(
                    ")==null || inputValue(frm.inp").append(
                    getData("ColumnNameInp")).append(")==\"\") {\n");
            if (getData("IsDisplayed").equals("Y"))
                validation.append("    setWindowElementFocus(frm.inp").append(
                        getData("ColumnNameInp")).append(");\n");
        ...
        if (!getData("ValueMin").equals("") || !getData("ValueMax").equals("")) {
            validation.append("  if (inputValue(frm.inp").append(
                    getData("ColumnNameInp")).append(")!=null && ");
            validation.append("inputValue(frm.inp").append(
                    getData("ColumnNameInp")).append(")!=\"\" && (");
            boolean valmin = false;
            if (!getData("ValueMin").equals("")) {
                validation.append("timecmp(frm.inp").append(
                        getData("ColumnNameInp")).append(".value, '").append(
                        getData("ValueMin")).append("')<0");
                valmin = true;
            }
    ...
    public String editMode() {
         ...
         * this.button.setReportEngine(getReportEngine()); textButton =
         * this.button.toString(); buttonClass = this.button.getType(); }
         */
        String[] discard = { "" };
        if (!getData("IsMandatory").equals("Y")) {
        ...
        XmlDocument xmlDocument = getReportEngine().readXmlTemplate(
                "org/openbravo/wad/controls/WADTime", discard)
                .createXmlDocument();
    ...
    public String toXml() {
        ...
        if (getData("IsParameter").equals("Y")) {
            text.append("<PARAMETER id=\"").append(getData("ColumnName"))
                    .append("\" name=\"").append(getData("ColumnName")).append(
                            "\" attribute=\"value\"/>");
        } else {
