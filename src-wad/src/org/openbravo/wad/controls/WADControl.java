...
public class WADControl {
    ...
    public void generateMessages() {
        try {
            invalid = WADControlData.getMessage(getConnection(),
                    getData("AD_Language"), "JSInvalid");
            missing = WADControlData.getMessage(getConnection(),
                    getData("AD_Language"), "JSMissing");
            range = WADControlData.getMessage(getConnection(),
                    getData("AD_Language"), "JSRange");
        } catch (Exception ex) {
            ex.printStackTrace();
        }
    }
    ...
    private void generateJSCode() {
        addImport("ValidationTextBox",
                "../../../../../web/js/default/ValidationTextBox.js");
        if (getData("IsMandatory").equals("Y")) {
            XmlDocument xmlDocument = getReportEngine().readXmlTemplate(
                    "org/openbravo/wad/controls/WADControlJSValidation")
                    .createXmlDocument();
    ...
    public String getOnChangeCode() {
        ...
        if (isComboReload.equals("Y"))
            text.append("reloadComboReloads").append(getData("AD_Tab_ID"))
                    .append("(this.name);");
    ...
    public String toString() {
        StringBuffer text = new StringBuffer();
        if (getData("IsDisplayed").equals("N")) {
            WADControl aux = new WADHidden(getData("ColumnName"),
                    getData("ColumnNameInp"), "data", false);
    ...
    public String toXml() {
        ...
        if (getData("IsParameter").equals("Y")) {
            ...
            text
                    .append("\" attribute=\"value\" default=\"N\" replaceCharacters=\"htmlPreformated\"/>");
        } else {
            text.append("<FIELD id=\"").append(getData("ColumnName"));
            text
                    .append("\" attribute=\"value\" replaceCharacters=\"htmlPreformated\" default=\"N\">");
    ...
    public String toLabelXML() {
        ...
        if (label.getLabelId() != null && !label.getLabelId().equals("")
                && label.getLabelPlaceHolderText() != null
                && !label.getLabelPlaceHolderText().equals("")) {
    ...
    private void createWADLabelControl() {
        ...
        label = new WADLabelControl(WADLabelControl.FIELD_LABEL, null, null,
                columnId, column, null, null, getData("IsLinkable"),
                getData("KeyColumnName"), getData("ColumnNameInp"),
                getData("AD_Table_ID"));
    }
}
