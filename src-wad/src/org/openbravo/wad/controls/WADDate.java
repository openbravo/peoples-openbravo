...
public class WADDate extends WADControl {
  ...
  public void initialize() {
    generateJSCode();
    this.button = new WADFieldButton("Calendar", getData("ColumnName"), getData("ColumnNameInp"), getData("Name"), "showCalendar('frmMain.inp" + getData("ColumnNameInp") + "', document.frmMain.inp" + getData("ColumnNameInp") + ".value, false);");
  }
  ...
  private void generateJSCode() {
    ...
    if (!getData("AD_Language").equals(""))
      addImport("calendarLang", "../../../../../web/js/jscalendar/lang/calendar-" + getData("AD_Language").substring(0, 2) + ".js");
    else addImport("calendarLang", "../../../../../web/js/jscalendar/lang/calendar-en.js");
    generateValidation();
    setCalloutJS();
  }
  ...
  private void generateValidation() {
    ...
    if (getData("ValueMin").trim().equals("") && getData("ValueMax").trim().equals("")) discard[1] = "isValueCheck";
    else {
    ...
    XmlDocument xmlDocument = getReportEngine().readXmlTemplate("org/openbravo/wad/controls/WADDateJSValidation", discard).createXmlDocument();
  ...
  public String editMode() {
    ...
    XmlDocument xmlDocument = getReportEngine().readXmlTemplate("org/openbravo/wad/controls/WADDate", discard).createXmlDocument();
  ...
  public String toXml() {
    ...
    XmlDocument xmlDocument = getReportEngine().readXmlTemplate("org/openbravo/wad/controls/WADDateXML", discard).createXmlDocument();
    xmlDocument.setParameter("columnName", getData("ColumnName"));
    return replaceHTML(xmlDocument.print());
  }
  ...
  public String toJava() {
    ...
    text.append("xmlDocument.setParameter(\"").append(getData("ColumnName")).append("_Format\", vars.getSessionValue(\"#AD_SqlDateFormat\"));");
