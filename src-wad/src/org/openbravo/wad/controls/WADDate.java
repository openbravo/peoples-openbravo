...
public class WADDate extends WADControl {
    ...
    public void initialize() {
        generateJSCode();
        this.button = new WADFieldButton("Calendar", getData("ColumnName"),
                getData("ColumnNameInp"), getData("Name"),
                "showCalendar('frmMain.inp" + getData("ColumnNameInp")
    ...
    private void generateValidation() {
        ...
        if (getData("ValueMin").trim().equals("")
                && getData("ValueMax").trim().equals(""))
        ...
        XmlDocument xmlDocument = getReportEngine().readXmlTemplate(
                "org/openbravo/wad/controls/WADDateJSValidation", discard)
                .createXmlDocument();
    ...
    public String editMode() {
        ...
        XmlDocument xmlDocument = getReportEngine().readXmlTemplate(
                "org/openbravo/wad/controls/WADDate", discard)
                .createXmlDocument();
    ...
    public String toXml() {
        ...
        if (getData("IsParameter").equals("Y"))
            discard[0] = "xx";
        XmlDocument xmlDocument = getReportEngine().readXmlTemplate(
                "org/openbravo/wad/controls/WADDateXML", discard)
                .createXmlDocument();
        xmlDocument.setParameter("columnName", getData("ColumnName"));
        return replaceHTML(xmlDocument.print());
    }
