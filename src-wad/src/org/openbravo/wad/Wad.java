...
/**
 * Main class of WAD project. This class manage all the process to generate the
...
public class Wad extends DefaultHandler {
     ...
     * The list of arguments that it can receive are the following ones:<br>
     * <ol>
     ...
     * <li>Boolean to indicate if it's gonna be made a complete generation or
     * not</li>
     * </ol>
     * 
     * @param argv
    ...
    public static void main(String argv[]) throws Exception {
        ...
        if (argv.length < 1) {
            log4j
                    .error("Usage: java Wad connection.xml [{% || Window} [destinyDir]]");
            return;
        }
        ...
        try {
            ...
            log4j.info("directory reference: " + dirReference
                    + wad.strSystemSeparator + "reference");
            ...
            final File fileFinReloads = new File(dirReference
                    + wad.strSystemSeparator + "ad_callouts");
            if (!fileFinReloads.exists()) {
                log4j.error("No such directory: "
                        + fileFinReloads.getAbsoluteFile());

                return;
            }

            final File fileReference = new File(dirReference
                    + wad.strSystemSeparator + "reference");
            if (!fileReference.exists()) {
                log4j.error("No such directory: "
                        + fileReference.getAbsoluteFile());
            ...
            if (dirWebClients != null && !dirWebClients.equals("")) {
                ...
                if (!fileWebXmlClient.exists()) {
                    log4j.warn("No such directory: "
                            + fileWebXmlClient.getAbsoluteFile());
    ...
    private void processActionButton(File fileReference) {
        try {
            ...
            e.printStackTrace();
            log4j
                    .error("Problem of ServletExceptio in process of ActionButtonData");
        ...
        try {
            ...
            if (fd != null) {
                for (int i = 0; i < fd.length; i++) {
                    ...
                    WadActionButton.buildXml(pool, xmlEngine, fileReference,
                            fd[i], vecFields, MAX_TEXTBOX_LENGTH);
                }
            }
        } catch (final ServletException e) {
            e.printStackTrace();
            log4j
                    .error("Problem of ServletExceptio in process of ActionButtonXml");
        } catch (final IOException e) {
            e.printStackTrace();
    ...
    private void processActionButtonHtml(File fileReference) {
        try {
            ...
            if (fd != null) {
                for (int i = 0; i < fd.length; i++) {
                    final Vector<Object> vecFields = new Vector<Object>();
                    WadActionButton.buildHtml(pool, xmlEngine, fileReference,
                            fd[i], vecFields, MAX_TEXTBOX_LENGTH,
                            MAX_SIZE_EDITION_1_COLUMNS, "", false,
                            calendarDescription, clockDescription,
                            calculatorDescription, jsDateFormat);
                }
            }
        } catch (final ServletException e) {
            e.printStackTrace();
            log4j
                    .error("Problem of ServletExceptio in process of ActionButtonHtml");
        } catch (final IOException e) {
            e.printStackTrace();
    ...
    private void processActionButtonGenerics(File fileReference) {
        try {
            ...
            XmlDocument xmlDocumentData = xmlEngine.readXmlTemplate(
                    "org/openbravo/wad/ActionButton_Responser")
                    .createXmlDocument();

            ActionButtonRelationData[] abrd = WadActionButton
                    .buildActionButtonCallGenerics(pool);
            xmlDocumentData.setData("structure1", abrd);
            xmlDocumentData.setData("structure2", abrd);
            xmlDocumentData.setData("structure3", abrd);

            WadUtility.writeFile(fileReference, "ActionButton_Responser.java",
                    xmlDocumentData.print());

            // Generic action button for java
            log4j.info("Processing ActionButton_ResponserJava.xml");
            xmlDocumentData = xmlEngine.readXmlTemplate(
                    "org/openbravo/wad/ActionButtonJava_Responser")
                    .createXmlDocument();
            ...
            WadUtility.writeFile(fileReference,
                    "ActionButtonJava_Responser.java", xmlDocumentData.print());

        } catch (final IOException e) {
            e.printStackTrace();
            log4j
                    .error("Problem of IOExceptio in process of ActionButton_Responser");
        }
    }

    /**
     * Generates the action button's xml files. These are the menu's action
     * buttons.
     * 
     * @param fileReference
     *            The path where to create the files.
    ...
    private void processActionButtonXmlGenerics(File fileReference) {
        try {
            ...
            e.printStackTrace();
            log4j
                    .error("Problem of ServletExceptio in process of ActionButtonXml Generics");
        } catch (final IOException e) {
            e.printStackTrace();
            log4j
                    .error("Problem of IOExceptio in process of ActionButtonXml Generics");
        }
    }

    /**
     * Generates the action button's html files. These are the menu's action
     * button
     * 
     * @param fileReference
     *            The path where to create the files.
        ...
        try {
            ...
            if (fd != null) {
                for (int i = 0; i < fd.length; i++) {
                    ...
                    WadActionButton.buildHtml(pool, xmlEngine, fileReference,
                            fd[i], vecFields, MAX_TEXTBOX_LENGTH,
                            MAX_SIZE_EDITION_1_COLUMNS, "", true,
                            calendarDescription, clockDescription,
                            calculatorDescription, jsDateFormat);
                }
            }
            ...
            e.printStackTrace();
            log4j
                    .error("Problem of ServletExceptio in process of ActionButtonHtml Generics");
        } catch (final IOException e) {
            e.printStackTrace();
            log4j
                    .error("Problem of IOExceptio in process of ActionButtonHtml Generics");
        }
    }
     ...
     * @throws IOException
     */
    private void processWebXml(File fileWebXml, File fileClients,
            String attachPath, String webPath) throws ServletException,
            IOException {
        try {
            log4j.info("Processing web.xml");
            final XmlDocument xmlDocument = xmlEngine.readXmlTemplate(
                    "org/openbravo/wad/webConf").createXmlDocument();
            final StringBuffer sb = new StringBuffer();

            try {
                if (fileClients != null) {
                    final BufferedReader fileBuffer = new BufferedReader(
                            new FileReader(fileClients));
            ...
            xmlDocument.setData("structureListener", WadData
                    .selectListener(pool));
            xmlDocument.setData("structureResource", WadData
                    .selectResource(pool));
            ...
            xmlDocument.setDataArray("reportFilterParams", "structure1",
                    filterParams);
            ...
            xmlDocument.setDataArray("reportServletParams", "structure1",
                    servletParams);
            xmlDocument.setData("structureFilterMapping", WadData
                    .selectFilterMapping(pool));
            xmlDocument.setData("structure2", WadData.selectMapping(pool));
            WadUtility.writeFile(fileWebXml, "web.xml",
                    "<?xml version=\"1.0\" encoding=\"UTF-8\"?>\n"
                            + xmlDocument.print());
        } catch (final IOException e) {
            e.printStackTrace();
     ...
     * @throws Exception
     */
    private void processTab(File fileFin, File fileFinReloads,
            TabsData tabsData, File fileTrl, String dirBaseTrl,
            String translateStr, File fileBase, File fileBaseAplication)
            throws Exception {
        try {
            final String tabNamePresentation = tabsData.realtabname;
            final String tabName = FormatUtilities.replace(tabNamePresentation);
            final String windowName = FormatUtilities
                    .replace(tabsData.windowname);
            ...
            final TabsData[] allTabs = getPrimaryTabs(tabsData.key,
                    tabsData.tabid, Integer.valueOf(tabsData.tablevel)
                            .intValue(), HEIGHT_TABS, INCR_TABS);
            final FieldsData[] fieldsData = FieldsData.select(pool,
                    tabsData.tabid);
            final EditionFieldsData efd[] = EditionFieldsData.select(pool,
                    tabsData.tabid);
            ...
            if (allTabs == null || allTabs.length == 0)
                throw new Exception("No tabs found for AD_Tab_ID: "
                        + tabsData.tabid + " - key: " + tabsData.key
                        + " - level: " + tabsData.tablevel);
            final TabsData[] tab1 = new TabsData[(allTabs.length > NUM_TABS) ? NUM_TABS
                    : allTabs.length];
            final TabsData[] tab2 = new TabsData[(allTabs.length > NUM_TABS) ? NUM_TABS
                    : 0];
            ...
            if (tabsData.issorttab.equals("Y")) {
                parentsFieldsData = FieldsData.parentsColumnNameSortTab(pool,
                        (parentTabIndex != -1 ? allTabs[parentTabIndex].tabid
                                : ""), tabsData.tableId);
                parentsFieldsNameData = FieldsData
                        .parentsColumnDisplayNameSortTab(pool, "",
                                tabsData.tableId);
            } else {
                parentsFieldsData = FieldsData.parentsColumnName(pool,
                        (parentTabIndex != -1 ? allTabs[parentTabIndex].tabid
                                : ""), tabsData.tabid);
                parentsFieldsNameData = FieldsData.parentsColumnDisplayName(
                        pool, "", tabsData.tabid,
                        (parentTabIndex != -1 ? allTabs[parentTabIndex].tabid
                                : ""));
            }
            ...
            if (parentTabIndex != -1
                    && (parentsFieldsData == null || parentsFieldsData.length == 0)) {
                parentsFieldsData = FieldsData.parentsColumnReal(pool,
                        allTabs[parentTabIndex].tabid, tabsData.tabid);
                parentsFieldsNameData = FieldsData
                        .parentsColumnDisplayNameReal(pool, "", tabsData.tabid,
                ...
                if (parentsFieldsData == null || parentsFieldsData.length == 0) {
                    log4j.error("No key found in parent tab: "
                            + allTabs[parentTabIndex].tabname);
                }
            }
            ...
            processTable(parentsFieldsData, tabsData.tabid, vecFields,
                    vecTables, vecWhere, vecOrder, vecParameters, tableName,
                    tabsData.windowtype, tabsData.tablevel, vecTableParameters,
                    fieldsData);
            ...
            for (final Enumeration<Object> e = vecFields.elements(); e
                    .hasMoreElements();) {
            ...
            for (final Enumeration<Object> e = vecTables.elements(); e
                    .hasMoreElements();) {
                final String tableElement = (String) e.nextElement();
                strTables.append((tableElement.trim().toLowerCase().startsWith(
                        "left join") ? " " : ", ")
                        + tableElement);
            }
            log4j.debug("Tables of select: " + strTables.toString());
            final StringBuffer strWhere = new StringBuffer();
            for (final Enumeration<Object> e = vecWhere.elements(); e
                    .hasMoreElements();) {
                final String whereElement = (String) e.nextElement();
                strWhere.append((!whereElement.startsWith(" AND") ? " AND "
                        : "")
                        + whereElement);
            }
            ...
            if (!tabsData.whereclause.equals("")) {
                ...
                strWhere.append(" AND "
                        + WadUtility.buildSQL(tabsData.whereclause,
                                vecTotalParameters));
                if (totalParameters < vecTotalParameters.size()) {
                    for (int h = totalParameters; h < vecTotalParameters.size(); h++) {
                        String strParam = (String) vecTotalParameters
                                .elementAt(h);
                        vecParameters.addElement(strParam);
                        strParam = strParam.substring(17, strParam
                                .lastIndexOf("\""));
                        whereClauseParams += ", Utility.getContext(this, vars, \""
                                + strParam + "\", windowId)";
                    }
                }
            }
            ...
            if (tabsData.orderbyclause.equals("")) {
                ...
                boolean first = true;
                for (final Enumeration<Object> e = vecOrder.elements(); e
                        .hasMoreElements();) {
            ...
            EditionFieldsData[] selCol = EditionFieldsData
                    .selectSerchFieldsSelection(pool, "", tabsData.tabid);
            if (selCol == null || selCol.length == 0)
                selCol = EditionFieldsData.selectSerchFields(pool, "",
                        tabsData.tabid);
            selCol = processSelCol(selCol, tableName);

            final String javaPackage = (!tabsData.javapackage.equals("") ? tabsData.javapackage
                    .replace(".", "/")
                    + "/"
                    : "")
            ...
            if (parentTabIndex != -1 && allTabs != null && allTabs.length > 0) {
                parentwhereclause = FieldsData.selectParentWhereClause(pool,
                        allTabs[parentTabIndex].tabid);
                ...
                if (vecParametersParent.size() > 0) {
                    for (int h = 0; h < vecParametersParent.size(); h++) {
                        String strParam = (String) vecParametersParent
                                .elementAt(h);
                        strParam = strParam.substring(17, strParam
                                .lastIndexOf("\""));
                        parentwhereclause += ", Utility.getContext(this, vars, \""
                                + strParam + "\", windowId)";
                    }
                }
                grandfatherTabIndex = parentTabId(allTabs,
                        allTabs[parentTabIndex].tabid);
                auxFieldsData = FieldsData
                        .parentsColumnName(
                                pool,
                                (grandfatherTabIndex != -1 ? allTabs[grandfatherTabIndex].tabid
                                        : ""), allTabs[parentTabIndex].tabid);
                if (grandfatherTabIndex != -1
                        && (auxFieldsData == null || auxFieldsData.length == 0)) {
                    auxFieldsData = FieldsData.parentsColumnReal(pool,
                            allTabs[grandfatherTabIndex].tabid,
                            allTabs[parentTabIndex].tabid);
                }
            }
            ...
            if (!tabsData.adProcessId.equals("")) {
                ...
                strDirectPrint = TabsData.directPrint(pool,
                        tabsData.adProcessId);
            }
            WADGrid gridControl = null;
            {
                ...
                gridProps.setProperty("deleteable", (!tabsData.isreadonly
                        .equals("Y") ? "true" : "false"));
                ...
                gridProps.setProperty("inpKeyName", "inp"
            ...
            if (tabsData.issorttab.equals("Y")) {
                 ...
                 *************************************************/
                processTabXSQLSortTab(parentsFieldsData, fileDir,
                        tabsData.tabid, tabName, tableName, windowName,
                        keyColumnName, tabsData.adColumnsortorderId,
                        tabsData.adColumnsortyesnoId, vecParameters,
                        vecTableParameters, tabsData.javapackage);
                /************************************************
                 * JAVA of the SORT TAB
                 *************************************************/
                processTabJavaSortTab(parentsFieldsData, fileDir,
                        tabsData.tabid, tabName, tableName, windowName,
                        keyColumnName, strTables.toString(), strOrder
                                .toString(), strWhere.toString(), vecFields,
                        isSOTrx, allTabs, tabsData.key, tabsData.accesslevel,
                        selCol, isSecondaryKey, grandfatherField,
                        tabsData.tablevel, tabsData.tableId,
                        tabsData.windowtype, tabsData.adColumnsortorderId,
                        whereClauseParams, parentwhereclause, strProcess,
                        strDirectPrint, tabsData.isreadonly.equals("Y"),
                        vecParameters, vecTableParameters, tabsData.javapackage);
                /************************************************
                 * XML of the SORT TAB
                 *************************************************/
                processTabXmlSortTab(parentsFieldsData, fileDir,
                        tabsData.tabid, tabName, keyColumnName);
                /************************************************
                 * HTML of the SORT TAB
                 *************************************************/
                processTabHtmlSortTab(parentsFieldsData, fileDir,
                        tabsData.tabid, tabName, tabsData.realwindowname,
                        keyColumnName, tabNamePresentation, allTabs,
                        strProcess, strDirectPrint, strParentNameDescription,
                        windowName, "");
            } else {
                /************************************************
                 * JAVA
                 *************************************************/
                processTabJava(efd, efdauxiliar, parentsFieldsData, fileDir,
                        tabsData.tabid, tabName, tableName, windowName,
                        keyColumnName, strTables.toString(), strOrder
                                .toString(), strWhere.toString(),
                        tabsData.filterclause, vecFields, vecParameters,
                        isSOTrx, allTabs, tabsData.key, tabsData.accesslevel,
                        selCol, isSecondaryKey, grandfatherField,
                        tabsData.tablevel, tabsData.tableId,
                        tabsData.windowtype, tabsData.isreadonly.equals("Y"),
                        whereClauseParams, parentwhereclause,
                        tabsData.editreference, strProcess, strDirectPrint,
                 ...
                 * XSQL
                 *************************************************/
                processTabXSQL(parentsFieldsData, fileDir, tabsData.tabid,
                        tabName, tableName, windowName, keyColumnName,
                        strFields.toString(), strTables.toString(), strOrder
                                .toString(), strWhere.toString(),
                        vecParameters, tabsData.filterclause, selCol,
                        tabsData.tablevel, tabsData.windowtype,
                        vecTableParameters, fieldsData, isSecondaryKey,
                        tabsData.javapackage);

                /************************************************
                 * JAVA of the combo reloads
                 *************************************************/
                processTabComboReloads(fileFinReloads, tabsData.tabid,
                        parentsFieldsData, vecFields, isSOTrx,
                        tabsData.accesslevel);

                /************************************************
                 * XML in Relation view
                 *************************************************/
                processTabXmlRelation(parentsFieldsData, fileDir,
                        tabsData.tabid, tabName, keyColumnName, gridControl);

                /************************************************
                 * HTML in Relation view
                 *************************************************/
                processTabHtmlRelation(parentsFieldsData, fileDir,
                        tabsData.tabid, tabName, keyColumnName,
                        tabsData.isreadonly.equals("Y"),
                        strParentNameDescription, gridControl, false, "",
                        tabNamePresentation, tabsData.tableId,
                        tabsData.accesslevel);

                /************************************************
                 * XML in Edition view
                 *************************************************/
                processTabXmlEdition(fileDir, tabsData.tabid, tabName,
                        tabsData.key, tabsData.isreadonly.equals("Y"), efd,
                        efdauxiliar, isSecondaryKey);

                /************************************************
                 * HTML in Edition view
                 *************************************************/
                processTabHtmlEdition(efd, efdauxiliar, fileDir,
                        tabsData.tabid, tabName, keyColumnName,
                        tabNamePresentation, tabsData.key, parentsFieldsData,
                        vecFields, tabsData.isreadonly.equals("Y"), isSOTrx,
                        tabsData.tableId, PIXEL_TO_LENGTH, "", true,
                        isSecondaryKey);
                processTabHtmlEdition(efd, efdauxiliar, fileDir,
                        tabsData.tabid, tabName, keyColumnName,
                        tabNamePresentation, tabsData.key, parentsFieldsData,
                        vecFields, tabsData.isreadonly.equals("Y"), isSOTrx,
                        tabsData.tableId, PIXEL_TO_LENGTH, "", false,
                        isSecondaryKey);
            }

        } catch (final ServletException e) {
            e.printStackTrace();
            log4j.error("Problem of ServletException in the file: "
                    + tabsData.tabid);
        } catch (final IOException e) {
            e.printStackTrace();
     ...
     */
    private EditionFieldsData[] processSelCol(EditionFieldsData[] selCol,
            String tableName) {
        ...
        if (selCol != null) {
            for (int i = 0; i < selCol.length; i++) {
                selCol[i].htmltext = "strParam" + selCol[i].columnname
                        + ".equals(\"\")";
                selCol[i].columnnameinp = FormatUtilities
                        .replace(selCol[i].columnname);
                ...
                if (WadUtility.isGeneralNumber(selCol[i].reference)
                    ...
                    aux.htmltext = "strParam" + aux.columnname
                            + ".equals(\"\")";
                    ...
                    if (WadUtility.isTimeField(selCol[i].reference)) {
                        selCol[i].xmltext += "TO_CHAR(" + tableName + "."
                                + selCol[i].realcolumnname
                                + ", 'HH24:MI:SS') >= ";
                        selCol[i].xsqltext = "TO_CHAR(" + tableName + "."
                                + selCol[i].realcolumnname
                                + ", 'HH24:MI:SS') >= ";
                    } else {
                        selCol[i].xmltext += "(" + tableName + "."
                                + selCol[i].realcolumnname + ") >= ";
                        selCol[i].xsqltext = tableName + "."
                                + selCol[i].realcolumnname + " >= ";
                    }
                    ...
                    selCol[i].xsqltext += "(?"
                            + (WadUtility.isTimeField(selCol[i].reference) ? ", 'HH24:MI:SS'"
                                    : "") + ") ";
                    aux.columnnameinp = FormatUtilities
                            .replace(selCol[i].columnname)
                            + "_f";
                    aux.xmltext = " + ((strParam" + aux.columnname
                            + ".equals(\"\") || strParam" + aux.columnname
                            + ".equals(\"%\"))?\"\":\" AND";
                    if (WadUtility.isTimeField(selCol[i].reference)) {
                        aux.xmltext += "TO_CHAR(" + tableName + "."
                                + aux.realcolumnname + ", 'HH24:MI:SS') < ";
                        aux.xsqltext = "TO_CHAR(" + tableName + "."
                                + aux.realcolumnname + ", 'HH24:MI:SS') < ";
                    } else {
                        aux.xmltext += "(" + tableName + "."
                                + aux.realcolumnname + ") < ";
                    ...
                    if (WadUtility.isLikeType(selCol[i].reference)
                            && !WadUtility
                                    .isSearchValueColumn(selCol[i].realcolumnname)) {
    ...
    private void processTable(FieldsData[] parentsFieldsData, String strTab,
            Vector<Object> vecFields, Vector<Object> vecTables,
            Vector<Object> vecWhere, Vector<Object> vecOrder,
            Vector<Object> vecParameters, String tableName, String windowType,
            String tablevel, Vector<Object> vecTableParameters,
            FieldsData[] fieldsDataSelectAux) throws ServletException,
            IOException {
        int ilist = 0;
        final int itable = 0;
        ...
        for (int i = 0; i < fieldsData.length; i++) {
            if (!fieldsData[i].columnname.equalsIgnoreCase("Created")
                ...
                if (WadUtility.isTimeField(fieldsData[i].reference)) {
                    vecFields.addElement("TO_CHAR(" + tableName + "."
                            + fieldsData[i].name + ", 'HH24:MI:SS') AS "
                            + fieldsData[i].name);
                } else if (fieldsData[i].reference.equals("20")) {
                    vecFields.addElement("COALESCE(" + tableName + "."
                            + fieldsData[i].name + ", 'N') AS "
                            + fieldsData[i].name);
                } else
                ...
                if (fieldsData[i].reference.equals("19") && // TableDir
                    ...
                    WadUtility.columnIdentifier(pool, tableName,
                            fieldsData[i].required.equals("Y"), fieldsData[i],
                            vecCounters, false, vecSubFields, vecTables,
                    ...
                    for (final Enumeration<Object> e = vecSubFields.elements(); e
                            .hasMoreElements();) {
                        ...
                        strFields.append("COALESCE(TO_CHAR(")
                                .append(tableField).append("),'') ");
                    }
                    strOrder = strFields.toString() + ")";
                    vecFields.addElement("(CASE WHEN " + tableName + "."
                            + fieldsData[i].name + " IS NULL THEN '' ELSE "
                            + strFields.toString() + ") END) AS "
                            + fieldsData[i].name + "R");
                } else if (fieldsData[i].reference.equals("17")
                        && fieldsData[i].isdisplayed.equals("Y")) { // List
                    final Vector<Object> vecSubFields = new Vector<Object>();
                    WadUtility.columnIdentifier(pool, tableName,
                            fieldsData[i].required.equals("Y"), fieldsData[i],
                            vecCounters, false, vecSubFields, vecTables,
                    ...
                    vecFields.addElement("(CASE WHEN " + tableName + "."
                            ...
                            + strFields.toString() + ") END) AS "
                            + fieldsData[i].name + "R");
                } else if (fieldsData[i].reference.equals("18")
                        && fieldsData[i].isdisplayed.equals("Y")) { // Table
                    final Vector<Object> vecSubFields = new Vector<Object>();
                    WadUtility.columnIdentifier(pool, tableName,
                            fieldsData[i].required.equals("Y"), fieldsData[i],
                            vecCounters, false, vecSubFields, vecTables,
                    ...
                    vecFields.addElement("(CASE WHEN " + tableName + "."
                            ...
                            + strFields.toString() + ") END) AS "
                            + fieldsData[i].name + "R");
                } else if (fieldsData[i].reference.equals("32")
                        && fieldsData[i].isdisplayed.equals("Y")) {
                    final Vector<Object> vecSubFields = new Vector<Object>();
                    WadUtility.columnIdentifier(pool, tableName,
                            fieldsData[i].required.equals("Y"), fieldsData[i],
                    ...
                    strOrder = strFields.toString() + ")";
                    vecFields.addElement("(CASE WHEN " + tableName + "."
                            + fieldsData[i].name + " IS NULL THEN '"
                            + IMAGE_DEFAULT + "' ELSE " + strFields.toString()
                            + ") END) AS " + fieldsData[i].name + "R");
                } else if ((fieldsData[i].reference.equals("30")
                        || fieldsData[i].reference.equals("31")
                        || fieldsData[i].reference.equals("35")
                        || fieldsData[i].reference.equals("25") || fieldsData[i].reference
                        .equals("800011"))
                    ...
                    vecFields.addElement("(CASE WHEN " + tableName + "."
                            ...
                            + strFields.toString() + ") END) AS "
                            + fieldsData[i].name + "R");
                } else if (fieldsData[i].reference.equals("21")
                        && fieldsData[i].isdisplayed.equals("Y")) { // Location
                    final StringBuffer strFields = new StringBuffer();
                    strFields
                            .append(" (CASE WHEN "
                                    + tableName
                                    + "."
                                    + fieldsData[i].columnname
                                    + " IS NULL THEN '' ELSE TO_CHAR(C_Location_Identifier("
                                    + tableName + "."
                ...
                } else if (fieldsData[i].reference.equals("28")
                        && fieldsData[i].isdisplayed.equals("Y")
                        && !fieldsData[i].referencevalue.equals("")
                        && !fieldsData[i].name
                                .equalsIgnoreCase("ChangeProjectStatus")) { // Button
                    ilist = Integer
                            .valueOf(vecCounters.elementAt(1).toString())
                            .intValue();
                    ilist++;
                    vecFields.addElement("list" + ilist + ".name as "
                            + fieldsData[i].name + "_BTN");
                    strOrder = "list" + ilist + ".name";
                    final StringBuffer strWhere = new StringBuffer();
                    if (fieldsData[i].name.equalsIgnoreCase("DocAction")) {
                        strWhere.append(" AND (CASE " + tableName + "."
                                + fieldsData[i].name
                                + " WHEN '--' THEN 'CL' ELSE TO_CHAR("
                                + tableName + "." + fieldsData[i].name
                                + ") END) = " + "list" + ilist + ".value");
                    } else {
                        strWhere.append(" AND " + tableName + "."
                                + fieldsData[i].name + " = TO_CHAR(list"
                                + ilist + ".value)");
                    }
                    vecTables.addElement("left join ad_ref_list_v list" + ilist
                            + " on (" + "list" + ilist + ".ad_reference_id = '"
                            + fieldsData[i].referencevalue + "' and list"
                            + ilist + ".ad_language = ? " + strWhere.toString()
                            + ")");
                    vecTableParameters
                            .addElement("<Parameter name=\"paramLanguage\"/>");
                    vecCounters.set(1, Integer.toString(ilist));
                } else {
                    strOrder = tableName + "." + fieldsData[i].name;
                }
                if (!fieldsData[i].reference.equals("23")
                        && !fieldsData[i].reference.equals("14")
                        && !fieldsData[i].reference.equals("34")
                        && !fieldsData[i].reference.equals("13")
                        && !fieldsData[i].reference.equals("26")
                        && !fieldsData[i].reference.equals("32")
                    ...
                    final String[] aux = {
                            ...
                            new String(
                                    strOrder
                                            + (fieldsData[i].name
                                                    .equalsIgnoreCase("DocumentNo") ? " DESC"
                                                    : "")) };
     ...
     * @throws IOException
     */
    private void processTabJavaSortTab(FieldsData[] parentsFieldsData,
            File fileDir, String strTab, String tabName, String tableName,
            String windowName, String keyColumnName, String strTables,
            String strOrder, String strWhere, Vector<Object> vecFields,
            String isSOTrx, TabsData[] allTabs, String strWindow,
            String accesslevel, EditionFieldsData[] selCol,
            boolean isSecondaryKey, String grandfatherField, String tablevel,
            String tableId, String windowType, String strColumnSortOrderId,
            String whereClauseParams, String parentwhereclause,
            String strProcess, String strDirectPrint, boolean strReadOnly,
            Vector<Object> vecParametersTop,
            Vector<Object> vecTableParametersTop, String javaPackage)
        ...
        xmlDocument = xmlEngine.readXmlTemplate(
                "org/openbravo/wad/javasourceSortTab", discard)
                .createXmlDocument();

        fileDir.mkdirs();
        xmlDocument.setParameter("class", tabName);
        xmlDocument
                .setParameter("package", (!javaPackage.equals("") ? javaPackage
                        + "." : "")
                        + windowName);
        ...
        xmlDocument.setParameter("keyData", Sqlc
                .TransformaNombreColumna(keyColumnName));
        ...
        if (WadUtility.findField(vecFields, "adOrgId"))
            ...
            xmlDocument.setParameter("orgId",
                    "Utility.getContext(this, vars, \"#AD_Org_ID\", windowId)");
        ...
        if (parentsFieldsData != null && parentsFieldsData.length > 0) {
            ...
            vecCounters2.addElement("0");
            WadUtility.columnIdentifier(pool, parentsFieldsData[0].tablename,
                    true, parentsFieldsData[0], vecCounters2, true, vecFields2,
                    vecTable2, vecWhere2, vecParameters2, vecTableParameters2,
                    sqlDateFormat);

            xmlDocument
                    .setParameter("parentLanguage",
                            (vecParameters2.size() > 0 || vecTableParameters2
        ...
        if (parentTab != -1) {
            xmlDocument.setParameter("parentClass", FormatUtilities
                    .replace(allTabs[parentTab].tabname));
            fieldsParentSession = FieldsData.selectSession(pool,
                    allTabs[parentTab].tabid);
            for (int i = 0; i < fieldsParentSession.length; i++) {
                fieldsParentSession[i].name = Sqlc
                        .TransformaNombreColumna(fieldsParentSession[i].name);
            ...
            auxiliarPFields = FieldsData.selectAuxiliar(pool, "",
                    allTabs[parentTab].tabid);
            if (auxiliarPFields != null) {
                for (int i = 0; i < auxiliarPFields.length; i++) {
                    ...
                    if (auxiliarPFields[i].defaultvalue.toUpperCase()
                            .startsWith("@SQL=")) {
                        auxiliarPFields[i].defaultvalue = FormatUtilities
                                .replace(allTabs[parentTab].tabname)
                                + "Data.selectAux"
                                + auxiliarPFields[i].reference
                                + "(this"
                                + WadUtility.getWadContext(
                                        auxiliarPFields[i].defaultvalue,
                                        vecFields, vecAuxiliarPFields,
                                        parentsFieldsData, false, isSOTrx,
                                        strWindow) + ")";
                    } else if (auxiliarPFields[i].defaultvalue.indexOf("@") != -1) {
                        auxiliarPFields[i].defaultvalue = WadUtility
                                .getTextWadContext(
                                        auxiliarPFields[i].defaultvalue,
                                        vecFields, vecAuxiliarPFields,
                                        parentsFieldsData, false, isSOTrx,
     ...
     * @throws IOException
     */
    private void processTabJava(EditionFieldsData[] allfields,
            EditionFieldsData[] auxiliarsData, FieldsData[] parentsFieldsData,
            File fileDir, String strTab, String tabName, String tableName,
            String windowName, String keyColumnName, String strTables,
            String strOrder, String strWhere, String strFilter,
            Vector<Object> vecFields, Vector<Object> vecParametersTop,
            String isSOTrx, TabsData[] allTabs, String strWindow,
            String accesslevel, EditionFieldsData[] selCol,
            boolean isSecondaryKey, String grandfatherField, String tablevel,
            String tableId, String windowType, boolean strReadOnly,
            String whereClauseParams, String parentwhereclause,
            String editReference, String strProcess, String strDirectPrint,
            Vector<Object> vecTableParametersTop,
            FieldsData[] fieldsDataSelectAux, WADControl relationControl,
            String javaPackage) throws ServletException, IOException {
        log4j.debug("Processing java: " + strTab + ", " + tabName);
        XmlDocument xmlDocument;
        final boolean isHighVolumen = (FieldsData.isHighVolume(pool, strTab)
                .equals("Y"));
        boolean hasParentsFields = true;
        final String createFromProcess = FieldsData.hasCreateFromButton(pool,
                strTab);
        ...
        final String strhasEncryption = FieldsData.hasEncryptionFields(pool,
                strTab);
        final boolean hasEncryption = (strhasEncryption != null && !strhasEncryption
                .equals("0"));
        final int parentTab = parentTabId(allTabs, strTab);
        final String hasTree = TableLinkData.hasTree(pool, strTab);
        final boolean noPInstance = (ActionButtonRelationData.select(pool,
                strTab).length == 0);
        final boolean noActionButton = FieldsData.hasActionButton(pool, strTab)
                .equals("0");
        ...
        final FieldsData[] auxiliarFields = FieldsData.selectAuxiliar(pool, "",
                strTab);
        if (auxiliarFields != null) {
            for (int i = 0; i < auxiliarFields.length; i++) {
                auxiliarFields[i].columnname = Sqlc
                        .TransformaNombreColumna(auxiliarFields[i].columnname);
                if (auxiliarFields[i].defaultvalue.toUpperCase().startsWith(
                        "@SQL=")) {
                    auxiliarFields[i].defaultvalue = tabName
                            ...
                            + "(this"
                            + WadUtility.getWadContext(
                                    auxiliarFields[i].defaultvalue, vecFields,
                                    vecAuxiliarFields, parentsFieldsData,
                                    false, isSOTrx, strWindow) + ")";
                } else if (auxiliarFields[i].defaultvalue.indexOf("@") != -1) {
                    auxiliarFields[i].defaultvalue = WadUtility
                            .getTextWadContext(auxiliarFields[i].defaultvalue,
                                    vecFields, vecAuxiliarFields,
                                    parentsFieldsData, false, isSOTrx,
                                    strWindow);
                } else {
                    auxiliarFields[i].defaultvalue = "\""
                            + auxiliarFields[i].defaultvalue + "\"";
                }
...

        {
            ...
            final EditionFieldsData[] efd = EditionFieldsData
                    .selectDisplayLogic(pool, strTab);
            if (efd != null) {
                for (int i = 0; i < efd.length; i++)
                    WadUtility.displayLogic(efd[i].displaylogic, vecDL,
                            parentsFieldsData, vecAuxiliarFields, vecFields,
                            strWindow, vecContext);
            }
            for (int i = 0; i < vecContext.size(); i++) {
                dl.append("var str");
                dl.append(FormatUtilities.replace(vecContext.elementAt(i)
                        .toString()));

                dl.append("=\\\"\" +");
                if (vecContext.elementAt(i).toString().equals("ShowAudit"))
                    dl.append("(isNew?\"N\":");
                dl.append("Utility.getContext(this, vars, \"").append(
                        vecContext.elementAt(i).toString()).append(
                        "\", windowId)");
                if (vecContext.elementAt(i).toString().equals("ShowAudit"))
                    dl.append(")");
...

        {
            ...
            final Vector<Object> vecDL = new Vector<Object>();
            final EditionFieldsData[] efd = EditionFieldsData
                    .selectReadOnlyLogic(pool, strTab);
            if (efd != null) {
                for (int i = 0; i < efd.length; i++)
                    WadUtility.displayLogic(efd[i].readonlylogic, vecDL,
                            parentsFieldsData, vecAuxiliarFields, vecFields,
                            strWindow, vecContext);
            }
            for (int i = 0; i < vecContext.size(); i++) {
                readOnlyLogic.append("var str");
                readOnlyLogic.append(FormatUtilities.replace(vecContext
                        .elementAt(i).toString()));
                readOnlyLogic
                        .append("=\\\"\" + Utility.getContext(this, vars, \"");
        ...
        if (!(parentsFieldsData == null || parentsFieldsData.length == 0)
            ...
            discard[28] = "";
        if (isSecondaryKey
                && !EditionFieldsData.isOrgKey(pool, strTab).equals("0")
                && !strTab.equals("170"))
            discard[29] = "";
        ...
        xmlDocument.setParameter("windowName", windowName);
        xmlDocument.setParameter("key", keyColumnName);
        xmlDocument.setParameter("from", generateStaticWhere(strTables,
                vecTableParametersTop));
        xmlDocument.setParameter("order", (!strOrder.equals("") ? strOrder
                .substring(9) : strOrder));
        final Vector<Object> vecTotalParameters = new Vector<Object>();
        ...
        xmlDocument.setParameter("where", generateStaticWhere(strWhere,
                vecParametersTop));
        ...
        xmlDocument.setParameter("grandfatherName", grandfatherField);
        xmlDocument.setParameter("defaultView", (FieldsData.isSingleRow(pool,
                strTab).equals("Y") ? "EDIT" : "RELATION"));
        xmlDocument.setParameter("whereClauseParams", whereClauseParams);
        xmlDocument.setParameter("parentwhereclause", parentwhereclause);
        xmlDocument.setParameter("reportPDF", strProcess);
        xmlDocument.setParameter("reportDirectPrint", strDirectPrint);
        ...
        if (parentsFieldsData.length > 0) {
            ...
            xmlDocument.setParameter("parentTabName",
                    parentsFieldsData[0].parentTabName);
        }
        xmlDocument.setParameter("keyData", Sqlc
                .TransformaNombreColumna(keyColumnName));
        ...
        xmlDocument.setParameter("createFromProcessId", ((Integer.valueOf(
                createFromProcess).intValue() > 0) ? createFromProcess : ""));
        xmlDocument.setParameter("postedProcessId", ((Integer.valueOf(
                postedProcess).intValue() > 0) ? postedProcess : ""));
        xmlDocument.setParameter("editReference", TabsData.formClassName(pool,
                editReference));
        xmlDocument.setParameter("hasTree", hasTree);
        xmlDocument.setParameter("isReadOnly", (strReadOnly ? "Y" : "N"));
        ...
        if (selCol != null) {
            for (int i = 0; i < selCol.length; i++) {
                ...
                strParamHighVolume += "String strParam" + selCol[i].columnname
                        + " = vars.getSessionValue(tabId + \"|param"
                        + selCol[i].columnname + "\");\n";
        ...
        xmlDocument.setParameter("searchComparations", strHighVolumeComp);
        {
            ...
            fieldsRelationStructure.append(Sqlc
                    .TransformaNombreColumna(keyColumnName));
            fieldsRelationStructure
                    .append(", \"\\r\", \"\"), \"\\n\", \"<br>\"), \"\\\"\", \"\\\\\\\"\")).append(\"\\\"\");\n");
                ...
                if (fieldsDataSelect[i].showinrelation.equals("Y")
                    ...
                    if (parentsFieldsData.length == 0
                            || !parentsFieldsData[0].name
                                    .equals(fieldsDataSelect[i].name)) {
                        fieldsRelationStructure
                                .append("strData.append(\",\");\n");
                        fieldsRelationStructure
                                .append("strData.append(\"\\\"\").append(Replace.replace(Replace.replace(Replace.replace(data[contadorData].");
                        if ((fieldsDataSelect[i].reference.equals("17")
                            ...
                            fieldsDataSelect[i].name = fieldsDataSelect[i].name
                                    + "R"; // _REF
                        } else if ((fieldsDataSelect[i].reference.equals("30")
                                || fieldsDataSelect[i].reference
                                        .equals("800011")
                        ...
                        fieldsRelationStructure
                                .append(Sqlc
                                        .TransformaNombreColumna(fieldsDataSelect[i].name));
                        fieldsRelationStructure
                                .append(", \"\\r\", \"\"), \"\\n\", \"<br>\"), \"\\\"\", \"\\\\\\\"\")).append(\"\\\"\");\n");
                    }
                }
            }
            xmlDocument.setParameter("fieldsRelation", fieldsRelationStructure
                    .toString());
        }
        if (WadUtility.findField(vecFields, "adClientId"))
            xmlDocument.setParameter("clientId", "data.adClientId");
        ...
        boolean defaultValue;
        {
            ...
            for (int i = 0; i < fieldsData1.length; i++) {
                if (!fieldsData1[i].name.equalsIgnoreCase("Created")
                    ...
                    fieldsData1[i].name = Sqlc
                            .TransformaNombreColumna(fieldsData1[i].name);
                    fieldsData1[i].columnname = fieldsData1[i].name;
                    defaultValue = false;
                    if (!fieldsData1[i].reference.equals("23")
                            && !WadUtility
                                    .isDateTimeField(fieldsData1[i].reference)
                            && !fieldsData1[i].reference.equals("20")
                            && !WadUtility
                                    .isDecimalNumber(fieldsData1[i].reference)
                            && !WadUtility
                                    .isQtyNumber(fieldsData1[i].reference)
                            && !WadUtility
                                    .isPriceNumber(fieldsData1[i].reference)
                            && !WadUtility
                                    .isIntegerNumber(fieldsData1[i].reference)
                            && !WadUtility
                                    .isGeneralNumber(fieldsData1[i].reference)
                            && WadUtility.columnRelationFormat(fieldsData1[i],
                                    false, MAX_COL_SIZE_RELATION))
                    ...
                    if (fieldsData1[i].iskey.equals("Y")) {
                        fieldsData1[i].xmltext = ", windowId + \"|"
                        ...
                        if (fieldsData1[i].required.equals("Y")
                            ...
                            && !fieldsData1[i].columnname
                                    .equalsIgnoreCase("Value") && !defaultValue) {
        ...
        final FieldsData[] fieldsSession = FieldsData.selectSession(pool,
                strTab);
        if (fieldsSession != null) {
            for (int i = 0; i < fieldsSession.length; i++) {
                fieldsSession[i].name = Sqlc
                        .TransformaNombreColumna(fieldsSession[i].name);
        ...
        if (fieldsSession != null) {
            for (int i = 0; i < fieldsSession.length; i++) {
                if (!fieldsSession[i].columnname.equals("")) {
                    fieldsSession[i].referencevalue += "_"
                            + fieldsSession[i].columnname;
                    ...
                    fieldsSession[i].whereclause += WadUtility.getWadContext(
                            fieldsSession[i].defaultvalue, vecFields,
                            vecAuxiliarFields, parentsFieldsData, false,
                            isSOTrx, strWindow);
        ...
        {
            final FieldsData[] docsNoFields = FieldsData.selectDocumentsNo(
                    pool, strTab);
            if (docsNoFields != null) {
                String field = "";
                for (int i = 0; i < docsNoFields.length; i++) {
                    docsNoFields[i].columnname = Sqlc
                            .TransformaNombreColumna(docsNoFields[i].columnname);
                    ...
                    docsNoFields[i].realname = "Utility.getDocumentNo(con, this, vars, windowId, \""
                            + docsNoFields[i].nameref + "\", ";
                    field = WadUtility.findField(pool, allfields,
                            auxiliarsData, "C_DocTypeTarget_ID");
                    if (!field.equals("")) {
                        docsNoFields[i].defaultvalue += "data[0]."
                                + Sqlc.TransformaNombreColumna(field);
                        docsNoFields[i].realname += "data."
                                + Sqlc.TransformaNombreColumna(field);
                    } else {
            ...
            for (int i = 0; i < docsNoFields.length; i++) {
                ...
                docNoNoConnFileds[i].defaultvalue = docsNoFields[i].defaultvalue
                        .replace("(con,", "(");
            }

            xmlDocument.setData("structure12", docNoNoConnFileds);
            {
                final FieldsData[] docsIdentify = FieldsData.selectIdentify(
                        pool, strTab);
                        ...
                        if (docsNoFields == null || docsNoFields.length == 0) {
                            ...
                            if (docsIdentify[i].issessionattr.equals("Y"))
                                docsIdentify[i].realname += ";\nvars.setSessionValue(windowId + \"|"
                                        + docsIdentify[i].columnname
                                        + "\", data."
                                        + Sqlc
                                                .TransformaNombreColumna(docsIdentify[i].columnname)
                                        + ")";
                        } else {
                            docsIdentify[i].realname = "data."
                                    + docsNoFields[0].columnname;
                        }
                        docsIdentify[i].columnname = Sqlc
                                .TransformaNombreColumna(docsIdentify[i].columnname);
                    }
                }
        ...
        xmlDocument.setData("structure11", auxiliarPFields);
        {
            final ActionButtonRelationData[] abrd = WadActionButton
                    .buildActionButtonCall(pool, strTab, tabName,
                            keyColumnName, isSOTrx, strWindow);
        ...
        // Encrypted Fields
        {
            final FieldsData[] encryptedData = FieldsData.selectEncrypted(pool,
                    strTab);
            if (encryptedData != null && encryptedData.length > 0) {
                for (int g = 0; g < encryptedData.length; g++) {
                    encryptedData[g].realname = Sqlc
                            .TransformaNombreColumna(encryptedData[g].realname);
                    encryptedData[g].name = FormatUtilities
                            .replace(encryptedData[g].name);
        ...
        // Button Fields
        {
            final FieldsData[] buttonData = FieldsData.selectButton(pool,
                    strTab);
            if (buttonData != null && buttonData.length > 0) {
                for (int g = 0; g < buttonData.length; g++) {
                    buttonData[g].realname = Sqlc
                            .TransformaNombreColumna(buttonData[g].realname);
                }
            }
        ...
        final FieldsData sfd[] = FieldsData
                .selectDefaultValue(pool, "", strTab);
        int isSelect = 0;
        for (int i = 0; i < sfd.length; i++) {
            if (!hasParentsFields
                    || !parentsFieldsData[0].name
                            .equalsIgnoreCase(sfd[i].columnname)) {
                if (sfd[i].defaultvalue.toUpperCase().startsWith("@SQL=")) {
                    sfd[i].defaultvalue = tabName
                            ...
                            + "(this"
                            + WadUtility
                                    .getWadContext(sfd[i].defaultvalue,
                                            vecFields, vecAuxiliarFields,
                                            parentsFieldsData, true, isSOTrx,
                                            strWindow) + ")";
                    ...
                    sfd[i].defaultvalue = "Utility.getDefault(this, vars, \""
                            + sfd[i].columnname + "\", \""
                            + WadUtility.toJavaString(sfd[i].defaultvalue)
                            + "\", \"" + strWindow + "\", \""
                            + WadUtility.getWadDefaultValue(sfd[i])
                            + "\", dataField)";
                } else {
                    sfd[i].defaultvalue = "\"\"";
                }
            ...
            if ((sfd[i].referencevalue.equals("30")
                    || sfd[i].referencevalue.equals("31")
                    || sfd[i].referencevalue.equals("35")
                    || sfd[i].referencevalue.equals("25") || sfd[i].reference
                    .equals("800011"))
                    && sfd[i].isdisplayed.equals("Y"))
                strDefaultValues.append(", " + tabName + "Data.selectDef"
                        + sfd[i].reference + "_" + (isSelect++) + "(this, "
                        + sfd[i].defaultvalue + ")");
            else if (sfd[i].referencevalue.equals("28")
                    && sfd[i].isdisplayed.equals("Y")
                    && !sfd[i].type.equals(""))
                strDefaultValues
                        .append(
                                ", (vars.getLanguage().equals(\"en_US\")?ListData.selectName(this, \"")
                        .append(sfd[i].type)
                        .append("\", ")
                        .append(sfd[i].defaultvalue)
                        .append(
                                "):ListData.selectNameTrl(this, vars.getLanguage(), \"")
                        .append(sfd[i].type).append("\", ").append(
                                sfd[i].defaultvalue).append("))");
            else if (sfd[i].referencevalue.equals("21")
                    && sfd[i].isdisplayed.equals("Y"))
                strDefaultValues
                        .append(", LocationSearchData.locationAddress(this, "
                                + sfd[i].defaultvalue + ")");
            else if (sfd[i].referencevalue.equals("32")
                    && sfd[i].isdisplayed.equals("Y"))
                strDefaultValues.append(", " + tabName + "Data.selectDef"
                        + sfd[i].reference + "_" + (isSelect++) + "(this, "
                        + sfd[i].defaultvalue + ")");
        ...
        for (int i = 0; i < allfields.length; i++) {
            ...
            try {
                auxControl = WadUtility.getControl(pool, allfields[i],
                        strReadOnly, tabName, "", xmlEngine, false, false,
                        false, hasParentsFields);
            ...
            if ((auxControl instanceof WADButton)
                    && (auxControl.getData("IsDisplayed").equals("Y"))) {
        ...
        xmlDocument.setParameter("controlsJavaCode", controlsJavaSource
                .toString());
    ...
    /**
     * Generates the where with the params as java vars, to put it in the java
     * file to be used in all the internal searchs, like gotoFirstRow...
     * 
     * @param strWhere
     ...
     */
    private String generateStaticWhere(String strWhere,
            Vector<Object> vecParameters) {
        ...
        while (pos != -1) {
            ...
            if (strParam.equalsIgnoreCase("paramLanguage"))
                ...
                result.append(" '\" + Utility.getContext(this, vars, \""
                        + strParam + "\", windowId) + \"' ");
     ...
     * @throws IOException
     */
    private void processTabXSQLSortTab(FieldsData[] parentsFieldsData,
            File fileDir, String strTab, String tabName, String tableName,
            String windowName, String keyColumnName,
            String strColumnSortOrderId, String strColumnSortYNId,
            Vector<Object> vecParametersTop,
            Vector<Object> vecTableParametersTop, String javaPackage)
            throws ServletException, IOException {
        ...
        if (parentsFieldsData == null || parentsFieldsData.length == 0)
            discard[0] = "parent"; // remove the parent tags
        xmlDocumentXsql = xmlEngine.readXmlTemplate(
                "org/openbravo/wad/datasourceSortTab", discard)
                .createXmlDocument();
        ...
        xmlDocumentXsql.setParameter("package", "org.openbravo.erpWindows."
                + windowName);
        xmlDocumentXsql.setParameter("package", "org.openbravo.erpWindows."
                + (!javaPackage.equals("") ? javaPackage + "." : "")
                + windowName);
        xmlDocumentXsql.setParameter("table", tableName);
        xmlDocumentXsql.setParameter("key", tableName + "." + keyColumnName);
        xmlDocumentXsql.setParameter("SortConditionField", FieldsData
                .columnName(pool, strColumnSortYNId));
        final String strSortField = FieldsData.columnName(pool,
                strColumnSortOrderId);
        xmlDocumentXsql.setParameter("SortField", strSortField);
        xmlDocumentXsql.setParameter("SortFieldInp", Sqlc
                .TransformaNombreColumna(strSortField));
        if (parentsFieldsData.length > 0) {
            xmlDocumentXsql.setParameter("keyParent", tableName + "."
                    + parentsFieldsData[0].name);
        }
        xmlDocumentXsql.setParameter("paramKey", Sqlc
                .TransformaNombreColumna(keyColumnName));
        if (parentsFieldsData.length > 0) {
        ...
        String strWhere = "";
        {
            ...
            final Vector<Object> vecWhere = new Vector<Object>();
            final FieldsData[] data = FieldsData.identifierColumns(pool,
                    tableName);
            log4j.debug("Total Identifiers for " + tableName + ": "
                    + data.length);
            if (data == null)
                strFields = "''";
            ...
            for (int i = 0; i < data.length; i++) {
                if (i != 0)
                    strFields += " || ' - ' || ";
                strFields += WadUtility.columnIdentifier(pool, tableName, true,
                        data[i], vecCounters, false, vecFields, vecTable,
                        vecWhere, vecParametersTop, vecTableParametersTop,
                        sqlDateFormat);
            }
            for (int i = 0; i < vecTable.size(); i++) {
                final String strAux = (String) vecTable.elementAt(i);
                strTables += (strAux.trim().toLowerCase().startsWith(
                        "left join") ? " " : ", ")
                        + strAux;
            }
        ...
        for (int i = 0; i < vecTableParametersTop.size(); i++) {
            strParameters.append(vecTableParametersTop.elementAt(i).toString())
                    .append("\n");
        }
        for (int i = 0; i < vecParametersTop.size(); i++) {
            strParameters.append(vecParametersTop.elementAt(i).toString())
                    .append("\n");
        }
        ...
        if (parentsFieldsData != null && parentsFieldsData.length > 0) {
            ...
            vecCounters.addElement("0");
            vecCounters.addElement("0");
            final String strText = WadUtility.columnIdentifier(pool,
                    parentsFieldsData[0].tablename, true, parentsFieldsData[0],
                    vecCounters, false, vecFields, vecTable, vecWhere,
            ...
            fieldsParent[0].defaultvalue = "SELECT (" + strText
                    + ") AS NAME FROM ";
            ...
            for (int s = 0; s < vecTable.size(); s++) {
                ...
                fieldsParent[0].defaultvalue += (strAux.trim().toLowerCase()
                        .startsWith("left join") ? " " : ", ")
                        + strAux;
            }
            fieldsParent[0].defaultvalue += " WHERE "
                    + parentsFieldsData[0].tablename + "."
                    + parentsFieldsData[0].name + " = ? ";
            for (int s = 0; s < vecWhere.size(); s++) {
                fieldsParent[0].defaultvalue += " AND "
                        + vecWhere.elementAt(s).toString();
            }
            fieldsParent[0].whereclause = "";
            for (int s = 0; s < vecTableParameters.size(); s++) {
                fieldsParent[0].whereclause += vecTableParameters.elementAt(s)
                        .toString()
                        + "\n";
            }
            ...
            for (int s = 0; s < vecParameters.size(); s++) {
                fieldsParent[0].whereclause += vecParameters.elementAt(s)
                        .toString()
                        + "\n";
            }
        ...
        if (parentsFieldsData != null && parentsFieldsData.length > 0) {
            ...
            final String strText = WadUtility.columnIdentifier(pool,
                    parentsFieldsData[0].tablename, true, parentsFieldsData[0],
                    vecCounters, true, vecFields, vecTable, vecWhere,
                    vecParameters, vecTableParameters, sqlDateFormat);
            final FieldsData[] fieldsParent = new FieldsData[1];
            fieldsParent[0] = new FieldsData();
            fieldsParent[0].defaultvalue = "SELECT (" + strText
                    + ") AS NAME FROM " + parentsFieldsData[0].tablename;
            for (int s = 0; s < vecTable.size(); s++) {
                final String strAux = (String) vecTable.elementAt(s);
                fieldsParent[0].defaultvalue += (strAux.trim().toLowerCase()
                        .startsWith("left join") ? " " : ", ")
     ...
     * @throws IOException
     */
    private void processTabXSQL(FieldsData[] parentsFieldsData, File fileDir,
            String strTab, String tabName, String tableName, String windowName,
            String keyColumnName, String strFields, String strTables,
            String strOrder, String strWhere, Vector<Object> vecParametersTop,
            String strFilter, EditionFieldsData[] selCol, String tablevel,
            String windowType, Vector<Object> vecTableParametersTop,
            FieldsData[] fieldsDataSelectAux, boolean isSecondaryKey,
            String javaPackage) throws ServletException, IOException {
        ...
        if ((!(isSecondaryKey && !EditionFieldsData.isOrgKey(pool, strTab)
                .equals("0")))
                || strTab.equals("170"))
            discard[7] = "hasOrgKey";
        else {
        ...
        xmlDocumentXsql = xmlEngine.readXmlTemplate(
                "org/openbravo/wad/datasource", discard).createXmlDocument();
        ...
        if (parentsFieldsData != null && parentsFieldsData.length > 0) {
            ...
            if (isSecondaryKey
                    && (!EditionFieldsData.isOrgKey(pool, strTab).equals("0"))) {
                xmlDocumentXsql.setParameter("paramKeyParentOrg",
                        "currentAdOrgId");
            }
        }
        ...
        // Auxiliar Fields
        {
            final FieldsData fieldsAux[] = FieldsData.selectAuxiliar(pool,
                    "@SQL=", strTab);
            for (int i = 0; i < fieldsAux.length; i++) {
                final Vector<Object> vecParametros = new Vector<Object>();
                fieldsAux[i].defaultvalue = WadUtility.getSQLWadContext(
                        fieldsAux[i].defaultvalue, vecParametros);
                final StringBuffer parametros = new StringBuffer();
                for (final Enumeration<Object> e = vecParametros.elements(); e
                        .hasMoreElements();) {
        ...
        // Default Fields
        {
            final FieldsData fieldsDef[] = FieldsData.selectDefaultValue(pool,
                    "", strTab);
            ...
            for (int i = 0; i < fieldsDef.length; i++) {
                ...
                if (fieldsDef[i].defaultvalue.startsWith("@SQL=")) {
                    fieldsDef[i].defaultvalue = WadUtility.getSQLWadContext(
                            fieldsDef[i].defaultvalue, vecParametros);
                    final StringBuffer parametros = new StringBuffer();
                    for (final Enumeration<Object> e = vecParametros.elements(); e
                            .hasMoreElements();) {
                        final String paramsElement = (String) e.nextElement();
                ...
                if ((fieldsDef[i].referencevalue.equals("30")
                        || fieldsDef[i].referencevalue.equals("31")
                        || fieldsDef[i].referencevalue.equals("35")
                        || fieldsDef[i].reference.equals("800011") || fieldsDef[i].referencevalue
                        .equals("25"))
                    ...
                    if (fieldsDef[i].referencevalue.equals("30"))
                        dataSearchs = EditionFieldsData.selectSearchs(pool, "",
                                fieldsDef[i].type);
                    if (dataSearchs == null || dataSearchs.length == 0) {
                        if (fieldsDef[i].referencevalue.equals("25"))
                            ...
                            tableN = "M_Product";
                        else if (fieldsDef[i].name
                                .equalsIgnoreCase("createdBy")
                                || fieldsDef[i].name
                                        .equalsIgnoreCase("updatedBy"))
                            tableN = "AD_User";
                        else
                            tableN = fieldsDef[i].name.substring(0,
                                    fieldsDef[i].name.length() - 3);
                        if (fieldsDef[i].referencevalue.equals("25"))
                    ...
                    itable2 = fieldsOfSearch2(tableN, fieldsDef[i].name,
                            fieldsDef[i].required, vecFields2, vecTables2,
                            vecWhere2, itable2, fieldsDef[i].referencevalue,
                            fieldsDef[i].type);
                    final StringBuffer strFields2 = new StringBuffer();
                    strFields2.append(" ( ");
                    boolean boolFirst = true;
                    for (final Enumeration<Object> e = vecFields2.elements(); e
                            .hasMoreElements();) {
                        final String tableField = (String) e.nextElement();
                        log4j.debug("  field: " + tableField);
                        if (boolFirst) {
                        ...
                        strFields2.append("COALESCE(TO_CHAR(").append(
                                tableField).append("), '') ");
                    }
                    ...
                    v.addElement(fd);
                } else if (fieldsDef[i].referencevalue.equals("32")
                        && fieldsDef[i].isdisplayed.equals("Y")) {
                    final FieldsData fd = new FieldsData();
                    fd.reference = fieldsDef[i].reference + "_" + (itable++);
                    fd.name = fieldsDef[i].columnname + "R";
                    ...
                    vecTables2.addElement(tableN + " table1");
                    final StringBuffer strFields2 = new StringBuffer();
                    strFields2.append(" ( table1.ImageURL ) AS ").append(
                            fieldsDef[i].columnname);
                    final StringBuffer fields = new StringBuffer();
                    fields.append("SELECT ").append(strFields2);
                    fields.append(" FROM ");
                    for (int j = 0; j < vecTables2.size(); j++) {
        ...
        {
            final ProcessRelationData[] data = ProcessRelationData.selectXSQL(
                    pool, strTab);
            if (data != null) {
                for (int i = 0; i < data.length; i++) {
                    String tableN = "";
                    if (data[i].adReferenceId.equals("28"))
                        ...
                        tableN = data[i].name.substring(0, data[i].searchname
                                .length() - 3);
                    String strName = "";
                    if (data[i].adReferenceId.equals("28"))
                    ...
                    final String strColumnName = FieldsData.columnIdentifier(
                            pool, tableN);
                    final StringBuffer fields = new StringBuffer();
        ...
        // processes
        {
            final ProcessRelationData fieldsAux[] = ProcessRelationData
                    .selectXSQLParams(pool, strTab);
        ...
        // Update
        {
            final FieldsData fieldsDataUpdate[] = FieldsData.selectUpdatables(
                    pool, strTab);
            for (int i = 0; i < fieldsDataUpdate.length; i++) { // *** i=1?
                fieldsDataUpdate[i].name = ((i > 0) ? ", " : "")
                        + fieldsDataUpdate[i].name;
                if (WadUtility.isTimeField(fieldsDataUpdate[i].reference))
                    ...
                    fieldsDataUpdate[i].xmlFormat = WadUtility.sqlCasting(pool,
                            fieldsDataUpdate[i].reference,
                            fieldsDataUpdate[i].referencevalue)
                            + "(?)";
            }
            xmlDocumentXsql.setData("structure3", fieldsDataUpdate);
        }
        {
            final FieldsData fieldsDataParameter[] = FieldsData
                    .selectUpdatables(pool, strTab);
            for (int i = 0; i < fieldsDataParameter.length; i++) {
                fieldsDataParameter[i].name = Sqlc
                        .TransformaNombreColumna(fieldsDataParameter[i].name);
            }
            ...
            for (int i = 0; i < fieldsDataValue.length; i++) {
                if (!fieldsDataValue[i].name.equalsIgnoreCase("Created")
                    ...
                    if (WadUtility.isTimeField(fieldsDataValue[i].reference))
                        ...
                        fieldsDataValue[i].name = ((i > 0) ? ", " : "")
                                + WadUtility.sqlCasting(pool,
                                        fieldsDataValue[i].reference,
        ...
        }
        {
            ...
            for (int i = 0; i < fieldsDataParameterInsert.length; i++) {
                if (!fieldsDataParameterInsert[i].name
                        .equalsIgnoreCase("Created")
                        && !fieldsDataParameterInsert[i].name
                                .equalsIgnoreCase("CreatedBy")
                        && !fieldsDataParameterInsert[i].name
                                .equalsIgnoreCase("Updated")
                        && !fieldsDataParameterInsert[i].name
        ...
        {
            final FieldsData fieldsDataDefaults[] = FieldsData
                    .selectDefaultValue(pool, "", strTab);
            final Vector<Object> vecDDef = new Vector<Object>();
            for (int i = 0; i < fieldsDataDefaults.length; i++) {
                boolean modified = false;
                if (parentsFieldsData == null
                        || parentsFieldsData.length == 0
                        || !parentsFieldsData[0].name
                                .equalsIgnoreCase(fieldsDataDefaults[i].columnname)) {
                    fieldsDataDefaults[i].name = Sqlc
                ...
                if ((fieldsDataDefaults[i].referencevalue.equals("30")
                    ...
                    f.name = (modified ? fieldsDataDefaults[i].name
                            : Sqlc
                                    .TransformaNombreColumna(fieldsDataDefaults[i].name))
                            + "r";
                    f.columnname = (modified ? fieldsDataDefaults[i].columnname
                            : Sqlc
                                    .TransformaNombreColumna(fieldsDataDefaults[i].columnname))
                            + "r";
        ...
        {
            final ActionButtonRelationData[] abrd = WadActionButton
                    .buildActionButtonSQL(pool, strTab);
    ...
    /**
     * Generates the combo reloads for the tab. Combo reloads are the callouts
     * to reloads the values of the subordinated combos in the tab.
     * 
     * @param fileDir
     *            Path where to generate the file.
     * @param strTab
     ...
     * @throws IOException
     */
    private void processTabComboReloads(File fileDir, String strTab,
            FieldsData[] parentsFieldsData, Vector<Object> vecFields,
            String isSOTrx, String accesslevel) throws ServletException,
            IOException {
        ...
        if (data == null || data.length == 0)
            return;
        final XmlDocument xmlDocument = xmlEngine.readXmlTemplate(
                "org/openbravo/wad/ComboReloads").createXmlDocument();
        xmlDocument.setParameter("tabId", strTab);
        ...
        for (int i = 0; i < data.length; i++) {
            final String code = data[i].whereclause
                    + ((!data[i].whereclause.equals("") && !data[i].referencevalue
                            .equals("")) ? " AND " : "")
                    + data[i].referencevalue;
            data[i].columnname = "inp"
                    + Sqlc.TransformaNombreColumna(data[i].columnname);
            data[i].whereclause = WadUtility.getComboReloadText(code,
                    vecFields, parentsFieldsData, vecReloads, "inp");
            if (data[i].whereclause.equals("") && data[i].type.equals("R"))
                data[i].whereclause = "\"inpadOrgId\"";
            if (data[i].reference.equals("17")
                    && data[i].whereclause.equals(""))
                data[i].whereclause = "\"inp" + data[i].columnname + "\"";
            if (!data[i].whereclause.equals("")
                    && data[i].isdisplayed.equals("Y")
                    && (data[i].reference.equals("17")
                            || data[i].reference.equals("18") || data[i].reference
                            .equals("19"))) {
                ...
                if (data[i].reference.equals("17")) { // List
                    ...
                    final FieldsData[] tables = FieldsData.selectColumnTable(
                            pool, strTab, data[i].id);
                    if (tables == null || tables.length == 0)
                        throw new ServletException(
                                "No se ha encontrado la Table para la columnId: "
                                        + data[i].id);
                    ...
                    final Vector<Object> vecParameters = new Vector<Object>();
                    final Vector<Object> vecTableParameters = new Vector<Object>();
                    WadUtility.columnIdentifier(pool, tables[0].tablename,
                            true, tables[0], 0, 0, true, vecFields1, vecTables,
                            vecWhere, vecParameters, vecTableParameters,
                            sqlDateFormat);
                    ...
                    data[i].xmltext += WadUtility.getWadComboReloadContext(
                            where.toString(), isSOTrx);
                    data[i].xmltexttrl = data[i].xmltext;
                    if (vecParameters.size() > 0
                            && vecTableParameters.size() == 0) {
                        data[i].xmltext += ", vars.getLanguage()";
                    ...
                    final FieldsData[] tableDir = FieldsData
                            .selectColumnTableDir(pool, strTab, data[i].id);
                    if (tableDir == null || tableDir.length == 0)
                        throw new ServletException(
                                "No se ha encontrado la TableDir para la columnId: "
                                        + data[i].id);
                    data[i].tablename = "TableDir";
                    data[i].htmltext = "select" + tableDir[0].referencevalue;
                    final String table_Name = tableDir[0].name.substring(0,
                            tableDir[0].name.length() - 3);
                    final Vector<Object> vecFields1 = new Vector<Object>();
                    final Vector<Object> vecTables = new Vector<Object>();
                    final Vector<Object> vecWhere = new Vector<Object>();
                    final Vector<Object> vecParameters = new Vector<Object>();
                    final Vector<Object> vecTableParameters = new Vector<Object>();
                    WadUtility.columnIdentifier(pool, table_Name, true,
                            tableDir[0], 0, 0, true, vecFields1, vecTables,
                            vecWhere, vecParameters, vecTableParameters,
                            sqlDateFormat);
                    data[i].xmltext = "";
                    if (vecTableParameters.size() > 0) {
                    ...
                    if (!tableDir[0].columnname.equals("")) {
                        ...
                        data[i].xmltext += WadUtility.getWadComboReloadContext(
                                tableDir[0].defaultvalue, isSOTrx);
                    } else {
                        data[i].tablename = "TableDir";
                    }
                    data[i].tablenametrl = data[i].tablename + "Trl";
     ...
     * @throws IOException
     */
    private void processTabXmlSortTab(FieldsData[] parentsFieldsData,
            File fileDir, String strTab, String tabName, String keyColumnName)
            throws ServletException, IOException {
        log4j.debug("Procesig relation sort tab xml: " + strTab + ", "
                + tabName);
        final String[] discard = { "" };
        if (parentsFieldsData == null || parentsFieldsData.length == 0)
            discard[0] = new String("sectionParent");
        final XmlDocument xmlDocumentRXml = xmlEngine.readXmlTemplate(
                "org/openbravo/wad/ConfigurationSortTab_Relation", discard)
                .createXmlDocument();
     ...
     * @param strDirectPrint
     *            Indicate if the process is a direct print or has a preview
     *            mode.
     ...
     * @throws IOException
     */
    private void processTabHtmlSortTab(FieldsData[] parentsFieldsData,
            File fileDir, String strTab, String tabName, String windowName,
            String keyColumnName, String tabNamePresentation,
            TabsData[] allTabs, String strProcess, String strDirectPrint,
            String strParentNameDescription, String WindowPathName,
            String strLanguage) throws ServletException, IOException {
        log4j.debug("Procesig relation sort tab html: " + strTab + ", "
                + tabName);
        ...
        xmlDocumentRHtml = xmlEngine.readXmlTemplate(
                "org/openbravo/wad/TemplateSortTab_Relation", discard)
                .createXmlDocument();
        xmlDocumentRHtml.setParameter("tab", tabNamePresentation);
        xmlDocumentRHtml.setParameter("form", tabName + "_Relation.html");
        xmlDocumentRHtml.setParameter("key", "inp"
                + Sqlc.TransformaNombreColumna(keyColumnName));
        xmlDocumentRHtml.setParameter("tabId", strTab);
        if (parentsFieldsData.length > 0) {
            xmlDocumentRHtml.setParameter("keyParent", "inp"
                    + Sqlc.TransformaNombreColumna(parentsFieldsData[0].name));
            xmlDocumentRHtml.setParameter("parentKeyNameDescription",
                    strParentNameDescription);
            xmlDocumentRHtml.setParameter("parentKeyName",
                    parentsFieldsData[0].name);
        }

        xmlDocumentRHtml.setParameter("subtabKey", tabName);
        WadUtility.writeFile(fileDir, tabName + "_Relation.html",
                xmlDocumentRHtml.print());
    }
     ...
     * @throws IOException
     */
    private void processTabXmlRelation(FieldsData[] parentsFieldsData,
            File fileDir, String strTab, String tabName, String keyColumnName,
            WADControl relationControl) throws ServletException, IOException {
        log4j.debug("Procesig relation xml: " + strTab + ", " + tabName);
        final String[] discard = { "" };
        if (parentsFieldsData == null || parentsFieldsData.length == 0)
            discard[0] = new String("sectionParent");
        final XmlDocument xmlDocumentRXml = xmlEngine.readXmlTemplate(
                "org/openbravo/wad/Configuration_Relation", discard)
                .createXmlDocument();
        xmlDocumentRXml.setParameter("class", tabName + "_Relation.html");
        xmlDocumentRXml.setParameter("key", keyColumnName);
        if (parentsFieldsData != null && parentsFieldsData.length > 0) {
        ...
        xmlDocumentRXml
                .setParameter("relationControl", relationControl.toXml());
        WadUtility.writeFile(fileDir, tabName + "_Relation.xml",
                "<?xml version=\"1.0\" encoding=\"UTF-8\"?>\n"
                        + xmlDocumentRXml.print());
    }
     ...
     * @throws IOException
     */
    private void processTabHtmlRelation(FieldsData[] parentsFieldsData,
            File fileDir, String strTab, String tabName, String keyColumnName,
            boolean isreadonly, String strParentNameDescription,
            WADControl control, boolean isTranslated, String adLanguage,
            String tabNamePresentation, String strTable, String accessLevel)
            throws ServletException, IOException {
        log4j.debug("Procesig relation html"
                + (isTranslated ? " translated" : "") + ": " + strTab + ", "
                + tabName);
        ...
        final XmlDocument xmlDocument = xmlEngine.readXmlTemplate(
                "org/openbravo/wad/Template_Relation", discard)
                .createXmlDocument();
        ...
        if (parentsFieldsData.length > 0) {
            xmlDocument.setParameter("keyParent", "inp"
                    + Sqlc.TransformaNombreColumna(parentsFieldsData[0].name));
            xmlDocument.setParameter("parentKeyNameDescription",
                    strParentNameDescription);
            xmlDocument
                    .setParameter("parentKeyName", parentsFieldsData[0].name);
        }
        xmlDocument.setParameter("importCSS", getVectorElementsNotRepeated(
                control.getCSSImport(), new Vector<String>(), 1));
        xmlDocument.setParameter("importJS", getVectorElementsNotRepeated(
                control.getImport(), new Vector<String>(), 2));
        final StringBuffer script = new StringBuffer();
        script.append(getVectorElementsNotRepeated(control.getJSCode(),
                new Vector<String>(), 0));
        ...
        WadUtility.writeFile(fileDir, tabName + "_Relation.html", xmlDocument
                .print());
    }

    /**
     * Manage the Vector of imports for the html files. Is in charged of keep
     * them with unique values.
     * 
     * @param data
     *            The main Vector.
     * @param addedElements
     ...
     */
    private String getVectorElementsNotRepeated(Vector<String[]> data,
            Vector<String> addedElements, int type) {
     ...
     * @throws IOException
     */
    private void processTabXmlEdition(File fileDir, String strTab,
            String tabName, String windowId, boolean isreadonly,
            FieldProvider[] efd, FieldProvider[] efdauxiliar,
        ...
        if (efdauxiliar != null) {
            for (int i = 0; i < efdauxiliar.length; i++) {
                final WADControl auxControl = new WADHidden(efdauxiliar[i]
                        .getField("columnname"), Sqlc
                        .TransformaNombreColumna(efdauxiliar[i]
                                .getField("columnname")), "", true);
        ...
        for (int i = 0; i < efd.length; i++) {
            WADControl auxControl = null;
            try {
                auxControl = WadUtility.getControl(pool, efd[i], isreadonly,
                        tabName, "", xmlEngine, false, false, false, false,
                        false);
            ...
            if (WadUtility.isNewGroup(auxControl, strFieldGroup)) {
                ...
                fieldLabel.setColumnName(EditionFieldsData.fieldGroupName(pool,
                        strFieldGroup));
     ...
     * @throws IOException
     */
    private void processTabHtmlEdition(FieldProvider[] efd,
            FieldProvider[] efdauxiliar, File fileDir, String strTab,
            String tabName, String keyColumnName, String tabNamePresentation,
            String windowId, FieldsData[] parentsFieldsData,
            Vector<Object> vecFields, boolean isreadonly, String isSOTrx,
            String strTable, double pixelSize, String strLanguage,
            boolean editable, boolean isSecondaryKey) throws ServletException,
            IOException {
        if (log4j.isDebugEnabled())
            log4j.debug("Procesig edition html"
                    + (strLanguage.equals("") ? "" : " translated") + ": "
                    + strTab + ", " + tabName);
        ...
        final XmlDocument xmlDocument = xmlEngine.readXmlTemplate(
                "org/openbravo/wad/Template_Edition", discard)
                .createXmlDocument();
        ;
        xmlDocument.setParameter("tab", tabNamePresentation);
        ...
        final EditionFieldsData efdDl[] = EditionFieldsData.selectDisplayLogic(
                pool, strTab);
        if (efdDl != null) {
            for (int i = 0; i < efdDl.length; i++) {
                WadUtility.displayLogic(efdDl[i].displaylogic, vecDisplayLogic,
                        parentsFieldsData, new Vector<Object>(), vecFields,
                        windowId, new Vector<Object>());
            }
        }
        ...
        if (!isreadonly) {
            final EditionFieldsData efdReadOnlyLogic[] = EditionFieldsData
                    .selectReadOnlyLogic(pool, strTab);
            if (efdReadOnlyLogic != null) {
                for (int i = 0; i < efdReadOnlyLogic.length; i++) {
                    WadUtility.displayLogic(efdReadOnlyLogic[i].readonlylogic,
                            vecReadOnlyLogic, parentsFieldsData,
                            new Vector<Object>(), vecFields, windowId,
                            new Vector<Object>());
                }
            }
        }
        ...
        if (efdauxiliar != null) {
            for (int i = 0; i < efdauxiliar.length; i++) {
                ...
                vecAuxiliar.addElement(FormatUtilities.replace(efdauxiliar[i]
                        .getField("columnname")));
            }
        }

        final FieldsData[] dataReload = FieldsData.selectValidationTab(pool,
                strTab);
        ...
        if (dataReload != null && dataReload.length > 0) {
            for (int z = 0; z < dataReload.length; z++) {
                ...
                if (code.equals("") && dataReload[z].type.equals("R"))
                    code = "@AD_Org_ID@";
                WadUtility.getComboReloadText(code, vecFields,
                        parentsFieldsData, vecReloads, "",
        ...
        for (int i = 0; i < efd.length; i++) {
            ...
            try {
                auxControl = WadUtility.getControl(pool, efd[i], isreadonly,
                        tabName, strLanguage, xmlEngine, (WadUtility
                                .isInVector(vecDisplayLogic, efd[i]
                                        .getField("columnname"))), WadUtility
                                .isInVector(vecReloads, efd[i]
                                        .getField("columnname")), WadUtility
                                .isInVector(vecReadOnlyLogic, efd[i]
                                        .getField("columnname")), false,
            ...
            if (WadUtility.isNewGroup(auxControl, strFieldGroup)) {
                if (fieldGroupHasDisplayLogic && (fieldGroupElements != 0)) {
                    ...
                    displayLogicFunction.append(WadUtility
                            .getDisplayLogicForGroups(strFieldGroup,
                ...
                final WadControlLabelBuilder fieldGroupLabelBuilder = new WadControlLabelBuilder(
                        fieldLabel);
                fieldGroupLabelBuilder.buildLabelControl();
                final String labelText = fieldGroupLabelBuilder
                        .getBasicLabelText();

                html.append("<td class=\"FieldGroupTitle\">").append(labelText)
                        .append("</td>");
                html
            ...
            if (auxControl.getData("IsDisplayed").equals("Y")
                    && fieldGroupHasDisplayLogic && !strFieldGroup.equals("")) {
                String displ = WadUtility.displayLogic(auxControl
                        .getData("DisplayLogic"), new Vector<Object>(),
                        parentsFieldsData, vecAuxiliar, vecFields, windowId,
                        new Vector<Object>());
                if (displ.length() != 0) {
                    displ = "(" + displ + ")";
                    if (!strGroupDisplayLogic.toString().contains(displ)) {
                        strGroupDisplayLogic
                                .append((fieldGroupElements == 0 ? "" : " || ")
                                        + displ);
                        fieldGroupElements++;
                    }
                } else {
            ...
            if (log4j.isDebugEnabled())
                log4j.debug("Column: " + auxControl.getData("ColumnName")
                        + ", col:  " + columnType);
            // hidden inputs
            if (auxControl.getData("IsDisplayed").equals("N")) {
                ...
                if (auxControl.getData("IsSameLine").equals("Y")) {
                    ...
                    html.append("<td");
                } else if (i < efd.length - 1
                        && efd[i + 1].getField("issameline").equals("Y")
                ...
                auxControl.setData("ColumnNameLabel", auxControl
                        .getData("ColumnName"));
                auxControl.setData("AdColumnId", efd[i].getField("adColumnId"));
                ...
                if (!label.equals("")) {
                    html.append(" class=\"TitleCell\" id=\"").append(
                            auxControl.getData("ColumnName")).append(
                            "_lbl_td\">").append(label.replace("\n", ""));
                    if (columnType == COLUMN_1_OF_1)
                        html.append("</td>\n<td colspan=\"3\" class=\"")
                                .append(auxControl.getType()).append(
                                        "_ContentCell\" id=\"").append(
                                        auxControl.getData("ColumnName"))
                                .append("_inp_td\">");
                    else
                        html.append("</td>\n<td class=\"").append(
                                auxControl.getType()).append(
                                "_ContentCell\" id=\"").append(
                                auxControl.getData("ColumnName")).append(
                                "_inp_td\">");
                } else {
                    html.append(" class=\"").append(auxControl.getType())
                            .append("_ContentCell\" id=\"").append(
                                    auxControl.getData("ColumnName")).append(
                                    "_inp_td\"");
                    if (columnType == COLUMN_1_OF_1)
                ...
                final boolean isCombo = (auxControl.getData("AD_Reference_ID")
                        .equals("17")
                        || auxControl.getData("AD_Reference_ID").equals("18") || auxControl
                        .getData("AD_Reference_ID").equals("19"));
                if (columnType == COLUMN_1_OF_1) {
                    if (Integer.valueOf(auxControl.getData("DisplayLength"))
                            .intValue() < (MAX_SIZE_EDITION_1_COLUMNS / 4)) {
                        auxControl.setData("DisplayLength", Double
                                .toString(MAX_SIZE_EDITION_1_COLUMNS
                                        * (isCombo ? 7.5 : 1)));
                        auxControl.setData("CssSize", "OneCell");
                    } else if (Integer.valueOf(
                            auxControl.getData("DisplayLength")).intValue() > (MAX_SIZE_EDITION_1_COLUMNS / 2)) {
                        auxControl.setData("DisplayLength", Double
                                .toString(MAX_SIZE_EDITION_1_COLUMNS
                                        * (isCombo ? 7.5 : 1)));
                        ...
                        auxControl.setData("DisplayLength", Double
                                .toString((MAX_SIZE_EDITION_1_COLUMNS / 2)
                                        * (isCombo ? 7.5 : 1)));
                        auxControl.setData("CssSize", "TwoCells");
                    }
                } else {
                    if (Integer.valueOf(auxControl.getData("DisplayLength"))
                            .intValue() > (MAX_SIZE_EDITION_2_COLUMNS / 2)) {
                        auxControl.setData("DisplayLength", Double
                                .toString(MAX_SIZE_EDITION_2_COLUMNS
                                        * (isCombo ? 7.5 : 1)));
                        auxControl.setData("CssSize", "TwoCells");
                    } else {
                        auxControl.setData("DisplayLength", Double
                                .toString((MAX_SIZE_EDITION_2_COLUMNS / 2)
                                        * (isCombo ? 7.5 : 1)));
                        auxControl.setData("CssSize", "OneCell");
                    }
                }
                ...
                displayLogicFunction.append(WadUtility.getDisplayLogic(
                        auxControl, new Vector<Object>(), parentsFieldsData,
                        vecAuxiliar, vecFields, windowId, new Vector<Object>(),
                        isreadonly));
                if (!isreadonly)
                    readOnlyLogicFunction.append(WadUtility.getReadOnlyLogic(
                            auxControl, new Vector<Object>(),
                            parentsFieldsData, vecAuxiliar, vecFields,
                            windowId, new Vector<Object>(), isreadonly));
            }
        }
        ...
        if (fieldGroupHasDisplayLogic && (fieldGroupElements != 0)) {
            strGroupDisplayLogic.append(")");
            displayLogicFunction.append(WadUtility.getDisplayLogicForGroups(
                    strFieldGroup, strGroupDisplayLogic));
        }
        ...
        for (final Enumeration<?> e = importsCSS.propertyNames(); e
                .hasMoreElements();) {
        ...
        for (final Enumeration<?> e = importsJS.propertyNames(); e
                .hasMoreElements();) {
            final String _name = (String) e.nextElement();
            ...
            if (!_name.equals("calendarLang") || calendarInserted) {
                sbImportJS.append("<script language=\"JavaScript\" src=\"")
                        .append(importsJS.getProperty(_name)).append(
        ...
        if (hasCalendar && !calendarLangInserted)
            sbImportJS.append("<script language=\"JavaScript\" src=\"").append(
                    importsJS.getProperty("calendarLang")).append(
                    "\" type=\"text/javascript\"></script>\n");
        xmlDocument.setParameter("importJS", sbImportJS.toString());
        final StringBuffer script = new StringBuffer();
        for (final Enumeration<?> e = javaScriptFunctions.propertyNames(); e
                .hasMoreElements();) {
            final String _name = (String) e.nextElement();
        ...
        String firstFocus;
        final EditionFieldsData[] ff = EditionFieldsData.selectFirstFocused(
                pool, strTab);
        ...
        script.append("\nfunction reloadComboReloads").append(strTab).append(
                "(changedField) {\n");
        script
        ...
        script.append("  var frm=document.frmMain;\n");
        script
                .append("  var key = eval(\"document.frmMain.\" + frm.inpKeyName.value);");
        ...
        xmlDocument.setParameter("script", script.toString());
        if (editable)
            WadUtility.writeFile(fileDir, tabName + "_Edition.html",
                    xmlDocument.print());
        else
            WadUtility.writeFile(fileDir, tabName + "_NonEditable.html",
                    xmlDocument.print());
    }
     ...
     */
    /**
     * Returns the subtabs for a given parent tab id. Also marks as selected one
     * of them.
     * 
     * @param vec
     ...
     * @throws ServletException
     */
    private void getSubTabs(Vector<Object> vec, String strTabParent,
            String strTabSelected) throws IOException, ServletException {
        TabsData[] aux = null;
     ...
     * @throws ServletException
     */
    private TabsData[] getPrimaryTabs(String strWindowId,
            String strTabSelected, int level, int heightTabs, int incrTabs)
     ...
     * @throws ServletException
     */
    private void debugTab(TabsData tab, String strTab, int level,
            int heightTabs, int incrTabs, int mayor) throws ServletException {
        ...
        if (strTab.equals(tab.tabid)) {
            ...
            if ((level + 1) >= Integer.valueOf(tab.tablevel).intValue())
                tab.href = "submitCommandForm('"
                        + ((level > Integer.valueOf(tab.tablevel).intValue()) ? "DEFAULT"
                                : "TAB")
                        + "', "
                        + ((level >= Integer.valueOf(tab.tablevel).intValue()) ? "false"
                                : "true") + ", null, '" + tabName
                        + "_Relation.html', 'appFrame');return false;";
            else
                tab.href = "return false;";
        ...
        final int height = ((mayor - Integer.valueOf(tab.tablevel).intValue())
                * incrTabs + heightTabs);
     ...
     * @throws IOException
     */
    private int parentTabId(TabsData[] allTabs, String tabId)
            throws ServletException, IOException {
        if (allTabs == null || allTabs.length == 0)
            return -1;
     ...
     * @throws ServletException
     */
    public void fieldsOfTableDir(String tableInit, String name,
            String required, Vector<Object> vecFields,
            Vector<Object> vecTables, Vector<Object> vecWhere)
            throws ServletException {
        ...
        vecTables.addElement("left join " + tableName + " on (" + tableInit
                + "." + name + " = " + tableName + "." + name + ")");
        for (int i = 0; i < fdi.length; i++) {
            if (fdi[i].reference.equals("19"))
                fieldsOfTableDir(tableName, fdi[i].name, fdi[i].required,
                        vecFields, vecTables, vecWhere);
            else
     ...
     * @throws ServletException
     */
    public int fieldsOfSearch(String tableInit, String name, String FilterName,
            String required, Vector<Object> vecFields,
            Vector<Object> vecTables, Vector<Object> vecWhere, int itable,
        ...
        vecTables.addElement("left join (select " + FilterName
                + fieldsAux.toString() + " from " + tableName + ") table"
                + tableNum + " on (" + tableInit + "." + name + " = table"
                + tableNum + "." + FilterName + ")");
        for (int i = 0; i < fdi.length; i++) {
            if (fdi[i].reference.equals("30") || fdi[i].reference.equals("31")
                    || fdi[i].reference.equals("35")
                    || fdi[i].reference.equals("25")
                    || fdi[i].reference.equals("800011")
                    || fdi[i].reference.equals("19")) {
                itable = fieldsOfSearch("table" + tableNum, fdi[i].name,
                        fdi[i].columnname, required, vecFields, vecTables,
                        vecWhere, itable, fdi[i].reference,
                        fdi[i].referencevalue);
            } else {
                vecFields.addElement("table" + tableNum + "."
                        + fdi[i].columnname);
            }
        }
        return itable;
    ...
    /**
     * Auxiliar method to get the query for a search control. This'll be used to
     * build combo's controls.
     * 
     * @param tableInit
     *            The name of the parent table.
     * @param name
    ...
    public int fieldsOfSearch2(String tableInit, String name, String required,
            Vector<Object> vecFields, Vector<Object> vecTables,
            Vector<Object> vecWhere, int itable, String reference,
            String referencevalue) throws ServletException {
        itable++;
        final int tableNum = itable;
        String tableName = "";
        EditionFieldsData[] dataSearchs = null;
        ...
        final FieldsData fdi[] = FieldsData.identifierColumns(pool, tableName);
        if (itable > 1) {
            vecTables.addElement(" left join " + tableName + " table"
                    + tableNum + " on (" + tableInit + "." + name + " = table"
                    + tableNum + "." + name + ")");
        }
        for (int i = 0; i < fdi.length; i++) {
            if (fdi[i].reference.equals("30")
                    || fdi[i].reference.equals("800011")
                    || fdi[i].reference.equals("31")
                    || fdi[i].reference.equals("35")
                    || fdi[i].reference.equals("25")) {
                itable = fieldsOfSearch2("table" + tableNum, fdi[i].columnname,
                        fdi[i].required, vecFields, vecTables, vecWhere,
                        itable, fdi[i].reference, fdi[i].referencevalue);
            } else {
                vecFields.addElement("table" + tableNum + "."
                        + fdi[i].columnname);
            }
    ...
    /**
     * Method to prepare the XmlEngine object, which is the one in charged of
     * the templates.
     * 
    ...
    public void createXmlEngine(String fileConnection) {
        ...
        log4j.debug("xmlEngine format file: "
                + xmlEngine.fileXmlEngineFormat.getAbsoluteFile());
