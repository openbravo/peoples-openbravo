...
public class WadActionButton {
    ...
    public static boolean isNumericType(String reference) {
        if (reference == null || reference.equals(""))
            return false;
        if (WadUtility.isGeneralNumber(reference)
                || WadUtility.isDecimalNumber(reference)
                || WadUtility.isPriceNumber(reference)
                || WadUtility.isIntegerNumber(reference)
     ...
     * @return Array of ActionButtonRelationData with the info to build the
     *         source.
     */
    public static ActionButtonRelationData[] buildActionButtonCall(
            ConnectionProvider conn, String strTab, String tabName,
            String keyName, String isSOTrx, String window) {
        ...
        if (fab != null) {
            for (int i = 0; i < fab.length; i++) {
                ...
                if (fab[i].realname.equalsIgnoreCase("DocAction")
                        ...
                        || (fab[i].realname.equalsIgnoreCase("Posted") && fab[i].adProcessId
                                .equals(""))
                        || (fab[i].realname.equalsIgnoreCase("CreateFrom") && fab[i].adProcessId
                                .equals(""))
                        || fab[i].realname
                                .equalsIgnoreCase("ChangeProjectStatus"))
                    fab[i].xmlid = "";
                fab[i].realname = FormatUtilities.replace(fab[i].realname);
                fab[i].columnname = Sqlc
                        .TransformaNombreColumna(fab[i].columnname);
                fab[i].htmltext = getFieldsLoad(fab[i], vecFields,
                        vecTotalFields);
                fab[i].javacode = getPrintPageJavaCode(conn, fab[i], vecFields,
                        vecParams, isSOTrx, window, tabName);
                ...
                if (!fab[i].adProcessId.equals("")
                        && !fab[i].adProcessId.equals("177")) {
                    try {
                        data = ProcessRelationData.selectParameters(conn, "",
                                fab[i].adProcessId);
                    } catch (final ServletException e) {
                    }
                    if (fab[i].realname.equalsIgnoreCase("ChangeProjectStatus")) {
                        ...
                        fab[i].processParams = "";
                    fab[i].processParams += getProcessParamsJava(data, fab[i],
                            vecParams, false);
                    fab[i].processCode = "ActionButtonData.process"
                            + fab[i].adProcessId + "(this, pinstance);\n";
                }
                fab[i].additionalCode = getAdditionalCode(fab[i], tabName,
                        keyName);
            }
        }
        return fab;
     ...
     *         source.
     */
    public static ActionButtonRelationData[] buildActionButtonCallGenerics(
    ...
    public static ActionButtonRelationData[] buildActionButtonCallGenericsJava(
        ...
        if (fab != null) {
            for (int i = 0; i < fab.length; i++) {
                ...
                fab[i].processCode = "new " + fab[i].classname
                        + "().execute(pb);";
            }
        }
        return fab;
    ...
    /**
     * Adds some fields to the vector of tab's fields, depending on the column
     ...
     */
    public static String getFieldsLoad(ActionButtonRelationData fd,
            Vector<Object> vecFields, Vector<Object> vecTotalFields) {
        if (fd == null)
            return "";
        final StringBuffer html = new StringBuffer();
        if (fd.columnname.equalsIgnoreCase("DocAction")) {
            html
                    .append("String strdocstatus = vars.getRequiredStringParameter(\"inpdocstatus\");\n");
        ...
        } else if (fd.columnname.equalsIgnoreCase("CreateFrom")
                && fd.adProcessId.equals("")) {
            html.append("String stradTableId = \"" + fd.adTableId + "\";\n");
            vecFields.addElement("stradTableId");
            vecTotalFields.addElement("AD_Table_ID");
        } else if (fd.columnname.equalsIgnoreCase("Posted")
                && fd.adProcessId.equals("")) {
            html.append("String stradTableId = \"" + fd.adTableId + "\";\n");
            vecFields.addElement("stradTableId");
            vecTotalFields.addElement("AD_Table_ID");
    ...
    /**
     * Auxiliar method that generates the printPage java code of the action
     * button.
     * 
     * @param conn
     ...
     */
    public static String getPrintPageJavaCode(ConnectionProvider conn,
            ActionButtonRelationData fd, Vector<Object> vecFields,
            Vector<Object> vecParams, String isSOTrx, String window,
            String tabName) {
        if (fd == null)
            return "";
        final StringBuffer html = new StringBuffer();
        for (int i = 0; i < vecFields.size(); i++) {
            ...
            html.append("xmlDocument.setParameter(\"" + field + "\", str"
                    + field + ");\n");
        }

        if (!fd.adProcessId.equals("")) {
            try {
                final ProcessRelationData[] data = ProcessRelationData
                        .selectParameters(conn, "", fd.adProcessId);
                boolean hasComboParameter = false;
                html.append("    try {\n");
                for (int i = 0; i < data.length; i++) {
                    if (data[i].adReferenceId.equals("17")
                            || data[i].adReferenceId.equals("18")
                ...
                for (int i = 0; i < data.length; i++) {
                    ...
                    html.append("\", ");
                    if (data[i].defaultvalue.equals("")
                            || data[i].defaultvalue.indexOf("@") == -1) {
                        strDefault = "\"" + data[i].defaultvalue + "\"";
                    } else if (data[i].defaultvalue.startsWith("@SQL=")) {
                        strDefault = tabName + "Data.selectActP" + data[i].id
                                + "_"
                                + FormatUtilities.replace(data[i].columnname);
                        strDefault += "(this"
                                + WadUtility
                                        .getWadContext(data[i].defaultvalue,
                                                vecFields, vecParams, null,
                                                false, isSOTrx, window);
                        strDefault += ")";
                    } else {
                        strDefault = WadUtility.getTextWadContext(
                                data[i].defaultvalue, vecFields, vecParams,
                    ...
                    if (data[i].adReferenceId.equals("17")
                            ...
                            || data[i].adReferenceId.equals("19")) {
                        html
                                .append("    comboTableData = new ComboTableData(vars, this, \"");
                        html.append(data[i].adReferenceId).append("\", \"");
                        html.append(data[i].columnname).append("\", \"");
                        html.append(data[i].adReferenceValueId)
                                .append("\", \"");
                        html.append(data[i].adValRuleId).append("\", ");

                        html
                                .append("Utility.getContext(this, vars, \"#AccessibleOrgTree\", \"\"), ");

                        html
                                .append("Utility.getContext(this, vars, \"#User_Client\", \"\"), 0");
                        html.append(");\n");
                        html
                                .append(
                        ...
                        html.append(data[i].columnname);
                        html
                                .append("\", \"liststructure\", comboTableData.select(false));\n");
                    ...
                    } else if (data[i].adReferenceId.equals("30")
                            || data[i].adReferenceId.equals("35")
                            || data[i].adReferenceId.equals("25")
                            || data[i].adReferenceId.equals("31")
                        ...
                        if (!tabName.equals("")) {
                            ...
                            html.append(FormatUtilities
                                    .replace(data[i].columnname));
                            html.append("(this, ");
                            html
                                    .append(((data[i].defaultvalue.equals("") || data[i].defaultvalue
                                            .indexOf("@") == -1) ? "\""
                                            + data[i].defaultvalue + "\""
        ...
        if (fd.columnname.equalsIgnoreCase("DocAction")) {
            html.append("xmlDocument.setParameter(\"processId\", \""
                    + fd.adProcessId + "\");\n");
            String strAux = "";
            try {
                strAux = ActionButtonRelationData.processDescription(conn,
                        fd.adProcessId);
            } catch (final ServletException e) {
            }
            html.append("xmlDocument.setParameter(\"processDescription\", \""
                    + strAux + "\");\n");
            html
            ...
            html
                    .append("FieldProvider[] dataDocAction = ActionButtonUtility.docAction(this, vars, strdocaction, \""
                            + fd.adReferenceValueId
                            + "\", strdocstatus, strProcessing, stradTableId);\n");
            html
                    .append("xmlDocument.setData(\"reportdocaction\", \"liststructure\", dataDocAction);\n");
            html.append("StringBuffer dact = new StringBuffer();\n");
            html.append("if (dataDocAction!=null) {\n");
            html
                    .append("  dact.append(\"var arrDocAction = new Array(\\n\");\n");
            ...
            html
                    .append("    if (i<dataDocAction.length-1) dact.append(\",\\n\");\n");
            html.append("  }\n");
            html.append("  dact.append(\");\");\n");
            html.append("} else dact.append(\"var arrDocAction = null\");\n");
            html
                    .append("xmlDocument.setParameter(\"array\", dact.toString());\n");
        } else if (fd.columnname.equalsIgnoreCase("ChangeProjectStatus")) {
            String strAux = "";
            html.append("xmlDocument.setParameter(\"processId\", \""
                    + fd.adProcessId + "\");\n");
            ...
            html.append("xmlDocument.setParameter(\"processDescription\", \""
                    + strAux + "\");\n");
            html
                    .append("xmlDocument.setParameter(\"projectaction\", strchangeprojectstatus);\n");
            html
            ...
            html
                    .append("  dact.append(\"var arrProjectAction = new Array(\\n\");\n");
            ...
            html
                    .append("    if (i<dataProjectAction.length-1) dact.append(\",\\n\");\n");
            html.append("  }\n");
            html.append("  dact.append(\");\");\n");
            html
                    .append("} else dact.append(\"var arrProjectAction = null\");\n");
            html
                    .append("xmlDocument.setParameter(\"array\", dact.toString());\n");
    ...
    /**
     * Returns the process call needed to fill the ad_pinstance's tables to
    ...
    public static String getProcessParamsJava(ProcessRelationData[] data,
            ActionButtonRelationData fd, Vector<Object> vecParams,
            boolean isGenericJava) {
        if (fd == null)
            return "";
        final StringBuffer html = new StringBuffer();
        ...
        if (data != null) {
            for (int i = 0; i < data.length; i++) {
                html.append("String str"
                        + Sqlc.TransformaNombreColumna(data[i].columnname)
                        + " = vars.getStringParameter(\"inp"
                        + Sqlc.TransformaNombreColumna(data[i].columnname)
                        + "\"");
                ...
                if (isGenericJava) {
                    html
                            ...
                            .append(
                                    Sqlc
                                            .TransformaNombreColumna(data[i].columnname))
                            .append("\", str")
                            .append(
                                    Sqlc
                                            .TransformaNombreColumna(data[i].columnname))
                    ...
                    html
                            .append("PInstanceProcessData.insertPInstanceParam"
                                    + (isNumericType(data[i].adReferenceId) ? "Number"
                                            : (isDateType(data[i].adReferenceId) ? "Date"
                                                    : ""))
                                    + "(this, pinstance, \""
                                    + data[i].seqno
                                    + "\", \""
     ...
     */
    public static String getAdditionalCode(ActionButtonRelationData fd,
            String tabName, String keyName) {
        if (fd == null)
            return "";
        final StringBuffer html = new StringBuffer();
        if (fd.columnname.equalsIgnoreCase("DocAction")) {
            html.append(tabName
                    + "Data.updateDocAction(this, strdocaction, str" + keyName
                    + ");\n");
            /*
             * } else if (fd.columnname.equalsIgnoreCase("ChangeProjectStatus"))
             * { html.append(tabName +
             * "Data.updateChangeProjectStatus(this, strchangeprojectstatus, str"
             * + keyName + ");\n");
             */
        } else if (fd.columnname.equalsIgnoreCase("PaymentRule")) {
        }
     ...
     */
    public static ActionButtonRelationData[] buildActionButtonSQL(
            ConnectionProvider conn, String strTab) {
        ActionButtonRelationData[] fab = null;
        try {
     ...
     * @throws IOException
     */
    public static void buildXml(ConnectionProvider conn, XmlEngine xmlEngine,
            File fileDir, FieldsData fd, Vector<Object> vecFields,
            int max_textbox_length) throws ServletException, IOException {
        final XmlDocument xmlDocument = xmlEngine.readXmlTemplate(
                "org/openbravo/wad/Configuration_ActionButton")
                .createXmlDocument();
        xmlDocument.setParameter("class", (fd.columnname + fd.reference)
                + ".html");
        final ProcessRelationData[] data = ProcessRelationData
                .selectParameters(conn, "", fd.reference);

        {
            final StringBuffer html = new StringBuffer();
            if (vecFields != null) {
                for (int i = 0; i < vecFields.size(); i++) {
                    html.append("<PARAMETER id=\"" + vecFields.elementAt(i)
                            + "\" name=\"" + vecFields.elementAt(i)
                            + "\" attribute=\"value\"/>\n");
                }
            }
        ...
        if (data != null) {
            for (int i = 0; i < data.length; i++) {
                ...
                try {
                    auxControl = WadUtility.getControl(conn, data[i], false,
                            (fd.columnname + fd.reference), "", xmlEngine,
    ...
    public static void buildHtml(ConnectionProvider conn, XmlEngine xmlEngine,
            File fileDir, FieldsData fd, Vector<Object> vecFields,
            int max_textbox_length, int max_size_edition_1_columns,
            String strLanguage, boolean isGeneric, String calendarDescription,
            String clockDescription, String calculatorDescription,
        ...
        final XmlDocument xmlDocument = xmlEngine.readXmlTemplate(
                "org/openbravo/wad/Template_ActionButton", discard)
                .createXmlDocument();
        final ProcessRelationData[] efd = ProcessRelationData.selectParameters(
                conn, strLanguage, fd.reference);
        ...
        if (efd != null) {
            ...
            for (int i = 0; i < efd.length; i++) {
                WADControl auxControl = null;
                try {
                    auxControl = WadUtility.getControl(conn, efd[i], false,
                            (fd.columnname + fd.reference), strLanguage,
                ...
                html.append("<tr><td class=\"TitleCell\">").append(
                        auxControl.toLabel().replace("\n", "")).append(
                        "</td>\n");
                html.append("<td class=\"").append(auxControl.getType())
                        .append("_ContentCell\"");
            ...
            if (hasCalendar && !calendarLangInserted)
                sbImportJS.append("<script language=\"JavaScript\" src=\"")
                        .append(importsJS.getProperty("calendarLang")).append(
                                "\" type=\"text/javascript\"></script>\n");
        ...
        WadUtility.writeFile(fileDir, (fd.columnname + fd.reference) + ".html",
                xmlDocument.print());
    }
     ...
     */
    /**
     * Sets the correct format parameter for the xml, which depends on the
     * column type.
     * 
     * @param data
     *            Object with the column info.
     ...
     */
    public static String xmlFields(ProcessRelationData fd, String completeName,
        ...
        if (forcedAttribute) {
            ...
            html.append("id=\"" + Sqlc.TransformaNombreColumna(fd.columnname)
                    + completeName + "\" ");
            html.append("name=\"" + Sqlc.TransformaNombreColumna(fd.columnname)
                    + completeName + "\" ");
            ...
            html.append(">");
            html.append("</PARAMETER>");
        } else if (fd.adReferenceId.equals("17")
                || fd.adReferenceId.equals("18")
                || fd.adReferenceId.equals("19")) { // List
            html.append(xmlFields(fd, completeName, maxTextboxLength, true));
            if (fd.isdisplayed.equals("Y")) {
                html.append("\n<SUBREPORT id=\"report"
                        + Sqlc.TransformaNombreColumna(fd.columnname)
                        + completeName + "\" name=\"report"
                        + Sqlc.TransformaNombreColumna(fd.columnname)
                        + completeName + "\"");
            ...
            html.append("name=\"" + Sqlc.TransformaNombreColumna(fd.columnname)
                    + completeName + "\" ");
            html.append(" attribute=\"value\">");
            html.append(Sqlc.TransformaNombreColumna(fd.columnname)
            ...
            html.append("name=\"" + Sqlc.TransformaNombreColumna(fd.columnname)
                    + completeName + "\">");
            html.append("</PARAMETER>");
        } else if ((fd.adReferenceId.equals("34") || fd.adReferenceId
                .equals("14"))
    ...
    public static String classRequiredUpdateable(ProcessRelationData efd) {
        ...
        if (efd.required.equals("Y"))
            htmltext.append(" class=\"").append(strType).append(" required")
                    .append(strAux).append(" ").append(classType).append(
                            "_OneCell_width\" ");
        else if (!strAux.equals(""))
            htmltext.append(" class=\"").append(strType).append(strAux).append(
     ...
     */
    public static String htmlFields(ProcessRelationData efd,
            String completeName, String completeID, boolean isupdateable,
            int maxTextboxLength, boolean forcedAttribute, boolean isdesigne) {
        final StringBuffer html = new StringBuffer();
        ...
        if (forcedAttribute) {
            ...
            html.append(" name=\"inp"
                    ...
                    + completeName);
            html.append("\" id=\""
                    + Sqlc.TransformaNombreColumna(efd.columnname) + completeID
                    + "\" value=\"");
            if (!isdesigne)
                html.append("xxV");
            if (WadUtility.isGeneralNumber(efd.adReferenceId)
            ...
            html.append("\">");
        } else if ((efd.adReferenceId.equals("17")
                || efd.adReferenceId.equals("18") || efd.adReferenceId
                .equals("19"))
            ...
            html.append(Sqlc.TransformaNombreColumna(efd.columnname)
                    + completeID + "\"></div>");
            html.append("</select>");
        } else if ((efd.adReferenceId.equals("34")
                || efd.adReferenceId.equals("14") || (!efd.adReferenceId
                .equals("20") && (Integer.valueOf(efd.fieldlength).intValue() > maxTextboxLength)))
                && efd.isdisplayed.equals("Y")) { // TEXTAREA
            html.append("<textarea cols=\"" + efd.fieldlength
                    + "\" rows=\"3\" name=\"inp"
                    + Sqlc.TransformaNombreColumna(efd.columnname));
            html.append(completeName + "\" id=\""
                    + Sqlc.TransformaNombreColumna(efd.columnname) + completeID
                    + "\" ");
            html.append(classRequiredUpdateable(efd));
            html.append(">");
            ...
            if (efd.isdisplayed.equals("Y") && isupdateable
                    && WadUtility.isDateField(efd.adReferenceId)) { // Date. Put
                                                                    // a
                                                                    // calendar
                html
                        .append(" onkeyup=\"auto_completar_fecha(this);return true;\"");
            } else if (efd.isdisplayed.equals("Y") && isupdateable
                    && WadUtility.isTimeField(efd.adReferenceId)) { // Time. Put
                                                                    // a clock
                html
                        .append(" onkeyup=\"auto_completar_hora(this, true);return true;\"");
            }
            html.append(">");
        }
     ...
     */
    public static String searchsCommand(ProcessRelationData efd,
            boolean fromButton, Vector<Object> vecFields,
            ConnectionProvider conn) {
        ...
        String searchName = ((efd.reference != null && efd.reference
                .equals("31")) ? "/info/Locator" : "/info/" + strName)
                + "_FS.html";
        EditionFieldsData[] fieldsSearch = null;
        try {
            if (efd.reference != null && efd.reference.equals("30")
                    && efd.adReferenceValueId != null
                    && !efd.adReferenceValueId.equals(""))
                fieldsSearch = EditionFieldsData.selectSearchs(conn, "I",
                        efd.adReferenceValueId);
        ...
        if (fieldsSearch != null && fieldsSearch.length > 0) {
            ...
            if (!fieldsSearch[0].referencevalue.equals("")) {
                for (int i = 0; i < fieldsSearch.length; i++) {
                    final String field = findField(vecFields,
        ...
        html.append("openSearch(null, null, '..").append(searchName).append(
                "', null, false, 'frmMain', 'inp").append(
                Sqlc.TransformaNombreColumna(efd.columnname)).append("', 'inp")
                .append(Sqlc.TransformaNombreColumna(efd.columnname)).append(
                        "_R', document.frmMain.inp").append(
                        Sqlc.TransformaNombreColumna(efd.columnname)).append(
                        "_R.value").append(params.toString()).append(");");
        return html.toString();
    }
    ...
    public static String searchs(ProcessRelationData efd, int maxTextboxLength,
        ...
        if (efd.adReferenceId.equals("31")) {
            ...
            html.append(htmlFields(efd, "_PLIST", "_PLIST", true,
                    maxTextboxLength, true, true));
            html.append(htmlFields(efd, "_PSTD", "_PSTD", true,
                    maxTextboxLength, true, true));
            html.append(htmlFields(efd, "_UOM", "_UOM", true, maxTextboxLength,
                    true, true));
            html.append(htmlFields(efd, "_PLIM", "_PLIM", true,
                    maxTextboxLength, true, true));
            html.append(htmlFields(efd, "_CURR", "_CURR", true,
                    maxTextboxLength, true, true));
        }
        html.append("<a href=\"#\"");
        html.append("onClick=\"" + searchsCommand(efd, true, vecFields, conn)
                + "return false;\" ");
        html.append("onMouseOut=\"window.status='';return true;\"");
        html
                .append("onMouseOver=\"window.status='")
                .append(efd.referenceName)
                .append("';return true;\" class=\"windowbutton\"><img width=\"")
                .append(IMAGE_EDITION_WIDTH).append("\" height=\"").append(
                        IMAGE_EDITION_HEIGHT).append("\" alt=\"").append(
                        efd.name).append("\" title=\"").append(efd.name)
                .append("\"");
        html.append(" border=\"0\" src=\"../../../../../web/images/" + strName
                + ".jpg\" id=\"button" + strName + "\"></a>");
        return html.toString();
    }
     ...
     */
    public static String productSearch(ProcessRelationData efd,
            int maxTextboxLength, Vector<Object> vecFields,
            ConnectionProvider conn) {
        final StringBuffer html = new StringBuffer();
