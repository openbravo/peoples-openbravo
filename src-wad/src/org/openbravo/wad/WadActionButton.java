...
public class WadActionButton {
  ...
  public static ActionButtonRelationData[] buildActionButtonCall(ConnectionProvider conn, String strTab, String tabName, String keyName, String isSOTrx, String window) {
    ...
    if (fab!=null) {
      for (int i=0;i<fab.length;i++) {
        ...
        if (fab[i].realname.equalsIgnoreCase("DocAction") || fab[i].realname.equalsIgnoreCase("PaymentRule") || (fab[i].realname.equalsIgnoreCase("Posted") && fab[i].adProcessId.equals("")) || (fab[i].realname.equalsIgnoreCase("CreateFrom") && fab[i].adProcessId.equals("")) || fab[i].realname.equalsIgnoreCase("ChangeProjectStatus")) fab[i].xmlid = "";
        ...
        fab[i].javacode = getPrintPageJavaCode(conn, fab[i], vecFields, vecParams, isSOTrx, window, tabName);
        ...
        if (!fab[i].adProcessId.equals("") && !fab[i].adProcessId.equals("177")) {
          ...
          } else fab[i].processParams = "";
          fab[i].processParams += getProcessParamsJava(data, fab[i], vecParams, false);
          fab[i].processCode = "ActionButtonData.process" + fab[i].adProcessId + "(this, pinstance);\n";
        }
  ...
  public static String getFieldsLoad(ActionButtonRelationData fd, Vector<Object> vecFields, Vector<Object> vecTotalFields) {
    ...
    if (fd.columnname.equalsIgnoreCase("DocAction")) {
      ...
      html.append("String strprojectstatus = vars.getRequiredStringParameter(\"inpprojectstatus\");\n");
  ...
  public static String getPrintPageJavaCode(ConnectionProvider conn, ActionButtonRelationData fd, Vector<Object> vecFields, Vector<Object> vecParams, String isSOTrx, String window, String tabName) {
    ...
    if (!fd.adProcessId.equals("")) {
      try {
        final ProcessRelationData[] data = ProcessRelationData.selectParameters(conn, "", fd.adProcessId);
        boolean hasComboParameter = false;
        html.append("    try {\n");
        for (int i=0; i<data.length;i++) {
          if (data[i].adReferenceId.equals("17") || data[i].adReferenceId.equals("18") || data[i].adReferenceId.equals("19")) 
        ...
        for (int i=0;i<data.length;i++) {
          ...
          if (data[i].defaultvalue.equals("") || data[i].defaultvalue.indexOf("@")==-1) {
            ...
            strDefault = tabName + "Data.selectActP"+ data[i].id + "_" + FormatUtilities.replace(data[i].columnname);
            strDefault += "(this" + WadUtility.getWadContext(data[i].defaultvalue, vecFields, vecParams, null, false, isSOTrx, window);
            strDefault += ")";
          } else {
            strDefault = WadUtility.getTextWadContext(data[i].defaultvalue, vecFields, vecParams, null, false, isSOTrx, window);
          }
          ...
          if (data[i].adReferenceId.equals("17") || data[i].adReferenceId.equals("18") || data[i].adReferenceId.equals("19")) {
            ...
            html.append("    xmlDocument.setParameter(\"").append(data[i].columnname).append("_Format\", vars.getSessionValue(\"#AD_SqlDateFormat\"));\n");
          } else if (data[i].adReferenceId.equals("30") || data[i].adReferenceId.equals("35") || data[i].adReferenceId.equals("25") || data[i].adReferenceId.equals("31") || data[i].adReferenceId.equals("800011")) {
            html.append("    xmlDocument.setParameter(\"");
            //html.append(Sqlc.TransformaNombreColumna(data[i].columnname));
            html.append(data[i].columnname);
            html.append("R\", ");
            if (!tabName.equals("")) {
              ...
              html.append("(this, ");
              html.append(((data[i].defaultvalue.equals("") || data[i].defaultvalue.indexOf("@")==-1)? "\"" + data[i].defaultvalue + "\"" : WadUtility.getWadContext(data[i].defaultvalue, vecFields, vecParams, null, false, isSOTrx, window)));
              html.append(")");
            } else {
              html.append("\"\"");
            }
    ...
    if (fd.columnname.equalsIgnoreCase("DocAction")) {
      ...
      html.append("FieldProvider[] dataDocAction = ActionButtonUtility.docAction(this, vars, strdocaction, \"" + fd.adReferenceValueId + "\", strdocstatus, strProcessing, stradTableId);\n");
      ...
      html.append("FieldProvider[] dataProjectAction = ActionButtonUtility.projectAction(this, vars, strchangeprojectstatus, \"" + fd.adReferenceValueId + "\", strprojectstatus);\n");
   ...
   */
  public static String getProcessParamsJava(ProcessRelationData[] data, ActionButtonRelationData fd, Vector<Object> vecParams, boolean isGenericJava) {
    if (fd==null) return "";
    final StringBuffer html = new StringBuffer();
    
    if (data!=null) {
      for (int i=0;i<data.length;i++) {
        html.append("String str" + Sqlc.TransformaNombreColumna(data[i].columnname) + " = vars.getStringParameter(\"inp" + Sqlc.TransformaNombreColumna(data[i].columnname) + "\"");
        if (data[i].adReferenceId.equals("20")) html.append(", \"N\"");
        html.append(");\n");
        if (isGenericJava) {
            html.append("params.put(\"").append(Sqlc.TransformaNombreColumna(data[i].columnname)).append("\", str").append(Sqlc.TransformaNombreColumna(data[i].columnname)).append(");\n");
        } else {
            html.append("PInstanceProcessData.insertPInstanceParam" + (isNumericType(data[i].adReferenceId)?"Number":(isDateType(data[i].adReferenceId)?"Date":"")) + "(this, pinstance, \"" + data[i].seqno + "\", \"" + data[i].columnname + "\", str" + Sqlc.TransformaNombreColumna(data[i].columnname) + ", vars.getClient(), vars.getOrg(), vars.getUser());\n");            
        }
  ...
  public static String getAdditionalCode(ActionButtonRelationData fd, String tabName, String keyName) {
    ...
    /*} else if (fd.columnname.equalsIgnoreCase("ChangeProjectStatus")) {
      html.append(tabName + "Data.updateChangeProjectStatus(this, strchangeprojectstatus, str" + keyName + ");\n");*/
    } else if (fd.columnname.equalsIgnoreCase("PaymentRule")) {
    }
    return html.toString();
  }
   ...
   * @throws IOException
   */
  public static void buildXml(ConnectionProvider conn, XmlEngine xmlEngine, File fileDir, FieldsData fd, Vector<Object> vecFields, int max_textbox_length) throws ServletException, IOException {
    final XmlDocument xmlDocument = xmlEngine.readXmlTemplate("org/openbravo/wad/Configuration_ActionButton").createXmlDocument();
...

    {
      ...
      if (vecFields!=null) {
        for (int i=0;i<vecFields.size();i++) {
          html.append("<PARAMETER id=\"" + vecFields.elementAt(i) + "\" name=\"" + vecFields.elementAt(i) + "\" attribute=\"value\"/>\n");
        }
      }
    ...
    if (data!=null) {
      for (int i=0;i< data.length; i++) {
        ...
        try {
          auxControl = WadUtility.getControl(conn, data[i], false, (fd.columnname + fd.reference), "", xmlEngine, false, false, false, false);
  ...
  public static void buildHtml(ConnectionProvider conn, XmlEngine xmlEngine, File fileDir, FieldsData fd, Vector<Object> vecFields, int max_textbox_length, int max_size_edition_1_columns, String strLanguage, boolean isGeneric, String calendarDescription, String clockDescription, String calculatorDescription, String jsDateFormat) throws ServletException, IOException {
    ...
    final XmlDocument xmlDocument = xmlEngine.readXmlTemplate("org/openbravo/wad/Template_ActionButton", discard).createXmlDocument();
    final ProcessRelationData[] efd = ProcessRelationData.selectParameters(conn, strLanguage, fd.reference);
    ...
    if (efd!=null) {
      ...
      for (int i=0;i<efd.length;i++) vecFields.addElement(efd[i].columnname);
      ...
      for (int i=0;i< efd.length; i++) {
        WADControl auxControl = null;
        try {
          auxControl = WadUtility.getControl(conn, efd[i], false, (fd.columnname + fd.reference), strLanguage, xmlEngine, false, false, false, false);
        } catch (final Exception ex) {
          throw new ServletException(ex);
        }
        ...
        html.append("</td>\n");
        if (auxControl.getData("CssSize").equals("OneCell")) html.append("<td></td>\n");
        html.append("<td></td>\n");
        html.append("</tr>\n");
        //Getting JavaScript
        {
  ...
  public static void xmlFormatAttribute(ProcessRelationData data) {
    if (data==null) return;
    if (WadUtility.isIntegerNumber(data.adReferenceId)) data.xmlFormat = "INTEGER";
    else if (WadUtility.isDecimalNumber(data.adReferenceId)) data.xmlFormat = "EURO";
    else if (WadUtility.isQtyNumber(data.adReferenceId)) data.xmlFormat = "QTY";
    else if (WadUtility.isPriceNumber(data.adReferenceId)) data.xmlFormat = "PRICE";
  }
  ...
  public static String xmlFields(ProcessRelationData fd, String completeName, int maxTextboxLength, boolean forcedAttribute) {
    ...
    if (forcedAttribute) {
      ...
      if (WadUtility.isDecimalNumber(fd.adReferenceId)) html.append(" format=\"euroEdition\"");
      else if (WadUtility.isQtyNumber(fd.adReferenceId)) html.append(" format=\"qtyEdition\"");
      else if (WadUtility.isPriceNumber(fd.adReferenceId)) html.append(" format=\"priceEdition\"");
      else if (WadUtility.isIntegerNumber(fd.adReferenceId)) html.append(" format=\"integerEdition\"");
      else if (WadUtility.isGeneralNumber(fd.adReferenceId)) html.append(" format=\"generalQtyEdition\"");
      html.append(">");
      html.append("</PARAMETER>");
    } else if (fd.adReferenceId.equals("17") || fd.adReferenceId.equals("18") || fd.adReferenceId.equals("19")) {  // List
      html.append(xmlFields(fd, completeName, maxTextboxLength, true));
      if (fd.isdisplayed.equals("Y")) {
        html.append("\n<SUBREPORT id=\"report" + Sqlc.TransformaNombreColumna(fd.columnname) + completeName + "\" name=\"report" + Sqlc.TransformaNombreColumna(fd.columnname) + completeName + "\"");
        ...
        html.append("  <ARGUMENT name=\"parameterListSelected\" withId=\"" + Sqlc.TransformaNombreColumna(fd.columnname) + completeName + "\"/>\n");
      ...
      html.append(">");
      html.append("</PARAMETER>");
    } else if (fd.adReferenceId.equals("28") && fd.isdisplayed.equals("Y") && !fd.adReferenceValueId.equals("")) {  // Button
      html.append("<PARAMETER ");
      html.append("id=\"" + Sqlc.TransformaNombreColumna(fd.columnname) + completeName + "\" ");
      html.append("name=\"" + Sqlc.TransformaNombreColumna(fd.columnname) + completeName + "\" ");
      html.append(" attribute=\"value\">");
      ...
      html.append("name=\"" + Sqlc.TransformaNombreColumna(fd.columnname) + completeName + "\">");
      html.append("</PARAMETER>");
    } else if ((fd.adReferenceId.equals("34") || fd.adReferenceId.equals("14")) && fd.isdisplayed.equals("Y")) {
      html.append("<PARAMETER ");
      html.append("id=\"" + Sqlc.TransformaNombreColumna(fd.columnname) + completeName + "\" ");
      html.append("name=\"" + Sqlc.TransformaNombreColumna(fd.columnname) + completeName + "\" ");
      html.append(">");
  ...
  public static String classRequiredUpdateable(ProcessRelationData efd) {
    ...
    String strAux="";
    if (WadUtility.isGeneralNumber(efd.adReferenceId) || WadUtility.isDecimalNumber(efd.adReferenceId) || WadUtility.isQtyNumber(efd.adReferenceId) || WadUtility.isPriceNumber(efd.adReferenceId) || WadUtility.isIntegerNumber(efd.adReferenceId)) {
      strAux=" number";
    }
    ...
    if (efd.required.equals("Y")) htmltext.append(" class=\"").append(strType).append(" required").append(strAux).append(" ").append(classType).append("_OneCell_width\" ");
    else if (!strAux.equals("")) htmltext.append(" class=\"").append(strType).append(strAux).append(" ").append(classType).append("_OneCell_width\" ");
  ...
  public static String htmlFields(ProcessRelationData efd, String completeName, String completeID, boolean isupdateable, int maxTextboxLength, boolean forcedAttribute, boolean isdesigne) {
    ...
    if (forcedAttribute) {
      ...
      html.append("</select>");
    } else if ((efd.adReferenceId.equals("34") || efd.adReferenceId.equals("14") || (!efd.adReferenceId.equals("20") && (Integer.valueOf(efd.fieldlength).intValue() > maxTextboxLength))) && efd.isdisplayed.equals("Y")){ //TEXTAREA
      html.append("<textarea cols=\"" + efd.fieldlength + "\" rows=\"3\" name=\"inp" + Sqlc.TransformaNombreColumna(efd.columnname));
      html.append(completeName + "\" id=\"" + Sqlc.TransformaNombreColumna(efd.columnname) + completeID + "\" ");
      html.append(classRequiredUpdateable(efd));
      html.append(">");
      if (!isdesigne) html.append("xxV");
      ...
      html.append("\"");
      if (efd.isdisplayed.equals("Y") && isupdateable && WadUtility.isDateField(efd.adReferenceId)) {  // Date. Put a calendar
        html.append(" onkeyup=\"auto_completar_fecha(this);return true;\"");
      } else if (efd.isdisplayed.equals("Y") && isupdateable && WadUtility.isTimeField(efd.adReferenceId)) {  // Time. Put a clock
  ...
  public static String findField(Vector<Object> vecFields, String token) {
    if (vecFields==null) return "";
    for (int i=0;i<vecFields.size();i++) {
      final String field = vecFields.elementAt(i).toString();
      if (field.equalsIgnoreCase(token)) return field;
    }
    return "";
  }
  ...
  public static String searchsCommand(ProcessRelationData efd, boolean fromButton, Vector<Object> vecFields, ConnectionProvider conn) {
    ...
    String searchName = ((efd.reference!=null && efd.reference.equals("31"))?"/info/Locator":"/info/" + strName) + "_FS.html";
    EditionFieldsData[] fieldsSearch = null;
    try {
      if (efd.reference!=null && efd.reference.equals("30") && efd.adReferenceValueId!=null && !efd.adReferenceValueId.equals("")) fieldsSearch = EditionFieldsData.selectSearchs(conn, "I", efd.adReferenceValueId);
    ...
    html.append("openSearch(null, null, '..").append(searchName).append("', null, false, 'frmMain', 'inp").append(Sqlc.TransformaNombreColumna(efd.columnname)).append("', 'inp").append(Sqlc.TransformaNombreColumna(efd.columnname)).append("_R', document.frmMain.inp").append(Sqlc.TransformaNombreColumna(efd.columnname)).append("_R.value").append(params.toString()).append(");");
    return html.toString();
  }
  ...
  public static String searchs(ProcessRelationData efd, int maxTextboxLength, Vector<Object> vecFields, ConnectionProvider conn) {
    ...
    html.append("onMouseOver=\"window.status='").append(efd.referenceName).append("';return true;\" class=\"windowbutton\"><img width=\"").append(IMAGE_EDITION_WIDTH).append("\" height=\"").append(IMAGE_EDITION_HEIGHT).append("\" alt=\"").append(efd.name).append("\" title=\"").append(efd.name).append("\"");
    html.append(" border=\"0\" src=\"../../../../../web/images/" + strName + ".jpg\" id=\"button" + strName + "\"></a>");
    return html.toString();
  }
